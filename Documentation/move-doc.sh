...
cat $DOCUMENT | sed 's#link:(?!(ht|f)tp://)#link:'${RELDIR}#  > $DOCUMENT.updated
if [ 0 == $? ]; then
...
for UPDATE in $(find . -name \*.txt | grep -v $DOCUMENT); do
	...
	if [ 0 == $? ]; then
		...
		cat $UPDATE | sed -E 's#link:((\.\./)*)'${ESC_HTML_DOCUMENT}#link:\\1${DIRPATH}/${HTML_DOCUMENT}#  > $UPDATE.updated
		if [ 0 == $? ]; then
			mv $UPDATE.updated $UPDATE && git add $UPDATE || echo Error: failed to update $UPDATE.
		else
...
echo Move $DOCUMENT to $NEWDOC
git mv $DOCUMENT $NEWDOC && mv $DOCUMENT.orig $NEWDOC.orig || echo Error: Failed to move $DOCUMENT to $NEWDOC

git commit -sm "[DO NOT SUBMIT] Documentation move: ${DOCUMENT} -> ${NEWDOC}"
