...
def postCheck(check) {
    def gerritPostUrl = url + "a/changes/${check.changeNum}/revisions/${check.sha1}/checks"
            ...
            catchError{
                retry(retryTimes){
                    ...
                    if (buildName == "Gerrit-codestyle"){
                        Builds.buildCodeStyle = slaveBuild
                        Builds.resultCodeStyle = slaveBuild.getResult()
                    } else {
                        Builds.buildsVerification[mode] = slaveBuild
                        Builds.resultsVerification[mode] = slaveBuild.getResult()
                    }
                }
            }
        }
...
def findFlakyBuilds() {
    def flaky = Builds.buildsVerification.findAll { it.value.getResult() == null ||
...
node ('master') {
    ...
    stage('Report to Gerrit'){
        ...
        Builds.buildsVerification.each { type, build -> postCheck(
