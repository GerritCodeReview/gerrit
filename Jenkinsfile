...
def getCheckResultFromBuild(GerritCheck check) {
        switch(check.build.result.toString()) {
            case 'SUCCESS':
                return "SUCCESSFUL"
            case 'NOT_BUILT':
            case 'ABORTED':
...
node ('master') {
    ...
    if (hasChangeNumber()) {
        ...
        stage('Report to Gerrit'){
            ...
            def check = new GerritCheck("codestyle", Builds.codeStyle)

            def checkResult = null
            def buildResult = "${check.build.result}"
            println "buildResult=${buildResult}"
            if (buildResult == 'SUCCESS') {
                checkResult = "SUCCESSFUL"
                println "checkResult=${checkResult}"
            } else if (buildResult == 'NOT_BUILT' || buildResult == 'ABORTED') {
                checkResult = "NOT_STARTED"
                println "checkResult=${checkResult}"
            } else {
                checkResult = "FAILED"
                println "checkResult=${checkResult}"
            ...
            println "uuid=${check.uuid} build.result=${check.build.result} checkResult=${checkResult} url=${check.consoleUrl}"
            gerritCheck(checks: [ "${check.uuid}" : "${checkResult}" ], url: "${check.consoleUrl}")
            postCheck(check)
