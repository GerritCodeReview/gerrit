...
class GerritCheck {
    ...
    def printCheckSummary() {
        println "----------------------------------------------------------------------------"
        println "Gerrit Check: ${uuid}=" + build.getResult().toString() + " to change " + changeNum + "/" + sha1
        println "----------------------------------------------------------------------------"
    }
...
def setResult(resultVerify, resultCodesyle) {
    def resAll = resultCodeStyle ?
        getLabelValue(resultVerify, resultCodeStyle) : resultVerify
    switch(resAll) {
        case 0: currentBuild.result = Result.ABORTED
                break
        case 1: currentBuild.result = Result.SUCCESS
                break
        case -1: currentBuild.result = Result.FAILURE
                break
    }
}
