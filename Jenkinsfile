...
def collectBuildModes(){
    if(GERRIT_BRANCH == "master" || GERRIT_BRANCH == "stable-3.0") {
        Builds.modes = ["notedb"]
    }
        ...
        catchError{
            retry(retryTimes){
                def slaveBuild = build job: "${buildName}", parameters: [
                    ...
                    string(name: 'BRANCH', value: GERRIT_PATCHSET_REVISION),
                    string(name: 'CHANGE_URL', value: GERRIT_CHANGE_URL),
                    string(name: 'MODE', value: mode),
                    string(name: 'TARGET_BRANCH', value: GERRIT_BRANCH)
                ], propagate: propagate
...
node ('master') {
    ...
    stage('Preparing'){
        ...
        build.setDescription(
            """<a href='${GERRIT_CHANGE_URL}' target='_blank'>""" +
    ...
    parallel(
        "CodeStyle": stage('Check Codestyle'){
            println "Checking codestyle of ${GERRIT_CHANGE_URL}"
            parallel "Gerrit-codestyle": prepareBuildsForMode("Gerrit-codestyle")
        },
        "Verification": stage('Verification'){
            println "Verifying Change ${GERRIT_CHANGE_URL}"
    ...
    stage('Post Check Result') {
        gerrit.postCheck(new GerritCheck(
            "codestyle", GERRIT_CHANGE_NUMBER, GERRIT_PATCHSET_REVISION,
            Builds.resultCodeStyle))
        Builds.resultsVerification.each { type, build -> gerrit.postCheck(
            new GerritCheck(type,GERRIT_CHANGE_NUMBER, GERRIT_PATCHSET_REVISION,
                build)
        )}
    }
}
