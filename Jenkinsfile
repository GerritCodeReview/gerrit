...
node('bazel-debian'){
  ...
  try {
    stage('Build (Buck'){
      ...
      ''')
      sh(script: 'mv $(find buck-out -name "*.war") buck-out/gen/gerrit.war')
    }
    stage('Build (Bazel)'){
      when {
        anyOf {
  ...
  stage('Publishing Artifacts (Buck)'){
    when {
        anyOf {
          branch = "stable-2.13"
          branch = "stable-2.12"
    ...
    archiveArtifacts(artifacts:
      "gerrit/buck-out/gen/*.war," +
      "gerrit/buck-out/gen/plugins/*/*.jar," +
      "gerrit/buck-out/gen/*api/*api.jar," +
      "gerrit/buck-out/gen/gerrit-acceptance-framework/*.jar",
      excludes: "gerrit/buck-out/gen/plugins/core*/**/*.jar")
  }
  stage('Publishing Artifacts (Bazel)'){
    when {
        anyOf {
          branch = "master"
          branch = "stable-2.16"
    ...
    archiveArtifacts(artifacts:
      "gerrit/bazel-bin/*.war," +
      "gerrit/bazel-genfiles/plugins/*/*.jar," +
