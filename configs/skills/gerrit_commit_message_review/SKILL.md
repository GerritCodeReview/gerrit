...
This change updates the review guidelines to require that commit
message reviews provide a complete, copy-pasteable revised message
when formatting issues are found. It also instructs the reviewer to
avoid pedantic feedback on minor casing or phrasing if the original
message is already clear and compliant.

Google-Bug-ID: b/505405738
Release-Notes: skip
Change-Id: I75ef56099ea36b8838a65746abb3a4771fcefd23
```
*(Rationale: Since the bug context is not explicitly visited or verified, the vacuous reference is completely removed from the body text. The metadata footer still correctly maintains the automated tracking link.)*

**Do (When bug context is visited/known - integrate descriptive context):**
```text
Suggest full commit message and avoid pedantic reviews

To address b/505405738, where developers reported being slowed down by
pedantic reviews and wanting ready-to-apply suggestions, this change
updates the guidelines to require a complete, copy-pasteable revised
message. It also instructs reviewers to tolerate minor casing or
phrasing variations if the original message is already compliant.

Google-Bug-ID: b/505405738
Release-Notes: skip
Change-Id: I75ef56099ea36b8838a65746abb3a4771fcefd23
```
*(Rationale: The bug reference in the body now adds actual value by explaining exactly what issue/feedback was reported in b/505405738, providing important context to future maintainers.)*
```
...
This change adds guidelines to SKILL.md requiring commit message
reviews to provide a complete, copy-pasteable revised message when
formatting issues are found. It also instructs the reviewer to avoid
pedantic feedback on minor casing or phrasing nits if the original
commit is already satisfactory.

Google-Bug-ID: b/505405738
```
*(Rationale: Highly concise and high signal-to-noise ratio. Jumps directly into the technical enhancement without vacuous issue reference prefix in the body, utilizing the metadata footer for bug linkage.)*
