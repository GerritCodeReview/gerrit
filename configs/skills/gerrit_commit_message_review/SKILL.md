...
Adhering to these standards ensures metadata traceability, and provides clear, long-term context to future developers.
...
**Don't:**
```text
Fixing the loading spinner bug in gr-reply-dialog.ts and adding tests (over 60 chars)
```

**Do:**
```text
Fix loading spinner and add test coverage (under 60 chars)
```
...
As defined in the project deletion security spec (go/gerrit-delete),
only system-level administrators should have the capability to
destroy project repositories in production environments.

Bug: gerrit:40012901
```
*(Rationale: Since this is a high-risk change (a security bypass), the body explicitly connects to the bug report gerrit:40012901 and links the authoritative security spec go/gerrit-delete. This establishes bulletproof reasoning and traceability for a critical modification.)*
