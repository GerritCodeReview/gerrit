...
#
# usage: download_gerrit_and_plugins.py [-h] [--version-file VERSION_FILE]
...
try:
    ...
    print('error creating directory %s: %s' %
        (path.dirname(args.gerrit_home), err), file=sys.stderr)
    exit(1)
...
if not result or result is None:
    print("Invalid Gerrit version " + artifacts_version['gerrit'], file=sys.stderr)
    exit(1)
...
for plugin in artifacts_version['plugins']:
    ...
    plugin_urls = [bazel_plugin_url, bazel_master_plugin_url]
