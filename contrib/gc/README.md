...
A specific repository can be set as usual using `--gitdir`:

```sh
git --gitdir="/var/gerrit/git/All-Users.git" gcplus
```
...
command will suppress all progress reports:
...
- `gc.prunepreserved`: Whether to prune preserved packs craeted by previous runs.
