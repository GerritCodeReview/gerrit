...
def commit_id(repo_dir, ref="HEAD"):
    try:
        ...
        raise GitCommandException(f"Unable to parse current commit ID.")


def add(repo_dir, files=["."]):
    try:
        cmd = ["git", "add"]
        cmd.extend(files)
        subprocess.run(cmd, cwd=repo_dir, capture_output=True, check=True)
    except subprocess.CalledProcessError as e:
        raise GitCommandException(f"Unable to add files to index.")
...
def commit(repo_dir, message):
    try:
        ...
        raise GitCommandException(f"Unable to commit.")
...
def push(repo_dir, remote, refspec):
    try:
        ...
        raise GitCommandException(f"Unable to push.")
...
def init(base_dir, repo_name, bare=False):
    try:
        ...
        raise GitCommandException(f"Unable to initialize git repo {repo_dir}.")
