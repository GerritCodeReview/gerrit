...
from src.git.config import GitConfigWriter
...
def repo(tmp_path_factory):
    ...
    repo_name = "test.git"
    git.init(dir, repo_name, bare=True)
...
def test_PackAllRefsAfterStep(repo, local_repo):
    ...
    while loose_ref_count < target_loose_ref_count:
        loose_ref_count += 1
        git.push(local_repo, "origin", f"HEAD:refs/heads/test{loose_ref_count}")
    ...
    git.push(local_repo, "origin", f"HEAD:refs/heads/test{target_loose_ref_count + 1}")
    task.run(repo)
