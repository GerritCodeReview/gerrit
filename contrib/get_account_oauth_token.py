...
This is an UNSUPPORTED reference/test utility, and it is entirely
...
after the token has expired: the /oauthtoken request passes through the
request-time OAuth refresh filter, which renews the token from the refresh token
before GetOAuthToken serves it.
