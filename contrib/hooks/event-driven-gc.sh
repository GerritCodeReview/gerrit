...
$(basename "$0") [--max-runners <MAX>] [--git-dir <GIT_DIR>] [--repos <REPOS_ROOT>] [--logs <REPACK_LOGS_ROOT>] [--semaphore <SEMAPHORE>]
...
MAX_RUNNERS=3
unset REPOS REPACK_LOGS SEMAPHORE
while [ $# -gt 0 ] ; do
    case "$1" in
...
[ -z "$REPACK_LOGS" ] && usage "--logs not specified!"
[ -z "$SEMAPHORE" ] && usage "--semaphore not specified!"
[ -z "$REPOS" ] && usage "--repos not specified!"
