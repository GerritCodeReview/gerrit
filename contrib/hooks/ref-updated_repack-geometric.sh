...
#
# Since this is an event based approach to repository maintenance, it is recommended
# that another time based GC approach, perhaps a more significant and costly one,
# repacking refs, creating bitmaps... be used in parallel with this script. This
# simple policy of "least effort" should keep most repos from degrading much even
# with very infrequent time based GCs.
#
# In order to be able to skip repacking for each update happening during repacking,
# this script returns immediately after starting repacking in the background. If this
# script were to instead block during repacking, it would simply delay repacking for
# those updates instead of having a consolidating effect. That being said, a smarter
# script might consider tracking that some updates happened after repacking started
# and ensure that it gets repacked once again (while still consolidating many
# updates), but that would likely no longer qualify as least effort.
#
...
gc_runner gc_lock git repack -d --no-write-bitmap-index --geometric=2 &
