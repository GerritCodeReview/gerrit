...
while getopts "p:i:f:t:b:s:h:d:j:?" o;
do
  case "$o" in
    p)
      START_PORT=$(($OPTARG))
...
COMMON_ARGS="-s $GERRIT_ADDON_SCRIPT --set host=$UPSTREAM_HOST --set polygerrit_ui_dir=$POLYGERRIT_UI_DIR --set test_components_path=$TEST_COMPONENTS_FILE --set fonts_path=$FONTS_FILE --set jar_plugins=$JAR_PLUGINS --mode regular --no-http2 --listen-host 127.0.0.1"
