...
def get_random_users(num_users):
  ...
  for _ in range(0, num_users):
    ...
    names.append({"firstname": first_name,
                  "lastname": last_name,
...
def generate_SSH_Keys(gerrit_users):
  for user in gerrit_users:
    ...
    subprocess.call("ssh-keygen -f " + key_file + " -N '' &> /dev/null",
                    shell=True)
...
def clean_up():
  tmp_path = raw_input("Temporary path will be removed: " + TMP_PATH +
...
def main():
  ...
  tmp_path = raw_input("Press enter or provide a different path: ")
  if len(tmp_path) > 0:
    TMP_PATH = tmp_path
  ...
  for idx, u in enumerate(gerrit_users):
    u["groups"].append(group_names[4 * idx / len(gerrit_users)])
    if idx % 5 == 0:
  ...
  projectNames = create_gerrit_projects(group_names)

  for idx, u in enumerate(gerrit_users):
    create_change(u, projectNames[4 * idx / len(gerrit_users)])
