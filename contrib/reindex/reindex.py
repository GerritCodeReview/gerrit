...
$ ./reindex.py -u gerrit-url
    ...
    parser.add_argument(
        ...
        "--since",
        dest="since",
        help="Timestamp since when to find all changes (e.g. 2020-05-21 13:40:26)",
    )
    parser.add_argument(
        "-f",
        "--file",
        ...
        dest="file",
        help="""File path to store list of changes if --since is given,
    ...
    parser.add_argument(
        ...
        default=100,
        dest="chunksize",
        help="chunk size used to reindex chunks of changes",
        type=int,
    )
    parser.add_argument(
        "--cert",
...
class Reindexer:
    ...
    def __init__(self):
        ...
        if self.options.cert:
            ...
            self.api = GerritRestAPI(url=self.options.url, auth=credentials, verify=certs)
        else:
    ...
    def _authenticate(self):
        if self.options.netrc:
            return HTTPBasicAuthFromNetrc(url=self.options.url)
    ...
    def _query(self):
        ...
        more_changes = True
        while more_changes:
            query = f"since:{self.options.since}&start={start}&skip-visibility"
    ...
    def _query_to_file(self):
        self.logger.debug(
            f"writing changes since {self.options.since} to file {self.options.file}:"
        )
    ...
    def _reindex(self):
        ...
        with open(self.options.file, "r") as input:
            changes = [line.strip() for line in input]
            with tqdm(total=len(changes), unit="changes", desc="Indexed") as pbar:
                for chunk in _chunked_iterable(changes, self.options.chunksize):
