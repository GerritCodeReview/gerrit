...
if (
    ("GERRIT_USERNAME" not in environ)
    or ("GERRIT_PASSWORD" not in environ)
    or ("GERRIT_URL" not in environ)
):
    print(
        """ERROR: Unable to run without the following environment variables:
...
print(
    "Running Python smoke-tests on {} as user {}".format(
        environ["GERRIT_URL"], environ["GERRIT_USERNAME"]
    )
)
...
rest.post(
    "/changes/", json={"project": project, "branch": "master", "subject": "test change"}
)
...
print("Python smoke-tests SUCCEEDED")
