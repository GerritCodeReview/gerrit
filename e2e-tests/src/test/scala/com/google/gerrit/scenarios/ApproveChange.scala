...
class ApproveChange extends GerritSimulation {
  ...
  private var numbers_copy: mutable.Queue[Int] = mutable.Queue[Int]()
  ...
  val test: ScenarioBuilder = scenario(uniqueName)
      ...
      .exec(session => {
        if (createChange.nonEmpty) {
          if (numbers_copy.isEmpty) {
            numbers_copy = createChange.get.numbers.clone()
          }
          session.set("number", numbers_copy.dequeue())
        } else {
          session
        }
      })
