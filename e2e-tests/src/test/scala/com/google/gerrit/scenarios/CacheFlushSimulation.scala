...
class CacheFlushSimulation extends GerritSimulation {
  ...
  private var cacheEntriesBeforeFlush: String = "0"

  def entriesBeforeFlush(entries: String): Unit = {
    cacheEntriesBeforeFlush = entries
  }

  def expectedEntriesAfterFlush(): String = {
    val expectedEntries = cacheEntriesBeforeFlush.toInt - 1
