...
find ./ -name '*.html' -type f -not -path "*/node_modules/*" -not -path "*/samples/*" -not -path "*/gr-diff/gr-diff-root.html" -not -path "*/embed/gr-diff.html"  -exec sh -c 'echo {}' \n \; >$html_files
...
rm polygerrit-ui/app/behaviors/gr-tooltip-behavior/gr-tooltip-behavior.html \
  ...
  polygerrit-ui/app/elements/shared/shared/gr-select/gr-select.html \
