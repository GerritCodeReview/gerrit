...
public abstract class AbstractNotificationTest extends AbstractDaemonTest {
  private static final String UNIQUE_SUFFIX_MARKER = "TEST";

  private String uniqueSuffix;

  @Before
  public void prepareUniqueSuffix() {
    uniqueSuffix = UNIQUE_SUFFIX_MARKER + String.format("%08x", resourcePrefix.hashCode());
  }

  private String withUniqueSuffix(String prefix) {
    return prefix + uniqueSuffix;
  }

  static String stripUniqueSuffix(String str) {
    int idx = str.lastIndexOf(UNIQUE_SUFFIX_MARKER);
    if (idx < 0) {
      return str;
    }
  ...
  protected class StagedUsers {
    ...
    public StagedUsers() throws Exception {
      synchronized (stagedUsers) {
        ...
        for (NotifyType watch : EnumSet.allOf(NotifyType.class)) {
