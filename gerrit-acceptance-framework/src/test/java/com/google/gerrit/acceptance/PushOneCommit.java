...
public class PushOneCommit {
  ...
  public interface Factory {
    PushOneCommit create(ReviewDb db, PersonIdent i,
    ...
    PushOneCommit create(ReviewDb db, PersonIdent i, TestRepository<?> testRepo,
        @Assisted String subject, @Assisted Map<String, String> files);
  ...
  @AssistedInject
  PushOneCommit(ChangeNotes.Factory notesFactory, ApprovalsUtil approvalsUtil,
      Provider<InternalChangeQuery> queryProvider, @Assisted ReviewDb db,
      @Assisted PersonIdent i, @Assisted TestRepository<?> testRepo)
      throws Exception {
    this(notesFactory, approvalsUtil, queryProvider, db, i, testRepo, SUBJECT,
  ...
  PushOneCommit(ChangeNotes.Factory notesFactory, ApprovalsUtil approvalsUtil,
    ...
    this(notesFactory, approvalsUtil, queryProvider, db, i, testRepo, SUBJECT,
        FILE_NAME, FILE_CONTENT, changeId);
  }
  ...
  PushOneCommit(ChangeNotes.Factory notesFactory, ApprovalsUtil approvalsUtil,
    ...
    this(notesFactory, approvalsUtil, queryProvider, db, i, testRepo, subject,
        ImmutableMap.of(fileName, content), changeId);
  }

  private PushOneCommit(ChangeNotes.Factory notesFactory,
      ApprovalsUtil approvalsUtil, Provider<InternalChangeQuery> queryProvider,
      ReviewDb db, PersonIdent i, TestRepository<?> testRepo, String subject,
      Map<String, String> files, String changeId) throws Exception {
    this.db = db;
    ...
    commitBuilder.message(subject).author(i)
  ...
  public Result execute(String ref) throws Exception {
    ...
    if (tag != null) {
      ...
      if (tag instanceof AnnotatedTag) {
        ...
        tagCommand.setAnnotated(true).setMessage(annotatedTag.message)
  ...
  public class Result {
    ...
    public ChangeData getChange() throws OrmException {
      return Iterables
          .getOnlyElement(queryProvider.get().byKeyPrefix(changeId));
    }
    ...
    public void assertChange(Change.Status expectedStatus, String expectedTopic,
    ...
    private void assertReviewers(Change c, TestAccount... expectedReviewers)
      ...
      Iterable<Account.Id> actualIds = approvalsUtil
          .getReviewers(db, notesFactory.createChecked(db, c)).all();
    ...
    public void assertErrorStatus() {
      ...
      assertThat(refUpdate.getStatus()).named(message(refUpdate))
