...
public abstract class PluginDaemonTest extends AbstractDaemonTest {
  ...
  protected void beforeTest(Config cfg, boolean memory, boolean enableHttpd)
      throws Exception {
    deployPlugin();
    super.beforeTest(cfg, memory, enableHttpd);
  }
  ...
  private void locatePaths() {
    ...
    Path buckOut =
        basePath.getRoot().resolve(basePath.subpath(0, buckOutIdx + 1));
  ...
  private void createTestSite() throws IOException {
    SitePaths sitePath = new SitePaths(testSite.toFile());
    pluginsPath = Files.createDirectories(sitePath.plugins_dir.toPath());
    Files.createDirectories(sitePath.tmp_dir.toPath());
  }
