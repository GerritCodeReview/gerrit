...
public class SystemGroupsTest extends AbstractDaemonTest {
  ...
  public void systemGroupsCreated_rest() throws IOException {
    ...
    Gson gson = new Gson();
    Map<String, Group> result = gson.fromJson(r,
        new TypeToken<Map<String, Group>>() {}.getType());
    Set<String> keys = result.keySet();
    assertTrue(keys.contains("Administrators"));
    assertTrue(keys.contains("Administrators"));
    assertTrue(keys.contains("Anonymous Users"));
    assertTrue(keys.contains("Non-Interactive Users"));
    assertTrue(keys.contains("Project Owners"));
    assertTrue(keys.contains("Registered Users"));
  }
}
