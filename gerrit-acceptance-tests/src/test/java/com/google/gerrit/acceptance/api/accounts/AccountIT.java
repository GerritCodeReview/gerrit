...
public class AccountIT extends AbstractDaemonTest {
  ...
  private void assertGpgKeyEmailNotificationSent(TestAccount account) {
    ...
    assertThat(m.headers().get("Subject").equals("[Gerrit Code Review] New GPG Keys Added"));
    assertThat(m.body()).contains("One or more new GPG keys have been added");
    assertThat(m.body()).contains("You can also manage your GPG keys by visiting");
    assertThat(m.body()).contains(canonicalWebUrl.get() + "#/settings/gpg-keys");
    assertThat(m.body()).contains("(while signed in as " + account.emailAddress + ")");
    sender.clear();
  }
}
