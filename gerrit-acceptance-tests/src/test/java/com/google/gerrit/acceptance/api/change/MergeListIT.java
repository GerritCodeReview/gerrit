...
public class MergeListIT extends AbstractDaemonTest {
  ...
  @Test
  public void whyDoesThisNotWork() throws Exception {
    TestRepository<InMemoryRepository> testRepo = cloneProject(project, admin);

    ObjectId initial =
        testRepo.getRepository().exactRef(HEAD).getLeaf().getObjectId();
    ...
    PushOneCommit.Result result = m.to("refs/for/master");
    result.assertOkStatus();
    RevCommit merge = result.getCommit();
    String changeId = result.getChangeId();

    DiffInfo diff = getMergeListDiff(changeId, 1);
    assertDiffForNewFile(diff, merge, MERGE_LIST,
        getMergeListContent(parent2));
  }
