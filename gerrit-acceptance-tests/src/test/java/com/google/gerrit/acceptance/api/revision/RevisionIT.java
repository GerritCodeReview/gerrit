...
public class RevisionIT extends AbstractDaemonTest {
  ...
  public void reviewTriplet() throws Exception {
    ...
    gApi.changes().id(project.get() + "~master~" + r.getChangeId())
        .revision(r.getCommit().name()).review(ReviewInput.approve());
  }
  ...
  public void reviewNumber() throws Exception {
    ...
    gApi.changes().id(r.getChangeId()).revision(2)
        .review(ReviewInput.approve());
  }
  ...
  public void postSubmitApprovalAfterVoteRemoved() throws Exception {
    ...
    setApiUser(admin);
    gApi.changes().id(changeId).reviewer(user.username)
  ...
  public void approvalCopiedDuringSubmitIsNotPostSubmit() throws Exception {
    ...
    PatchSetApproval psa = Iterators.getOnlyElement(cd.currentApprovals()
        .stream().filter(a -> !a.isLegacySubmit()).iterator());
  ...
  public void cherryPick() throws Exception {
    ...
    gApi.projects().name(project.get()).branch(in.destination)
    ...
    Collection<ChangeMessageInfo> messages = gApi.changes()
        .id(project.get() + "~master~" + r.getChangeId()).get().messages;
  ...
  public void cherryPickToSameBranch() throws Exception {
    ...
    in.message =
        "it generates a new patch set\n\nChange-Id: " + r.getChangeId();
    ChangeInfo cherryInfo =
        gApi.changes().id(project.get() + "~master~" + r.getChangeId())
            .revision(r.getCommit().name()).cherryPick(in).get();
  ...
  public void cherryPickToSameBranchWithRebase() throws Exception {
    ...
    PushOneCommit push = pushFactory.create(db, admin.getIdent(), testRepo,
        subject, "another_file.txt", "another content");
    ...
    String newParent =
        cherryInfo.revisions.get(cherryInfo.currentRevision).commit.parents
            .get(0).commit;
  ...
  public void cherryPickConflict() throws Exception {
    ...
    PushOneCommit push = pushFactory.create(db, admin.getIdent(), testRepo,
        PushOneCommit.SUBJECT, PushOneCommit.FILE_NAME, "another content");
  ...
  public void cherryPickToExistingChange() throws Exception {
    PushOneCommit.Result r1 = pushFactory
    ...
    PushOneCommit.Result r2 = pushFactory.create(db, admin.getIdent(), testRepo,
        SUBJECT, FILE_NAME, "b", r1.getChangeId()).to("refs/for/foo");
    ...
    try {
      ...
      assertThat(e.getMessage())
          .isEqualTo("Cannot create new patch set of change " + info(t2)._number
  ...
  public void cherryPickMergeRelativeToDefaultParent() throws Exception {
    ...
    ChangeInfo cherryPickedChangeInfo =
        gApi.changes().id(mergeChangeResult.getChangeId()).current()
            .cherryPick(cherryPickInput).get();

    Map<String, FileInfo> cherryPickedFilesByName =
        cherryPickedChangeInfo.revisions
            .get(cherryPickedChangeInfo.currentRevision).files;
  ...
  public void canRebase() throws Exception {
    ...
    PushOneCommit.Result r2 = push.to("refs/for/master");
    boolean canRebase = gApi.changes().id(r2.getChangeId())
        .revision(r2.getCommit().name()).canRebase();
    ...
    canRebase = gApi.changes().id(r3.getChangeId())
        .revision(r3.getCommit().name()).canRebase();
  ...
  public void setUnsetReviewedFlag() throws Exception {
    ...
    gApi.changes().id(r.getChangeId()).current()
        .setReviewed(PushOneCommit.FILE_NAME, true);

    assertThat(Iterables.getOnlyElement(
        gApi.changes().id(r.getChangeId()).current().reviewed()))
  ...
  public void mergeable() throws Exception {
    ...
    PushOneCommit push1 = pushFactory.create(db, admin.getIdent(), testRepo,
        PushOneCommit.SUBJECT, PushOneCommit.FILE_NAME, "push 1 content");
    ...
    PushOneCommit push2 = pushFactory.create(db, admin.getIdent(), testRepo,
        PushOneCommit.SUBJECT, PushOneCommit.FILE_NAME, "push 2 content");
  ...
  public void files() throws Exception {
    PushOneCommit.Result r = createChange();
    Map<String, FileInfo> files = gApi.changes().id(r.getChangeId())
        .revision(r.getCommit().name()).files();
    assertThat(files).hasSize(2);
    assertThat(Iterables.all(files.keySet(),
        f -> f.matches(FILE_NAME + '|' + COMMIT_MSG))).isTrue();
  }
  ...
  public void filesOnMergeCommitChange() throws Exception {
    ...
    assertThat(gApi.changes().id(r.getChangeId()).revision(r.getCommit().name())
        .files().keySet()).containsExactly(COMMIT_MSG, MERGE_LIST, "foo",
            "bar");

    // list files against parent 1
    assertThat(gApi.changes().id(r.getChangeId()).revision(r.getCommit().name())
        .files(1).keySet()).containsExactly(COMMIT_MSG, MERGE_LIST, "bar");

    // list files against parent 2
    assertThat(gApi.changes().id(r.getChangeId()).revision(r.getCommit().name())
        .files(2).keySet()).containsExactly(COMMIT_MSG, MERGE_LIST, "foo");
  }
  ...
  public void diffDeletedFile() throws Exception {
    ...
    PushOneCommit.Result r = pushFactory.create(db, admin.getIdent(), testRepo)
        .rm("refs/for/master");
    DiffInfo diff = gApi.changes().id(r.getChangeId())
        .revision(r.getCommit().name()).file(FILE_NAME).diff();
  ...
  public void diffOnMergeCommitChange() throws Exception {
    ...
    // automerge
    diff = gApi.changes().id(r.getChangeId()).revision(r.getCommit().name())
        .file("foo").diff();
  ...
  public void contentType() throws Exception {
    ...
    String endPoint = "/changes/" + r.getChangeId() + "/revisions/"
        + r.getCommit().name() + "/files/" + FILE_NAME + "/content";
  ...
  public void drafts() throws Exception {
    ...
    DraftApi draftApi = gApi.changes().id(r.getChangeId())
        .revision(r.getCommit().name()).createDraft(in);
    assertThat(draftApi.get().message).isEqualTo(in.message);
    assertThat(gApi.changes().id(r.getChangeId()).revision(r.getCommit().name())
        .draft(draftApi.get().id).get().message).isEqualTo(in.message);
    assertThat(gApi.changes().id(r.getChangeId()).revision(r.getCommit().name())
        .drafts()).hasSize(1);

    in.message = "good catch!";
    assertThat(gApi.changes().id(r.getChangeId()).revision(r.getCommit().name())
        .draft(draftApi.get().id).update(in).message).isEqualTo(in.message);

    assertThat(gApi.changes().id(r.getChangeId()).revision(r.getCommit().name())
        .draft(draftApi.get().id).get().author.email).isEqualTo(admin.email);
  ...
  public void comments() throws Exception {
    ...
    Map<String, List<CommentInfo>> out = gApi.changes().id(r.getChangeId())
        .revision(r.getCommit().name()).comments();
    ...
    List<CommentInfo> list = gApi.changes().id(r.getChangeId())
        .revision(r.getCommit().name()).commentsAsList();
    ...
    assertThat(gApi.changes().id(r.getChangeId()).revision(r.getCommit().name())
        .comment(comment.id).get().message).isEqualTo(in.message);
  }
  ...
  public void patchWithPath() throws Exception {
    ...
    ChangeApi changeApi = gApi.changes().id(r.getChangeId());
    BinaryResult bin =
        changeApi.revision(r.getCommit().name()).patch(FILE_NAME);
    ByteArrayOutputStream os = new ByteArrayOutputStream();
    bin.writeTo(os);
    String res = new String(os.toByteArray(), UTF_8);
  ...
  public void actions() throws Exception {
    ...
    assertThat(current(r).actions().keySet()).containsExactly("submit",
        "cherrypick", "description", "rebase");
  ...
  public void deleteVoteOnNonCurrentPatchSet() throws Exception {
    ...
    setApiUser(admin);
    gApi.changes().id(r.getChangeId()).revision(r.getCommit().getName())
        .reviewer(user.getId().toString()).deleteVote("Code-Review");
  }
  ...
  public void deleteVoteOnCurrentPatchSet() throws Exception {
    ...
    Map<String, Short> m = gApi.changes().id(r.getChangeId()).current()
        .reviewer(user.getId().toString()).votes();
  ...
  private void assertContent(PushOneCommit.Result pushResult, String path,
    ...
    BinaryResult bin = gApi.changes().id(pushResult.getChangeId())
        .revision(pushResult.getCommit().name()).file(path).content();
    ByteArrayOutputStream os = new ByteArrayOutputStream();
    bin.writeTo(os);
    String res = new String(os.toByteArray(), UTF_8);
  ...
  private void assertDiffForNewFile(PushOneCommit.Result pushResult,
      String path, String expectedContentSideB) throws Exception {
    DiffInfo diff = gApi.changes().id(pushResult.getChangeId())
        .revision(pushResult.getCommit().name()).file(path).diff();
  ...
  private PushOneCommit.Result createCherryPickableMerge(String parent1FileName,
    ...
    PushOneCommit.Result changeAResult =
        pushFactory.create(db, admin.getIdent(), testRepo, "change a",
            parent1FileName, "Content of a").to("refs/for/" + branchAName);

    testRepo.reset(initialCommit);
    PushOneCommit.Result changeBResult =
        pushFactory.create(db, admin.getIdent(), testRepo, "change b",
            parent2FileName, "Content of b").to("refs/for/" + branchBName);
  ...
  private ApprovalInfo getApproval(String changeId, String label)
    ...
    return li.all.stream().filter(a -> a._accountId == accountId).findFirst()
        .get();
  }
