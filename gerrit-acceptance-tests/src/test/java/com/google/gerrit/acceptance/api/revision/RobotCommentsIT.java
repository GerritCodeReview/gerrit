...
public class RobotCommentsIT extends AbstractDaemonTest {
  ...
  @Test
  public void queryChangesWithUnresolvedCommentsNum() throws Exception {
    assume().that(notesMigration.enabled()).isTrue();
    ...
    addRobotComment(r1.getChangeId(),
        createRobotCommentInputWithMandatoryFields());
    addRobotComment(r2.getChangeId(),
        createRobotCommentInputWithMandatoryFields());
    ...
    try {
      ...
      // if we allow user to resolve a robot comment, then this test should
      // be modified.
      assertThat(result.unresolvedCommentsNum).isEqualTo(0);
    } finally {
      enableDb(ctx);
    }
  }
