...
public class SubmoduleSubscriptionsWholeTopicMergeIT
  ...
  public void subscriptionUpdateOfManyChanges() throws Exception {
    ...
    createSubmoduleSubscription(superRepo, "master", "subscribed-to-project",
        "master");

    ObjectId subHEAD = subRepo.branch("HEAD").commit().insertChangeId()
        .message("some change").add("a.txt", "a contents ").create();
    ...
    RevCommit c1 = subRepo.branch("HEAD").commit().insertChangeId()
        .message("first change").add("asdf", "asdf\n").create();
    subRepo.git().push().setRemote("origin")
    ...
    RevCommit c2 = subRepo.branch("HEAD").commit().insertChangeId()
        .message("qwerty").add("qwerty", "qwerty").create();

    RevCommit c3 = subRepo.branch("HEAD").commit().insertChangeId()
        .message("qwerty followup").add("qwerty", "qwerty\nqwerty\n").create();
    subRepo.git().push().setRemote("origin")
        .setRefSpecs(new RefSpec("HEAD:refs/for/master/" + name("topic-foo")))
        .call();
    ...
    expectToHaveSubmoduleState(superRepo, "master", "subscribed-to-project",
        subRepoId);
    ...
    assertThat(preview)
        .containsKey(new Branch.NameKey(p1, "refs/heads/master"));
    assertThat(preview)
        .containsKey(new Branch.NameKey(p2, "refs/heads/master"));
  ...
  public void updateManySubmodules() throws Exception {
    ...
    ObjectId sub1Id =
        pushChangeTo(sub1, "refs/for/master", "some message", "same-topic");
    ObjectId sub2Id =
        pushChangeTo(sub2, "refs/for/master", "some message", "same-topic");
    ObjectId sub3Id =
        pushChangeTo(sub3, "refs/for/master", "some message", "same-topic");
    ...
    assertWithMessage(
        "submodule subscription update " + "should have made one commit")
  ...
  public void doNotUseFastForward() throws Exception {
    ...
    ObjectId superId = pushChangeTo(superRepo, "refs/for/master",
  ...
  public void sameProjectDifferentBranchDifferentPaths() throws Exception {
    ...
    allowMatchingSubmoduleSubscription("sub", "refs/heads/master",
        "super-project", "refs/heads/master");
    allowMatchingSubmoduleSubscription("sub", "refs/heads/dev", "super-project",
        "refs/heads/master");
    ...
    ObjectId subMasterId = pushChangeTo(sub, "refs/for/master", "some message",
        "b.txt", "content b", "same-topic");

    sub.reset(devHead);
    ObjectId subDevId = pushChangeTo(sub, "refs/for/dev", "some message in dev",
        "b.txt", "content b", "same-topic");
    ...
    expectToHaveSubmoduleState(superRepo, "master", "sub-master", sub,
        "master");
  ...
  public void nonSubmoduleInSameTopic() throws Exception {
    ...
    ObjectId subId =
        pushChangeTo(sub, "refs/for/master", "some message", "same-topic");
    ObjectId standAloneId = pushChangeTo(standAlone, "refs/for/master",
        "some message", "same-topic");
  ...
  public void recursiveSubmodules() throws Exception {
    ...
    ObjectId bottomHead = pushChangeTo(bottomRepo, "refs/for/master",
        "some message", "same-topic");
    ObjectId topHead =
    ...
    expectToHaveSubmoduleState(midRepo, "master", "bottom-project", bottomRepo,
        "master");
    expectToHaveSubmoduleState(topRepo, "master", "mid-project", midRepo,
        "master");
  }
  ...
  public void triangleSubmodules() throws Exception {
    ...
    expectToHaveSubmoduleState(topRepo, "master", "mid-project", midRepo,
        "master");
    expectToHaveSubmoduleState(topRepo, "master", "bottom-project", bottomRepo,
        "master");
  }
  ...
  public void projectCircularSubscriptionWholeTopic() throws Exception {
    ...
    ObjectId subMasterHead = pushChangeTo(subRepo, "refs/for/master", "b.txt",
        "content b", "some message", "same-topic");
  ...
  public void projectNoSubscriptionWholeTopic() throws Exception {
    ...
    ObjectId aHead = pushChangeTo(repoA, "refs/for/master", "master.txt",
        "content master A", "some message in a master.txt", "same-topic");

    // create a change for master branch in repo b
    ObjectId bHead = pushChangeTo(repoB, "refs/for/master", "master.txt",
        "content master B", "some message in b master.txt", "same-topic");

    // create a change for dev branch in repo a
    repoA.reset(a0);
    ObjectId aDevHead = pushChangeTo(repoA, "refs/for/dev", "dev.txt",
        "content dev A", "some message in a dev.txt", "same-topic");

    // create a change for dev branch in repo b
    repoB.reset(b0);
    ObjectId bDevHead = pushChangeTo(repoB, "refs/for/dev", "dev.txt",
        "content dev B", "some message in b dev.txt", "same-topic");
  ...
  public void twoProjectsMultipleBranchesWholeTopic() throws Exception {
    ...
    allowMatchingSubmoduleSubscription("project-b", "refs/heads/master",
