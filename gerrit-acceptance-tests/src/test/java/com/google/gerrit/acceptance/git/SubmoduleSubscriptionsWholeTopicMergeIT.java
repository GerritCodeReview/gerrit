...
public class SubmoduleSubscriptionsWholeTopicMergeIT
  ...
  public void testSameProjectDifferentBranchDifferentPaths() throws Exception {
    ...
    String superProject = "super-project";
    String subProject = "sub-project";
    TestRepository<?> superRepo = createProjectWithPush(superProject);
    TestRepository<?> subRepo = createProjectWithPush(subProject);

    allowMatchingSubmoduleSubscription(subProject, "refs/heads/master",
        superProject, "refs/heads/master");
    allowMatchingSubmoduleSubscription(subProject, "refs/heads/dev",
        superProject, "refs/heads/master");

    RevCommit initialHeadSub =
        getRemoteHead(new Project.NameKey(name(subProject)), "master");
    BranchInput in = new BranchInput();
    in.revision = initialHeadSub.name();
    gApi.projects().name(name(subProject)).branch("dev").create(in);

    Config config = new Config();
    prepareSubmoduleConfigEntry(config, subProject, "sub-master", "master");
    prepareSubmoduleConfigEntry(config, subProject, "sub-dev", "dev");
    pushSubmoduleConfig(superRepo, "master", config);

    ObjectId subMasterId =
        pushChangeTo(subRepo, "refs/for/master", "some message", "b.txt",
            "content b", topic);

    subRepo.reset(initialHeadSub);
    ObjectId subDevId =
        pushChangeTo(subRepo, "refs/for/dev", "some message in dev", "b.txt",
            "content b", topic);
    ...
    expectToHaveSubmoduleState(superRepo, "master", "sub-master", subRepo, "master");
    expectToHaveSubmoduleState(superRepo, "master", "sub-dev", subRepo, "dev");
