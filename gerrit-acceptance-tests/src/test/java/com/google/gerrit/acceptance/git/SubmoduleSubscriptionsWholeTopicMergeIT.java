...
public class SubmoduleSubscriptionsWholeTopicMergeIT
  ...
  public void testProjectNoSubscriptionWholeTopic() throws Exception {
    ...
    ObjectId aHead =
        pushChangeTo(repoA, "refs/for/master", "master.txt", "content master",
            "some message in master.txt", "same-topic");

    // create a change for master branch in repo b
    ObjectId bHead =
        pushChangeTo(repoB, "refs/for/master", "master.txt", "content master",
            "some message in master.txt", "same-topic");

    // create a change for dev branch in repo a
    repoA.reset(a0);
    ObjectId aDevHead =
        pushChangeTo(repoA, "refs/for/dev", "dev.txt", "content dev",
            "some message in dev.txt", "same-topic");

    // create a change for dev branch in repo b
    repoB.reset(b0);
    ObjectId bDevHead =
        pushChangeTo(repoB, "refs/for/dev", "dev.txt", "content dev",
            "some message in dev.txt", "same-topic");
    ...
    assertThat(
        ...
        .contains("some message in master.txt");
    assertThat(
        getRemoteHead(name("project-a"), "refs/heads/dev").getShortMessage())
        .contains("some message in dev.txt");
    assertThat(
