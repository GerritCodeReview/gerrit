...
public class PushForReviewIT extends AbstractDaemonTest {
  ...
  @Test
  public void testPushForMasterWithCc() throws GitAPIException, OrmException, IOException, JSchException {
    // cc one user
    ...
    push = new PushOneCommit();
    ref = "refs/for/master/" + topic + "%cc=" + admin.email + ",cc=" + user.email + ",cc=" + user2.email;
    r = push.to(ref);
    assertOkStatus(r, ref);
    assertChange(push.changeId, Change.Status.NEW, PushOneCommit.SUBJECT, topic);
  ...
  @Test
  public void testPushForMasterWithReviewer() throws GitAPIException, OrmException, IOException, JSchException {
    // add one reviewer
    TestAccount user = accounts.create("user", "user@example.com", "User");
    PushOneCommit push = new PushOneCommit();
    String topic = "my/topic";
    ...
    PushResult r = push.to(ref);
    assertOkStatus(r, ref);
    assertChange(push.changeId, Change.Status.NEW, PushOneCommit.SUBJECT, topic, user);
    ...
    assertChange(push.changeId, Change.Status.NEW, PushOneCommit.SUBJECT, topic, user, user2);
  ...
  @Test
  public void testPushForNonExistingBranch() throws GitAPIException, OrmException, IOException {
    PushOneCommit push = new PushOneCommit();
  ...
  private void assertReviewers(Change c, TestAccount... expectedReviewers) throws OrmException {
    ...
    for (PatchSetApproval psa : db.patchSetApprovals().byPatchSet(c.currentPatchSetId())) {
      assertTrue("unexpected reviewer " + psa.getAccountId(), expectedReviewerIds.remove(psa.getAccountId()));
    }
  ...
  private static void assertErrorStatus(PushResult result, String expectedMessage, String ref) {
    assertStatus(Status.REJECTED_OTHER_REASON, expectedMessage, result, ref);
  }

  private static void assertStatus(Status expectedStatus, String expectedMessage, PushResult result,
      String ref) {
