...
public class ChangeReviewersIT extends AbstractDaemonTest {
  ...
  public void addGroupAsReviewer() throws Exception {
    ...
    final int largeGroupSize = PostReviewers.DEFAULT_MAX_REVIEWERS + 1;
    final int mediumGroupSize = PostReviewers.DEFAULT_MAX_REVIEWERS_WITHOUT_CHECK + 1;
    List<TestAccount> users = Lists.newArrayListWithCapacity(largeGroupSize);
    List<String> largeGroupUsernames = Lists.newArrayListWithCapacity(largeGroupSize);
    List<String> mediumGroupUsernames = Lists.newArrayListWithCapacity(mediumGroupSize);
    for (int i = 0; i < largeGroupSize; i++) {
      users.add(accounts.create(name("u" + i), name("u" + i + "@example.com"), "Full Name " + i));
