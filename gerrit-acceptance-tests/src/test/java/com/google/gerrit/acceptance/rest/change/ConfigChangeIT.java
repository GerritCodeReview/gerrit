...
public class ConfigChangeIT extends AbstractDaemonTest {
  ...
  public void rejectDoubleInheritance() throws Exception {
    ...
    PushOneCommit push = pushFactory.create(
        ...
        config);
    PushOneCommit.Result a = push.to(RefNames.REFS_CONFIG)
        ;
    a.assertErrorStatus();
    a.assertMessage("error: cannot inherit from multiple projects");

    // Reset permission to the default for this class
    setApiUser(user);
  }
