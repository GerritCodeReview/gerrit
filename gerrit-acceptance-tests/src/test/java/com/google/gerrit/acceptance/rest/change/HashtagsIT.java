...
public class HashtagsIT extends AbstractDaemonTest {
  ...
  @Test
  public void testHashtagsFromCommitMessage() throws Exception {
    ...
    // if hashtag exists in current patch-set's commit message, need to delete
    // it firstly from current patchset commit message before remove if from
    // hashtags pool
    try {
      POST(changeId, null, "tag1");
    } catch (Exception e) {
      fail(e.getMessage());
    }
    assertResult(GET(changeId), created);
  ...
  private static List<String> toHashtagList(RestResponse r)
      throws IOException {
    List<HashtagInfo> result =
        newGson().fromJson(r.getReader(),
            new TypeToken<List<HashtagInfo>>() {}.getType());
    return FluentIterable.from(result)
        .transform(new Function<HashtagInfo, String>() {
          @Override
          public String apply(HashtagInfo input) {
            return input.name;
          }
        }).toList();
  }

  private String getCommitMessageWithHashtagsFooter(String... hashtags) {
    StringBuilder msg = new StringBuilder("Add hashtags from commit message. ");
    msg.append("\n\n");
    for (String f : hashtags) {
      msg.append(FOOTER_KEY_HASHTAGS).append(":").append(f).append("\n");
    }
    return msg.toString();
  }

  private PushOneCommit.Result newChange(String... hashtags)
      throws GitAPIException, IOException {
    PushOneCommit push =
        pushFactory.create(db, admin.getIdent(),
            getCommitMessageWithHashtagsFooter(hashtags),
            PushOneCommit.FILE_NAME, PushOneCommit.FILE_CONTENT);
    return push.to(git, "refs/for/master");
  }

  private PushOneCommit.Result amendChange(String changeId, String... hashtags)
      throws GitAPIException, IOException {
    PushOneCommit push =
        pushFactory.create(db, admin.getIdent(),
            getCommitMessageWithHashtagsFooter(hashtags),
