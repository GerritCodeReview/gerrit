...
public class SubmitByMergeIfNecessaryIT extends AbstractSubmitByMerge {
  ...
  public void openChangeForTargetBranchPreventsMerge() throws Exception {
    ...
    RevCommit stable = getRemoteHead(project, "stable");
    RevCommit ontop = testRepo.commit()
        .message("Something on top of the fix of the stable branch")
        .add("b", "test")
        .parent(stable)
        .insertChangeId()
        .create();
    // This call fails unexpectedly with a NullPointerException
    testRepo.git()
        .branchCreate()
        .setName("test")
        .setStartPoint(ontop)
    ...
    testRepo.git().push()
      .setRemote("origin")
      .setRefSpecs(new RefSpec("refs/heads/test:refs/for/master"))
      .call();

    String changeId = GitUtil.getChangeId(testRepo, ontop).get();
    approve(changeId);
    submitWithConflict(changeId, "");
  }

  @Test
  public void openChangeForIrrelevantPreventsMerge() throws Exception {
