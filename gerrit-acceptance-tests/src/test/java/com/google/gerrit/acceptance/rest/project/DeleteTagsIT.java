...
import static com.google.gerrit.acceptance.rest.project.TagAssert.assertRefNames;
...
import com.google.gerrit.extensions.api.projects.DeleteTagInput;
...
public class DeleteTagsIT extends AbstractDaemonTest {
  private static final List<String> TAGS = ImmutableList.of(
      "refs/tags/test-1", "refs/tsgs/test-2", "refs/tags/test-3");
  ...
  public void deleteTagsForbidden() throws Exception {
    ...
    try {
      ...
      assertThat(e).hasMessage(errorMessageForBranches(TAGS));
    }
    setApiUser(admin);
    assertTags(TAGS);
  }
  ...
  public void deleteTagsNotFoundContinue() throws Exception {
    ...
    DeleteTagsInput input = new DeleteBranchesInput();
  ...
  private void assertTags(List<String> tags) throws Exception {
    List<String> expected = Lists.newArrayList(
        "HEAD", RefNames.REFS_CONFIG, "refs/tags/master");
    expected.addAll(tags);
    assertRefNames(expected, project().tags().get());
  }
