...
public class ListProjectsIT extends AbstractDaemonTest {
  ...
  public void listWithHiddenAndReadonlyProjects() throws Exception {
    ...
    assertThatNameList(gApi.projects().list().withState(ProjectState.HIDDEN).ge$
        .containsExactly(hidden);
    assertThatNameList(gApi.projects().list().withState(ProjectState.READ_ONLY)$
        .containsExactly(readonly);
    assertThatNameList(gApi.projects().list().withState(ProjectState.ACTIVE).ge$
        .containsExactly(allProjects, allUsers, project)
        .inOrder();
