...
public class TagsIT extends AbstractDaemonTest {
  private static final List<String> testTags = ImmutableList.of("tag-A",
      "tag-B", "tag-C", "tag-D", "tag-E", "tag-F", "tag-G", "tag-H");
  ...
  public void listTags() throws Exception {
    ...
    assertTagList(FluentIterable.from(ImmutableList.of("tag-C", "tag-D")),
        result);
  ...
  public void lightweightTag() throws Exception {
    ...
    eventRecorder.assertRefUpdatedEvents(project.get(), result.ref, null,
        result.revision);
  }
  ...
  public void annotatedTag() throws Exception {
    ...
    eventRecorder.assertRefUpdatedEvents(project.get(), result2.ref, null,
        result2.revision);
  }
  ...
  public void createAnnotatedTagNotAllowed() throws Exception {
    ...
    exception.expect(AuthException.class);
    exception
        .expectMessage("Cannot create annotated tag \"" + R_TAGS + "test\"");
    tag(input.ref).create(input);
  }
