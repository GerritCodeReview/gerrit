...
public class CommentsIT extends AbstractDaemonTest {
  ...
  public void deleteCommentByAdminRemoveAllData() throws Exception {
    DeleteCommentInput input = new DeleteCommentInput();
    input.reason = "spam comment";
    input.removeAllData = true; // Remove the comment permanently.
    deleteCommentByAdmin(input);
  }
  ...
  private void deleteCommentByAdmin(DeleteCommentInput input) throws Exception {
    ...
    ReviewInput reviewInput = new ReviewInput();
    CommentInput comment1 = newComment(fileName, Side.REVISION, 0,
        "My password: abc123", false);
    CommentInput comment2 = newComment(fileName, Side.REVISION, 1,
        "nit: long line", false);
    ...
    String expectedMsg = "Comment removed by: " + admin.username
        + (input.reason.isEmpty() ? "" : ("; Reason: " + input.reason))
        + ".";

    if (notesMigration.writeChanges()) {
      commitDataListAfter = getCommitDataList(result.getChange().getId());
      assertCommitDataList(commitDataListBefore, commitDataListAfter,
          targetCommentInfo.id, expectedMsg, input.removeAllData);
    }
    ...
    List<CommentInput> actualComments = Lists.transform(after.get(fileName),
        infoToInput(fileName));
    if (input.removeAllData) {
      assertThat(actualComments).containsExactly(comment2, comment3, comment4);
    } else {
      // Change comment1's message to the expected message.
      comment1.message = expectedMsg;
      assertThat(actualComments).containsExactly(comment1, comment2,
          comment3, comment4);
    }
  }
  ...
  private List<ChangeNotesCommitData> getCommitDataList(Change.Id changeId)
    ...
    try (Repository repo = repoManager.openRepository(project);
      ...
      while ((commit = walk.next()) != null) {
        walk2.reset();
        commitDataList.add(0, new ChangeNotesCommitData(commit,
   ...
   *
   * If removeAllData is true:
   * - All the commits should not contain the target comment any more after
   *   deleting. For the commit putting in the target comment before, its
   *   corresponding new commit should have its committer and message updated.
   * If removeAllData is false:
   * - All the commits, which contain the target comment before, should still
   *   contain the comment with the message updated. For the commit putting in
   *   the target comment before, its corresponding new commit should have
   *   its committer updated, too.
   *
   * All the other metas of the commits should be exactly the same.
   */
  private void assertCommitDataList(List<ChangeNotesCommitData> beforeDelete,
      List<ChangeNotesCommitData> afterDelete, String targetUUID,
      String newMsg, boolean removeAllData) {
    ...
    for (int i = 0; i < beforeDelete.size(); ++i) {
      ...
      Map<String, com.google.gerrit.reviewdb.client.Comment>
          commentMapBefore = before.commentsMap;
      Map<String, com.google.gerrit.reviewdb.client.Comment>
          commentMapAfter = after.commentsMap;
      ...
      if (removeAllData) {
        if (commentMapBefore.containsKey(targetUUID)) {
          if (isFirstCommitWithTarget) {
            ...
            int num = commentMapAfter.size();
            String expectMsg = commitBefore.getFullMessage().replace(
                "(" + (num + 1) + " comment)", "(" + num + " comment)");
            assertThat(commitAfter.getFullMessage()).isEqualTo(expectMsg);

            isFirstCommitWithTarget = false;
          } else {
            assertThat(commitAfter.getCommitterIdent())
                .isEqualTo(commitBefore.getCommitterIdent());
            assertThat(commitAfter.getFullMessage())
                .isEqualTo(commitBefore.getFullMessage());
          }

          assertThat(commentMapAfter.containsKey(targetUUID)).isFalse();
          commentMapAfter.put(targetUUID, commentMapBefore.get(targetUUID));
        ...
        if (commentMapBefore.containsKey(targetUUID)) {
          ...
          com.google.gerrit.reviewdb.client.Comment comment =
              commentMapAfter.get(targetUUID);
      ...
      assertThat(commitAfter.getAuthorIdent())
          .isEqualTo(commitBefore.getAuthorIdent());
      assertThat(commitAfter.getEncoding())
          .isEqualTo(commitBefore.getEncoding());
      assertThat(commitAfter.getEncodingName())
          .isEqualTo(commitBefore.getEncodingName());
    }
  }
