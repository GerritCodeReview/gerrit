...
public class SubmittedTogetherIT extends AbstractDaemonTest {
  ...
  @Test
  @GerritConfig(name = "repository.*.defaultSubmitType", value = "CHERRY_PICK")
  ...
  private void assertSubmittedTogether(String chId, String... expected)
      throws Exception {
    List<ChangeInfo> actual = getSubmittedTogether(chId);
    assertThat(actual).hasSize(expected.length);
    for (int i = 0; i < actual.size(); i++) {
      ChangeInfo a = actual.get(i);
      String e = expected[i];
      assertThat(a.changeId).isEqualTo(e);
    }
  }

  private RevCommit getRemoteHead()
      throws IOException {
  ...
  private List<ChangeInfo> getSubmittedTogether(String changeId)
      throws RestApiException {
    return gApi.changes().id(changeId).submittedTogether();
  }
