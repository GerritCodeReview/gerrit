...
public class MailProcessorIT extends AbstractDaemonTest {
  ...
  @Test
  public void testParseAndPersistChangeMessage() throws Exception {
    String changeId = createChangeForTest();
  ...
  @Test
  public void testParseAndPersistInlineComment() throws Exception {
    String changeId = createChangeForTest();
    ChangeInfo changeInfo = gApi.changes().id(changeId).get();
    List<CommentInfo> comments = gApi.changes().id(changeId)
        .current().commentsAsList();
  ...
  @Test
  public void testParseAndPersistFileComment() throws Exception {
    String changeId = createChangeForTest();
    ChangeInfo changeInfo = gApi.changes().id(changeId).get();
    List<CommentInfo> comments = gApi.changes().id(changeId)
        .current().commentsAsList();
    ...
    // Path is not returned on the CommentInfo, but as key in the map
    assertThat(comments.get(0).path).isEqualTo("gerrit-server/test.txt");
  }

  @Test
  public void testParseAndPersistMessageTwice() throws Exception {
    String changeId = createChangeForTest();
    ChangeInfo changeInfo = gApi.changes().id(changeId).get();
    List<CommentInfo> comments = gApi.changes().id(changeId)
        .current().commentsAsList();
  ...
  private String createChangeForTest() throws Exception {
    // Create change
