...
public class NoteDbPrimaryIT extends AbstractDaemonTest {
  ...
  private void testReadOnly(Change.Id id) throws Exception {
    ...
    try {
      ...
      Optional<Throwable> oe = Throwables.getCausalChain(e).stream()
          .filter(x -> x instanceof OrmException).findFirst();
      assertThat(oe.isPresent()).named("OrmException in causal chain of " + e)
          .isTrue();
  ...
  public void rebuildReviewDb() throws Exception {
    ...
    rin.comments =
        ImmutableMap.of(PushOneCommit.FILE_NAME, ImmutableList.of(cin));
    gApi.changes().id(id.get()).current().review(ReviewInput.approve());
    ...
    ChangeBundle noteDbBundle = ChangeBundle.fromNotes(commentsUtil,
        notesFactory.create(db, project, id));
    ...
    ChangeMessage bogusMessage = ChangeMessagesUtil.newMessage(
        ReviewDbUtil.unwrapDb(db), c.currentPatchSetId(),
        identifiedUserFactory.create(admin.getId()), TimeUtil.nowTs(),
        "some message", null);
    ...
    ChangeBundle reviewDbBundle =
        bundleReader.fromReviewDb(ReviewDbUtil.unwrapDb(db), id);
