...
public class ProjectWatchIT extends AbstractDaemonTest {
  ...
  public void watchProject() throws Exception {
    ...
    PushOneCommit.Result r = pushFactory
        .create(db, admin.getIdent(), watchedRepo, "my subject", "a", "a1")
        .to("refs/for/master");
    r.assertOkStatus();
    ...
    r = pushFactory.create(db, admin.getIdent(), notWatchedRepo,
        "other subject", "a", "a1").to("refs/for/master");
    r.assertOkStatus();
  ...
  public void watchFile() throws Exception {
    ...
    TestRepository<InMemoryRepository> watchedRepo =
        cloneProject(new Project.NameKey(watchedProject), admin);
    PushOneCommit.Result r = pushFactory
        .create(db, admin.getIdent(), watchedRepo, "my subject", "a.txt", "a1")
        .to("refs/for/master");
    r.assertOkStatus();
    ...
    // notification
    r = pushFactory.create(db, admin.getIdent(), testRepo,
        "other subject", "b.txt", "b1").to("refs/for/master");
    r.assertOkStatus();
  ...
  private void watch(String project) throws RestApiException {
    watch(project, null);
  }
