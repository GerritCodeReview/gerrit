...
public class GlobalCapability {
   ...
   * <p>
   * This can be used to deny build bots from emailing reviewers and instead
   * only email the change authors. The allow rules are evaluated before deny
   * rules, however the default is to allow emailing of reviewers if
   * no explicit rule is matched.
   */
