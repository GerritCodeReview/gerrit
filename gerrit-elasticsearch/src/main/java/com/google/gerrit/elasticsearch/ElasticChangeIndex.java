...
class ElasticChangeIndex extends AbstractElasticIndex<Change.Id, ChangeData>
  ...
  static class ChangeMapping {
    ...
    ChangeMapping() {
      ElasticMapping.Builder openChangesBuilder = new ElasticMapping.Builder();
      ElasticMapping.Builder closedChangesBuilder = new ElasticMapping.Builder();
      Schema<ChangeData> schema = ChangeSchemaDefinitions.INSTANCE.getLatest();
      for (FieldDef<?, ?> field : schema.getFields().values()) {
        String name = field.getName();
        FieldType<?> fieldType = field.getType();
        if (fieldType == FieldType.EXACT) {
          openChangesBuilder.addExactField(name);
          closedChangesBuilder.addExactField(name);
        } else if (fieldType == FieldType.TIMESTAMP) {
          openChangesBuilder.addTimestamp(name);
          closedChangesBuilder.addTimestamp(name);
        } else if (fieldType == FieldType.INTEGER
            || fieldType == FieldType.INTEGER_RANGE
            || fieldType == FieldType.LONG) {
          openChangesBuilder.addNumber(name);
          closedChangesBuilder.addNumber(name);
        } else if (fieldType == FieldType.PREFIX
            || fieldType == FieldType.FULL_TEXT
            || fieldType == FieldType.STORED_ONLY) {
          openChangesBuilder.addString(name);
          closedChangesBuilder.addString(name);
        } else {
          openChangesBuilder.add(name, fieldType.getName());
          closedChangesBuilder.add(name, fieldType.getName());
        }
      }
      openChanges = openChangesBuilder.build();
      closedChanges = closedChangesBuilder.build();
    }
  }
  private static final Logger log =
      LoggerFactory.getLogger(ElasticChangeIndex.class);
  private static final ChangeMapping MAPPING = new ChangeMapping();
  ...
  public static String getMappingProperties(Gson gson) {
    ImmutableMap<String, ChangeMapping> mappings = ImmutableMap.of("mappings", MAPPING);
  ...
  protected String getMappings() {
    return getMappingProperties(gson);
  }
