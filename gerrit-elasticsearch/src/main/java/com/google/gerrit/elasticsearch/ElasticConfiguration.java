...
class ElasticConfiguration {
  ...
  ElasticConfiguration(@GerritServerConfig Config cfg) {
    this.username = cfg.getString("index", null, "username");
    this.password = cfg.getString("index", null, "password");
    this.requestCompression = cfg.getBoolean("index", null, "requestCompression", false);
    this.connectionTimeout =
        cfg.getTimeUnit("index", null, "connectionTimeout", 3000, TimeUnit.MILLISECONDS);
    this.maxConnectionIdleTime =
        cfg.getTimeUnit("index", null, "maxConnectionIdleTime", 3000, TimeUnit.MILLISECONDS);
    this.maxTotalConnection = cfg.getInt("index", null, "maxTotalConnection", 1);
    this.readTimeout =
        (int) cfg.getTimeUnit("index", null, "readTimeout", 3000, TimeUnit.MICROSECONDS);
