...
import com.google.gwt.thirdparty.guava.common.base.MoreObjects;
...
public class ElasticVersionManager implements LifecycleListener {
  ...
  private final ElasticVersionDiscovery versionDiscovery;
  ...
  ElasticVersionManager(
      ...
      ElasticVersionDiscovery versionDiscovery) {
  ...
  private <K, V, I extends Index<K, V>> TreeMap<Integer, Version<V>> scanVersions(IndexDefinition<K, V, I> def) throws IOException {
    TreeMap<Integer, Version<V>> versions = new TreeMap<>();
    for (Schema<V> schema : def.getSchemas().values()) {
      int v = schema.getVersion();
      versions.put(v, new Version<>(schema, v, IndexUtils.getReady(sitePaths, def.getName(), schema.getVersion())));
    }
    ...
    try {
      for (String version : versionDiscovery.discover(prefix, def.getName())) {
        ...
        if (!versions.containsKey(v)) {
          versions.put(v, new Version<V>(null, v, IndexUtils.getReady(sitePaths, def.getName(), v)));
        }
      }
    } catch (IOException e) {
  ...
  private <V> void markNotReady(
      ...
      Collection<Version<V>> inUse) throws IOException {
