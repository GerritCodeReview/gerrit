...
class ElasticsearchChangeIndex implements ChangeIndex, LifecycleListener {
  ...
  ElasticsearchChangeIndex(
    ...
    String indexUrl = cfg.getString("index", null, "url");
    if (Strings.isNullOrEmpty(indexUrl)) {
    ...
    try {
      ...
      throw new IllegalStateException(
          String.format("index.url is malformed: %s", indexUrl));
    }
    ...
    log.info(String.format("cluster [%s] hostname [%s] port [%d]", cluster, hostname, port));
  ...
  private void add(XContentBuilder builder, Values<ChangeData> values)
      throws IOException {
    ...
    if (type == FieldType.INTEGER || type == FieldType.INTEGER_RANGE) {
      for (Object value : values.getValues()) {
        builder.field(name, (Integer) value);
      }
  ...
  public ChangeDataSource getSource(Predicate<ChangeData> p, int start,
    ...
    //TODO: Add support for ordering/sorting
    return new QuerySource(indexes, toQuery(p), start, limit);
  }

  private BaseQueryBuilder toQuery(Predicate<ChangeData> p)
      throws QueryParseException {
    //FIXME: If the query includes an "owner:" operator, the string
    //       returned by toString is wrong.
  ...
  private class QuerySource implements ChangeDataSource {
    ...
    private final BaseQueryBuilder query;
    private final int start;
    private final int limit;

    public QuerySource(List<String> indexes, BaseQueryBuilder query,
    ...
    public ResultSet<ChangeData> read() throws OrmException {
      log.info(String.format("read() query: [%s]", query.toString()));
      try {
        SearchHits hits = client.prepareSearch(INDEX_NAME)
          .setFrom(start)
          .setSize(limit)
