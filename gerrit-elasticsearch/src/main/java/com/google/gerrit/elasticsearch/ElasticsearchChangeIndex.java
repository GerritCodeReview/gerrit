...
import static com.google.gerrit.server.query.change.ChangeQueryBuilder.hasNonTrivialSortKeyAfter;
...
class ElasticsearchChangeIndex implements ChangeIndex, LifecycleListener {
  ...
  ElasticsearchChangeIndex(
    ...
    // Whether to refresh the index after adding or removing a change.
    // This setting should only be set to true during testing, and is not
    // documented.
    refresh = cfg.getBoolean("index", "elasticsearch", "refresh", false);
  ...
  private static List<Sort> getSorts(Schema<ChangeData> schema,
      Predicate<ChangeData> p) {
    if (SortKeyPredicate.hasSortKeyField(schema)) {
      Sorting order = hasNonTrivialSortKeyAfter(schema, p)
          ? Sorting.ASC : Sorting.DESC;
      return ImmutableList.of(
          new Sort(ChangeField.SORTKEY.getName(), order));
    } else {
      return ImmutableList.of(
  ...
  private class QuerySource implements ChangeDataSource {
    ...
    public QuerySource(List<String> indexes, Predicate<ChangeData> p,
      ...
      SearchSourceBuilder searchSourceBuilder =
          new SearchSourceBuilder()
              .query(QueryBuilders.queryString(q))
              .from(start)
              .size(limit);
      ...
      for (Sort sort : getSorts(schema, p)) {
