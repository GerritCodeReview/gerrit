...
final class ElasticTestUtils {
  ...
  static void createIndexes(Node node) {
    ChangeMapping openChangeMapping = new ChangeMapping(ChangeSchemaDefinitions.INSTANCE.getLatest());
    ChangeMapping closedChangeMapping = new ChangeMapping(ChangeSchemaDefinitions.INSTANCE.getLatest());
    AccountMapping accountMapping = new AccountMapping(AccountSchemaDefinitions.INSTANCE.getLatest());
    openChangeMapping.closedChanges = null;
    closedChangeMapping.openChanges = null;
    node.client()
        .admin()
        .indices()
        .prepareCreate("gerrit")
        .addMapping(ElasticChangeIndex.OPEN_CHANGES, gson.toJson(openChangeMapping))
        .addMapping(ElasticChangeIndex.CLOSED_CHANGES, gson.toJson(closedChangeMapping))
        .addMapping(ElasticAccountIndex.ACCOUNTS, gson.toJson(accountMapping))
        .execute()
        .actionGet();
  }

  static void deleteIndexes(Node node) {
  ...
  private static String getHttpPort(Node node)
    ...
    if (httpAddress.indexOf(':') < 0) {
      throw new RuntimeException(
          "Seams that port is not included in Elastcsearch http_address");
    }
