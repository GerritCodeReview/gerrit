...
public class DynamicSet<T> implements Iterable<T> {
  ...
  public static <T> void setOf(Binder binder, Class<T> member) {
    ...
    Key<DynamicSet<T>> key = (Key<DynamicSet<T>>) Key.get(
        Types.newParameterizedType(DynamicSet.class, member));
    binder.bind(key)
   ...
   */
  public ReloadableRegistrationHandle<T> add(Key<?> key, T item) {
  ...
  private class ReloadableHandle implements ReloadableRegistrationHandle<T> {
    ...
    ReloadableHandle(AtomicReference<T> ref, Key<?> key, T item) {
      this.ref = ref;
      this.key = key;
      this.item = item;
    }
