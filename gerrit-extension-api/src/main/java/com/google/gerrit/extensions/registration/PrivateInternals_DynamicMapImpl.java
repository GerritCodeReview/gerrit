...
public class PrivateInternals_DynamicMapImpl<T> extends DynamicMap<T> {
  ...
  public ReloadableRegistrationHandle<T> put(
      String pluginName, Key<?> key,
      T item) {
    String exportName = ((Export) key.getAnnotation()).value();
    final NamePair np = new NamePair(pluginName, exportName);
    items.put(np, item);
  ...
  private class ReloadableHandle implements ReloadableRegistrationHandle<T> {
    ...
    ReloadableHandle(NamePair np, Key<?> key, T item) {
      this.np = np;
      this.key = key;
      this.item = item;
    }
    ...
    @Override
    public ReloadableHandle replace(Key<?> newKey, T newItem) {
