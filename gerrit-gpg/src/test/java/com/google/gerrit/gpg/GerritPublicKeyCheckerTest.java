...
public class GerritPublicKeyCheckerTest {
  ...
  public void defaultGpgCertificationDoesNotMatchEmail() throws Exception {
    ...
    GerritPublicKeyChecker checker = checkerFactory.create(user);
    assertProblems(
        checker.check(TestKey.key5().getPublicKey()),
        "Key must contain a valid certification for one of the following "
          + "identities:\n"
          + "  gerrit:user\n"
          + "  nobody@example.com\n"
  ...
  public void manualCertificationMatchesExternalId() throws Exception {
    ...
    GerritPublicKeyChecker checker = checkerFactory.create(user);
    assertNoProblems(checker.check(TestKey.key5().getPublicKey()));
  }
  ...
  public void checkValidTrustChainAndCorrectExternalIds() throws Exception {
    ...
    assertProblems(
        checkerA.check(keyB.getPublicKey(), store),
        "Key is expired",
        "Key must contain a valid certification for one of the following"
            + " identities:\n"
            + "  gerrit:user\n"
    ...
    assertProblems(
        checkerB.check(keyB.getPublicKey(), store),
        "Key is expired");
    assertProblems(
        checkerB.check(keyA.getPublicKey(), store),
        "Key must contain a valid certification for one of the following"
            + " identities:\n"
            + "  gerrit:userB\n"
  ...
  public void checkTrustChainWithExpiredKey() throws Exception {
    ...
    GerritPublicKeyChecker checker = checkerFactory.create(user);
    assertProblems(
        checker.check(keyA.getPublicKey(), store),
  ...
  @Test
  public void checkTrustChainWithoutSpecifyingExpectedKey() throws Exception {
    // A---Bx
    //  \
    //   \---C---D
    //        \
  ...
  public void keyLaterInTrustChainMissingUserId() throws Exception {
    ...
    GerritPublicKeyChecker checkerA = checkerFactory.create(user);
    assertProblems(checkerA.check(keyA.getPublicKey(), store),
        "No path to a trusted key",
        "Certification by B007D490"
            + " (355D 5B98 FECE 6199 83CD  C91D 5760 6987 B007 D490) is valid,"
  ...
  private void assertProblems(CheckResult result, String... expected)
      throws Exception {
