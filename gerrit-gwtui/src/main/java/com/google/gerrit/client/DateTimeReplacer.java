...
class DateTimeReplacer {
  ...
  private static final int ONE_EXPANSION_LEN =
      "\" HH:mm:ss.SSS -ZZZZ\"".length();
  ...
  static String replaceTimestamps(String query, String tz) {
    StringBuilder sb = new StringBuilder(
        query.length() + 2 * ONE_EXPANSION_LEN);
  ...
  private static int replaceTimestamp(String query, int start, boolean inQuote,
    ...
    StringBuilder term = new StringBuilder(query.length() + ONE_EXPANSION_LEN);
    inQuote = false;
    int i;
