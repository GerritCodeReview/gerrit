...
public class ErrorDialog extends PopupPanel {
  ...
  /**
   * Add more feature to the button to handle the error properly if needed
   * @param msg error message
   */
  private void enhanceButton(String msg) {
    // Redirection added for 'My.*' menus after an expired session
    ...
    // If any My.Changes item is selected after an expired session,
    // users are redirected to the login page if they select Continue
    else if (msg.equals(Gerrit.C.notFoundBody())) {
      closey.addClickHandler(new ClickHandler() {
        @Override
        public void onClick(ClickEvent event) {
          hide();
