...
public class Gerrit implements EntryPoint {
  ...
  private static void populateBottomMenu(RootPanel btmmenu, HostPageData hpd) {
    ...
    String reportBugUrl = info().gerrit().report_bug_url();
    if (reportBugUrl != null) {
      String reportBugText = info().gerrit().report_bug_text();
      Anchor a = new Anchor(
  ...
  private static void refreshMenuBar(boolean populateMyMenu) {
    ...
    if (signedIn) {
      ...
      switch (authInfo.authType()) {
        ...
        case HTTP:
        case HTTP_LDAP:
          if (authInfo.login_url() != null) {
            String signinText = authInfo.login_text() == null
                ? C.menuSignIn()
                : authInfo.login_text();
            menuRight.add(anchor(signinText, authInfo.login_url()));
          }
          break;
        ...
        case CUSTOM_EXTENSION:
          if (authInfo.register_url() != null) {
            String registerText = authInfo.register_text() == null
                ? C.menuRegister()
                : authInfo.register_text();
            menuRight.add(anchor(registerText, authInfo.register_url()));
          }
          menuRight.addItem(C.menuSignIn(), new Command() {
            @Override
            public void execute() {
