...
public class RecentlyAccessed implements Serializable {
  ...
  public static synchronized RecentlyAccessed get() {
    if (instance == null) {
      instance = load();
    }
    return instance;
  ...
  private static RecentlyAccessed load() {
    ...
    if (s != null && s.startsWith(PROJECTS_KEY)) {
      ...
      for (final String p : projectNames) {
        projects.add(new Project.NameKey(p));
      }
    }
   ...
   */
  public synchronized void add(final Project.NameKey project) {
   ...
   */
  public synchronized List<Project.NameKey> getProjects() {
    if (limit()) {
      save();
    }
   ...
   *         <code>false</code>
   */
  private synchronized boolean limit() {
