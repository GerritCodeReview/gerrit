...
class ContactPanelShort extends Composite {
  ...
  protected void onInitUI() {
    ...
    int row = 0;
    if (!Gerrit.getServerInfo().auth().canEdit(FieldName.USER_NAME)
        && Gerrit.getServerInfo().auth().siteHasUsernames()) {
      ...
      if (Gerrit.getServerInfo().auth().editFullNameUrl() != null) {
        ...
        edit.addClickHandler(new ClickHandler() {
          ...
          public void onClick(ClickEvent event) {
            Window.open(Gerrit.getServerInfo().auth().editFullNameUrl(),
                "_blank", null);
          }
        });
        nameLine.add(edit);
  ...
  private boolean canRegisterNewEmail() {
    return Gerrit.getServerInfo().auth()
        .canEdit(Account.FieldName.REGISTER_NEW_EMAIL);
  }
