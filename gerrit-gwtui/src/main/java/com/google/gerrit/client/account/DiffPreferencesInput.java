...
public class DiffPreferencesInput extends JavaScriptObject {
  public static DiffPreferencesInput create(DiffPreferencesInfo original) {
    DiffPreferencesInput input = createObject().cast();
    input.setContext(original.context());
    input.setExpandAllComments(original.expand_all_comments());
    input.setIgnoreWhitespace(original.ignore_whitespace());
    input.setIntralineDifference(original.intraline_difference());
    input.setLineLength(original.line_length());
    input.setManualReview(original.manual_review());
    input.setRetainHeader(original.retain_header());
    input.setShowLineEndings(original.show_line_endings());
    input.setShowTabs(original.show_tabs());
    input.setShowWhitespaceErrors(original.show_whitespace_errors());
    input.setSkipDeleted(original.skip_deleted());
    input.setSkipUncommented(original.skip_uncommented());
    input.setSyntaxHighlighting(original.syntax_highlighting());
    input.setTabSize(original.tab_size());
    return input;
  }
