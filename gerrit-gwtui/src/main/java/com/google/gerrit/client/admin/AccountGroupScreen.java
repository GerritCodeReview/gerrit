...
public class AccountGroupScreen extends AccountScreen {
  ...
  private IncludedGroupTable includedGroups;
  ...
  private Panel includedGroupPanel;
  private AddIncludedGroupBox addIncludedGroupBox;
  private Button delIncludedGroup;
  ...
  private void initIncludedGroupList() {
    addIncludedGroupBox = new AddIncludedGroupBox();

    addIncludedGroupBox.addClickHandler(new ClickHandler() {
      @Override
      public void onClick(final ClickEvent event) {
        doAddNewIncludedGroup();
      }
    });

    includedGroups = new IncludedGroupTable();

    delIncludedGroup = new Button(Util.C.buttonDeleteGroupMembers());
    delIncludedGroup.addClickHandler(new ClickHandler() {
      @Override
      public void onClick(final ClickEvent event) {
        includedGroups.deleteChecked();
      }
    });

    includedGroupPanel = new FlowPanel();
    includedGroupPanel.add(new SmallHeading(Util.C.headingIncludedGroups()));
    includedGroupPanel.add(addIncludedGroupBox);
    includedGroupPanel.add(includedGroups);
    includedGroupPanel.add(delIncludedGroup);
    add(includedGroupPanel);
  }
  ...
  void doAddNewIncludedGroup() {
    final String groupName = addIncludedGroupBox.getText();
    if (groupName.length() == 0) {
      return;
    }

    addIncludedGroupBox.setEnabled(false);
    Util.GROUP_SVC.addIncludedGroup(groupId, groupName,
        new GerritCallback<GroupDetail>() {
          public void onSuccess(final GroupDetail result) {
            addIncludedGroupBox.setEnabled(true);
            addIncludedGroupBox.setText("");
            if (result.groups != null && result.includedGroups != null) {
  ...
  private class IncludedGroupTable extends FancyFlexTable<AccountGroupIncludedGroup> {
    IncludedGroupTable() {
    ...
    void deleteChecked() {
      final HashSet<AccountGroupIncludedGroup.Key> keys =
          new HashSet<AccountGroupIncludedGroup.Key>();
      for (int row = 1; row < table.getRowCount(); row++) {
        final AccountGroupIncludedGroup k = getRowItem(row);
        if (k != null && ((CheckBox) table.getWidget(row, 1)).getValue()) {
          keys.add(k.getKey());
        }
      }
      if (!keys.isEmpty()) {
        Util.GROUP_SVC.deleteIncludedGroups(groupId, keys,
            new GerritCallback<VoidResult>() {
              public void onSuccess(final VoidResult result) {
                for (int row = 1; row < table.getRowCount();) {
                  final AccountGroupIncludedGroup k = getRowItem(row);
    ...
    void insertMember(final AccountGroupIncludedGroup k) {
      final int row = table.getRowCount();
      table.insertRow(row);
      applyDataRowStyle(row);
      populate(row, k);
    ...
    void display(final List<AccountGroupIncludedGroup> result) {
      while (1 < table.getRowCount())
        table.removeRow(table.getRowCount() - 1);

      for (final AccountGroupIncludedGroup k : result) {
        final int row = table.getRowCount();
        table.insertRow(row);
        applyDataRowStyle(row);
        populate(row, k);
    ...
    void populate(final int row, final AccountGroupIncludedGroup k) {
      AccountGroup.Id id = k.getIncludedGroupId();
