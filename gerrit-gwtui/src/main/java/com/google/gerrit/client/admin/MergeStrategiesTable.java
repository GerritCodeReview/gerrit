 ...
 */
public class MergeStrategiesTable extends FancyFlexTable<MergeStrategySection> {
  ...
  public MergeStrategiesTable() {
    ...
    table.setText(0, 3, Util.C.columnAutomaticMerge());
  ...
  public boolean addNew(final MergeStrategySection section) {
    ...
    if (canAdd) {
      ...
      final FlexCellFormatter fmt = table.getFlexCellFormatter();
      fmt.setVisible(row, 4, true);
      fmt.setVisible(0, 4, true);
    }
  ...
  public void populate(final int row, final MergeStrategySection ms) {
    ...
    final FlexCellFormatter fmt = table.getFlexCellFormatter();
    if (ms.getSubmitType() != null) {
      table.setText(row, 2, Util.toLongString(ms.getSubmitType()));
    } else {
      table.setText(row, 2, "---");
      fmt.setAlignment(row, 2, HasHorizontalAlignment.ALIGN_CENTER,
          HasVerticalAlignment.ALIGN_MIDDLE);
    }

    if (ms.isUseContentMerge() != null) {
      if (ms.isUseContentMerge() == MergeStrategySection.UseContentMerge.TRUE) {
        table.setWidget(row, 3, new Image(Gerrit.RESOURCES.greenCheck()));
      }
    } else {
      table.setText(row, 3, "---");
      fmt.setAlignment(row, 3, HasHorizontalAlignment.ALIGN_CENTER,
          HasVerticalAlignment.ALIGN_MIDDLE);
    }

    setDeleteAvailable(row);

    fmt.setVisible(row, 4, false);

    setRowItem(row, ms);
  }

  private void setDeleteAvailable(final int row) {
    ...
    deleteMergeStrategyButton.addClickHandler(new ClickHandler() {
      ...
      public void onClick(ClickEvent event) {
        setUndoDeleteAvailable(row);
        MergeStrategySection r = getRowItem(row);
        r.setDeleted(true);
      }
    });

    table.setWidget(row, 4, deleteMergeStrategyButton);

    final FlexCellFormatter fmt = table.getFlexCellFormatter();
    fmt.setStyleName(row, 1, Gerrit.RESOURCES.css().dataCell());
    fmt.setStyleName(row, 2, Gerrit.RESOURCES.css().dataCell());
    fmt.setStyleName(row, 3, Gerrit.RESOURCES.css().dataCell());
    fmt.setAlignment(row, 3, HasHorizontalAlignment.ALIGN_CENTER,
        HasVerticalAlignment.ALIGN_MIDDLE);
    fmt.setStyleName(row, 4, Gerrit.RESOURCES.css().iconCell());
  }

  private void setUndoDeleteAvailable(final int row) {
    final Image undoImage = new Image(AdminResources.I.undoNormal());
    final PushButton undoDeleteButton = new PushButton(undoImage);

    undoDeleteButton.setStylePrimaryName(AdminResources.I.css()
        .deleteMergeStrategyButton());
    undoDeleteButton.addClickHandler(new ClickHandler() {
    ...
    table.setWidget(row, 4, undoDeleteButton);

    final FlexCellFormatter fmt = table.getFlexCellFormatter();
    fmt.setStyleName(row, 1, AdminResources.I.css().deletedMergeStrategy());
    fmt.setStyleName(row, 2, AdminResources.I.css().deletedMergeStrategy());
    fmt.setStyleName(row, 3, AdminResources.I.css().deletedMergeStrategy());
    fmt.setStyleName(row, 4, Gerrit.RESOURCES.css().iconCell());
  }
  ...
  public void showDeleteColumn(final boolean show) {
    for (int row = 1; row < table.getRowCount(); row++) {
      ...
      if (r != null) {
        final FlexCellFormatter fmt = table.getFlexCellFormatter();
        fmt.setVisible(0, 4, show);
        fmt.setVisible(row, 4, show);
      }
    }
  }
}
