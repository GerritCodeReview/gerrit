...
public class ProjectAccessEditor extends Composite implements
  ...
  private void setWebLinks() {
    GitWebInfo gwInfo = Gerrit.info().gitWeb();
    if (gwInfo != null) {
      toAnchor(gwInfo.toFileHistory(new Branch.NameKey(value.getProjectName(),
          RefNames.REFS_CONFIG), "project.config"), gwInfo.getLinkName());
    }
    List<WebLinkInfoCommon> links = value.getFileLogLinks();
    if (links != null) {
  ...
  private Anchor toAnchor(WebLinkInfoCommon info) {
    Anchor a = new Anchor();
    a.setHref(info.url);
