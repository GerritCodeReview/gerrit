...
import com.google.gerrit.client.account.ProjectCapabilities;
...
public class ProjectBranchesScreen extends ProjectScreen {
  ...
  protected void onLoad() {
    ...
    ProjectCapabilities.all(getProjectKey(),
        new GerritCallback<ProjectCapabilities>() {
          @Override
          public void onSuccess(ProjectCapabilities result) {
            addPanel.setVisible(result
                .canPerform(ProjectCapabilities.CREATE_REF));
          }
    ...
    ProjectApi.getBranches(getProjectKey(),
        new ScreenLoadCallback<JsArray<BranchInfo>>(this) {
          ...
          public void preDisplay(final JsArray<BranchInfo> result) {
            ...
            branchTable.checkRefs(checkedRefs);
            enableForm();
          }
        });
  }
  ...
  private class BranchesTable extends FancyFlexTable<BranchInfo> {
    ...
    void checkRefs(Set<String> refs) {
      for (int row = 1; row < table.getRowCount(); row++) {
        final BranchInfo k = getRowItem(row);
