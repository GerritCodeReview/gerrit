  ...
  private class BranchesTable extends NavigationTable<BranchInfo> {
    ...
    void insert(BranchInfo info) {
      if (table.getRowCount() <= pageSize) {
        ...
        setupNavigationLink(next, match,
            ProjectBranchesScreen.this.start + pageSize);
      }
    }

    private boolean matches(BranchInfo info) {
      if (match == null || match.equals("")
          || info.getShortName().contains(match)) {
        return true;
      }
      if (match.startsWith("^")) {
        String matchRegex = match.substring(1);
        if (matchRegex.endsWith("$") && !matchRegex.endsWith("\\$")) {
          matchRegex = match.substring(0, match.length() - 1);
        }
        return RegExp.compile(matchRegex).test(info.getShortName());
      } else {
        return false;
      }
    }
