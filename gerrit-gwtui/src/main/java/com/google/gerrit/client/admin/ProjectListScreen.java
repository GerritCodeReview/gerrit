...
public class ProjectListScreen extends Screen {
  ...
  private boolean[] canBeDeletedArray;
  ...
  private class ProjectTable extends NavigationTable<ProjectData> {
    ProjectTable() {
      ...
      keysNavigation.add(new OpenKeyCommand(0, KeyCodes.KEY_ENTER, Util.C
          .projectListOpen()));
      keysNavigation.add(new SpaceSelectedCommand(0, 32, "Select Project"));
    ...
    void display(final List<ProjectData> result) {
      ...
      canBeDeletedArray = new boolean[result.size()];

      for (final ProjectData k : result) {
        final int row = table.getRowCount();
        table.insertRow(row);
        applyDataRowStyle(row);
        canBeDeletedArray[row] = k.canBeDeleted();
        populate(row, k);
      }
    }
    ...
    class SpaceSelectedCommand extends KeyCommand {
      public SpaceSelectedCommand(int mask, int key, String help) {
        super(mask, key, help);
      }
      ...
      public void onKeyPress(KeyPressEvent event) {
        ...
        final Widget radioButton = table.getWidget(i, 1);
        if (radioButton instanceof RadioButton) {
          ((RadioButton) radioButton).setFocus(true);
          ((RadioButton) radioButton).setValue(true);
          deleteButton.setEnabled(canBeDeletedArray[i]);
        }
      }
