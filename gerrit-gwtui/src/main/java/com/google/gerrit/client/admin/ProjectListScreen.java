...
public class ProjectListScreen extends AccountScreen {
    ...
    searchProjects.setSearchListener(new SearchListener() {
      ...
      public void onSearch(List<ProjectRightsBased> filterProjectsList,
        ...
        if (searchAll) {
          ...
          for (final ProjectRightsBased k : filterProjectsList) {
            if (treeNodesMap != null) {
              final TreeNode treeNode = treeNodesMap.get(k.getProjectData().getId());
              if (treeNode != null) {
        ...
        for (ProjectRightsBased filteredProjects : filterProjectsList) {
          if (selectedProjectsList.contains(filteredProjects.getProjectData()
              .getName())) {
            setProjectCheck(filteredProjects.getProjectData().getName(), true);
          }
        }
      }
  ...
  private void expandTreeNode(TreeNode parentNode) {
    ...
    if (parentNode.getParentName() != null) {
      final TreeNode parent = treeNodesMap.get(parentNode.getProjectId());
  ...
  private void loadProjects() {
    Util.PROJECT_SVC.visibleProjects(new ScreenLoadCallback<List<ProjectRightsBased>>(this) {
      @Override
  ...
  private class ProjectTable extends NavigationTable<ProjectRightsBased> {
    ...
    @Override
    protected Object getRowItemKey(final ProjectRightsBased item) {
      return item.getProjectData().getNameKey();
    }
    ...
    private String link(final ProjectRightsBased item) {
      return Dispatcher.toProjectAdmin(item.getProjectData().getNameKey(),
    ...
    void populate(final int row, final ProjectRightsBased k) {

      /* Add a checkBox column for each row, except for "All projects" row */
      if (!k.getProjectData().getNameKey().equals(Gerrit.getConfig().getWildProject())) {
        ...
        ch.addValueChangeHandler(new ValueChangeHandler<Boolean>() {
          ...
          public void onValueChange(ValueChangeEvent<Boolean> event) {
            ...
            hideOnSuccessPanel();
            setSelectedProjectsTable(checked, k.getProjectData().getName());
          }
        });
      ...
      table.setWidget(row, 2, new Hyperlink(k.getProjectData().getName(), link(k)));

      table.setText(row, 3, k.getProjectData().getDescription());

      final String parentName = k.getProjectData().getParentName();

      if (parentName != null) {
        table.setText(row, 4, parentName);
      }
    ...
    public CheckBox getCheckBoxByProject(String projectName) {
      ...
      for (int row = 1; row < projectRowCount; row++) {
        ProjectRightsBased p = getRowItem(row);
        if ((p != null) && (p.getProjectData().getName().equals(projectName))) {
