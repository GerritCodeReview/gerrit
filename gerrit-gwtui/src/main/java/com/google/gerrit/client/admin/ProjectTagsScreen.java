...
import com.google.gerrit.client.actions.ActionButtonTag;
...
public class ProjectTagsScreen extends PaginatedProjectScreen {
  ...
  public ProjectTagsScreen(final Project.NameKey toShow) {
    super(toShow);
  }
  ...
  protected void onInitUI() {
    ...
    addTag = new Button(Util.C.buttonAddBranch());
    ...
    delTag = new Button(Util.C.buttonDeleteBranch());
  ...
  private class TagsTable extends NavigationTable<TagInfo> {
    ...
    TagsTable() {
      table.setWidth("");
      table.setText(0, 2, Util.C.columnTagName());
    ...
    void populate(int row, TagInfo k) {
      ...
      if (k.actions() != null) {
        ...
        for (ActionInfo a : Natives.asList(k.actions().values())) {
          actionsPanel.add(new ActionButtonTag(getProjectKey(), k, a));
        }
      }
  ...
  private class Query {
    ...
    void showList(JsArray<TagInfo> result) {
      ...
      if (result.length() <= pageSize) {
        ...
        tagTable.displaySubset(Natives.asList(result), 0,
            result.length() - 1);
