 ...
 */
public class RefMergeStrategiesTable extends
    FancyFlexTable<MergeStrategySection> {
  private String revision;

  public RefMergeStrategiesTable() {
  ...
  /**
   * It adds a new ref merge strategy (rows).
   */
   ...
   */
  public void display(final List<MergeStrategySection> refMergeStrategies) {
    while (1 < table.getRowCount())
      table.removeRow(table.getRowCount() - 1);

    for (final MergeStrategySection r : refMergeStrategies) {
      final int row = table.getRowCount();
      table.insertRow(row);
      applyDataRowStyle(row);
      populate(row, r);
