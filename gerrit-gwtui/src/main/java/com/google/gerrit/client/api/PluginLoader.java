...
public class PluginLoader extends DialogBox {
  ...
  public static void load(List<String> plugins,
      AsyncCallback<VoidResult> callback, int loadTimeout) {
    if (plugins == null || plugins.isEmpty()) {
      callback.onSuccess(VoidResult.create());
    } else {
      self = new PluginLoader(callback, loadTimeout);
      self.load(plugins);
      self.startTimers();
      self.center();
    }
  ...
  private PluginLoader(AsyncCallback<VoidResult> cb, int loadTimeout) {
