...
public class ChangeIdParser {
  public static class Result {
    ...
    public Result(Change.Id changeId, @Nullable Project.NameKey project) {
     ...
     * token.
     *
     * @return the length of the {@code Change.Id} if no project was porsed from the token. The
  ...
  public static Result parse(String token) {
    ...
    if (cId != null) {
      return new Result(new Change.Id(cId), null);
    }
    ...
    if (delimiter > 0) {
      ...
      if (n != null) {
        return new Result(new Change.Id(n), new Project.NameKey(project));
      }
    }
