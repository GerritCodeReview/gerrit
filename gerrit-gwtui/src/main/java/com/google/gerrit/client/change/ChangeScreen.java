...
public class ChangeScreen extends Screen {
  ...
  public ChangeScreen(
    ...
    this.lc = new LocalComments(changeId, project);
  ...
  private void initIncludedInAction(ChangeInfo info) {
    if (info.status() == Status.MERGED) {
      includedInAction =
          new IncludedInAction(info.legacyId(), project, style, headerLine, includedIn);
  ...
  private void initRevisionsAction(
    ...
    patchSetsAction =
        new PatchSetsAction(
            info.legacyId(), info.projectNameKey(), revision, edit, style, headerLine, patchSets);
    ...
    if (Gerrit.isSignedIn()) {
      ...
      if (info.status().isOpen()) {
        if (isEditModeEnabled(info, rev)) {
          ...
          addFileAction =
              new AddFileAction(changeId, project, info.revision(revision), style, addFile, files);
          deleteFileAction =
              new DeleteFileAction(changeId, project, info.revision(revision), style, addFile);
          renameFileAction =
              new RenameFileAction(changeId, project, info.revision(revision), style, addFile);
        } else {
  ...
  void onPublishEdit(@SuppressWarnings("unused") ClickEvent e) {
    EditActions.publishEdit(changeId, project, publishEdit, rebaseEdit, deleteEdit);
  }

  @UiHandler("rebaseEdit")
  void onRebaseEdit(@SuppressWarnings("unused") ClickEvent e) {
    EditActions.rebaseEdit(changeId, project, publishEdit, rebaseEdit, deleteEdit);
  }

  @UiHandler("deleteEdit")
  void onDeleteEdit(@SuppressWarnings("unused") ClickEvent e) {
    if (Window.confirm(Resources.C.deleteChangeEdit())) {
      EditActions.deleteEdit(changeId, project, publishEdit, rebaseEdit, deleteEdit);
    }
  }

  @UiHandler("publish")
  void onPublish(@SuppressWarnings("unused") ClickEvent e) {
    ChangeActions.publish(changeId, project, revision, publish, deleteRevision);
  }

  @UiHandler("deleteRevision")
  void onDeleteRevision(@SuppressWarnings("unused") ClickEvent e) {
    if (Window.confirm(Resources.C.deleteDraftRevision())) {
      ChangeActions.delete(changeId, project, revision, publish, deleteRevision);
    }
  }
  ...
  public void registerKeys() {
    ...
    keysNavigation.add(
        new KeyCommand(0, 'R', Util.C.keyReloadChange()) {
          @Override
          public void onKeyPress(KeyPressEvent event) {
            Gerrit.display(PageLinks.toChange(changeId, project));
          }
        });
    keysNavigation.add(
