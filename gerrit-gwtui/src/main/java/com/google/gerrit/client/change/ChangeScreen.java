...
public class ChangeScreen extends Screen {
      ...
      if (revision.equals(revisions.get(i).name())) {
        if (0 <= i + offset && i + offset < revisions.length()) {
          Gerrit.display(PageLinks.toChange(
              ...
              revisions.get(i + offset)._number()), diffType));
          return;
        }
        return;
      }
      ...
      group.add(new AsyncCallback<NativeMap<FileInfo>>() {
        ...
        public void onSuccess(NativeMap<FileInfo> m) {
          files.set(
              ...
              style, reply, fileTableMode, diffType, edit != null);
  ...
  private void renderDiffBaseListBox(ChangeInfo info) {
    ...
    if (parents.length() > 1) {
      ...
      diffBase.addItem(Util.C.baseDiffItem(), DiffType.BASE.name());
      if (diffType != null) {
        selectedIdx =
            diffBase.getItemCount() - (diffType == DiffType.BASE ? 1 : 2);
      } else {
