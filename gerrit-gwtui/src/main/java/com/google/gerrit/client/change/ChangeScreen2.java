...
public class ChangeScreen2 extends Screen {
  ...
  protected void onLoad() {
    ...
    if (revision == null) {
      opts.add(ListChangesOption.CURRENT_REVISION);
      opts.add(ListChangesOption.CURRENT_COMMIT);
      opts.add(ListChangesOption.CURRENT_FILES);
    } else {
      // TODO Load only the selected commit.
      opts.add(ListChangesOption.ALL_COMMITS);
    }
  ...
  private void loadDiff(final RevisionInfo rev, CallbackGroup group) {
    AsyncCallback<NativeMap<FileInfo>> filesCallback =
    ...
    if (rev.has_files()) {
      FileInfo f = JavaScriptObject.createObject().cast();
      rev.files().put(Patch.COMMIT_MSG, f);
      rev.files().copyKeysIntoChildren("path");
      filesCallback.onSuccess(rev.files());
    } else {
      DiffApi.list(changeId.get(), rev.name(), group.add(filesCallback));
    }
  }
