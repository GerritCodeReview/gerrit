...
public class ChangeScreen2 extends Screen {
  ...
  private PopupPanel replyPopup;
  private Map<String, Short> currentLabels;
  ...
  void onReply(ClickEvent e) {
    ...
    final PluginSafePopupPanel popup = new PluginSafePopupPanel(true);
    popup.setStyleName(style.replyBox());
    popup.addAutoHidePartner(reply.getElement());
  ...
  private void renderChangeInfo(ChangeInfo info) {
    ...
    reply.setVisible(Gerrit.isSignedIn());
  ...
  private boolean renderLabels(ChangeInfo info) {
    ...
    quickApproveInput = null;
    currentLabels = new HashMap<String, Short>();
    ...
    for (int row = 0; row < names.size(); row++) {
      ...
      if (label.all() != null) {
        ...
        for (ApprovalInfo ai : Natives.asList(label.all())) {
          ...
          if (Gerrit.isSignedIn()
              && Gerrit.getUserAccount().getId().get() == ai._account_id()) {
            currentLabels.put(name, ai.value());
          }
        }
      ...
      initQuickApprove(info, name, label);
    }
    ...
    quickApprove.setVisible(quickApproveInput != null);
    return canSubmit;
  }

  private void initQuickApprove(ChangeInfo info, String name, LabelInfo label) {
    if (quickApproveInput == null
        && info.has_permitted_labels()
      ...
      int n = values.length();
      if (n > 0) {
        String v = values.get(n - 1);
        if (v.startsWith("+")) {
          quickApproveInput = ReviewInput.create();
          quickApproveInput.label(name, Short.parseShort(v.substring(1)));
          quickApproveText.setInnerText(name + v);
        }
      }
    }
  }
