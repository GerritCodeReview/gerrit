...
class CommitBox extends Composite {
  ...
  void set(CommentLinkProcessor commentLinkProcessor,
    ...
    addParentsRows(commit.parents(), hasNotCurrentParents(revInfo));
    addChangeIdRow(change.change_id());
  }

  private boolean hasNotCurrentParents(RevisionInfo revInfo) {
    ...
    return parentNotCurrent;
  }

  private void addParentsRows(JsArray<CommitInfo> parents, boolean parentNotCurrent) {
    FlowPanel headerPanel = new FlowPanel();
    Label parentsHeader = new Label("Parent(s)");
    parentsHeader.setStyleName(style.parentHeader());
    headerPanel.add(parentsHeader);
    if (parentNotCurrent) {
      Label rebaseMarker = new Label("\u25CF");
      rebaseMarker.setVisible(true);
      rebaseMarker.setStyleName(style.rebaseMarker());
      headerPanel.add(rebaseMarker);
    }
    for (CommitInfo commit : Natives.asList(parents)) {
      addCommitRow(headerPanel, commit, true);
