...
class DownloadBox extends Composite {
  interface Binder extends UiBinder<VerticalPanel, DownloadBox> {}
  ...
  DownloadBox(ChangeInfo change, String revision, PatchSet.Id psId) {
    ...
    commandTable.setStyleName(style.table());
    scheme.setStyleName(style.scheme());
  ...
  private void renderCommands() {
    ...
    if (scheme.getItemCount() > 0) {
      ...
      for (String commandName : Natives.keys(fetchInfo.commands())) {
        String command = fetchInfo.command(commandName);
        if (command.contains("@*:")) {
          command = command.replace("@*:",
              "@" + Window.Location.getHostName() + ":");
        }
        CopyableLabel copyLabel = new CopyableLabel(command);
  ...
  private void insertPatch() {
    ...
    spacer.setStyleName(style.spacer());
    p.add(spacer);
    p.add(patchZip);
  ...
  private void insertCommand(String commandName, Widget w) {
    ...
    commandTable.getCellFormatter().addStyleName(row, 0, style.column());
    if (commandName != null) {
