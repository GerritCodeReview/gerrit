...
public class FileTable extends FlowPanel {
  ...
  private String url(FileInfo info) {
    return info.binary()
        ? Dispatcher.toUnified(base, curr, project, info.path())
        : mode == Mode.REVIEW
            ? Dispatcher.toPatch(base, curr, project, info.path())
            : Dispatcher.toEditScreen(curr, project, info.path());
  }
      ...
      ChangeEditApi.delete(
          ...
          new AsyncCallback<VoidResult>() {
            ...
            public void onSuccess(VoidResult result) {
              Gerrit.display(PageLinks.toChangeInEditMode(curr.getParentKey(), project));
            }
