...
class Labels extends Grid {
  ...
  private static void onRemoveReviewer(NativeEvent event) {
    ...
    if (user != null) {
      ...
      ChangeApi.reviewer(changeId.get(), Project.NameKey.asStringOrNull(screen.getProject()), user)
          .delete(
              new GerritCallback<JavaScriptObject>() {
                @Override
                public void onSuccess(JavaScriptObject result) {
                  if (screen.isCurrentView()) {
                    Gerrit.display(PageLinks.toChange(changeId, screen.getProject()));
                  }
                }
              });
    }
  ...
  private static void onRemoveVote(NativeEvent event) {
    ...
    if (user != null && vote != null) {
      ...
      final Change.Id changeId = screen.getPatchSetId().getParentKey();
      ChangeApi.vote(
              changeId.get(), Project.NameKey.asStringOrNull(screen.getProject()), user, vote)
          .delete(
              new GerritCallback<JavaScriptObject>() {
                @Override
                public void onSuccess(JavaScriptObject result) {
