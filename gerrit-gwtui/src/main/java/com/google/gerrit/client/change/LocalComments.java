...
public class LocalComments {
  ...
  private static class InlineComment {
    ...
    protected InlineComment(PatchSet.Id psId, CommentInfo commentInfo) {
      this.psId = psId;
  ...
  public boolean hasReplyComment() {
    if (storage.getKeys().contains(getReplyCommentName())) {
      return true;
    }
    return false;
  }
  ...
  private static boolean isInlineComment(String key) {
    if (key.startsWith("patchCommentEdit-") || key.startsWith("patchReply-")
