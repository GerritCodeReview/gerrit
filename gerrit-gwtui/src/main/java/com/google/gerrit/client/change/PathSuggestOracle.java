...
class PathSuggestOracle extends HighlightSuggestOracle {
  ...
  PathSuggestOracle(Change.Id changeId, @Nullable Project.NameKey project, RevisionInfo revision) {
  ...
  protected void onRequestSuggestions(final Request req, final Callback cb) {
    RestApi api =
        ChangeApi.revision(changeId.get(), Project.NameKey.asStringOrNull(project), revision.name())
            .view("files");
