...
public class ProjectChangeId {
  ...
  public static ProjectChangeId create(String token) {
    String mutatableToken = token;
    // Try parsing /c/project/+/numericChangeId where token is project/+/numericChangeId
    int delimiter = mutatableToken.indexOf(PageLinks.PROJECT_CHANGE_DELIMITER);
    Project.NameKey project = null;
    if (delimiter > 0) {
      project = new Project.NameKey(token.substring(0, delimiter));
      mutatableToken =
          mutatableToken.substring(delimiter + PageLinks.PROJECT_CHANGE_DELIMITER.length());
    }

    // Try parsing /c/numericChangeId where token is numericChangeId
    int s = mutatableToken.indexOf('/');
    if (s > 0) {
      mutatableToken = mutatableToken.substring(0, s);
    }
    // Special case: project/+/1233,edit/
    s = mutatableToken.indexOf(",edit");
    if (s > 0) {
      mutatableToken = mutatableToken.substring(0, s);
    }
    Integer cId = tryParse(mutatableToken);
    if (cId != null) {
