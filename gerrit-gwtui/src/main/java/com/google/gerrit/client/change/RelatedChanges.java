...
class RelatedChanges extends TabPanel {
  ...
  void set(final ChangeInfo info, final String revision) {
    ...
    if (info.status().isOpen()) {
      setForOpenChange(group, info, revision);
    }
    ...
    ChangeList.query(cherryPicksQuery.toString(),
        ...
        group.add(new TabChangeListCallback(Tab.CHERRY_PICKS, info.project(), revision)));
    ...
    if (info.topic() != null && !"".equals(info.topic())) {
      ...
      ChangeList.query(topicQuery.toString(),
          EnumSet.of(ListChangesOption.CURRENT_REVISION, ListChangesOption.CURRENT_COMMIT),
          group.add(new TabChangeListCallback(Tab.SAME_TOPIC, info.project(), revision)));
    }

    group.addFinal(new AsyncCallback<Void>() {
      @Override
  ...
  private void setForOpenChange(
      CallbackGroup group, final ChangeInfo info, final String revision) {
    ...
    if (info.mergeable()) {
      ...
      ChangeList.query(conflictsQuery.toString(),
          EnumSet.of(ListChangesOption.CURRENT_REVISION, ListChangesOption.CURRENT_COMMIT),
          group.add(new TabChangeListCallback(Tab.CONFLICTING_CHANGES, info.project(), revision)));
    }
  }
  ...
  private abstract class TabCallback<T> implements AsyncCallback<T> {
    ...
    public final void onSuccess(T result) {
      ...
      if (changes.length() > 0) {
        ...
        setTabEnabled(tabInfo, true);
      }
    }

    @Override
    public final void onFailure(Throwable err) {
