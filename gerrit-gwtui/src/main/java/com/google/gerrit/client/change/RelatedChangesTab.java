...
class RelatedChangesTab implements IsWidget {
  ...
  private final class DisplayCommand implements RepeatingCommand {
    ...
    private void computeMerged() {
      if (!merged.contains(revision)) {
        while (mergedPos > 0) {
          CommitInfo c = changes.get(mergedPos).commit();
          JsArrayString b = changes.get(mergedPos).branches();
          //check if related change is also merged in the branch
          //of current change under review
          boolean sameBranchMerged = false;
          for (int i = 0; i < b.length(); i++) {
            if (b.get(i).equals(changes.get(0).branch()))
              sameBranchMerged = true;
          }
          if (b.get(0) != null && (sameBranchMerged == false)) {
            merged.add(c.commit());
          }
          mergedPos--;
        }
  ...
  private class RowSafeHtml implements SafeHtml {
    ...
    RowSafeHtml(ChangeAndCommit info, boolean notConnected, boolean isMerged) {
      this.info = info;
