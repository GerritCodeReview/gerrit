...
class RevisionBox extends Composite {
  interface Binder extends UiBinder<HTMLPanel, RevisionBox> {}
  ...
  private static final native void init(String o) /*-{
    $wnd[o] = $entry(function(e,i) {
      return @com.google.gerrit.client.change.RevisionBox::onOpen(Lcom/google/gwt/dom/client/NativeEvent;I)(e,i);
    });
  }-*/;

  private static boolean onOpen(NativeEvent e, int idx) {
    if (link.handleAsClick(e.<Event> cast())) {
      RevisionBox t = getRevisionBox(e);
      if (t != null) {
        t.onOpenRow(idx);
        e.preventDefault();
        return false;
  ...
  private static RevisionBox getRevisionBox(NativeEvent event) {
    com.google.gwt.user.client.Element e = event.getEventTarget().cast();
    for (e = DOM.getParent(e); e != null; e = DOM.getParent(e)) {
      EventListener l = DOM.getEventListener(e);
      if (l instanceof RevisionBox) {
        return (RevisionBox) l;
      }
    }
    return null;
  }
  ...
  RevisionBox(Change.Id changeId, String revision) {
  ...
  private void revision(SafeHtmlBuilder sb, int index, RevisionInfo r) {
    ...
    String an = shortName(c.author());
    String cn = shortName(c.committer());
    sb.openTd();
    sb.append(an);
  ...
  private static String shortName(GitPerson p) {
    if (p != null) {
      String n = p.name();
      int s = n.indexOf(' ');
