...
public class ChangeEditApi {
  ...
  public static void get(
    ...
    RestApi api;
    String projectStr = project == null ? null : project.get();
    if (id.get() != 0) {
      // Read from a published revision, when change edit doesn't
      // exist for the caller, or is not currently active.
      api = ChangeApi.revision(id, projectStr).view("files").id(path).view("content");
    } else if (Patch.COMMIT_MSG.equals(path)) {
      api = editMessage(id.getParentKey().get(), projectStr).addParameter("base", base);
    } else {
      api = editFile(id.getParentKey().get(), projectStr, path).addParameter("base", base);
    }
    api.get(cb);
  }
