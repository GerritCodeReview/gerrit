...
public class ChangeInfoBlock extends Composite {
  ...
  public Widget branch(final Change chg) {
    ...
    edit.addClickHandler(new  ClickHandler() {
      ...
      public void onClick(final ClickEvent event) {
        new MoveChangeDialog(chg, new AsyncCallback<ChangeDetail>() {
            @Override
            public void onSuccess(ChangeDetail result) {
              ChangeInfoBlock.this.onChangeUpdate(result);
            }
  ...
  private class MoveChangeDialog extends CommentedChangeActionDialog {
    ListBox destinations;
    Change change;

    MoveChangeDialog(Change chg, AsyncCallback<ChangeDetail> cb) {
      super(chg.currPatchSetId(), cb,
              Util.C.alterBranchTitle(), Util.C.headingAlterBranchMessage(),
              Util.C.buttonAlterBranchSend(), Util.C.buttonAlterBranchCancel(),
              Gerrit.RESOURCES.css().revertChangeDialog(), Gerrit.RESOURCES.css().revertMessage(),
  //            Util.M.revertChangeDefaultMessage(chg.getSubject(), chgSet().getRevision().get())) {
              "");
    ...
    protected void onLoad() {
      super.onLoad();
      Util.PROJECT_SVC.listBranches(change.getProject(),
            ...
            if (!result.getNoRepository()) {
              for (Branch dest : result.getBranches()) {
                ...
                if (!ref.equals(dest.getName()) &&
                  ...
                  // HEAD and refs/meta/config skipped
/*                  if (!dest.equals(currentBranch) &&
                    !dest.equals("refs/meta/config") && !dest.equals("HEAD")) {*/
