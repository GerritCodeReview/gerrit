...
public class ChangeTable2 extends NavigationTable<ChangeInfo> {
  ...
  private void populateChangeRow(final int row, final ChangeInfo c,
    ...
    table.setWidget(row, C_ID,
        new TableChangeLink(String.valueOf(c.legacy_id()), c));
    ...
    table
        .setWidget(
            row,
            C_BRANCH,
            new BranchLink(c.project_name_key(), c.status(), c.branch(), c
                .topic()));
    if (Gerrit.isSignedIn()
        && Gerrit.getUserAccount().getGeneralPreferences()
    ...
    for (int idx = 0; idx < labelNames.size(); idx++, col++) {
      ...
      if (label.rejected() != null) {
        user =
            getReviewCategoryDisplayInfo(reviewCategoryStrategy,
                label.rejected());
