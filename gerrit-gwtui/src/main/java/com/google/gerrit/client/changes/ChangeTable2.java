...
public class ChangeTable2 extends NavigationTable<ChangeInfo> {
  ...
  private void populateChangeRow(final int row, final ChangeInfo c) {
    if (Gerrit.isSignedIn()) {
      final Image img = new Image(c.starred()
          ? Gerrit.RESOURCES.starFilled()
          : Gerrit.RESOURCES.starOpen());
      ...
      table.setWidget(row, C_STAR, img);
    }
    ...
    table.setWidget(row, C_PROJECT,
        new ProjectLink(c.project_name_key(), c.status()));
    table.setWidget(row, C_BRANCH, new BranchLink(
        c.project_name_key(),
        c.status(), c.branch(), c.topic()));
    table.setText(row, C_LAST_UPDATE, shortFormat(c.updated()));

    boolean displayName = Gerrit.isSignedIn()
        && Gerrit.getUserAccount()
            .getGeneralPreferences()
            .isDisplayPersonNameInReviewCategory();
  ...
  private void toggleStar(final ChangeInfo c, final Image img) {
    final boolean have = c.starred();
    final boolean want = !have;

    img.setResource(want ? Gerrit.RESOURCES.starFilled() : Gerrit.RESOURCES.starOpen());

    ToggleStarRequest req = new ToggleStarRequest();
    req.toggle(c.legacy_id(), want);
    Util.LIST_SVC.toggleStars(req, new GerritCallback<VoidResult>() {
      ...
      public void onFailure(Throwable caught) {
        ...
        img.setResource(have ? Gerrit.RESOURCES.starFilled() : Gerrit.RESOURCES.starOpen());
      }
    });
  }
  ...
  public class StarKeyCommand extends NeedsSignInKeyCommand {
    ...
    public void onKeyPress(final KeyPressEvent event) {
      ...
      if (c != null && Gerrit.isSignedIn()) {
        toggleStar(c, (Image) table.getWidget(row, C_STAR));
      }
    }
  }
