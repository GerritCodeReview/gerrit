...
public class ChangeTable2 extends NavigationTable<ChangeInfo> {
  ...
  private static String getAbbreviation(String name, String token) {
    ...
    if (name != null) {
      ...
      RegExp re = RegExp.compile("[A-Za-z]+");
      if (!re.test(nameList.get(0))) {
        // setup abbreviation for non-latin based names
        abbrev.append(nameList.get(0).toUpperCase());
        if (nameList.size() > 1) {
          abbrev.append(" ("+
                        nameList.get(nameList.size()-1).substring(0, 1).toUpperCase()+
                        ")");
        }
      } else {
