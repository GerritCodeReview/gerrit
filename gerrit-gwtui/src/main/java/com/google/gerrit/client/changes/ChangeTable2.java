...
public class ChangeTable2 extends NavigationTable<ChangeInfo> {
  ...
  public ChangeTable2(boolean showApprovals) {
  ...
  protected void onStarClick(final int row) {
    ...
    if (c != null && Gerrit.isSignedIn()) {
       ChangeCache.get(c.legacy_id()).getStarCache().toggleStar();
    }
  }
  ...
  private void populateChangeRow(final int row, final ChangeInfo c) {
    ChangeCache cache = ChangeCache.get(c.legacy_id());
    // TODO(sop): cache.getChangeInfoCache().set(c);

    final String idstr = c.id_abbreviated();
    ...
    if (c.status() != null && c.status() != Change.Status.NEW) {
      s += " (" + c.status().name() + ")";
    }
    if (! c.isLatest()) {
      s += " [OUTDATED]";
    ...
    return new GerritCallback<ApprovalSummarySet>() {
      ...
      public void onSuccess(final ApprovalSummarySet as) {
        ...
        for (int row = dataBegin; row < dataBegin + rows; row++) {
          final ChangeInfo c = getRowItem(row);
          if (ids.containsKey(c.legacy_id())) {
            displayApprovals(row, ids.get(c.legacy_id()), aic, highlightUnreviewed);
          }
        }
      }
    };
