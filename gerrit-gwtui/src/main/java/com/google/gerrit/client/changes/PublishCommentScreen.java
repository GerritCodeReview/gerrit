...
import com.google.gerrit.client.changes.ChangeInfo.DetailedLabelInfo;
...
public class PublishCommentScreen extends AccountScreen implements
  ...
  private void initApprovals(final PatchSetPublishDetail r, final Panel body) {
    for (String labelName : change.detailed_labels()) {
      initLabel(r, labelName, body);
    }
  }

  private void initLabel(PatchSetPublishDetail r, String labelName, Panel body) {
    DetailedLabelInfo label = change.detailed_label(labelName);
    List<String> values = new ArrayList<String>(label.permitted_values());
    if (values.isEmpty()) {
      return;
    }
    ...
    Short prior = null;
    for (ApprovalInfo app : label.approvals()) {
      if (app.email().equals(Gerrit.getUserAccount().getPreferredEmail())) {
        prior = app.value();
        break;
      }
    }
  ...
  private static class ValueRadioButton extends RadioButton {
    final DetailedLabelInfo label;
    final String value;

    ValueRadioButton(DetailedLabelInfo label, String value) {
      super(label.name());
      this.label = label;
      this.value = value;
    }
