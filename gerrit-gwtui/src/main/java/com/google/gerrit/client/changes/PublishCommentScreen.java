...
public class PublishCommentScreen extends AccountScreen implements
  ...
  private void initLabel(PatchSetPublishDetail r, String labelName, Panel body) {
    ...
    for (String value : values) {
      ...
      if (lastState != null && patchSetId.equals(lastState.patchSetId)
          && lastState.approvals.containsKey(label.id())) {
  ...
  private static class ValueRadioButton extends RadioButton {
    ...
    ValueRadioButton(String name, LabelInfo info, String v) {
      super(name);
      labelName = name;
      value = new ApprovalCategoryValue(new ApprovalCategoryValue.Id(
          new ApprovalCategory.Id(info.id()), parseLabelValue(v)),
          info.value(v));
    }
  }
  ...
  private static class SavedState {
    ...
    final Map<ApprovalCategoryValue.Id, Short> approvals;

    SavedState(final PublishCommentScreen p) {
      patchSetId = p.patchSetId;
      message = p.message.getText();
      approvals = new HashMap<ApprovalCategoryValue.Id, Short>();
      for (final ValueRadioButton b : p.approvalButtons) {
        if (b.getValue()) {
          approvals.put(b.value.getId(), b.value.getValue());
        }
      }
    }
  }
