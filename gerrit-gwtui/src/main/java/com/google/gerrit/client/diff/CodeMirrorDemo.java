...
public class CodeMirrorDemo extends Screen {
  ...
  private void render(DiffInfo diff) {
    ...
    for (int i = 0; i < regions.length(); i++) {
      ...
      if (current.ab() != null) {
        ...
        lineB = colorLine(delta, lineB, true);
      } else if (current.a() != null && current.b() == null) {
        int delta = current.a().length();
        padLines(cmB, delta, lineB - 1);
        lineA = colorLine(delta, lineA, false);
      } else { // TODO: Implement intraline
        lineA = colorLine(current.a().length(), lineA, false);
        lineB = colorLine(current.b().length(), lineB, true);
      }
    }
  }
  ...
  private int colorLine(int delta, int line, boolean isInsert) {
