...
public class CodeMirrorDemo extends Screen {
  ...
  private void render(DiffInfo diff) {
    ...
    Configuration insertOpt = Configuration.create()
        .set("className", "insert").set("readOnly", "true");
    Configuration deleteOpt = Configuration.create()
        .set("className", "delete").set("readOnly", "true");
    int lineA = 0, lineB = 0;
    for (int i = 0; i < regions.length(); i++) {
      ...
      if (current.ab() != null) {
        ...
        for (int j = 0; j < delta; j++) {
          ...
          LineCharacter from = LineCharacter.create(lineB, 0).setLine(lineB)
              .setCh(0);
        ...
        for (int j = 0; j < delta; j++) {
          ...
          LineCharacter from = LineCharacter.create(lineA, 0).setLine(lineA)
              .setCh(0);
