...
public class CodeMirrorDemo extends Screen {
  ...
  private static final int DFLT_CONTEXT = 10;
  ...
  protected void onLoad() {
    ...
    DiffApi.diff(revision, path)
      .base(base)
      .wholeFile() // TODO: Use context(num) instead
      .intraline()
  ...
  private SkipBar renderSkipHelper(CodeMirror cm, SkippedLine skip) {
    ...
    TextMarker marker = cm.markText(CodeMirror.Pos(start),
        CodeMirror.Pos(start + size), getReplacedWithConfig(bar.getElement()));
  ...
  Configuration getReplacedWithConfig(Element ele) {
    return Configuration.create().set("replacedWith", ele);
  }
