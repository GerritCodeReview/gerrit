...
class CommentBoxHeader extends Composite {
  ...
  void setNameAndDate(AccountInfo author, Timestamp when, boolean isDraft) {
    String dateText = FormatUtil.shortFormatDayTime(when);
    if (isDraft) {
      setAuthorNameText(author, "(Draft)");
   ...
   private void setAuthorNameText(AccountInfo author, String text) {
