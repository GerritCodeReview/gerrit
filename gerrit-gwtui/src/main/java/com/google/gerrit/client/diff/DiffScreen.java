...
abstract class DiffScreen extends Screen {
  ...
  DiffScreen(
    ...
    header = new Header(keysNavigation, base, revision, project, path, diffScreenType, prefs);
  ...
  protected void onLoad() {
    ...
    final CommentsCollections comments = new CommentsCollections(base, revision, project, path);
  ...
  public void registerKeys() {
    ...
    keysNavigation.add(new UpToChangeCommand(revision, project, 0, 'u'));
    keysNavigation.add(
  ...
  private Runnable openEditScreen(CodeMirror cm) {
    return () -> {
      ...
      String token = Dispatcher.toEditScreen(revision, project, path, line);
      if (!Gerrit.isSignedIn()) {
      ...
      group.addListener(
          new GerritCallback<Void>() {
            ...
            public void onSuccess(Void result) {
              ...
              Gerrit.display(
                  PageLinks.toChange(changeId, project, base.asString(), rev),
