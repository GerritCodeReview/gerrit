...
abstract class DiffScreen extends Screen {
  ...
  protected void onLoad() {
    ...
    call.get(group2.add(new AsyncCallback<ChangeInfo>() {
      ...
      public void onSuccess(ChangeInfo info) {
        ...
        getDiffTable().set(prefs, list, diff, edit != null, currentPatchSet,
  ...
  void registerCmEvents(final CodeMirror cm) {
    ...
    if (revision.get() != 0) {
      ...
      keyMap.on("C", getCommentManager().insertNewDraft(cm));
    }
    cm.addKeyMap(keyMap);
  }
  ...
  public void registerKeys() {
    ...
    keysAction.add(new KeyCommand(
      ...
      @Override
      public void onKeyPress(KeyPressEvent event) {
        if (getDiffTable() instanceof SideBySideTable) {
          ((SideBySideTable) getDiffTable()).toggleA().run();
        }
      }
    });
  ...
  void setShowIntraline(boolean b) {
    if (b && getIntraLineStatus() == DiffInfo.IntraLineStatus.OFF) {
      ...
      getDiffTable().removeStyleName(DiffTable.style.noIntraline());
    } else {
      getDiffTable().addStyleName(DiffTable.style.noIntraline());
    }
  }
  ...
  void setThemeStyles(boolean d) {
    if (d) {
      getDiffTable().addStyleName(DiffTable.style.dark());
    } else {
      getDiffTable().removeStyleName(DiffTable.style.dark());
    }
  }
      ...
      public void handle(CodeMirror instance, final int line, String gutter,
        ...
        if (clickEvent.getButton() == NativeEvent.BUTTON_LEFT
          ...
          Scheduler.get().scheduleDeferred(new ScheduledCommand() {
            @Override
            public void execute() {
              getCommentManager().newDraft(cm, line + 1);
            }
          });
        }
      }
  ...
  abstract LineOnOtherInfo lineOnOther(DisplaySide side, int line);
