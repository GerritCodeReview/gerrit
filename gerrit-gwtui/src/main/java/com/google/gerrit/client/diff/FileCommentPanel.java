...
class FileCommentPanel extends Composite {
  ...
  FileCommentPanel(CommentManager manager, DiffTable table, DisplaySide side) {
  ...
  void createOrEditFileComment() {
    if (!Gerrit.isSignedIn()) {
      Gerrit.doSignIn(manager.getSideBySide2().getToken());
      return;
    }

    if (boxes.isEmpty()) {
      addFileComment(manager.newFileDraft(side));
    } else {
