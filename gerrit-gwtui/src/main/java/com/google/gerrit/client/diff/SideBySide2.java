...
public class SideBySide2 extends Screen {
      ...
      .get(cmGroup.addFinal(new GerritCallback<DiffInfo>() {
        ...
        public void onSuccess(DiffInfo diffInfo) {
          ...
          if (prefs.syntaxHighlighting()) {
            ...
            if (fileSize.compareTo(FileSize.REASONABLE) > 0) {
  ...
  private void display(final CommentsCollections comments) {
    ...
    if (prefs.syntaxHighlighting() && fileSize.compareTo(FileSize.REASONABLE) > 0) {
  ...
  private CodeMirror newCM(
      ...
      DisplaySide side,
      Element parent) {
    String mode = fileSize == FileSize.REASONABLE
        ? getContentType(meta)
        : null;
