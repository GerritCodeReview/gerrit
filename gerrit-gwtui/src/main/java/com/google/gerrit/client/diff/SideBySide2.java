...
public class SideBySide2 extends Screen {
  ...
  public void onShowView() {
    ...
    if (startSide == null && diff.meta_b() != null) {
      DiffChunkInfo d = coloringManager.getFirstDiffChunk();
      if (d != null) {
        startSide = d.getSide();
  ...
  private void registerCmEvents(final CodeMirror cm) {
    ...
    cm.addKeyMap(KeyMap.create()
        ...
        .on("P", coloringManager.diffChunkNav(cm, Direction.PREV))
  ...
  private void display(final CommentsCollections comments) {
    ...
    coloringManager = new ColoringManager(this, cmA, cmB, diffTable.sidePanel);
    ...
    new ScrollSynchronizer(diffTable, cmA, cmB, coloringManager.getLineMapper());
  ...
  DisplaySide getSideFromCm(CodeMirror cm) {
    return cm == cmA ? DisplaySide.A : DisplaySide.B;
  }

  LineOnOtherInfo lineOnOther(DisplaySide side, int line) {
    return coloringManager.getLineMapper().lineOnOther(side, line);
  }
        ...
        defer(new Runnable() {
          ...
          public void run() {
            ...
            LineOnOtherInfo info =
                lineOnOther(getSideFromCm(cm), cm.getLineNumber(handle));
            if (info.isAligned()) {
    ...
    return new Runnable() {
      ...
      public void run() {
        if (cm.hasVimSearchHighlight()) {
          ...
          coloringManager.diffChunkNav(cm, Direction.NEXT).run();
        }
      }
    };
  }
  ...
  private RenderLineHandler resizeLinePadding(final DisplaySide side) {
    return new RenderLineHandler() {
      ...
      public void handle(CodeMirror cm, LineHandle lh, Element e) {
        ...
        coloringManager.resizePadding(cm, lh, side);
      }
    };
  }
