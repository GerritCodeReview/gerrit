...
class SideBySideCommentManager extends CommentManager {
  ...
  CommentGroup group(DisplaySide side, int line) {
    SideBySideCommentGroup existing =
        (SideBySideCommentGroup) map(side).get(line);
    if (existing != null) {
      return existing;
    }
    ...
    existing = (SideBySideCommentGroup) map(side.otherSide()).get(otherLine);
