...
class SkipBar extends Composite {
  ...
  SkipBar(CodeMirrorDemo diffView, CodeMirror cm) {
  ...
  private void expandAll() {
    diffView.getHiddenSkipMap().remove(
  ...
  private void expandBefore() {
    ...
    setWidget(newWidget);
    updateSkipNum();
    diffView.getHiddenSkipMap().put(cm.getLineHandle(end), numSkipLines);
  }
  ...
  private void expandAfter() {
    ...
    marker = cm.markText(CodeMirror.pos(fromTo.getFrom().getLine()),
        ...
        COLLAPSED);
    updateSkipNum();
    Map<LineHandle, Integer> hiddenSkipMap = diffView.getHiddenSkipMap();
