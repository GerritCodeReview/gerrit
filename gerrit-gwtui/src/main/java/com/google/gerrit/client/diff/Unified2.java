...
public class Unified2 extends DiffScreen {
  ...
  private void renderSkipHelper(SkippedLine skip) {
    ...
    if (markStart == -1) {
      ...
      cm.addLineClass(0, LineClassWhere.WRAP, Resources.I.diffTableStyle().hideNumber());
    } else {
  ...
  private void clearActiveLine() {
    if (cm.hasActiveLine()) {
      ...
      cm.removeLineClass(activeLine,
          LineClassWhere.BACKGROUND, Resources.I.diffTableStyle().activeLineBg());
      ...
      public void run() {
        ...
        Scheduler.get().scheduleDeferred(new ScheduledCommand() {
          ...
          public void execute() {
            ...
            cm.addLineClass(
                handle, LineClassWhere.WRAP, Resources.I.diffTableStyle().activeLine());
            cm.addLineClass(
                handle, LineClassWhere.BACKGROUND, Resources.I.diffTableStyle().activeLineBg());
          }
        });
      }
    };
  ...
  private LinePair getLinePairFromCmLine(int cmLine) {
    ...
    if (res >= 0) {
      ...
      if (res > 0) {
        DiffChunkInfo info = diffChunks.get(res - 1);
        int delta = info.cmLine + info.end - info.start + 1 - cmLine;
        if (delta > 0) {
          return info.side == DisplaySide.A
