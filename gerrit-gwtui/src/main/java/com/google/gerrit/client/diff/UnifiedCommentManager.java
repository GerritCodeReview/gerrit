...
class UnifiedCommentManager extends CommentManager {
  ...
  private void renderPublished(DisplaySide forSide, JsArray<CommentInfo> in) {
    for (CommentInfo info : Natives.asList(in)) {
      ...
      if (side != null) {
        UnifiedCommentGroup group = group(side, info.line());
   ...
   */
  void insertNewDraft(DisplaySide side, int cmLine) {
    if (cmLine == 0) {
      getDiffScreen().getSkipManager().ensureFirstLineIsVisible();
    }

    CommentGroup group = group(side, cmLine);
  ...
  DraftBox addDraftBox(DisplaySide side, CommentInfo info) {
    int cmLine = ((Unified2) getDiffScreen()).getCmLine(side, info.line());
    UnifiedCommentGroup group = group(side, cmLine);
  ...
  List<SkippedLine> splitSkips(int context, List<SkippedLine> skips) {
    ...
    for (int boxLine : groupMap.tailMap(1).keySet()) {
      ...
      for (SkippedLine skip : skips) {
        ...
        int startLine = ((Unified2) getDiffScreen()).getCmLineFromLinePair(
            new LinePair(skip.getStartA(), skip.getStartB()));
  ...
  private void newDraft(CodeMirror cm) {
    int cmLine = cm.getLineNumber(cm.getActiveLine()) + 1;
    LinePair pair = ((Unified2) getDiffScreen()).getLinePairFromCmLine(cmLine);
    DisplaySide side = pair.getLineA() == null ? DisplaySide.B : DisplaySide.A;
  ...
  private UnifiedCommentGroup group(DisplaySide side, int cmLine) {
    UnifiedCommentGroup w = groupMap.get(cmLine);
    if (w != null) {
      return w;
    }

    UnifiedCommentGroup group = newGroup(side, cmLine);
  ...
  private UnifiedCommentGroup newGroup(DisplaySide side, int line) {
    return new UnifiedCommentGroup(this, ((Unified2) getDiffScreen()).getCm(), side, line);
  }
}
