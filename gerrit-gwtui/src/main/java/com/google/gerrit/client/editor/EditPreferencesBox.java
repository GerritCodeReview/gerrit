...
class EditPreferencesBox extends Composite {
  ...
  void onKeyMap(@SuppressWarnings("unused") ChangeEvent e) {
    prefs.keyMap(KeyMap.valueOf(keyMap.getValue(keyMap.getSelectedIndex())));
    String t = prefs.keyMap().name().toLowerCase();
    view.getEditor().setOption("keyMap", t);
  }
  ...
  private void setKeyMap(KeyMap v) {
    String name = v != null ? v.name() : KeyMap.DEFAULT.name();
  ...
  private void initKeyMap() {
    keyMap.addItem(
        KeyMap.DEFAULT.name().toLowerCase(),
        KeyMap.DEFAULT.name());
    keyMap.addItem(
        KeyMap.EMACS.name().toLowerCase(),
        KeyMap.EMACS.name());
    keyMap.addItem(
        KeyMap.VIM.name().toLowerCase(),
        KeyMap.VIM.name());
  }
