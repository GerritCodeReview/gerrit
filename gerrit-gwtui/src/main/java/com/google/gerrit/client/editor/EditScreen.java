...
public class EditScreen extends Screen {
          ...
          new AsyncCallback<EditInfo>() {
            ...
            public void onSuccess(final EditInfo result) {
              DiffApi.diff(revision, path)
                .base(PatchSet.Id.fromRef(result.basePatchSet()))
                .get(group1.addFinal(new AsyncCallback<DiffInfo>() {
                  @Override
                  public void onSuccess(DiffInfo diffInfo) {
                    baseContent = diffInfo.metaA() == null ? "" : diffInfo.textA();
                  }
    ...
    ChangeEditApi.get(revision, path,
        group2.add(new HttpCallback<NativeString>() {
          ...
          public void onSuccess(HttpResponse<NativeString> fc) {
            content = fc;
            if (revision.get() > 0 && fc != null && fc.getResult() != null) {
              baseContent = fc.getResult().asString();
            }
  ...
  private void initEditor(HttpResponse<NativeString> file) {
    ...
    if (file != null && file.getResult() != null) {
      editContent = file.getResult().asString();
      if (prefs.syntaxHighlighting()) {
    ...
    cmEdit = mv.editor();
    cmOrig.setValue(baseContent == null ? "" : baseContent);
