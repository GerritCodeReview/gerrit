...
public class PatchScriptSettingsPanel2 extends Composite {
  private static Binder uiBinder = GWT.create(Binder.class);

  interface Binder extends UiBinder<Widget, PatchScriptSettingsPanel2> {
  }
  ...
  public PatchScriptSettingsPanel2(ListenableAccountDiffPreference prefs) {
  ...
  public void setEnableSmallFileFeatures(final boolean on) {
    ...
    if (element.getPropertyInt("value") == AccountDiffPreference.WHOLE_FILE_CONTEXT) {
      if (enableSmallFileFeatures) {
        ...
        if (AccountDiffPreference.CONTEXT_CHOICES[context.getSelectedIndex()] ==
  ...
  private void persistDiffPreferences() {
    setEnabled(false);
    DiffPreferenceInput in = DiffPreferenceInput.create();
    final AccountDiffPreference p = listenablePrefs.get();
    in.fromAccountDiffPreference(p);
  ...
  private static class DiffPreferenceInput extends JavaScriptObject {
    static DiffPreferenceInput create() {
      return createObject().cast();
    }

    final void fromAccountDiffPreference(AccountDiffPreference p) {
      diff_preference0(p.getContext(),
      p.isExpandAllComments(),
      p.getIgnoreWhitespace().name(),
      p.isIntralineDifference(),
      p.getLineLength(),
      p.isManualReview(),
      p.isRetainHeader(),
      p.isShowLineEndings(),
      p.isShowTabs(),
      p.isShowWhitespaceErrors(),
      p.isSkipDeleted(),
      p.isSkipUncommented(),
    ...
    private final native void diff_preference0(short context,
