...
public class PatchSetSelectBox extends Composite {
  ...
  public void display(final PatchSetDetail detail, final PatchScript script, Patch.Key key,
    ...
    if (detail.getInfo().getParents().size() > 1) {
      baseLink = getLink(PatchUtil.C.patchBaseAutoMerge(), null);
    } else {
      baseLink = getLink(PatchUtil.C.patchBase(), null);
    }
    ...
    for (Patch patch : script.getHistory()) {
      ...
      Anchor anchor = getLink(Integer.toString(psId.get()), psId);
      links.add(anchor);
  ...
  private Anchor getLink(String label, final PatchSet.Id id) {
