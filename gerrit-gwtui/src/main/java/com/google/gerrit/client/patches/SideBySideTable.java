...
public class SideBySideTable extends AbstractPatchContentTable {
  ...
  SideBySideTable(boolean preview) {
    super(preview);
  }
  ...
  protected void render(final PatchScript script, final PatchSetDetail detail) {
    ...
    if (!isDisplayBinary) {
      ...
      if (hasDifferences(script)) {
        if (renderPreview()) {
          appendPreviewLine(script, nc, detail.getProject());
        } else {
          int lastA = 0;
          int lastB = 0;
  ...
  private void appendPreviewLine(PatchScript script,
      SafeHtmlBuilder m, Project.NameKey project) {
    m.openTr();
    m.setAttribute("valign", "center");
    m.setAttribute("align", "center");
  ...
  private SafeHtml createPreview(String url, Side side) {
    ...
    autoResizeScript.append("}");
    m.setAttribute("onLoad", autoResizeScript.toString());

    m.closeElement("iframe");
    return m.toSafeHtml();
  }
