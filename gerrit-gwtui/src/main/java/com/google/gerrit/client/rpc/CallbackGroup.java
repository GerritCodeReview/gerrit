 ...
 * processing it.
 * <p>
 * In the event of an unchecked exception during {@code onSuccess} or {@code
 * onFailure}, all other callback's methods are still called, and the
 * <em>first</em> caught exception will be rethrown from the <em>last</em>
 * method.
 */
public class CallbackGroup {
  ...
  public <T> com.google.gwtjsonrpc.common.AsyncCallback<T> add(
  ...
  private void onFailure(Throwable caught) {
    ...
    failed = true;
    Throwable caughtDuringOnFailure = null;
    for (Object o : callbacks) {
      try {
        if (o instanceof AsyncCallback) {
          @SuppressWarnings("unchecked")
        ...
        if (caughtDuringOnFailure == null) {
          caughtDuringOnFailure = t;
        }
      }
    }
    propagate(caught);
