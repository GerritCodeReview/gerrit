...
public class RestApi {
  ...
  public <T extends JavaScriptObject> void send(final AsyncCallback<T> cb) {
    ...
    req.setHeader("Accept", JsonUtil.JSON_TYPE);
    req.setCallback(new RequestCallback() {
      @Override
      public void onResponseReceived(Request req, Response res) {
        RpcStatus.INSTANCE.onRpcComplete();
        int status = res.getStatusCode();
        if (status != 200 || !isJsonBody(res)) {
        ...
        if (!json.startsWith(JSON_MAGIC)) {
          cb.onFailure(new InvocationException("Invalid JSON"));
          return;
        }

        T data;
        try {
          data = parse(parser, json.substring(JSON_MAGIC.length()));
  ...
  private static native <T extends JavaScriptObject> T parse(
      JavaScriptObject p, String d)
  /*-{ return p(d); }-*/;

  private static final JavaScriptObject parser;
  static {
  ...
  /*-{
    ...
    return function(d) { return eval('(' + d + ')'); };
  }-*/;
}
