...
public class LabelsEditor extends Composite {
  ...
  private void doAddNew() {
    ...
    if (arbitraryLabels != null) {
      if (!labelText.matches("([a-z,A-Z,0-9]+\\-)*[a-z,A-Z,0-9]+")) {
        Window.alert(Util.C.invalidCharacters());
      } else if (!isLabelAdded(labelText)) {
        ...
        Window.alert(Util.C.invalidLabel());
      }
    }
    nameTxtBox.setText("");
  }
  ...
  private void initLabel(final String labelKey, final boolean canDelete) {
    ...
    deleteLabelButton.addClickHandler(new ClickHandler() {
      ...
      public void onClick(ClickEvent event) {
        ...
        final ChangeLabel changeLabel = new ChangeLabel(changeId, labelKey);
        Util.MANAGE_SVC.deleteLabel(changeLabel,
            new GerritCallback<VoidResult>() {
              @Override
              public void onSuccess(VoidResult result) {
                g.removeFromParent();
