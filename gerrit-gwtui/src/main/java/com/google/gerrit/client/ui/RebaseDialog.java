...
public abstract class RebaseDialog extends ActionDialog {
  private SuggestBox base;
  private List<ChangeInfo> changes;

  public RebaseDialog(final FocusWidget enableOnFailure, final String project, final String branch) {
    super(enableOnFailure, true, Util.C.rebaseTitle(), null);
    ...
    base = new SuggestBox(
      new HighlightSuggestOracle() {
        ...
        protected void onRequestSuggestions(Request request, Callback done) {
          LinkedList<ChangeSuggestion> suggestions = new LinkedList<ChangeSuggestion>();
    ...
    final CheckBox cb = new CheckBox(Util.C.rebaseConfirmMessage());
    cb.addClickHandler(new ClickHandler() {
      @Override
      public void onClick(ClickEvent event) {
        boolean checked = ((CheckBox) event.getSource()).getValue();
        if (checked) {
          final FlowPanel mwrap = (FlowPanel) cb.getParent();
          cb.setVisible(false);

          // suggest the most recent 1000 changes
          ChangeList.next("project:" + project + " AND branch:" + branch + " AND NOT age:90d", 0, 1000,
