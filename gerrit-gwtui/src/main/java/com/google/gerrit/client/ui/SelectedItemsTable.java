...
public class SelectedItemsTable extends FlexTable {
  ...
  public void setDeleteListener(TableDeleteListener deleteListener) {
  ...
  public void addRow(final String rowText) {
    ...
    removeSelecteProjectButton.setStylePrimaryName(Gerrit.RESOURCES.css().buttonTrash());
    removeSelecteProjectButton.addClickHandler(new ClickHandler() {
      ...
      public void onClick(ClickEvent event) {
        ...
        if (deleteListener != null) {
          deleteListener.onDeleteRow(rowText);
        }
      }
    });
    ...
    final FlexCellFormatter fmt = getFlexCellFormatter();
    fmt.addStyleName(row, 0, Gerrit.RESOURCES.css().selectedProjectsDataCell());
    fmt.addStyleName(row, 0, Gerrit.RESOURCES.css().leftMostCell());
    fmt.addStyleName(row, 1, Gerrit.RESOURCES.css().selectedProjectsDataCell());
    fmt.setAlignment(row, 1, HasHorizontalAlignment.ALIGN_CENTER, HasVerticalAlignment.ALIGN_MIDDLE);
  ...
  /** Listener to delete button click event. */
  public static interface TableDeleteListener {
