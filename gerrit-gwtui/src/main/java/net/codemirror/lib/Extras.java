...
public class Extras {
  ...
  public final void setAnnotations(BlameInfo blameInfo) {
    if (blameInfo.blames().length() > 0) {
      ...
      for (int i = 0, blameNum = 0; i < numLines; i++) {
        if (lines.containsKey(i)) {
          BlameInfo.Line line = lines.get(i);
          Date commitTime = new Date(line.time() * 1000L);
          shortId = line.id().substring(0, 8);
          shortBlame = C.shortBlameMsg(
              shortId, format.format(commitTime), line.author());
          detailedBlame = C.detailedBlameMsg(line.id(), line.author(),
              FormatUtil.mediumFormat(commitTime), line.commitMsg());
          blameNum++;
        } else {
          shortBlame = "";
        }
        annotations.push(LintLine.create(shortBlame, detailedBlame,
            shortId + "-" + side + "-" + i, Pos.create(i), blameNum % 2 == 0));
      }
