...
public class Extras {
  ...
  public final native boolean isAnnotated(CodeMirror c) /*-{
    var gutters = c.getOption("gutters"), hasLintGutter = false;
    for (var i = 0; i < gutters.length; ++i) {
      if (gutters[i] == @net.codemirror.lib.Extras::ANNOTATION_GUTTER_ID) {
        return true;
      }
    }
    return false;
  ...
  public final BlameInfo.Line getBlame(int lineNum) {
    ...
    if (blameInfo != null) {
      JsArray<BlameInfo.Line> blames = blameInfo.blames();
      for (BlameInfo.Line line : Natives.asList(blames)) {
        for (BlameInfo.Line.FromTo fromTo : Natives.asList(line.fromTo())) {
