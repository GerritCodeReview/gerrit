...
public abstract class FilterProxyType<FilterType extends Filter> implements Filter, StopPluginListener {
  private final DynamicSet<FilterType> filters;

  private DynamicSet<FilterType> initializedFilters;
  private FilterConfig filterConfig;

  FilterProxyType(DynamicSet<FilterType> filters) {
   ...
   */
  private synchronized boolean initFilterIfNeeded(FilterType filter) throws ServletException {
    boolean ret = true;
  ...
  public void doFilter(ServletRequest req, ServletResponse res, final FilterChain last)
    ...
    final Iterator<FilterType> itr = filters.iterator();
    new FilterChain() {
      @Override
      public void doFilter(ServletRequest req, ServletResponse res)
          throws IOException, ServletException {
        while (itr.hasNext()) {
          FilterType filter = itr.next();
  ...
  public synchronized void destroy() {
    Iterable<FilterType> filtersToDestroy = initializedFilters;
    initializedFilters = new DynamicSet<>();
    for (FilterType filter : filtersToDestroy) {
      filter.destroy();
    }
  }
  ...
  private synchronized void cleanUpInitializedFilters() {
    Iterable<FilterType> filtersToCleanUp = initializedFilters;
    initializedFilters = new DynamicSet<>();
    for (FilterType filter : filtersToCleanUp) {
      if (filters.contains(filter)) {
        initializedFilters.add(filter);
      } else {
        filter.destroy();
