...
public class UniversalWebLoginFilter implements Filter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response,
    ...
    CurrentUser currentUser = loggedInBefore ? userProvider.get() : null;
    ...
    if (!loggedInBefore && loggedInAfter) {
      for (WebLoginListener loginListener : webLoginListeners) {
        currentUser = userProvider.get().asIdentifiedUser();
        loginListener.onLogin(currentUser.asIdentifiedUser(), httpRequest,
                              wrappedResponse);
      }
    } else if (loggedInBefore && !loggedInAfter) {
      for (WebLoginListener loginListener : webLoginListeners) {
        loginListener.onLogout(currentUser.asIdentifiedUser(), httpRequest,
                               wrappedResponse);
      }
    }
