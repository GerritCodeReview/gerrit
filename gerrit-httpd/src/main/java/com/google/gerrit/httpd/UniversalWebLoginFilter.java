...
public class UniversalWebLoginFilter implements Filter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response,
    ...
    Account.Id loggedAccountId = loggedInBefore ? session.get().getAccountId() : null;
    ...
    if (!loggedInBefore && loggedInAfter) {
      for (WebLoginListener loginListener : webLoginListeners) {
        loggedAccountId = session.get().getAccountId();
        loginListener.onLogin(loggedAccountId.get(), httpRequest, wrappedResponse);
      }
    } else if (loggedInBefore && !loggedInAfter) {
      for (WebLoginListener loginListener : webLoginListeners) {
        loginListener.onLogout(loggedAccountId.get(), httpRequest, wrappedResponse);
      }
    }
