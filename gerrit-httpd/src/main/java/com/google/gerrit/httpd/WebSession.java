...
public final class WebSession {
  ...
  private static final long UPDATE_WAIT_MILLISECONDS = 5 * 60 * 1000; // 5 min
  ...
  WebSession(final HttpServletRequest request,
      ...
      SecureRandom prng) throws OrmException {
  ...
  private ActiveSession createSession(final ActiveSession.Key key,
      ...
      final AccountExternalId.Key lastLogin, String xsrfToken) throws OrmException {
  ...
  private boolean needsCacheRefresh(ActiveSession as) {
    ...
    return now() - as.getLastSeen().getTime() > UPDATE_WAIT_MILLISECONDS;
  }
