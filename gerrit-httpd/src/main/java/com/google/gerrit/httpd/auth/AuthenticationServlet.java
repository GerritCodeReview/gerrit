...
public class AuthenticationServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
      ...
      ExternalId.Key key = ExternalId.Key.from(new AccountExternalId.Key(user.getUUID().uuid()));
  ...
  private Account.Id create(ReviewDb db, AuthUser user, Key key) throws OrmException {
  ...
  private Account.Id getUserAccount(AuthUser user, ExternalId.Key key) {
    try (ReviewDb db = schema.open()) {
      ...
      //TODO(dpursehouse) this needs to be reworked
      //if (externalId == null) {
