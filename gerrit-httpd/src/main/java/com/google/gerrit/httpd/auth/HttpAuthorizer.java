...
public class HttpAuthorizer implements Filter {
  ...
  @Inject
  HttpAuthorizer(AuthBackend authBackend, AccountCache accountCache,
      Provider<WebSession> session, DynamicSet<HttpAuthProtocolHandler> handlers,
  ...
  public void doFilter(ServletRequest req, ServletResponse resp,
    ...
    if (authRequest.getUsername() != null) {
      try {
        ...
        AccountState who = accountCache.getByUsername(user.getUsername());
  ...
  private HttpAuthRequest select(HttpServletRequest req,
      HttpServletResponse resp) throws AuthProtocolException {
    ...
    return anonymousHandler.handle(req, resp);
  }
