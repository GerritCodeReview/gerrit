 ...
 *
 * Callers that are trying to instantiate this class should be checking the
...
public class HttpBasicAuthRequest extends AuthRequest {
  ...
  public HttpBasicAuthRequest(HttpServletRequest req) throws IOException {
    ...
    if (hdr == null) {
      // We are anonymous at this stage
      username = password = null;
      return;
    }

    if (!hdr.startsWith(BASIC_AUTH_PREFIX)) {
      throw new IllegalArgumentException("Cannot parse an " + hdr
          + " authentication as Basic Auth");
    }
