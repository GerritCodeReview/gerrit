...
class BecomeAnyAccountLoginServlet extends HttpServlet {
  ...
  private AuthResult byUserName(final String userName) {
    try {
      List<AccountState> accountStates =
          accountQuery.byExternalId(SCHEME_USERNAME, userName);
  ...
  private AuthResult create() throws IOException {
    try {
      return accountManager.authenticate(new AuthRequest(
          ExternalId.Key.create(SCHEME_UUID, UUID.randomUUID().toString())));
