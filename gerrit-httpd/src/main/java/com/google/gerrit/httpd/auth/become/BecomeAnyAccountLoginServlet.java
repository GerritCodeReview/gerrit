...
public class BecomeAnyAccountLoginServlet extends HttpServlet {
  ...
  BecomeAnyAccountLoginServlet(final Provider<WebSession> ws,
    ...
    Element becomeLink = HtmlDomUtil.find(doc, "become_link");
    Element breakElement = HtmlDomUtil.find(doc, "break");
    Node parent = becomeLink.getParentNode();
    parent.removeChild(becomeLink);
    parent.removeChild(breakElement);
    ...
    for (Account a : accounts) {
      ...
      Element become = (Element) becomeLink.cloneNode(true);
      become.setAttribute("href", "?user_name=" + userName);
      become.setTextContent(userName);
      parent.appendChild(become);

      parent.appendChild(breakElement.cloneNode(true));
    }
