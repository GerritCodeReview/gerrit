...
public class OAuthProtocol {
  ...
  public static boolean isGerritLogin(HttpServletRequest request) {
  ...
  String retrieveUser(String authToken) throws IOException {
    ...
    try {
      JsonObject userJson = getUserJson(httpGetGitHubUserInfo(get));
      return getLogin(userJson);
    } catch (IOException e) {
  ...
  private String getAccessToken(JsonElement accessTokenJson) throws IOException {
    ...
    if (accessTokenString != null) {
      ...
      throw new IOException(String.format(
          "Invalid JSON '%s': cannot find access_token field", accessTokenJson));
    }
  }
  ...
  private JsonObject getUserJson(InputStream userContentStream)
    ...
    JsonElement userJson =
        gson.fromJson(new InputStreamReader(userContentStream, Charsets.UTF_8),
            JsonElement.class);
