...
class HTMLNodeFindReplace extends NodeVisitor {
  ...
  HTMLNodeFindReplace(List<RegexFindReplace> regexFindReplaceList) {
    ...
    regexPatterns = getPatterns(regexFindReplaceList);
  }

  private HashMap<String, Pattern> getPatterns(
      List<RegexFindReplace> regexFindReplaceList) {
    HashMap<String, Pattern> patterns = new HashMap<String, Pattern>();

    for (RegexFindReplace regexFindReplace : regexFindReplaceList) {
      String find = regexFindReplace.find();
      if(patterns.get(find) == null) {
        patterns.put(find, Pattern.compile(find));
      }
    }
