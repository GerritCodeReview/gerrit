...
class HttpPluginServlet extends HttpServlet
  ...
  private void onDefault(PluginHolder holder,
    ...
    if ("".equals(file)) {
      ...
      if (!exists(entry) && file.endsWith("/index.html")) {
        ...
        sendAutoIndex(jar, pfx, holder.plugin.getName(), res);
      } else if (exists(entry) && entry.getName().endsWith(".md")) {
        sendMarkdownAsHtml(jar, entry, holder.plugin.getName(), res);
  ...
  private void sendAutoIndex(JarFile jar,
    ...
    if (!docs.isEmpty()) {
      ...
      for(JarEntry entry : docs) {
        ...
        if (rsrc.endsWith(".html")) {
          title = rsrc.substring(0, rsrc.length() - 5);
        } else if (rsrc.endsWith(".md")) {
          title = extractTitleFromMarkdown(jar, entry);
          if (Strings.isNullOrEmpty(title)) {
            title = rsrc.substring(0, rsrc.length() - 3);
          }
    ...
    if (!cmds.isEmpty()) {
      ...
      for(JarEntry entry : cmds) {
        ...
        String title = rsrc;
        if (rsrc.endsWith(".html")) {
          title = rsrc.substring(4, rsrc.length() - 5);
        } else if (rsrc.endsWith(".md")) {
          title = extractTitleFromMarkdown(jar, entry);
          if (Strings.isNullOrEmpty(title)) {
            title = rsrc.substring(4, rsrc.length() - 3);
          }
          rsrc = rsrc.substring(0, rsrc.length() - 3) + ".html";
        }
        md.append(String.format("* [%s](%s)\n", title, rsrc));
    ...
    sendMarkdownAsHtml(md.toString(), pluginName, res);
  }

  private void sendMarkdownAsHtml(String md, String pluginName,
      HttpServletResponse res) throws UnsupportedEncodingException, IOException {
    md = md.replaceAll("@PLUGIN@", pluginName);
    md = md.replaceAll("@SSH_HOST@", sshHost);
    md = md.replaceAll("@SSH_PORT@", "" + sshPort);
    String url = webUrl.get();
    if (Strings.isNullOrEmpty(url)) {
      url = "http://review.example.com/";
    }
    md = md.replaceAll("@URL@", url);

    byte[] html = new MarkdownFormatter()
      .markdownToDocHtml(md, "UTF-8");
  ...
  private void sendMarkdownAsHtml(JarFile jar, JarEntry entry,
      String pluginName, HttpServletResponse res) throws IOException {
    ...
    sendMarkdownAsHtml(txtmd, pluginName, res);
  }
