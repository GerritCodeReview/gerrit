...
public class ProjectDocServlet extends HttpServlet {
    ...
    try {
      ...
      try {
        ...
        if (receivedETag != null) {
          eTag = computeETag(key.project, revId, revIdB, key.file);
        ...
        if (key.file.endsWith(".md")) {
          rsc = docCache.getUnchecked(
            new ProjectDocResourceKey(key.project, key.file, revId, revIdB));
        ...
        if (rsc != Resource.NOT_FOUND) {
          res.setHeader(HttpHeaders.ETAG,
              Objects.firstNonNull(eTag,
                  computeETag(key.project, revId, revIdB, key.file)));
        }
  ...
  private static class ResourceKey {
    ...
    static ResourceKey fromPath(String path) {
      ...
      return new ResourceKey(project, file, revision, revisionB);
    }

    private ResourceKey(String p, String f, String r, String rB) {
