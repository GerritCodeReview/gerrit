...
public abstract class ResourceServlet extends HttpServlet {
  ...
  private static final int CACHE_FILE_SIZE_LIMIT = 100 << 10;
  ...
  protected ResourceServlet(Cache<Path, Resource> cache,
      boolean refresh) {
    this.cache = cache;
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse rsp)
    ...
    long size = FileUtil.size(p);
    if (size < 0 || size > CACHE_FILE_SIZE_LIMIT) {
      stream(p, req, rsp);
      return;
    }
  ...
  private void stream(Path p, HttpServletRequest req,
