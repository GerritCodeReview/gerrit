...
public class StaticModule extends ServletModule {
  ...
  private static final int GERRIT_UI_COOKIE_MAX_AGE =
      Ints.checkedCast(Duration.standardDays(365).getStandardSeconds());
  ...
  private static class PolyGerritFilter implements Filter {
    ...
    private boolean isPolyGerritCookie(HttpServletRequest req) {
      UiPreference siteDefault = UiPreference.GWT;
      if (options.enablePolyGerrit() && !options.enableGwtUi()) {
        siteDefault = UiPreference.POLYGERRIT;
      }
      ...
      if (all != null) {
        for (Cookie c : all) {
          if (GERRIT_UI_COOKIE.equals(c.getName())) {
            ...
            UiPreference pref =
                Enums.getIfPresent(UiPreference.class, v).or(siteDefault);
    ...
    private static boolean setPolyGerritCookie(HttpServletRequest req,
