...
class ParameterParser {
  ...
  <T> boolean parse(T param, HttpServletRequest req, HttpServletResponse res)
    ...
    try {
      clp.parseOptionMap(splitQueryString(req.getQueryString()));
  ...
  private static Multimap<String, String> splitQueryString(String queryString)
      throws UnsupportedEncodingException {
    Multimap<String, String> args = LinkedHashMultimap.create();
    if (!Strings.isNullOrEmpty(queryString)) {
      for (String kvPair : Splitter.on('&').split(queryString)) {
        Iterator<String> itr = Splitter.on('=').split(kvPair).iterator();
        String key = decode(itr.next());
        if (!RESERVED_KEYS.contains(key)) {
          while (itr.hasNext()) {
            args.put(key, decode(itr.next()));
          }
        }
      }
    }
