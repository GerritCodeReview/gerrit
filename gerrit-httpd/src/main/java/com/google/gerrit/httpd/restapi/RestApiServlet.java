  ...
  private Object parseJson(HttpServletRequest req, Class<Object> type)
    ...
    if (isJson(req.getContentType())) {
      ...
      try {
        ...
        return OutputFormat.JSON.newGson().fromJson(json, JsonElement.class);
      } finally {
        br.close();
      }
    } else if ("text/plain".equals(req.getContentType())) {
      BufferedReader br = req.getReader();
      try {
    ...
    } else if ("DELETE".equals(req.getMethod()) && req.getContentType() == null) {
      return null;
    } else {
