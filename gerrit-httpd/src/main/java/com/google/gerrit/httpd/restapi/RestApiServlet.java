...
public class RestApiServlet extends HttpServlet {
  ...
  protected final void service(HttpServletRequest req, HttpServletResponse res)
    ...
    try {
      ...
      checkAccessAnnotations(rc.getClass(), null);

      RestResource rsrc = TopLevelResource.INSTANCE;
      if (path.isEmpty()) {
        if ("GET".equals(req.getMethod())) {
          viewData.view = rc.list();
        } else if (rc instanceof AcceptsPost && "POST".equals(req.getMethod())) {
          @SuppressWarnings("unchecked")
          AcceptsPost<RestResource> ac = (AcceptsPost<RestResource>) rc;
          viewData.view = ac.post(rsrc);
        } else {
        ...
        try {
          ...
          if (rc instanceof AcceptsCreate
            ...
            viewData.view = ac.create(rsrc, id);
            status = SC_CREATED;
          } else {
            throw e;
          }
  ...
  private ViewData view(
    ...
    List<String> p;
    p = splitProjection(projection);
    if (p.size() == 2) {
      RestView<RestResource> view =
          views.get(p.get(0), method + "." + p.get(1));
      if (view != null) {
        return new ViewData(view, p.get(0));
      }
    ...
    if (r.size() == 1) {
      Map.Entry<String, RestView<RestResource>> entry = Iterables.getFirst(r.entrySet(), null);
      return new ViewData(entry.getValue(), entry.getKey());
    } else if (r.isEmpty()) {
      throw new ResourceNotFoundException(projection);
    } else {
  ...
  private void checkAccessAnnotations(ViewData viewData) throws AuthException {
    checkAccessAnnotations(viewData.view.getClass(), viewData.pluginName);
  }

  private void checkAccessAnnotations(Class<? extends Object> clazz, String pluginName)
      throws AuthException {
    RequiresCapability rc =
        clazz.getAnnotation(RequiresCapability.class);
    if (rc != null) {
      CurrentUser user = globals.currentUser.get();
  ...
  private static class ViewData {
    ...
    String pluginName;
    ViewData() {}
    ViewData(RestView<RestResource> view) {
      this.view = view;
    }
    ViewData(RestView<RestResource> view, String pluginName) {
      this(view);
