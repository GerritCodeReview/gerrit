...
import com.google.gerrit.extensions.restapi.NullResource;
...
public abstract class RestApiServlet extends HttpServlet {
  ...
  protected final void service(HttpServletRequest req, HttpServletResponse res)
    ...
    try {
      ...
      RestResource rsrc = NullResource.INSTANCE;
      RestView<RestResource> view;
      if (path.isEmpty()) {
        view = rc.list();
      } else {
        rsrc = rc.parse(rsrc, path.remove(0));
      ...
      replyError(res, SC_BAD_REQUEST, "Invalid " + JSON_TYPE);
    } catch (Exception e) {
      handleException(e, req, res);
    }
  }

  private Object parseJson(HttpServletRequest req, Class<Object> type)
      throws IOException {
  ...
  private static boolean acceptsJson(HttpServletRequest req) {
    return req != null && isJson(req.getHeader("Accept"));
  }
