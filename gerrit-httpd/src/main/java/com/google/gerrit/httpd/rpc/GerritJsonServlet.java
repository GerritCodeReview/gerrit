...
//    return withAudit(super.lookupMethod(methodName));
  ...
  private void audit() {
    try {
      ...
      if (note != null) {
        ...
        final Object result = extractResult(call, gson);

        audit.track(new AuditRecord(sid, username, what, call.getWhen(), args,
  ...
  // FIXME: no other way found atm... tried EVERYTHING!
  private Object extractResult(GerritCall call, final Gson gson) {
    try {
      Field field = ActiveCall.class.getDeclaredField("result");
      field.setAccessible(true);
      return field.get(call);
    } catch (Exception e) {
      LOG.error("Unable to access to call result", e);
      return null;
    }
  }
