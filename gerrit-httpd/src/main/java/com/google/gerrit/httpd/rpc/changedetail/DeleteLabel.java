...
public class DeleteLabel extends Handler<VoidResult> {

  interface Factory {
    DeleteLabel create(@Assisted ChangeLabel changeLabel);
  }
  ...
  @Inject
  DeleteLabel(final ChangeControl.Factory changeControlFactory,
      final ReviewDb db, @Assisted final ChangeLabel changeLabel) {
    ...
    try {
      ...
      if (changeControl.canEditChangeLabels()) {
        ...
        throw new Failure(new PermissionDeniedException(
            "User does not have permission to delete labels."));
      }
