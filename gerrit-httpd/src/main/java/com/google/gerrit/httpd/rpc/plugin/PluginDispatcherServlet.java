...
public class PluginDispatcherServlet extends RestApiServlet {
  static final Logger log = LoggerFactory.getLogger(PluginDispatcherServlet.class);
  private static final long serialVersionUID = 1L;
  private final ListPluginsServlet listServlet;
  private final ReloadPluginsServlet reloadServlet;

  @Inject
  PluginDispatcherServlet(final Provider<CurrentUser> currentUser,
      ListPluginsServlet list, ReloadPluginsServlet reload) {
    super(currentUser);
    listServlet = list;
    reloadServlet = reload;
  }
