...
class ChangeProjectSettings extends Handler<ProjectDetail> {
  ...
  public ProjectDetail call() throws NoSuchProjectException, OrmException {
    ...
    try {
      git = mgr.openRepository(projectName.get());
    ...
    try {
      ...
      //
      ProjectConfig config = ProjectConfig.read(git);
      config.getProject().copySettingsFrom(update);
      CommitBuilder cb = new CommitBuilder();
      cb.setAuthor(currentUser.newCommitterIdent());
      cb.setCommitter(cb.getAuthor());
      cb.setMessage("Modified project settings\n");
      if (config.commit(cb, git)) {
        mgr.setProjectDescription(projectName.get(), update.getDescription());
