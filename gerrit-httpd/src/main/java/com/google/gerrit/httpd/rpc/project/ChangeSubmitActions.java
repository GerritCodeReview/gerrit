...
public class ChangeSubmitActions extends Handler<VoidResult> {
  interface Factory {
    ChangeSubmitActions create(@Assisted Project.NameKey projectName,
        @Assisted ObjectId base,
        @Assisted List<SubmitActionSection> sectionList,
  ...
  private List<SubmitActionSection> sectionList;
  ...
  public VoidResult call() throws NoSuchRefException, NoSuchProjectException,
    ...
    try {
      ...
      final Set<SubmitActionSection> submitActionSections =
          new HashSet<SubmitActionSection>(sectionList);

      for (SubmitActionSection section : submitActionSections) {
        final String name = section.getName();
        if (!projectControl.isVisible()
            || !projectControl.controlForRef(name).isOwner()) {
          continue;
        }
      ...
      for (String name : toDelete) {
        if (projectControl.controlForRef(name).isOwner()) {
          config.remove(config.getSubmitActionSection(name));
        }
      }
      ...
      if (message != null && !message.isEmpty()) {
        ...
        md.setMessage("Modify submit action\n");
      }
