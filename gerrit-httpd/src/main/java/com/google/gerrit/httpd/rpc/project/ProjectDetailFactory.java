...
class ProjectDetailFactory extends Handler<ProjectDetail> {
  ...
  public ProjectDetail call() throws NoSuchProjectException {
    ...
    // This block of code will be removed when integrating Submit Action UI change.
    final Project p = projectState.getProject();

    p.setSubmitType(SubmitActionSection.SubmitType.MERGE_IF_NECESSARY.name());
    for (SubmitActionSection section : projectState
        .getLocalSubmitActionSections()) {
      if (section.getName() != null
        ...
        if (section.isUseContentMerge() != null) {
          p.setUseContentMerge( //
              section.isUseContentMerge() == SubmitActionSection.UseContentMerge.TRUE
                  ? true : false);
        }
        break;
      }
