...
public class SetParent extends Handler<String> {
  ...
  @Override
  public String call() throws OrmException {
    StringBuilder errorMessage = null;

    Project.NameKey parentNameKey;

    if (parentName != null && parentName.isEmpty()) {
      parentNameKey = wildProject;
    } else {
      parentNameKey = new Project.NameKey(parentName);
    }

    if (projectCache.get(parentNameKey) == null) {
      errorMessage = new StringBuilder();
      errorMessage.append("error: Project '" + parentName + "' not found\n");

      // all projects could not have parent set
      errorMessage.append("All:");
    }

    if (errorMessage == null) {
      final PerformSetParent perfSetParent = performSetParent.create(parentName, childProjects);
      errorMessage = perfSetParent.setParent();
    }
