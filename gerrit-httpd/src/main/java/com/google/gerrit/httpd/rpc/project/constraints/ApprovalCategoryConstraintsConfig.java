...
public class ApprovalCategoryConstraintsConfig {
  ...
  public void checkIfAllowed(ApprovalCategory.Id categoryId, String ref,
    ...
    for (Allow c : allows) {
      if (c.isAllowed(ref, min, max)) {
        return;
      }
    }

    throw new ForbiddenRefRightException(allows);
  }
  ...
  private void parse(Config cfg, ApprovalCategory.Id categoryId)
    ...
    for (String allow : allows) {
      allowList.add(parseAllow(allow));
    }
  }

  static private Allow parseAllow(String value)
      throws InvalidConstraintException {
    try {
      String refPattern;
      ...
      throw new InvalidConstraintException(value, e);
    }
  }
