...
public class LuceneChangeIndex implements ChangeIndex {
  ...
  private ChangeData toChangeData(Document doc) {
    ...
    if ("0".equals(mergeable)) {
      cd.setMergeable(Optional.of(false));
    } else if ("1".equals(mergeable)) {
      cd.setMergeable(Optional.of(true));
    } else {
      cd.setMergeable(Optional.<Boolean> absent());
    }
