...
public class LuceneChangeIndex implements ChangeIndex {
  ...
  @Inject
  LuceneChangeIndex(SitePaths sitePaths) throws IOException {
  ...
  private Document toDocument(ChangeData cd) throws IOException {
    ...
    for (ChangeField f : ChangeField.values()) {
      switch (f.getType()) {
        case INTEGER:
          result.add(
              new IntField(f.getName(), (Integer) f.get(cd), store(f)));
          break;
        case EXACT_REPEATABLE:
          @SuppressWarnings("unchecked") // Type checked in ChangeField.
