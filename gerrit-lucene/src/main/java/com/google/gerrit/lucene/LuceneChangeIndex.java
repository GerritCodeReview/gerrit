...
public class LuceneChangeIndex implements ChangeIndex {
  ...
  private ChangeData toChangeData(Document doc) {
    ...
    // Reviewed-by.
    String[] reviewedBy = doc.getValues(REVIEWEDBY_FIELD);
    if (reviewedBy.length == 1
        && reviewedBy[0].equals(ChangeField.NOT_REVIEWED)) {
      cd.setReviewedBy(ImmutableSet.<Account.Id> of());
    } else {
      Set<Account.Id> accounts =
          Sets.newHashSetWithExpectedSize(reviewedBy.length);
      for (String r : reviewedBy) {
        accounts.add(new Account.Id(Integer.parseInt(r)));
      }
