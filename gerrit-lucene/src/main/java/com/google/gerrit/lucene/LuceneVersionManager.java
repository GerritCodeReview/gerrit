...
class LuceneVersionManager implements LifecycleListener {
  ...
  public void start() {
    ...
    LuceneChangeIndex searchIndex =
        indexFactory.create(search.schema, readOnly);
    indexes.setSearchIndex(searchIndex);
    for (Version v : versions.values()) {
      if (v.schema != null) {
        if (v.version != search.version) {
          indexes.addWriteIndex(indexFactory.create(v.schema, readOnly));
        } else {
  ...
  private TreeMap<Integer, Version> scanVersions(Config cfg) {
    ...
    for (File f : sitePaths.index_dir.listFiles()) {
      ...
      if (v == null || versionStr.length() != 4) {
        log.warn("Unrecognized version in index directory: %s",
            f.getAbsolutePath());
        continue;
      }
