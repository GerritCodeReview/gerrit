...
public class OnlineReindexer {
  ...
  private static int version(Index<ChangeData, ChangeDataSource> i) {
    return i.getSchema().getVersion();
  }

  private void reindex() {
    Index<ChangeData, ChangeDataSource> index = checkNotNull(indexes.getWriteIndex(version),
    ...
    List<Index<ChangeData, ChangeDataSource>> toRemove = Lists.newArrayListWithExpectedSize(1);
    for (Index<ChangeData, ChangeDataSource> i : indexes.getWriteIndexes()) {
      if (version(i) != version(index)) {
        toRemove.add(i);
      }
    }
    for (Index<ChangeData, ChangeDataSource> i : toRemove) {
      try {
        i.markReady(false);
