...
import com.google.gerrit.lucene.AutoCommitWriter.AutoCommitType;
    ...
    if (commitPeriod < 0) {
      writer =
          new TrackingIndexWriter(new AutoCommitWriter(dir,
              writerConfig.getLuceneConfig(), AutoCommitType.BUFFER_CONFIG_ONLY));
    } else if (commitPeriod == 0) {
      writer =
          new TrackingIndexWriter(new AutoCommitWriter(dir,
              writerConfig.getLuceneConfig(), AutoCommitType.AT_ONCE));
    } else {
      final AutoCommitWriter delegateWriter =
          new AutoCommitWriter(dir, writerConfig.getLuceneConfig(),
              AutoCommitType.PERIOD);
