...
class OAuthWebFilter implements Filter {
  ...
  OAuthWebFilter(Provider<CurrentUser> currentUserProvider,
      ...
      Provider<OAuthSession> oauthLogin) {
    this.currentUserProvider = currentUserProvider;
    this.oauthServiceProviders = oauthServiceProviders;
    this.oauthSessionProvider = oauthLogin;
  }
  ...
  public void doFilter(ServletRequest request, ServletResponse response,
    ...
    } else if (oauthServiceProvider.isAutoLogin()
        && !oauthSession.isLoggedIn()) {
      httpResponse.sendRedirect("/login");
    } else {
      if (oauthSession.isLoggedIn()) {
        httpRequest =
            new AuthenticatedHttpRequest(httpRequest,
                oauthServiceProvider.getAuthHttpHeader(),
