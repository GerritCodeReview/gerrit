...
public class LocalUsernamesToLowerCase extends SiteProgram {
  ...
  @Inject
  private ExternalIdsBatchUpdate externalIdsUpdate;
    ...
    if (extId.key().isScheme(SCHEME_GERRIT)) {
      ...
      if (!localUser.equals(localUserLowerCase)) {
        ExternalId extIdLowerCase =
            ExternalId.create(SCHEME_GERRIT, localUserLowerCase,
                extId.accountId(), extId.email(), extId.password());
        externalIdsUpdate.replace(extId, extIdLowerCase);
      }
    }
  }
}
