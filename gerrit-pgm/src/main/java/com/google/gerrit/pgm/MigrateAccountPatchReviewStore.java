...
public class MigrateAccountPatchReviewStore extends SiteProgram {
  ...
  public int run() throws Exception {
    ...
    try (Connection sourceCon = sourceJdbcAccountPatchReviewStore.getConnection();
      ...
      long offset = 0;
      List<Row> rows = selectRows(sourceStmt, offset, chunkSize);
      while (!rows.isEmpty()) {
        insertRows(targetCon, targetStmt, rows);
        offset += rows.size();
        rows = selectRows(sourceStmt, offset, chunkSize);
      }
    }
    return 0;
  }
  ...
  public List<Row> selectRows(PreparedStatement stmt, long offset, long limit)
      throws SQLException {
