...
public class MigrateToReviewDb extends SiteProgram {
  ...
  public int run() throws Exception {
    ...
    try {
      ...
      NotesMigrationState reqState = READ_WRITE_WITH_SEQUENCE_NOTE_DB_PRIMARY;
      if (s != reqState) {
        throw die(
            "Must be in NoteDb with change tables not yet disabled (" + reqState + "), found " + s);
      }
      ...
      pm.endTask();
      Collections.sort(ids, Comparator.comparingInt(i -> i.changeId().get()));
      ...
      ListenableFuture<?> future =
          Futures.allAsList(
              ids.stream()
                  ...
                  .map(executor::submit)
                  .collect(toList()));
      future.addListener(mpm::end, MoreExecutors.directExecutor());
      mpm.waitFor(future);
    ...
    public void run() {
      try (ManualRequestContext ctx = requestContext.open()) {
        try {
          ...
          doneTask.update(1);
        } catch (OrmException | IOException | RuntimeException e) {
