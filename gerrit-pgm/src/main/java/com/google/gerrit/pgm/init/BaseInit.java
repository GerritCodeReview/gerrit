...
import static com.google.gerrit.pgm.util.SecureStoreProvider.getSecureStoreClassFromGerritConfig;
...
public class BaseInit extends SiteProgram {
  ...
  private SiteInit createSiteInit() {
    ...
    final String secureStoreClassName = discoverSecureStoreClass(sitePath);
  ...
  private String discoverSecureStoreClass(File sitePath) {
    ...
    try {
      currentSecureStore = getSecureStoreClassFromGerritConfig(new SitePaths(sitePath));
      if (!Strings.isNullOrEmpty(currentSecureStore)) {
        throw new RuntimeException(
            String
            .format(
                "SecureStore is already set to %s. To change it use switch-secure-store program.",
                currentSecureStore));
      }
    } catch (FileNotFoundException e) {
