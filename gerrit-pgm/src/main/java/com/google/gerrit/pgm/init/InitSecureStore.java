...
public class InitSecureStore {
  ...
  public Injector init() throws Exception {
    Map<String, SecureStoreData> secureStores =
        getAllSecureStores();
    ...
    SecureStoreData selectedPlugin =
  ...
  private Map<String, SecureStoreData> getAllSecureStores()
    ...
    for (PluginData pluginData : plugins) {
      ...
      Set<SecureStoreData> stores =
          SecureStoreProvider.findSecureStores(file, pluginData.name);
      for (SecureStoreData store : stores) {
        secureStores.put(store.getStoreName(), store);
      }
    }
    return secureStores;
  }
