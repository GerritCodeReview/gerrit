...
import com.google.gerrit.extensions.events.ClaSignupListener;
...
public class ChangeHookApiListener implements
  ...
  public void onCommentAdded(CommentAddedListener.Event ev) {
    ...
    try {
      hooks.doCommentAddedHook(getChange(ev.getChange()),
        getAccount(ev.getAuthor()), getPatchSet(ev.getRevision()),
        ev.getComment(), approvals, db.get());
    } catch (OrmException e) {
  ...
  public void onMergeFailed(MergeFailedListener.Event ev) {
    try {
     hooks.doMergeFailedHook(getChange(ev.getChange()),
         getAccount(ev.getSubmitter()), getPatchSet(ev.getRevision()),
         ev.getReason(), db.get());
    } catch (OrmException e) {
  ...
  public void onChangeAbandoned(ChangeAbandonedListener.Event ev) {
    try {
      hooks.doChangeAbandonedHook(getChange(ev.getChange()),
          getAccount(ev.getAbandoner()), getPatchSet(ev.getRevision()),
          ev.getReason(), db.get());
    } catch (OrmException e) {
    }
  }
  ...
  public void onChangeRestored(ChangeRestoredListener.Event ev) {
    try {
      hooks.doChangeRestoredHook(getChange(ev.getChange()),
          getAccount(ev.getRestorer()), getPatchSet(ev.getRevision()),
          ev.getReason(), db.get());
    } catch (OrmException e) {
    }
  }
  ...
  @Override
  public void onClaSingup(ClaSignupListener.Event ev) {
    hooks.doClaSignupHook(getAccount(ev.getAccount()), ev.getClaName());
  }
