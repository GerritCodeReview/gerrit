...
public class ChangeHookRunner {
     ...
     * Called with arguments: <change id> <repo name> <branch> <comment author> <comment> [--Category1 Score --Category2 Score ... --CategoryN Score]
     */
    ...
    public void doPatchsetCreatedHook(final Change change, final PatchSet patchSet) {
        if (patchsetCreatedHook != null && patchsetCreatedHook.exists()) {
     ...
     */
    @SuppressWarnings("deprecation")
    public void doCommentAddedHook(final Change change, final Account account, final String comment, final Map<String, Short> approvals) {
        if (commentAddedHook != null && commentAddedHook.exists()) {
            final List<String> args = new ArrayList<String>();
            ...
            for (Map.Entry<String, Short> approval : approvals.entrySet()) {
                ...
                args.add(Short.toString(approval.getValue()));
            }
    ...
    public void doChangeMergedHook(final Change change, final Account account, final PatchSet patchSet) {
        if (changeMergedHook != null && changeMergedHook.exists()) {
            final List<String> args = new ArrayList<String>();
    ...
    public void doChangeAbandonedHook(final Change change, final Account account, final String reason) {
        if (changeAbandonedHook != null && changeAbandonedHook.exists()) {
            final List<String> args = new ArrayList<String>();
    ...
    private synchronized void runHook(final Repository repo, final List<String> args) {
        ...
        hookQueue.schedule(new Runnable() {
