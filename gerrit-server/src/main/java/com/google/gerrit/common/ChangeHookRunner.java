...
public class ChangeHookRunner {
    ...
    public void doChangeMergedHook(final Change change, final Account account, final PatchSet patchSet) {
        ...
        doRefChangedHook(change.getDest(), ObjectId.fromString(patchSet.getRevision().get()), account);
    }
     ...
     */
    public void doRefChangedHook(final Branch.NameKey refName, final ObjectId newId, final Account account) {
      final RefChangedEvent event = new RefChangedEvent();

      event.change = eventFactory.asChangeAttribute(refName, newId);
      ...
      runHook(openRepository(refName.getParentKey()), refChangedHook, args);
    }
