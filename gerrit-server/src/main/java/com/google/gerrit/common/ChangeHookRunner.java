...
public class ChangeHookRunner implements ChangeHooks, LifecycleListener {
    ...
    public void doHashtagsEditedHook(final Change change, final Account account,
        final Set<String> added, final Set<String> removed,
      ...
      if (added != null && added.size() > 0) {
        event.added = new String[added.size()];
        int i = 0;
        for (String hashtag : added) {
          event.added[i++] = hashtag;
         }
      }
      if (removed != null && removed.size() > 0) {
        event.removed = new String[removed.size()];
        int i = 0;
        for (String hashtag : removed) {
          event.removed[i++] = hashtag;
         }
      }
      fireEvent(change, event, db);
