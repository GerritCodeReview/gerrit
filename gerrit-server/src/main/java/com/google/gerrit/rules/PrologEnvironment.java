...
public class PrologEnvironment extends BufferingPrologControl {
  ...
  public void setPredicate(String pkg, String functor, Term... args) {
    ...
    } catch (ExistenceException err) {
      if (!pkg.startsWith("com.google.gerrit.rules.")) {
        try {
          pkg = "com.google.gerrit.rules.common";
          setPredicate(pkg, "default_" + functor, args);
        } catch (ExistenceException err2) {
  ...
  private void init() {
    if (!intialized) {
      intialized = true;
      if (!initialize(Prolog.BUILTIN, "com.google.gerrit.rules.common")) {
