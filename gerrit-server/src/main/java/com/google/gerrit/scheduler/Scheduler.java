...
/**
 * Scheduler using Quartz to schedule recurring tasks
 */
@Singleton
public class Scheduler implements LifecycleListener {
  ...
  private Properties quartzConfig;
  ...
  public Scheduler(Injector injector, DynamicMap<Class<? extends Job>> jobs,
    ...
    this.jobs = jobs;
    this.config = config;
    quartzConfig = new Properties();
    quartzConfig.put("org.quartz.threadPool.threadCount", "1");
    quartzConfig.put("org.quartz.scheduler.skipUpdateCheck", "true");
  }

  @Override
  public void start() {
    try {
      StdSchedulerFactory f = new StdSchedulerFactory(quartzConfig);
  ...
  private void scheduleJobs() {
    for (Entry<Class<? extends Job>> jobClassEntry: jobs) {
 ...
 private void scheduleCronJob(String jobName, Class<? extends Job> jobClass) {
    ...
    String schedule = config.getString("job", jobName, "schedule");
    if (schedule != null && schedule != "") {
      ...
      log.info("No schedule configured for job ''{0}'' implemented by class ''{1}''", jobName, jobClass);
    }
  }
}
