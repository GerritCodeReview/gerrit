...
public class ApprovalCopier {
  ...
  private List<PatchSetApproval> getForPatchSet(ReviewDb db, ChangeControl ctl,
    ...
    try {
      ...
      try {
        ...
        for (PatchSet priorPs : patchSets.descendingMap().values()) {
          PatchSet.Id prior = priorPs.getId();
          List<PatchSetApproval> priorApprovals = all.get(prior);
          if (prior.equals(psId) || priorApprovals.isEmpty()) {
            continue;
          }
  ...
  private static <T> T previous(NavigableSet<T> s, T v) {
    ...
    return head != null ? head.last() : null;
  }
}
