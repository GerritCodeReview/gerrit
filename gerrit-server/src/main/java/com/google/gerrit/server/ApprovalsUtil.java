...
public class ApprovalsUtil {
  ...
  public List<PatchSetApproval> copyApprovalsToLatestPatchSet(
      @GerritServerConfig Config cfg, Change change) throws OrmException {
    return copyApprovalsToLatestPatchSet(cfg, db, change);
  }
    ...
    for (PatchSetApproval a : patchSetApprovals) {
      ...
      if (!ApprovalCategory.SUBMIT.equals(a.getCategoryId())) {
        ...
        if ((a.getPatchSetId().equals(source) &&
             type.getCategory().isCopyMinScore() &&
             type.isMaxNegative(a)) ||
             cfg.getBoolean("approvals", a.getCategoryId().get(),
                 "stickThroughRebase", false)) {
          db.patchSetApprovals().insert(
              Collections.singleton(new PatchSetApproval(dest, a)));
        }
      }
