...
public class AtomicUtil {
   ...
   * submitted state. Excepting the argument change.
   *
   * @param db
   * @param change
   * @return
  ...
  public static boolean isAllMerging(ReviewDb db, Change change)
      throws OrmException {
    List<Change> atomicGroup = getAtomicGroupByAnyMember(db, change);
    if (atomicGroup != null) {
      for (Change entryChange : atomicGroup) {
        if ((entryChange.getStatus() != Change.Status.SUBMITTED)
            && (entryChange.getStatus() != Change.Status.MERGED)) {
          if (!entryChange.equals(change)) {
            // do not consider the own attribute
            // change status
            return false;
   ...
   * @throws OrmException
   */
  public static boolean isAllMergeable(ReviewDb db, Change change)
      throws OrmException {
