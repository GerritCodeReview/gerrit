...
public class CommentsUtil {
  ...
  public static List<PatchLineComment> toPatchLineComments(
      final Change.Id changeId, final PatchLineComment.Status status,
      Iterable<Comment> comments) {
    return FluentIterable.from(comments)
        .transform(c -> PatchLineComment.from(changeId, status, c)).toList();
  }
