...
public class PatchLineCommentsUtil {
  ...
  public PatchLineComment get(ReviewDb db, ChangeNotes notes,
    ...
    List<PatchLineComment> comments = Lists.newArrayList(byChange(db, notes));
  ...
  public List<PatchLineComment> byChange(ReviewDb db,
    ...
    Map<String, Ref> matchingRefs = getRefMapAllUsers(RefNames.REFS_USER);
    for (String refName : matchingRefs.keySet()) {
      String suffix = RefNames.REFS_DRAFT_PREFIX +
          Integer.toString(notes.getChangeId().get());
      if (!refName.endsWith(suffix)) {
        continue;
      }
      Account.Id account = Account.Id.fromRef(refName);
  ...
  public List<PatchLineComment> draftByAuthor(ReviewDb db,
    ...
    Map<String, Ref> refs = getRefMapAllUsers(RefNames.refsUsers(author));
  ...
  private Map<String, Ref> getRefMapAllUsers(String prefix) throws OrmException {
