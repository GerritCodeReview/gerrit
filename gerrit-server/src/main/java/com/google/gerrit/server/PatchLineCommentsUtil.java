...
public class PatchLineCommentsUtil {
  ...
  public void addPublishedComments(ReviewDb db, ChangeUpdate update,
    ...
    for (PatchLineComment c: comments) {
      addPublishedComment(db, update, c);
    }
  }

  private void addPublishedComment(ReviewDb db, ChangeUpdate update,
      PatchLineComment c) throws OrmException {
    if (c != null) {
      update.putComment(c);
      db.patchComments().upsert(Collections.singleton(c));
    }
  }
}
