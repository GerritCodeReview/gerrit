...
public class ReviewersUtil {
    ...
    for (ReviewerSuggestion r : reviewerSuggestionPlugins) {
      ...
      for (SuggestedReviewer s : r.suggestReviewers(query,
        ...
        if (reviewerScores.containsKey(s.account)) {
          reviewerScores.put(s.account,
              reviewerScores.get(s.account) + s.score);
        } else {
  ...
  private Map<Account.Id, Double> defaultReviewersForEmptyQuery()
      throws OrmException{
    ...
    try {
      ...
      Map<Account.Id, Double> suggestions = new HashMap<>();
      for (ChangeData cd : result.entities()) {
        for (Account.Id id : cd.getReviewers().all()) {
          if (suggestions.containsKey(id)) {
            suggestions.put(id, suggestions.get(id) + 1);
          } else {
            suggestions.put(id, 1d);
          }
        }
      }
      return suggestions;
  ...
  private Map<Account.Id, Double> defaultRankingForCandidateList(
      List<Account.Id> candidates,
      ProjectControl projectControl) throws OrmException {
    // Get each reviewers activity based on number of reviews (weighted 1.5d),
    // number of comments (weighted 0.5d) and number of owned changes
    // (weighted 1d).
    Map<Account.Id, Double> reviewers = new HashMap<>();
  ...
  private void addReviewerWeightFromIndexQuery(
      ...
      Map<Account.Id, Double> reviewers,
    ...
    try {
      ...
      if (reviewers.containsKey(id)) {
        reviewers.put(id, reviewers.get(id) +
            (double) result.entities().size() * weight);
      } else {
        reviewers.put(id, (double) result.entities().size() * weight);
      }
    } catch (QueryParseException e) {
      reviewers.put(id, 0d);
    }
