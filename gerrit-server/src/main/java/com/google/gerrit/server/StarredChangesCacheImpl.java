...
public class StarredChangesCacheImpl implements StarredChangesCache {
  ...
  public static class StarredChangesChacheProvider
    ...
    private synchronized void createDefaultCache() {
      if (defaultStarredChangesCache != null) {
        return;
      }
    ...
    public void start() {
      ...
      if (cache == null) {
        ...
        // create default starred-changes cache which is loaded asynchronously
        createDefaultCache();
      }
    }
  ...
  private void loadAsync() {
    asyncLoader = workQueue.getDefaultQueue().submit(new Runnable() {
      @Override
      public void run() {
        load();
      }
  ...
  @Override
  public ImmutableSet<Change.Id> byAccount(Account.Id accountId,
      final String label) {
    ensureLoaded();
    return ImmutableSet.copyOf(
        Maps.filterEntries(cache.getMapByFirstKey(accountId),
  ...
  @Override
  public ImmutableSet<Account.Id> byChange(Change.Id changeId, final String label) {
    ensureLoaded();
    return ImmutableSet.copyOf(
        Maps.filterEntries(cache.getMapBySecondKey(changeId),
  ...
  @Override
  public ImmutableSet<Account.Id> unstarAll(Change.Id changeId) {
    ensureLoaded();
