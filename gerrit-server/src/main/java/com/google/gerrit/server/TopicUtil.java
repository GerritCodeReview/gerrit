...
public class TopicUtil {
  ...
  public static void abandon(final ChangeSet.Id changeSetId,
    ...
    if (updatedTopic == null) {
      ...
      //
      toAbandon = db.changes().byTopicOpenAll(topicId).toList();
    ...
    // Email the reviewers
    //
    // TODO Add Topic support to AbandonedSender
    // Meanwhile, sending emails in "behalf" of the last change of the topic
    //
    if (!toAbandon.isEmpty()) {
      final AbandonedSender cm = abandonedSenderFactory.create(toAbandon.get(toAbandon.size() - 1));
  ...
  public static void revert(final ChangeSet.Id changeSetId,
    ...
    // Email the reviewers
    //
    // TODO Add Topic support to RevertedSender
    // Meanwhile, sending emails in "behalf" of the last change of the topic
    //
    if (!toRevert.isEmpty()) {
      final RevertedSender cm = revertedSenderFactory.create(toRevert.get(toRevert.size() - 1));
      cm.setFrom(user.getAccountId());
      cm.setTopicMessage(tmsg);
      cm.send();
    }
  ...
  public static void restore(final ChangeSet.Id changeSetId,
    ...
    // Email the reviewers
    //
    // TODO Add Topic support to RestoredSender
    // Meanwhile, sending emails in "behalf" of the last change of the topic
    //
    if (lastChange != null) {
  ...
  throws OrmException, EmailException {
    ...
    // Email the reviewers
    //
    // TODO Add Topic support to AddReviewerSender
    // Meanwhile, sending emails in "behalf" of the last change of the topic
    //
