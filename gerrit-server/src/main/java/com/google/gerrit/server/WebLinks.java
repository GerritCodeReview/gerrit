...
public class WebLinks {
   ...
   */
  public FluentIterable<WebLinkInfo> getPatchSetLinks(
      final Project.NameKey project, final String commit) {
    return filterLinks(
        patchSetLinks,
        new Function<WebLink, WebLinkInfo>() {
   ...
   * @return Links for patch sets.
   */
  public FluentIterable<WebLinkInfo> getParentLinks(
      final Project.NameKey project, final String revision) {
    return filterLinks(
        parentLinks,
        new Function<WebLink, WebLinkInfo>() {
   ...
   */
  public FluentIterable<WebLinkInfo> getFileLinks(
  ...
  public FluentIterable<WebLinkInfo> getFileHistoryLinks(
    ...
    return filterLinks(
        ...
        new Function<WebLink, WebLinkInfo>() {
          ...
          @Override
          public WebLinkInfo apply(WebLink webLink) {
            return ((FileHistoryWebLink) webLink).getFileHistoryWebLink(project, revision, file);
          }
        });
  }
  ...
  public FluentIterable<WebLinkInfoCommon> getFileHistoryLinksCommon(
    ...
    return FluentIterable.from(fileHistoryLinks)
        .transform(
            new Function<WebLink, WebLinkInfoCommon>() {
              @Override
              public WebLinkInfoCommon apply(WebLink webLink) {
                WebLinkInfo info =
                    ((FileHistoryWebLink) webLink).getFileHistoryWebLink(project, revision, file);
                if (info == null) {
                  return null;
                }
  ...
  public FluentIterable<DiffWebLinkInfo> getDiffLinks(
    ...
    return FluentIterable.from(diffLinks)
        .transform(
            new Function<WebLink, DiffWebLinkInfo>() {
              @Override
              public DiffWebLinkInfo apply(WebLink webLink) {
                return ((DiffWebLink) webLink)
                    .getDiffLink(
                        project,
                        changeId,
                        patchSetIdA,
  ...
  private FluentIterable<WebLinkInfo> filterLinks(
      DynamicSet<? extends WebLink> links, Function<WebLink, WebLinkInfo> transformer) {
