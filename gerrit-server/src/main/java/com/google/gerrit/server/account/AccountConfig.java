...
public class AccountConfig extends VersionedMetaData implements ValidationError.Sink {
  ...
  private Account parse(Config cfg) {
    ...
    account.setFullName(cfg.getString(ACCOUNT, null, KEY_FULL_NAME));

    String preferredEmail = cfg.getString(ACCOUNT, null, KEY_PREFERRED_EMAIL);
    ...
    account.setStatus(cfg.getString(ACCOUNT, null, KEY_STATUS));
    return account;
  }
  ...
  protected boolean onSave(CommitBuilder commit) throws IOException, ConfigInvalidException {
    ...
    if (revision != null) {
      ...
      commit.setAuthor(new PersonIdent(commit.getCommitter(), registeredOn));
    }
