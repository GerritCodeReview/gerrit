...
public class AccountExternalIdCacheImpl implements AccountExternalIdCache {
  ...
  private static final String BY_ACCOUNT_EMAIL = "ext_id_account_email";
  ...
  protected static class AccountIdAndEmail {
    @Column(id = 1)
    ...
    protected AccountIdAndEmail() {
    }

    public AccountIdAndEmail(Account.Id id, String email) {
      this.id = id;
      this.email = email;
    }
  }
  ...
  public static Module module() {
    return new CacheModule() {
      ...
      protected void configure() {
        ...
        final TypeLiteral<Cache<AccountIdAndEmail, AccountExternalIdList>> byAccountEmailType =
            new TypeLiteral<Cache<AccountIdAndEmail, AccountExternalIdList>>() {};
        core(byAccountEmailType, BY_ACCOUNT_EMAIL).populateWith(
            ByAccountEmailLoader.class);
  ...
  private final Cache<AccountIdAndEmail, AccountExternalIdList> byAccountEmail;
  ...
  AccountExternalIdCacheImpl(
      ...
      @Named(BY_ACCOUNT_EMAIL) Cache<AccountIdAndEmail, AccountExternalIdList> byAccountEmail,
      @Named(BY_EMAIL) Cache<EmailWrapper, AccountExternalIdList> byEmail) {
    this.byAccountEmail = byAccountEmail;
    this.byKey = byKey;
  ...
  public List<AccountExternalId> byAccountEmail(Account.Id id, String email) {
    return byAccountEmail.get(new AccountIdAndEmail(id, email)).list;
  }
  ...
  public void evict(AccountExternalId id) {
    ...
    byAccountEmail.remove(new AccountIdAndEmail(id.getAccountId(), id
        .getEmailAddress()));
  ...
  static class ByAccountEmailLoader extends
      EntryCreator<AccountIdAndEmail, AccountExternalIdList> {
    private final SchemaFactory<ReviewDb> schema;

    @Inject
    ByAccountEmailLoader(SchemaFactory<ReviewDb> schema) {
      this.schema = schema;
    }

    @Override
    public AccountExternalIdList createEntry(AccountIdAndEmail idEmail)
        throws Exception {
      final ReviewDb db = schema.open();
      try {
        return new AccountExternalIdList(db.accountExternalIds()
            .byAccountEmail(idEmail.id, idEmail.email).toList());
      } finally {
        db.close();
      }
    }
