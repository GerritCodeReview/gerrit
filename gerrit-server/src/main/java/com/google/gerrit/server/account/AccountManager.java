...
public class AccountManager {
  ...
  public AuthResult authenticate(AuthRequest who) throws AccountException, IOException {
    ...
    try {
      try (ReviewDb db = schema.open()) {
        ExternalId id = findExternalId(who.getExternalId());
        if (id == null) {
    ...
    if (newEmail != null && !newEmail.equals(oldEmail)) {
      ...
      externalIdsUpdateFactory
          ...
          .replace(
              db,
              extId,
              ExternalId.create(extId.key(), extId.accountId(), newEmail, extId.plainPassword()));
    }
  ...
  private AuthResult create(ReviewDb db, AuthRequest who)
    ...
    ExternalId extId =
        ExternalId.createWithEmail(who.getExternalId(), newId, who.getEmailAddress());
  ...
  public AuthResult link(Account.Id to, AuthRequest who)
    ...
    try (ReviewDb db = schema.open()) {
      ExternalId extId = findExternalId(who.getExternalId());
      if (extId != null) {
        ...
        externalIdsUpdateFactory
            .create()
            .insert(db, ExternalId.createWithEmail(who.getExternalId(), to, who.getEmailAddress()));
    ...
    try (ReviewDb db = schema.open()) {
      Collection<ExternalId> filteredExtIdsByScheme =
          ExternalId.from(db.accountExternalIds().byAccount(to).toList())
              .stream()
              .filter(e -> e.key().isScheme(who.getExternalId().scheme()))
              .collect(toSet());
    ...
    try (ReviewDb db = schema.open()) {
      ...
      if (extId != null) {
        if (!extId.accountId().equals(from)) {
          throw new AccountException(
              "Identity '" + who.getExternalId().get() + "' in use by another account");
        }
        externalIdsUpdateFactory.create().delete(db, extId);
        ...
        throw new AccountException("Identity '" + who.getExternalId().get() + "' not found");
      }
