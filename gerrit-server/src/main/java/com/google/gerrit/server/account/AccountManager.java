    ...
    try {
      try (ReviewDb db = schema.open()) {
        ...
        if (id == null && who.getUserName() != null) {
          ...
          if (existingId != null) {
            log.info("User already has an account, ink new identity to the existing account.");
            // we detected inconsistency in the database, having a record for scheme "username:"
            // but no record for scheme "gerrit:" and that we are trying to recover, by linking
