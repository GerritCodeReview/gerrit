...
public class DeleteExternalIds implements
  ...
  public Response<?> apply(AccountResource resource, List<String> externalIds)
    ...
    Map<AccountExternalId.Key, AccountExternalId> externalIdMap =
        getAccountExternalIds(resource.getUser());
  ...
  private Map<AccountExternalId.Key, AccountExternalId> getAccountExternalIds(
    ...
    Collection<AccountExternalId> idList =
        externalIdCache.byAccount(user.getAccountId());

    for (AccountExternalId id : idList) {
      if (id.isScheme(SCHEME_USERNAME)) {
        id.setCanDelete(false);
      } else {
        AccountExternalId.Key last = user.getLastLoginExternalIdKey();
        id.setCanDelete(
            (last == null) || (!last.get().equals(id.getExternalId())));
      }

      idMap.put(id.getKey(), id);
    }
    return idMap;
  }
}
