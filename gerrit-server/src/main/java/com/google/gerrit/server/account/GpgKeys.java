...
public class GpgKeys implements
  ...
  private final ListGpgKeys list;
    ...
    try (PublicKeyStore store = storeProvider.get()) {
      ...
      for (PGPPublicKeyRing keyRing : store.get(keyId)) {
        ...
        if (Arrays.equals(key.getFingerprint(), fp)) {
          return new AccountResource.GpgKey(parent.getUser(), key);
        }
      }
    }
  ...
  static GpgKeyInfo toJson(PGPPublicKey key) throws IOException {
