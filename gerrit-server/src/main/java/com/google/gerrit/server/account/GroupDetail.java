...
public class GroupDetail {
  ...
  public List<Account.Id> getDirctMembers() {
    return groupBackend.loadMembers(groupUUID);
  }

  public List<AccountGroup.UUID> getDirectIncludes() {
    return groupBackend.loadIncludes(groupUUID, project);
  }
  ...
  private Set<Account> getGroupMembers(final AccountGroup.UUID groupUUID,
    ...
    List<Account.Id> m = groupBackend.loadMembers(groupUUID);
    if (m != null && !m.isEmpty()) {
      for (final Account.Id member : m) {
        members.add(accountCache.get(member).getAccount());
      }
    }
    List<AccountGroup.UUID> gs = groupBackend.loadIncludes(groupUUID, project);
