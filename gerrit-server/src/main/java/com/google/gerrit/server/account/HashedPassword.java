...
public class HashedPassword {
  ...
  /**
   * The decode method decodes a hashed password encoded with {@link #encode}. It throws
   * DecoderException for malformed input.
   */
  public static HashedPassword decode(String encoded) throws DecoderException {
    if (!encoded.startsWith(ALGORITHM + ":")) {
      throw new DecoderException("unrecognized algorithm in "  + encoded);
    }

    String[] fields = encoded.split(":");
    Preconditions.checkState(fields.length == 4);
    if (fields.length != 4) {
      throw new DecoderException("want 4 fields in "  + encoded);
    }
