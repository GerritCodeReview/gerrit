...
class StarredChanges implements
  ...
  static class Create implements RestModifyView<AccountResource, EmptyInput> {
    ...
    @Override
    public Response<ChangeInfo> apply(AccountResource rsrc, EmptyInput in)
      ...
      try {
        ...
        return Response.ok(json.format(change.getChange()));
      }
      return Response.created(json.format(change.getChange()));
    }
  }
  ...
  static class Put implements
    ...
    @Inject
    Put(Provider<CurrentUser> self, ChangeJson json) {
      this.self = self;
      this.json = json;
    }

    @Override
    public ChangeInfo apply(AccountResource.StarredChange rsrc, EmptyInput in)
        throws AuthException, OrmException {
      if (self.get() != rsrc.getUser()) {
        throw new AuthException("not allowed update starred changes");
      }
      return json.format(rsrc.getChange());
    }
  }
  ...
  static class Delete implements
    ...
    @Override
    public Response<Object> apply(AccountResource.StarredChange rsrc,
