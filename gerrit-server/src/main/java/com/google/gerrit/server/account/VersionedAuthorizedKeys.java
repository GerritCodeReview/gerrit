...
public class VersionedAuthorizedKeys extends VersionedMetaData {
  ...
  public static class Accessor {
    ...
    public AccountSshKey addKey(Account.Id accountId, String pub)
      ...
      VersionedAuthorizedKeys authorizedKeys = open(accountId);
      try {
        AccountSshKey key = authorizedKeys.addKey(pub);
        commit(authorizedKeys);
        return key;
      } finally {
        authorizedKeys.git.close();
      }
    }
  ...
  protected boolean onSave(CommitBuilder commit) throws IOException {
    ...
    saveUTF8(AuthorizedKeys.FILE_NAME, AuthorizedKeys.serialize(keys.values()));
    return true;
  }
   ...
   */
  public AccountSshKey addKey(String pub) throws InvalidSshKeyException {
    checkState(keys != null, "SSH keys not loaded yet");
   ...
   */
  public boolean markKeyInvalid(int seq) {
    checkState(keys != null, "SSH keys not loaded yet");
