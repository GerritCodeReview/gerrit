...
public class WatchConfig extends VersionedMetaData implements AutoCloseable {
  ...
  protected boolean onSave(CommitBuilder commit)
    ...
    for (Map.Entry<ProjectWatchKey, Set<NotifyType>> e : projectWatches
      ...
      List<NotifyType> notifyTypes = new ArrayList<>(e.getValue());
      Collections.sort(notifyTypes, new Comparator<NotifyType>() {
        @Override
        public int compare(NotifyType nt1, NotifyType nt2) {
          return nt1.name().compareTo(nt2.name());
        }});
      StringBuilder notifyValue = new StringBuilder();
      notifyValue.append(filter != null ? filter : AccountProjectWatch.FILTER_ALL)
          .append(" [")
