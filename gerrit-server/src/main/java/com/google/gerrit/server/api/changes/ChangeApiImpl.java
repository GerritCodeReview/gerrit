...
import com.google.gerrit.server.change.GetPrivate;
...
class ChangeApiImpl implements ChangeApi {
  ...
  private final GetPrivate getPrivate;
  ...
  @Override
  public boolean isPrivate() throws RestApiException {
    Response<String> result = getPrivate.apply(change);
    return result.statusCode() == SC_OK && result.value().equals("ok");
  }

  @Override
  public void setPrivate(boolean isPrivate) throws RestApiException {
    try {
      if (isPrivate) {
        putPrivate.apply(change, new PutPrivate.Input());
      } else {
        deletePrivate.apply(change, new DeletePrivate.Input());
      }
    } catch (UpdateException e) {
      throw new RestApiException("Cannot set change to be private", e);
    }
  }
