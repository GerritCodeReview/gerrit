...
public class UniversalAuthBackend implements AuthBackend {
  ...
  public AuthUser authenticate(final AuthRequest request) throws AuthException {
    ...
    if (isEmpty(authResults)) {
      String msg =
          String.format("Cannot find credentials for user %s",
              request.getUsername());
      throw new MissingCredentialsException(msg);
    } else if (size(authResults) > 1) {
      String msg = getAmbiguosAuthExceptionMessage(authResults);
  ...
  private String getAmbiguosAuthExceptionMessage(Iterable<AuthUser> authResults) {
