...
@Singleton class Helper {
  ...
  List<LdapQuery.Result> findGroup(final Helper.LdapSchema schema,
    ...
    final HashMap<String, String> params = new HashMap<>();
    params.put(LdapRealm.GROUPNAME, groupName);

    final List<LdapQuery.Result> res = new ArrayList<>();
    for (LdapQuery query : schema.getGroupQueryList(new HashSet<>(
        schema.groupName.getParameterNames()))) {
      res.addAll(query.query(ctx, params));
    }
    return res;
  }
  ...
  class LdapSchema {
    ...
    List<LdapQuery> getGroupQueryList(final Set<String> returnAttributes) {
      for (LdapQuery q : groupQueryList) {
        q.setReturnAttributes(returnAttributes);
      }
