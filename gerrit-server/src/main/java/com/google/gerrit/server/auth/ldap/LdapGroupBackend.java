...
public class LdapGroupBackend implements GroupBackend {
  ...
  LdapGroupBackend(
      Helper helper,
      @Named(GROUP_CACHE) Cache<String, Set<AccountGroup.UUID>> membershipCache,
  ...
  private static String cnFor(String dn) {
    try {
      Enumeration<String> parts = new LdapName(dn).getAll();
      while (parts.hasMoreElements()) {
        String part = parts.nextElement();
        if ((part.length() >=3) && part.substring(0, 3).equalsIgnoreCase("cn=")) {
  ...
  private boolean exists(String groupDn) {
    // TODO(cranger): do a cache lookup
    try {
      final DirContext ctx = helper.open();
      try {
        final Name compositeGroupName = new CompositeName().add(groupDn);
        final Attribute in = ctx.getAttributes(compositeGroupName).get("cn");
        return in.size() != 0;
      } finally {
        try {
          ctx.close();
      ...
      log.warn("Cannot query LDAP for group", e);
    }
    return false;
  }
  ...
  private Set<GroupReference> suggestLdap(String name) {
    ...
    try {
      ...
      try {
        ...
        // TODO(cranger): should this have * appended?
