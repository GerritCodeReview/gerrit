...
/**
 * Implementation of GroupBackend for the internal group system, which includes
 * group membership checks with LDAP data.
 */
public class LdapGroupBackend implements GroupBackend {
  ...
  private static boolean isLdapUUID(AccountGroup.UUID uuid) {
    return uuid.get().startsWith("ldapgroup:");
  }
  ...
  private static AccountGroup.ExternalNameKey externalNameKey(AccountGroup.UUID uuid) {
    ...
    return new AccountGroup.ExternalNameKey(uuid.get().substring(LDAP_GROUP.length()));
  }
  ...
  private GroupReference groupReference(LdapQuery.Result res) throws NamingException {
    ...
    return new GroupReference(
        new AccountGroup.UUID(LDAP_GROUP + res.getDN()), "ldapgroup/" + name);
  }
  ...
  public Set<GroupReference> suggest(String name) {
    Set<GroupReference> groups = suggestLdap(name);
    groups.addAll(internalBackend.suggest(name));
    return groups;
  }
  ...
  private Set<GroupReference> suggestLdap(String name) {
    Set<GroupReference> out = Sets.newHashSet();
    try {
      ...
      try {
        ...
        for (String groupBase : schema.groupBases) {
          LdapQuery query = new LdapQuery(
              groupBase, schema.groupScope, filter, Collections.<String>emptySet());
