...
public class LdapRealmBackend implements RealmBackend {
  ...
  private LoadingCache<String, Set<com.google.gerrit.reviewdb.client.AccountGroup.UUID>> membershipCache;
  ...
  public boolean handles(UUID uuid) {
    return uuid.get().startsWith(DOMAIN);
  }
  ...
  public UserData getUserData(AuthUser user) {
    ...
    try {
      ...
      try {
        ...
        userData.setExternalId(user.getUUID().get());
