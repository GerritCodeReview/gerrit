...
public class OAuthTokenCache {
  ...
  public OAuthToken get(String userName) {
    OAuthToken accessToken = decrypt(cache.getIfPresent(userName));
  ...
  public void put(OAuthUserInfo user, OAuthToken accessToken) {
    if (user != null) {
      cache.put(user.getUserName(), encrypt(accessToken));
    }
  }
  ...
  private OAuthToken encrypt(OAuthToken token) {
    if (token == null || encrypter.get() == null) {
      return token;
    }
    return encrypter.get().encrypt(token);
  }
  ...
  private OAuthToken decrypt(OAuthToken token) {
    ...
    return encrypter.get().decrypt(token);
  }
}
