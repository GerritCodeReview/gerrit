      ...
      public void run() {
        for (DynamicMap.Entry<Cache<?, ?>> e : cacheMap) {
          ...
          if (c instanceof PersistentCache) {
            perMemEnt.set(metricNameOf(e), c.size());
            perMemHit.set(metricNameOf(e), Math.round(c.stats().hitRate() * 100));
            PersistentCache.DiskStats d =
                ((PersistentCache) c).diskStats();
            perDiskEnt.set(metricNameOf(e), d.size());
            perDiskHit.set(metricNameOf(e), hitRatio(d));
          } else {
            memEnt.set(metricNameOf(e), c.size());
            memHit.set(metricNameOf(e), Math.round(c.stats().hitRate() * 100));
          }
        }
      }
    });
