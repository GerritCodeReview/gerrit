...
public class Abandon implements RestModifyView<ChangeResource, AbandonInput>,
  ...
  private static boolean currentUserEquals(CurrentUser a, CurrentUser b) {
    ...
    if (a.isInternalUser() && b.isInternalUser()) {
      if (a instanceof PluginUser && b instanceof PluginUser) {
        return a.getUserName().equals(b.getUserName());
      }
      return a instanceof PluginUser == b instanceof PluginUser;
    }
    if (a.isInternalUser() != b.isInternalUser()) {
      return false;
    }
    return a instanceof AnonymousUser == b instanceof AnonymousUser;
  }
}
