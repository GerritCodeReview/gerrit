  ...
  public void postUpdate(Context ctx) throws OrmException, NoSuchChangeException {
    ...
    if (runHooks) {
      ...
      if (!approvals.isEmpty()) {
        ...
        Map<String, Short> approvalsStatus = new HashMap<>();
        for (LabelType lt : labels){
          allApprovals.put(lt.getName(), (short) 0);
//          approvalsStatus.put(lt.getName(), (short)0);
          approvalsStatus.put(lt.getName(), null);
        }
        for (Map.Entry<String, Short> entry : approvals.entrySet()) {
          if (entry.getValue() != 0) {
            allApprovals.put(entry.getKey(), entry.getValue());
//            approvalsStatus.put(entry.getKey(), (short)1);
            approvalsStatus.put(entry.getKey(), (short)0);
          }
        }
