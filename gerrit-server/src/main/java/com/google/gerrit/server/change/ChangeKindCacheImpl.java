...
public class ChangeKindCacheImpl implements ChangeKindCache {
  ...
  private static class Loader implements Callable<ChangeKind> {
    ...
    public ChangeKind call() throws IOException {
      ...
      if (rw == null) {
        @SuppressWarnings("resource") // Manually closed in finally block.
        Repository r = repoManager.openRepository(projectName);
        repo = r;
        rw = new RevWalk(repo);
  ...
  private static ChangeKind getChangeKindInternal(
    ...
    if (patch.getId().get() > 1) {
      try (Repository repo = repoManager.openRepository(change.getProject());
        ...
        kind = getChangeKindInternal(cache, rw, repo.getConfig(), changeDataFactory.create(db, change), patch);
      } catch (IOException e) {
        // Do nothing; assume we have a complex change
        log.warn(
            "Unable to get change kind for patchSet "
