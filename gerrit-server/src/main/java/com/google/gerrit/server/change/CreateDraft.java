...
class CreateDraft implements RestModifyView<RevisionResource, Input> {
  ...
  public Response<CommentInfo> apply(RevisionResource rsrc, Input in)
    ...
    } else if ((in.line == null || in.line == 0) && in.range != null) {
      throw new BadRequestException("range must be null if line is null or 0");
    }

    int line;
    if (in.range != null || in.line != null) {
      line = in.range != null ? in.range.getEndLine() : in.line;
    } else {
      line = 0;
    }
