...
public class CreateDraftComment implements RestModifyView<RevisionResource, DraftInput> {
  ...
  public Response<CommentInfo> apply(RevisionResource rsrc, DraftInput in)
    ...
    try (BatchUpdate bu =
      ...
      bu.execute();
      return Response.created(commentJson.get().setFillAccounts(false).format(op.comment));
    }
  }
  ...
  private class Op extends BatchUpdate.Op {
    ...
    public boolean updateChange(ChangeContext ctx) throws ResourceNotFoundException, OrmException {
      ...
      comment =
          new PatchLineComment(
              new PatchLineComment.Key(
                  new Patch.Key(ps.getId(), in.path), ChangeUtil.messageUUID(ctx.getDb())),
              line,
              ctx.getAccountId(),
      ...
      plcUtil.putComments(ctx.getDb(), ctx.getUpdate(psId), Collections.singleton(comment));
