...
public class DeletePrivate implements RestModifyView<ChangeResource, DeletePrivate.Input>,
  ...
  @Inject
  DeletePrivate(Provider<ReviewDb> dbProvider,
  ...
  public Response<String> apply(ChangeResource rsrc, DeletePrivate.Input input)
    ...
    if (!self.get().getAccountId().equals(rsrc.getChange().getOwner())) {
      throw new AuthException("not allowed to delete private flag");
    }
  ...
  public Description getDescription(ChangeResource rsrc) {
    boolean canMarkPrivate = false;
    try {
      canMarkPrivate = rsrc.getControl().isOwner() ||
          rsrc.getControl().isReviewer(dbProvider.get());
    } catch (OrmException e) {
      log.error("Cannot check canMarkPrivate status. Assuming false.", e);
    }
    return new UiAction.Description()
        .setLabel("Unmark private")
        .setTitle("Unmark change as private")
        .setVisible(canMarkPrivate && rsrc.getChange().isPrivate());
  }
}
