...
public class DeleteReviewer implements RestModifyView<ReviewerResource, Input> {
  ...
  private class Op extends BatchUpdate.Op {
    ...
    @Override
    public boolean updateChange(ChangeContext changeCtx)
        throws AuthException, ResourceNotFoundException, OrmException {
      Account.Id reviewerId = reviewer.getId();
      currChange = changeCtx.getChange();
      currPs = psUtil.current(dbProvider.get(), changeCtx.getNotes());

      newApprovals = new HashMap<>();
      oldApprovals = new HashMap<>();
      LabelTypes labelTypes = changeCtx.getControl().getLabelTypes();
      // removing a reviewer will remove all her votes
      for (LabelType lt : labelTypes.getLabelTypes()) {
        newApprovals.put(lt.getName(), (short) 0);
      }

      del = Lists.newArrayList();
      StringBuilder msg = new StringBuilder();
      for (PatchSetApproval a : approvals(changeCtx, reviewerId)) {
        if (changeCtx.getControl().canRemoveReviewer(a)) {
          del.add(a);
      ...
      changeCtx.getDb().patchSetApprovals().delete(del);
      ChangeUpdate update = changeCtx.getUpdate(currPs.getId());
      update.removeReviewer(reviewerId);

      changeMessage = new ChangeMessage(
          new ChangeMessage.Key(currChange.getId(),
              ChangeUtil.messageUUID(changeCtx.getDb())),
          changeCtx.getUser().getAccountId(), changeCtx.getWhen(),
          currPs.getId());
      changeMessage.setMessage(msg.toString());
      cmUtil.addChangeMessage(changeCtx.getDb(), update, changeMessage);
