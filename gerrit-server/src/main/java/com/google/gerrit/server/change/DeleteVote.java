...
public class DeleteVote implements RestModifyView<VoteResource, Input> {
  ...
  public Response<?> apply(VoteResource rsrc, Input input)
    ...
    try (BatchUpdate bu = batchUpdateFactory.create(db.get(),
          rsrc.getReviewer().getChange(), ctl.getUser(), TimeUtil.nowTs())) {
      bu.addOp(
          rsrc.getReviewer().getChange().getId(),
          new Op(rsrc.getReviewer().getUser().getAccountId(), rsrc.getLabel()));
      bu.execute();
    }
  ...
  private class Op extends BatchUpdate.Op {
    private final Account.Id acountId;
    private final String label;

    private Op(Account.Id acountId, String label) {
    ...
    public void updateChange(ChangeContext ctx)
        throws OrmException, ResourceConflictException, AuthException,
