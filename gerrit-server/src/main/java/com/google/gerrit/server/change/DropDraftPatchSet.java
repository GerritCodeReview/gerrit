...
public class DropDraftPatchSet implements RestModifyView<RevisionResource, Input>,
  ...
  public Object apply(RevisionResource rsrc, Input input)
    ...
    // if the last ps was deleted, delete the change
    if (dbProvider.get().patchSets().byChange(
            change.getId()).toList().size() == 0) {
  ...
  private PatchSetInfo previousPatchSetInfo(final PatchSet.Id patchSetId)
      throws PatchSetInfoNotAvailableException {
    PatchSet.Id id =
        new PatchSet.Id(patchSetId.getParentKey(), patchSetId.get() - 1);
    return patchSetInfoFactory.get(dbProvider.get(), id);
  }
