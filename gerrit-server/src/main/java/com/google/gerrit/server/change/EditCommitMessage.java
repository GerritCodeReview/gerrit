...
class EditCommitMessage implements RestModifyView<RevisionResource, Input>,
  ...
  @Inject
  EditCommitMessage(final Provider<ReviewDb> dbProvider,
  ...
  public ChangeInfo apply(RevisionResource rsrc, Input input)
    ...
    try {
      git = gitManager.openRepository(rsrc.getControl()
    ...
    try {
      return json.format(ChangeUtil.editCommitMessage(patchSetId, rsrc
          .getControl().getRefControl(), (IdentifiedUser) rsrc.getControl()
          .getCurrentUser(), input.message, dbProvider.get(),
