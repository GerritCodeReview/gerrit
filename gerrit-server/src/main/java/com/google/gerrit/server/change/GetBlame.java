...
public class GetBlame implements RestReadView<FileResource> {
  ...
  private static class BlameLine {
    public BlameInfo.Meta meta;
    public int lineNum;
  }
  ...
  private List<BlameLine> blame(ObjectId id, String path,
  ...
  private static void addLines(List<BlameLine> result,
  ...
  private static List<BlameInfo.Blame> packBlames(
    ...
    for (int i = 1; i <= numLines; ++i) {
      BlameLine current = blames.get(i - 1);
      int j = i;
      BlameLine next = null;
      do {
        j = j + 1;
