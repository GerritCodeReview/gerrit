...
public class GetRelated implements RestReadView<RevisionResource> {
  ...
  private static GitRepositoryManager gitMgrStatic;
  private final Provider<ReviewDb> dbProvider;
  private static Change changeStatic;
  ...
  private static IncludedInDetail resolveBranches(RevCommit commit) throws Exception {
    Repository git = gitMgrStatic.openRepository(changeStatic.getProject());
    RevWalk rw = new RevWalk(git);
  ...
  public static class ChangeAndCommit {
    ...
    ChangeAndCommit(@Nullable Change change, @Nullable PatchSet ps, RevCommit c) {
      ...
      IncludedInDetail _detail = new IncludedInDetail();
      try {
       _detail = resolveBranches(c);
       } catch (Exception e) {
       e.printStackTrace();
     }
     branches = _detail.getBranches();
    }
  }
}
