...
public class GetRevisionActions implements ETagView<RevisionResource> {
  ...
  private void buildETag(Hasher h,
    ...
    if (view instanceof HasETag) {
      ...
      h.putUnencodedChars(Strings.nullToEmpty(d.getTitle()))
  ...
  private static void hashViewId(Hasher h,
      DynamicMap.Entry<RestView<RevisionResource>> e) {
    h.putUnencodedChars(e.getPluginName()).putByte((byte) 0)
     .putUnencodedChars(e.getExportName()).putByte((byte) 0);
  }
}
