...
public class HashtagsUtil {
  ...
  public Set<String> parseCommitMessageHashtags(RevCommit commit,
      ChangeControl ctl) {
    Set<String> toAdd = detectCommitMessageHashtags(commit);
    try {
      if (toAdd.isEmpty()) {
        ...
        if (toAdd.isEmpty()) {
          for (HashtagValidationListener validator : hashtagValidationListeners) {
            validator.validateHashtags(ctl.getChange(), toAdd, null);
          }
        }
      }
  ...
  public Set<String> getHashtagsInCurrentPSCommitMessage(Change change)
    ...
    return detectCommitMessageHashtags(rw.parseCommit(objectId));
  }
}
