...
@Singleton
public class Ignore implements RestModifyView<ChangeResource, Ignore.Input>, UiAction<ChangeResource> {
  ...
  public Object apply(ChangeResource rsrc, Input input)
    ...
    try {
      stars.star(self.get().getAccountId(), rsrc.getProject(), rsrc.getChange().getId(),
          ImmutableSet.of("ignore"), ImmutableSet.of());
  ...
  private boolean isIgnored(ChangeResource rsrc) {
    try {
      return stars.byChange(rsrc.getChange().getId(), "ignore").contains(self.get().getAccountId());
    } catch (OrmException e) {
