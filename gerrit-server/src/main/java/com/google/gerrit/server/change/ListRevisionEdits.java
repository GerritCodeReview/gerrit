...
class ListRevisionEdits implements RestReadView<ChangeResource> {
  private static final Logger log =
      LoggerFactory.getLogger(ListRevisionEdits.class);

  // Factory is needed to break circular dependency between
  // ListRevisionEdits and RevisionEdits
  interface Factory {
    ListRevisionEdits create();
  }

  private final RevisionEdits revisionEdits;
  private final RevisionEditUtil editUtil;
  private final AccountByEmailCache byEmailCache;
  private final Provider<CurrentUser> userProvider;

  @Inject
  ListRevisionEdits(RevisionEdits revisionEdits,
      RevisionEditUtil editUtil,
      AccountByEmailCache byEmailCache,
      Provider<CurrentUser> userProvider) {
    this.revisionEdits = revisionEdits;
  ...
  public Map<String, RevisionInfo> apply(ChangeResource rsrc)
    ...
    Optional<RevisionEdit> edit = editUtil.byChange(rsrc.getChange());
    if (edit.isPresent()) {
      RevisionEdit src = edit.get();
      res.put(src.getRevision().get(), toRevisionInfo(rsrc, src));
    }
    return res;
  }

  private RevisionInfo toRevisionInfo(ChangeResource rsrc, RevisionEdit edit) {
    RevisionInfo out = new RevisionInfo();
    // Mark revision edit with unique number 0.
    // We can do that, as we have max. 1 revision edit
    // per change per user.
    out._number = 0;
    ...
    for (UiAction.Description d : UiActions.from(
        revisionEdits,
        new RevisionEditResource(rsrc, edit),
        userProvider)) {
    ...
    try {
      ...
      log.warn("Cannot load revision edit fot change: {}",
  ...
  private CommitInfo toCommit(RevisionEdit edit)
