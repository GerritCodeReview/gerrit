...
public class MergeabilityCache {
  ...
  public static class EntryKey implements Serializable {
    ...
    public EntryKey(ObjectId commit, ObjectId into, SubmitType submitType,
      ...
      checkArgument((dest == null && repo == null)
          || (dest != null && repo != null),
          "must specify both repo and dest, or neither");
      this.dest = dest;
      this.repo = repo;
      this.db = db;
    }
  ...
  public static class Loader extends CacheLoader<EntryKey, Boolean> {
    ...
    public Boolean load(EntryKey key)
      ...
      checkArgument(key.dest != null && key.repo != null && key.db != null,
          "Key cannot be loaded without destination, repo, and db: %s", key);
      if (key.into.equals(ObjectId.zeroId())) {
        return true; // Assume yes on new branch.
      }
      try {
        Map<String, Ref> refs = key.repo.getAllRefs();
        RevWalk rw = CodeReviewCommit.newRevWalk(key.repo);
        try {
