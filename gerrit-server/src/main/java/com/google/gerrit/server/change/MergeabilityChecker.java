...
public class MergeabilityChecker implements GitReferenceUpdatedListener {
  ...
  public void update(Change change) throws IOException {
    try {
      new ChangeUpdateTask(schemaFactory, change).call();
  ...
  private class ChangeUpdateTask implements Callable<Void> {
    ...
    public Void call() throws Exception {
      ...
      try {
        ...
        if (info.mergeable != change.isMergeable()) {
          change.setMergeable(info.mergeable);
          indexer.indexAsync(db.changes().get(change.getId()));
        }
        return null;
      } finally {
        tl.setContext(old);
