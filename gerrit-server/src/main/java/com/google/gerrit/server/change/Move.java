...
public class Move implements RestModifyView<ChangeResource, MoveInput> {
  ...
  public ChangeInfo apply(ChangeResource req, MoveInput input)
      throws AuthException, RestApiException,
      ResourceConflictException, OrmException, IOException {
    ReviewDb db = dbProvider.get();
    ChangeControl control = req.getControl();
    Change change = req.getChange();
    if (!control.canMoveTo(RefNames.fullName(input.destination))) {
    ...
    try (  Repository repo = repoManager.openRepository(projectKey);
      ...
      if (refId == null) {
        throw new ResourceConflictException("Destination " + input.destination +
    ...
    if (!destChanges.isEmpty()) {
      ...
      throw new ResourceConflictException("Destination " + destKey.getShortName()
