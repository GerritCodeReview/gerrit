...
public class PostVerification implements RestModifyView<RevisionResource, VerifyInput> {
  ...
  public Response<?> apply(RevisionResource revision, VerifyInput input)
    ...
    if (input.jobs == null) {
      throw new BadRequestException("verification must not be null");
    }
    ...
    try {
      ...
      dirty |= updateLabels(revision, input.jobs);
      if (dirty) {
  ...
  private boolean updateLabels(RevisionResource rsrc,
      Map<String, VerificationInfo> jobs) throws OrmException {
    Preconditions.checkNotNull(jobs);

    List<PatchSetVerification> ups = Lists.newArrayList();
    Map<String, PatchSetVerification> current = scanLabels(rsrc);

    for (Map.Entry<String, VerificationInfo> ent : jobs.entrySet()) {
      String name = ent.getKey();
      PatchSetVerification c = current.remove(name);
      if (c != null) {
        if (c.getValue() != ent.getValue().value) {
          c.setValue(ent.getValue().value);
        ...
        c = new PatchSetVerification(new PatchSetVerification.Key(
                ...
                new LabelId(name)),
            ent.getValue().value, TimeUtil.nowTs());
        c.setGranted(timestamp);
        c.setUrl(ent.getValue().url);
        ups.add(c);
      }
  ...
  private Map<String, PatchSetVerification> scanLabels(RevisionResource rsrc)
      throws OrmException {
