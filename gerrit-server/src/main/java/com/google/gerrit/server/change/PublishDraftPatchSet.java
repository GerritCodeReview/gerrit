...
public class PublishDraftPatchSet implements RestModifyView<RevisionResource, Input>,
  ...
  public static class CurrentRevision implements
    ...
    public Response<?> apply(ChangeResource rsrc, Input input)
        throws RestApiException, UpdateException {
      return publish.apply(rsrc.getControl().getCurrentUser(), rsrc.getChange(),
  ...
  private class Op extends BatchUpdate.Op {
    ...
    public void updateRepo(RepoContext ctx)
      ...
      if (ps == null) {
        ...
        // just the revision, which is immutable.
    ...
    private void addReviewers(ChangeContext ctx) throws OrmException {
      ...
      recipients.remove(accountId(ctx));
    ...
    private void sendReplacePatchSet(Context ctx)
      ...
      ChangeMessage msg =
          new ChangeMessage(new ChangeMessage.Key(change.getId(),
              ChangeUtil.messageUUID(ctx.getDb())), accountId(ctx),
              ctx.getWhen(), psId);
    ...
    private Account.Id accountId(Context ctx) {
      return ((IdentifiedUser) ctx.getUser()).getAccountId();
    }
  }
}
