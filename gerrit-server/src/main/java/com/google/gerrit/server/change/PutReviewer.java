...
import com.google.gerrit.server.change.PutReviewer.Input;
...
class PutReviewer implements RestModifyView<ReviewerResource, Input> {
  ...
  @Inject
  PutReviewer(AddReviewerSender.Factory addReviewerSenderFactory,
      ...
      ApprovalCategory.Id addReviewerCategoryId,
      Config cfg,
      ChangeHooks hooks,
    ...
    this.addReviewerCategoryId = addReviewerCategoryId;
    this.cfg = cfg;
    this.hooks = hooks;
  ...
  @Override
  public PutResult apply(ReviewerResource rsrc, Input input)
    ...
    if (rsrc.getAccount() != null) {
      return putAccount(rsrc);
    } else {
  ...
  private PutResult putAccount(ReviewerResource rsrc) throws OrmException,
    ...
    addReviewers(rsrc, result, ImmutableSet.of(rsrc.getAccount()));
    return result;
  }

  private PutResult putGroup(ReviewerResource rsrc, Input input)
      throws ResourceNotFoundException, AuthException, NoSuchGroupException,
      NoSuchProjectException, OrmException, NoSuchChangeException,
      EmailException {
    AccountGroup group =
        groupCache.get(new AccountGroup.NameKey(rsrc.getOriginalId()));
    if (group == null) {
      throw new ResourceNotFoundException(rsrc.getOriginalId());
    }
    PutResult result = new PutResult();
    ...
    Set<Account> reviewers = Sets.newLinkedHashSet();
  ...
  private void addReviewers(ChangeResource rsrc, PutResult result,
      Set<Account> reviewers) throws OrmException, EmailException,
      NoSuchChangeException {
    result.reviewers = Lists.newArrayListWithCapacity(reviewers.size());
    for (Account account : reviewers) {
      ReviewerInfo info = addReviewer(rsrc, account);
      if (info != null) {
  ...
  private ReviewerInfo addReviewer(ChangeResource rsrc, Account reviewer)
      throws OrmException, NoSuchChangeException {
    Change change = rsrc.getChange();
    PatchSet.Id psid = change.currentPatchSetId();
    if (exists(psid, reviewer.getId())) {
      return null;
    }
    ...
    List<PatchSetApproval> toInsert = ImmutableList.of(
        dummyApproval(change, psid, reviewer.getId()));
    db.get().patchSetApprovals().insert(toInsert);
    ReviewerInfo info = new ReviewerInfo(reviewer.getId());
    return json.format(info, rsrc.getChange(), toInsert);
  }
