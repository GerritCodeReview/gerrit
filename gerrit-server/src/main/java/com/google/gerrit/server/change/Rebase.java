...
public class Rebase implements RestModifyView<RevisionResource, RebaseInput>,
  ...
  public ChangeInfo apply(RevisionResource rsrc, RebaseInput input)
    ...
    } else if (!canRebase(rsrc.getPatchSet().getId())) {
      throw new ResourceConflictException("cannot rebase merge commits");
    }

    String baseRev = null;
    if (input != null && input.base != null && input.base.trim().length() > 0) {
      PatchSet basePatchSet = parseBase(input.base);
      if (basePatchSet == null) {
        throw new ResourceConflictException("base revision is missing: " + input.base);
      } else if (!rsrc.getControl().isPatchVisible(basePatchSet, db)) {
        throw new AuthException("base revision not accessible: " + input.base);
      } else if (change.getId().equals(basePatchSet.getId().getParentKey())) {
        // only depend on itself and no other patch sets ==> remove existing deps
  ...
  private boolean canRebase(final PatchSet.Id patchSetId) {
    try {
      // prevent rebase of exotic changes (merge commit, no ancestor).
      return (dbProvider.get().patchSetAncestors().ancestorsOf(patchSetId).toList().size() == 1);
  ...
  private PatchSet parseBase(final String base) throws OrmException {
    ...
    if (basePatchSetId != null) {
      ...
      } catch (Exception e) {  // IAE in Gerrit 2.9
      }
    }
    ...
    } catch (Exception e) {  // IAE in Gerrit 2.9, NumberFormatException in Gerrit 2.11
    }
  ...
  public UiAction.Description getDescription(RevisionResource resource) {
    return new UiAction.Description()
      ...
      .setVisible(resource.getChange().getStatus().isOpen()
          ...
          && canRebase(resource.getPatchSet().getId()));
  }
