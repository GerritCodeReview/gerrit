...
public interface ReviewerSuggestion {
   ...
   */
  List<SuggestedReviewer> suggestReviewers(
      Change.Id changeId, String query, Set<Account.Id> candidates);

  /**
   * Get the weight that all scores from this plugin will be multiplied with.
   * Plugins should make this configurable for the user using the plugin config.
