...
public class Reviewers implements
  ...
  public ReviewerResource parse(ChangeResource changeResource, String id)
    ...
    for (Account.Id reviewerId : fetchAccountIds(changeResource)) {
      if (reviewerId.equals(accountId)) {
  ...
  private List<Account.Id> fetchAccountIds(ChangeResource changeResource)
      throws OrmException {
    ReviewDb db = dbProvider.get();
    Change.Id changeId = changeResource.getChange().getId();
    List<Account.Id> accountIds = new ArrayList<Account.Id>();
