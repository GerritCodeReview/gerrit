...
public class Submit implements RestModifyView<RevisionResource, SubmitInput>,
  ...
  public ChangeMessage getConflictMessage(RevisionResource rsrc)
      throws OrmException {
    Optional<ChangeMessage> result =
        FluentIterable.from(cmUtil.byPatchSet(dbProvider.get(), rsrc.getNotes(),
            rsrc.getPatchSet().getId())).filter(new Predicate<ChangeMessage>() {
              @Override
