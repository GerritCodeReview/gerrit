  ...
  public List<ChangeInfo> apply(ChangeResource resource)
    ...
    try {
      ...
      if (c.getStatus().isOpen()) {
        ...
        for (ChangeData cd : queryProvider.get().byChangeSet(c.getSubmissionId())) {
          ids.add(cd.getId());
        }
      } else {
        // ABANDONED
        throw new IllegalStateException("Changes in state " + c.getStatus() +
                  " cannot be queried for submitted_together");
      }
      if (ids.size() <= 1) {
