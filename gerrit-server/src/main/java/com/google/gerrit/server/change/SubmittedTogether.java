...
public class SubmittedTogether implements RestReadView<ChangeResource> {
  ...
  enum BranchOnlyValues {
    TRUE, FALSE;
  }

  @Option(name = "--branch-only", aliases = {"-b"}, usage = "Control submition of whole topic")
  private BranchOnlyValues branchOnly;
  ...
  private List<ChangeData> getForOpenChange(Change c)
    ...
    ChangeSet cs = mergeSuperSet.completeChangeSet(dbProvider.get(), c,
        branchOnly != null ? branchOnly == BranchOnlyValues.FALSE : Submit.wholeTopicEnabled(cfg));
