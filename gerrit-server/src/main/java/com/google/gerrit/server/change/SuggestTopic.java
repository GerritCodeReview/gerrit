...
public class SuggestTopic implements RestReadView<ChangeResource> {
  ...
  public void setLimit(int l) {
    this.limit =
        l <= 0 ? maxSuggestedTopics : Math.min(l,
            maxSuggestedTopics);
  }
  ...
  SuggestTopic(Provider<CurrentUser> currentUser,
    ...
    if (suggestTopics) {
      this.maxSuggestedTopics =
          cfg.getInt("suggest", "maxSuggestedTopics", DEFAULT_MAX_SUGGESTED);
    } else {
      this.maxSuggestedTopics = DEFAULT_MAX_SUGGESTED;
    }
    this.dbProvider = dbProvider;
    this.limit = this.maxSuggestedTopics;
  ...
  private List<String> suggestTopics()
    ...
    for (Change c : db.changes().suggestByTopic(a, b, limit)) {
