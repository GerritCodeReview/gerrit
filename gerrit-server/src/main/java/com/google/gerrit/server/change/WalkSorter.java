...
class WalkSorter {
  ...
  private Set<PatchSet.Id> includePatchSets;
  ...
  public WalkSorter includePatchSets(Set<PatchSet.Id> patchSets) {
    includePatchSets = patchSets;
    return this;
  }
  ...
  public Iterable<PatchSetData> sort(Iterable<ChangeData> in)
    ...
    for (Map.Entry<Project.NameKey, Collection<ChangeData>> e
      ...
      List<PatchSetData> forProject = sortProject(e.getKey(), e.getValue());
      if (!forProject.isEmpty()) {
        sortedByProject.add(forProject);
      }
    }
  ...
  private Multimap<RevCommit, PatchSetData> byCommit(RevWalk rw,
    ...
    return !byCommit.isEmpty() ? byCommit : null;
  }

  private boolean shouldInclude(PatchSet ps) {
    return includePatchSets == null || includePatchSets.contains(ps.getId());
  }
