...
class WalkSorter {
  ...
  private List<PatchSetData> sortProject(Project.NameKey project, Collection<ChangeData> in)
    ...
    try (Repository repo = repoManager.openRepository(project);
      ...
      Multimap<RevCommit, RevCommit> pending = MultimapBuilder.hashKeys().arrayListValues().build();
  ...
  private static Multimap<RevCommit, RevCommit> collectChildren(Set<RevCommit> commits) {
    Multimap<RevCommit, RevCommit> children = MultimapBuilder.hashKeys().arrayListValues().build();
