...
public class DeleteDraft implements Callable<ReviewResult> {

  public interface Factory {
    DeleteDraft create(PatchSet.Id patchSetId);
  }
  ...
  @Inject
  DeleteDraft(ChangeControl.Factory changeControlFactory,
      ReviewDb db, IdentifiedUser currentUser,
      GitRepositoryManager gitManager, ReplicationQueue replication,
  ...
  public ReviewResult call() throws NoSuchChangeException, OrmException {
    ...
    if (!control.isOwner() && !control.isVisible(db)) {
      result.addError(new ReviewResult.Error(
          ReviewResult.Error.Type.CHANGE_NOT_VISIBLE));
      return result;
    }
    ...
    try {
      ...
      result.addError(new ReviewResult.Error(
          ReviewResult.Error.Type.GIT_ERROR));
    }
