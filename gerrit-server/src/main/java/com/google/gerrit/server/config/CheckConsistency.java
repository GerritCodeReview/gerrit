...
import com.google.gerrit.extensions.api.config.ConsistencyCheck;
...
public class CheckConsistency implements RestModifyView<ConfigResource, ConsistencyCheckInput> {
  ...
  public ConsistencyCheckInfo apply(ConfigResource resource, ConsistencyCheckInput input)
    ...
    if (input == null || input.checks == null || input.checks.isEmpty()) {
      throw new BadRequestException("input required");
    }

    ConsistencyCheckInfo consistencyCheckInfo = new ConsistencyCheckInfo();
    consistencyCheckInfo.results = new HashMap<>();
    for (ConsistencyCheck check : input.checks) {
      switch (check) {
        case ACCOUNT_EXTERNAL_IDS:
          consistencyCheckInfo.results.put(check, externalIdsConsistencyChecker.check());
          break;
        default:
          throw new BadRequestException(String.format("unsupported check: %s", check.name()));
      }
    }
