...
public class ConfigUtil {
  ...
  /**
   * Resolve groups from group names, via the database. Group names
   * not found in the database will be skipped.
   *
   * @param dbfactory database to resolve from.
   * @param groupNames group names to resolve.
   * @param log log for any warnings and errors.
   * @param groupNotFoundWarning formatted message to output to the log for each group name
   * which is not found in the database. <code>{0}</code> will be replaced with the group name.
   * @return the actual groups resolved from the database. If no groups
   * are found, returns an empty {@code Set}, never {@code null}.
   */
  ...
  public static Set<AccountGroup.Id> groupsFor(
          SchemaFactory<ReviewDb> dbfactory, String[] groupNames, Logger log) {
    return groupsFor(dbfactory, groupNames, log, "Group \"{0}\" not in database, skipping.");
  }
