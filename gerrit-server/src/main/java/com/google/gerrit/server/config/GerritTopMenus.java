...
class GerritTopMenus {
  private static final Logger log = LoggerFactory
      .getLogger(GerritTopMenus.class);
  ...
  @Inject
  GerritTopMenus(Provider<CurrentUser> cup, Provider<GetPreferences> gp,
  ...
  Collection<TopMenu.MenuEntry> getTopMenuBar(Project.NameKey projectNameKey) {
    ...
    projectsMenuItems.add(subMenu("menuProjectsList",
        hashLink(PageLinks.ADMIN_PROJECTS)));

    if (projectNameKey != null) {
      projectsMenuItems.addAll(Arrays
          .asList(
              projectSubMenu(projectNameKey, "menuProjectsInfo", null),
    ...
    if (signedIn) {
      ...
      if (capabilities.canViewPlugins()) {
        topMenu
            .add(menu(
                GerritTopMenu.PLUGINS,
                subMenu("menuPluginsInstalled",
                    hashLink(PageLinks.ADMIN_PLUGINS))));
      }
    }
  ...
  private List<MenuItem> getMySubMenu(IdentifiedUser user) {
    ...
    GeneralPreferencesInfo preferences;
    try {
      preferences = getPreferences.get().apply(account);
      return preferences.my;
    } catch (AuthException | OrmException | IOException
  ...
  private MenuItem subMenuDocumentation(String nameKey, String link) {
    return new MenuItem(constProps.getProperty(nameKey), "/Documentation/"
        + link, "_blank");
  }
