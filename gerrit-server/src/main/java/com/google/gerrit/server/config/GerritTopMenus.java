...
class GerritTopMenus {
  ...
  Collection<TopMenu.MenuEntry> getTopMenuBar(boolean populateMyMenu,
    ...
    projectsMenuItems
        .add(subMenu("menuProjectsList", projectLink(PageLinks.ADMIN_PROJECTS)));
    ...
    if (signedIn) {
      if (capabilities.canCreateProject()) {
        projectsMenuItems.add(subMenu("menuProjectsCreate",
            projectLink(PageLinks.ADMIN_CREATE_PROJECT)));
      }
  ...
  private String projectLink(String screenName, Project.NameKey nameKey) {
    return projectLink(screenName) + nameKey.toString();
  }

  private String projectLink(String screenName) {
    return "#" + screenName;
  }
