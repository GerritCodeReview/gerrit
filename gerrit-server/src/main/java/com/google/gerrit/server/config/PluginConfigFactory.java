...
public class PluginConfigFactory {
  ...
  public Config getFromPluginConfig(Project.NameKey projectName,
      String pluginName) throws NoSuchProjectException {
    return getProjectPluginConfig(projectName, pluginName).get();
  }

  /**
   * Returns the configuration for the specified plugin that is stored in the
   * '<plugin-name>.config' file of the specified project. Parameters which are
   * not set in the '<plugin-name>.config' of this project are inherited from
   * the parent projects '<plugin-name>.config' files.
   *
   * E.g.:
   * child project:
   ...
   *         exist
   */
  public Config getFromPluginConfigWithInheritance(Project.NameKey projectName,
      String pluginName) throws NoSuchProjectException {
    return getProjectPluginConfig(projectName, pluginName).getWithInheritance();
  }

  private ProjectLevelConfig getProjectPluginConfig(
      Project.NameKey projectName, String pluginName)
      throws NoSuchProjectException {
    ProjectState projectState = projectCache.get(projectName);
    if (projectState == null) {
