...
public class ListDashboards {
  ...
  private Map<String, DashboardInfo> loadDashboards(
    ...
    try {
      ...
      while (treeWalk.next()) {
        ...
        info.id = createId(info.projectName, info.refName, info.name);
  ...
  private static String createId(final String projectName,
      final String refName, final String dashboardName) {
    return projectName + ":" + refName + ":" + dashboardName;
  }
