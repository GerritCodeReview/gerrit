...
public class ChangeEditInfoJson {
  ...
  public static EditInfo toEditInfo(ChangeEditData data) throws IOException {
    EditInfo out = new EditInfo();
    out.commit = fillCommit(data.getEditCommit());
    out.actions = fillActions(data);
    return out;
  }
  ...
  private static Map<String, ActionInfo> fillActions(ChangeEditData data) {
    ...
    PatchSet.Id current = data.getEdit().getChange().currentPatchSetId();
    PatchSet basePs = data.getBasePatchSet();
