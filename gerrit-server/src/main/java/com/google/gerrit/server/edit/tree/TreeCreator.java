...
public class TreeCreator {
  ...
  private List<DirCacheEditor.PathEdit> getDesiredPathEdits(
      Repository repository) throws IOException {
    List<DirCacheEditor.PathEdit> pathEdits = new LinkedList<>();
    for (TreeModification treeModification : treeModifications) {
      pathEdits.addAll(treeModification.getNecessaryPathEdits(repository));
    }
    return pathEdits;
  }

  private void applyPathEdits(DirCache tree,
  ...
  private ObjectId writeAndGetId(DirCache tree, Repository repository)
      throws IOException {
