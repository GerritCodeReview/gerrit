...
public class EventsMetrics implements EventListener {
  ...
  public EventsMetrics(MetricMaker metricMaker) {
    ...
    this.scopedEvents = metricMaker.newCounter("scoped-events",
        new Description("Events per Project and Actor").setRate().setUnit("triggered events"), type,
        project, actor);

    events = metricMaker.newCounter("events",
        new Description("Triggered events").setRate().setUnit("triggered events"), type);
  }
