...
public class MergeFailed {
  private static final Logger log =
      LoggerFactory.getLogger(MergeFailed.class);

  private final DynamicSet<MergeFailedListener> listeners;
  private final ChangeEventUtil util;

  @Inject
  MergeFailed(DynamicSet<MergeFailedListener> listeners, ChangeEventUtil util) {
    this.listeners = listeners;
    this.util = util;
  }

  public void fire(ChangeInfo change, RevisionInfo revision,
      AccountInfo submitter, String reason) {
    Event e = new Event(change, revision, submitter, reason);
    for (MergeFailedListener l : listeners) {
      l.onMergeFailed(e);
    }
  }
  ...
  private static class Event extends AbstractRevisionEvent
      implements MergeFailedListener.Event {
