...
public class ChangeDeleted {
  ...
  public void fire(Change change, Account deleter, Timestamp when, NotifyHandling notifyHandling) {
    ...
    try {
      Event event =
          new Event(util.changeInfo(change), util.accountInfo(deleter), when, notifyHandling);
  ...
  private static class Event extends AbstractChangeEvent implements ChangeDeletedListener.Event {
    Event(ChangeInfo change, AccountInfo deleter, Timestamp when, NotifyHandling notify) {
      super(change, deleter, when, notify);
    }
  }
}
