...
public class VoteDeleted {
  ...
  public void fire(ChangeInfo change, RevisionInfo revision,
      ...
      NotifyHandling notify, AccountInfo remover, Timestamp when) {
    if (!listeners.iterator().hasNext()) {
      return;
    }
    Event event = new Event(
        change, revision, approvals, oldApprovals, notify, remover, when);
    for (VoteDeletedListener l : listeners) {
      try {
        l.onVoteRemoved(event);
      } catch (Exception e) {
  ...
  public void fire(Change change, PatchSet ps,
      ...
      NotifyHandling notify, Account remover, Timestamp when) {
    if (!listeners.iterator().hasNext()) {
      return;
    }
    try {
      fire(util.changeInfo(change),
          ...
          notify, util.accountInfo(remover), when);
