...
public class UiActions {
  ...
  public <R extends RestResource> Iterable<UiAction.Description> from(
    ...
    List<PermissionBackendCondition> visible =
        descs.stream().flatMap(u -> Streams.stream(visibleCondition(u))).collect(toList());
    List<PermissionBackendCondition> enabled =
        descs.stream().flatMap(u -> Streams.stream(enabledCondition(u))).collect(toList());
    permissionBackend.user(userProvider).bulkEvaluateTest(Iterables.concat(visible, enabled));
