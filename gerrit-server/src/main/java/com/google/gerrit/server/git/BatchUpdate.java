...
public class BatchUpdate implements AutoCloseable {
  public static abstract class ChangeOp {
  ...
  private final List<PostOp> postOps;
  ...
  public void addChangeOp(ChangeOp op) {
  ...
  public void addPostOp(PostOp update) {
    postOps.add(update);
  }
  ...
  // TODO(dborowitz): Remove doRefUpdates once callers are fully migrated.
  public void execute(boolean doRefUpdates) throws UpdateException {
    if (doRefUpdates) {
      executeRefUpdates();
    }
    executeChangeOps();
