...
public class ChangeMergeQueue implements MergeQueue {
  ...
  private final long defaultDelay = 30;
  private final TimeUnit defaultDelayUnit = TimeUnit.SECONDS;
  ...
  private synchronized boolean start(final Set<Branch.NameKey> branches) {
    ...
    if (requests.get(branches) != null) {
      // The same set of branches are currently in flight, so we'll reschedule
      // ourselves.
      reschedule(req, defaultDelay, defaultDelayUnit);
      return false;
    }
