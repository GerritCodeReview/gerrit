...
public class ChangeSet {
  ...
  public List<Change.Id> ids() {
    List<Change.Id> ret = new ArrayList<>();
  ...
  public Multimap<Project.NameKey, Branch.NameKey> branchesByProject()
      throws OrmException {
    ListMultimap<Project.NameKey, Branch.NameKey> ret =
        ArrayListMultimap.create();
    for (ChangeData cd : changeData) {
      Project.NameKey project = cd.change().getProject();
      Branch.NameKey branch = cd.change().getDest();
      if (!ret.containsEntry(project, branch)){
        ret.put(project, branch);
      }
    }
    return ret;
  }
  ...
  public ImmutableSet<ChangeData> changes() {
    return changeData;
  }
