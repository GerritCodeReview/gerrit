...
/**
 * This class offers an API to merge change sets instead of all changes on a
 * given branch, which was the way Gerrit merged changes historically.
 *
 * TODO(sbeller): This is currently just a wrapper around the existing MergeOp.
 *                Transform the combination of MergeOp and this class into
 *                code that actually operates on changesets on the merge level
...
class ChangeSetMergeOp {
  ...
  public void merge(ChangeSet changes) {
    ...
    try {
      subOp.updateSuperProjects(changes.branches());
