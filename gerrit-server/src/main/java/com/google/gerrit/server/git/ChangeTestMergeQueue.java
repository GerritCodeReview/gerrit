...
public class ChangeTestMergeQueue implements TestMergeQueue {
  ...
  @Override
  public synchronized void addToQueue(Change change) {
    queue.put(change, new TestMergeEntry(change));
  }
  ...
  @Override
  public synchronized void consumesQueue() {
    for (Map.Entry<Change, TestMergeEntry> e : queue.entrySet()) {
      TestMergeEntry testMerge = e.getValue();
      scheduleJob(testMerge);
    }
  }
