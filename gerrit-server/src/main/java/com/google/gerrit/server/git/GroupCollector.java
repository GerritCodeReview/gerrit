...
public class GroupCollector {
  ...
  private static interface Lookup {
    Iterable<String> lookup(PatchSet.Id psId) throws OrmException;
  }
  ...
  GroupCollector(
    ...
    this(
        ...
        new Lookup() {
          @Override
          public Iterable<String> lookup(PatchSet.Id psId) throws OrmException {
  ...
  GroupCollector(
      Multimap<ObjectId, PatchSet.Id> patchSetsBySha,
      final Multimap<PatchSet.Id, String> groupLookup) {
    this(
        patchSetsBySha,
        new Lookup() {
          @Override
          public Iterable<String> lookup(PatchSet.Id psId) {
  ...
  void visit(RevCommit c) {
    ...
    if (thisCommitGroups.isEmpty()) {
      ...
      String firstParentGroup = null;
      for (String parentGroup : parentGroupsNewInThisPush) {
        if (firstParentGroup == null) {
          firstParentGroup = parentGroup;
          groups.put(c, firstParentGroup);
        } else {
          groupAliases.put(parentGroup, firstParentGroup);
        }
      }
