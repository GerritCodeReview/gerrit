...
public class LocalDiskRepositoryManager implements GitRepositoryManager {
  ...
  public void renameRepository(final Project.NameKey source,
      final Project.NameKey destination, final boolean force)
    ...
    // Precursory check if destination exists before grabbing the locks
    if (destinationDirectory.exists() && !force) {
    ...
    if (compare == 0) {
      ...
      throw new ProjectRenamingFailedException("Source and destination "
          + "repository are the same file: " + destinationDirectory);
    } else if (compare < 0) {
    ...
    try {
      lock2.lock();
      try {
        // Checking again whether or no destination exists. This time after
        // having grabbed the locks.
        if (destinationDirectory.exists()) {
          if (force) {
            try {
              FileUtils.delete(destinationDirectory, FileUtils.RECURSIVE);
            } catch (IOException e) {
              throw new ProjectRenamingFailedException( "Could not remove " +
                  "existing destination: " + destinationDirectory, e);
            }
          }
        }
   ...
   * <p>
   * This is used when we have a tree structure such as a/b/c/d.git and
   * a/b/e.git - if we delete a/b/c/d.git, we no longer need a/b/c/.
   *
