...
public class LocalDiskRepositoryManager implements GitRepositoryManager {
  ...
  public void renameRepository(String name, String newName)
    ...
    try {
      ...
      try {
        ...
        removeRepositoryFromCache(name);
  ...
  /**
   * Removes the repository with the specified name from the
   * {@link RepositoryCache}. Removing the repository from the cache ensures
   * that all resources held by the repository are freed. In particular the
   * object database of the repository is closed which releases all open file
   * handles for this repository.
   *
   * @param name the name of the repository that should be removed from the
   *        repository cache
   * @throws RepositoryNotFoundException the name does not denote an existing
   *         repository
   */
  private void removeRepositoryFromCache(final String name) throws RepositoryNotFoundException {
    try {
      final FileKey loc = FileKey.lenient(new File(basePath, name), FS.DETECTED);
      final Repository repository = RepositoryCache.open(loc);
      repository.close();
