...
public class MergeOp {
  ...
  private final MergeSuperSet.Factory mergeSuperSetFactory;
  ...
  MergeOp(AccountCache accountCache,
      ...
      MergeSuperSet.Factory mergeSuperSetFactory,
    ...
    this.mergeSuperSetFactory = mergeSuperSetFactory;
  ...
  public void merge(boolean checkPermissions) throws NoSuchChangeException,
    ...
    try {
      openSchema();
      ChangeSet cs = mergeSuperSetFactory.create(changes,
          Submit.wholeTopicEnabled(cfg)).completeChangeSet();
  ...
  private void integrateIntoHistory(ChangeSet cs)
    ...
    try {
      ...
      for (Project.NameKey project : cs.repos()) {
        ...
        for (Branch.NameKey branch : cs.byProjectBranches().get(project)) {
      ...
      logDebug("Write out the updates");
      for (Project.NameKey project : cs.repos()) {
        openRepository(project);
        for (Branch.NameKey branch : cs.byProjectBranches().get(project)) {
