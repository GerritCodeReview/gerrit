  ...
  public void merge(boolean checkPermissions) throws NoSuchChangeException,
    ...
    try {
      ...
      if (Submit.wholeTopicEnabled(cfg)) {
        cs = mergeSuperSet.completeChangeSetIncludingTopics(db, changes);
      } else {
        cs = mergeSuperSet.completeChangeSet(db, changes);
      }
      logDebug("Calculated to merge {}", cs);
      if (checkPermissions) {
        logDebug("Submitting all depending ancestors while enforcing submit rules");
        submitAllChanges(cs, false);
        logDebug("Checking permissions");
        checkPermissions(cs);
      } else {
        logDebug("Submitting all depending ancestors ignoring submit rules");
