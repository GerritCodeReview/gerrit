...
public class MergeSuperSet {
      ...
      try (Repository repo = repoManager.openRepository(project);
        ...
        for (Change.Id cId : changes.changesByProject().get(project)) {
          ...
          if (!hashes.isEmpty()) {
            ...
            checkState(hashes.size() == destChanges.size());
            for (String hash : hashes) {
              boolean found = false;
              for (ChangeData cdv : destChanges) {
                if (cdv.currentPatchSet().getRevision().equals(new RevId(hash))) {
                  found = true;
                  break;
                }
              }
              if (!found) {
                checkState(false, "programmer error: byCommitsOnBranch doesn't "
                    + "return correct and complete values");
              }
            }
            ...
            for (ChangeData chd : destChanges) {
              ...
              if (chg.getStatus() != Change.Status.NEW
                  ...
                  && chg.getStatus() != Change.Status.MERGED) {
  ...
  public ChangeSet completeChangeSetIncludingTopics(
    ...
    boolean done = false;
    ChangeSet newCs = completeChangeSet(db, changes);
    while (!done) {
