...
public abstract class PriorityLockedQueue<R, T extends PriorityLockedQueue.Task<R>> {
  ...
  /**
   * This will schedule a task to be processed in the future.
   * <p>
   * The {@code minimumDelay} which is measured in milliseconds determines the
   * minimum time it waits until a scheduling is attempted. If the resource
   ...
   * @param minimumDelay the minimum delay in milliseconds when this task should be
   *        run.
   */
  public synchronized void schedule(T rtask, final long minimumDelay) {
    queue.add(new TimedResourceTask(rtask, minimumDelay));
    scheduleQueueCheck(minimumDelay);
  }

  /**
   * Tries processing a task.
   * <p>
   ...
   *          or if it was started when running asynchronously.
