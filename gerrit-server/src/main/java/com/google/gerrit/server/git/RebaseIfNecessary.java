...
public class RebaseIfNecessary extends SubmitStrategy {
   ...
   * it returns null.
   * @param c the {@link CodeReviewCommit} that will be checked to see if it is
   *   out of date, and that newer {@link CodeReviewCommit} from the same change exists.
   * @return the newest {@link CodeReviewCommit} from the same change as the
   *   supplied commit if a newer commit exists, otherwise null.
   */
  protected CodeReviewCommit getNewestCommit(CodeReviewCommit c) {
    try {
