...
public class SignedPushPreReceiveHook implements PreReceiveHook {
  ...
  public void onPreReceive(ReceivePack rp,
    ...
    Writer msgOut = new OutputStreamWriter(rp.getMessageOutputStream());
    try {
  ...
  private void verifySignature(PushCertificate cert,
    ...
    if (key == null) {
      msgOut.write("No valid public key found for "
  ...
  private PGPPublicKey readPublicKey(long keyId) throws IOException {
    try (Repository repo = repoManager.openRepository(allUsers);
      ...
      Ref ref = repo.getRefDatabase().exactRef("refs/heads/public-keys");
      if (ref == null) {
        return null;
      }
