...
class SimilarityMatcher {
  ...
  Result matchChanges(
      List<RevCommit> commits, List<ChangeData> changes, Repository repo)
    ...
    try (RevWalk walk = new RevWalk(repo)) {
      ...
      for (ChangeData change : changes) {
        ...
        changeFeatures.put(
            change,
            new Features(
                walk, walk.parseCommit(
                    ObjectId.fromString(ps.getRevision().get())), repo));
      }

      ImmutableList.Builder<RevCommit> newCommits = ImmutableList.builder();
      ImmutableMap.Builder<RevCommit, ChangeData> updateChanges =
          ImmutableMap.builder();
      for (RevCommit commit : commits) {
        Features commitFeatures = new Features(walk, commit, repo);

        ChangeData similarChange = null;
        for (Map.Entry<ChangeData, Features> entry :
            changeFeatures.entrySet()) {
          if (SimilarityDecisionFunction.INSTANCE.isSimilar(
              commitFeatures, entry.getValue())) {
      ...
      return new Result(
          newCommits.build(), updateChanges.build(),
          ImmutableList.copyOf(changeFeatures.keySet()));
    }
  }
