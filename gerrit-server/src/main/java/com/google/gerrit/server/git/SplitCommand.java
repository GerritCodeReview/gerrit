...
public class SplitCommand {
   ...
   */
  public static class SubtreeFilter extends RevFilter {

    HashMap<String, ObjectId> footerBuffer = new HashMap<String, ObjectId>();
    HashSet<RevCommit> subTrees;
    ...
    public void setSplitters(Set<SplitContext> splitters) {
    ...
    public boolean include(RevWalk walker, RevCommit cmit)
      ...
      if (includeBoundarySubtrees && subTrees.contains(cmit)) {
        ...
        footerBuffer.clear();
        parseSubtreeFooters(repo, cmit, footerBuffer);
        for (String subtreeName : footerBuffer.keySet()) {
          ...
          if (splitters != null) {
            for (SplitContext splitter : splitters) {
              splitter.mNewParents.put(subtreeCommit, NO_SUBTREE);
            }
            for (SplitContext splitter : splitters) {
  ...
  private static abstract class SplitContext {
   ...
   * named subtree.
   */
  private class NameBasedSplitContext extends SplitContext {
  ...
  private Set<SplitContext> mAllSplitters = new HashSet<SplitContext>();
  private Repository mRepo;

  protected SplitCommand(Repository aRepo) {
    mRepo = aRepo;
  }
  ...
  /**
   * Parses a commit message and removes and "Sub-Tree:" footer lines.
   * @throws IOException
   */
  ...
  public Set<SplitContext> getSplitContexts() {
    return mAllSplitters;
  }
