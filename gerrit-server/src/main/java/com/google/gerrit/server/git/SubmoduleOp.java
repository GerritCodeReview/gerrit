...
public class SubmoduleOp {
  ...
  public interface Factory {
    SubmoduleOp create(Branch.NameKey destBranch, CodeReviewCommit mergeTip,
  ...
  public SubmoduleOp(@Assisted final Branch.NameKey destBranch,
      @Assisted CodeReviewCommit mergeTip, @Assisted RevWalk rw,
  ...
  public void update() throws SubmoduleException {
    try {
      this.schema = schemaFactory.open();
          ...
          if (url != null && url.length() > 0 && path != null
            ...
            if ((pathIsRelative)
              ...
              final Branch.NameKey target =
                  new Branch.NameKey(
                      new Project.NameKey(destProject.getName()),
                      destBranch.get());
        ...
        if (msgbuf.length() > 0) {
          ...
          for (final Subscription s : subscribers) {
            if (updatedSubscribers.contains(s.getSubscriber())) {
              ...
              try {
                updateGitlinks(s.getSubscriber(), modules, pathes,
                    msgbuf.toString());
  ...
  private void updateGitlinks(final Branch.NameKey subscriber,
    ...
    try {
      for (final Map.Entry<Branch.NameKey, ObjectId> me : modules.entrySet()) {
        CodeReviewCommit c = (CodeReviewCommit) rw.parseCommit(me.getValue());
        msgbuf.append("\nProject: ");
        ...
        if (submitter != null) {
          ...
          if (emails.size() == 1
            ...
            author = new PersonIdent(c.getAuthorIdent(), dt, tz);
          } else {
            author = who.newCommitterIdent(dt, tz);
          }
        } else {
          author = myIdent;
        }
      ...
      pdb = repoManager.openRepository(subscriber.getParentKey().get());
  ...
  private String getSrvName(String srvUrl) {
    ...
    if (begin >= 2) {
      ...
      if (e > 0 && e < end) end = e;
      if (end >= begin) return srvUrl.substring(begin, end);
    }
    return srvUrl;
  }
