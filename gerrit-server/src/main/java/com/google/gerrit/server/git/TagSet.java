...
import static org.eclipse.jgit.lib.ObjectIdSerialization.*;
...
class TagSet {
  ...
  void prepare(TagMatcher m) {
    ...
    try {
      for (Ref currentRef : m.include) {
        ...
        ObjectId savedObjectId = savedRef.objectId;
        ...
        try {
          ...
          if (rw.isMergedInto(savedCommit, currentCommit)) {
            ...
            savedRef.objectId = currentRef.getObjectId();
            m.mask.set(savedRef.flag);
            continue;
          }
  ...
  void writeObject(ObjectOutputStream out) throws IOException {
    ...
    for (Map.Entry<String, CachedRef> e : refs.entrySet()) {
      ...
      writeNotNull(out, e.getValue().objectId);
    }
