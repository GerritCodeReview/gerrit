...
public class WorkQueue {
  ...
  public WorkQueue(MetricMaker metrics, IdGenerator idGenerator, int defaultThreadPoolSize) {
    ...
    this.defaultQueue = createQueueWithMetrics(defaultThreadPoolSize, "WorkQueue");
  }
  ...
  /**
   * Create a new executor queue with metrics.
   * As metric names must be unique, this method is not intended to be used in plugins.
   * Here is why: during plugin reload operation, it will attempt to create already
   * existing metric, which fails with Exception that prevents the plugin to load.
   */
  public Executor createQueueWithMetrics(int poolsize, String prefix) {
    return createQueue(poolsize, prefix, true);
  }

  /**
   * Create a new executor queue.
   * Note: no metrics will be available for the queue that is created using this method.
   * This make it safe to be used from plugins.
   * To create queue with metrics please use createQueueWithMetrics method.
   */
