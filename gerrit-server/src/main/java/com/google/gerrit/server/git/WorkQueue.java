...
public class WorkQueue {
  ...
  public static class Task<V> implements RunnableScheduledFuture<V> {
    ...
    public String toString() {
      ...
      try {
        Class<?> trustedListenableFutureTask = Class.forName(
            "com.google.common.util.concurrent.TrustedListenableFutureTask");
        if (runnable.getClass().isAssignableFrom(trustedListenableFutureTask)) {
          ...
          for (Field field : runnable.getClass().getDeclaredFields()) {
            if (field.getType()
                .isAssignableFrom(trustedFutureInterruptibleTask)) {
