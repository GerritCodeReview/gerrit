...
public class RebaseSubmitStrategy extends SubmitStrategy {
  boolean rebaseAlways;

  RebaseSubmitStrategy(SubmitStrategy.Arguments args, boolean alwaysRebase) {
    super(args);
    this.rebaseAlways = alwaysRebase;
  }
  ...
  public List<SubmitStrategyOp> buildOps(
    ...
    while (!sorted.isEmpty()) {
      ...
      if (first && args.mergeTip.getInitialTip() == null) {
        // TODO(tandrii): Cherry-Pick strategy does this too.
  ...
  private class RebaseOneOp extends SubmitStrategyOp {
    ...
    public void updateRepoImpl(RepoContext ctx)
      ...
      if (args.mergeUtil
        ...
        // TODO(tandrii): this is copy-paste from Cherry-Pick, but gets the job done.
        args.rw.parseBody(toMerge); // TODO(tandrii): why parseBody here but ParseCommit for rebase?
        newPatchSetId = ChangeUtil.nextPatchSetId(
            args.repo, toMerge.change().currentPatchSetId());
        // TODO(tandrii): extension point to customize this message.
        ...
        try {
          ...
          checkState(false, "MergeConflictException on message edit must not happen");
          return; // Never happens, the above line should raise exception.
        } catch (MergeIdenticalTreeException mie) {
          // TODO(tandrii): tHis Means mergeTip.tree == toMerge.tree. Can this happen?
        ...
        // TODO(tandrii): extension point to customize commit message while rebasing.
  ...
  private class RebaseMultipleParentsOp extends SubmitStrategyOp {
    ...
    public void updateRepoImpl(RepoContext ctx)
      ...
      // configured.
      // TODO(tandrii): can this ever result in fast forward? If so, deal with it.
      // I'm not yet sure.
