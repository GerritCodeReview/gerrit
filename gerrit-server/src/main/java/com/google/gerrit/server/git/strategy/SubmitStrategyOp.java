...
abstract class SubmitStrategyOp extends BatchUpdate.Op {
  ...
  public final void updateChange(ChangeContext ctx) throws Exception {
    ...
    try {
      ...
      if (s == CommitMergeStatus.CLEAN_MERGE) {
        msg = message(ctx, txt + getByAccountName(ctx, commit));
      } else if (s == CommitMergeStatus.CLEAN_REBASE
          || s == CommitMergeStatus.CLEAN_PICK) {
        msg = message(ctx, txt + " as " + commit.name()
            + getByAccountName(ctx, commit));
  ...
  private void setApproval(ChangeContext ctx, IdentifiedUser user)
    ...
    logDebug("Add approval for " + id + " from user " + user);
  ...
  private void approve(ChangeContext ctx, BatchMetaDataUpdate batch,
    ...
    LabelNormalizer.Result normalized =
        args.labelNormalizer.normalize(ctx.getChangeControl(), byKey.values());
    ...
    ctx.getDb().patchSetApprovals().upsert(normalized.getNormalized());
    ctx.getDb().patchSetApprovals().delete(normalized.deleted());

    try {
      saveToBatch(ctx, batch, update, normalized);
    } catch (IOException e) {
      throw new OrmException(e);
    }
  }

  private void saveToBatch(ChangeContext ctx, BatchMetaDataUpdate batch,
    ...
    for (Account.Id accountId : byUser.rowKeySet()) {
      if (!accountId.equals(ctx.getUser().getAccountId())) {
        // TODO(dborowitz): Support voting on behalf of other users to avoid
        // multiple commits here.
        ChangeUpdate thisUserUpdate = ctx.newChangeUpdate(accountId);
        thisUserUpdate.setPatchSetId(callerUpdate.getPatchSetId());
        thisUserUpdate.setSubject("Finalize approvals at submit");
        putApprovals(thisUserUpdate, byUser.row(accountId));
        batch.write(thisUserUpdate, new CommitBuilder());
      }
    }

    // Write these approvals to caller update rather than update from ctx, since
    // this method is also called to normalize approvals in another patch set,
    // which is a separate ChangeUpdate instance.
  ...
  private Account getAccount(Context ctx, CodeReviewCommit codeReviewCommit) {
    Account account = null;
    PatchSetApproval submitter = args.approvalsUtil.getSubmitter(
        ctx.getDb(), codeReviewCommit.notes(), codeReviewCommit.getPatchsetId());
    if (submitter != null) {
      account = args.accountCache.get(submitter.getAccountId()).getAccount();
    }
    return account;
  }

  private String getByAccountName(Context ctx,
      CodeReviewCommit codeReviewCommit) {
    Account account = getAccount(ctx, codeReviewCommit);
  ...
  private void setMerged(ChangeContext ctx, ChangeMessage msg)
    ...
    CodeReviewCommit commit = args.commits.get(c.getId());
    PatchSet.Id mergedId = c.currentPatchSetId();
    submitter = args.approvalsUtil.getSubmitter(db, commit.notes(), mergedId);
    mergedPatchSet = db.patchSets().get(mergedId);
    ...
    if (msg != null) {
      args.cmUtil.addChangeMessage(db, ctx.getChangeUpdate(), msg);
    }
  }
  ...
  public final void postUpdate(Context ctx) throws Exception {
    ...
    try {
      args.mergedSenderFactory.create(
          getId(),
