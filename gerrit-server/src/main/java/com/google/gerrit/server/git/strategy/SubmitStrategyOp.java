...
abstract class SubmitStrategyOp extends BatchUpdate.Op {
  ...
  private void validateRefUpdate(RepoContext ctx, ObjectId currentTip, ObjectId newTip)
    ...
    args.onSubmitValidatorsFactory.create().validateBranchUpdate(
        new OnSubmitValidationListener.Arguments(ctx.getProject(),
            ctx.getRepository(), getDest(), currentTip, newTip,
            ctx.getRevWalk()));
  }
