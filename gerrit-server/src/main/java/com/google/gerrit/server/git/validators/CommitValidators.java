...
public class CommitValidators {
  ...
  public List<CommitValidationListener> getForReceiveCommits() {
    List<CommitValidationListener> rcValidators =
        new LinkedList<CommitValidationListener>();
    rcValidators.add(new UploadPermissionValidator(refControl));
    rcValidators.add(new AmendedGerritMergeCommitValidationListener(refControl,
        gerritIdent));
    ...
    rcValidators.add(new PluginCommitValidationListener(
        commitValidationListeners));
    return rcValidators;
  }

  private static class ChangeIdValidator implements CommitValidationListener {
    private final RefControl refControl;
    private final String canonicalWebUrl;
    private final SshInfo sshInfo;
  ...
  private static class ConfigValidator implements CommitValidationListener {
    private final RefControl refControl;
    private final Repository repo;
      ...
      if (GitRepositoryManager.REF_CONFIG.equals(refControl.getRefName())) {
        try {
          ...
          if (!cfg.getValidationErrors().isEmpty()) {
            ...
            throw new CommitValidationException(
                "invalid project configuration", messages);
          }
        } catch (Exception e) {
          log.error("User " + currentUser.getUserName()
              + " tried to push invalid project configuration "
  ...
  private static class UploadPermissionValidator implements
    ...
    public List<CommitValidationMessage> onCommitReceived(
      ...
      return new LinkedList<CommitValidationMessage>();
    }
  }

  private static class PluginCommitValidationListener implements
      CommitValidationListener {
    private final DynamicSet<CommitValidationListener> CommitValidationListeners;
  ...
  private static class SignedOffByValidator implements CommitValidationListener {
    private final RefControl refControl;
  ...
  private static class AuthorUploaderValidator implements
      CommitValidationListener {
    private final RefControl refControl;
    private final String canonicalWebUrl;
  ...
  private static class CommitterUploaderValidator implements
      CommitValidationListener {
    private final RefControl refControl;
    private final String canonicalWebUrl;
  ...
  private static class AmendedGerritMergeCommitValidationListener implements
      CommitValidationListener {
    private final PersonIdent gerritIdent;
    private final RefControl refControl;
