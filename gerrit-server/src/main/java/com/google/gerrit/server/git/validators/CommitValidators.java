...
public class CommitValidators {
  ...
  public List<CommitValidator> getForReceiveCommits() {
    List<CommitValidator> rcValidators = new LinkedList<CommitValidator>();
    rcValidators.add(new UploadPermissionValidator(refControl));
    rcValidators.add(new AmendedGerritMergeCommitValidator(refControl,
        gerritIdent));
    rcValidators.add(new AuthorUploaderValidator(refControl, canonicalWebUrl));
    rcValidators
        .add(new CommitterUploaderValidator(refControl, canonicalWebUrl));
    rcValidators.add(new SignedOffByValidator(refControl, canonicalWebUrl));
    rcValidators
        .add(new ChangeIdValidator(refControl, canonicalWebUrl, sshInfo));
    rcValidators.add(new ConfigValidator(refControl, repo));
    rcValidators.add(new PluginCommitValidator(commitValidationListeners));
    return rcValidators;
  }

  private static class ChangeIdValidator implements CommitValidator {
    private final RefControl refControl;
    private final String canonicalWebUrl;
    private final SshInfo sshInfo;
    ...
    @Override
    public boolean validateCommit(CommitReceivedEvent receiveEvent,
        CommitValidationCallback callback) {

      LinkedList<CommitValidationMessage> messages =
      ...
      if (MagicBranch.isMagicBranch(receiveEvent.command.getRefName())
        ...
        if (idList.isEmpty()) {
          if (projectControl.getProjectState().isRequireChangeID()) {
            ...
            addFixedCommitMsgWithChangeId(errMsg, receiveEvent.commit,
                currentUser, canonicalWebUrl, sshInfo, messages);
  ...
  private static class ConfigValidator implements CommitValidator {
    private final RefControl refControl;
    private final Repository repo;
    ...
    public boolean validateCommit(CommitReceivedEvent receiveEvent,
      ...
      // If this is the special project configuration branch, validate the
      // config.
  ...
  private static class UploadPermissionValidator implements CommitValidator {
    private final RefControl refControl;

    public UploadPermissionValidator(RefControl refControl) {
      this.refControl = refControl;
    }
    ...
    public boolean validateCommit(CommitReceivedEvent receiveEvent,
      ...
      if (receiveEvent.commit.getParentCount() > 1
        ...
        callback.onRejected("you are not allowed to upload merges",
            new LinkedList<CommitValidationMessage>());
        return false;
      }
      callback.onAccepted(new LinkedList<CommitValidationMessage>());
      return true;
    }
  }

  private static class PluginCommitValidator implements CommitValidator {
    private final DynamicSet<CommitValidationListener> commitValidators;

    public PluginCommitValidator(
        final DynamicSet<CommitValidationListener> commitValidators) {
      this.commitValidators = commitValidators;
    }
    ...
    public boolean validateCommit(CommitReceivedEvent receiveEvent,
      ...
      for (CommitValidationListener validator : commitValidators) {
        CommitValidationResult validationResult =
            validator.onCommitReceived(receiveEvent);
  ...
  private static class SignedOffByValidator implements CommitValidator {
    private final RefControl refControl;
  ...
  private static class AuthorUploaderValidator implements CommitValidator {
    private final RefControl refControl;
    private final String canonicalWebUrl;
    ...
    public boolean validateCommit(CommitReceivedEvent receiveEvent,
      ...
      if (!currentUser.getEmailAddresses().contains(author.getEmailAddress())
        ...
        addInvalidEmailError(receiveEvent.commit, "author", author,
            currentUser, canonicalWebUrl, messages);
  ...
  private static class CommitterUploaderValidator implements CommitValidator {
    private final RefControl refControl;
    private final String canonicalWebUrl;
    ...
    public boolean validateCommit(CommitReceivedEvent receiveEvent,
      ...
      if (!currentUser.getEmailAddresses()
          ...
          && !refControl.canForgeCommitter()) {
        addInvalidEmailError(receiveEvent.commit, "committer", committer,
            currentUser, canonicalWebUrl, messages);
  ...
  private static class AmendedGerritMergeCommitValidator implements
      CommitValidator {
    private final PersonIdent gerritIdent;
    private final RefControl refControl;

    public AmendedGerritMergeCommitValidator(final RefControl refControl,
        final PersonIdent gerritIdent) {
      this.refControl = refControl;
      this.gerritIdent = gerritIdent;
    }
    ...
    public boolean validateCommit(CommitReceivedEvent receiveEvent,
      ...
      if (receiveEvent.commit.getParentCount() > 1
        ...
        callback.onRejected("do not amend merges not made by you",
            new LinkedList<CommitValidationMessage>());
        return false;
      }
      callback.onAccepted(new LinkedList<CommitValidationMessage>());
