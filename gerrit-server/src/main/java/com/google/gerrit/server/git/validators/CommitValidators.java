...
public class CommitValidators {
  ...
  public interface Factory {
    CommitValidators create(RefControl refControl, SshInfo sshInfo, Repository repo);
  }
  ...
  public List<CommitValidationMessage> validateForReceiveCommits(
      CommitReceivedEvent receiveEvent, NoteMap rejectCommits) throws CommitValidationException {
    ...
    if (MagicBranch.isMagicBranch(receiveEvent.command.getRefName())
        || ReceiveCommits.NEW_PATCHSET.matcher(receiveEvent.command.getRefName()).matches()) {
      validators.add(
          new ChangeIdValidator(refControl, canonicalWebUrl, installCommitMsgHookCommand, sshInfo));
    }
  ...
  public List<CommitValidationMessage> validateForGerritCommits(CommitReceivedEvent receiveEvent)
