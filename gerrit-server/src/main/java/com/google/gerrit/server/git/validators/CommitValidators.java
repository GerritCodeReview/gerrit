...
public class CommitValidators {
  ...
  public List<CommitValidationMessage> validateForReceiveCommits(
    ...
    validators.add(new AuthorUploaderValidator(refControl, canonicalWebUrl, realm));
    validators.add(new CommitterUploaderValidator(refControl, canonicalWebUrl, realm));
  ...
  public static class AuthorUploaderValidator implements CommitValidationListener {
    ...
    public AuthorUploaderValidator(RefControl refControl, String canonicalWebUrl, Realm realm) {
      this.refControl = refControl;
      this.canonicalWebUrl = canonicalWebUrl;
      this.realm = realm;
    }
  ...
  private static CommitValidationMessage getInvalidEmailError(RevCommit c,
    ...
    if (currentUser.getEmailAddresses().isEmpty()) {
      ...
      if (currentUser.getEmailAddresses().size() == 1) {
        sb.append("ERROR:  The following address is currently registered for you:\n");
      } else {
        sb.append("ERROR:  The following addresses are currently registered for you:\n");
      }
