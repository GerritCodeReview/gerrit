...
public class OnSubmitValidators {
  ...
  public void validate(Project.NameKey project, Repository repo,
      Map<String, ReceiveCommand> commands) throws IntegrationException {
    try {
      for (String refName : commands.keySet()) {
        if (refName.startsWith(RefNames.REFS_CHANGES)) continue;
        ReceiveCommand cmd = commands.get(refName);
        Arguments args = new Arguments(project, repo, refName, cmd.getOldId(),
            cmd.getNewId());
