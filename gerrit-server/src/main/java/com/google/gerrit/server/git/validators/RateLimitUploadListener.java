...
public class RateLimitUploadListener implements UploadValidationListener {
  ...
  static RateLimiter createSmoothBurstyRateLimiter(
    ...
    try {
      ...
      rl = (RateLimiter) burstyRateLimiterConstructor.newInstance(stopwatch, maxBurstSeconds);
  ...
  private LoadingCache<Account.Id, RateLimiter> rateLimits;
    ...
    if (!limiter.tryAcquire()) {
      throw new RateLimitException(
          MessageFormat.format(
              "Exceeded rate limit of {0} fetch requests/hour", limiter.getRate() * 3600));
    }
  }
}
