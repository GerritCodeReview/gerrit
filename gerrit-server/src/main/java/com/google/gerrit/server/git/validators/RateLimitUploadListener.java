...
import static com.google.gerrit.server.git.validators.RateLimiterModule.CACHE_NAME_ACCOUNTID;
import static com.google.gerrit.server.git.validators.RateLimiterModule.CACHE_NAME_REMOTEHOST;
...
public class RateLimitUploadListener implements UploadValidationListener {
  ...
  private final LoadingCache<Account.Id, RateLimiter> limitsPerAccount;
  private final LoadingCache<String, RateLimiter> limitsPerRemoteHost;

  @Inject
  RateLimitUploadListener(
      Provider<CurrentUser> user,
      @Named(CACHE_NAME_ACCOUNTID) LoadingCache<Account.Id, RateLimiter> limitsPerAccount,
      @Named(CACHE_NAME_REMOTEHOST) LoadingCache<String, RateLimiter> limitsPerRemoteHost) {
    this.user = user;
  ...
  public void onBeginNegotiate(
    ...
    if (u.isIdentifiedUser()) {
      ...
      try {
        limiter = limitsPerAccount.get(accountId);
      ...
      try {
        limiter = limitsPerRemoteHost.get(remoteHost);
      } catch (ExecutionException e) {
        String msg =
