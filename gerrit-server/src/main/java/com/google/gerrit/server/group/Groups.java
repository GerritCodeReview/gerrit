...
public class Groups {
  ...
  /**
   * Returns the existing groups of which the specified account is a member.
   *
   * <p><strong>Note</strong>: This method returns an empty stream if the account doesn't exist. The
   * returned groups are limited to only the existing ones.
   *
   * @param db the {@code ReviewDb} instance to use for lookups
   * @param accountId the ID of the account
   * @return a stream of the UUIDs of the groups which exist and of which the account is a member
   * @throws OrmException if an error occurs while reading from ReviewDb
   */
  public Stream<AccountGroup.UUID> getGroupsWithMember(ReviewDb db, Account.Id accountId)
      throws OrmException {
    ResultSet<AccountGroupMember> accountGroupMembers =
  ...
  /**
   * Returns the existing parent groups of the specified (sub)group.
   *
   ...
   * exist. The returned parent groups are limited to only the existing ones.
   *
   * @param db the {@code ReviewDb} instance to use for lookups
   * @param includedGroupUuid the UUID of the subgroup
   * @return a stream of the UUIDs of the existing parent groups
   * @throws OrmException if an error occurs while reading from ReviewDb
   */
