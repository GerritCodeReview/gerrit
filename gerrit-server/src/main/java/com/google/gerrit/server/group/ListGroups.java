...
public class ListGroups implements RestReadView<TopLevelResource> {
  ...
  private List<AccountGroup> filterGroups(Collection<AccountGroup> groups) {
    ...
    Pattern pattern = Strings.isNullOrEmpty(matchRegex) ? null : Pattern.compile(matchRegex.toLowerCase(Locale.US));
    for (AccountGroup group : groups) {
      if (!Strings.isNullOrEmpty(matchSubstring)) {
        ...
        if (!pattern.matcher(group.getName().toLowerCase(Locale.US)).matches()) {
          continue;
        }
      }
