...
class PutMembers implements RestModifyView<GroupResource, Input> {
  static class Input {
    ...
    public Input(Account.Id oneMember) {
      _oneMember = Integer.toString(oneMember.get());
    }

    public static Input init(Input input)  {
      if (input == null) {
        input = new Input();
      } else if (!Strings.isNullOrEmpty(input._oneMember)) {
      ...
      if (input.members == null) {
        input.members = Collections.emptyList();
      }
      return input;
    }
  }
  ...
  public List<MemberInfo> apply(GroupResource resource, Input input)
    ...
    for (final String nameOrEmail : input.members) {
      ...
      try {
        ...
        badRequest.addError(e.getMessage());
        continue;
      }
  ...
  static class PutMember implements RestModifyView<MemberResource, PutMember.Input> {
    static class Input {
    }
    ...
    public Object apply(MemberResource resource, PutMember.Input input)
        throws AuthException, BadRequestException, ResourceConflictException,
        Exception {
      return new PutMembers(groupControlFactory, accountManager, authConfig,
          accountResolver, accountCache, db, self).apply(resource.getParent(),
          new PutMembers.Input(resource.getUser().getAccountId()));
    }
  }
}
