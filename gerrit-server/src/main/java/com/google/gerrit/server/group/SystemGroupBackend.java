...
public class SystemGroupBackend implements GroupBackend {
  ...
  static {
    SortedMap<String, GroupReference> n =
  ...
  public Collection<GroupReference> suggest(String name, ProjectControl project) {
    SortedMap<String, GroupReference> matches = names.tailMap(name);
    if (matches.isEmpty()) {
      return Collections.emptyList();
    }

    List<GroupReference> r = new ArrayList<>(matches.size());
    for (GroupReference g : matches.values()) {
      if (String.CASE_INSENSITIVE_ORDER.compare(name, g.getName()) <= 0) {
        r.add(g);
      } else {
        break;
      }
