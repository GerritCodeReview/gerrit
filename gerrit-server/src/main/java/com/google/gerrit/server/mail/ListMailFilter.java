...
public class ListMailFilter implements MailFilter {
  private static final Logger log =
      LoggerFactory.getLogger(ListMailFilter.class);
  ...
  ListMailFilter(@GerritServerConfig Config cfg) {
    this.mode = cfg.getEnum("receiveemail", "listfilter", "mode",
        ListFilterMode.OFF);
    String[] addresses = cfg.getStringList("receiveemail", "listfilter",
        "patterns");
    String concat = Arrays.asList(addresses).stream()
  ...
  public boolean shouldProcessMessage(MailMessage message) {
    ...
    if (mode == ListFilterMode.WHITELIST && !match ||
      ...
      log.info("Mail message from " + message.from() +
          " rejected by list filter");
      return false;
    }
    return true;
  }
