...
public class MailSenderApiListener implements
  ...
  public void onChangeAbandoned(ChangeAbandonedListener.Event event) {
    ...
    cm.setChangeMessage(event.getReason());
    try {
      cm.send();
  ...
  public void onChangeRestored(ChangeRestoredListener.Event event) {
    Change.Id changeId = new Change.Id(event.getChange()._number);
    RestoredSender cm = restoredSenderFactory.create(
        new Project.NameKey(event.getChange().project),
        changeId);
    AccountInfo account = event.getRestorer();
    if (account != null) {
      cm.setFrom(new Account.Id(account._accountId.intValue()));
    }
    cm.setChangeMessage(event.getReason());
  ...
  public void onChangeReverted(ChangeRevertedListener.Event event) {
    Change.Id changeId = new Change.Id(event.getChange()._number);
    RevertedSender cm = revertedSenderFactory.create(
        new Project.NameKey(event.getChange().project),
        changeId);
    cm.setFrom(
        new Account.Id(event.getRevertChange().owner._accountId.intValue()));
    cm.setChangeMessage(event.getRevertChange().subject);
    try {
      cm.send();
    } catch (EmailException e) {
      log.error(
