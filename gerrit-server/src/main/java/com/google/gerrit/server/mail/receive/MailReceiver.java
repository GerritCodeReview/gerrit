...
public abstract class MailReceiver implements LifecycleListener {
  ...
  /**
   * Check if deletion for an email is pending.
   * @param id Identifier of the email
   * @return true of deletion is pending for this email, false otherwise.
   */
  protected boolean isDeletionPending(String id) {
    synchronized (pendingDeletion) {
      return pendingDeletion.contains(id);
    }
  }

  /**
   * Mark an email as deleted successfully.
   * @param id Identifier of the email
   */
  protected void markEmailAsDeleted(String id) {
    synchronized (pendingDeletion) {
