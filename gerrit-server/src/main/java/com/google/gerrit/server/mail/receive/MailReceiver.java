    ...
    timer.scheduleAtFixedRate(
        new TimerTask() {
          ...
          public void run() {
            ...
            } catch (IOException | MailParsingException | MailTransferException e) {
              log.error("Exception while fetching messages", e);
            }
          }
        },
        0l,
        ...
        Callable<?> task =
            () -> {
              try {
                ...
                // All Callables are executed on a WorkQueue, so exceptions need to be caught here
                // and logged properly.
