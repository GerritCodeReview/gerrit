...
/**
 * Parses raw email content received through POP3 or IMAP into an internal {@code MailMessage}.
 */
public class RawMailParser {
   ...
   *
   * @param raw {@code String} payload as received over the wire
   * @return Parsed {@code MailMessage}
   ...
   * @param textBuilder {@code StringBuilder) to append all plaintext parts
   * @param htmlBuilder {@code StringBuilder) to append all html parts
   * @throws IOException in case if a failure while transforming the input to a {@code String}
   */
