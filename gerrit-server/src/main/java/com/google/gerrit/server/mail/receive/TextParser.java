...
public class TextParser {
   ...
   * @param comments Comments previously persisted on the change in the same
   *                 ordering as they appear in the UI and are sent in outbound
   *                 email
   * @param changeURL Canonical change URL that points to the change and patch
   *                  set on this Gerrit instance.
   *                  Example: https://go-review.googlesource.com/#/c/91570/3
  ...
  public static List<MailComment> parse(
    ...
    for (String line : lines) {
      if (line.startsWith("> ")) {
        ...
        if (line.equals("> " + changeURL + "/" +
            perspectiveComment.key.filename)) {
          lastEncounteredFileName = perspectiveComment.key.filename;
          if (perspectiveComment.lineNbr == 0) {
        ...
        } else if (line.equals("> " + changeURL + "/" +
            perspectiveComment.key.filename + "@" +
            perspectiveComment.lineNbr)) {
          lastEncounteredComment = perspectiveComment;
          iter.next();
        }
      } else {
