...
public class MetricRegistryPluginProxy extends MetricRegistry implements LifecycleListener {
  private final MetricRegistry coreRegistry;
  private final Set<String> pluginMetrics;

  public MetricRegistryPluginProxy(MetricRegistry coreRegistry) {
    this.coreRegistry = coreRegistry;
    this.pluginMetrics = Collections.synchronizedSet(new HashSet<String>());
  }

  @Override
  public <T extends Metric> T register(String name, T metric)
      throws IllegalArgumentException {
    pluginMetrics.add(name);
  ...
  @Override
  public void stop() {
    for (String metricName : pluginMetrics) {
      coreRegistry.remove(metricName);
    }
  }
}
