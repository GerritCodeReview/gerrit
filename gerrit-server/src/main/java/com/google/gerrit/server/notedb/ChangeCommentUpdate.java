...
public class ChangeCommentUpdate {
   ...
   * @param metaRef The meta branch of target change.
   * @param target The target comment to delete.
   * @param newMsg The message used to replace the target comment's message.
   * @return The new change state in NoteDb.
   * @throws IOException
  ...
  public NoteDbChangeState updateComment(
    ...
    if (commitDataList.size() == 0) {
      throw new IllegalStateException(
          String.format(
              "Cannot delete comment %s on change %s because the first commit"
                  + " on the change meta branch unexpectedly contains comments.",
              target.key, change.getChangeId()));
    }
    ...
    if (!newHeadId.equals(ObjectId.zeroId()) && !newHeadId.equals(metaRef.getObjectId())) {
      ...
      throw new IllegalStateException(
          String.format(
              "fail to delete comment %s on change %s.", target.key, change.getChangeId()));
    }
