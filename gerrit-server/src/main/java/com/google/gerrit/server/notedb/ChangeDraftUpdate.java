...
public class ChangeDraftUpdate extends AbstractChangeUpdate {
  ...
  public void updateComment(PatchLineComment c) throws OrmException {
    ...
    // We check to see if this comment existed previously as a draft,
    // but this could cause a race condition if there is a delete and an update
    // happening at the same time and both operations believe that the comment
    // exists so if the delete happens first, then the update will fail.
    // However, this is an acceptable risk because it is okay for this update
    // to fail.
    if (migration.readComments()) {
  ...
  public void deleteComment(PatchLineComment c) {
    verifyComment(c);
    // See comment above about potential race condition.
    if (migration.readComments()) {
