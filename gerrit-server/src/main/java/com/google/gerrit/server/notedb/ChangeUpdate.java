...
public class ChangeUpdate extends VersionedMetaData {
  ...
  public void putComment(PatchLineComment comment) {
    checkArgument(psId != null);
    checkArgument(CommentsInNotesUtil.psId(comment).equals(psId));
  ...
  private void writeCommentsToNoteMap(NoteMap noteMap,
    ...
    List<PatchLineComment> commentsForThisPS = allComments.get(psId);
    ...
    noteId = inserter.insert(
        Constants.OBJ_BLOB, note, 0, note.length);
