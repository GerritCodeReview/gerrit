...
public class CommentsInNotesUtil {
  ...
  private static final String LENGTH = "Length-in-bytes";
  ...
  public static List<PatchLineComment> parseNote(byte[] note,
      Change.Id changeId) throws ConfigInvalidException, ParseException {
    ...
    PatchSet.Id psId = parsePsId(note, curr, changeId, enc, isForBase);
    ...
    while (p.value < sizeOfNote) {
      ...
      c = parseComment(note, p, previousFileName, psId, revId,
          changeId, isForBase, enc);
      result.add(c);
    }
    return result;
  }

  public static PatchSet.Id psId(PatchLineComment plc) {
    return plc.getKey().getParentKey().getParentKey();
  }

  private static PatchLineComment parseComment(byte[] note, MutableInteger ptr,
      String currentFileName, PatchSet.Id psId, RevId revId, Change.Id changeId,
      boolean isForBase, Charset enc)
          throws ConfigInvalidException, ParseException {
    int curr = ptr.value;

    // Check if there is a new file.
    boolean newFile =
        (RawParseUtils.match(note, curr, FILE.getBytes(UTF_8))) != -1;
    if (newFile || currentFileName == null) {
    ...
    String parentUUID = hasParent ?
        parseStringField(note, curr, changeId, enc, PARENT)
        : null;
    if (hasParent) {
      curr = RawParseUtils.nextLF(note, curr);
    }
  ...
  private static String parseStringField(byte[] note, int curr, Change.Id changeId,
    ...
    String s = RawParseUtils.decode(enc, note, startOfField, endOfLine - 1);
    return s;
  }
   ...
   *    line and the other fields as 0. If the comment range line in the note
  ...
  private static PatchSet.Id parsePsId(byte[] note, int curr,
      Change.Id changeId, Charset enc, boolean isForBase)
      throws ConfigInvalidException {
    if (isForBase) {
      checkHeaderLineFormat(note, curr, BASE_PATCH_SET, enc, changeId);
    } else {
      checkHeaderLineFormat(note, curr, PATCH_SET, enc, changeId);
    }
    int startOfPSId =
  ...
  private static String parseFilename(byte[] note, int curr, Id changeId,
  ...
  private static Timestamp parseTimestamp(byte[] note, int curr,
      ...
      throws ParseException, ConfigInvalidException {
    int endOfLine = RawParseUtils.nextLF(note, curr);
  ...
  private static void checkResult(Object o, String fieldName, Id changeId)
      throws ConfigInvalidException {
    if (o == null) {
  ...
  private static void checkResult(int i, String fieldName, Change.Id changeId)
      throws ConfigInvalidException {
    if (i <= 0) {
      throw parseException(changeId, "could not parse %s", fieldName);
    }
  ...
  private static Account.Id parseIdent(PersonIdent ident, Id changeId)
      throws ConfigInvalidException {
  ...
  private void appendHeaderField(OutputStreamWriter writer,
   ...
   *
   * @param comments
   *            A list the comments to be written to the given note byte array.
  ...
  public byte[] buildNote(List<PatchLineComment> comments)
    ...
    for (PatchLineComment c : comments) {
      PatchSet.Id currentPsId = psId(c);
      checkArgument(psId.equals(currentPsId),
          "The comments being added must all have the same PatchSet.Id");
      checkArgument(side == c.getSide(),
          "The comments being added must all have the same side");
      ...
      writer.write(formatTime(serverIdent, c.getWrittenOn()));
      writer.write("\n");
