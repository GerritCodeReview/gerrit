...
public class NoteDbChangeState {
  public static class Delta {
    ...
    final Optional<ObjectId> newChangeMetaId;
    final ImmutableMap<Account.Id, ObjectId> newDraftIds;

    Delta(Change.Id changeId, Optional<ObjectId> newChangeMetaId,
        Map<Account.Id, ObjectId> newDraftIds) {
      this.changeId = checkNotNull(changeId);
      this.newChangeMetaId = checkNotNull(newChangeMetaId);
      if (newDraftIds != null) {
        this.newDraftIds = ImmutableMap.copyOf(newDraftIds);
      } else {
        this.newDraftIds = ImmutableMap.of();
      }
    }
  }
  ...
  static NoteDbChangeState parse(Change.Id id, String str) {
    ...
    Map<Account.Id, ObjectId> draftIds =
        Maps.newHashMapWithExpectedSize(parts.size());
  ...
  public static void applyDelta(Change change, Delta delta) {
    ...
    if (oldStr == null
        && (delta == null || !delta.newChangeMetaId.isPresent())) {
      return; // Can't construct a complete state, bail.
    }
    ...
    change.setNoteDbState(
        new NoteDbChangeState(change.getId(), changeMetaId, draftIds)
            .toString());
  }
