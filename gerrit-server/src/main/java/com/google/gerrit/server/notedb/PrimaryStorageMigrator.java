...
public class PrimaryStorageMigrator {
   ...
   * NoteDb.)
   * <p>
   * If this method throws an error, then the primary storage of the change is
   * probably not NoteDb, and moreover, the change may be read-only until a
   * least expires. If the caller chooses to retry, they should wait until the
   * read-only lease expires; this method will fail relatively quickly if called
   * on a read-only change. Note that if the change is read-only after this
   * method throws an exception, that does not necessarily guarantee that the
   * read-only lease was acquired during that particular method invocation; the
   * method may have in fact failed because another thread acquired the lease
   * first.
   *
   * @param id change ID.
   *
