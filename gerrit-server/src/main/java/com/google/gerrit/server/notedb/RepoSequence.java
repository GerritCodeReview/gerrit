 ...
 * <p>
 * The current sequence number is stored as ASCII in a blob pointed to by a ref
 * in the {@code refs/sequences/*} namespace. Multiple processes can share the
 * same sequence by incrementing the counter using normal git ref updates. To
 * amortize the cost of these ref updates, processes can increment the counter
 * by a larger number and hand out numbers from that range in memory until they
 * run out. This means concurrent processes will hand out somewhat non-monotonic
 * numbers.
 */
public class RepoSequence {
  static final String REFS_SEQUENCES = "refs/sequences/";
  ...
  RepoSequence(GitRepositoryManager repoManager, Project.NameKey projectName,
      String name, int start, int batchSize) {
  ...
  RepoSequence(GitRepositoryManager repoManager, Project.NameKey projectName,
    ...
    this.refName = REFS_SEQUENCES + checkNotNull(name, "name");
    this.start = start;
