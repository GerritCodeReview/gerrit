...
class RevisionNoteBuilder {
  ...
  Multimap<PatchSet.Id, PatchLineComment> buildCommentMap() {
  ...
  void buildNoteJson(ChangeNoteUtil noteUtil,
    ...
    data.comments = new ArrayList<>(comments.size());
    for (PatchLineComment plc : PLC_ORDER.sortedCopy(comments.values())) {
      data.comments.add(new RevisionNoteData.Comment(plc));
    }
    data.serverId = noteUtil.getServerId();
  ...
  void buildNoteHomebrew(ChangeNoteUtil noteUtil,
                     OutputStream out) throws IOException {
    if (pushCert != null) {
