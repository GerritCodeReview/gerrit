...
public class NoteDbMigrator implements AutoCloseable {
  ...
  public static class Builder {
     ...
     * may be set to false. However, admins may still choose to use a gap, for example to make it
     * easier to distinguish changes that were created before and after the NoteDb migration.
     *
     * @param sequenceGap sequence gap size; must be non-negative, or null to use the value from
     *     {@code noteDb.changes.sequenceBatchSize} in {@code gerrit.config}.
     * @return this.
     */
    ...
    public NoteDbMigrator build() throws MigrationException {
      return new NoteDbMigrator(
          ...
          trial,
          forceRebuild,
          sequenceGap != null ? sequenceGap : Sequences.getChangeSequenceGap(cfg));
    }
  }
  ...
  private NoteDbMigrator(
    ...
    if (sequenceGap < 0) {
      throw new MigrationException("Sequence gap must be non-negative");
    }
