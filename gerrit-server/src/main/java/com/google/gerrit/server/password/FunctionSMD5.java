...
/** Implements the same storage format as OpenLDAP's SMD5 encryption. */
class FunctionSMD5 implements PasswordFunction {
  ...
  public String encrypt(Random rng, String password) {
    try {
      ...
      if (hash.length != HASH_SIZE) {
        throw new RuntimeException("MD5 produced " + hash.length + " bytes");
      }
