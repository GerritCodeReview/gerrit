...
class EditTransformer {
  ...
  private static Stream<ContextAwareEdit> toEdits(PatchListEntry patchListEntry) {
    return patchListEntry
        .getEdits()
        .stream()
        .map(edit -> new ContextAwareEdit(patchListEntry, edit));
  }
  ...
  private static String getOldFilePath(PatchListEntry patchListEntry) {
    return patchListEntry.getOldName() != null
  ...
  private static class ContextAwareEdit {
    ...
    ContextAwareEdit(PatchListEntry patchListEntry, Edit edit) {
      this(
          patchListEntry.getOldName(),
          patchListEntry.getNewName(),
          edit.getBeginA(),
    ...
    ContextAwareEdit(
      ...
      this.oldFilePath = oldFilePath != null ? oldFilePath : newFilePath;
