...
class IntraLineLoader implements Callable<IntraLineDiff> {
  ...
  private static int getShift(CharText text, int lf, int start, int end) {
    ...
    if (lf < start
        && ((lf == 0  && isOnlyWhitespace(text, lf, end))
        || (text.charAt(lf) == '\n' && isOnlyWhitespace(text, lf + 1, end)))) {
      int nb = text.charAt(lf) == '\n' ? lf + 1 : 0;
      int p = 0;
      ...
      while (nb < (end - p - 1)) {
        if (cmp.equals(text, nb, text, end - p - 1)) {
          p++;
        } else {
          break;
        }
