...
class IntraLineLoader implements Callable<IntraLineDiff> {
  ...
  private static Edit combineCloseEdits(CharText a, CharText b, Edit prev, Edit next) {
    if (prev != null
        && (next.getBeginA() - prev.getEndA() <= 5 || next.getBeginB() - prev.getEndB() <= 5)
  ...
  private static Edit shiftEdit(CharText a, CharText b, Edit edit) {
    ...
    return shifted == 0 ? edit : new Edit(beginA, endA, beginB, endB);
  }

  private static boolean canShiftLeft(CharText t, int begin, int end) {
    if (begin == 0 || t.charAt(begin-1) == '\n') {
      return false;
    }
    return begin == end || t.charAt(begin-1) == t.charAt(end-1);
  }

  private static boolean canShiftRight(CharText t, int begin, int end) {
    if (end >= t.size() || t.charAt(end) == '\n') {
      return false;
    }
    return begin == end || t.charAt(begin) == t.charAt(end);
  }
}
