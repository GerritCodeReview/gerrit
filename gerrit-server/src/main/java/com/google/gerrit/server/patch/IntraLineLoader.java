...
class IntraLineLoader implements Callable<IntraLineDiff> {
  ...
  static int getWhitespaceShift(CharText text, int lf, int start, int end) {
    CharTextComparator cmp = new CharTextComparator();

    // If the part from line start to our edit contains of whitespace only and the edit itself
    // consists of the same whitespaces only we will assume that the edit was indeed an indentation change.
    // In this case we will shift the diff to the left margin.
    if (lf < start && text.charAt(lf) == '\n' && isOnlyWhitespace(text, lf + 1, start)) {
      int nb = lf + 1;
      int p = 0;

      // Check that the line start and our edit consist of the same whitespaces only. Otherwise
      // we would be displaying wrong information if the whitespaces were not consistently of one
      // type only.
      while (nb < (end - p)) {
    ...
    // Otherwise we assume to not have an indentation change and try to shift the diff to the right by comparing
    // the edit's head and tail and shifting over if they match.
    } else {
      int p = 0;
  ...
  static IntraLineDiff compute(Text aText, Text bText, List<Edit> edits)
    ...
    for (int i = 0; i < edits.size(); i++) {
      Edit text = edits.get(i);

      if (text.getType() == Edit.Type.REPLACE) {
        CharText a = new CharText(aText, text.getBeginA(), text.getEndA());
        CharText b = new CharText(bText, text.getBeginB(), text.getEndB());
        CharTextComparator cmp = new CharTextComparator();
        ...
        for (int j = 0; j < wordEdits.size(); j++) {
          ...
          if (ab < ae && isOnlyWhitespace(a, ab, ae)) {
            ...
            int shift = getWhitespaceShift(a, lf, ab, ae);
            ab = ab + shift;
            ae = ae + shift;
          }
          ...
          if (bb < be && isOnlyWhitespace(b, bb, be)) {
            ...
            int shift = getWhitespaceShift(b, lf, bb, be);
            bb = bb + shift;
            be = be + shift;
          }
        ...
        edits.set(i, new ReplaceEdit(text, wordEdits));
      }
    }
