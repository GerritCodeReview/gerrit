...
class AutoRegisterScript {
  ...
  boolean scan(Class<?> scriptClass) throws InvalidPluginException {
    if (Module.class.isAssignableFrom(scriptClass)) {
      if (scriptModule != null) {
        throw new InvalidPluginException(
            "Duplicate Guice Module classes detected in script: "
                + scriptModule.getClass() + " and " + scriptClass);
      }
      try {
        scriptModule = (Module) scriptClass.newInstance();
      } catch (InstantiationException | IllegalAccessException e) {
        throw new InvalidPluginException("Cannot instantiate Script module class " + scriptClass, e);
      }
      return true;
    } else {
