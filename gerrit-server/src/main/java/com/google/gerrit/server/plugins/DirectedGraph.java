 ...
 * Source: http://keithschwarz.com/interesting/code/?dir=topological-sort
 *
...
public final class DirectedGraph<T> implements Iterable<T> {
   ...
   * is represented by a map from edges to doubles.
   */
  private final Map<T, Set<T>> mGraph = new HashMap<T, Set<T>>();

  /**
   * Adds a new node to the graph. If the node already exists, this function is
   * a no-op.
   *
   ...
   * first node to the second node. If either node does not exist in the graph,
   * throws a NoSuchElementException.
   *
   * @param start The start node.
