...
public interface PluginContentScanner {
   ...
   */
  <T> Optional<T> getResource(String resourcePath, Class<? extends T> resourceClass);

  /**
   * Return the InputStream of the resource associated to a path
   * @param resourcePath full path of the resource inside the plugin package
   * @return the resource input stream or Optional.absent() if the resource was not found
   * @throws IOException if there was an I/O problem accessing the resource
   */
  Optional<InputStream> getResourceInputStream(String resourcePath) throws IOException;
