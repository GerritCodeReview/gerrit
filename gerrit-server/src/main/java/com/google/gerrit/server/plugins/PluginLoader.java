...
public class PluginLoader implements LifecycleListener {
  ...
  private final ImmutableCollection<Plugin> requiredPlugins;
  ...
  public PluginLoader(SitePaths sp,
      ...
      @RequiredPlugins ImmutableCollection<Plugin> required) {
    sitePaths = sp;
    env = pe;
    srvInfoImpl = sii;
  ...
  private void runPlugin(String name, File jar, Plugin oldPlugin)
    ...
    Plugin newPlugin = loadPlugin(name, jar);
    try {
      newPlugin.load(this);
      runPlugin(newPlugin, oldPlugin);
    } catch (Throwable err) {
  ...
  private Plugin loadPlugin(String name, File srcJar) {
    return new Plugin(sitePaths, name, srcJar, false);
  }
