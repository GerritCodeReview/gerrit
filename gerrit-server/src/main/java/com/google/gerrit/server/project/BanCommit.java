...
public class BanCommit implements RestModifyView<ProjectResource, Input> {
  ...
  private com.google.gerrit.server.git.BanCommit.Factory banCommitFactory;

  @Inject
  public BanCommit(com.google.gerrit.server.git.BanCommit.Factory banCommitFactory) {
    this.banCommitFactory = banCommitFactory;
  }
    ...
    if (input != null && input.commits != null && !input.commits.isEmpty()) {
      ...
      try {
        BanCommitResult result =
            banCommitFactory.create().ban(rsrc.getControl(), commitsToBan, input.reason);
