...
public class ChangeControl {
  public static class GenericFactory {
    ...
    public ChangeControl validateFor(Change.Id changeId, CurrentUser user)
        throws NoSuchChangeException, OrmException {
      List<ChangeControl> ctls = changeFinder.findChanges(changeId, user);
      if (ctls.size() != 1) {
        throw new NoSuchChangeException(changeId);
      }
      ChangeControl ctl = ctls.get(0);

      // reload change, index may be stale
      Change change =
          notesFactory.create(db.get(), ctl.getProject().getNameKey(), changeId)
              .getChange();
