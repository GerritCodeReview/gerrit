...
class CreateProject implements RestModifyView<TopLevelResource, Input> {
  ...
  public Object apply(TopLevelResource resource, Input input)
    ...
    args.projectDescription = Strings.nullToEmpty(input.description);
    try {
      args.submitType =
          Strings.isNullOrEmpty(input.submitType)
              ? SubmitType.MERGE_IF_NECESSARY : SubmitType
                  .valueOf(input.submitType);
    } catch (java.lang.IllegalArgumentException e) {
      throw new BadRequestException(String.format("Wrong submit type: %s",
          input.submitType));
    }
    if (input.branches != null && input.branches.length > 0) {
      args.branch = Arrays.asList(input.branches);
    } else {
