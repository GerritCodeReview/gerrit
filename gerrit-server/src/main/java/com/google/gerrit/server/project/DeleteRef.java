...
public class DeleteRef {
  ...
  public void delete(ProjectResource project, List<String> refsToDelete)
      throws OrmException, IOException, ResourceConflictException {
    if (!refsToDelete.isEmpty()) {
      try (Repository r = repoManager.openRepository(project.getNameKey())) {
        if (refsToDelete.size() == 1) {
          deleteSingleRef(project, r, refsToDelete.get(0));
        } else {
          deleteMultipleRefs(project, r, refsToDelete);
        }
      }
    }
  }

  public void delete(RefResource ref)
      throws OrmException, IOException, ResourceConflictException{
    delete(ref, ImmutableList.of(ref.getRef()));
  }

  private void deleteSingleRef(ProjectResource project, Repository r, String ref)
    ...
    switch (result) {
      ...
      case FAST_FORWARD:
      case FORCED:
        referenceUpdated.fire(project.getNameKey(), u, ReceiveCommand.Type.DELETE,
  ...
  private void deleteMultipleRefs(ProjectResource project, Repository r, List<String> refs)
      throws OrmException, IOException, ResourceConflictException {
    BatchRefUpdate batchUpdate = r.getRefDatabase().newBatchUpdate();
    for (String ref : refs) {
      batchUpdate.addCommand(createDeleteCommand(project, r, ref));
    }
