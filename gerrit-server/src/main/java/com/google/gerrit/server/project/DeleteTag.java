...
public class DeleteTag implements RestModifyView<TagResource, Input> {
  ...
  @Override
  public Response<?> apply(TagResource rsrc, Input input) throws AuthException,
      ResourceConflictException, OrmException, IOException {
    if (!rsrc.getControl().controlForRefTag(rsrc.getTagKey()).canDelete()) {
      throw new AuthException("Cannot delete tag");
    }
    if (!queryProvider.get().setLimit(1)
        .byTagOpen(rsrc.getTagKey()).isEmpty()) {
      throw new ResourceConflictException("tag " + rsrc.getTagKey()
          + " has open changes");
    }
    ...
    try (Repository r = repoManager.openRepository(rsrc.getNameKey())) {
      ...
      switch (result) {
        ...
        default:
          log.error("Cannot delete " + rsrc.getTagKey() + ": " + result.name());
          throw new ResourceConflictException("cannot delete t: " + result.name());
      }
    }
