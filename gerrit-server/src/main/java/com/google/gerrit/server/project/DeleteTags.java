...
public class DeleteTags
  ...
  public Response<?> apply(ProjectResource resource, DeleteTagsInput input)
    ...
    try (Repository r = repoManager.openRepository(resource.getNameKey())
        Git git = new Git(r)) {
      DeleteTagCommand deleteTag = git.tagDelete().setTags(input.tags);
      for (String tag : input.tags) {
        deleteTag.addCommand(createDeleteCommand(project, r, tag));
      }
      try (RevWalk rw = new RevWalk(r)) {
        deleteTag.execute(rw, NullProgressMonitor.INSTANCE);
      }
  ...
  private void postDeletion(ProjectResource project, ReceiveCommand cmd) {
    String tag = RefUtil.validateTagRef(project.getTagInfo().ref);
    RefControl refControl = project.getControl().controlForRef(tag);
