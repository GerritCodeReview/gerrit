...
public class DeleteTags
  ...
  private ReceiveCommand createDeleteCommand(ProjectResource project,
      Repository r, String tag)
          throws OrmException, IOException, ResourceConflictException {
    Ref ref = r.getRefDatabase().getRef(tag);
    ReceiveCommand command;
    if (ref == null) {
      command = new ReceiveCommand(ObjectId.zeroId(), ObjectId.zeroId(), tag);
    ...
    Tag.NameKey tagKey =
        new Tag.NameKey(project.getNameKey(), ref.getName());
    if (!project.getControl().controlForRef(tagKey).canDelete()) {
      command.setResult(Result.REJECTED_OTHER_REASON,
          "it doesn't exist or you do not have permission to delete it");
    }
    if (!queryProvider.get().setLimit(1).byTagOpen(tagKey).isEmpty()) {
      command.setResult(Result.REJECTED_OTHER_REASON, "it has open changes");
    }
    RefUpdate u = r.updateRef(tag);
  ...
  private void appendAndLogErrorMessage(StringBuilder errorMessages,
    ...
    switch (cmd.getResult()) {
      case REJECTED_CURRENT_TAG:
        msg = format("Cannot delete %s: it is the current tag",
            cmd.getRefName());
        break;
