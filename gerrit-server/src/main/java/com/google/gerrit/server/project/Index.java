...
public class Index implements RestModifyView<ProjectResource, ProjectInput> {
  ...
  private void index(Project.NameKey project) throws Exception {
    ...
    try (Repository repo = repoManager.openRepository(project);
      ...
      Set<Change.Id> alreadyPresentInIndex = fromIndex(project);
      ...
      Set<Change.Id> toReIndex =
          allById.values().stream().map(cd -> cd.getId()).collect(Collectors.toSet());
      Set<Change.Id> toDeleteFormIndex = Sets.difference(alreadyPresentInIndex, toReIndex);
      deleteFromIndex(toDeleteFormIndex);
      NullProgressMonitor pm = NullProgressMonitor.INSTANCE;
      new ProjectIndexer(indexer, allById, repo, pm, pm, null).call();
