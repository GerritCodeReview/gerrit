...
public class ListProjects implements RestReadView<TopLevelResource> {
  ...
  public SortedMap<String, ProjectInfo> display(@Nullable OutputStream displayOutputStream)
    ...
    SortedMap<Project.NameKey, ProjectNode> sortedMap = null;
    try {
      ...
      if (showTree && !format.isJson()) {
        sortedMap = toSortedMap(projectStates);
      } else {
      ...
      if (stdout == null) {
        ...
        stdout.print('\n');
      } else if (showTree && sortedMap != null && sortedMap.size() > 0) {
        printProjectTree(stdout, sortedMap);
      }
      return null;
    } finally {
      if (stdout != null) {
