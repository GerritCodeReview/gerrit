...
public class ListTags implements RestReadView<ProjectResource> {
  ...
  public List<TagInfo> apply(ProjectResource resource)
    ...
    try (Repository repo = getRepository(resource.getNameKey());
      ...
      ProjectControl pcntl = resource.getControl();
      Map<String, Ref> all =
          visibleTags(pcntl, repo, repo.getRefDatabase().getRefs(Constants.R_TAGS));
      for (Ref ref : all.values()) {
        tags.add(createTagInfo(ref, rw, pcntl.controlForRef(ref.getName()), pcntl, links));
      }
    }
  ...
  public TagInfo get(ProjectResource resource, IdString id)
    ...
    try (Repository repo = getRepository(resource.getNameKey());
      ...
      if (ref != null
          && !visibleTags(pcntl, repo, ImmutableMap.of(ref.getName(), ref)).isEmpty()) {
        return createTagInfo(ref, rw, control.controlForRef(ref.getName()), pctl, links);
      }
    }
  ...
  public static TagInfo createTagInfo(Ref ref, RevWalk rw, RefControl control,
      ProjectControl pcntl, WebLinks links) throws MissingObjectException, IOException {
  ...
  private Map<String, Ref> visibleTags(
      ProjectControl pcntl, Repository repo, Map<String, Ref> tags) {
    return new VisibleRefFilter(
            tagCache, changeNotesFactory, changeCache, repo, pcntl, dbProvider.get(), false)
        .filter(tags, true);
  }
}
