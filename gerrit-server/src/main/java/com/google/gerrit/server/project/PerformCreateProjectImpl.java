...
public class PerformCreateProjectImpl implements PerformCreateProject {
  ...
  private final boolean createEmptyCommitRequired;
  ...
  PerformCreateProjectImpl(
    ...
    this.createEmptyCommitRequired =
    ...
    if (err.length() == 0) {
      if (!permissionsOnly) {
        final Repository repo =
            repoManager.createRepository(new NameKey(projectName));
        try {
          repo.create(true);
          ...
          repoManager.setProjectDescription(new NameKey(projectName),
              projectDescription);
  ...
  private void createEmptyCommit(final Repository repo, final String ref)
    ...
    try {
      ...
      switch (result) {
        case NEW:
          rq.scheduleUpdate(new NameKey(projectName), ref);
          break;
        default: {
