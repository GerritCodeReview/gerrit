...
public class PerformCreateProjectImpl implements PerformCreateProject {
  ...
  public void createProject() throws RepositoryNotFoundException, IOException,
    ...
    try {
      ...
      handleRepositoryExistsException(ee.getNameOfExistingProject());
    }
  }

  private void handleRepositoryExistsException(final Project.NameKey projectName)
      throws RepositoryNotFoundException, CreateProjectParamsException {
    Repository repo = repoManager.openRepository(projectName);
    try {
      if (repo.getObjectDatabase().exists()) {
        throw new CreateProjectParamsException("fatal: project \""
            + projectName + "\" exists");
      }
    } finally {
      repo.close();
  ...
  private void createProjectConfig() throws IOException, ConfigInvalidException {
    ...
    try {
      ...
      if (createProjectArgs.getNewParent() != null) {
        newProject.setParentName(createProjectArgs.getNewParent().getProject().getName());
      }
  ...
  private StringBuilder validateParameters() {
    ...
    if (createProjectArgs.getNewParent() != null
        && createProjectArgs.getNewParent()!=null) {
      final ProjectState state =
          projectCache
              .get(new Project.NameKey(createProjectArgs.getNewParent().getProject().getName()));
