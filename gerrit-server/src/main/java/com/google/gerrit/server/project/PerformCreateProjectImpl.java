...
public class PerformCreateProjectImpl implements PerformCreateProject {
  ...
  @Inject
  PerformCreateProjectImpl(@ProjectOwnerGroups Set<AccountGroup.UUID> projectOwnerGroups,
  ...
  public void createProject() throws RepositoryNotFoundException, IOException,
    ...
    if (err.length() == 0) {
      ...
      try {
        ...
        u.disableRefLog();
        u.link(createProjectArgs.getBranch());

        createProjectConfig();
        repoManager.setProjectDescription(nameKey, createProjectArgs
            .getProjectDescription());

        if (createProjectArgs.shouldCreateEmptyCommit()) {
          createEmptyCommit(repo, Constants.HEAD);
        }

        rq.replicateNewProject(nameKey, createProjectArgs.getBranch());
      } finally {
        repo.close();
      }
    } else {
  ...
  private void createEmptyCommit(final Repository repo, final String ref)
      throws IOException {
    ...
    try {
      ...
      switch (result) {
        case NEW:
          rq.scheduleUpdate(new NameKey(createProjectArgs.getProjectName()),
              ref);
          break;
        default: {
