...
public class PerformSetParentImpl implements PerformSetParent {
  ...
  public interface Factory {
    PerformSetParentImpl create(NameKey parentNameKey, List<Project.NameKey> childProjects);
  }
  ...
  PerformSetParentImpl(final ProjectCache projectCache, final ReviewDb db,
      @Assisted final NameKey parentNameKey, @Assisted List<Project.NameKey> childProjects) {
    this.db = db;
  ...
  public void setParent() throws OrmException, NoSuchProjectException, OperationNotExecutedException {
    ...
    if (numberOfFailedProjects == childProjects.size() && childProjects.size() > 1) {
