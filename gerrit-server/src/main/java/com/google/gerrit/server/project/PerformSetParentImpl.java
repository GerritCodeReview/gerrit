...
public class PerformSetParentImpl implements PerformSetParent {
  ...
  public interface Factory {
    PerformSetParentImpl create(String parentName, List<Project.NameKey> childProjects);
  }
  ...
  public StringBuilder setParent() throws OrmException {
    ...
    if (parentName != null && !parentName.isEmpty() && !parentName.equals(wildProject.get())) {
      newParentKey = new Project.NameKey(parentName);

      final ProjectState state = projectCache.get(newParentKey);
    ...
    for (final Project.NameKey pc : childProjects) {
      ...
      if (grandParents.contains(pc) || pc.equals(newParentKey)) {
