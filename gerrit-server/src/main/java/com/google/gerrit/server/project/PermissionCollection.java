  ...
  abstract static class SeenRule {
    ...
    static SeenRule create(AccessSection section, Permission permission,
      ...
      return new AutoValue_PermissionCollection_SeenRule(
          section.getName(), permission.getName(), rule.getGroup().getUUID());
    }

    // TODO(dborowitz); The old implementation of eq(UUID, UUID) returned false
    // when a == b == null; if intentional, we need to implement equals by hand.
  }
}
