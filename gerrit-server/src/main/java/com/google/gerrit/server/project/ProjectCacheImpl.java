...
public class ProjectCacheImpl implements ProjectCache {
  ...
  static class ByNameLoader extends EntryCreator<Project.NameKey, ProjectState> {
    private final ProjectState.Factory projectStateFactory;
    private final SchemaFactory<ReviewDb> schema;

    @Inject
    ByNameLoader(ProjectState.Factory psf, SchemaFactory<ReviewDb> sf) {
      projectStateFactory = psf;
      schema = sf;
    }
    ...
    public ProjectState createEntry(Project.NameKey key) throws Exception {
      ...
      try {
        ...
        return projectStateFactory.create(db.projects().get(pn.getId()), rights);
      } finally {
        db.close();
      }
    }
