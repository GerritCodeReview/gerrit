...
public class ProjectControl {
  ...
  boolean match(AccountGroup.UUID uuid, boolean isChangeOwner) {
    if (SystemGroupBackend.PROJECT_OWNERS.equals(uuid)) {
      ...
      try {
        GroupControl i = controlFactory.controlFor(uuid);
        GroupResource gr = new GroupResource(i);
        GroupInfo info = json.format(i.getGroup());
        info.includes = listIncludes.get().apply(gr);
        for (GroupInfo gi : info.includes) {
          Set<UUID> tmp = user.getEffectiveGroups().getKnownGroups();
          for (UUID t : tmp) {
            if (t.toString().equals(gi.id)) {
              return true;
            }
          }
        }
