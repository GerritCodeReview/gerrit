...
public class ProjectUtil {
  ...
  public String addParent(ProjectControl cCtrl, ProjectControl pCtrl)
    ...
    if (getAncestors(Collections.singleton(pKey)).contains(cKey)) {
  ...
  private Set<Project.NameKey> getAncestors(Set<Project.NameKey> parents) {
    final Set<Project.NameKey> ancestors = new HashSet<Project.NameKey>();
    ancestors.add(wildProject);
    ancestors.addAll(parents);

    for (final Project.NameKey p: parents) {
      final ProjectState s = projectCache.get(p);
      if (s != null) {
        ancestors.addAll(getAncestors(s.getProject().getParents()));
      }
    }
    return ancestors;
  }

  public static void populateParents(final ReviewDb db, final Project p)
      throws OrmException {
