...
public class RefControl {
  ...
  private boolean canPerform(ApprovalCategory.Id actionId, short level) {
    ...
    allRights.addAll(getLocalRights());

    if (actionId.canInheritFromWildProject()) {
      allRights.addAll(getInheritedRights());
    }

    // Filter the rights for the correct approval category.
    List<RefRight> actionRights = new ArrayList<RefRight>();
    for (final RefRight right : allRights) {
      if (right.getApprovalCategoryId().equals(actionId)) {
        actionRights.add(right);
      }
    }

    // Sort in descending refPattern length
    Collections.sort(actionRights, new RefRight.RefPatternOrder());

    List<RefRight> mostSpecific = filterMostSpecific(actionRights);

    for (RefRight right : mostSpecific) {
  ...
  private List<RefRight> getLocalRights() {
    return filter(projectControl.getProjectState().getLocalRights());
  }

  private List<RefRight> getInheritedRights() {
