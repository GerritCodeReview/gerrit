...
import static com.google.gerrit.reviewdb.ApprovalCategory.PUSH_TAG_LIGHTWEIGHT;
...
public class RefControl {
  ...
  public boolean canCreate(RevWalk rw, RevObject object) {
    ...
    if (object instanceof RevCommit) {
      if (owner) {
        return true;
      } else if (getRefName().startsWith(Constants.R_TAGS)) {
        return canPerform(PUSH_TAG, PUSH_TAG_LIGHTWEIGHT);
      } else {
        return canPerform(PUSH_HEAD, PUSH_HEAD_CREATE);
      }
