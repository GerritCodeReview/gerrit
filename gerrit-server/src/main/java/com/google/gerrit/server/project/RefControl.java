...
public class RefControl {
  ...
  private Map<String, List<Rule>> permissions;
  ...
  public boolean canUpload() {
    return canPerform(Permission.UPLOAD);
  }
  ...
  private boolean canPushWithForce() {
    for (Rule rule : access(Permission.PUSH)) {
      if (rule.isForce()) {
        return true;
      }
    }
  ...
  public List<Range> getLabelRanges() {
    List<Range> r = new ArrayList<Range>();
    for (Map.Entry<String, List<Rule>> e : permissions().entrySet()) {
  ...
  public Range getRange(String permission) {
  ...
  private static Range toRange(String permissionName, List<Rule> ruleList) {
    int min = 0;
    int max = 0;
    ...
    return new Range(permissionName, min, max);
  }
  ...
  private List<Rule> access(String permissionName) {
    List<Rule> r = permissions().get(permissionName);
    return r != null ? r : Collections.<Rule> emptyList();
  }
  ...
  private Map<String, List<Rule>> permissions() {
    if (permissions == null) {
      ...
      permissions = new HashMap<String, List<Rule>>();
          ...
          for (Rule rule : permission.getRules()) {
            if (matchGroup(rule.getGroup().getUUID())) {
              ...
              if (seen.add(s) && !rule.isDeny()) {
                List<Rule> r = permissions.get(permission.getName());
                if (r == null) {
                  r = new ArrayList<Rule>(2);
  ...
  private static class SeenRule {
    ...
    SeenRule(AccessSection section, Permission permission, Rule rule) {
