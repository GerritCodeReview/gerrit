...
import static com.google.gerrit.reviewdb.ApprovalCategory.ADD_BRANCH;
import static com.google.gerrit.reviewdb.ApprovalCategory.DELETE_BRANCH;
...
public class RefControl {
  ...
  public boolean canCreate(RevWalk rw, RevObject object) {
    ...
    if (object instanceof RevCommit) {
      return owner || canPerform(PUSH_HEAD, PUSH_HEAD_CREATE) || (webUI && canPerform(BRANCH_ADMIN, ADD_BRANCH));
  ...
  public boolean canDelete() {
    switch (getCurrentUser().getAccessPath()) {
      case WEB_UI:
        return isOwner() || canPerform(PUSH_HEAD, PUSH_HEAD_REPLACE) || canPerform(BRANCH_ADMIN, DELETE_BRANCH);
