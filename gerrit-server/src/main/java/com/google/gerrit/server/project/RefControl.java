...
public class RefControl {
  ...
  public boolean isVisibleByRegisteredUsers() {
    ...
    Set<Project.NameKey> allows = Sets.newHashSet();
    Set<Project.NameKey> blocks = Sets.newHashSet();
    for (PermissionRule rule : access) {
      if (rule.isBlock()) {
        blocks.add(rule.getProject());
        ...
        allows.add(rule.getProject());
      }
    }
    for (Project.NameKey p : blocks) {
      if (!allows.contains(p)) {
        return false;
      }
    }
    return !allows.isEmpty();
  }
  ...
  private static PermissionRange toRange(String permissionName,
    ...
    Map<Project.NameKey, AllowedRange> ranges = Maps.newHashMap();
    for (PermissionRule rule : ruleList) {
      Project.NameKey p = rule.getProject();
