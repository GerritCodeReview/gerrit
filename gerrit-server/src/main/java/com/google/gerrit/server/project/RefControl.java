...
public class RefControl {
   ...
   * this object.
   *
   * @return {@code true} if the user specified can submit a change to the Git
   *         ref
   */
  ...
  public boolean canPerform(ApprovalCategory.Id actionId, short level) {
    ...
    if (refRights.isEmpty()) {
      // No rights specific to this reference? Use project settings.
      return projectControl.canPerform(actionId, level);
    }
  ...
  private Collection<RefRight> getRefRights() {
    Collection<RefRight> all = projectControl.getProjectState().getRefRights();
    List<RefRight> mine = new ArrayList<RefRight>(all.size());
    for (RefRight right : all) {
      if (matches(refName, right.getRefPattern())) {
        mine.add(right);
      }
    }
    return mine;
