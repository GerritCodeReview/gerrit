...
public class RefControl {
  ...
  public boolean canSubmit() {
    ...
    final Set<AccountGroup.Id> groups =
      projectControl.getCurrentUser().getEffectiveGroups();
    int val = Integer.MIN_VALUE;
    for (final RefRight right : refRights) {
      if (right.getApprovalCategoryId().equals(SUBMIT)
  ...
  public boolean canUpload() {
    ...
    int val = Integer.MIN_VALUE;
    for (final RefRight right : refRights) {
      if (right.getApprovalCategoryId().equals(UPLOAD)
          && groups.contains(right.getAccountGroupId())
          && matches(refName, right.getRefPattern())) {
        if (val < 0 && right.getMaxValue() > 0) {
          // If one of the user's groups had denied them access, but
  ...
  private static boolean matches(String refName, String refPattern) {
