...
public class SetAccess implements RestModifyView<ProjectResource, ProjectAccessInput> {
  ...
  public ProjectAccessInfo apply(ProjectResource rsrc, ProjectAccessInput input)
    ...
    List<AccessSection> removals = getAccessSections(groups, input.remove);
    List<AccessSection> additions = getAccessSections(groups, input.add);
  ...
  private List<AccessSection> getAccessSections(
      Map<AccountGroup.UUID, GroupInfo> groups, Map<String, AccessSectionInfo> sectionInfos)
    ...
    for (Map.Entry<String, AccessSectionInfo> entry : sectionInfos.entrySet()) {
      ...
      for (Map.Entry<String, PermissionInfo> permissionEntry :
        ...
        for (Map.Entry<String, PermissionRuleInfo> permissionRuleInfoEntry :
          ...
          AccountGroup.UUID id = new AccountGroup.UUID(permissionEntry.getKey());
          try {
            GroupControl control = groupControlFactory.controlFor(id);
            if (control.isVisible()) {
              GroupInfo info = groupJson.format(control.getGroup());
              info.id = null;
              groups.put(id, info);
            }
