...
/**
 * Project renaming task to update the parent of all child projects
 */
public class ReparentChildProjectsTask implements Task {
  private static final Logger log = LoggerFactory
      .getLogger(ReparentChildProjectsTask.class);
  ...
  public interface Factory extends Task.Factory {
    ReparentChildProjectsTask create(@Assisted("source") Project.NameKey source,
        @Assisted("destination") Project.NameKey destination);
  }

  @Inject
  public ReparentChildProjectsTask(MetaDataUpdate.Server metaDataUpdateFactory,
    ...
    this.updatedProjectNameKeys = Sets.newHashSet();
  }
    ...
    try {
      for (Project.NameKey child : projectCache.all()) {
        ...
        if (childProjectState != null) {
          ...
          if (source.equals(childParentNameKey)) {
            try {
