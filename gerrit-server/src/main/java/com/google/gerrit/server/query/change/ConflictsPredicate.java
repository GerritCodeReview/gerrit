...
class ConflictsPredicate extends OrPredicate<ChangeData> {
  ...
  private static List<Predicate<ChangeData>> predicates(
    ...
    for (final Change c : changes) {
      final Cache cache = new Cache(c, db, projectCache);
      ...
      predicatesForOneChange.add(new OperatorPredicate<ChangeData>(
        ...
        public boolean match(ChangeData object) throws OrmException {
          ...
          ConflictKey conflictsKey =
              new ConflictKey(cache.getTestAgainst(), other, submitType,
                  cache.getProjectState().isUseContentMerge());
            ...
            try {
              ...
              try {
                ...
                CodeReviewCommit commit =
                    (CodeReviewCommit) rw.parseCommit(cache.getTestAgainst());
  ...
  private static class Cache {
    ...
    Cache(Change change, Provider<ReviewDb> db, ProjectCache projectCache) {
      this.change = change;
      this.db = db;
