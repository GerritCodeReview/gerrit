    ...
    for (final Change c : changes) {
      ...
      predicatesForOneChange.add(new OperatorPredicate<ChangeData>(
        ...
        public boolean match(ChangeData object) throws OrmException {
          ...
          ProjectState projectState = projectCache.get(c.getProject());
          if (projectState == null) {
            throw new IllegalStateException(
                new NoSuchProjectException(c.getProject()));
          }
          ObjectId other = ObjectId.fromString(
              object.currentPatchSet(db).getRevision().get());
          ConflictKey conflictsKey =
              new ConflictKey(cache.getTestAgainst(db, c), other, submitType,
                  projectState.isUseContentMerge());
          Boolean conflicts = conflictsCache.get(conflictsKey);
          if (conflicts != null) {
            return conflicts;
          }
          try {
