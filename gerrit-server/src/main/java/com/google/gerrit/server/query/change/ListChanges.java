...
public class ListChanges {
  ...
  private boolean isChangeReviewed(ChangeData cd) throws OrmException {
    if (user instanceof IdentifiedUser) {
      ...
      // To improve performance we use "db.changeMessages().byPatchSet()"
      // instead of "cd.messages()"
      List<ChangeMessage> messages = db.get().changeMessages().byPatchSet(patchSetId).toList();
