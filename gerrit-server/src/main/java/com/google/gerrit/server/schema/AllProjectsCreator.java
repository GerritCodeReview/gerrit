...
public class AllProjectsCreator {
  ...
  private void initAllProjects(Repository git)
    ...
    grant(config, heads, Permission.EDIT_TOPIC_NAME, true, admin, owners, changeOwner);
    grant(config, heads, Permission.ABANDON, changeOwner);
    grant(config, heads, Permission.REBASE, changeOwner);
    grant(config, heads, Permission.REMOVE_REVIEWER, changeOwner);
    grant(config, heads, Permission.VIEW_DRAFTS, changeOwner);
    grant(config, heads, Permission.PUBLISH_DRAFTS, changeOwner);
    grant(config, heads, Permission.DELETE_DRAFTS, changeOwner);
