...
public class SchemaCreator {
  ...
  private SystemConfig initSystemConfig(final ReviewDb c) throws OrmException {
    ...
    anonymous = newGroup(c, "Anonymous Users", null);
    ...
    registered = newGroup(c, "Registered Users", null);
    ...
    s.anonymousGroupUUID = anonymous.getGroupUUID();

    s.registeredGroupId = registered.getId();
    s.registeredGroupUUID = registered.getGroupUUID();
  ...
  private void initWildCardProject() throws IOException, ConfigInvalidException {
    ...
    try {
      ...
      p.setName(DEFAULT_WILD_NAME.get());
      ...
      Rule review = rule(config, registered);
      review.setRange(-1, 1);
      ...
      all.getPermission(Permission.UPLOAD, true) //
      ...
      commit.setMessage("Initialized Gerrit Code Review " + Version.getVersion());
  ...
  private Rule rule(ProjectConfig config, AccountGroup group) {
    return new Rule(config.resolve(group));
  }
