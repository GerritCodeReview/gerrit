...
public class Schema_106 extends SchemaVersion {
  ...
  protected void migrateData(ReviewDb db, UpdateUI ui) throws OrmException {
    ...
    runParallelTasks(
        createExecutor(ui, getThreadCount(repoList.size())),
        repoList,
        (repo) -> createRefLog((Project.NameKey) repo),
        ui);
  }

  private int getThreadCount(int repoCount) {
    int procs = Runtime.getRuntime().availableProcessors();
    return Math.min(procs * THREADS_PER_CPU, repoCount);
  }
