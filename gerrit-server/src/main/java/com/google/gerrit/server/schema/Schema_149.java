...
public class Schema_149 extends SchemaVersion {
  private static final String COMMIT_MSG = "Rewrite refs/for/ ACLs to use refs/heads/";
        ...
        for (AccessSection accessSection : config.getAccessSections()) {
          ...
          if (accessSection.getName().startsWith("refs/for/")) {
              ...
              switch (permission.getName()) {
                ...
                case ADD_PATCH_SET:
                  ...
                  for (PermissionRule rule : permission.getRules()) {
                    updateRule(config, destinationRef, permission, rule);
            ...
            for (Permission permission : accessSection.getPermissions()) {
              if (permission.getName().equals(PUSH)) {
                for (PermissionRule rule : permission.getRules()) {
                  updateRule(config, sourceRef, permission, rule);
                  configUpdated = true;
                  break;
                }
              }
  ...
  public static void updateRule(
    ...
    AccessSection s = config.getAccessSection(destinationRef, true);
    Permission p =
        s.getPermission(
            sourcePermission.getName().equals(PUSH) ? CREATE_REVIEW : sourcePermission.getName(),
            true);
    p.setExclusiveGroup(sourcePermission.getExclusiveGroup());
    PermissionRule r = rule(config, sourceRule.getGroup());
    r.setForce(sourceRule.getForce());
