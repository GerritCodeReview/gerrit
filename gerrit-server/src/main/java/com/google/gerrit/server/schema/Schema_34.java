...
class Schema_34 extends SchemaVersion {
  @Inject
  Schema_34() {
    super(new Provider<SchemaVersion>() {
      public SchemaVersion get() {
        throw new ProvisionException("Cannot upgrade from 33 or lower");
      }
    });
  }
  ...
  protected void upgradeFrom(UpdateUI ui, CurrentSchemaVersion curr,
    ...
    throw new OrmException("Cannot upgrade from " + curr.versionNbr
        + "; run first gerrit version 2.1.5 or 2.1.4 to upgrade the"
        + " schema at least to version 34"
        + "; then restart this new gerrit version.");
  }
}
