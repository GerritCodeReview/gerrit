    ...
    for( AccountGroupInclude agi : db.accountGroupIncludes().iterateAllEntities() ) {
      ...
      for( AccountGroupIncludeAudit agia : rs ) {
        destAuditEntry = new AccountGroupIncludeByUUIDAudit(destIncludeEntry,
            agia.getAddedBy(), agia.getKey().getAddedOn());
        destAuditEntry.removed(agia.getRemovedBy(), agia.getRemovedOn());
        destAuditTable.insert(Collections.singleton(destAuditEntry));
      }
