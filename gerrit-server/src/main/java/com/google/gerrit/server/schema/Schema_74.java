...
public class Schema_74 extends SchemaVersion {
  ...
  protected void migrateData(final ReviewDb db, final UpdateUI ui)
    ...
    ResultSet oldGroupIncludes = conn.createStatement().executeQuery("SELECT * FROM account_group_includes");
    while (oldGroupIncludes.next()) {
      AccountGroup.Id oldGroupId = new AccountGroup.Id(oldGroupIncludes.getInt("group_id"));
      int oldIncludeId = oldGroupIncludes.getInt("include_id");
      ...
      oldAuditsQuery.setInt(2, oldIncludeId);
