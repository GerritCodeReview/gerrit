...
public class Schema_80 extends SchemaVersion {
  ...
  @Inject
  Schema_80(Provider<Schema_79> prior, SitePaths sitePaths,
  ...
  private void migrateStartReplicationCapability(ReviewDb db, File[] matches)
    ...
    if (matches == null || matches.length == 0) {
      ad.what = Description.Action.REMOVE;
    } else {
      ad.what = Description.Action.RENAME;
      ad.prefix = nameOf(matches[0]);
    }
    migrateStartReplicationCapability(db, ad);
  }
  ...
  private void migrateStartReplicationCapability(ReviewDb db, Description d)
    ...
    try {
      ...
      Permission startReplication = null;
      for (Permission permission : capabilities.getPermissions()) {
        if (permission.getName().equals("startReplication")) {
          startReplication = permission;
          break;
        }
      }
      ...
      md.setMessage("Upgrade to Gerrit Code Review schema 80\n");
      config.commit(md);
    } finally {
      git.close();
    }
