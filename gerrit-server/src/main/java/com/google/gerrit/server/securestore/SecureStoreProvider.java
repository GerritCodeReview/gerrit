...
public class SecureStoreProvider implements Provider<DynamicItem<SecureStore>> {
  ...
  public DynamicItem<SecureStore> get() {
    ...
    for (SecureStoreData store : stores) {
      if (store.getStoreName().equals(secureStore)) {
        ...
        return DynamicItem.<SecureStore> of(new SecureStoreWrapper(injector
            .getInstance(secureStoreImpl)));
      }
    }
    throw new SecureStoreException(String.format(
