...
public class SecureStoreProvider implements Provider<SecureStore> {
  ...
  public SecureStore get() {
    ...
    String msg =
        String.format("Cannot find secure store with name %s in plugin %s",
            storeName, pluginName);
    throw new SecureStoreException(msg);
  }
  ...
  private Set<SecureStoreData> getSecureStores(String pluginName) {
    final String pluginFileName = pluginName + ".jar";
    File[] plugins = paths.plugins_dir.listFiles(new FilenameFilter() {
      @Override
      public boolean accept(File file, String name) {
        return name.equals(pluginFileName);
      }
    });
    if (plugins.length != 1) {
      String msg = String.format("Cannot find plugin file: $s", pluginFileName);
      throw new SecureStoreException(msg);
    }
    Set<SecureStoreData> result = Sets.newHashSet();
    try {
      result.addAll(findSecureStores(plugins[0], pluginName));
