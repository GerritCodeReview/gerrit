 ...
 * </ul>
 *
 * Levenshtein distance is a measure of the similarity between two strings. The
 * distance is the number of deletions, insertions, or substitutions required to
 * transform one string into another.
 *
 * For example, if given refs/heads/m* and refs/heads/*, the distances are 5 and
 * 6. It means that refs/heads/m* is more specific because it's closer to
 * refs/heads/master than refs/heads/*.
 *
 * Another example could be refs/heads/* and refs/heads/[a-zA-Z]*, the distances
 * are both 6. Both are infinite, but refs/heads/[a-zA-Z]* has more transitions,
 * which after all turns it more specific.
 */
public class RegexRefsComparator implements Comparator<String> {

  private String refName;

  public RegexRefsComparator(String refName) {
  ...
  private int transitions(String pattern) {
    if (RefControl.isRE(pattern)) {
      return RefControl.toRegExp(pattern).toAutomaton().getNumberOfTransitions();
