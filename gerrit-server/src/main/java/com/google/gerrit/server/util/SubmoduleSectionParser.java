...
public class SubmoduleSectionParser {
  ...
  private SubmoduleSubscription parse(final String id) {
    SubmoduleSubscription parsedSubscription = null;
    ...
    Branch.NameKey submoduleBranch = null;
    ...
    try {
      if (url != null && url.length() > 0 && path != null && path.length() > 0
        ...
        if ((urlIsRelative)
          ...
          if (revision.equals(".")) {
            revision = superProjectBranch.get();
    ...
    if (submoduleBranch != null) {
      parsedSubscription =
          new SubmoduleSubscription(superProjectBranch, submoduleBranch, path);
    }
