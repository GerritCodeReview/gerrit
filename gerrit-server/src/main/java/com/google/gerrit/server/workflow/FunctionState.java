...
public class FunctionState {
  ...
  private static Map<Id, Collection<RefRight>> indexRefRights(
      final Collection<RefRight> rights, final String destRef) {
    ...
    for (final RefRight r : rights) {
      if (RefControl.matches(destRef, r.getRefPattern())) {
  ...
  public void applyRightFloor(final PatchSetApproval a) {
    ...
    // Find the maximal range granted to the user for the ref
    short refMinAllowed = 0, refMaxAllowed = 0;
    boolean foundRefRight = false;
    ...
    if (foundRefRight) {
      minAllowed = refMinAllowed;
      maxAllowed = refMaxAllowed;
    } else {
      minAllowed = projectMinAllowed;
