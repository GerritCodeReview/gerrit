...
%%
check_label_range_permission(Type, ExpValue, ok(Who)) :-
  commit_label(label(Type, ExpValue), Who),
  call((
    user:in_group(Who, Group),
    user:grant_range(Type, Group, Min, Max)
  )),
  Min @=< ExpValue, ExpValue @=< Max
  .
check_label_range_permission(Type, ExpValue, ask(Group)) :-
  call(user:grant_range(Type, Group, Min, Max)),
  Min @=< ExpValue, ExpValue @=< Max
  .
