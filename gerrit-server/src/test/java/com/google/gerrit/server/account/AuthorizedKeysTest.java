...
public class AuthorizedKeysTest {
  ...
  private static void assertParse(StringBuilder authorizedKeys,
    ...
    SortedMap<Integer, Optional<AccountSshKey>> parsedKeys =
        AuthorizedKeys.parse(accountId, authorizedKeys.toString());
    assertThat(parsedKeys.values()).containsExactlyElementsIn(expectedKeys);
    int seq = 1;
    for(Map.Entry<Integer, Optional<AccountSshKey>> e : parsedKeys.entrySet()) {
      assertThat(e.getKey()).isEqualTo(seq++);
      if (e.getValue().isPresent()) {
        assertThat(e.getValue().get().getAccount()).isEqualTo(accountId);
        assertThat(e.getValue().get().getKey().get()).isEqualTo(e.getKey());
      }
    }
  }
  ...
  private static String addInvalidKey(List<Optional<AccountSshKey>> keys,
    ...
    key.setInvalid();
    keys.add(Optional.of(key));
    return AuthorizedKeys.INVALID_KEY_COMMENT + key.getSshPublicKey() + "\n";
  }
