...
public class ExternalIdsConfigTest {
  ...
  public void parseExternalIdsConfig() throws Exception {
    ...
    Multimap<String, ExternalId> externalIds =
        ExternalIdsConfig.parse(new Account.Id(1000000), cfg);
  ...
  public void parseExternalIdsConfigWithInvalidScheme() throws Exception {
    Config cfg = new Config();
    cfg.fromText("[scheme \"INVALID\"]\n"
        + "  id = jdoe\n");

    exception.expect(ConfigInvalidException.class);
    exception.expectMessage(
        "Invalid scheme INVALID in external ids of account 1000000");
    ExternalIdsConfig.parse(new Account.Id(1000000), cfg);
  }
