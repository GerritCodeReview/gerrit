...
/** Unit test for {@link BatchUpdate} */
public class BatchUpdateTest {
  ...
  public void addRefUpdateFromFastForwardCommit() throws Exception {
    ...
    final RevCommit masterTip = repo.parseBody(masterCommit);
    RevCommit branchCommit =
        repo.branch("branch").commit().parent(masterTip).create();
    final RevCommit branchTip = repo.parseBody(branchCommit);
    ...
    try (BatchUpdate bu = batchUpdateFactory
      ...
      bu.addRepoOnlyOp(new RepoOnlyOp() {
        ...
        public void updateRepo(RepoContext ctx) throws Exception {
          ctx.addRefUpdate(
              new ReceiveCommand(masterTip.getId(), branchTip.getId(),
                  "refs/heads/master"));
        }
      });
      bu.execute();
