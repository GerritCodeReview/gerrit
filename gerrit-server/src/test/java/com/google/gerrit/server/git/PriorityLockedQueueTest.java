...
public class PriorityLockedQueueTest {
  ...
  AtomicLongMap<String> processedEntries;
  ...
  public void setup() {
    ...
    processedEntries = AtomicLongMap.create();
  }
  ...
  public void testOneProcessLockingDuringProcessing() throws Exception {
    ...
    assertThat(processedEntries.get("A")).isEqualTo(1);
  ...
  public void testOneProcessLockingDuringScheduling() throws Exception {
    ...
    testPriorityLockedQueue.latchAttempt = new CountDownLatch(1);
    ...
    testPriorityLockedQueue.latchAttempt.countDown();
  ...
  public void testParallelScheduling() throws Exception {
    TestingTask task1 = new TestingTask(Sets.newHashSet("C"), false, 0, 1);
    TestingTask task2 = new TestingTask(Sets.newHashSet("D"), false, 0, 1);
    ...
    task1.awaitAfter();
    task2.awaitAfter();
    assertThat(processedEntries.get("C")).isEqualTo(1);
    assertThat(processedEntries.get("D")).isEqualTo(1);
  }


  @Test
  public void testResourceConflict() throws Exception {
    TestingTask task1 = new TestingTask(Sets.newHashSet("E", "F"), false, 0, 1);
    TestingTask task2 = new TestingTask(Sets.newHashSet("E", "G"), false, 0, 0);
    ...
    assertThat(processedEntries.get("E")).isEqualTo(2);
    assertThat(processedEntries.get("F")).isEqualTo(1);
    assertThat(processedEntries.get("G")).isEqualTo(1);
  }
  ...
  class PriorityLockedQueueUnderTest extends
    ...
    CountDownLatch latchAttempt = new CountDownLatch(0);
    ...
    void processTestingTask(final TestingTask task) {
      synchronized (processedEntries) {
        for (String s : task.resources()) {
          processedEntries.addAndGet(s, 1);
        }
      }
    }

    @Override
    protected boolean tryRunTask(final TestingTask task, boolean async,
        long timeout) {
      try {
        latchAttempt.await();
      } catch (InterruptedException e) {
      ...
      return super.tryRunTask(task, async, timeout);
    }
