...
public class ProjectConfigTest extends LocalDiskRepositoryTestCase {
  ...
  public void testReadConfig() throws Exception {
    RevCommit rev = util.commit(util.tree( //
        util.file("groups", util.blob("X Developers\n")), //
        util.file("project.config", util.blob(""//
            + "[access \"refs/heads/*\"]\n" //
            + "  doNotInherit = read submit create\n" //
    ...
    Permission upload = section.getPermission(Permission.UPLOAD);

    assertFalse(create.getInherit());
    assertFalse(submit.getInherit());
    assertFalse(read.getInherit());
    assertTrue(upload.getInherit());
  }
