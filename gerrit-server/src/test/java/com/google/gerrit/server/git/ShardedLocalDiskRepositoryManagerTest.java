...
public class ShardedLocalDiskRepositoryManagerTest {
  ...
  private ShardedLocalDiskRepositoryManager repoManager;
  ...
  public void setUp() throws IOException {
    ...
    expect(configMock.getAllBasePaths()).andReturn(new String[] {}).anyTimes();
    replay(configMock);
    repoManager =
        new ShardedLocalDiskRepositoryManager(site, cfg,
  ...
  public void testAlternateRepositoryLocation() throws IOException {
    ...
    Project.NameKey someProjectKey = new Project.NameKey("someProject");
    reset(configMock);
    expect(configMock.getBasePath(someProjectKey)).andReturn(
        alternateBasePath.getAbsolutePath()).anyTimes();
    expect(configMock.getAllBasePaths()).andReturn(
  ...
  public void testListReturnRepoFromProperLocation() throws IOException {
    ...
    reset(configMock);
    expect(configMock.getBasePath(altPathProject)).andReturn(
        alternateBasePath.getAbsolutePath()).anyTimes();
    expect(configMock.getBasePath(misplacedProject2)).andReturn(
        alternateBasePath.getAbsolutePath()).anyTimes();
    expect(configMock.getAllBasePaths()).andReturn(
        new String[] {alternateBasePath.getAbsolutePath()}).anyTimes();
    replay(configMock);
    ...
    createRepository(repoManager.getBasePath(basePathProject).toFile(),
        misplacedProject2);
  ...
  private void createRepository(File directory, Project.NameKey projectName)
      throws IOException {
  ...
  public void testRelativeAlternateLocation() {
    configMock = createNiceMock(RepositoryConfig.class);
    expect(configMock.getAllBasePaths()).andReturn(new String[] {"repos"})
        .anyTimes();
    replay(configMock);
    repoManager =
        new ShardedLocalDiskRepositoryManager(site, cfg,
