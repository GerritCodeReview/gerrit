...
public class SubmoduleOpTest extends LocalDiskRepositoryTestCase {
  ...
  public void testEmptyCommit() throws Exception {
    ...
    expect(subscriptions.bySubscription(branchNameKey)).andReturn(
        emptySubscriptions);
  ...
  public void testOneSubscriberToUpdate() throws Exception {
    ...
    expect(subscriptions.bySubscription(sourceBranchNameKey)).andReturn(
        subscribers);
    ...
    final ResultSet<SubmoduleSubscription> emptySubscriptions =
        new ListResultSet<SubmoduleSubscription>(new ArrayList<SubmoduleSubscription>());
    expect(subscriptions.bySubscription(targetBranchNameKey)).andReturn(
        emptySubscriptions);
  ...
  private void doOnlySubscriptionTableOperations(
    ...
    expect(schema.submoduleSubscriptions()).andReturn(subscriptions);
    expect(subscriptions.bySubscriber(mergedBranch)).andReturn(
    ...
    for (SubmoduleSubscription extracted : extractedSubscriptions) {
      existingProjects.add(extracted.getSubscription().getParentKey());
    }
    ...
    final List<SubmoduleSubscription> alreadySubscribeds = new ArrayList<SubmoduleSubscription>();
    ...
    final List<SubmoduleSubscription> subscriptionsToRemove =
        new ArrayList<SubmoduleSubscription>(previousSubscriptions);
    final List<SubmoduleSubscription> subscriptionsToInsert =
    ...
    expect(schema.submoduleSubscriptions()).andReturn(subscriptions);
    expect(subscriptions.bySubscription(mergedBranch)).andReturn(
        new ListResultSet<SubmoduleSubscription>(new ArrayList<SubmoduleSubscription>()));
  ...
  private static StringBuilder buildSubmoduleSection(final String name,
      final String path, final String url, final String revision) {
