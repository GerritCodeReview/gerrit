...
/**
 * This is no test but a demo of the difference between default RateLimiter and one with a higher
 * maxBurstSeconds which can only be used by hacking the non-public constructor of the hidden
 ...
 * doesn't allow for any bursts since it hard-codes the maximum time which can be used to store
 * unused permits to 1 seconds.
 */
public class RateLimiterDemoTest extends TestCase {
  ...
  private void explore(double permitsPerSecond, int maxBurstSeconds) throws InterruptedException {
    int permitInterval = (int) Math.round(1 / permitsPerSecond);
    ...
    int w = permitInterval;
    do {
      measure(w);
      w = w * 2;
    } while (w < 8 * maxBurstSeconds);
  }
