...
public class IndexRewriteTest {
  ...
  public void testThreeLevelTreeWithAllIndexPredicates() throws Exception {
    ...
    assertEquals(
        query(parse("status:new OR status:submitted OR status:draft"
            + " OR status:workinprogress OR status:merged")),
  ...
  public void testGetPossibleStatus() throws Exception {
    ...
    assertEquals(EnumSet.of(SUBMITTED, DRAFT, MERGED, ABANDONED, WORKINPROGRESS),
        status("-is:new"));
