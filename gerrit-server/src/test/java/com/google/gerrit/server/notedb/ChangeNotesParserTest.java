...
public class ChangeNotesParserTest extends AbstractChangeNotesTest {
  ...
  public void parseWorkInProgress() throws Exception {
    ...
    state = assertParseSucceeds(
        "New change that doesn't declare WIP\n"
            + "\n"
            + "Patch-set: 1\n");
    assertThat(state.hasReviewStarted()).isTrue();
  }
  ...
  private RevCommit writeCommit(String body, PersonIdent author, boolean initWorkInProgress) throws Exception {
