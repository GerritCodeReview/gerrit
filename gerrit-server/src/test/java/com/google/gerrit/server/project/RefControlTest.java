...
public class RefControlTest extends TestCase {
  ...
  public void testUnblockOnDifferentRefPattern1() {
    grant(local, PUSH, anonymous, "refs/*").setBlock();
    grant(local, PUSH, devs, "refs/heads/master");

    ProjectControl u = user(devs);
    assertTrue("u can push", u.controlForRef("refs/heads/master").canUpdate());
    assertFalse("u can't push", u.controlForRef("refs/heads/stable").canUpdate());
  }

  public void testUnblockOnDifferentRefPattern2() {
    grant(local, PUSH, anonymous, "refs/heads/*").setBlock();
    grant(local, PUSH, devs, "refs/*");
