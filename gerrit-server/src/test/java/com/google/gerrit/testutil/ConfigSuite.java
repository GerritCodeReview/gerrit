...
public class ConfigSuite extends Suite {
  ...
  private static List<Runner> runnersFor(Class<?> clazz) {
    ...
    try {
      ...
      for (Map.Entry<String, org.eclipse.jgit.lib.Config> e :
          callConfigMapMethod(configMap).entrySet()) {
  ...
  private static Map<String, org.eclipse.jgit.lib.Config> callConfigMapMethod(Method m) {
    ...
    checkArgument(
        String.class.getTypeName().equals(types[0].getTypeName()),
        "The map returned by %s must have String as key",
        m);
    checkArgument(
        org.eclipse.jgit.lib.Config.class.getTypeName().equals(types[1].getTypeName()),
