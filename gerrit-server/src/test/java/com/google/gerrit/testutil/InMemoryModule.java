...
public class InMemoryModule extends FactoryModule {
  ...
  private Module luceneIndexModule() {
    try {
      ...
      if (version > 0) {
        singleVersions.put(ChangeIndexDefintion.NAME, version);
      }
      Class<?> clazz =
          Class.forName("com.google.gerrit.lucene.LuceneIndexModule");
      Constructor<?> c = clazz.getConstructor(Map.class, int.class);
      return (Module) c.newInstance(singleVersions, 0);
