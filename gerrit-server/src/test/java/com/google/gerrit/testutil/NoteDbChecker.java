...
public class NoteDbChecker {
  ...
  public void rebuildAndCheckAllChanges() throws Exception {
    rebuildAndCheckChanges(
        Iterables.transform(getUnwrappedDb().changes().all(), ReviewDbUtil.changeIdFunction()));
  }
  ...
  public void rebuildAndCheckChanges(Iterable<Change.Id> changeIds) throws Exception {
  ...
  private List<ChangeBundle> readExpected(Iterable<Change.Id> changeIds) throws Exception {
    ReviewDb db = getUnwrappedDb();
    boolean old = notesMigration.readChanges();
    try {
      notesMigration.setReadChanges(false);
      List<Change.Id> sortedIds = ReviewDbUtil.intKeyOrdering().sortedCopy(changeIds);
  ...
  private void checkActual(List<ChangeBundle> allExpected, List<String> msgs) throws Exception {
    ...
    try {
      ...
      for (ChangeBundle expected : allExpected) {
        ...
        try {
          actual =
              ChangeBundle.fromNotes(plcUtil, notesFactory.create(db, c.getProject(), c.getId()));
        } catch (Throwable t) {
