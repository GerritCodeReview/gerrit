...
class SshLog implements LifecycleListener {
  ...
  private String extractWhat(DispatchCommand dcmd, boolean hideSensitive) {
    ...
    return hideSensitive ? extractWhatWithMasking(dcmd) : extractWhatWithoutMasking(dcmd);
  }

  private String extractWhatWithoutMasking(DispatchCommand dcmd) {
  ...
  private String extractWhatWithMasking(DispatchCommand dcmd) {
    StringBuilder commandName = new StringBuilder(dcmd.getCommandName());
    String[] args = dcmd.getArguments();
    boolean maskNext = false;
    for (int i = 1; i < args.length; i++) {
      if (maskNext) {
        commandName.append(".").append(MASK);
        maskNext = false;
        continue;
      }
      String arg = args[i];
      String key = extractKey(arg);
      if (dcmd.isSensitiveParameter(key)) {
