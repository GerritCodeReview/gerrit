...
class SshLog implements LifecycleListener {
  ...
  void onExecute(DispatchCommand dcmd, int exitValue, SshSession sshSession) {
    ...
    audit(ctx, status, cmd, parameters);
  }

  private ListMultimap<String, ?> extractParameters(DispatchCommand dcmd, boolean hideSensitive) {
    if (dcmd == null) {
    ...
    for (int i = 2; i < cmdArgs.length; i++) {
      ...
      if (arg.startsWith("--") && eqPos > 0) {
        ...
        String value =
            hideSensitive && dcmd.isSensitiveParameter(param) ? MASK : arg.substring(eqPos + 1);
      ...
      if (paramName == null) {
        ...
        parms.put(paramName, hideSensitive && dcmd.isSensitiveParameter(paramName) ? MASK : arg);
        paramName = null;
      }
    }
    if (paramName != null) {
  ...
  private String extractWhat(DispatchCommand dcmd, boolean hideSensitive) {
    ...
    for (int i = 1; i < args.length; i++) {
      String arg = args[i];
      if (!hideSensitive) {
        commandName.append(".").append(arg);
        continue;
      }
      int eqPos = arg.indexOf('=');
      if (eqPos > 0) {
        String param = arg.substring(0, eqPos);
        if (dcmd.isSensitiveParameter(param)) {
          arg = param + "=" + MASK;
        }
      } else if (dcmd.isSensitiveParameter(arg)) {
        commandName.append(".").append(arg);
        arg = MASK;
        i++;
      }
