...
final class AdminSetParent extends BaseCommand {
  ...
  private List<Project.NameKey> getAllParents(final Project.NameKey projectName) {
    final List<Project.NameKey> parents = new ArrayList<Project.NameKey>();
    final ProjectState e = projectCache.get(projectName);
    if (e != null) {
      final Project.NameKey parentName = getParentName(e.getProject());
      if (parentName != null) {
        parents.add(parentName);
        parents.addAll(getAllParents(parentName));
      }
    }
    return parents;
  }
