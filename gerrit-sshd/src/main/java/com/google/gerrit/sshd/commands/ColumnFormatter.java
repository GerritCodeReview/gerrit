...
public class ColumnFormatter {
   ...
   *        characters found in the column text are ever escaped, the column
   *        separator must be a consequence of non-printable characters if
   *        the output needs to be unambiguously parsed.
   */
  public ColumnFormatter(final PrintWriter out, final String columnSeparator) {
    this.out = out;
  ...
  public void nextLine() {
    if (!firstColumn) {
      // Avoid println() to ensure consistent behavior regardless of
      // the default line separator for the platform where Gerrit is run.
      out.print('\n');
      firstColumn = true;
    }
  }
