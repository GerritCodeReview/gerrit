...
final class CreateProject extends BaseCommand {
    ...
    startThread(new CommandRunnable() {
      ...
      public void run() throws Exception {
        ...
        try {
          ...
          throw new Failure(
              1,
              "fatal: the name does not denote an existing repository, or the name cannot be read as a repository. ",
              err);
        } catch (IOException err) {
          throw new Failure(1, "fatal: IO exception. ", err);
        } catch (ConfigInvalidException err) {
          throw new Failure(1,
              "fatal: invalid configuration of " + projectName, err);
        } catch (Exception err) {
  ...
  private void createProject() throws RepositoryNotFoundException, IOException,
       ConfigInvalidException, CreateProjectParamsException {
