...
public class GarbageCollectionCommand extends BaseCommand {
  ...
  private void collectGarbage() {
    ...
    final GarbageCollectionResult result =
        garbageCollectionFactory.create().run(projectNames,
            new TextGarbageCollectionProgressMonitor(stdout));
    if (result.hasErrors()) {
  ...
  private class TextGarbageCollectionProgressMonitor extends
      TextProgressMonitor implements GarbageCollectionProgressMonitor {
    private final PrintWriter out;

    public TextGarbageCollectionProgressMonitor(final PrintWriter out) {
      super(out);
      this.out = out;
    }

    @Override
    public void startGarbageCollection(NameKey projectName) {
      out.print("collecting garbage for \"" + projectName + "\":\n");
    }

    @Override
    public void endGarbageCollection() {
      out.print("done.\n\n");
    }
  }
}
