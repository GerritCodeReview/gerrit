...
public class SetMembersCommand extends BaseCommand {
  ...
  @Option(name = "--add", aliases = {"-a"}, metaVar = "USER",
      usage = "user that should be added as group member")
  private List<Account.Id> accountsToAdd = new ArrayList<Account.Id>();

  @Option(name = "--remove", aliases = {"-r"}, metaVar = "USER",
      usage = "user that should be removed from the group")
  private List<Account.Id> accountsToRemove = new ArrayList<Account.Id>();

  @Option(name = "--include", aliases = {"-i"}, metaVar = "GROUP",
  ...
  @Option(name = "--remove-group", aliases = {"-rg"}, metaVar = "GROUP",
      usage = "group that should be removed from the group")
  private List<AccountGroup.Id> groupsToRemove = new ArrayList<AccountGroup.Id>();

  @Argument(index = 0, required = true, multiValued = true, metaVar = "GROUP",
  ...
  private boolean modifyOne(final AccountGroup.Id groupId) throws Exception {
    ...
    ok &= processResult(groupId, result, false);

    result = removeGroupIncludeFactory.create(groupId, groupsToRemove).call();
    ok &= processResult(groupId, result, false);

    result = addGroupMemberFactory.create(groupId, accountsToAdd).call();
    ok &= processResult(groupId, result, true);
  ...
  private boolean processResult(final AccountGroup.Id groupId,
      final GroupMemberResult result, final boolean add) {
