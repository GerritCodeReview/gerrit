  ...
  private void validate() throws UnloggedFailure {
    if(isBothSet(contentMerge, noContentMerge)) {
      throw new UnloggedFailure(1,
          "--use-content-merge and --no-content-merge options are mutually exclusive.");
    }
    if(isBothSet(contributorAgreements, noContributorAgreements)) {
      throw new UnloggedFailure(1,
          "--use-contributor-agreements and --no-contributor-agreements options are mutually exclusive.");
    }
    if(isBothSet(signedOffBy, noSignedOffBy)) {
      throw new UnloggedFailure(1,
          "--use-signed-off-by and --no-signed-off-by options are mutually exclusive.");
    }
    if(isBothSet(requireChangeID, noRequireChangeID)) {
      throw new UnloggedFailure(1,
          "--use-change-id and --no-change-id options are mutually exclusive.");
    }
  }

  private static boolean isBothSet(final Boolean on, final Boolean off) {
