...
final class ShowCaches extends SshCommand {
  ...
  private void threadSummary() {
    ...
    if (showThreads) {
      ...
      threadLoop: for (long id : threadMXBean.getAllThreadIds()) {
        ...
        for (String p : prefixes) {
          if (name.startsWith(p)) {
            ...
            count.put(p, info.getThreadState(), c != null ? c + 1 : 1);
            continue threadLoop;
          }
        }
        Integer c = count.get(other, state);
        count.put(other, info.getThreadState(), c != null ? c + 1 : 1);
      }
      ...
      stdout.print(String.format("  %-22s", other));
      for (Thread.State s : Thread.State.values()) {
        stdout.print(String.format(" %14d", nullToZero(count.get(other, s))));
      }
      stdout.print('\n');
    }
    stdout.print('\n');
