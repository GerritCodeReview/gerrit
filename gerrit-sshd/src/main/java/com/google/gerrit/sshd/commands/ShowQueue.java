...
final class ShowQueue extends SshCommand {
  ...
  private List<QueueTaskInfo> getSortedTaskInfoList() {
    final List<QueueTaskInfo> taskInfos =
        workQueue.getTaskInfos(new TaskInfo.Factory<QueueTaskInfo>() {
          @Override
    ...
    Collections.sort(taskInfos, new Comparator<QueueTaskInfo>() {
      ...
      public int compare(QueueTaskInfo aTaskInfo, QueueTaskInfo bTaskInfo) {
        ...
        return aTaskInfo.getTaskString(taskNameWidth).compareTo(
            bTaskInfo.getTaskString(taskNameWidth));
      }
    });
    return taskInfos;
  }
  ...
  public class QueueTaskInfo implements TaskInfo {
    private long delay;
    private Task.State state;
    private Task<?> task;

    public QueueTaskInfo(Task<?> task) {
      this.task = task;
