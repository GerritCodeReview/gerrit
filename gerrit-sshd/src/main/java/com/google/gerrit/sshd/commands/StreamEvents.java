...
final class StreamEvents extends BaseCommand {
  ...
  private void addChangeListener() throws OrmException {
    EventSource src = source.get();
    if (src != null) {
      if (sequenceId == null) {
        src.addChangeListener(listener, currentUser);
      } else {
        src.addChangeListener(listener, currentUser, sequenceId, db.get());
      }
    }
  }
  ...
  private void removeChangeListener() {
    EventSource src = source.get();
    if (src != null) {
      src.removeChangeListener(listener);
    }
  }
