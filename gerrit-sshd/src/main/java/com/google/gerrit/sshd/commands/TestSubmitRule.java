...
final class TestSubmitRule extends SshCommand {
  ...
  @Option(name = "--use-filters", aliases = {"-f"},
      usage = "Also consult submit filter rules in parent projects")
  ...
  protected void run() throws UnloggedFailure {
    ...
    try {
      ...
      for (SubmitRecord r : res) {
        if (format.isJson()) {
          ProcessedSubmitStatus processedSubmitStatus = new ProcessedSubmitStatus();
          processedSubmitStatus.value = r.status.toString();

          List<ApprovalAttribute> approvalAttributes = new LinkedList<ApprovalAttribute>();
          for(Label l : r.labels) {
            ApprovalAttribute approvalAttribute = new ApprovalAttribute();
            approvalAttribute.type = l.label;
            approvalAttribute.value = l.status.toString();
            if(l.appliedBy != null) {
              Account a = accountCache.get(l.appliedBy).getAccount();
              AccountAttribute accountAttribute = new AccountAttribute();
              accountAttribute.email = a.getPreferredEmail();
              accountAttribute.name = a.getFullName();
              accountAttribute.username = a.getUserName();
              approvalAttribute.by = accountAttribute;
            }
            approvalAttributes.add(approvalAttribute);
          }
         processedSubmitStatus.approvals = approvalAttributes;
         format.newGson().toJson(processedSubmitStatus, new TypeToken<ProcessedSubmitStatus>() {}.getType(), outWriter);
         outWriter.print('\n');
        } else {
          for(Label l : r.labels) {
            outWriter.print(l.label + ": " + l.status);
            if(l.appliedBy != null) {
              AccountInfo a = new AccountInfo(accountCache.get(l.appliedBy).getAccount());
              outWriter.print(" by " + a.getNameEmail(anonymousCowardName));
            }
            outWriter.print('\n');
          }
          outWriter.println("\n" + r.status);
  ...
  private static class ProcessedSubmitStatus {
