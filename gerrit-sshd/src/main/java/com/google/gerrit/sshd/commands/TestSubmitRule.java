...
final class TestSubmitRule extends SshCommand {
  ...
  @Option(name = "--no-filters", aliases = {"-f"},
      usage = "Don't run the submit_filter/2 from the parent projects of the specified change.")
    ...
    try {
      ...
      for (SubmitRecord r : res) {
        if (format.isJson()) {
          ...
          submitRecord.status = r.status.toString();
          ...
          for(Label l : r.labels) {
            ...
            label.label = l.label;
            label.status= l.status.toString();
            if(l.appliedBy != null) {
              ...
              label.by = accountAttribute;
            }
            submitLabels.add(label);
          }
          submitRecord.labels = submitLabels;
          format.newGson().toJson(submitRecord, new TypeToken<SubmitRecordAttribute>() {}.getType(), outWriter);
          outWriter.print('\n');
        } else {
          for(Label l : r.labels) {
