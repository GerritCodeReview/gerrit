...
/**
 * This class formats a graph of objects as tree. The formatted tree is printed
 * to the given {@link PrintWriter}. The object graph that should be formatted
 * and printed as tree must be provided as a graph of {@link TreeNode}'s. The
 * tree formatter is able to detect recursions in the graph of TreeNode's.
 * Recursive nodes will be specially formatted and not further expanded. If the
 * graph of TreeNode's can contain different instances of TreeNode's that
 * represent the same object, the implementor of the TreeNode must take care to
 * implement the {@link Object#equals(Object)} and {@link Object#hashCode()}
 * methods. Otherwise the TreeFormatter is not able to detect the recursion.
 */
