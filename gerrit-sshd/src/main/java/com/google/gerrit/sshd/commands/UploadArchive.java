...
public class UploadArchive extends AbstractGitCommand {
  ...
  static class Options {
    @Option(name = "-f", aliases = {"--format"},
            usage = "Format of the resulting archive: tar or zip. If this option is not given, " +
                    "and the output file is specified, the format is inferred from the filename" +
                    "if possible (e.g. writing to \"foo.zip\" makes the output to be in the zip " +
                    "format). Otherwise the output format is tar.")
    private String format = "tar";

    @Option(name = "--prefix", usage = "Prepend prefix to each pathname in the archive")
    private String prefix = null;

    @Option(name = "-0", usage = "Store the files instead of deflating them.")
    private boolean level0 = false;
    @Option(name = "-1", usage = "Fastest compression level.")
    ...
    @Option(name = "-9",
            usage = "Highest and slowest compression level. You can specify any number from 1 to 9 to " +
    ...
    @Argument(index = 1, multiValued = true,
              usage = "Without an optional path parameter, all files and subdirectories " +
   ...
   * The compression level is stored in a thread local storage, which must be set before createArchiveOutputStream()
   */
  static class ZipFormat implements ArchiveCommand.Format<ArchiveOutputStream>  {

    org.eclipse.jgit.archive.ZipFormat zipFormat = new org.eclipse.jgit.archive.ZipFormat();

    public ArchiveOutputStream createArchiveOutputStream(OutputStream s) {
      ArchiveOutputStream archiveStream = zipFormat.createArchiveOutputStream(s);
      if (archiveStream instanceof ZipArchiveOutputStream) {
        ZipArchiveOutputStream zipStream = (ZipArchiveOutputStream)archiveStream;
    ...
    public void putEntry(ArchiveOutputStream out, String path, FileMode mode, org.eclipse.jgit.lib.ObjectLoader loader)
        throws IOException {
    ...
    private static final ThreadLocal<Integer> m_level = new ThreadLocal<Integer>();
  }

  /** Find the requested Zip compression and select it in ZipFormat.
   */
  private void setupZipCompressionLevel() {
    int value = Arrays.asList(m_options.level0, m_options.level1, m_options.level2, m_options.level3, m_options.level4,
        m_options.level5, m_options.level6, m_options.level7, m_options.level8, m_options.level9).indexOf(true);
    if (value >= 0)
      ZipFormat.setLevel(value);
  }

  /** Map of archive formats adapters.
   * Keep a map of all our archive formats, and register/unregister dynamically on each request.
   * This allows the format objects to be unloaded and re-loaded if the plugin is removed or updated.
   */
  private static final ImmutableMap<String, ArchiveCommand.Format<ArchiveOutputStream>> s_formats;
  static {
    ImmutableMap.Builder<String, ArchiveCommand.Format<ArchiveOutputStream>> builder = ImmutableMap.builder();
    final List<ArchiveCommand.Format<ArchiveOutputStream>> formats = Arrays.asList(
        new TarFormat(),
        new TgzFormat(),
        new Tbz2Format(),
        new TxzFormat(),
   ...
   * If the function succeeds, the unregisterArchiveFormat() should be called afterwards to
   * allow the resources to be released. In case of success, the function also configures Zip
   * compression level.
   *
   * The ArchiveCommand class supports registering the same object multiple times for the same
   * file extension: this way, multiple instances of this class can safely register the same format
   * at the same time.
   *
   ...
   * This function should only be called after the call to registerArchiveFormat() has succeeded.
   * The function also resets the Zip compression level.
   * @param ext The file extension
   */
   ...
   * This method gets the arguments from input stream, in Pkt-line format, then parses
   * then to fill the m_options object.
   */
  ...
  protected void runImpl() throws IOException, Failure {
    ...
    try {
      ...
      try {
        ...
        SideBandOutputStream sidebandOut = new SideBandOutputStream(SideBandOutputStream.CH_DATA,
                                                                    SideBandOutputStream.MAX_BUF, out);
        new ArchiveCommand(repo)
            .setFormat(format)
            .setTree(treeId)
            .setPaths((m_options.path == null ? Collections.emptyList() : m_options.path).toArray(new String[0]))
            .setPrefix(m_options.prefix)
      ...
      SideBandOutputStream sidebandError = new SideBandOutputStream(SideBandOutputStream.CH_ERROR,
                                                                    SideBandOutputStream.MAX_BUF, out);
