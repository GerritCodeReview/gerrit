...
public class UploadArchive extends AbstractGitCommand {
  ...
  private void setupZipCompressionLevel() {
    int value = Arrays.asList(m_options.level0, m_options.level1, m_options.level2,
        m_options.level3, m_options.level4, m_options.level5, m_options.level6,
        m_options.level7, m_options.level8, m_options.level9).indexOf(true);
    if (value >= 0)
  ...
  protected void readArguments() throws IOException, Failure {
    ...
    try {
      ...
      if (m_options.path == null)
        m_options.path = Collections.emptyList();
    ...
    try {
      ...
      try {
        ...
        new ArchiveCommand(repo)
            .setFormat(format)
            .setTree(treeId)
            .setPaths(m_options.path.toArray(new String[0]))
