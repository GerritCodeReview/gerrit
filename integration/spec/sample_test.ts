...
describe('Account creation and login flows', async () => {
  let page: Page;
  before(async() => {
  ...
  it('login flow should work', async () => {
    ...
    expect(await avatar?.evaluate((node) => (node as HTMLElement).innerText)).to.equal('SIMPLY');
  });
});
