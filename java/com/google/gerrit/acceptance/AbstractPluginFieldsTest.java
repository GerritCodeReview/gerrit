...
public class AbstractPluginFieldsTest extends AbstractDaemonTest {
  ...
  protected static class PluginDefinedExceptionModule extends AbstractModule {
    @Override
  ...
  protected void getChangeWithPluginDefinedException(BulkPluginInfoGetterWithId getter)
      throws Exception {
    Change.Id id = createChange().getChange().getId();
    assertThat(getter.call(id).get(id)).isNull();

    try (AutoCloseable ignored = installPlugin("my-plugin", PluginDefinedExceptionModule.class)) {
