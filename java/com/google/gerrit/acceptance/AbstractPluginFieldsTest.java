...
public class AbstractPluginFieldsTest extends AbstractDaemonTest {
  ...
  protected void getChangeWithPluginDefinedNullAttribute(BulkPluginInfoGetterWithId getter)
      throws Exception {
    getChangeWithNullBulkAttribute(getter, PluginDefinedNullAttributeModule.class);
  }
  ...
  protected void getChangeWithNullBulkAttribute(
      BulkPluginInfoGetterWithId getter, Class<? extends Module> moduleClass) throws Exception {
    Change.Id id = createChange().getChange().getId();
  ...
  protected void getSingleChangeWithPluginDefinedBulkAttribute(BulkPluginInfoGetterWithId getter)
      throws Exception {
    getSingleChangeWithPluginDefinedBulkAttribute(getter, PluginDefinedSimpleAttributeModule.class);
  }

  protected void getSingleChangeWithPluginDefinedBulkAttribute(
      BulkPluginInfoGetterWithId getter, Class<? extends Module> moduleClass) throws Exception {
    Change.Id id = createChange().getChange().getId();
  ...
  protected void getEvenChangesWithPluginDefinedBulkAttribute(BulkPluginInfoGetter getter)
      throws Exception {
    Change.Id evenChange = null, oddChange = null;
    ...
    try (AutoCloseable ignored =
      ...
      pluginInfos = getter.call();
      assertThat(pluginInfos.get(oddChange)).isEmpty();
  ...
  protected static Map<Change.Id, List<MyInfo>> pluginInfosFromChangeInfos(
    ...
    changeInfos.forEach(
        ci -> {
          out.put(Change.id(ci._number), pluginInfoFromChangeInfo(ci));
        });
    return out;
  }
