...
public class GerritServerTestRule implements TestRule {
    ...
    getTestInjector()
        .createChildInjector(
            new AbstractModule() {
              ...
              @Named("Admin")
              @Singleton
              SshSession provideAdminSshSession() {
                return adminSshSession;
              }

              @Provides
              @Named("User")
              @Singleton
              SshSession provideUserSshSession() {
                return userSshSession;
              }
            })
        .injectMembers(test);
  }
  ...
  public void initSsh() throws Exception {
    if (testRequiresSsh
      ...
      try (ManualRequestContext ctx = requestScopeOperations.setNestedApiUser(test.user.id())) {
      ...
      try (ManualRequestContext ctx = requestScopeOperations.setNestedApiUser(test.admin.id())) {
  ...
  public ProjectResetter createProjectResetter(
      Function2<AllProjectsName, AllUsersName, ProjectResetter.Config> resetConfigSupplier)
      throws Exception {
