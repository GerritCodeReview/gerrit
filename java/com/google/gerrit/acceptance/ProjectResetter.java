...
public class ProjectResetter implements AutoCloseable {
  ...
  public void close() throws Exception {
    // Restore all saved refs if they were modified.
    Multimap<Project.NameKey, String> keptRefsByProject =
        MultimapBuilder.hashKeys().arrayListValues().build();
    Multimap<Project.NameKey, String> restoredRefsByProject =
        MultimapBuilder.hashKeys().arrayListValues().build();
    for (Map.Entry<Project.NameKey, Collection<RefState>> e :
      ...
      try (Repository repo = repoManager.openRepository(e.getKey())) {
        for (RefState refState : e.getValue()) {
          ...
          assertThat(result).isAnyOf(RefUpdate.Result.FORCED, RefUpdate.Result.NEW);
          restoredRefsByProject.put(e.getKey(), refState.ref());
        }
      }
    }

    // Delete all other matching refs.
    Multimap<Project.NameKey, String> deletedRefsByProject =
        MultimapBuilder.hashKeys().arrayListValues().build();
    for (Map.Entry<Project.NameKey, Collection<String>> e :
        refsPatternByProject.asMap().entrySet()) {
      try (Repository repo = repoManager.openRepository(e.getKey())) {
    ...
    if (accountCreator != null && !deletedAccounts.isEmpty()) {
  ...
  private Set<Project.NameKey> projectsWithConfigChanges(
    ...
    return projects
        .entries()
        .stream()
        .filter(e -> e.getValue().contains(RefNames.REFS_CONFIG))
        .map(Map.Entry::getKey)
        .collect(toSet());
  }

  private Set<Account.Id> accountIds(Collection<String> refs) {
    return refs.stream().map(r -> Account.Id.fromRef(r)).filter(Objects::nonNull).collect(toSet());
  }
}
