...
public class PerPatchsetOperationsImpl implements PerPatchsetOperations {
  ...
  private class RobotCommentAdditionOp implements BatchUpdateOp {
    ...
    private FixSuggestion toFixSuggestion(TestFixSuggestionCreation testFixSuggestionCreation) {
      String fixId = ChangeUtil.messageUuid();
      return new FixSuggestion(
          fixId,
          testFixSuggestionCreation.description().orElse("fix description"),
          testFixSuggestionCreation.fixReplacements().stream()
    ...
    private FixReplacement toFixReplacement(
      ...
      return new FixReplacement(
          testFixReplacementCreation.path().orElse("fix path"),
          testFixReplacementCreation
              .range()
              .map(PerPatchsetOperationsImpl::toEntitiesCommentRange)
              .orElse(new com.google.gerrit.entities.Comment.Range(12, 23, 34, 42)),
          testFixReplacementCreation.replacement().orElse("replacement"));
    }
  }
}
