 ...
 * domain-separated by {@code "gerrit-oauth-token-encryption-v1"}. Input key bytes are zeroed after
 * derivation.
 *
...
public final class OAuthTokenAesGcmEncrypter implements OAuthTokenEncrypter {
  ...
  private static final ThreadLocal<Cipher> CIPHERS =
      ThreadLocal.withInitial(OAuthTokenAesGcmEncrypter::newCipher);
  ...
  public OAuthTokenAesGcmEncrypter(byte[] masterKeyBytes) {
    ...
    try {
      byte[] keyBytes = hkdfSha256(masterKeyBytes, HKDF_INFO, DERIVED_KEY_BYTES);
      try {
  ...
  static byte[] hkdfSha256(byte[] ikm, byte[] info, int length) throws GeneralSecurityException {
  ...
  private static Cipher newCipher() {
    try {
      ...
      throw new IllegalStateException("Failed to initialize OAuth token cipher", e);
    }
  }
