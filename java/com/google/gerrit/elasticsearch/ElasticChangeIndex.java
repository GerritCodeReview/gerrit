        ...
        field.setIfPossible(
            ...
            new FieldDef.IndexedDocument() {
              ...
              public String stringValue() {
                return source.get(field.getName()).getAsString();
              }

              @Override
              public Iterable<String> stringValues() {
                return StreamSupport.stream(
                        source.get(field.getName()).getAsJsonArray().spliterator(), false)
                    .map(f -> f.getAsString())
              ...
              public Integer integerValue() {
                return source.get(field.getName()).getAsInt();
              }
              ...
              public Long longValue() {
                return source.get(field.getName()).getAsLong();
              }
                ...
                return Timestamp.from(
                    Instant.from(
                        DateTimeFormatter.ISO_INSTANT.parse(
                            source.get(field.getName()).getAsString())));
              }

              @Override
              public byte[] rawValue() {
                return decodeBase64(source.get(field.getName()).getAsString());
              }
              ...
              public Iterable<byte[]> rawValues() {
                return StreamSupport.stream(
                        source.get(field.getName()).getAsJsonArray().spliterator(), false)
                    .map(f -> decodeBase64(f.getAsString()))
                    .collect(toImmutableList());
              }
            });
      }
