...
class ElasticChangeIndex extends AbstractElasticIndex<Change.Id, ChangeData>
  ...
  protected ChangeData fromDocument(JsonObject json, Set<String> fields) {
    ...
    ChangeData cd =
        changeDataFactory.create(
            parseProtoFrom(decode(c.getAsString()), ChangeProtoConverter.INSTANCE));
  ...
  private Iterable<byte[]> getByteArray(JsonObject source, String name) {
    ...
    return element != null
        ? Iterables.transform(element.getAsJsonArray(), e -> decode(e.getAsString()))
  ...
  private void decodeSubmitRecords(
    ...
    ChangeField.parseSubmitRecords(
        FluentIterable.from(records)
            .transform(i -> new String(decode(i.getAsString()), UTF_8))
            .toList(),
        opts,
        out);
  }
