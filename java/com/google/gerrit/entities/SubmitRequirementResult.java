...
public abstract class SubmitRequirementResult {
  ...
  static class GsonTypeAdapter extends TypeAdapter<SubmitRequirementResult> {
    ...
    public void write(JsonWriter out, SubmitRequirementResult srResult) throws IOException {
      ...
      String srSerial = SubmitRequirement.typeAdapter().toJson(srResult.submitRequirement());
      out.beginObject();
      out.name(KEY_SUBMIT_REQUIREMENT).jsonValue(srSerial);
      if (srResult.applicabilityExpressionResult().isPresent()) {
        SubmitRequirementExpressionResult app = srResult.applicabilityExpressionResult().get();
        String appSerial = expressionResultAdapter.toJson(app);
        out.name(KEY_APPLICABILITY_EXPRESSION_RESULT).jsonValue(appSerial);
      }
      SubmitRequirementExpressionResult submit = srResult.submittabilityExpressionResult();
      String submitSerial = expressionResultAdapter.toJson(submit);
      out.name(KEY_SUBMITTABILITY_EXPRESSION_RESULT).jsonValue(submitSerial);
      if (srResult.overrideExpressionResult().isPresent()) {
        SubmitRequirementExpressionResult override = srResult.overrideExpressionResult().get();
        String overrideSerial = expressionResultAdapter.toJson(override);
        out.name(KEY_OVERRIDE_EXPRESSION_RESULT).jsonValue(overrideSerial);
      }
