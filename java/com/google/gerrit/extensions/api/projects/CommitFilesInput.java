 ...
 *
 * <p>Used by both the direct-commit and the create-change-for-review variants of the branch content
 * API.
 */
public class CommitFilesInput {
  ...
  /**
   * Optional base commit (SHA-1) for the operation.
   *
   * <p>For a direct commit this is the expected current commit of the target branch: the request is
   * rejected if the branch no longer points at it (optimistic concurrency / lost-update
   * protection).
   *
   * <p>For the create-change variant this is the commit the created change is based on; when unset,
