...
public class ChangeInfoSubject extends Subject {
   ...
   *
   * The 0-value votes and non-existing votes are treated as equal votes. In other word, if expectedVote has value zero, then the actual vote can be either 0 or not present at all and vice-verse.
   */
  public void hasExactlyVotes(Vote... expectedVotes) {
    ...
    Arrays.stream(expectedVotes).filter(v -> v.value() == 0 && !actualVotes.contains(v)).forEach(actualVotes::add);
    assertWithMessage("Votes are different.").that(actualVotes).containsExactlyElementsIn(expectedVotes);
  }
  ...
  private static Set<Vote> getAllNonZeroVotes(Map<String, LabelInfo> labels) {
    ...
    for(Entry<String, LabelInfo> entry: labels.entrySet()) {
      ...
      allApprovals.stream()
          ...
          .map(apprvoalInfo -> vote(entry.getKey(), Account.id(apprvoalInfo._accountId), apprvoalInfo.value)).forEach(votes::add);
    }
    return votes;
  }
