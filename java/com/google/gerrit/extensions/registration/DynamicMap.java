...
public class DynamicMap<T> implements Iterable<DynamicMap.Entry<T>> {
  ...
  public ReloadableRegistrationHandle<T> put(String pluginName, Key<T> key, Provider<T> item) {
    ...
    items.put(np, item);
    return new DynamicMap.ReloadableHandle(np, key, item);
  }
  ...
  private class ReloadableHandle implements ReloadableRegistrationHandle<T> {
    ...
    @Override
    public DynamicMap.ReloadableHandle replace(Key<T> newKey, Provider<T> newItem) {
      if (items.replace(np, item, newItem)) {
        return new DynamicMap.ReloadableHandle(np, newKey, newItem);
      }
      return null;
    }
  }
