...
public class ObjectIds {
  ...
  public static final int ABBREVIATED_STRING_LENGTH = 7;
  ...
  public static String abbreviateName(AnyObjectId id, ObjectReader reader) throws IOException {
    return abbreviateName(id, ABBREVIATED_STRING_LENGTH, reader);
  }
   ...
   *
   * @param id object ID.
   * @param n minimum number of hex chars.
   * @param reader object reader for determining uniqueness.
  ...
  private static void checkValidLength(int n) {
    checkArgument(n > 0);
    checkArgument(n <= Constants.OBJECT_ID_STRING_LENGTH);
  }
