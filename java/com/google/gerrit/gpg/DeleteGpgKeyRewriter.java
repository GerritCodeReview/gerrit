...
public class DeleteGpgKeyRewriter {
  ...
  public RevCommit rewriteHistory(ObjectInserter inserter, ObjectReader reader, RevWalk revWalk, RevCommit currentTip)
    ...
    while ((originalCommit = revWalk.next()) != null) {
      ...
      List<PGPPublicKeyRing> keys = PublicKeyStore.get(reader, notes, keyObjectId, fingerprintToDelete.get());
  ...
  private AnyObjectId rewriteCommit(RevCommit originalCommit, RevCommit parentCommit, ObjectInserter inserter,
      NoteMap notes) throws IOException {
  ...
  private void deleteFromNotes(ObjectReader reader, ObjectInserter inserter, NoteMap notes, List<PGPPublicKeyRing> keys,
      Fingerprint fingerprintToDelete) throws IOException {
    ...
    if (!toWrite.isEmpty()) {
      ...
      List<PGPPublicKeyRing> keyRings = PublicKeyStore.get(reader, notes, keyObjectId, fingerprintToDelete.get());
