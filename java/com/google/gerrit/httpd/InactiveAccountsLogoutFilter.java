 ...
 * and instead we check for each request with a signed-in session if the account is still active, and
 * log out if it is not.
 *
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (session.isSignedIn()) {
      ...
      if (currentUser.isIdentifiedUser()) {
        IdentifiedUser identifiedUser = currentUser.asIdentifiedUser();
        if (!identifiedUser.getAccount().isActive()) {
          ...
          logger.atInfo().log(
              ...
              identifiedUser.getAccountId().get(),
              session);
