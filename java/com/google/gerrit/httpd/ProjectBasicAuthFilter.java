...
class ProjectBasicAuthFilter implements Filter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (isSignedInForRequest(req) || verify(req, rsp)) {
      logger.atInfo().log("Executing request %s", req.getRequestURI());
  ...
  private boolean isSignedInForRequest(HttpServletRequest req) {
    ...
    boolean isSignedInForRequest =
        currentSession.isSignedIn()
            && (isGitRequest || isRead || currentSession.isAccessPathOk(AccessPath.REST_API));
    logger.atFine().log(
        "HTTP:%s %s signedIn=%s (isGitRequest=%s, isRead=%s)",
        req.getMethod(), req.getRequestURI(), isSignedInForRequest, isGitRequest, isRead);
