...
public class RemoteUserUtil {
    ...
    try {
      trustedProxySubnets =
          trustedProxyNetworks.stream()
              .filter(
                  not(
                      isSingleIPMatching())) // Single IP matching should be dealt with
                                             // trustedProxyNetworks instead of a slower subnet
                                             // matching
              .map(SubnetUtils::new)
  ...
  private static Predicate<String> isSingleIPMatching() {
    return network -> network.endsWith("/32");
  }
