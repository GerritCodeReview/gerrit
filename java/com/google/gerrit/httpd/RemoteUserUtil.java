...
public class RemoteUserUtil {
   ...
   * @param trustedProxyNetworks the trusted reverse proxy networks that performed the user authentication
   * @return the extracted username or null.
   */
  @Nullable
  public static String getRemoteUser(HttpServletRequest req, String loginHeader, Set<String> trustedProxyNetworks) {
  ...
  private static boolean isRequestFromTrustedProxyNetworks(HttpServletRequest req, Set<String> trustedProxyNetworks) {
    if (trustedProxyNetworks.isEmpty()) {
      ...
      if (isIpv6Address(remoteAddress)) {
        logger.atWarning().log("IPv6 remote address: %s - trusted proxy enforcement supports only IPv4, HTTP header rejected", remoteAddress);
        return false;
      }
      ...
      if (trustedProxyNetworks.stream().noneMatch(cidr -> matchesIpv4Cidr(remoteAddress, cidr))) {
        logger.atWarning().log("Untrusted remote address: %s - authentication via HTTP header rejected", remoteAddress);
        return false;
      }
