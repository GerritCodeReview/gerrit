...
abstract class DocServlet extends ResourceServlet {
  ...
  @Override
  protected boolean shouldProcessResourceBeforeWrite(
  ...
  @Override
  protected Resource processResourceBeforeWrite(
    ...
    Document document = parser.parseInput(new String(resource.raw, StandardCharsets.UTF_8), "");
    ...
    String body =
        document
            .outputSettings(new Document.OutputSettings().prettyPrint(false).indentAmount(0))
            .outerHtml();
    return new Resource(
        resource.lastModified, resource.contentType, body.getBytes(StandardCharsets.UTF_8));
  }
}
