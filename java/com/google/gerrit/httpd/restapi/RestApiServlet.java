...
public class RestApiServlet extends HttpServlet {
  ...
  protected final void service(HttpServletRequest req, HttpServletResponse res)
    ...
    try (PerThreadCache ignored = PerThreadCache.create()) {
      ...
      } else if (viewData.view instanceof RestDeleteView<?, ?, ?>) {
        @SuppressWarnings("unchecked")
        RestDeleteView<RestResource, RestResource, Object> m =
            (RestDeleteView<RestResource, RestResource, Object>) viewData.view;
  ...
  private static Type inputType(RestDeleteView<RestResource, RestResource, Object> m) {
    // MyCreateView implements RestDeleteView<SomeResource, SomeResource, MyInput>
    TypeLiteral<?> typeLiteral = TypeLiteral.get(m.getClass());

    // RestDeleteView<SomeResource, SomeResource, MyInput>
    // This is smart enough to resolve even when there are intervening subclasses, even if they have
    // reordered type arguments.
    TypeLiteral<?> supertypeLiteral = typeLiteral.getSupertype(RestDeleteView.class);
