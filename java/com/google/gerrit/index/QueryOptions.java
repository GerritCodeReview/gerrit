...
public abstract class QueryOptions {
  public static QueryOptions create(IndexConfig config, int start, int limit, Set<String> fields) {
    return create(config, start, null, limit, limit, fields);
  }

  public static QueryOptions create(
      IndexConfig config, int start, int pageSize, int limit, Set<String> fields) {
    return create(config, start, null, pageSize, limit, fields);
  }
  ...
  public static QueryOptions create(
    ...
    return new AutoValue_QueryOptions(
        config, start, searchAfter, pageSize, limit, ImmutableSet.copyOf(fields));
  }
  ...
  public QueryOptions convertForBackend() {
    ...
    return create(config(), 0, null, pageSize, limit, fields());
  }
  ...
  public QueryOptions withPageSize(int pageSize) {
    return create(config(), start(), searchAfter(), pageSize, limit(), fields());
  }

  public QueryOptions withLimit(int newLimit) {
    return create(config(), start(), searchAfter(), pageSize(), newLimit, fields());
  }

  public QueryOptions withStart(int newStart) {
    return create(config(), newStart, searchAfter(), pageSize(), limit(), fields());
  }
  ...
  public QueryOptions withSearchAfter(Object newSearchAfter) {
    ...
    return create(config(), start(), newSearchAfter, pageSize(), limit(), fields()).withStart(0);
  }

  public QueryOptions filterFields(Function<QueryOptions, Set<String>> filter) {
    return create(config(), start(), searchAfter(), pageSize(), limit(), filter.apply(this));
  }
}
