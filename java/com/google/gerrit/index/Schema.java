...
public class Schema<T> {
  ...
  private Field<T,?>.FieldSpec<T,?> checkSame(Field<T, ?>.FieldSpec<T, ?> f1,
      Field<T, ?>.FieldSpec<T, ?> f2) {
   ...
   * {@link #getField(FieldSpec, FieldSpec...)} or {@link #hasField(FieldSpec)} to looking up fields by
   * name
   *
   ...
   */
  @SafeVarargs
  public final Optional<Field<T, ?>.FieldSpec<T, ?>> getField(Field<T, ?>.FieldSpec<T, ?> first, Field<T, ?>.FieldSpec<T, ?>... rest) {
  ...
  public String toString() {
    return MoreObjects.toStringHelper(this).addValue(indexFields.keySet()).addValue(fieldSpecs.keySet()).toString();
  }
