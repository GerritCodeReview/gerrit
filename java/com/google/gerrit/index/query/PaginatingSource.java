  ...
  public ResultSet<T> read() {
    ...
    return new LazyResultSet<>(
        () -> {
          ...
          if (last != null
              && source instanceof Paginated && skipped) {
            ...
            while (pageResultSize == pageSize && r.size() <= limit) { // get 1 more than the limit
              ...
              ResultSet<T> next =
                  indexConfig.paginationType().equals(PaginationType.OFFSET)
                      ...
                      : ((indexConfig.paginationType().equals(PaginationType.SEARCH_AFTER)) ? p.restart(searchAfter, pageSize): p.restart(nextStart, pageSize, pageSize));
