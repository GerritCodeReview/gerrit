...
public abstract class AbstractFakeIndex<K, V> implements Index<K, V> {
  ...
  AbstractFakeIndex(Schema<V> schema, SitePaths sitePaths, String indexName) {
    ...
    this.indexedDocuments = new LinkedHashMap<>(); // Keep insert order
  }
  ...
  public void replace(V doc) {
    synchronized (indexedDocuments) {
      // Dedicated removal is important to emulate a 'sorted by update recency' behavior
      indexedDocuments.remove(keyFor(doc));
    ...
    public DataSource<ChangeData> getSource(Predicate<ChangeData> p, QueryOptions opts) {
      ...
      return new DataSource<ChangeData>() {
        ...
        public ResultSet<ChangeData> read() {
          // ChangeData is mutable. Insert a defensive copy.
          // TODO(hiesel): To emulate the index behavior we need to read/write the fields to
          // ChangeData. Maybe add a reverse mapping to FieldDef.
          return new ListResultSet<>(
              delegate.read().toList().stream()
                  .map(cd -> changeDataFactory.create(cd.project(), cd.getId()))
                  .collect(toImmutableList()));
        }
