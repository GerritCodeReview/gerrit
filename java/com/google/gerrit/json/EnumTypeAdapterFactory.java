 ...
 * special behavior: input which doesn't match any existing enum value is rejected.
 */
public class EnumTypeAdapterFactory implements TypeAdapterFactory {
  ...
  public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> typeToken) {
    ...
    return (TypeAdapter<T>) new EnumTypeAdapter(defaultEnumAdapter);
  }
  ...
  private static class EnumTypeAdapter<T extends Enum<T>> extends TypeAdapter<T> {
    ...
    public EnumTypeAdapter(TypeAdapter<T> defaultEnumAdapter) {
    ...
    public T read(JsonReader in) throws IOException {
      ...
      if (enumValue == null) {
        throw new JsonSyntaxException("Expected an existing enum value.");
      }
      return enumValue;
    }
