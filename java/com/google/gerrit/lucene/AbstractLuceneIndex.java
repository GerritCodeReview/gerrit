...
public abstract class AbstractLuceneIndex<K, V> implements Index<K, V> {
  ...
  void add(Document doc, Values<V> values) {
    ...
    com.google.gerrit.index.Field<V, ?>.FieldSpec<V, ?> fieldSearchSpec = values.getField();
    Store store = store(values.getField());
    TypeToken<?> fieldType = fieldSearchSpec.getField().fieldType();
    SearchOptions searchOptions = fieldSearchSpec.getSearchOptions();

    if (fieldType.equals(com.google.gerrit.index.Field.INTEGER_TYPE) || fieldType.equals(com.google.gerrit.index.Field.ITERABLE_INTEGER_TYPE)) {
    ...
    } else if (fieldType.equals(com.google.gerrit.index.Field.STRING_TYPE) || fieldType.equals(com.google.gerrit.index.Field.ITERABLE_STRING_TYPE)) {
      ...
      if(fieldSearchSpec.getField().isProtoType() || fieldSearchSpec.getField().isProtoIterableType()) {
        for (Object value : values.getValues()) {
          doc.add(new StoredField(name, Protos.toByteArray((MessageLite) value)));
        }
      }else if (fieldType.equals(com.google.gerrit.index.Field.BYTE_ARRAY_TYPE) || fieldType.equals(com.google.gerrit.index.Field.ITERABLE_BYTE_ARRAY_TYPE)){
        for (Object value : values.getValues()) {
  ...
  public IllegalArgumentException badFieldType(com.google.gerrit.index.Field<V, ?> field, FieldSpec fieldSpec) {
    return new IllegalArgumentException(
        String.format("search spec [%s, %s] is not supported on field [%s, %s]",
  ...
  protected FieldBundle toFieldBundle(Document doc) {
    Map<String, com.google.gerrit.index.Field<V, ?>.FieldSpec<V, ?>> allFields = getSchema().getFieldSpecs();
    ...
    for (IndexableField field : doc.getFields()) {
      ...
      if (type.equals(com.google.gerrit.index.Field.STRING_TYPE) || type.equals(
          com.google.gerrit.index.Field.ITERABLE_STRING_TYPE)) {
        rawFields.put(field.name(), field.stringValue());
      } else if (type.equals(com.google.gerrit.index.Field.INTEGER_TYPE) || type.equals(
          com.google.gerrit.index.Field.ITERABLE_INTEGER_TYPE)) {
      ...
      } else if (fieldSpec.getSearchOptions().equals(SearchOptions.STORE_ONLY) && isStoredByteType(
          fieldSpec.getField())) {
  ...
  private static boolean isStoredByteType(com.google.gerrit.index.Field field) {
    return field.fieldType().equals(com.google.gerrit.index.Field.BYTE_ARRAY_TYPE) || field.fieldType().equals(
        com.google.gerrit.index.Field.ITERABLE_BYTE_ARRAY_TYPE) || field
        .isProtoIterableType() || field.isProtoType();
  }

  private static Field.Store store(com.google.gerrit.index.Field<?, ?>.FieldSpec<?, ?> f) {
    return f.getField().stored() ? Field.Store.YES : Field.Store.NO;
  }
