...
public abstract class AbstractLuceneIndex<K, V> implements Index<K, V> {
  ...
  protected class LuceneQuerySource implements DataSource<V> {
    ...
    private final Function<Document, V> fromDocumentMapper;

    LuceneQuerySource(
        QueryOptions opts, Query query, Sort sort, Function<Document, V> fromDocumentMapper) {
      this.opts = opts;
      this.query = query;
      this.sort = sort;
    ...
    public ResultSet<V> read() throws OrmException {
      return readImpl(fromDocumentMapper);
    }
