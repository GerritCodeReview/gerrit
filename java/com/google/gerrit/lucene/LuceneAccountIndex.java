...
public class LuceneAccountIndex extends AbstractLuceneIndex<Account.Id, AccountState>
  ...
  private static Term idTerm(Account.Id id) {
    return QueryBuilder.stringTerm(ID2.getName(), Integer.toString(id.get()));
  }
  ...
  void add(Document doc, Values<AccountState> values) {
    ...
    if (f == ID2) {
      ...
      doc.add(new NumericDocValuesField(ID2_SORT_FIELD, Long.valueOf(v)));
    } else if (f == FULL_NAME) {
  ...
  private Sort getSort() {
    return new Sort(
        ...
        new SortField(ID2_SORT_FIELD, SortField.Type.LONG, false));
  }

  @Override
  protected AccountState fromDocument(Document doc) {
    Account.Id id = Account.id(Integer.valueOf(doc.getField(ID2.getName()).stringValue()));
