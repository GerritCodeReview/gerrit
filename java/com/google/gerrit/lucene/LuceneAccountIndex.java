...
public class LuceneAccountIndex extends AbstractLuceneIndex<Account.Id, AccountState>
  ...
  private static final String ID_STR_SORT_FIELD = sortFieldName(ID_STR);
  ...
  private static Term idTerm(Account.Id id) {
    return QueryBuilder.stringTerm(ID_STR.getName(), Integer.toString(id.get()));
  }
  ...
  void add(Document doc, Values<AccountState> values) {
    ...
    if (f == ID_STR) {
      ...
      doc.add(new NumericDocValuesField(ID_STR_SORT_FIELD, Long.valueOf(v)));
    } else if (f == FULL_NAME) {
  ...
  private Sort getSort() {
    return new Sort(
        ...
        new SortField(ID_STR_SORT_FIELD, SortField.Type.LONG, false));
  }

  @Override
  protected AccountState fromDocument(Document doc) {
    Account.Id id = Account.id(Integer.valueOf(doc.getField(ID_STR.getName()).stringValue()));
