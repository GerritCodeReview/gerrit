...
public class LuceneChangeIndex implements ChangeIndex {
  ...
  private static final String CHECK_STATE_FIELD = ChangeField.CHECK_STATE.getName();
  ...
  private ChangeData toChangeData(
    ...
    if (fields.contains(CHECK_STATE_FIELD)) {
      decodeCheckState(doc, cd);
    }
    return cd;
  }
  ...
  private void decodeCheckState(ListMultimap<String, IndexableField> doc, ChangeData cd) {
    IndexableField f = Iterables.getFirst(doc.get(CHECK_STATE_FIELD), null);
    if (f != null) {
      cd.setCheckState(CheckState.parse(f.binaryValue().utf8ToString()));
    }
  }
