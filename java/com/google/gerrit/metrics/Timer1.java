...
public abstract class Timer1<F1> implements RegistrationHandle {
  ...
  public static class Context<F1> extends TimerContext {
    ...
    public void record(long elapsed, ImmutableList<String> parentOperations) {
      timer.record(fieldValue, elapsed, NANOSECONDS, parentOperations);
    }
  }
  ...
  public final void record(F1 fieldValue, long value, TimeUnit unit) {
    record(fieldValue, value, unit, LoggingContext.getInstance().getParentOperations());
  }

  private final void record(
      F1 fieldValue, long value, TimeUnit unit, ImmutableList<String> parentOperations) {
          ...
          .addPerformanceLogRecord(
              () ->
                  PerformanceLogRecord.create(
                      name, durationNanos, parentOperations, getMetadata(fieldValue)));
      logger.atFinest().log(
