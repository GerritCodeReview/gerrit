...
public class ChangeExternalIdCaseSensitivityMigrator {

  private static final FluentLogger logger = FluentLogger.forEnclosingClass();

  interface Factory {
    ChangeExternalIdCaseSensitivityMigrator create(
  ...
  @Inject
  public ChangeExternalIdCaseSensitivityMigrator(
  ...
  private void recomputeExternalIdNoteId(ExternalIdNotes extIdNotes, ExternalId extId)
    ...
    if (extId.isScheme(SCHEME_GERRIT) || extId.isScheme(SCHEME_USERNAME)) {
      ...
      ExternalId.Key keyId = keyFactory.create(extId.key().scheme(), extId.key().id(), !isUserNameCaseInsensitive);
      if (!keyId.sha1().getName().equals(updatedKey.sha1().getName())) {
        logger.atInfo().log("Converting note name of external ID: %s", keyId);
        ExternalId updatedExtId =
            externalIdFactory.create(
                updatedKey, extId.accountId(), extId.email(), extId.password(), extId.blobId());
        ExternalId oldExtId =
            externalIdFactory.create(
                keyId, extId.accountId(), extId.email(), extId.password(), extId.blobId());
