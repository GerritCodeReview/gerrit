...
import com.google.gerrit.pgm.init.VersionedAuthorizationTokensOnInit;
...
public class MigratePasswordsToTokens extends SiteProgram {
  ...
  @Inject private VersionedAuthorizationTokensOnInit.Factory tokenFactory;
        ...
        .createChildInjector(
            new FactoryModule() {
              ...
              protected void configure() {
                ...
                factory(VersionedAuthorizationTokens.Factory.class);
  ...
  private void migratePasswordToToken(ExternalId extId, ExternalIdNotes extIdNotes)
      throws DuplicateKeyException, IOException, ConfigInvalidException, TokenConflictException {
    String hashedPassword = extId.password();
    if (hashedPassword == null) {
      return;
    }
    Account.Id accountId = extId.accountId();
    VersionedAuthorizationTokensOnInit authorizationTokens = tokenFactory.create(accountId).load();
    if (authorizationTokens.getToken(LEGACY_ID) != null) {
      logger.atWarning().log(
          "Account %d has already a legacy token, not adding another one", accountId.get());
      return;
    }
    authorizationTokens.addToken(LEGACY_ID, hashedPassword);
    authorizationTokens.save("Migration of HTTP password to token");
