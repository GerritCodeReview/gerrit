...
public class MigratePasswordsToTokens extends SiteProgram {
  ...
  @Inject private VersionedAuthorizationTokens.Accessor tokenAccessor;
  ...
  private void migratePasswordToToken(ExternalId extId, ExternalIdNotes extIdNotes)
    ...
    if (tokenAccessor.getToken(accountId, LEGACY_ID) != null) {
      logger.atWarning().log(
          "Account %d has already a legacy token, not adding another one", accountId.get());
      return;
    }
    tokenAccessor.addToken(extId.accountId(), LEGACY_ID, hashedPassword);

    ExternalId updatedExtId =
        externalIdFactory.create(extId.key(), extId.accountId(), extId.email(), null);
