...
public final class HttpLogLayout extends AbstractStringLayout {
  ...
  public String toSerializable(final LogEvent event) {
    ...
    buf.append(' ');
    opt(buf, HttpLog.P_CONTENT_LENGTH);

    buf.append(' ');
    dq_opt(buf, HttpLog.P_REFERER);

    buf.append(' ');
    dq_opt(buf, HttpLog.P_USER_AGENT);

    buf.append('\n');
    return buf.toString();
  }

  private void opt(StringBuilder buf, String key) {
  ...
  private void dq_opt(StringBuilder buf, String key) {
    String val = ThreadContext.get(key);
    if (val == null) {
      buf.append('-');
    } else {
