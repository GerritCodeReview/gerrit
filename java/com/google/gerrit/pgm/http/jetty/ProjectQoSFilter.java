...
public class ProjectQoSFilter implements Filter {
  ...
  private void setState(ServletRequest req, RequestState state) {
    switch (state) {
      case SUSPENDED:
        req.setAttribute(SUSPENDED_ATT, Boolean.TRUE);
        req.setAttribute(RESUMED_ATT, Boolean.FALSE);
        break;
      case CANCELED:
        req.setAttribute(CANCELED_ATT, Boolean.TRUE);
        break;
      case RESUMED:
        req.setAttribute(RESUMED_ATT, Boolean.TRUE);
        break;
      case INITIAL:
      case UNEXPECTED:
      default:
  ...
  private AsyncContext suspend(ServletRequest request) {
    ...
    setState(request, RequestState.SUSPENDED);
    return asyncContext;
  }
  ...
  private final class TaskThunk implements CancelableRunnable {
    ...
    @Override
    public void cancel() {
      setState(asyncContext.getRequest(), RequestState.CANCELED);
      resume();
    }

    private void resume() {
      ServletRequest candidate = asyncContext.getRequest();
      if (RequestState.SUSPENDED.equals(getState(candidate))) {
        setState(candidate, RequestState.RESUMED);
