...
public class ProjectQoSFilter implements Filter {
  ...
  public enum RequestState {
    ...
    private static void set(ServletRequest req, RequestState state) {
      switch (state) {
        case SUSPENDED:
  ...
  private AsyncContext suspend(ServletRequest request) {
    ...
    RequestState.set(request, RequestState.SUSPENDED);
    return asyncContext;
  }
  ...
  private final class TaskThunk implements CancelableRunnable {
    ...
    @Override
    public void cancel() {
      RequestState.set(asyncContext.getRequest(), RequestState.CANCELED);
      resume();
    }

    private void resume() {
      ServletRequest candidate = asyncContext.getRequest();
      if (RequestState.SUSPENDED.equals(RequestState.get(candidate))) {
        RequestState.set(candidate, RequestState.RESUMED);
