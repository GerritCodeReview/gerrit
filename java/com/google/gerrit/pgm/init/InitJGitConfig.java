...
class InitJGitConfig implements InitStep {
  ...
  public void run() {
    ...
    try {
      jgitConfig.load();
      if (!jgitConfig.getNames("gc").contains("auto")) {
        jgitConfig.setBoolean("gc", null, "auto", false);
        jgitConfig.save();
        ui.error("Auto-configured \"gc.auto = false\" to disable auto-gc after git-receive-pack.");
      } else if (jgitConfig.getBoolean("gc", "auto", true)) {
        ui.error(
            "WARNING: JGit option auto.gc is set to true. This is not recommended in Gerrit.\n"
                ...
                + "gc should be scheduled as a separate process.");
      }
    } catch (IOException e) {
