  ...
  public void run() {
    ...
    try {
      ...
      if (!jgitConfig
        ...
        jgitConfig.setString(ConfigConstants.CONFIG_PROTOCOL_SECTION, null,
            ConfigConstants.CONFIG_KEY_VERSION, TransferConfig.ProtocolVersion.V2.version());
        jgitConfig.save();
        ui.error(String.format(
            "Auto-configured \"%s.%s = %s\" to activate git wire protocol version 2.",
            ConfigConstants.CONFIG_PROTOCOL_SECTION, ConfigConstants.CONFIG_KEY_VERSION,
            TransferConfig.ProtocolVersion.V2.version()));
      } else {
        String version = jgitConfig.getString(ConfigConstants.CONFIG_PROTOCOL_SECTION,
            null,
            ConfigConstants.CONFIG_KEY_VERSION);
        if (!TransferConfig.ProtocolVersion.V2.version().equals(version)) {
          ui.error(
              String.format(
              "WARNING: JGit option \"%s.%s = %s\". It's recommended to activate git\n"
                  + "wire protocol version 2 to improve git fetch performance operation.",
                  ConfigConstants.CONFIG_PROTOCOL_SECTION, ConfigConstants.CONFIG_KEY_VERSION,
                  version));
        }
      }
