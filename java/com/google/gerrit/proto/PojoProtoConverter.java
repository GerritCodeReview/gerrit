...
public class PojoProtoConverter {
  ...
  public static PojoProtoConverter INSTANCE = new PojoProtoConverter();

  // Proto
  Map<Class<?>, PojoToProto> byPojoType;
  Map<Class<? extends Message>, PojoToProto> byProtoType;
  ...
  PojoProtoConverter() {
    byPojoType = new HashMap<>();
    byProtoType = new HashMap<>();
    byProtoEnum = new HashMap<>();
    byJavaEnum = new HashMap<>();
  }

  PojoToProto getByJavaType(Class klass) {
    PojoToProto j2p = byPojoType.get(klass);
    if (j2p != null) return j2p;

    j2p = new PojoToProto(klass);
    byPojoType.put(klass, j2p);
    byProtoType.put(j2p.protoClass, j2p);
    return j2p;
  }

  PojoToProto getByProtoType(Message msg) {
    PojoToProto j2p = byProtoType.get(msg.getClass());
    if (j2p != null) return j2p;
  ...
  private static class PojoToProto {
    ...
    Map<Integer, Field> pojoFieldByTag;

    PojoToProto(Class pojoClass) {
      try {
        this.pojoClass = pojoClass;
        String protoName = "com.google.gerrit.proto.Rest$" + pojoClass.getSimpleName();
      ...
      pojoFieldByTag = new HashMap<>();
      Descriptor messageDesc = defaultInstance.getDescriptorForType();

      for (Field jField : this.pojoClass.getFields()) {
        if (Modifier.isStatic(jField.getModifiers())) continue;

        if (jField.getDeclaringClass() != this.pojoClass) {
          continue;
        }
        ...
        if (pField == null) {
          throw new IllegalStateException(
              String.format(
                  ...
                  protoField.protoTag(), this.pojoClass.getSimpleName()));
        }
        ...
        if (pojoFieldByTag.containsKey(protoField.protoTag())) {
          throw new IllegalStateException(
              String.format("type %s has duplicate protoTag %d", pojoClass, protoField.protoTag()));
        }
        pojoFieldByTag.put(protoField.protoTag(), jField);
      }

      if (CustomPojo.class.isAssignableFrom(pojoClass)) {
        return;
      }

      for (FieldDescriptor pField : protoDesc.getFields()) {
        if (!pojoFieldByTag.containsKey(pField.getNumber())) {
          throw new IllegalStateException(
              String.format(
                  "type %s does not declare @ProtoField for tag %d (field %s)",
                  pojoClass, pField.getNumber(), pField.getName()));
        }
      }
    }
    ...
    protected Object toProtoValue(FieldDescriptor protoDesc, Object src) {
      ...
      if (protoDesc.getType() == FieldDescriptor.Type.MESSAGE) {
        return PojoProtoConverter.toProtoMessage(src);
      }
      if (protoDesc.getType() == FieldDescriptor.Type.ENUM) {
        return PojoProtoConverter.toProtoEnum(src);
      }
    ...
    Message toProtoMessage(Object restObj) {
      checkState(restObj.getClass() == pojoClass);
      if (restObj instanceof CustomPojo) {
        return ((CustomPojo) restObj).toProto();
      }

      Message.Builder b = defaultInstance.newBuilderForType();
      populateProtoMessage(b, restObj);
      return b.build();
    }

    void populateProtoMessage(Message.Builder b, Object restObj) {
      for (Map.Entry<Integer, Field> e : pojoFieldByTag.entrySet()) {
    ...
    Object fromProtoValue(Object obj) {
      ...
      if (obj instanceof EnumValueDescriptor) {
        return PojoProtoConverter.fromProtoEnum((EnumValueDescriptor) obj);
      }
      if (obj instanceof Message) {
        return PojoProtoConverter.fromProtoMessage((Message) obj);
      }
      return obj;
    }
    ...
    Object fromProtoMessage(Message msg) {
      ...
      Object j = newPojo();
      populatePojo(j, j.getClass(), msg);
      return j;
    }

    void populatePojo(Object j, Class jClass, Message msg) {
      if (CustomPojo.class.isAssignableFrom(jClass)) {
  ...
  public static Message toProtoMessage(Object restObj) {
    PojoToProto j2p = INSTANCE.getByJavaType(restObj.getClass());
    return j2p.toProtoMessage(restObj);
  }

  public static void populateProto(Message.Builder builder, Object restObj, Class restClass) {
    PojoToProto j2p = INSTANCE.getByJavaType(restClass);
    j2p.populateProtoMessage(builder, restObj);
  }

  public static Object fromProtoMessage(Message msg) {
    PojoToProto j2p = INSTANCE.getByProtoType(msg);
    return j2p.fromProtoMessage(msg);
  }

  public static void populatePojo(Object pojo, Class pojoClass, Message msg) {
    PojoToProto j2p = INSTANCE.getByProtoType(msg);
    j2p.populatePojo(pojo, pojoClass, msg);
  }
  ...
  public void addAllTypes() {
    ...
    for (ClassInfo classInfo : classPath.getAllClasses()) {
      ...
      try {
        PojoToProto j2p = getByJavaType(instance.getClass());
        j2p.toProtoMessage(instance);
