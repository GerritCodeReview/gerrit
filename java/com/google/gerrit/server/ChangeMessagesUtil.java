...
public class ChangeMessagesUtil {
   ...
   * #replaceTemplates}.
   */
  public static final String NAME_EMAIL_TEMPLATE = "<ACCOUNT_NAME_EMAIL_%s>";

  public static final Pattern NAME_EMAIL_TEMPLATE_PATTERN =
      Pattern.compile(String.format(NAME_EMAIL_TEMPLATE, "([0-9]+)"));
  ...
  public ChangeMessage newMessage(ChangeContext ctx, String templateMessage, @Nullable String tag) {
    return newMessage(
        ctx.getChange().currentPatchSetId(), ctx.getUser(), ctx.getWhen(), templateMessage, tag);
  }
   ...
   */
  public ChangeMessage newMessage(
      PatchSet.Id psId,
      CurrentUser user,
      Timestamp when,
    ...
    m.setDetailedMessage(replaceTemplates(templateMessage));
    m.setTag(tag);
  ...
  public static String getAccountTemplate(Account.Id accounId) {
    return String.format(NAME_EMAIL_TEMPLATE, accounId.toString());
  }
   ...
   * sources.
   */
  private String replaceTemplates(String changeMessage) {
    Matcher matcher = NAME_EMAIL_TEMPLATE_PATTERN.matcher(changeMessage);
  ...
  public List<ChangeMessage> byChange(ChangeNotes notes) {
    ImmutableList<ChangeMessage> changeMessages = notes.load().getChangeMessages();
    changeMessages.stream()
        .filter(changeMessage -> ChangeMessagesUtil.isAutogeneratedByGerrit(changeMessage.getTag()))
        .forEach(
            changeMessage ->
                changeMessage.setDetailedMessage(replaceTemplates(changeMessage.getMessage())));
  ...
  public static ChangeMessageInfo createChangeMessageInfo(
    ...
    cmi.message = message.getDetailedMessage();
