...
public class ChangeMessagesUtil {
  ...
  /**
   * Adds {@code messageTemplate} and {@code tag} to {@code update}.
   *
   ...
   *
   * @param update update that adds {@code messageTemplate}.
   * @param messageTemplate message in template form, that should be applied to the update.
   * @param tag tag that should be added to the update.
  ...
  public String addChangeMessage(ChangeContext ctx, String messageTemplate, @Nullable String tag) {
  ...
  public static String getAccountTemplate(Account.Id accountId) {
    return String.format(ChangeMessage.ACCOUNT_TEMPLATE, accountId.toString());
  }
  ...
  /**
   * Since {@link ChangeMessage} might contain user input, we might still want to query index by
   * them. Since index is a data storage, we don't want to persist User Identifiable Information.
   *
   * <p>Cut templates so we don't match comments on placeholders.
   */
  public static String cutTemplates(ChangeMessage changeMessage) {
    Matcher matcher = ChangeMessage.ACCOUNT_TEMPLATE_PATTERN.matcher(changeMessage.getMessage());
