...
public class CommentsUtil {
  ...
  public static void linkCommentsToChangeMessages(
    ...
    // First sort by timestamp, then by authorId so that we could move on to the next change message in case multiple accounts left comments at the same timestamp.
    ArrayList<ChangeMessage> sortedChangeMessages =
        changeMessages.stream()
            .sorted( comparing(ChangeMessage::getWrittenOn).thenComparingInt(c -> c.getAuthor().get()))
            .collect(toCollection(ArrayList::new));

    ArrayList<CommentInfo> sortedCommentInfos =
        comments.stream().sorted(comparing(CommentInfo::getUpdated).thenComparingInt(c -> c.author._accountId)).collect(toCollection(ArrayList::new));
    ...
    for (CommentInfo comment : sortedCommentInfos) {
      ...
      while (cmItr < sortedChangeMessages.size()) {
        ...
        if (isAfter(comment, cm) || !Objects.equals(cm.getAuthor().get(), comment.author._accountId) || (skipAutoGeneratedMessages && isAutoGenerated(cm))) {
          cmItr += 1;
        } else {
          break;
        }
