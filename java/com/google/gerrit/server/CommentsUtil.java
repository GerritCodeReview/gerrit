...
public class CommentsUtil {
  ...
  public Optional<RobotComment> getPublishedRobotComment(ChangeNotes notes, Comment.Key key) {
    return robotCommentsByChange(notes).stream().filter(c -> key.equals(c.key)).findFirst();
  }

  public Optional<Comment> getPublishedComment(ChangeNotes notes, Comment.Key key) {
    Optional<HumanComment> humanComment = getPublishedHumanComment(notes, key);
    Optional<RobotComment> robotComment = getPublishedRobotComment(notes, key);
    if (humanComment.isPresent()) {
      return Optional.of(humanComment.get());
    }
    if (robotComment.isPresent()) {
      return Optional.of(robotComment.get());
    }
