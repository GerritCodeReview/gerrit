 ...
 * </uL>
 */
public abstract class DeleteZombieComments<Key> implements AutoCloseable {
  @AutoValue
  ...
  public void execute() throws IOException {
    setup();
    List<Key> emptyDrafts = filterByCleanupPercentage(listEmptyDrafts(), "empty");
    ListMultimap<Key, HumanComment> alreadyPublished = listDraftCommentsThatAreAlsoPublished();
    if (dryRun) {
      logInfo(
          String.format(
  ...
  protected abstract List<Key> listAllDrafts() throws IOException;

  protected abstract List<Key> listEmptyDrafts() throws IOException;

  protected abstract void deleteEmptyDraftsByKey(Collection<Key> keys) throws IOException;

  protected abstract void deleteZombieDrafts(ListMultimap<Key, HumanComment> drafts)
      throws IOException;

  protected abstract Change.Id getChangeId(Key key);

  protected abstract Account.Id getAccountId(Key key);

  protected abstract String loggable(Key key);
  ...
  private List<Key> filterByCleanupPercentage(List<Key> drafts, String reason) {
  ...
  @VisibleForTesting
  public ListMultimap<Key, HumanComment> listDraftCommentsThatAreAlsoPublished()
      throws IOException {
    List<Key> draftKeys = filterByCleanupPercentage(listAllDrafts(), "all-drafts");
    changeProjectMap.putAll(mapChangesWithDraftsToProjects(draftKeys));

    ListMultimap<Key, HumanComment> zombieDrafts = ArrayListMultimap.create();
    Set<ChangeUserIDsPair> visitedSet = new HashSet<>();
    for (Key key : draftKeys) {
      try {
        ...
        List<HumanComment> zombieDraftsForChangeAndAuthor =
            drafts.stream()
   ...
   */
  private Map<Change.Id, Project.NameKey> mapChangesWithDraftsToProjects(List<Key> drafts) {
