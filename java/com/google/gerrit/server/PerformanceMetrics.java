...
public class PerformanceMetrics implements PerformanceLogger {
  ...
  public void log(String operation, long durationMs) {
    if (OPERATION_LATENCY_METRIC_NAME.equals(operation)) {
      ...
      // In practice his should not happen since PerformanceLoggers are only called on close() of
    ...
    operationsLatency.record(
        operation, "", TraceContext.getTraceId().orElse(""), durationMs, TimeUnit.MILLISECONDS);
  }
  ...
  private String formatChangeIdentifier(Metadata metadata) {
    if (!metadata.projectName().isPresent() && !metadata.changeId().isPresent()) {
      return "";
    }
