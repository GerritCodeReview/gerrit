...
public class AccountManager {
  ...
  public void link(Account.Id to, Collection<ExternalId> extIds)
    ...
    Map<ExternalId.Key, ExternalId> existingExtIds = new HashMap<>();
    for (ExternalId id : externalIds.get(keys)) {
      existingExtIds.put(id.key(), id);
    }
    for (ExternalId extId : extIds) {
      if (existingExtIds.get(extId.key()) == null) {
