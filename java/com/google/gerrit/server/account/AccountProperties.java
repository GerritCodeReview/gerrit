...
public class AccountProperties {
   ...
   *
   * <p>If the account is not a robot ({@code isRobot} is false) then {@code account.isRobot} is
   * unset since {@code false} is the default if this field is missing.
   *
   * @param cfg the config
   * @param isRobot whether the account is a robot
   */
  private static void setIsRobot(Config cfg, boolean isRobot) {
    if (isRobot) {
