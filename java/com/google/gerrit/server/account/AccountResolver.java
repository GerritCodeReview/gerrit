...
public class AccountResolver {
  ...
  public Set<Account.Id> findAll(String nameOrEmail)
    ...
    if (m.matches()) {
      Optional<Account.Id> id = Account.Id.parse(m.group(1));
      if (id.isPresent()) {
        return Streams.stream(accounts.get(id.get())).map(a -> id.get()).collect(toImmutableSet());
      }
    }

    if (nameOrEmail.matches("^[1-9][0-9]*$")) {
      Optional<Account.Id> id = Account.Id.parse(nameOrEmail);
      if (id.isPresent()) {
        return Streams.stream(accounts.get(id.get())).map(a -> id.get()).collect(toImmutableSet());
      }
    }
