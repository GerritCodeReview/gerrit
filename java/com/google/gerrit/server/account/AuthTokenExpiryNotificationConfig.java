...
public class AuthTokenExpiryNotificationConfig {
  ...
  static final String AUTH_SECTION = "auth";
  static final String TOKEN_EXPIRY_SUBSECTION = "token-expiry";
  static final String TOKEN_EXPIRY_NOTIFY = "notify";
  static final String TOKEN_EXPIRY_NOTIFY_DAYS_BEFORE = "notifyDaysBefore";
  ...
  public AuthTokenExpiryNotificationConfig(@GerritServerConfig Config config) {
    this.enabled =
        config.getBoolean(AUTH_SECTION, TOKEN_EXPIRY_SUBSECTION, TOKEN_EXPIRY_NOTIFY, true);
    String[] configNotificationDays =
        config.getStringList(
            AUTH_SECTION, TOKEN_EXPIRY_SUBSECTION, TOKEN_EXPIRY_NOTIFY_DAYS_BEFORE);
   ...
   */
  public boolean shouldBeNotified(Instant checkTime, Instant expirationDate) {
    if (!enabled) {
      return false;
    }

    // Check if token already expired
    if (expirationDate.isBefore(checkTime)) {
      return false;
    }

    long daysUntilExpiration =
        ChronoUnit.DAYS.between(checkTime.truncatedTo(ChronoUnit.DAYS), expirationDate);
