...
public class GroupIncludeCacheImpl implements GroupIncludeCache {
  ...
  static class AllExternalInMemoryLoader
    ...
    private final LoadingCache<String, ImmutableList<AccountGroup.UUID>> persisted;
    private final ExternalGroupsKeyReader keyReader;
    private final Groups groups;

    @Inject
    AllExternalInMemoryLoader(
        @Named(PERSISTED_EXTERNAL_NAME)
            LoadingCache<String, ImmutableList<AccountGroup.UUID>> persisted,
        ExternalGroupsKeyReader keyReader,
        Groups groups) {
    ...
    public ImmutableList<AccountGroup.UUID> load(String key) throws Exception {
      String cacheKey = keyReader.currentKey();
      return persisted.get(
          cacheKey,
          () -> {
            try (TraceTimer timer = TraceContext.newTimer("Loading all external groups")) {
              return groups.getExternalGroups(keyReader.getRefs()).collect(toImmutableList());
            }
          });
    }
  }

  static class EmptyLoader extends CacheLoader<String, ImmutableList<AccountGroup.UUID>> {

    @Override
    public ImmutableList<UUID> load(String key) throws Exception {
      throw new IllegalStateException("The loader should not be invoked");
    }
  }
