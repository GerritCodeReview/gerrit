...
public class PasswordMigrator implements Runnable {
  ...
  public void run() {
    ...
    for (ExternalId extId : todo) {
      createToken(extId, expirationDate);
    ...
    mpm.end();
    try {
      deletePasswordsInExternalId(todo);
    } catch (IOException | ConfigInvalidException e) {
    ...
    sw.stop();
    logger.atInfo().log(
        "Finished migrating HTTP passwords. %d/%d finished successfully in %d min. %d tasks"
            + " failed.",
        doneTask.getCount(), todo.size(), sw.elapsed(TimeUnit.MINUTES), failedTask.getCount());
  }

  private void createToken(ExternalId extId, Optional<Instant> expirationDate) {
  ...
  private void deletePasswordsInExternalId() throws IOException, ConfigInvalidException {
    ...
    try (Repository repo = repoManager.openRepository(allUsers)) {
      ...
      extIdNotes.replace(extIdsWithPassword, updatedExtIds);
