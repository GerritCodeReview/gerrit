  ...
  public boolean isServiceUser(Account.Id user) {
    ...
    while (!toTraverse.isEmpty()) {
      ...
      if (hasExternalSubgroup) {
        ...
        logger.atFine().log(
            "Group %s has subgroups: account %d is "
                + (res ? "" : "NOT")
                + " a service user because it "
                + (res ? " a member of one of them" : "NOT a member of any of them"),
            currentGroup,
            user.get());
        return res;
      }
    ...
    logger.atFine().log(
        "Account %d is not service user because does not belong to any Service Users group or subgroup",
        user.get());
    return false;
  }
}
