...
public class TokenCacheImpl implements TokenCache {
  ...
  public static Module module() {
    return new CacheModule() {
      ...
      protected void configure() {
        cache(CACHE_NAME, Account.Id.class, new TypeLiteral<Iterable<TokenCacheEntry>>() {})
            .loader(Loader.class);
  ...
  @Inject
  TokenCacheImpl(@Named(CACHE_NAME) LoadingCache<Account.Id, Iterable<TokenCacheEntry>> cache) {
    this.cache = cache;
  }

  @Override
  public Iterable<TokenCacheEntry> get(Account.Id accountId) {
    try {
  ...
  static class Loader extends CacheLoader<Account.Id, Iterable<TokenCacheEntry>> {
    ...
    @Override
    public Iterable<TokenCacheEntry> load(Account.Id accountId) throws Exception {
