...
public class ExternalIdFactory {
  ...
  public ExternalId createUsername(
    ...
    return createWithPassword(externalIdKeyFactory.create(ExternalId.SCHEME_USERNAME, id), accountId, null, plainPassword);
  }
  ...
  public ExternalId create(
    ...
    if (key.scheme() != null
        && (key.scheme().equals(ExternalId.SCHEME_USERNAME) || key.scheme().equals(ExternalId.SCHEME_GERRIT))) {
      return ExternalId.create(
          key, accountId, this.isUserNameCaseInsensitive, Strings.emptyToNull(email), Strings.emptyToNull(hashedPassword), blobId);
    }
    return ExternalId.create(
        key, accountId, false, Strings.emptyToNull(email), Strings.emptyToNull(hashedPassword), blobId);
  }
  ...
  public ExternalId parse(String noteId, Config externalIdConfig, ObjectId blobId)
    ...
    String email = externalIdConfig.getString(ExternalId.EXTERNAL_ID_SECTION, externalIdKeyStr, ExternalId.EMAIL_KEY);
    String password =
        externalIdConfig.getString(ExternalId.EXTERNAL_ID_SECTION, externalIdKeyStr, ExternalId.PASSWORD_KEY);
  ...
  private static int readAccountId(String noteId, Config externalIdConfig, String externalIdKeyStr)
      throws ConfigInvalidException {
    String accountIdStr =
        externalIdConfig.getString(ExternalId.EXTERNAL_ID_SECTION, externalIdKeyStr, ExternalId.ACCOUNT_ID_KEY);
    ...
    try {
      int accountId =
          externalIdConfig.getInt(ExternalId.EXTERNAL_ID_SECTION, externalIdKeyStr, ExternalId.ACCOUNT_ID_KEY, -1);
      if (accountId < 0) {
        throw invalidConfig(
            noteId,
            String.format(
                "Value %s for '%s.%s.%s' is invalid, expected account ID",
                accountIdStr, ExternalId.EXTERNAL_ID_SECTION, externalIdKeyStr, ExternalId.ACCOUNT_ID_KEY));
      }
      return accountId;
