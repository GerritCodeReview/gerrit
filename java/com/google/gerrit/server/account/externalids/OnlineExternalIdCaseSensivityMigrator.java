...
public class OnlineExternalIdCaseSensivityMigrator implements LifecycleListener {
  ...
  public OnlineExternalIdCaseSensivityMigrator(
    ...
    this.executor = workQueue.createQueue(1, "MigrateExternalIdCase", true);
    this.dryRun =
        globalConfig.getBoolean("auth", "userNameCaseInsensitiveOnlineMigrationDryRun", false);
  ...
  public void migrate() {
    if (!isUserNameCaseInsensitive) {
      logger.atSevere().log(
          "External IDs online migration requires auth.userNameCaseInsensitive to be set to true. Skipping migration!");
      return;
    }
    executor.execute(
        () -> {
