...
public class StringEncryptor {
  ...
  private void createAndSaveSecretKey() {
    if (Strings.isNullOrEmpty(getKeyFromConfig())) {
      try {
        SecretKeySpec key = createSecretKey(randomString().toCharArray(), newSalt());
        secureStore.set(SECTION, null, KEY, base64Encode(key.getEncoded()));
      } catch (GeneralSecurityException e) {
  ...
  private SecretKeySpec createSecretKey(char[] password, byte[] salt)
    ...
    return new SecretKeySpec(key.getEncoded(), config.algorithm());
  }
