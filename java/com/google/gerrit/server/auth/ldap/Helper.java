...
class Helper {
  ...
  private DirContext kerberosOpen(Properties env)
    ...
    try {
      ...
      LdapRealm.logger.atWarning().withCause(e.getException()).log("Internal error");
      return null;
    } finally {
      ctx.logout();
    }
    ...
    if (groupDNs.add(groupDN)
      ...
      if (cachedParentsDNs == null) {
        ...
        try {
          ...
          LdapRealm.logger.atWarning().withCause(e).log("Could not find group %s", groupDN);
        }
    ...
    LdapType discoverLdapType(DirContext ctx) {
      try {
        ...
        LdapRealm.logger
            ...
            .withCause(e)
            .log(
                "Cannot discover type of LDAP server at %s, assuming the server is RFC 2307 compliant.",
                server);
