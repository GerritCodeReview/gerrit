...
class DefaultMemoryCacheFactory implements MemoryCacheFactory {
  ...
  private <K, V> Caffeine<K, V> create(CacheDef<K, V> def) {
    ...
    builder = builder.removalListener(newRemovalListener(def));
  ...
  @SuppressWarnings("unchecked")
  private <V, K> CaffeineToGuavaRemovalListener<K, V> newRemovalListener(CacheDef<K, V> def) {
    return new CaffeineToGuavaRemovalListener<K, V>(
