...
class DefaultMemoryCacheFactory implements MemoryCacheFactory {
  ...
  private <K, V> Caffeine<K, V> create(CacheDef<K, V> def) {
    ...
    if (has(def.configKey(), "maxAge")) {
      ...
      builder.expireAfterWrite(
          logCache(def, "expireAfterWrite", expireAfterWrite.toNanos()), NANOSECONDS);
    }
    ...
    if (has(def.configKey(), "expireFromMemoryAfterAccess")) {
      ...
      builder.expireAfterAccess(
          logCache(def, "expireAfterAccess", expireAfterAccess.toNanos()), NANOSECONDS);
    }
    ...
    if (has(def.configKey(), "refreshAfterWrite")) {
      ...
      builder.refreshAfterWrite(
          logCache(def, "refreshAfterWrite", refreshAfterWrite.toNanos()), NANOSECONDS);
    }

    return builder;
  }

  private <T> T logCache(CacheDef<?, ?> def, String paramName, T paramValue) {
    logger.atInfo().log("[cache %s] %s=%d", def.configKey(), paramName, paramValue);
    return paramValue;
  }
