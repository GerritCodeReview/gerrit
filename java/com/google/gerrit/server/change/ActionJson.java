...
public class ActionJson {
  ...
  private ImmutableMap<String, ActionInfo> toActionMap(
    ...
    // frontend treats an absent entry as the default-allow state, so allowed
    // and unaware-backend cases share the no-payload path.
    try {
      boolean permitted =
          permissionBackend
              .user(rsrc.getUser())
    ...
    for (UiAction.Description d : descs) {
      ...
      if (AI_REVIEW_ACTION_ID.equals(d.getId()) && !d.isEnabled()) {
