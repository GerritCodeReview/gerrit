 ...
 * thus ensure that the change ETag changes when the plugin data was changed. This way it is ensured
 * that callers do not see outdated ChangeInfos.
 *
...
public interface ChangeETagComputation {
   ...
   * cheap. Take good care to not do any expensive computations when implementing this.
   *
   * @param changeId ID of the change for which the ETag should be computed.
   * @return the ETag
   */
  String getETag(Change.Id changeId);
}
