...
public class ChangeJson {
  ...
  private final boolean cacheChangeQueryResultsById;
  ...
  ChangeJson(
    ...
    this.cacheChangeQueryResultsById = cfg.getBoolean("index", "cacheChangeQueryResultsById", true);
  ...
  private List<ChangeInfo> toChangeInfos(
    ...
    try (Timer0.Context ignored = metrics.toChangeInfosLatency.start()) {
      ...
      for (int i = 0; i < changes.size(); i++) {
        ...
        // (1) Caching
        // (2) Reusing
        boolean isCacheable = cacheQueryResultsById && (i != changes.size() - 1);
