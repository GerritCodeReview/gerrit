...
public class ChangeJson {
  ...
  private Collection<ChangeData> ensureLoaded(Collection<ChangeData> all) {
    ...
    try (TraceTimer timer =
      ...
      // load the data for each change in an own thread
      List<ListenableFuture<ChangeData>> indexFutures = new ArrayList<>();
      all.forEach(cd -> indexFutures.add(executor.submit(() -> loadChangeData.apply(cd))));
      return Futures.allAsList(indexFutures).get();
