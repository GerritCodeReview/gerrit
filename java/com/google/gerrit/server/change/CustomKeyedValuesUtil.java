...
public class CustomKeyedValuesUtil {
  ...
  public static String cleanupKey(String hashtag) {
    hashtag = CharMatcher.whitespace().trimLeadingFrom(hashtag);
    hashtag = CharMatcher.whitespace().trimTrailingFrom(hashtag);
    return hashtag;
  }

  static ImmutableMap<String, String> extractCustomKeyedValues(ImmutableMap<String, String> input) throws InvalidCustomKeyedValueException {
    if (input == null) {
    ...
    for (Map.Entry<String, String> customKeyedValue : input.entrySet()) {
      ...
      String key = cleanupKey(customKeyedValue.getKey());
      if (!key.isEmpty()) {
        continue;
      }
  ...
  static ImmutableSet<String> extractCustomKeys(ImmutableSet<String> input) throws InvalidCustomKeyedValueException {
    if (input == null) {
    ...
    for (String customKey : input) {
      ...
      String key = cleanupKey(customKey);
      if (!key.isEmpty()) {
        continue;
      }
      builder.add(key);
