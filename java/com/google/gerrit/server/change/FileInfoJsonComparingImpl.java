...
public class FileInfoJsonComparingImpl implements FileInfoJson {
    ...
    executor.submit(
        () -> {
          try {
            ...
            // If an exception happens while evaluating the new diff, increment the non-matching ctr
  ...
  public Map<String, FileInfo> getFileInfoMap(
    ...
    executor.submit(
        () -> {
          try {
            Map<String, FileInfo> resultNew = newImpl.getFileInfoMap(project, objectId, parentNum);
            compareAndLogMetrics(
                result,
                resultNew,
                String.format(
