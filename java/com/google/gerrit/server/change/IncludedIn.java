...
public class IncludedIn {
          ...
          ext -> {
            ListMultimap<String, String> extIncludedIns =
                ext.getIncludedIn(
                    project.get(),
                    rev.name(),
                    sortedShortNames(d.tags().stream().map(Ref::getName)),
                    sortedShortNames(d.branches().stream().map(Ref::getName)));
   ...
   */
  private Map<String, Ref> filterReadableRefs(Project.NameKey project, ImmutableSet<Ref> inputRefs)
      throws IOException, PermissionBackendException {
    PermissionBackend.ForProject perm = permissionBackend.currentUser().project(project);
    Repository repo = repoManager.openRepository(project);
    return perm.filter(
        inputRefs.stream().collect(Collectors.toList()), repo, RefFilterOptions.defaults());
  }

  private ImmutableSortedSet<String> sortedShortNames(Collection<String> refs) {
    return sortedShortNames(refs.stream());
  }

  private ImmutableSortedSet<String> sortedShortNames(Stream<String> refs) {
    return refs.map(Repository::shortenRefName).collect(toImmutableSortedSet(naturalOrder()));
  }
}
