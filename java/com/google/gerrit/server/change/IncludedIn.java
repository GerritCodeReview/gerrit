...
public class IncludedIn {
  ...
  public IncludedInInfo apply(Project.NameKey project, String revisionId)
    ...
    try (Repository r = repoManager.openRepository(project);
      ...
      return new IncludedInInfo(
          ...
          filterReadableTags(project, d.tags()),
  ...
  private ImmutableList<String> filterReadableBranches(
    ...
    return filterReadableRefs(project, inputRefs, /* isTags= */ false);
  }

  private ImmutableList<String> filterReadableTags(
      Project.NameKey project, Collection<String> inputRefs) {
    return filterReadableRefs(project, inputRefs, /* isTags= */ true);
  }

  /**
   * Filter readable branches or tags according to ref read permissions.
   *
   * @param project specific Gerrit project.
   * @param inputRefs a list of branches (in short name) as strings
   * @param isTags if true, {@code inputRefs} are assumed to to be tag names, otherwise they are
   *     assumed as branch names.
   */
  private ImmutableList<String> filterReadableRefs(
      Project.NameKey project, Collection<String> inputRefs, boolean isTags) {
    ...
    for (String ref : inputRefs) {
      try {
        ...
        if (isTags) {
          perm.ref(RefNames.REFS_TAGS + ref).check(RefPermission.READ);
        } else {
