...
public class MergeabilityCacheImpl implements MergeabilityCache {
    ...
    return new CacheModule() {
      ...
      protected void configure() {
        persist(CACHE_NAME, EntryKey.class, Boolean.class)
            ...
            .version(1)
            .keySerializer(new EntryKey.Serializer())
  ...
  public static class EntryKey {
    ...
    @VisibleForTesting
    static class Serializer implements CacheSerializer<EntryKey> {
      private final EnumCacheSerializer<SubmitType> submitTypeSerializer =
          new EnumCacheSerializer<>(SubmitType.class);

      @Override
      public byte[] serialize(EntryKey object) throws IOException {
        ...
        b.setSubmitType(submitTypeSerializer.serializeToString(object.getSubmitType()));
        b.setMergeStrategy(object.getMergeStrategy());
        return ProtoCacheSerializers.toByteArray(b.build());
      }

      @Override
      public EntryKey deserialize(byte[] in) throws IOException {
        MergeabilityKeyProto proto = MergeabilityKeyProto.parseFrom(in);
        byte[] buf = new byte[Constants.OBJECT_ID_LENGTH];
        ...
        return new EntryKey(
            commit,
            into,
            submitTypeSerializer.deserialize(proto.getSubmitTypeBytes().toByteArray()),
