...
public class PluginDefinedAttributesFactories {
  ...
  public static ImmutableListMultimap<Change.Id, PluginDefinedInfo> createAll(
    ...
    ImmutableListMultimap.Builder<Change.Id, PluginDefinedInfo> pdisByChangeBuilder =
        ImmutableListMultimap.builder();
    infoFactories.forEach(
        e -> tryCreate(cds, beanProvider, e.getPluginName(), e.get(), pdisByChangeBuilder));
    ImmutableListMultimap<Change.Id, PluginDefinedInfo> result = pdisByChangeBuilder.build();
    return !result.isEmpty() ? result : null;
  }
      ...
      infoFactory
          ...
          .forEach(
              (id, pdi) -> {
                if (pdi != null) {
                  pdi.name = plugin;
                  pdisByChangeBuilder.put(id, pdi);
                }
              });
    } catch (RuntimeException ex) {
