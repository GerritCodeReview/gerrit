...
public class ReaddOwnerUtil {
  ...
  public void readdOwnerForInactiveOpenChanges(BatchUpdate.Factory updateFactory) {
    if (cfg.getReaddAfter() <= 0 || cfg.getMailSender().isEmpty()) {
      return;
    }

    String mailSender = cfg.getMailSender().get();
    IdentifiedUser identifiedUser;
    try {
      identifiedUser = resolver.resolve(mailSender).asUniqueUser();
    } catch (ConfigInvalidException | IOException | UnresolvableAccountException e) {
      logger.atSevere().withCause(e).log(
          "Could not resolve account with username %s from attentionset config", mailSender);
      return;
    }
    ...
    try {
      ...
      for (Project.NameKey project : ownerAdds.keySet()) {
        try (BatchUpdate bu = updateFactory.create(project, identifiedUser, TimeUtil.now())) {
          for (ChangeData changeData : ownerAdds.get(project)) {
            Account.Id ownerId = changeData.change().getOwner();
            if (!inAttentionSet(changeData, ownerId) && !isServiceUser(ownerId)) {
              logger.atFine().log(
                  "Batch owner for add to AS of change %s in project %s",
                  changeData.getId(), project.get());
              bu.addOp(changeData.getId(), opFactory.create(ownerId, cfg.getReaddMessage(), true));
              ownersAdded++;
            }
          }
          bu.execute();
  ...
  private boolean isServiceUser(Account.Id accountId) {
    return groupCache
