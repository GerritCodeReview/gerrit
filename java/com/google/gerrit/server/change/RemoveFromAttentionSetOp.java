...
public class RemoveFromAttentionSetOp implements BatchUpdateOp {
  ...
  public interface Factory {
    RemoveFromAttentionSetOp create(IdentifiedUser attentionUser, String reason);
  }
    ...
    update.setAttentionSetUpdates(
        ImmutableList.of(
            AttentionSetUpdate.createForWrite(
                attentionUser.getAccountId(), Operation.REMOVE, reason)));
    addMessage(ctx, update);
    return true;
  }

  private void addMessage(ChangeContext ctx, ChangeUpdate update) {
    String message = "Removed from attention set: " + attentionUser.getNameEmail();
