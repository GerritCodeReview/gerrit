...
public class SetCustomKeyedValuesOp implements BatchUpdateOp {
  ...
  public boolean updateChange(ChangeContext ctx)
    ...
    try {
      ...
      ImmutableMap.Builder<String, String> updated = ImmutableMap.builder();
      ImmutableMap.Builder<String, String> added = ImmutableMap.builder();
      ...
      validationListeners.runEach(
          l -> l.validateCustomKeyedValues(update.getChange(), added.build(), removed.build()),
      ...
      for (Map.Entry<String, String> existing : existingCustomKeyedValues.entrySet()) {
        if (tryingToRemove.contains(existing.getKey())) {
          continue;
        }
        if (tryingToAdd.containsKey(existing.getKey())
            && !tryingToAdd.get(existing.getKey()).equals(existing.getValue())) {
          // Unfortunately ImmutableMap.builder doesn't allow multiple entries for
          // a key and picking the last. So we have to explictly avoid copying over
          // existing keys if they don't match new keys we want to add.
          // This condition is the inverse of the condition in the loop below.
          // An alternative solution would be to use a HashMap as temporary storage.
          // A cleaner solution would be to do updated.buildKeepingLast() below,
          // but that is not available in the current version.
          continue;
        }
        updated.put(existing.getKey(), existing.getValue());
      }
      for (Map.Entry<String, String> add : tryingToAdd.entrySet()) {
        if (existingCustomKeyedValues.containsKey(add.getKey())
            && existingCustomKeyedValues.get(add.getKey()).equals(add.getValue())) {
          continue;
        }
        update.addCustomKeyedValue(add.getKey(), add.getValue());
        added.put(add.getKey(), add.getValue());
        updated.put(add.getKey(), add.getValue());
      }
      toAdd = added.build();
      toRemove = removed.build();
      updatedCustomKeyedValues = updated.build();
      return true;
