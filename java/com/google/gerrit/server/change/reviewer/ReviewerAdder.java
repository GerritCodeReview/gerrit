...
public class ReviewerAdder {
  ...
  public abstract static class Result {
    static Builder builder(Input input) {
      return new AutoValue_ReviewerAdder_Result.Builder().input(input);
    }
  ...
  public ReviewerAddition prepare(ChangeNotes notes, ImmutableList<Input> inputs)
      throws OrmException, NoSuchProjectException, ConfigInvalidException, IOException,
  ...
  private void prepareImpl(ChangeNotes notes, Result.Builder resultBuilder)
      throws NoSuchProjectException, OrmException, ConfigInvalidException, IOException,
          PermissionBackendException {
    ...
    try {
      if (Strings.isNullOrEmpty(resultBuilder.input().input())) {
        resultBuilder.appendError("Empty input");
        return;
      }
  ...
  private boolean resolveAsAccount(ChangeNotes notes, Result.Builder resultBuilder)
      throws OrmException, ConfigInvalidException, IOException, PermissionBackendException {
    ...
    resultBuilder
        ...
        .set(
            input.state(),
            input.options().ignoreIfChangeOwner() && id.equals(notes.getChange().getOwner())
                ? ImmutableList.of()
                : ImmutableList.of(id));
    return true;
  }
  ...
  private boolean resolveAsEmail(ChangeNotes notes, Result.Builder resultBuilder)
    ...
    if (addr == null || !validator.isValid(addr.getEmail())) {
      resultBuilder.appendError(
          MessageFormat.format(ChangeMessages.get().reviewerInvalid, input.input()));
      return false;
    }
