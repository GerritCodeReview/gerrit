  ...
  abstract static class RepresentedPreferences {
    ...
    static RepresentedPreferences fromTypedString(String str) throws ConfigInvalidException {
      try {
        ...
        CachedPreferencesType prefType;
        try {
          List<String> parts = Splitter.on(TYPE_DELIMITER).limit(2).splitToList(str);
          prefType = CachedPreferencesType.valueOf(parts.get(0));
          representingString = parts.get(1);
        } catch (IndexOutOfBoundsException | IllegalArgumentException e) {
          // If type cannot be extracted, assumes the string was serialized before types were added.
          prefType = CachedPreferencesType.GIT_CONFIG;
          representingString = str;
        }
        switch (prefType) {
          case GIT_CONFIG:
            Config cfg = new Config();
            cfg.fromText(representingString);
          ...
          case USER_PREFERENCES_PROTO:
            ...
            TextFormat.merge(representingString, builder);
