...
public abstract class ScheduleConfig {
  ...
  private static long computeJitter(
    ...
    try {
      String jitter = rc.getString(section, subsection, keyJitter);
      if (jitter == null) {
        return 0L; // No jitter if key not present
      }
      return ConfigUtil.getTimeUnit(
          rc, section, subsection, keyJitter, INVALID_CONFIG, TimeUnit.MILLISECONDS);
    } catch (IllegalArgumentException e) {
      logger.atSevere().log("%s", e.getMessage());
      return INVALID_CONFIG;
    }
