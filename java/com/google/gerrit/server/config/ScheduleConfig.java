 ...
 *       executions are {@code Wed 10:30}, {@code Fri 10:30}. A random delay between 0 and 30
 *       minutes will be added to each scheduled start time.
 *   <li>
 *       <pre>
 * foo.startTime = 06:00
 * foo.interval = 1 day
...
public abstract class ScheduleConfig {
  ...
  private boolean isInvalidOrMissing(long interval, long initialDelay, long jitter) {
    ...
    if (jitter != INVALID_CONFIG && jitter < 0) {
      logger.atSevere().log("Invalid jitter value \"%d\" for \"%s\": must be >= 0", jitter, key);
  ...
  private static long computeJitter(
    ...
    try {
      return ConfigUtil.getTimeUnit(rc, section, subsection, keyJitter, 0L, TimeUnit.MILLISECONDS);
    } catch (IllegalArgumentException e) {
      logger.atSevere().log("%s", e.getMessage());
      return INVALID_CONFIG;
    }
  ...
  private static long computeInitialDelay(long interval, String start) {
    return computeInitialDelay(interval, start, computeNow(), 0L); // No jitter by default
  }
  ...
  private static long computeInitialDelay(
    ...
    try {
      ...
      if (jitter > 0) {
        ...
        delay += random.nextLong(jitter + 1); // 0 to jitter inclusive
      }
      return delay;
