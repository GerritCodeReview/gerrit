...
public class ServerConfigCacheImpl implements ServerConfigCache {
	...
	public ServerConfigData get() throws ExecutionException {
		ServerConfigData cachedConfig = serverConfigCache.get(
			SINGLETON_KEY,
			() -> {
				try (AutoCloseable unused = new ManualRequestContext(new AnonymousUser(), requestContextProvider.get())) {
		...
		serverInfo.submitRequirementDashboardColumns = cachedConfig.serverInfo().submitRequirementDashboardColumns;
