...
public class StreamEventsApiListener
  ...
  public static class StreamEventsApiListenerModule extends AbstractModule {
    ...
    protected void configure() {
      ...
      if (config.getBoolean("events", "stream-events", "enableRefUpdatedEvents", true)) {
        DynamicSet.bind(binder(), GitReferenceUpdatedListener.class)
            .to(StreamEventsApiListener.class);
      }
      if (config.getBoolean("events", "stream-events", "enableBatchRefUpdatedEvents", true)) {
  ...
  public void onGitBatchRefUpdate(GitBatchRefUpdateListener.Event ev) {
    ...
    Supplier<AccountAttribute> submitterSupplier =
        Suppliers.memoize(
            () -> {
              AccountInfo accountInfo = ev.getUpdater();
              AccountAttribute submitter = new AccountAttribute();
              submitter.accountId = accountInfo._accountId;
              submitter.name = accountInfo.name;
              submitter.email = accountInfo.email;
              submitter.username = accountInfo.username;
              return submitter;
            });
