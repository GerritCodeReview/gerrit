...
public class AttentionSetObserver {
  ...
  public void fire(
    ...
    HashSet<AccountInfo> added = new HashSet<>();
    HashSet<AccountInfo> removed = new HashSet<>();
    switch (update.operation()) {
      case ADD:
        added.add(util.accountInfo(target));
        break;
      case REMOVE:
        removed.add(util.accountInfo(target));
        break;
    }
  ...
  private static class Event extends AbstractChangeEvent implements AttentionSetListener.Event {
    private final Set<AccountInfo> added;
    private final Set<AccountInfo> removed;

    Event(
        ChangeInfo change,
        AccountInfo editor,
        Set<AccountInfo> added,
        Set<AccountInfo> removed,
        Instant when) {
    ...
    @Override
    public Set<AccountInfo> usersAdded() {
      return added;
    }

    @Override
    public Set<AccountInfo> usersRemoved() {
      return removed;
    }
  }
}
