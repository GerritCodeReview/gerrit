 ...
 *   List of replacements, where each replacement has:
 *       srcRange - part the original text to be
 *                  replaced, inserted or deleted (see {@link Comment.Range} for details)
 *       replacement - text to be set instead of srcRange
 *   Replacements ranges must not intersect.
 *
 * Output:
 *   Final text (aka finalText)
 *   List of Edit, where each Edit is an instance of {ReplaceEdit}
 ...
 *
 * Note: algorithm implemented in this way to avoid string.replace operations. It has complexity
 * O( R*lg R + max(len(originalText), len(finalText)) ), where R = len(replacements)
 *
 * Main steps:
 ...
 *
 *      - If previously create ReplaceEdit ends on the same or previous line as srcRange.startLine,
 ...
 * 2) srcPos and dstPos tracks current position as 3 number:
 * - line number
 * - column number
