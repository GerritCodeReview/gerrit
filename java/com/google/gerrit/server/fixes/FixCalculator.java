...
public class FixCalculator {
  ...
  public static FixResult calculateFix(Text oldContent, List<FixReplacement> fixReplacements) throws ResourceConflictException {
    ...
    if(!sortedReplacements.isEmpty() && sortedReplacements.get(0).range.startLine == 0) {
      throw new ResourceConflictException(
          String.format("Cannot calculate fix replacement for range %s",
    ...
    for (FixReplacement fixReplacement : sortedReplacements) {
      try {
        ...
        throw new ResourceConflictException(
            String.format("Cannot calculate fix replacement for range %s",
                toString(fixReplacement.range)), e);
      }
    }
    collector.finish();
  ...
  private static class ContentCollector {
    ...
    private void processSrcContent(int toLine, int toColumn, boolean append) throws IndexOutOfBoundsException{
