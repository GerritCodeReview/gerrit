...
/**
 * Produces final version of an input content with all fixes applied together with
 * list of edits.
* */
public class FixCalculator {
   ...
   * @throws ResourceConflictException if the fixReplacements contains invalid data
   *  (for example, if an item points to an invalid range of if some ranges are intersected).
   */
  public static FixResult calculateFix(Text originalContent, List<FixReplacement> fixReplacements)
    ...
    ContentBuilder builder = new ContentBuilder(originalContent);
