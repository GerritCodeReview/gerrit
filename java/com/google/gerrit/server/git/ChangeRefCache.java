...
public class ChangeRefCache {
   ...
   *
   * <p>Uses a CopyOnWriteArraySet internally to keep track of projects that are already
   * bootstrapped. This is efficient because we read from the set on every call to this method to
   * check if bootstrapping is required. Writes occur only if we bootstrapped, so once per project.
   *
