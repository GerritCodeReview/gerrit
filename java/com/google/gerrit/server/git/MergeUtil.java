...
public class MergeUtil {
    ...
    if (m.merge(false, mergeTip, originalCommit)) {
      ...
      tree =
          mergeWithConflicts(
              ...
              dc,
              "BASE",
              getBaseCommit(rw, m).orElse(null),
  ...
  public static Optional<RevCommit> getBaseCommit(RevWalk rw, Merger merger) throws IOException {
    ObjectId baseCommitId = merger.getBaseCommitId();
    if (baseCommitId == null) {
      //  baseCommitId is null if a base has been set manually on the merger or if the merged
      // commits do not have a common predecessor
      // (e.g. if 2 initial commits or 2 commits with unrelated histories are merged)
      return Optional.empty();
    }

    try {
      return Optional.of(rw.parseCommit(baseCommitId));
