...
public class MultiProgressMonitor implements RequestStateProvider {
  ...
  private Optional<Long> timeoutInMinutes = Optional.empty();
  ...
  public <T> T waitFor(Future<T> workerFuture, long timeoutTime, TimeUnit timeoutUnit)
    ...
    if (timeoutTime > 0) {
      ...
      timeoutInMinutes = Optional.of(MINUTES.convert(timeoutTime, timeoutUnit));
    } else {
      deadline = 0;
    }
  ...
  public void checkIfCancelled(OnCancelled onCancelled) {
    if (clientDisconnected) {
      ...
      onCancelled.onCancel(
          ...
          timeoutInMinutes.map(t -> String.format("timeout=%dm", t)).orElse(null));
    }
  }
}
