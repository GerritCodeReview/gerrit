...
public class PermissionAwareRepositoryManager {
  public static Repository wrap(Repository delegate, PermissionBackend.ForProject forProject) {
    return wrap(delegate, forProject, RefFilterOptions.defaults());
  }
  ...
  public static Repository wrap(
    ...
    if (delegate instanceof PermissionAwareRepository) {
      new PermissionAwareRepository(
          ((PermissionAwareRepository) delegate).unwrap(), forProject, opts);
    }
    return new PermissionAwareRepository(delegate, forProject, opts);
  }
