...
public class WorkQueue {
  ...
  private class Executor extends ScheduledThreadPoolExecutor {
    private static class Schedule {
      ...
      Schedule(long interval, TimeUnit unit) {
        this.interval = interval;
        this.unit = unit;
      }
    }
    private final ConcurrentHashMap<Integer, Task<?>> all;
    private final ConcurrentHashMap<Integer, Schedule> delayMap;
    ...
    public ScheduledFuture<?> scheduleAtFixedRate(
      ...
      delayMap.put(command.hashCode(), new Schedule(period, unit));
    ...
    public ScheduledFuture<?> scheduleWithFixedDelay(
      ...
      delayMap.put(command.hashCode(), new Schedule(delay, unit));
  ...
  public static class Task<V> implements RunnableScheduledFuture<V> {
    ...
    public void run() {
      if (runningState.compareAndSet(null, State.STARTING)) {
        ...
        retry = true;
        Executor.Schedule delaySchedule = executor.delayMap.get(runnable.hashCode());
        // Set the re-schedule delay time to one-third of the original period converted to nanoseconds.
        long delay = delaySchedule.unit.toNanos(delaySchedule.interval / 3);
        Future<?> unusedFuture = executor.schedule(this, delay, TimeUnit.NANOSECONDS);
      }
    }
