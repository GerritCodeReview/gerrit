...
public class AsyncReceiveCommits implements PreReceiveHook {
  ...
  private static class Metrics {
    private Histogram1 changes;
    private Timer1 latencyPerChange;
    private Counter0 timeouts;
    ...
    Metrics(MetricMaker metricMaker) {
      changes =
          metricMaker.newHistogram(
              ...
              Field.ofEnum(
                  ResultChangeIds.Key.class, "type of update (replace, create, autoclose)"));
      latencyPerChange =
    ...
    if (totalChanges > 0) {
      metrics.latencyPerChange.record(
          resultChangeIds.get(ResultChangeIds.Key.AUTOCLOSED).isEmpty()
              ? "CREATE_REPLACE"
              : ResultChangeIds.Key.AUTOCLOSED,
