...
public abstract class ReceiveCommitsResult {
  ...
  public abstract static class Builder {
    private EnumMap<ChangeStatus, Set<Change.Id>> changes;

    Builder() {
      changes = Maps.newEnumMap(ChangeStatus.class);
      Arrays.stream(ChangeStatus.values()).forEach(k -> changes.put(k, new HashSet<>()));
    }
    ...
    public ReceiveCommitsResult build() {
      ImmutableMap.Builder<ChangeStatus, ImmutableSet<Change.Id>> c = ImmutableMap.builder();
