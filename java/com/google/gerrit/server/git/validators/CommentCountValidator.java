...
public class CommentCountValidator implements CommentValidator {
  ...
  public ImmutableList<CommentValidationFailure> validateComments(
    ...
    Map<Account.Id, Integer> existingPerUser = new HashMap<>();
    notes
        .getHumanComments()
        .values()
        .forEach(
            c -> {
              if (c.author != null) {
                existingPerUser.merge(c.author.getId(), 1, Integer::sum);
              }
            });
        ...
        .forEach(
            cm -> {
              if (cm.getAuthor() != null) {
                existingPerUser.merge(cm.getAuthor(), 1, Integer::sum);
              }
            });

    int existing = existingPerUser.getOrDefault(userId, 0);
