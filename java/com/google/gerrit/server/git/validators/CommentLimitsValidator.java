...
public class CommentLimitsValidator implements CommentValidator {
  private static final int MAX_COMMENT_LENGTH = 16 << 10;
  ...
  public ImmutableList<CommentValidationFailure> validateComments(
    ...
    return comments.stream()
        .filter(c -> c.getText().length() > MAX_COMMENT_LENGTH)
        .map(
            c ->
                c.failValidation(
                    String.format(
                        "Comment too large (%d > %d)", c.getText().length(), MAX_COMMENT_LENGTH)))
