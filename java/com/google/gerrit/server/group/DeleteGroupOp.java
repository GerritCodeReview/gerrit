...
public class DeleteGroupOp implements RepoOnlyOp {
  ...
  @Override
  public void updateRepo(RepoContext ctx) throws IOException, ConfigInvalidException {
    ...
    try (Repository repo =
        repoManager.openRepository(Project.nameKey(AllUsersNameProvider.DEFAULT))) {
      ObjectInserter inserter = repo.newObjectInserter();
      BatchRefUpdate bru = repo.getRefDatabase().newBatchUpdate();
      List<GroupReference> groups = new ArrayList<>(GroupNameNotes.loadAllGroups(repo));
      GroupReference groupToDelete =
          GroupNameNotes.loadGroup(repo, AccountGroup.nameKey(group.getName())).get();
      groups.remove(groupToDelete);
      GroupNameNotes.updateAllGroups(repo, inserter, bru, groups, serverUser);
      inserter.flush();
