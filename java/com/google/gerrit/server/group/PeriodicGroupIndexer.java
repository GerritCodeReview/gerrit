  ...
  private static class Lifecycle implements LifecycleListener {
    ...
    public void start() {
      ...
      checkState(
          ...
          "No index.scheduledIndexer schedule configuration");
      checkState(
          delay >= 0 && interval >= 0, "Invalid index.scheduledIndexer schedule configuration");
      if (interval > 0) {
      ...
      if (groupRefs != null) {
        for (String groupRefPart : groupRefs.keySet()) {
          if (!newGroupRefs.containsKey(groupRefPart)) {
            // deleted group
            groupIndexer.index(AccountGroup.UUID.fromRefPart(groupRefPart));
            reindexCounter++;
          }
        }
      }
