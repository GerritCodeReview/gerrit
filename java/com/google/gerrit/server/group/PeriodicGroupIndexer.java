 ...
 * groups. The ref states of the group refs are cached so that on each run new, updated and deleted
 * groups can be detected. Only those groups are then reindexed. On startup of a slave all groups
 * are reindexed once. This means callers of slaves may observe outdated group information until the
 * next indexing happens. The interval on which group indexing is done is configurable by setting
 * {@code index.scheduledIndexer.interval} in {@code gerrit.config}. By default group indexing is
...
public class PeriodicGroupIndexer implements Runnable {
  ...
  private static class Lifecycle implements LifecycleListener {
    ...
    @Override
    public void start() {
      ScheduleConfig scheduleConfig = new ScheduleConfig(cfg, "index", "scheduledIndexer");
      scheduleConfig.setDefaults("00:00", TimeUnit.MINUTES.toMillis(5));
      long interval = scheduleConfig.getInterval();
      long delay = scheduleConfig.getInitialDelay();
      checkState(
          interval != MISSING_CONFIG && delay != MISSING_CONFIG,
          "No slave.reindexer schedule configuration");
      checkState(delay >= 0 && interval >= 0, "Invalid slave.reindexer schedule configuration");
  ...
  private ImmutableMap<String, ObjectId> groupRefs;
  ...
  public synchronized void run() {
    try (Repository allUsers = repoManager.openRepository(allUsersName)) {
      ImmutableMap<String, ObjectId> newGroupRefs =
          allUsers
              .getRefDatabase()
              .getRefs(RefNames.REFS_GROUPS)
              .entrySet()
              .stream()
              .collect(toImmutableMap(Map.Entry::getKey, e -> e.getValue().getObjectId()));
      ...
      for (Map.Entry<String, ObjectId> e : newGroupRefs.entrySet()) {
        if (groupRefs == null
          ...
          groupIndexer.get().index(AccountGroup.UUID.fromRefPart(e.getKey()));
          reindexCounter++;
        }
      }
      if (groupRefs != null) {
        for (String groupRefPart : groupRefs.keySet()) {
          if (!newGroupRefs.containsKey(groupRefPart)) {
            // deleted group
            groupIndexer.get().index(AccountGroup.UUID.fromRefPart(groupRefPart));
            reindexCounter++;
          }
        }
      }
      groupRefs = newGroupRefs;
      log.info("Run group indexer, {} groups reindexed", reindexCounter);
    } catch (IOException e) {
      log.error("Failed to reindex groups", e);
    }
  }
}
