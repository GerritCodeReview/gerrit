...
public class GroupsConsistencyChecker {
  ...
  public List<ConsistencyProblemInfo> check() throws OrmException, IOException {
    ...
    try (Repository repo = repoManager.openRepository(allUsersName)) {
      List<ConsistencyProblemInfo> problems = globalChecker.check(repo);
      if (problems.isEmpty()) {
        ImmutableMap<AccountGroup.UUID, InternalGroup> byUUID = globalChecker.getUuidToGroupMap();
  ...
  private List<ConsistencyProblemInfo> checkGroup(
      InternalGroup g, ImmutableMap<AccountGroup.UUID, InternalGroup> byUUID) throws IOException {
  ...
  private List<ConsistencyProblemInfo> checkCycle(
      InternalGroup root, ImmutableMap<AccountGroup.UUID, InternalGroup> byUUID) {
    List<ConsistencyProblemInfo> problems = new ArrayList<>();
