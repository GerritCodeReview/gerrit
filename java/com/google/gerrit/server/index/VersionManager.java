...
public abstract class VersionManager implements LifecycleListener {
  ...
  public void startIndexForTest(String name) {
    OnlineReindexer<?, ?, ?> reindexer = reindexers.get(name);
  ...
  @VisibleForTesting
  public void setLowestIndexForTest(String name) {
    setLowestIndexVersion(defs.get(name));
  }

  private <K, V, I extends Index<K, V>> void setLowestIndexVersion(IndexDefinition<K, V, I> def) {
