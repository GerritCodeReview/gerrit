...
public class StalenessChecker {
  ...
  public boolean isStale(Account.Id id, boolean deleted) throws IOException {
    ...
    Optional<FieldBundle> result =
        i.getRaw(id, IndexedAccountQuery.createOptions(indexConfig, 0, 1, FIELDS));
    if (!result.isPresent() != deleted) {
      // No document in the index although account was not deleted or
      // document in the index although account was deleted.
      return true;
    } else if (deleted) {
      // Account was deleted and there is no document in the index.
      return false;
    }
