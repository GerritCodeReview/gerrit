...
import com.google.gerrit.server.git.MultiProgressMonitor.EndMonitor;
...
public class AllChangesIndexer extends SiteIndexer<Change.Id, ChangeData, ChangeIndex> {
  ...
  private VolatileTask projTask;
  private VolatileTask doneTask;
  private Task failedTask;
  private Task collectingProjects;
  ...
  private class SliceScheduler {
    ...
    final List<ListenableFuture<?>> sliceIndexerFutures;
    final AtomicBoolean ok;
    final AtomicInteger changeCount = new AtomicInteger(0);
    final AtomicInteger projectsFailed = new AtomicInteger(0);
    List<ListenableFuture<?>> sliceCreationFutures = new ArrayList<>();

    public SliceScheduler(
        ChangeIndex index, List<ListenableFuture<?>> sliceIndexerFutures, AtomicBoolean ok) {
      this.index = index;
      this.sliceIndexerFutures = sliceIndexerFutures;
      this.ok = ok;
    }

    private void schedule() throws ProjectsCollectionFailure {
      ImmutableSortedSet<Project.NameKey> projects = projectCache.all();
      int projectCount = projects.size();
      collectingProjects = mpm.beginSubTask("Slicing projects", projectCount);
      ...
      public Void call() throws IOException {
        try (Repository repo = repoManager.openRepository(name)) {
          ...
          if (size > 0) {
            ...
            doneTask.updateTotalWork(size);
            projTask.updateTotalWork(slices);
