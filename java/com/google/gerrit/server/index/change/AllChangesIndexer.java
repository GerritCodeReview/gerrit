...
public class AllChangesIndexer extends SiteIndexer<Change.Id, ChangeData, ChangeIndex> {
  ...
  private class SliceCreator {
      ...
      for (Project.NameKey project : projectCache.all()) {
        try {
          ...
          verboseWriter.println(
              project + " slice creation took " + sw2.elapsed(TimeUnit.SECONDS) + "s");
        } catch (IOException e) {
                ...
                c -> {
                  ProjectSlice slice =
                      sliceByProject.computeIfAbsent(
                          project,
                          s -> ProjectSlice.create(project, sliceNumber.getAndIncrement()));
                  slice.changes().add(c);
    ...
    private void popAndQueueIf(ProjectSlice projectSlice, Predicate<ProjectSlice> pred) {
      if (pred.test(projectSlice)) {
