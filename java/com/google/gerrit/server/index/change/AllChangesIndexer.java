...
public class AllChangesIndexer extends SiteIndexer<Change.Id, ChangeData, ChangeIndex> {
  ...
  public abstract static class ProjectSlice {
    ...
    private static ProjectSlice create(
        Project.NameKey name, Set<ChangeNotesResult> changes, int number) {
      return new AutoValue_AllChangesIndexer_ProjectSlice(name, changes, number);
    }
  }
  ...
  private SiteIndexer.Result indexAll(ChangeIndex index, List<ProjectSlice> projectSlices) {
    ...
    for (ProjectSlice slice : projectSlices) {
      Project.NameKey name = slice.name();
      ListenableFuture<?> future =
          executor.submit(
              reindexProjectSlice(
                  ...
                  doneTask,
                  failedTask));
      String description = "project " + name + " (slice-" + slice.number() + ")";
  ...
  public Callable<Void> reindexProject(
    ...
    Set<ChangeNotesResult> changes = new HashSet<>();
    try (Repository repo = repoManager.openRepository(project)) {
      changes = notesFactory.scan(repo, project, null).collect(Collectors.toSet());
    } catch (IOException e) {
      logger.atSevere().log(e.getMessage());
    }
    if (!changes.isEmpty()) {
  ...
  private class SliceCreator {
                  ...
                  c -> {
                    ProjectSlice projectSlice =
                        sliceByProject.computeIfAbsent(
                            project, s -> createSlice(project, sliceNumber.incrementAndGet()));
                    projectSlice.changes().add(c);
    ...
    private ProjectSlice createSlice(Project.NameKey project, int sliceNumber) {
      return ProjectSlice.create(project, new HashSet<ChangeNotesResult>(), sliceNumber);
    }

    private void popAndQueueIfFull(ProjectSlice projectSlice) {
      popAndQueueIf(projectSlice, ps -> ps.changes().size() == PROJECT_SLICE_MAX_REFS);
    }

    private void popAndQueue(Project.NameKey project) {
      ProjectSlice projectSlice = sliceByProject.get(project);
      if (projectSlice != null) {
        popAndQueueIf(projectSlice, ps -> ps.changes().size() > 0);
      }
    }

    private void popAndQueueIf(ProjectSlice projectSlice, Predicate<ProjectSlice> p) {
      if (p.test(projectSlice)) {
        filledSlices.add(sliceByProject.remove(projectSlice.name()));
      }
    }
  }
