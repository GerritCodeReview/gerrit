...
public class AllChangesIndexer extends SiteIndexer<Change.Id, ChangeData, ChangeIndex> {
  ...
  public abstract static class ProjectSlice {
    private final int projectSliceMaxChanges = 1000;
    ...
    public boolean isFull() {
      return changes().size() == projectSliceMaxChanges;
    }
  ...
  private class SliceCreator {
    ...
    private List<ProjectSlice> create(ProjectCache projectCache) throws ProjectsCollectionFailure {
      ...
      for (Project.NameKey project : projectCache.all()) {
        try {
          new ProjectSliceCreator(project).create();
        } catch (IOException e) {
    ...
    private class ProjectSliceCreator {
      Project.NameKey project;
      int sliceNumber = 0;
      ProjectSlice slice;

      public ProjectSliceCreator(NameKey project) {
        this.project = project;
      }
      ...
      private void create() throws IOException {
        ...
        if (sliceNumber > 0) {
          verboseWriter.println(
              "Submitting " + project + " for indexing in " + sliceNumber + " slices");
        }
      }

      private void addToFilled(ProjectSlice slice) {
