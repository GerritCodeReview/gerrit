...
public class AllChangesIndexer extends SiteIndexer<Change.Id, ChangeData, ChangeIndex> {
  ...
  /**
   * Wrapper class over an object allowing it to be used in an anonymous inner class or a lambda
   * statement.
   */
  ...
  public Callable<Void> reindexProject(
    ...
    try (Repository repo = repoManager.openRepository(project)) {
      Set<ChangeNotesResult> changes =
          notesFactory.scan(repo, project, null).parallel().collect(Collectors.toSet());
  ...
  private class SliceCreator {
    ...
    private void create(Project.NameKey project) throws IOException {
      ...
      Reference<ProjectSlice> slice = new Reference<>();
      sliceNumber.val = 0;
      try (Repository repo = repoManager.openRepository(project)) {
        notesFactory
            ...
            .forEach(
                c -> {
                  if (slice.val == null) {
                    slice.val = ProjectSlice.create(project, sliceNumber.val++);
                  }
                  slice.val.changes().add(c);
                  slice.val = popAndQueueIfFull(slice.val);
                });
        if (slice.val != null) {
          popAndQueueIf(slice.val, s -> s.changes().size() > 0);
        }
      }
      if (sliceNumber.val > 0) {
        verboseWriter.println(
            "Submitting " + project + " for indexing in " + sliceNumber.val + " slices");
      }
    }

    private ProjectSlice popAndQueueIfFull(ProjectSlice slice) {
      return popAndQueueIf(slice, s -> s.changes().size() == PROJECT_SLICE_MAX_REFS);
    }

    private ProjectSlice popAndQueueIf(ProjectSlice slice, Predicate<ProjectSlice> pred) {
      if (pred.test(slice)) {
