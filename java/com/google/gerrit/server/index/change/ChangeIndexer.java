...
public class ChangeIndexer {
  ...
  private final Optional<ChangeIndex> latestWriteIndex;
  ...
  ChangeIndexer(
    ...
    this.latestWriteIndex = getLatestWriteIndex();
  }
  ...
  public boolean isChangeAlreadyIndexed(Change.Id id) {
    if (latestWriteIndex.isEmpty()) {
      return false;
    }
    return latestWriteIndex.get().get(id, IndexedChangeQuery.oneResult()).isPresent();
  }

  public Optional<ChangeIndex> getLatestWriteIndex() {
  ...
  /**
   * Asynchronously check if a change is stale, and reindex if it is. The index.autoReindexIfStale
   * option will be ignored to enforce the reindexing of stale indexes.
   *
   * <p>Always run on the batch executor, even if this indexer instance is configured to use a
   * different executor.
   *
  ...
  public void autoReindexLatestWriteIndexIfStale(ChangeData cd) {
    ...
    if (latestWriteIndex.isEmpty()) {
      task = new ReindexIfStaleTask(cd.project(), cd.getId());
    }
    task = new ReindexIfStaleInGivenIndexTask(cd.project(), cd.getId(), latestWriteIndex.get());
    if (queuedReindexIfStaleTasks.add(task)) {
  ...
  private class ReindexIfStaleInGivenIndexTask extends ReindexIfStaleTask {
    private final ChangeIndex indexToCheck;
    ...
    protected StalenessCheckResult check() {
      return stalenessChecker.check(id, indexToCheck);
    }
  }
