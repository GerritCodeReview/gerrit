...
public final class PendingIndexUpdate {
  ...
  void recover(Path file) throws IOException {
    Intent intent = GSON.fromJson(Files.readString(file), Intent.class);
    if (intent == null || intent.project() == null || intent.operation() == null) {
      logger.atWarning().log("Malformed pending index intent, skipping %s", intent);
