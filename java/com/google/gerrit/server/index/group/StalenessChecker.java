...
public class StalenessChecker {
  ...
  public boolean isStale(AccountGroup.UUID id) throws IOException, OrmException {
    ...
    Optional<InternalGroup> result =
        i.get(id, IndexedGroupQuery.createOptions(indexConfig, 0, 1, FIELDS));
    if (!result.isPresent()) {
      return true; // Not in index, but caller wants it to be.
    }
    InternalGroup group = result.get();
    try (Repository repo = repoManager.openRepository(allUsers)) {
      Ref ref = repo.findRef(RefNames.refsGroups(id));
      ObjectId head = ref == null ? ObjectId.zeroId() : ref.getObjectId();
      return !head.equals(group.getRefState());
    }
  }
}
