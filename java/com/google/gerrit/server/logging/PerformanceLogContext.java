...
public class PerformanceLogContext implements AutoCloseable {
  ...
  public PerformanceLogContext(DynamicSet<PerformanceLogger> performanceLoggers) {
    ...
    this.oldPerformanceLogRecords = LoggingContext.getInstance().getPerformanceLogEntries();
  ...
  public void close() {
    if (LoggingContext.getInstance().isPerformanceLogging()) {
      runEach(performanceLoggers, LoggingContext.getInstance().getPerformanceLogEntries());
    }

    // Restore old state.
    LoggingContext.getInstance().performanceLogging(oldPerformanceLogging);
    LoggingContext.getInstance().setPerformanceLogEntries(oldPerformanceLogRecords);
  }
