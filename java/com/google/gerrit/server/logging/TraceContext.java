...
public class TraceContext implements AutoCloseable {
  ...
  public static TraceContext newTrace(
      boolean trace, @Nullable String traceId, TraceIdConsumer traceIdConsumder) {
    if (!trace) {
      return DISABLED;
    }

    return TraceContext.open().startTracing(trace, traceId, traceIdConsumder);
  }
   ...
   */
  public TraceContext startTracing(
    ...
    return addTag(RequestId.Type.TRACE_ID, newTraceId).forceLogging();
  }
