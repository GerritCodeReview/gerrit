...
public class AutoReplyMailFilter implements MailFilter {
  ...
  public boolean shouldProcessMessage(MailMessage message) {
    for (String header : message.additionalHeaders()) {
      if (header.startsWith(Metadata.PRECEDENCE.withDelimiter())) {
        String prec =
            header.substring(Metadata.PRECEDENCE.withDelimiter().length()).trim();
      ...
      } else if (header.startsWith(Metadata.AUTO_SUBMITTED.withDelimiter())) {
        String autoSubmitted =
            header.substring(Metadata.AUTO_SUBMITTED.withDelimiter().length()).trim();
