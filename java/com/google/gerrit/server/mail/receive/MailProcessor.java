...
public class MailProcessor {
  ...
  private void persistComments(
    ...
    try (ManualRequestContext ctx = oneOffRequestContext.openAs(sender)) {
      ...
      int maxComments = gerritConfig.getInt("change", null, "maxComments", 5_000);  // ö extract
      if (existingComments + parsedComments.size() > maxComments) {  //ö
        sendRejectionEmail(message, InboundEmailRejectionSender.Error.COMMENT_REJECTED);  //ö
        return;
      }
