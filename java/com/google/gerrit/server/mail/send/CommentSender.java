...
public class CommentSender extends ReplyToChangeSender {
  ...
  protected void setupSoyContext() {
    ...
    if (isChangeNoLongerSubmittable()) {
      soyContext.put(
          "newlyUnsatisfiedSubmitRequirements",
          getSubmitRequirementsThatHaveNewStatus(SubmitRequirementResult.Status.UNSATISFIED));
      soyContext.put(
  ...
  private ImmutableList<String> getSubmitRequirementsThatHaveNewStatus(
      SubmitRequirementResult.Status status) {
    Map<SubmitRequirement, SubmitRequirementResult> preUpdateSubmitRequirementResults =
        preUpdateSubmitRequirementResultsSupplier.get();
    return postUpdateSubmitRequirementResults.entrySet().stream()
        .filter(
            e ->
                status.equals(e.getValue().status())
                    && (!preUpdateSubmitRequirementResults.containsKey(e.getKey())
                        || !status.equals(
                            preUpdateSubmitRequirementResults.get(e.getKey()).status())))
