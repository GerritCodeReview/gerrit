...
public class ChangeNoteUtil {
  ...
  private static class AttentionStatusInNoteDb {
    ...
    AttentionStatusInNoteDb(String personIndent, boolean removal, String reason) {
  ...
  public static AttentionStatus attentionStatusFromJson(String attentionString, Gson gson) {
    ...
    // ö What if this failed? And below: optional absent?
    Optional<Account.Id> account = NoteDbUtil.parseIdent(personIdent);
    return AttentionStatus.create(account.get(), inNoteDb.removal, inNoteDb.reason);
  }

  // ö move Gson to this class?
  public String attentionStatusToJson(
      AttentionStatus attentionStatus, PersonIdent serverIdent, Gson gson) {
    // ö date hack
    PersonIdent personIdent =
        newIdent(attentionStatus.account(), Date.from(Instant.now()), serverIdent);
    // ö extract this common code: (ChangeUpdate.addIdent())
    StringBuilder stringBuilder = new StringBuilder();
    PersonIdent.appendSanitized(stringBuilder, personIdent.getName());
    stringBuilder.append(" <");
    PersonIdent.appendSanitized(stringBuilder, personIdent.getEmailAddress());
    stringBuilder.append('>');
    return gson.toJson(
        new AttentionStatusInNoteDb(
            stringBuilder.toString(), attentionStatus.removal(), attentionStatus.reason()));
  }
}
