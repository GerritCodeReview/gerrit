...
public class CommitRewriter {
  ...
  public ObjectId backfillChange(RevWalk revWalk, ObjectInserter inserter, Ref ref)
    ...
    while ((originalCommit = revWalk.next()) != null) {
      ...
      PersonIdent fixedAuthorIdent = changeNoteUtil
          .newAccountIdIdent(author.get(), authorIdent.getWhen(), authorIdent.getTimeZone());
      PersonIdent fixedCommitterIdent = changeNoteUtil
          .newAccountIdIdent(committer.get(), committerIdent.getWhen(),
              committerIdent.getTimeZone());
      String fixedCommitMessage = fixedCommitMessage(originalCommit);
      boolean needsFix =
          !fixedAuthorIdent.equals(authorIdent) || !fixedCommitterIdent.equals(committerIdent)
  ...
  private String fixedCommitMessage(RevCommit revCommit) {
    ...
    String changeSubject = RawParseUtils.decode(
        enc,
        raw,
    ...
    msg.append("\n\n");
    String changeMessage = RawParseUtils.decode(
        enc,
        raw,
    ...
    for (FooterLine fl : footerLines) {
      if (fl.getKey().toLowerCase().equals(FOOTER_ASSIGNEE) && !fl.getValue().isEmpty()) {
        Optional<Account.Id> assignee = NoteDbUtil
            .parseIdent(RawParseUtils.parsePersonIdent(fl.getValue()));
