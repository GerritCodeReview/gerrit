...
public class CommitRewriter {
  ...
  private Optional<String> getFixedIdentString(String originalIdentString) {
    ...
    String fixedIdentString =
        ChangeNoteUtil.formatAccountIdentString(account.get(), originalIdent.getEmailAddress());
    if (fixedIdentString.equals(originalIdentString)) {
      return Optional.empty();
    }
    return Optional.of(fixedIdentString);
  }
  ...
  private Optional<String> fixedCommitMessage(RevCommit revCommit) {
    ...
    for (FooterLine fl : footerLines) {
      if (fl.getKey().toLowerCase().equals(FOOTER_ASSIGNEE.getName().toLowerCase())
          && !fl.getValue().isEmpty()) {
        Optional<String> fixedAssignee = getFixedIdentString(fl.getValue());
