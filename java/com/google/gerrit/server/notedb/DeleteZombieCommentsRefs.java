...
public class DeleteZombieCommentsRefs {
  ...
  private final int CHUNK_SIZE = 100; // log progress after deleting every CHUNK_SZ refs
  private final GitRepositoryManager repoManager;
  private final AllUsersName allUsers;
  private Repository allUsersRepo;
  private Integer cleanupPercentage = null;

  public interface Factory {
    DeleteZombieCommentsRefs create(Integer cleanupPercentage);
  }
  ...
  public void setCleanupPercentage(int cleanupPercentage) {
    this.cleanupPercentage = cleanupPercentage;
  }
  ...
  public void execute() throws IOException {
    ...
    if (cleanupPercentage != null) {
      System.out.println(String.format("Cleanup percentage = %d", cleanupPercentage));
      zombieRefs =
          zombieRefs.stream()
              .filter(ref -> getChangeIdShort(ref.getName()) % 100 < cleanupPercentage)
  ...
  private int getChangeIdShort(String refName) {
    // ref name has the form "refs/draft-comments/$change_id%100/$change_id/$user_id"
    int secondSlash = StringUtils.ordinalIndexOf(refName, "/", 2);
    int thirdSlash = StringUtils.ordinalIndexOf(refName, "/", 3);
    return Integer.parseInt(refName.substring(secondSlash + 1, thirdSlash));
  }
}
