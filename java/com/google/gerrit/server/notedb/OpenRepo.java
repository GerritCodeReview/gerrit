...
class OpenRepo implements AutoCloseable {
  ...
  <U extends AbstractChangeUpdate> void addUpdates(
    ...
    for (Map.Entry<String, Collection<U>> e : all.asMap().entrySet()) {
      ...
      logger.atInfo().log("Adding update for OpenRepo %s: refName=%s oldRev=%s", this, refName, old);
      ...
      for (U update : updates) {
        ...
        logger.atInfo().log("Updated for OpenRepo %s: refName=%s oldRev=%s => newRef=%s", this, refName, old, next);
