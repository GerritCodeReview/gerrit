...
public class RepoSequence {
  ...
  RepoSequence(
    ...
    checkArgument(
        (max - floor) > batchSize,
        "batchSize needs to smaller than the total number of allocated sequence numbers. "
            + "batchSize=%s, total number of sequences=%s",
        batchSize,
        (max - floor));
          ...
          throw new OrmException(
              String.format(
                  "This gerrit instance has exhausted the allocated change sequences."
                      + "(counter + batchSize) > max. (%s + %s) > %s.",
                  counter, batchSize, max));
        }
        acquire(batchSize);
      }
      return counter++;
