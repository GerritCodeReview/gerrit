...
class ThreadLocalRepoRefCache {
  ...
  private static ThreadLocal<Map<Project.NameKey, RepoRefCache>> threadLocalValue =
      ThreadLocal.withInitial(
          () -> {
            ThreadLocalCacheCleaner.get().registerCleaner(ThreadLocalRepoRefCache::clear);
            return new HashMap<>();
          });
   ...
   */
  static RepoRefCache get(Project.NameKey projectName, Repository repo) {
    return threadLocalValue.get().computeIfAbsent(projectName, key -> new RepoRefCache(repo));
  }
