...
public class NoteDbMigrator implements AutoCloseable {
   ...
   * had to be fixed.
   *
   * <p>As of this writing, there are only two case where this happens: when a change has no patch
  ...
  private boolean canSkipPrimaryStorageMigration(ReviewDb db, Change.Id id) {
    ...
    try {
      if (Iterables.isEmpty(unwrapDb(db).patchSets().byChange(id))) {
        return true;
      }
    } catch (Exception e) {
      logger.atSevere().withCause(e).log(
    ...
    try {
      return projectCache.get(unwrapDb(db).changes().get(id).getProject()) == null;
    } catch (Exception e) {
      logger.atSevere().withCause(e).log(
          "Error checking if change %s can be skipped, assuming no", id);
      return false;
