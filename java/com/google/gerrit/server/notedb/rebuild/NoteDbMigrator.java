...
public class NoteDbMigrator implements AutoCloseable {
   ...
   * had to be fixed.
   *
   * <p>As of this writing, there are only two case where this happens is when a change has no patch
  ...
  private boolean canSkipPrimaryStorageMigration(ReviewDb db, Change.Id id) {
    ...
    // Check whether the change has any patch sets
    try {
      skipMigration = Iterables.isEmpty(unwrapDb(db).patchSets().byChange(id));
    } catch (Exception e) {
      logger.atSevere().withCause(e).log(
    ...
    try {
      Change change = unwrapDb(db).changes().get(id);
      skipMigration = projectCache.get(change.getProject()) == null;
    } catch (Exception e) {
      logger.atSevere().withCause(e).log(
          "Error checking if change %s can be skipped, assuming no", id);
      return false;
