...
class DiffContentCalculator {
  ...
  DiffCalculatorResult calculateDiffContent(
    ...
    if (srcA.src == srcB.src && srcA.size() <= context && edits.isEmpty()) {
      ...
      DiffContent diffContent = new DiffContent(diffA.build(), SparseFileContent
          .create(ImmutableList.of(), srcB.size()));
      ...
      DiffContent diffContent = packContent(srcA, srcB,
          diffPrefs.ignoreWhitespace != Whitespace.IGNORE_NONE, sortedEdits, MAX_CONTEXT);
