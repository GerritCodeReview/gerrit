...
public class DiffMappings {
  ...
  public static Mapping toMapping(List<Edit> edits, String oldName, String newName, Patch.ChangeType changeType) {
    FileMapping fileMapping = toFileMapping(changeType, oldName, newName);
    ImmutableSet<RangeMapping> rangeMappings = toRangeMappings(edits);
    return Mapping.create(fileMapping, rangeMappings);
  }
  ...
  private static FileMapping toFileMapping(Patch.ChangeType changeType, String oldName, String newName) {
    switch (changeType) {
      case ADDED:
