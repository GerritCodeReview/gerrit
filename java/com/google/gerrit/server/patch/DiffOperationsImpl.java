...
public class DiffOperationsImpl implements DiffOperations {
  ...
  public FileDiffOutput getModifiedFile(
    ...
    FileDiffCacheKey key =
        createFileDiffCacheKey(
            project, oldCommit, newCommit, fileName, DEFAULT_DIFF_ALGORITHM, whitespace);
    return getModifiedFileForKey(key);
  }
   ...
   * results, i.e. due to timeouts in the cache loader, this method requests the diff again using
  ...
  private ImmutableMap<String, FileDiffOutput> getModifiedFilesForKeys(List<FileDiffCacheKey> keys)
    ...
    for (FileDiffCacheKey key : fileDiffs.keySet()) {
      ...
      if (diff.isNegative()) {
        FileDiffCacheKey fallbackKey =
            key.toBuilder()
                .diffAlgorithm(DiffAlgorithm.HISTOGRAM_NO_FALLBACK)
                // We don't want to use timeouts while using the fallback algorithm
                .useTimeout(false)
                .build();
    ...
    result.putAll(fileDiffCache.getAll(fallbackKeys));
    return buildResult(result.build().values());
  }

  /**
   * Converts a collection of {@link FileDiffOutput} to a {@link ImmutableMap} of string to {@link
   * FileDiffOutput}. The result map keys represent the old file path for deleted files, or the new
   * path otherwise.
   */
  private ImmutableMap<String, FileDiffOutput> buildResult(
  ...
  private static FileDiffCacheKey createFileDiffCacheKey(
    ...
    return FileDiffCacheKey.builder()
        ...
        // Default is to use timeout enforcement
        .useTimeout(true)
        .build();
  }
