...
public class DiffsImpl implements Diffs {
  ...
  private Map<String, FileDiffOutput> getModifiedFiles(
      ...
      throws DiffNotAvailableException {
    Map<String, FileDiffOutput> files = new TreeMap<>();
    try {
      ...
      for (Map.Entry<FileDiffCacheKey, FileDiffOutput> entry : fileDiffs.entrySet()) {
        ...
        FileDiffOutput entity = entry.getValue();
        if (entity.changeType().get() == Patch.ChangeType.DELETED) {
          files.put(entity.oldPath().get(), fileDiffOutput);
        } else {
          files.put(entity.newPath().get(), fileDiffOutput);
        }
      }
  ...
  private boolean allDueToRebase(FileDiffOutput fileDiffOutput) {
  ...
  private ModifiedFilesCacheKey createModifiedFilesKey(
  ...
  private FileDiffCacheKey createFileDiffCacheKey(
