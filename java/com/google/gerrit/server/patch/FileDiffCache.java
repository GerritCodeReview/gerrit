...
public class FileDiffCache {
  ...
  static class FileDiffLoader extends CacheLoader<Key, PatchListEntry> {
    ...
    private Map<Key, PatchListEntry> forFiles(
        ...
        throws IOException, ExecutionException {
      Map<Key, AllFileGitDiffs> fileToAllDiffs = new AllDiffsEvaluator(repo, ins, rw).call(keys);
      Map<Key, PatchListEntry> result = new HashMap<>();
    ...
    private class AllDiffsEvaluator {
      ...
      private Map<Key, AllFileGitDiffs> call(List<Key> keys)
        ...
        getGitDiffsForManyKeys(
            ...
            (Key key) -> null,
            (Key key, GitFileDiffCache.Key gitKey) -> fileToAllDiffs.get(key).main.key = gitKey,
            (Key key, GitFileDiff val) -> fileToAllDiffs.get(key).main.value = val,
            true);
        ...
        getGitDiffsForManyKeys(
            ...
            (Key key, GitFileDiffCache.Key gitKey) -> fileToAllDiffs.get(key).oldVsPar.key = gitKey,
            (Key key, GitFileDiff val) -> fileToAllDiffs.get(key).oldVsPar.value = val,
            false);
        ...
        getGitDiffsForManyKeys(
            ...
            (Key key, GitFileDiffCache.Key gitKey) -> fileToAllDiffs.get(key).newVsPar.key = gitKey,
            (Key key, GitFileDiff val) -> fileToAllDiffs.get(key).newVsPar.value = val,
            false);
        ...
        getGitDiffsForManyKeys(
            ...
            (Key key, GitFileDiffCache.Key gitKey) -> fileToAllDiffs.get(key).parVsPar.key = gitKey,
            (Key key, GitFileDiff val) -> fileToAllDiffs.get(key).parVsPar.value = val,
            false);
      ...
      private void getGitDiffsForManyKeys(
          ...
          BiConsumer<Key, GitFileDiffCache.Key> keyFiller,
          BiConsumer<Key, GitFileDiff> valueFiller,
          boolean oldVsNewDiff)
          throws IOException, ExecutionException {
        Map<Key, GitFileDiffCache.Key> keyToGitKeys = new HashMap<>();
        List<GitFileDiffCache.Key> gitKeys = new ArrayList<>();
        for (Key key : keys) {
          if (fileToAllDiffs.get(key).hasRebaseEdits || oldVsNewDiff) {
      ...
      private void backFill(
        ...
        for (Key k : keyToGitKeysMap.keySet()) {
          ...
          if (gitFileDiff != null) {
            valueFiller.accept(k, gitFileDiff);
          }
        }
      }
    }
