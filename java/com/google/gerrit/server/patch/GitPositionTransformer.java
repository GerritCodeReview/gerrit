...
public class GitPositionTransformer {
  ...
  private static <T> ImmutableList<PositionedEntity<T>> shiftRangesInOneFile(
    ...
    while (entityIndex < sortedEntities.size() && mappingIndex < sortedMappings.size()) {
      ...
      if (mapping.oldEndLine() <= entity.position().lineRange().start()) {
        shiftedAmount = mapping.newEndLine() - mapping.oldEndLine();
        mappingIndex++;
      } else if (entity.position().lineRange().end() <= mapping.oldStartLine()) {
  ...
  private static ImmutableList<RangeMapping> sortByOldStartEnd(Set<RangeMapping> mappings) {
    return mappings.stream()
        .sorted(comparing(RangeMapping::oldStartLine).thenComparing(RangeMapping::oldEndLine))
        .collect(toImmutableList());
  }
  ...
  public abstract static class RangeMapping {
    ...
    /** Start line of the range in the source tree. (inclusive) */
    public abstract int oldStartLine();

    /** End line of the range in the source tree. (exclusive) */
    public abstract int oldEndLine();

    /** Start line of the range in the target tree. (inclusive) */
    public abstract int newStartLine();

    /** End line of the range in the target tree. (exclusive) */
    public abstract int newEndLine();

    /**
     * Creates a new {@code RangeMapping}.
     *
     * @param oldStart see {@link #oldStartLine()}
     * @param oldEnd see {@link #oldEndLine()}
     * @param newStart see {@link #newStartLine()}
     * @param newEnd see {@link #newEndLine()}
     */
    public static RangeMapping create(int oldStart, int oldEnd, int newStart, int newEnd) {
      return new AutoValue_GitPositionTransformer_RangeMapping(oldStart, oldEnd, newStart, newEnd);
    }
  }
