...
public class IntraLineLoader implements Callable<IntraLineDiff> {
  ...
  public static IntraLineDiff compute(
    ...
    // Re-wrap the shared engine's edits into Gerrit's own ReplaceEdit before
    // they reach the persistent "intraline" cache. Load-bearing, not cosmetic:
    // IntraLineDiff.writeObject() serializes a ReplaceEdit's character-level
    // internal edits only when the edit is an instanceof Gerrit's ReplaceEdit.
    // The Gitiles ReplaceEdit is a sibling type (both extend jgit Edit), so
    // caching it directly would silently drop the intraline marks and change
    // the on-disk byte layout, invalidating entries in live Gerrit instances.
    // Share only the compute engine (above); keep the cached value Gerrit's own.
    for (Edit e : computed) {
