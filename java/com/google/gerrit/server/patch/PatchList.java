...
public class PatchList implements Serializable {
  ...
  static final Comparator<ChangeType> CHANGE_TYPE_CMP =
      new Comparator<ChangeType>() {
        // We use the ChangeType comparator for a rare case when PatchList contains two entries
        // for the same file, e.g. {ADDED, DELETED}. We return a single entry according to the
        // following order. Check the following bug for an example case:
        // https://bugs.chromium.org/p/gerrit/issues/detail?id=13914.
        private final List<ChangeType> order =
            ImmutableList.of(
                ChangeType.ADDED,
                ChangeType.RENAMED,
                ChangeType.MODIFIED,
