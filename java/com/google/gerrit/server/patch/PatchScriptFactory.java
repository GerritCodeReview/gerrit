...
public class PatchScriptFactory implements Callable<PatchScript> {
  ...
  private final PatchScriptStrategy strategy;
  ...
  PatchScriptFactory(
    ...
    this.strategy = new PatchScriptFromPatchListStrategy(grm, psUtil, builderFactory, patchListCache, commentsUtil, editReader, userProvider, notes, fileName, patchSetA, patchSetB, -1, diffPrefs);
  }
  ...
  PatchScriptFactory(
    ...
    this.strategy = new PatchScriptFromPatchListStrategy(grm, psUtil, builderFactory, patchListCache, commentsUtil, editReader, userProvider, notes, fileName,null, patchSetB, parentNum, diffPrefs);
  }
/*
  @AssistedInject
  PatchScriptFactory(
...
*/
  public void setLoadHistory(boolean load) {
    strategy.setLoadHistory(load);
  }

  public void setLoadComments(boolean load) {
    strategy.setLoadComments(load);
  }
  ...
  public PatchScript call()
    ...
    return strategy.createPatchScript();
  }

  private interface PatchScriptStrategy {
    PatchScript createPatchScript() throws LargeObjectException, AuthException;
    void setLoadHistory(boolean load);
    void setLoadComments(boolean load);
  }

  private static class PatchScriptFromPatchListStrategy implements PatchScriptStrategy {
    ...
    PatchScriptFromPatchListStrategy(GitRepositoryManager repoManager, PatchSetUtil psUtil, Provider<PatchScriptBuilder> builderFactory, PatchListCache patchListCache, CommentsUtil commentsUtil, ChangeEditUtil editReader, Provider<CurrentUser> userProvider, ChangeNotes notes, String fileName, PatchSet.Id patchSetA, PatchSet.Id patchSetB, int parentNum, DiffPreferencesInfo diffPrefs) {
      ...
      this.psa = patchSetA;
      this.psb = patchSetB;
      this.changeId = patchSetB.changeId();
    ...
    @Override
    public PatchScript createPatchScript() throws LargeObjectException, AuthException {
      try (Repository git = repoManager.openRepository(notes.getProjectName())) {
        try {
          ...
          Optional<CommentDetail> comments = loadComments(content, changeEdit, history.orElse(ImmutableList.of()));
    ...
    private Optional<CommentDetail> loadComments(PatchListEntry content, boolean changeEdit, ImmutableList<Patch> history) {
      if (!loadComments) {
        return Optional.empty();
      }
      return new CommentsLoader(psa, psb, userProvider, notes, commentsUtil)
          .load(changeEdit, content.getChangeType(), content.getOldName(), content.getNewName(), history);
    }
    ...
    private Optional<ImmutableList<Patch>> loadHistory(PatchListEntry content, boolean changeEdit) {
      ...
      return Optional.of(
          loader.load(
              changeEdit, content.getChangeType(), content.getOldName()));
    }
  }
  ...
  private static class HistoryLoader {
    ...
    private ImmutableList<Patch> load(
        boolean changeEdit, ChangeType changeType, String oldName) {
  ...
  private static class CommentsLoader {
    ...
    private Optional<CommentDetail> load(
        boolean changeEdit, ChangeType changeType, String oldName, String newName, ImmutableList<Patch> history) {
