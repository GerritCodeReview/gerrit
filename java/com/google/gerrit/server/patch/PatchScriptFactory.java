...
public class PatchScriptFactory implements Callable<PatchScript> {
  ...
  static class Metrics {
    ...
    Metrics(MetricMaker metricMaker) {
      ...
      totalDiffs =
          metricMaker.newCounter(
              "diff/get_diff/total_diffs",
              new Description("Total number of diff requests.").setRate().setUnit("count"));
    }
  }
  ...
  private static boolean areEqualPatchscripts(PatchScript ps1, PatchScript ps2) {
    return ps1.getChangeType().equals(ps2.getChangeType())
        ...
        && equalStrings(ps1.getOldName(), ps2.getOldName())
        && equalStrings(ps1.getNewName(), ps2.getNewName())
