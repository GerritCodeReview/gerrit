...
public class SubmitWithStickyApprovalDiff {
  ...
  public String apply(ChangeNotes notes, CurrentUser currentUser)
    ...
    if (notes.getPatchSets().get(currentPatchsetId) == null) {
      // In some submit strategies, the current patch-set doesn't exist yet as it's being created
      // during the submit. In that case, we assign the current patch-set to be one patch-set
      // behind.
      currentPatchsetId = PatchSet.id(currentPatchsetId.changeId(), currentPatchsetId.get() - 1);
    }
    ...
    PatchList patchList =
        getPatchList(
            ...
            notes.getPatchSets().get(currentPatchsetId),
    ...
    for (PatchListEntry patchListEntry :
      ...
      diff +=
          getDiffForFile(
              notes, currentPatchsetId, latestApprovedPatchsetId, patchListEntry, currentUser);
    }
    return diff;
  }
