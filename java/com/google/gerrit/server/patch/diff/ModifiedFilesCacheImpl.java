  ...
  static class ModifiedFilesLoader extends CacheLoader<Key, ImmutableList<ModifiedFile>> {
    ...
    private ImmutableList<ModifiedFile> loadModifiedFiles(Key key, RevWalk rw)
      ...
      Pair<RevCommit, RevCommit> oldAndNewCommits =
          DiffUtil.areRelated(rw, key.aCommit(), key.bCommit());
      ...
      Set<String> touchedFiles =
          getTouchedFilesWithParents(
              key,
              oldAndNewCommits.getLeft().getParent(0).getId(),
              oldAndNewCommits.getRight().getParent(0).getId(),
              rw);
    ...
    private Set<String> getTouchedFilesWithParents(
      ...
      try {
        ...
        logger.atWarning().log(
            "Failed to retrieve the touched files files "
                + "commits (%s, %s) and parents (%s, %s): %s",
  ...
  public abstract static class Key implements Serializable {
    ...
    public enum Serializer implements CacheSerializer<Key> {
      ...
      public byte[] serialize(Key key) {
        ...
        return GitModifiedFilesCacheImpl.Key.Serializer.INSTANCE.serialize(gitKey);
      }

      @Override
      public Key deserialize(byte[] in) {
        GitModifiedFilesCacheImpl.Key gitKey =
            GitModifiedFilesCacheImpl.Key.Serializer.INSTANCE.deserialize(in);
        return Key.builder()
