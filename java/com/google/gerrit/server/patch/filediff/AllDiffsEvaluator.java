...
/**
 * A helper class that computes the 4 {@link GitFileDiff}s for a list of {@link FileDiffCacheKey}s:
 *
 * <p>
 *
 * <ul>
 ...
 * </ul>
 *
 * The 4 {@link GitFileDiff} are stored in the entity class {@link AllFileGitDiffs}. We use these
...
class AllDiffsEvaluator {
  ...
  Map<WrappedKey, AllFileGitDiffs> execute(List<WrappedKey> wrappedKeys)
      throws DiffNotAvailableException {
    ImmutableMap.Builder<WrappedKey, AllFileGitDiffs> keyToAllDiffs =
        ImmutableMap.builderWithExpectedSize(wrappedKeys.size());
    ...
    for (WrappedKey wrappedKey : wrappedKeys) {
      ...
      if (oldVsParentDiffs.containsKey(key) && !oldVsParentDiffs.get(key).gitDiff().isEmpty()) {
        builder.oldVsParDiff(Optional.of(oldVsParentDiffs.get(key)));
      }

      if (newVsParentDiffs.containsKey(key) && !newVsParentDiffs.get(key).gitDiff().isEmpty()) {
        builder.newVsParDiff(Optional.of(newVsParentDiffs.get(key)));
      }

      if (parentsDiffs.containsKey(key) && !parentsDiffs.get(key).gitDiff().isEmpty()) {
        builder.parVsParDiff(Optional.of(parentsDiffs.get(key)));
      }
  ...
  /**
   * Convert a list of {@link WrappedKey} to their corresponding {@link GitFileDiffCacheKey} which
   * can be used to call the underlying {@link GitFileDiffCache}.
   *
    ...
    for (WrappedKey key : keys) {
      try {
        ...
        if (path != null) {
          result.put(
              key.key(),
              createGitKey(key.key(), oldCommitFn.apply(key), newCommitFn.apply(key), path, rw));
        }
      } catch (IOException e) {
