...
class AllDiffsEvaluator {
   ...
   * @param aCommitFn a function to compute the old commit that will be used in the git diff.
   * @param bCommitFn a function to compute the new commit that will be used in the git diff.
  ...
  private Map<FileDiffCacheKey, GitFileDiffCacheKey> createGitKeys(
    ...
    for (AugmentedFileDiffCacheKey key : keys) {
      try {
        ...
        // Check the caller's code path about the correrctness of the computation in this case.
