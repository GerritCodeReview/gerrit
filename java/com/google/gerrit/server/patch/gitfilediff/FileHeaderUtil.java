...
public class FileHeaderUtil {
  ...
  static PatchType getPatchType(FileHeader header) {
    ...
    if (patchType != PatchType.BINARY) {
      int start = header.getStartOffset();
      int end = header.getEndOffset();
      byte[] buf = ArrayUtils.subarray(header.getBuffer(), start, end);
      if (Arrays.asList(ArrayUtils.toObject(buf)).contains(NUL)) {
        // It's really binary, but Git couldn't see the nul early enough to realize its binary, and
        // instead produced the diff.
        //
        // Force it to be a binary; it really should have been that.
        patchType = PatchType.BINARY;
      }
    }
    return patchType;
  }
