...
class DefaultRefFilter {
  ...
  private final @Nullable SearchingChangeCacheImpl searchingChangeCache;
  ...
  private GitVisibleChangeFilter visibleChangesFilter;
  ...
  DefaultRefFilter(
    ...
    this.searchingChangeCache = searchingChangeCache;
  ...
  ImmutableList<Ref> filter(Collection<Ref> refs, Repository repo, RefFilterOptions opts)
    ...
    visibleChangesFilter =
        GitVisibleChangeFilter.create(
            searchingChangeCache,
            changeNotesFactory,
            projectState.getNameKey(),
            permissionBackendForProject,
            repo,
            numberOfChanges(refs));
    logger.atFinest().log(
    ...
    for (Ref ref : refs) {
      ...
      if (opts.filterMeta() && isMetadata(refName)) {
        ...
        if (hasAccessDatabase) {
          resultRefs.add(ref);
        } else if (!visibleChangesFilter.isVisible(changeId)) {
   ...
   * the provided refs.
   */
  private static long numberOfChanges(Collection<Ref> refs) {
    return refs.stream()
        .filter(
            r ->
                r.getName().startsWith(RefNames.REFS_CHANGES)
                    && r.getName().endsWith(RefNames.META_SUFFIX))
        .count();
  }
  ...
  private boolean visibleEdit(String name) throws PermissionBackendException {
    ...
    if (user.isIdentifiedUser()
        ...
        && visibleChangesFilter.isVisible(id)) {
      logger.atFinest().log("Own change edit ref is visible: %s", name);
      return true;
    }

    if (visibleChangesFilter.isVisible(id)) {
      // Default to READ_PRIVATE_CHANGES as there is no special permission for reading edits.
      Optional<BranchNameKey> dest = visibleChangesFilter.getDestination(id);
      boolean canRead =
          dest.isPresent()
              && permissionBackendForProject
                  .ref(dest.get().branch())
