...
public class DefaultRefFilter {
  ...
  Result filterRefs(
    ...
    boolean classifierCanShortcut = classifier.hasShortcuttableRefs();
    ...
    for (Ref ref : refs) {
      ...
      if (opts.filterMeta() && isMetadata(refName)) {
        ...
        if (!RefNames.isGerritRef(leafName)
            ...
            && classifierCanShortcut) {
          ReadAccessClassifier.Decision decision = classifier.classify(leafName);
          if (decision == ReadAccessClassifier.Decision.VISIBLE) {
            logger.atFinest().log("Include ref %s -> %s because classifier returned VISIBLE", refName, leafName);
            metrics.classifierShortcutCount.increment();
            resultRefs.add(ref);
            continue;
          } else if (decision == ReadAccessClassifier.Decision.INVISIBLE) {
            logger.atFinest().log(
                "Filter out ref %s -> %s because classifier returned INVISIBLE", refName, leafName);
            metrics.classifierShortcutCount.increment();
            continue;
          }
          logger.atFinest().log(
                  "Ref %s -> %s requires a full visibility check.", refName, leafName);
