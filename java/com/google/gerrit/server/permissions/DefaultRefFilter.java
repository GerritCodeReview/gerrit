...
class DefaultRefFilter {
  ...
  private final GitVisibleChangeFilter.Factory visibleChangesCacheFactory;

  private GitVisibleChangeFilter visibleChangesCache;
  ...
  DefaultRefFilter(
      ...
      GitVisibleChangeFilter.Factory visibleChangesCacheFactory,
  ...
  ImmutableList<Ref> filter(Collection<Ref> refs, Repository repo, RefFilterOptions opts)
    ...
    long numberOfChangesToBeFiltered =
        refs.stream()
            ...
            .count();
    visibleChangesCache =
        visibleChangesCacheFactory.create(projectControl, repo, numberOfChangesToBeFiltered);
    logger.atFinest().log(
    ...
    if (visibleChangesCache.isVisible(id)) {
      ...
      boolean canRead =
          permissionBackendForProject
              .ref(visibleChangesCache.getDestination(id).branch())
