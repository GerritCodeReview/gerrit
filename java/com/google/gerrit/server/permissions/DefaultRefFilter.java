...
class DefaultRefFilter {
  ...
  Result filterRefs(List<Ref> refs, Repository repo, RefFilterOptions opts)
    ...
    for (Ref ref : refs) {
      Change.Id changeId;
      if (opts.filterMeta() && isMetadata(ref.getName())) {
        logger.atFinest().log("Filter out metadata ref %s", ref.getName());
        continue;
      } else if (isTag(ref)) {
        ...
        // visibility of these refs
        // just fine. But instead, we use highly-optimized logic that looks only on the last 10k
        // most recent changes
        // using
