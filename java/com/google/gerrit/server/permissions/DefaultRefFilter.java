...
class DefaultRefFilter {
          ...
          .filter(
              r ->
                  !r.getName().startsWith(RefNames.REFS_CHANGES)
                      && !!r.getName().startsWith(RefNames.REFS_META))
  ...
  private boolean canSeeSingleChangeRef(String refName) throws PermissionBackendException {
    ...
    // even if the change is not part if the set of most recent changes that
    // SearchingChangeCacheImpl returns.
    Change.Id cId = Change.Id.fromRef(refName);
    checkNotNull(cId, "invalid change id for ref " + refName);
    ChangeNotes notes;
    try {
