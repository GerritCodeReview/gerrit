...
/** Default implementation for {@code WithUser} based on configs in the meta config branch. */
public class DefaultWithUserImpl extends PermissionBackend.WithUser {
  ...
  DefaultWithUserImpl(
      ProjectCache projectCache, ProjectControl.Factory projectControlFactory, CurrentUser user) {
  ...
  private boolean allow(Collection<PermissionRule> rules) {
    return user.getEffectiveGroups()
        .containsAnyOf(
            rules
                .stream()
                .filter(r -> r.getAction() == PermissionRule.Action.ALLOW)
