...
public class PerThreadProjectCache {
  ...
  private final Map<PerThreadCache.Key<ProjectControl>, ProjectControl> projectControlByKey =
      Maps.newHashMapWithExpectedSize(PER_THREAD_PROJECT_CACHE_SIZE);

  private PerThreadProjectCache() {}

  public static ProjectControl getOrCompute(
      PerThreadCache.Key<ProjectControl> key, Supplier<ProjectControl> loader) {
    ...
    if (perThreadCache != null) {
      ...
      return perThreadProjectCache.get(key, loader);
    }
    return loader.get();
  }

  protected ProjectControl get(
      PerThreadCache.Key<ProjectControl> key, Supplier<ProjectControl> loader) {
    ProjectControl value = projectControlByKey.get(key);
    if (value == null) {
      value = loader.get();
      if (projectControlByKey.size() < PER_THREAD_PROJECT_CACHE_SIZE) {
        projectControlByKey.put(key, value);
      }
    }
    return value;
  }
