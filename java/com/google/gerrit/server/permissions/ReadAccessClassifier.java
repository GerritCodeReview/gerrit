...
public class ReadAccessClassifier {
  ...
  /**
   * Sections with BLOCK or DENY rules requiring a full ACL check. Checked before allowMatchers
   * because BLOCK/DENY takes precedence.
   */
  ...
  /**
   * Returns {@code true} if this classifier has any allow matchers that could return {@link
   * Decision#VISIBLE} for at least some refs. When this returns {@code false}, every ref would get
   * {@link Decision#NEEDS_FULL_CHECK} or {@link Decision#INVISIBLE}, so callers can skip the
   * classifier entirely and go straight to the full visibility check without any loss.
   */
