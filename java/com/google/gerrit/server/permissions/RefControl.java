    ...
    for (List<Permission> projectRules : downwardPerProject) {
      ...
      for (Permission p : projectRules) {
        ...
        if (overrideFound) {
          ...
          logger.atFine().log("Exclusive override found for permission %s", permissionName);
          continue projectLoop;
        }
        ...
        for (PermissionRule pr : p.getRules()) {
          ...
          if (isBlock(pr, withForce) && projectControl.match(pr, isChangeOwner)) {
            logger.atFine().log("Block rule found for permission %s", permissionName);
            blocked = true;
            break;
          }
        }
        ...
        if (blocked) {
          logger.atFine().log("Permission %s is blocked", permissionName);
          return true;
        }
      }
    }
