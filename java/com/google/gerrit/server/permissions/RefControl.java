...
class RefControl {
  ...
  private PermissionRange toRange(String permissionName, boolean isChangeOwner) {
    ...
    // NOSUBMIT - check if we need handling for exclusive
    for (Permission p : relevant.getBlocks(permissionName)) {
  ...
  private boolean isBlocked(String permissionName, boolean isChangeOwner, boolean withForce) {
    ...
    // from.
    List<Permission> orderedPermissions = relevant.getBlocks(permissionName);
    for (Permission p : orderedPermissions) {
      if (p.getExclusiveGroup()) {
        ...
        // NOSUBMIT - this should only take effect if there is a BLOCK rule in the same project.
      }
