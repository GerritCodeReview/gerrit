...
public class PluginContext<T> {
  ...
  @FunctionalInterface
  public interface ExtensionImplFunctionAllowingException<T, R, X extends Exception> {
  ...
  @FunctionalInterface
  public interface ExtensionFunctionAllowingException<
      T extends Extension<?>, R, X extends Exception> {
  ...
  static <T> void runLogExceptions(
    ...
    T extensionPoint = extension.get();
    if (extensionPoint == null) {
      return;
    }

    try (TraceContext traceContext = newTrace(extension)) {
      extensionImplConsumer.run(extensionPoint);
    } catch (Throwable e) {
      logger.atWarning().withCause(e).log(
          "Failure in %s of plugin %s", extensionPoint.getClass(), extension.getPluginName());
    }
  }
  ...
  static <T, R, X extends Exception> R call(
      Extension<T> extension,
      ExtensionImplFunctionAllowingException<T, R, X> function,
      Class<X> exceptionClass)
      throws X {
    try (TraceContext traceContext = newTrace(extension)) {
      try {
        return function.call(extension.get());
      } catch (Exception e) {
  ...
  static <T, R, X extends Exception> R call(
      Extension<T> extension,
      ExtensionFunctionAllowingException<Extension<T>, R, X> function,
      Class<X> exceptionClass)
      throws X {
    try (TraceContext traceContext = newTrace(extension)) {
      try {
        return function.call(extension);
      } catch (Exception e) {
        // The only exception that can be thrown is X, but we cannot catch X since it is a generic
        // type.
        Throwables.throwIfInstanceOf(e, exceptionClass);
