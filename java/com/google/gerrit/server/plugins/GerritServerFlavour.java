...
/**
 * Servlet flavour (EE8 {@code javax.servlet} vs EE11 {@code jakarta.servlet}) a Gerrit build ships,
 ...
 *       be assumed servlet-neutral, so an EE11 Gerrit rejects it rather than risk loading a {@code
 *       javax.servlet}-coupled plugin that would fail at runtime.
 ...
 *   <li><b>{@code ee11}</b> — {@code jakarta.servlet}; loads only on an EE11 Gerrit. The bazlets
 *       {@code gerrit_plugin(flavour = "ee11")} macro injects this.
 * </ul>
 */
public enum GerritServerFlavour {
  ...
  /** The {@code Gerrit-Flavour} value for this flavour ({@code ee8} / {@code ee11}). */
  ...
  private static GerritServerFlavour detect() {
    ...
    // jakarta.servlet (EE11) or javax.servlet (EE8, the default).
    return isPresent("jakarta.servlet.Servlet") ? EE11 : EE8;
  }
   ...
   * @return the declared flavour ({@link #EE8}, {@link #EE11} or {@link #ANY}).
   * @throws InvalidPluginException if a non-null value is not one of ee8/ee11/any.
   */
  public static GerritServerFlavour parsePluginFlavour(String pluginName, @Nullable String value)
    ...
    throw new InvalidPluginException(
        String.format(
            "Plugin '%s' declares an unknown %s '%s'; expected ee8, ee11 or any.",
