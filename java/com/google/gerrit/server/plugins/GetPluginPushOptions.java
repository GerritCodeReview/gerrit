...
public class GetPluginPushOptions implements RestReadView<PluginResource> {
  ...
  @Inject
  GetPluginPushOptions(DynamicSet<PluginPushOption> pluginPushOption) {
    this.pluginPushOptions = pluginPushOption;
  }

  @Override
  public Response<ImmutableList<PluginPushOption>> apply(PluginResource resource) {
    return Response.ok(
        StreamSupport.stream(this.pluginPushOptions.entries().spliterator(), /* parallel= */ false)
