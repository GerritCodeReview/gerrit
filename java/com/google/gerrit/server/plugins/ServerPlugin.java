...
public class ServerPlugin extends Plugin {
  ...
  private void loadGuiceModules(Manifest manifest, ClassLoader classLoader)
    ...
    if (!Strings.isNullOrEmpty(sshName)
        && getApiType() != Plugin.ApiType.PLUGIN
        && getApiType() != Plugin.ApiType.PLUGIN_API) {
  ...
  private void startPlugin(PluginGuiceEnvironment env) throws Exception {
    ...
    if (env.hasSshModule()) {
      ...
      if (getApiType() == ApiType.PLUGIN || getApiType() == ApiType.PLUGIN_API) {
  ...
  private Injector newRootInjector(PluginGuiceEnvironment env) {
    ...
    modules.addAll(env.getApiModules());
