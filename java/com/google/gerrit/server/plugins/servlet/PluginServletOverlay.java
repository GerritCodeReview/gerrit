 ...
 * {@code jakarta.servlet} by the {@code to_jakarta} transform under the EE10 flavour -- lives here,
 ...
 * {@code server} depends on; it must NOT depend on {@code server} (and cannot live in {@code httpd},
 * which depends on {@code server}).
 *
 * <p>The type-name constants below stay {@code String}s so callers keep using their own {@code is()}
 * hierarchy walk -- the seam never needs to reach back into the loader.
 */
