...
public class OverrideSubmitTypeEvaluator implements SubmitTypeEvaluator {

  private final ProjectCache projectCache;
  private final SubmitRequirementChangeQueryBuilder.Factory queryBuilderFactory;

  @Inject
  private OverrideSubmitTypeEvaluator(
  ...
  public SubmitTypeRecord evaluate(ChangeData cd) {
    ...
    ImmutableSet<SubmitTypeOverrideRule> overrides =
        ImmutableSet.copyOf(projectState.getSubmitTypeOverrideRules());
    return evaluateWithOverrides(cd, overrides);
  }

  public SubmitTypeRecord evaluateWithOverrides(
      ChangeData cd, Set<SubmitTypeOverrideRule> overrides) {
    for (SubmitTypeOverrideRule submitTypeOverride : overrides) {
      try {
