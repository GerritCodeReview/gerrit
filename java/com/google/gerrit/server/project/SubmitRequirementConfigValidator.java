...
public class SubmitRequirementConfigValidator implements CommitValidationListener {
  ...
  public List<CommitValidationMessage> onCommitReceived(CommitReceivedEvent event)
    ...
    try {
      ...
      Map<String, SubmitRequirement> previousSubmitRequirements =
          projectConfig.getSubmitRequirementSections().isEmpty()
              ? Map.of()
              : getPreviousSubmitRequirements(event);
      ...
      for (SubmitRequirement submitRequirement :
        ...
        if (!submitRequirement.equals(
            previousSubmitRequirements.getOrDefault(submitRequirement.name(), null))) {
  ...
  private Optional<ProjectConfig> getPreviousProjectConfig(CommitReceivedEvent receiveEvent)
      throws IOException, ConfigInvalidException {
    return receiveEvent.commit.getParentCount() == 0
        ? Optional.empty()
        : Optional.of(getProjectConfig(receiveEvent, receiveEvent.commit.getParent(0)));
  }

  private Map<String, SubmitRequirement> getPreviousSubmitRequirements(CommitReceivedEvent event)
      throws ConfigInvalidException, IOException {
    return getPreviousProjectConfig(event)
        .map(ProjectConfig::getSubmitRequirementSections)
        .orElse(Map.of());
  }
}
