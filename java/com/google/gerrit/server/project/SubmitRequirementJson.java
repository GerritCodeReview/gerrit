...
public class SubmitRequirementJson {
  public static SubmitRequirementInfo format(SubmitRequirement sr) {
    ...
    if (sr.applicabilityExpression().isPresent()) {
      info.applicabilityExpression = sr.applicabilityExpression().get().expression();
    }
    if (sr.overrideExpression().isPresent()) {
      info.overrideExpression = sr.overrideExpression().get().expression();
    }
    info.submittabilityExpression = sr.blockingExpression().expression();
