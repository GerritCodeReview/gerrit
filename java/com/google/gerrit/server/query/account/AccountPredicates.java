...
public class AccountPredicates {
  public static boolean hasActive(Predicate<AccountState> p) {
    return QueryBuilder.find(p, AccountPredicate.class, AccountField.ACTIVE_FIELD_SPEC.getName()) != null;
  }
  ...
  public static Predicate<AccountState> defaultPredicate(
    ...
    if (canSeeSecondaryEmails) {
      ...
      if (schema.hasField(AccountField.NAME_PART_NO_SECONDARY_EMAIL_SPEC)) {
        ...
        if (schema.hasField(AccountField.PREFERRED_EMAIL_PREFIX_SPEC)) {
  ...
  public static Predicate<AccountState> id(Schema<AccountState> schema, Account.Id accountId) {
    return new AccountPredicate(
        schema.hasField(AccountField.ID_FIELD_SPEC) ? AccountField.ID_FIELD_SPEC : AccountField.ID_STR_FIELD_SPEC,
  ...
  public static Predicate<AccountState> preferredEmailExact(String email) {
    return new AccountPredicate(
        AccountField.PREFERRED_EMAIL_EXACT_SPEC, AccountQueryBuilder.FIELD_PREFERRED_EMAIL_EXACT, email);
  }
  ...
  static class AccountPredicate extends IndexPredicate<AccountState> {
    AccountPredicate(Field<AccountState, ?>.FieldSpec<AccountState, ?> def, String value) {
      super(def, value);
    }

    AccountPredicate(Field<AccountState, ?>.FieldSpec<AccountState, ?> def, String name, String value) {
