...
public class ApprovalQueryBuilder extends QueryBuilder<ApprovalContext, ApprovalQueryBuilder> {
  ...
  public Predicate<ApprovalContext> approverin(String group) throws QueryParseException {
    return userin(UserInPredicate.Field.APPROVER, group);
  }

  @Operator
  public Predicate<ApprovalContext> uploaderin(String group) throws QueryParseException {
    return userin(UserInPredicate.Field.UPLOADER, group);
  }
  ...
  private Predicate<ApprovalContext> userin(UserInPredicate.Field field, String group) throws QueryParseException{
    // for plugins the value will be operandName_pluginName
    List<String> names = PLUGIN_SPLITTER.splitToList(group);
    if (names.size() == 2) {
    ...
    return userInPredicate.create(field, parseGroupOrThrow(group));
  }
