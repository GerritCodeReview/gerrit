...
/**
 * Predicate that matches patch set patchset approvals we want to copy if the change between the old
 * and new patch set is of a certain kind.
 */
public class ChangeKindPredicate extends ApprovalPredicate {
  ...
  public boolean match(ApprovalContext ctx) {
    if (!ctx.patchSetApproval().patchSetId().changeId().equals(ctx.target().changeId())) {
      // No support for cherry picks yet.
      return false;
    }

    if (ctx.patchSetApproval().patchSetId().get() + 1 != ctx.target().get()) {
      // We only support copying approvals to the next consecutive patch set.
      return false;
    }
