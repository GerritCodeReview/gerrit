...
public class ListOfFilesUnchangedPredicate extends ApprovalPredicate {
  ...
  public boolean match(ApprovalContext ctx) {
    ...
    try {
      Map<String, FileDiffOutput> modifiedLastPatchSet =
          diffOperations.listModifiedFilesAgainstParent(
              ctx.changeNotes().getProjectName(), targetPatchSet.commitId(), parentNum);
      Map<String, FileDiffOutput> modifiedPriorToLastPatchSet =
          diffOperations.listModifiedFilesAgainstParent(
              ctx.changeNotes().getProjectName(), sourcePatchSet.commitId(), parentNum);
      return match(modifiedLastPatchSet, modifiedPriorToLastPatchSet);
  ...
  public boolean match(
    ...
    for (String file : modifiedFiles1.keySet()) {
      FileDiffOutput lastPatchSetFile = modifiedFiles1.get(file);
      FileDiffOutput priorToLastPatchSetFile = modifiedFiles2.get(file);
      if (priorToLastPatchSetFile == null) {
        return false;
      }
      if (!priorToLastPatchSetFile.changeType().equals(lastPatchSetFile.changeType())) {
        return false;
      }
    }
    return true;
