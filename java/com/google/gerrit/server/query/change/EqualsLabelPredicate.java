...
public class EqualsLabelPredicate extends ChangeIndexPredicate {
  ...
  protected boolean match(ChangeData cd, short value, Account.Id approver) {
    ...
    if (account != null && !account.equals(approver) && !account.equals(ChangeQueryBuilder.OWNER_ACCOUNT_ID)) {
      return false;
    }

    if(account != null && account.equals(ChangeQueryBuilder.OWNER_ACCOUNT_ID) && !cd.change().getOwner().equals(approver)) {
      return false;
    }
