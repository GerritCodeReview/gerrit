...
public class EqualsLabelPredicates {
  ...
  private static class Matcher {
    ...
    public boolean match(ChangeData cd) {
      ...
      if (account != null && account.equals(ChangeQueryBuilder.HUMAN_REVIEWERS_ACCOUNT_ID)) {
        ...
        logger.atFine().log(
            "change %s doesn't match because it misses matching approvals from some human"
                + " reviewers: %s",
    ...
    private boolean isMagicUser() {
      return account != null
          && (account.equals(ChangeQueryBuilder.OWNER_ACCOUNT_ID)
              ...
              || account.equals(ChangeQueryBuilder.HUMAN_REVIEWERS_ACCOUNT_ID));
    }
  }
