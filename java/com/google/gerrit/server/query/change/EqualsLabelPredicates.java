  ...
  private static class Matcher {
    ...
    public boolean match(ChangeData cd) {
      ...
      if (Integer.valueOf(0).equals(count)) {
        ...
        logger.atFine().log(
            "%s=%s doesn't match change %s because the count was specified as 0 which is not supported.",
      ...
      if (labelType == null) {
        logger.atFine().log(
            "%s=%s doesn't match change %s because the label is not defined by its project %s (label type = %s)",
      ...
      if (!hasVote && expVal == 0) {
        logger.atFine().log(
            "%s=%s matches change %s because the is no vote for label %s",
      ...
      if (count == null) {
        if (matchingVotes >= 1) {
          logger.atFine().log(
              "%s=%s matches change %s because there are %s matching votes (count was not specified, hence 1 or more votes are needed)",
              label, expVal, cd.change().getChangeId(), matchingVotes);
          return true;
        }
        logger.atFine().log(
            "%s=%s doesn't match change %s because there no matching votes (count was not specified, hence 1 or more votes are needed)",
            label, expVal, cd.change().getChangeId());
        return false;
      }
      ...
      if (matchingVotes == count) {
        logger.atFine().log(
            "%s=%s matches change %s because there are %s matching votes which matches the expected count %s",
            label, expVal, cd.change().getChangeId(), matchingVotes, count);
        return true;
      }
      logger.atFine().log(
          "%s=%s doesn't match change %s because there are %s matching votes which doesn't match the expected count %s",
          label, expVal, cd.change().getChangeId(), matchingVotes, count);
      return false;
    }
    ...
    private boolean match(ChangeData cd, PatchSetApproval psa) {
      ...
      if (account != null) {
        ...
        if (account.equals(ChangeQueryBuilder.NON_CONTRIBUTOR_ACCOUNT_ID)) {
          if ((cd.currentPatchSet().uploader().equals(approver)
            ...
            logger.atFine().log(
                "vote %s on change %s doesn't match since it is not from a contibutor (uploader: %s, committer: %s, author: %s)",
                psa,
      ...
      if (group != null && !reviewer.getEffectiveGroups().contains(group)) {
        logger.atFine().log(
            "vote %s on change %s doesn't match since the approver %s is not a member of the expected group %s",
