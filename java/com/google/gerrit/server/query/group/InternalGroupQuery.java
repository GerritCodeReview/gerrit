...
public class InternalGroupQuery extends InternalQuery<InternalGroup, InternalGroupQuery> {
  ...
  public Map<AccountGroup.UUID, ImmutableSet<AccountGroup.UUID>> bySubgroups(
    ...
    List<Predicate<InternalGroup>> predicates = new ArrayList<>();
    subgroupIds.stream().forEach(sId -> predicates.add(GroupPredicates.subgroup(sId)));
    // Be conservative: If we need to look up more than half of the maxTerms, query one by one
    // instead.
    List<InternalGroup> groups =
        predicates.size() < indexConfig.maxTerms() / 2
    ...
    Map<AccountGroup.UUID, Set<AccountGroup.UUID>> parents =
        Maps.newHashMapWithExpectedSize(groups.size());
    subgroupIds.stream().forEach(c -> parents.put(c, new HashSet<>()));
    for (InternalGroup parent : groups) {
      for (AccountGroup.UUID child : parent.getSubgroups()) {
        parents.get(child).add(parent.getGroupUUID());
      }
    }
