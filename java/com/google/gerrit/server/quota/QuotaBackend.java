 ...
 * DefaultQuotaBackend} which calls all plugins that implement {@link QuotaEnforcer}.
 *
 ...
 *
 * <pre>
 *   quotaBackend.currentUser().project(projectName).requestThrowing("/projects/create");
 *   quotaBackend.user(user).requestThrowing("/restapi/config/put");
 *   quotaBackend.currentUser().account(accountId).request("/restapi/accounts/emails/validate");
 *   quotaBackend.currentUser().project(projectName).request("/projects/git/upload", numBytesInPush);
 * </pre>
 *
...
public interface QuotaBackend {
  ...
  /**
   * @see #currentUser(). Use this method only if you can't guarantee that the request is for the
   *     current user (e.g. impersonation).
   */
  ...
  interface WithResource {
    ...
    default QuotaResponse.Aggregated request(String quotaGroup) {
      return request(quotaGroup, 1);
    }

    /**
     * Issues a single quota request for {@code 1} token and throws a {@link QuotaException} in case
     * any enforcer denied the request.
     */
    default void requestThrowing(String quotaGroup) throws QuotaException {
      requestThrowing(quotaGroup, 1);
    }

    /** Issues a single quota request for {@code numTokens} tokens. */
    QuotaResponse.Aggregated request(String quotaGroup, long numTokens);

    /**
     * Issues a single quota request for {@code numTokens} tokens and throws a {@link
     * QuotaException} in case any enforcer denied the request.
     */
    default void requestThrowing(String quotaGroup, long numTokens) throws QuotaException {
      QuotaResponse.Aggregated result = request(quotaGroup, numTokens);
      if (!result.isOk()) {
        throw new QuotaException(result.errorMessages());
      }
    }
     ...
     */
    QuotaResponse.Aggregated requestNoDeduction(String quotaGroup, long tokens);
  }
}
