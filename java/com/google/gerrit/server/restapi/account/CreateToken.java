 ...
 *
 * <p>This REST endpoint handles {@code PUT /accounts/<account-identifier>/token} requests.
 *
...
public class CreateToken
    implements RestCollectionCreateView<AccountResource, AccountResource.Token, TokenInput> {
  ...
  private final VersionedAuthorizationTokens.Accessor tokensAccessor;
  ...
  CreateToken(
      ...
      VersionedAuthorizationTokens.Accessor tokensAccessor) {
    this.self = self;
  ...
  @Override
  public Response<String> apply(AccountResource rsrc, IdString id, TokenInput input)
    ...
    input.token = Strings.emptyToNull(input.token);
  ...
  public Response<String> apply(IdentifiedUser user, String id, String newToken)
      throws IOException, ConfigInvalidException, TokenConflictException {
    tokensAccessor.addPlainToken(user.getAccountId(), id, newToken);
    tokenCache.evict(user.getAccountId());
    try {
      emailFactories
          .createOutgoingEmail(
              PASSWORD_UPDATED,
    ...
    return Strings.isNullOrEmpty(newToken) ? Response.none() : Response.ok(newToken);
  }
