...
public class DeleteAccount implements RestModifyView<AccountResource, Input> {
  ...
  private final Provider<PublicKeyStoreUtil> gpgPublicKeyStoreUtilProvider;
  ...
  public DeleteAccount(
      ...
      Provider<PublicKeyStoreUtil> gpgPublicKeyStoreUtilProvider) {
    this.self = self;
    ...
    this.gpgPublicKeyStoreUtilProvider = gpgPublicKeyStoreUtilProvider;
  }
  ...
  public Response<?> apply(AccountResource rsrc, Input unusedInput)
    ...
    try {
      ...
      accountsUpdateProvider.get().delete("Deleting user", user.getAccountId(), user.state());
    } catch (Exception e) {
  ...
  private void deleteGpgKeys(IdentifiedUser user) {
    if (gpgPublicKeyStoreUtilProvider == null || gpgPublicKeyStoreUtilProvider.get() == null) {
      return;
    }
    PersonIdent committer = serverIdent.get();
    PersonIdent author = user.newCommitterIdent(committer);
    try {
      PublicKeyStoreUtil storeUtil = gpgPublicKeyStoreUtilProvider.get();
      List<RefUpdate.Result> deletedKeyResults =
          storeUtil.deletePgpKeys(
              storeUtil.listGpgKeysForUser(user.getAccountId()), committer, author);
      for (RefUpdate.Result saveResult : deletedKeyResults) {
        if (saveResult != RefUpdate.Result.NO_CHANGE
          ...
          throw new StorageException(String.format("Failed to delete GPG key: %s", saveResult));
        }
      }
    } catch (Exception e) {
      throw new StorageException("Failed to delete GPG keys.", e);
    }
  }
  ...
  private void deleteChangeEdits(Account.Id accountId) throws IOException {
    ...
    List<ChangeData> changesWithEdits = queryProvider.get().byEditor(accountId);
