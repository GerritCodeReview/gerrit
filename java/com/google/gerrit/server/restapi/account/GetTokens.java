...
import com.google.gerrit.server.account.AuthTokenCacheEntry;
...
public class GetTokens implements RestReadView<AccountResource> {
  ...
  @Inject
  GetTokens(
      Provider<CurrentUser> self, PermissionBackend permissionBackend, AuthTokenCache tokenCache) {
    this.self = self;
  ...
  @Override
  public Response<List<TokenInfo>> apply(AccountResource rsrc)
  ...
  public List<TokenInfo> apply(IdentifiedUser user) {
    ...
    for (AuthTokenCacheEntry token : tokenCache.get(user.getAccountId())) {
      tokenInfos.add(newTokenInfo(token));
    }
    return tokenInfos;
  }

  public static TokenInfo newTokenInfo(AuthTokenCacheEntry token) {
    TokenInfo info = new TokenInfo();
    info.id = token.getToken().id();
    return info;
  }
}
