...
import com.google.gerrit.extensions.api.accounts.TokenInfo;
...
public class GetTokens implements RestReadView<AccountResource> {
  ...
  @Inject
  GetTokens(
      Provider<CurrentUser> self, PermissionBackend permissionBackend, TokenCache tokenCache) {
    this.self = self;
  ...
  public List<TokenInfo> apply(IdentifiedUser user) {
    ...
    for (TokenCacheEntry token : tokenCache.get(user.getAccountId())) {
      tokenInfos.add(newTokenInfo(token));
    }
    return tokenInfos;
  }

  public static TokenInfo newTokenInfo(TokenCacheEntry token) {
