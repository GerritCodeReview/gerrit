...
import static com.google.gerrit.server.account.AuthTokenCacheImpl.LEGACY_ID;
...
public class PutHttpPassword implements RestModifyView<AccountResource, HttpPasswordInput> {
  ...
  public Response<String> apply(AccountResource rsrc, HttpPasswordInput input)
    ...
    if (tokenAccessor.getToken(rsrc.getUser().getAccountId(), LEGACY_ID).isPresent()) {
      ...
      tokenCache.evict(rsrc.getUser().getAccountId());
      if (isDeleteOp) {
        try {
          emailFactories
              .createOutgoingEmail(
    ...
    return Response.created(
        putToken.apply(rsrc, IdString.fromDecoded(DEFAULT_ID), tokenInput).value().token);
  }
}
