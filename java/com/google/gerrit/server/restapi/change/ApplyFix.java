...
@Singleton
public class ApplyFix implements RestModifyView {
  // This should never be called, just put for syntactic correctness.
  @Override
  public Response<?> apply(RestResource resource, Object input)
      throws AuthException, BadRequestException, ResourceConflictException, Exception {
    return null;
  }

  public static class InputNothing implements RestModifyView<FixResource, Input> {
  ...
  public static class InputDirectFix implements RestModifyView<RevisionResource, DirectFixInput> {
    private final GitRepositoryManager gitRepositoryManager;
    private final FixReplacementInterpreter fixReplacementInterpreter;
    private final ChangeEditModifier changeEditModifier;
    private final ChangeEditJson changeEditJson;
    ...
    public Response<EditInfo> apply(
      ...
      try (Repository repository = gitRepositoryManager.openRepository(project)) {
        for (FixReplacementInfo fixI : directFixInput.getFixReplacementInfos()) {
          Range temp_range = new Range(fixI.range);
          FixReplacement fixR = new FixReplacement(fixI.path, temp_range, fixI.replacement);
          fixReplacementList.add(fixR);
        }
        CommitModification commitModification =
            fixReplacementInterpreter.toCommitModification(
