...
public class ApplyPatch implements RestModifyView<ChangeResource, ApplyPatchPatchSetInput> {
  ...
  private String buildCommitMessage(
    ...
    if (!Strings.isNullOrEmpty(input.commitMessage)) {
      messageWithNoFooters = input.commitMessage;
      if (input.commitMessage.contains(FooterConstants.CHANGE_ID.getName())) {
      ...
      messageWithNoFooters = removeFooters(latestPatchset.getFullMessage(), footerLines);
    }
    String commitMessage =
