...
import modules.jgit.org.eclipse.jgit.src.org.eclipse.jgit.patch.PatchApplier;
...
public class ApplyPatch
  ...
  public Response<ChangeInfo> apply(ChangeResource rsrc, ApplyPatchInput input)
    ...
    try (Repository repo = gitManager.openRepository(project);
      ...
      try {
        ...
        PatchApplier.ApplyPatchResult applyResult = applier.applyPatch(patchStream);
        treeId = applyResult.appliedTree;
      ...
      try (BatchUpdate bu = batchUpdateFactory.create(project, identifiedUser, TimeUtil.now())) {
        ...
        changeId = insertPatchSet(bu, repo, patchSetInserterFactory, destChange.notes(), commit);
        bu.execute();
        // DO NOT SUBMIT - where was that copied from?
        ChangeInfo changeInfo = json.noOptions().format(rsrc.getProject(), changeId);
        changeInfo.containsGitConflicts =
            !commit.getFilesWithGitConflicts().isEmpty() ? true : null;
        // DO NOT SUBMIT - should handle conflicts otherwise. See CherryPickChange::cherryPick
  ...
  private static Change.Id insertPatchSet(
      BatchUpdate bu,
      Repository git,
      PatchSetInserter.Factory patchSetInserterFactory,
