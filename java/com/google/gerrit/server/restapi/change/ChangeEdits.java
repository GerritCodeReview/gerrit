  ...
  public static class Put implements RestModifyView<ChangeEditResource, Put.Input> {
    ...
    public Response<?> apply(ChangeEditResource rsrc, Put.Input input)
      ...
      if (input.upload != null && input.upload == true) {
       // return apply(rsrc.getChangeResource(), rsrc.getPath(), RawInputUtil.create(Base64.decode(input.contents.getBytes(), 0, input.contents.getBytes().length)));

        return apply(rsrc.getChangeResource(), rsrc.getPath(), RawInputUtil.create(Base64.decode(input.contents)));
      }
