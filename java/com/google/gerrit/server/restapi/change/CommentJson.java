...
public class CommentJson {
  ...
  private abstract class BaseCommentFormatter<F extends Comment, T extends CommentInfo> {
    ...
    protected void addCommentContext(List<T> allComments) {
      ...
      ImmutableMap<CommentContextKey, ContextLines> allContext = commentContextCache.getAll(keys);
      for (T c : allComments) {
        c.contextLines = toContextLineInfoList(allContext.get(createCommentContextKey(c)));
      }
    }

    protected List<ContextLineInfo> toContextLineInfoList(ContextLines contextLines) {
      List<ContextLineInfo> result = new ArrayList<>();
      for (Map.Entry<Integer, String> e : contextLines.lines().entrySet()) {
    ...
    protected CommentContextKey createCommentContextKey(T r) {
      return CommentContextKey.create(project, changeId, r.id, r.path, r.patchSet);
    }
