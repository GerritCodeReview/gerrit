...
public class CommentJson {
  ...
  private abstract class BaseCommentFormatter<F extends Comment, T extends CommentInfo> {
    ...
    protected void addCommentContext(List<T> allComments) {
      ...
      ImmutableMap<CommentContextKey, ImmutableList<ContextLine>> allContext =
          commentContextCache.getAll(project, changeId, keys);
      for (T c : allComments) {
        c.contextLines =
            allContext.get(createCommentContextKey(c)).stream()
                .map(ctx -> toContextLine(ctx))
                .collect(toList());
      }
    }

    protected CommentContextKey createCommentContextKey(T r) {
      return CommentContextKey.create(r.id, r.path, r.patchSet);
    }
