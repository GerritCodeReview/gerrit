...
public class DeleteChange extends RetryingRestModifyView<ChangeResource, Input, Object>
  ...
  protected Response<Object> applyImpl(
    ...
    try (BatchUpdate bu =
      ...
      bu.addOp(id, new DeleteChangeEditOp(editUtil, edit));
      bu.execute();
    }
  ...
  private static class DeleteChangeEditOp implements BatchUpdateOp {

    private final ChangeEditUtil editUtil;
    private final Optional<ChangeEdit> changeEdit;

    DeleteChangeEditOp(ChangeEditUtil editUtil, Optional<ChangeEdit> changeEdit) {
      this.editUtil = editUtil;
    ...
    public boolean updateChange(ChangeContext ctx) throws IOException {
      if (changeEdit.isPresent()) {
        editUtil.delete(changeEdit.get());
      }
      return true;
    }
  }
