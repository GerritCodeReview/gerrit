...
public class DeleteChangeMessage
  ...
  public Response<ChangeMessageInfo> applyImpl(
    ...
    ChangeMessage target = null;
    for (ChangeMessage message : messages) {
      if (message.getKey().get().equals(input.uuid)) {
        target = message;
        break;
      }
    }
    ...
    if (target == null) {
      throw new ResourceNotFoundException(String.format("change message %s not found", input.uuid));
    }
    ...
    target.setMessage(newChangeMessage);
    ...
    ChangeMessageInfo info = changeMessagesUtil.createChangeMessageInfo(target, accountLoader);
    accountLoader.fill();
