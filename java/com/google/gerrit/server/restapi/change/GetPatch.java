...
public class GetPatch implements RestReadView<RevisionResource> {
   ...
   *
   * <p>The reason these legacy types are here is to force paleolithic browsers like IE6 to not do cross site scripting.
   * We have since invented X-Content-Type-Options: nosniff, which every browser released since IE8 supports,
   * making this madness unnecessary in the modern era, thus the raw mode being available.
   *
        ...
        switch (outputType) {
          ...
          case RAW:
            bin
                ...
                .setAttachmentName(download ? fileName(rw, commit) + ".patch" : null);
        }
