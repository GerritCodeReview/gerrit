...
public class GetPortedComments implements RestReadView<FileResource> {
  ...
  @Override
  public Response<List<CommentInfo>> apply(FileResource fileResource)
    ...
    ImmutableList<HumanComment> portedComments =
        commentPorter.portComments(
            ...
            allComments,
            ImmutableList.of(
                new UnresolvedCommentFilter(),
                new FilePathFilter(fileResource.getPatchKey().fileName())));
    return Response.ok(format(portedComments));
  }

  private List<CommentInfo> format(List<HumanComment> comments) throws PermissionBackendException {
    return commentJson
        .get()
        .setFillAccounts(true)
        .setFillPatchSet(true)
        .newHumanCommentFormatter()
        .formatAsList(comments);
  }

  static class FilePathFilter implements HumanCommentFilter {
    ...
    public ImmutableList<HumanComment> filter(ImmutableList<HumanComment> comments) {
      return comments.stream()
          .filter(c -> c.key.filename.equals(fileName))
