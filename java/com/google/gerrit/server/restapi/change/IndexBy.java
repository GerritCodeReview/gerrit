...
import com.google.gerrit.extensions.api.changes.ChangesQueryInput;
...
public class IndexBy implements RestModifyView<TopLevelResource, ChangesQueryInput> {
  ...
  public Response.Accepted apply(TopLevelResource resource, ChangesQueryInput input)
    ...
    do {
      ...
      queryChanges.setLimit(500);
      queryChanges.setStart(changes.size());
      changes.addAll((List<ChangeInfo>) queryChanges.apply(resource));
      more = !changes.isEmpty() && Iterables.getLast(changes)._moreChanges != null;
    } while (more);
    @SuppressWarnings("unused")
    ListenableFuture<?> unused = executor.submit(indexChanges(changes));
    return Response.accepted("Changes submitted for indexing");
  }

  private Runnable indexChanges(List<ChangeInfo> changes) {
    return () -> {
      for (ChangeInfo changeInfo : changes) {
        ...
        try (ManualRequestContext ctx = requestContext.open()) {
          ...
          if (changeNotes == null) {
            ...
            log.warn(
                "Change {} was not found in the ReviewDb/NoteDb and was removed from the index",
                changeId);
          } else {
