...
public class ListChangeComments extends ListChangeDrafts {
    ...
    for (CommentInfo comment : comments) {
      while (cmItr < changeMessages.size()
          && (comment.updated.after(changeMessages.get(cmItr).getWrittenOn())
              || isAutoGenerated(changeMessages.get(cmItr)))) {
        cmItr += 1;
      }
  ...
  private boolean isAutoGenerated(ChangeMessage message) {
    return message.getTag() != null
        && message.getTag().startsWith(ChangeMessagesUtil.AUTOGENERATED_TAG_PREFIX);
  }
}
