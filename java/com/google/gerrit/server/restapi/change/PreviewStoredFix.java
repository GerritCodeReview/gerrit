...
public class PreviewStoredFix implements RestReadView<FixResource> {
  ...
  public Response<Map<String, DiffInfo>> apply(FixResource resource)
    ...
    Map<String, DiffInfo> result =
        previewFixForAllFiles(repoManager, patchSet, notes, state, fixReplacementsPerFilePath);
    return Response.ok(result);
  }

  Map<String, DiffInfo> previewFixForAllFiles(
      GitRepositoryManager repoManager,
      PatchSet patchSet,
      ChangeNotes notes,
      ProjectState state,
  ...
  DiffInfo previewStoredFixForSingleFile(
      Repository git,
      PatchSet patchSet,
      ProjectState state,
      ChangeNotes notes,
