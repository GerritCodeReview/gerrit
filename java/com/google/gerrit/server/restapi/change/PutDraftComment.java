...
public class PutDraftComment implements RestModifyView<DraftCommentResource, DraftInput> {
  ...
  public Response<CommentInfo> apply(DraftCommentResource rsrc, DraftInput in)
    ...
    ChangeNotes changeNotes = changeNotesFactory.createChecked(rsrc.getChange());
    if (in == null || in.message == null || in.message.trim().isEmpty()) {
      ...
      throw new BadRequestException("range endLine must be on the same line as the comment");
        ...
        && !commentsUtil.getPublishedHumanComment(changeNotes, in.inReplyTo).isPresent()
        && !commentsUtil.getRobotComment(changeNotes, in.inReplyTo).isPresent()) {
