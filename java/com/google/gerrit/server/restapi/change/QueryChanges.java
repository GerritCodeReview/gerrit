...
public class QueryChanges implements RestReadView<TopLevelResource>, DynamicOptions.BeanReceiver {
  ...
  private ChangeQueryProcessor newConfiguredProcessor()
      throws BadRequestException, QueryParseException, PermissionBackendException {
    ChangeQueryProcessor qp = queryProcessorProvider.get();
    if (qp.isDisabled()) {
      throw new QueryParseException("query disabled");
    }
    qp.setUserProvidedLimit(limit != null ? limit : 0, /* applyDefaultLimit */ true);
    if (start != null) {
      if (start < 0) {
    ...
    if (noLimit != null && !AnonymousUser.class.isAssignableFrom(userProvider.get().getClass())) {
      qp.setNoLimit(noLimit);
    }
    if (allowIncompleteResults != null) {
      qp.setAllowIncompleteResults(allowIncompleteResults);
    }
    dynamicBeans.forEach((p, b) -> qp.setDynamicBean(p, b));
    return qp;
  }
  ...
  private List<List<ChangeInfo>> queryAndFilter(List<String> q, int depth)
    ...
    ChangeQueryProcessor qp = newConfiguredProcessor();
    List<QueryResult<ChangeData>> results = qp.query(qb.parse(q));
    List<List<ChangeInfo>> res = json.create(options, qp.getInfosFactory()).format(results);
