...
public class QueryChanges implements RestReadView<TopLevelResource>, DynamicOptions.BeanReceiver {
   ...
   * change index anymore. Back-filling is gated with the {@link
   * ListChangesOption#BACKFILL_STARRED_FIELD}.
   */
  private List<List<ChangeInfo>> backFillStarFields(List<List<ChangeInfo>> changeInfos) {
    if (!userProvider.get().isIdentifiedUser()) {
      // un-identified users don't have stars
      return changeInfos;
    }
    if (!options.contains(ListChangesOption.BACKFILL_STARRED_FIELD)) {
      return changeInfos;
    }
    try {
