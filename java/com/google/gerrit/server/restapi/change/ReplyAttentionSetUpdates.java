...
public class ReplyAttentionSetUpdates {
  ...
  private void addAllAuthorsOfCommentThreads(
        ...
        projectCache
            ...
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        String.format(
                            "No Code-Review label for project \"%s\"",
                            changeNotes.getChange().getProject())))
            .getMax();

    ImmutableSet<Account.Id> maxCrApprovers =
        changeNotes.getApprovals().all().get(changeNotes.getCurrentPatchSet().id()).stream()
            .filter(
                a -> a.label().equals(LabelId.CODE_REVIEW) && a.value() == maxCRValue.getValue())
            .map(a -> a.accountId())
    ...
    for (CommentThread<HumanComment> thread : repliedToCommentThreads) {
      ...
      boolean ignoreVoteCheck = thread.unresolved() && isOwnerOrUploader;
