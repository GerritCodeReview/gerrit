...
public class RevertSubmission
  ...
  public Response<List<ChangeInfo>> apply(ChangeResource changeResource, RevertInput input)
      throws Exception {
    ProjectResource projectResource = new ProjectResource(projectCache.get(changeResource.getProject()),changeResource.getUser());
    ...
    for (int i =0; i < sortedPatchSets.size(); i ++) {
      ChangeResource change = getChangeResource(sortedPatchSets.get(i).data().getId());
      ChangeInfo changeInfo = revert.apply(change, input).value();
      // input.base_commit = getChangeResource(Change.id(changeInfo._number)).getNotes().getCurrentPatchSet().commitId().getName();
      input.base_change = changeInfo.changeId;
      //TODO: make reverts on top of each other somehow. Also make sure different repositories don't stack reverts.
      results.add(changeInfo);


      // BranchInput branchInput = new BranchInput();
      // branchInput.revision = getChangeResource(Change.id(changeInfo._number)).getNotes().getCurrentPatchSet().commitId().getName();
      // createBranch.apply(projectResource, IdString.fromDecoded("revert_branch_" + i), branchInput); //x: change name of branch
      // if(i > 0) { //x: save latest branch.
      //   CherryPickInput cherryPickInput = new CherryPickInput();
      //   cherryPickInput.destination = "revert_branch_1";
      //   cherryPickInput.message = "cherry pick bla bla\n" + "Change-Id: " + changeInfo.changeId +"\n";
      //   RevisionResource revisionResource = new RevisionResource(change,
      //       change.getNotes().getCurrentPatchSet());
      //   cherryPick.apply(revisionResource, cherryPickInput);
      // }

    }

    for(ChangeInfo result : results){
      result.topic = "revert-topic";    //TODO: generate something random, or name it as OLD_TOPIC-revert
      TopicInput topicInput = new TopicInput();
      topicInput.topic = result.topic;
      String x = putTopic.apply(getChangeResource(Change.id(result._number)),topicInput).value();
    }
