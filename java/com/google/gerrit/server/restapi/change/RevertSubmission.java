...
public class RevertSubmission
      ...
      while (sortedChangesInProject.hasNext()) {
        ...
        cherryPickInput.base =
            cherryPickInput.base != null
                ? cherryPickInput.base
                : getBase(changeNotes, commitIdsInProjectAndBranch);
   ...
   * parents only commits that are part of that submission 2. A commit that is part of the
   * submission. If neither of those are true, it just continues the search by going to the parents.
   *
   * <p>If 1 is true, it will return that commit. If 2 is true, it will return the commit that
   * WalkSorter has decided that it should be the first commit reverted (e.g changeNotes, which is
   * also the commit that is the first in the topological sorting).
   *
    ...
    try (Repository git = repoManager.openRepository(changeNotes.getProjectName());
      ...
      while (!commitsToSearch.isEmpty()) {
        ...
        if (revCommit.getParentCount() > 1
            && commitIds.containsAll(Arrays.asList(revCommit.getParents()))) {
