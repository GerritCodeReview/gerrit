...
public class RevertSubmission
  ...
  public Response<List<ChangeInfo>> apply(ChangeResource changeResource, RevertInput input)
    ...
    ProjectResource projectResource =
        new ProjectResource(
            projectCache.get(changeResource.getProject()), changeResource.getUser());
    ...
    for (ChangeData changeData : changeDatas) {
      ...
      if (!change.isMerged()) {
        throw new ResourceConflictException(
            String.format(
                "ChangeId %s is %s", changeData.getId().toString(), ChangeUtil.status(change)));
      }
    ...
    PatchSet startPatchSet =
        psUtil.get(changeResource.getNotes(), changeResource.getChange().currentPatchSetId());
    ...
    for (int i = 0; i < sortedPatchSets.size(); i++) {
      ...
      input.revertParent =
          getChangeResource(Change.id(changeInfo._number))
              ...
              .commitId()
              .getName();
      // TODO: make reverts on top of each other somehow. Also make sure different repositories
      // don't stack reverts.
    ...
    for (ChangeInfo result : results) {
      result.topic =
          "revert-topic"; // TODO: generate something random, or name it as OLD_TOPIC-revert
      TopicInput topicInput = new TopicInput();
      topicInput.topic = "revert-topic";
      String ignored =
          putTopic.apply(getChangeResource(Change.id(result._number)), topicInput).value();
    }
