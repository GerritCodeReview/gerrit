...
public class Submit
    ...
    for (BranchNameKey branch : cbb.keySet()) {
      ...
      try (Repository repo = repoManager.openRepository(branch.project());
        ...
        String mergeStrategy =
            mergeUtilFactory
                .create(
                    projectCache
                        .get(branch.project())
                        .orElseThrow(illegalState(branch.project())))
                .mergeStrategyName();
        ...
        for (ChangeData change : targetBranch) {
          ...
          if (dependsOnOutdatedPatchSet(commit, currentCommitsToChange, cs)
  ...
  private static boolean dependsOnOutdatedPatchSet(
      RevCommit commit, Map<ObjectId, ChangeData> currentCommitsToChange, ChangeSet cs) {
    for (RevCommit parent : commit.getParents()) {
      ObjectId parentId = parent.getId();
      // If the parent is the current patchset of a change in the changeset, it is up to date.
