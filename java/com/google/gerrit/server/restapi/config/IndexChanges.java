    ...
    for (ChangeIdentifier changeInfo : changeIds) {
      ...
      if (changes.size() > 1) {
        throw new BadRequestException(
            "Ambiguous change ID " + changeInfo.changeId() + " in project " + changeInfo.project());
      }
      if (!changes.isEmpty() && input.deleteMissing) {
        try {
          ...
          // Delete using virtualId: for imported changes the index document is keyed
          // by virtualId (≠ legacyId), so deleting by legacyId would silently miss it.
          logger.atWarning().log("Deleting change %s from index", cd.virtualId());
