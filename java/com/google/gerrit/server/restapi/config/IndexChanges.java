...
public class IndexChanges implements RestModifyView<ConfigResource, Input> {
  ...
  public Response<String> apply(ConfigResource resource, Input input) throws BadRequestException {
    ...
    List<ChangeInfo> changeIds = new ArrayList<>();
    for (String id : input.changes) {
      changeIds.add(getChangeIdentifier(id));
    }

    for (ChangeInfo changeInfo : changeIds) {
      List<ChangeData> changes = queryProvider.get().byChangeNumber(changeInfo.changeId());
  ...
  record ChangeInfo(Project.NameKey project, Change.Id changeId) {}

  ChangeInfo getChangeIdentifier(String id) throws BadRequestException {
    ...
    return new ChangeInfo(Project.nameKey(id.substring(0, tilde)), changeId);
  }
}
