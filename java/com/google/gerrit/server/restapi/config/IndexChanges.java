    ...
    for (String id : input.changes) {
      ...
      if (notes.isEmpty()) {
        ...
        if (input.deleteMissing) {
          ...
          Optional<Change.Id> changeId = Change.Id.tryParse(id.substring(tilde + 1));
          if (changeId.isEmpty()) {
            logger.atSevere().log(
                String.format("When deleteMissing is true, Change ID must be in form project~changeNumber,skipping indexing for change %s", id));
            continue;
          }
