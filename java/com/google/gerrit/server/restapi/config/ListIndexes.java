...
public class ListIndexes implements RestReadView<ConfigResource> {
  ...
  private Map<String, IndexInfo> getIndexInfos() {
    ...
    indexInfos.put(
        lowercase(IndexType.ACCOUNTS),
        IndexInfo.fromIndexCollection(IndexType.ACCOUNTS, accountIndexes));
    indexInfos.put(
        lowercase(IndexType.CHANGES),
        IndexInfo.fromIndexCollection(IndexType.CHANGES, changeIndexes));
    indexInfos.put(
        lowercase(IndexType.GROUPS), IndexInfo.fromIndexCollection(IndexType.GROUPS, groupIndexes));
    indexInfos.put(
        lowercase(IndexType.PROJECTS),
        IndexInfo.fromIndexCollection(IndexType.PROJECTS, projectIndexes));
    return indexInfos;
  }

  private String lowercase(IndexType indexType) {
