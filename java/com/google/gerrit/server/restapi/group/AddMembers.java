  ...
  Account findAccount(String nameOrEmailOrId)
    ...
    try {
      ...
      if (accountResolver.find(nameOrEmailOrId) == null) {
        // the account does not exist, try to create it automatically
        Optional<Account> autoCreatedAccount =
            AutoAccountCreator.createAccount(autoAccountCreators, nameOrEmailOrId);
        if (autoCreatedAccount.isPresent()) {
          return autoCreatedAccount.get();
        }
      }
      throw e;
    }
