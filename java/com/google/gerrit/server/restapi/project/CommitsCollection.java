...
public class CommitsCollection implements ChildCollection<ProjectResource, CommitResource> {
  ...
  public boolean canRead(ProjectState state, Repository repo, RevCommit commit) throws IOException {
    ...
    // Try to see if the commit is visible from anything that is not a change ref. We have already
    // checked change refs using the change index, so we can spare any further checks.
    List<Ref> refs =
        repo.getRefDatabase()
            .getRefs()
            .stream()
