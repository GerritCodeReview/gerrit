...
public class CommitsIncludedInRefs implements RestReadView<ProjectResource> {
  ...
  @Override
  public Response<Map<String, Collection<String>>> apply(ProjectResource resource)
    ...
    return Response.ok(
        includedInRefs.apply(
            resource.getNameKey(),
            commits.stream().collect(Collectors.toSet()),
            refs.stream().collect(Collectors.toSet())));
  }
}
