...
import com.google.gerrit.server.project.SubmitRequirementExpressionValidator;
...
public class CreateSubmitRequirement
  ...
  public SubmitRequirement createSubmitRequirement(
    ...
    if (Strings.isNullOrEmpty(input.submittableIf)) {
      throw new BadRequestException("submittability_expression is required");
    }
    if (input.canOverrideInChildProjects == null) {
      input.canOverrideInChildProjects = false;
    }
    SubmitRequirement submitRequirement =
        SubmitRequirement.builder()
            .setName(name)
            .setDescription(Optional.ofNullable(input.description))
            .setApplicabilityExpression(SubmitRequirementExpression.of(input.applicableIf))
            .setSubmittabilityExpression(SubmitRequirementExpression.create(input.submittableIf))
            .setOverrideExpression(SubmitRequirementExpression.of(input.overrideIf))
            .setAllowOverrideInChildProjects(input.canOverrideInChildProjects)
            .build();
