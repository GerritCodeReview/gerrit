...
public class DeleteChanges implements RestModifyView<ProjectResource, DeleteChangesInput> {
  ...
  public Response<?> apply(ProjectResource resource, DeleteChangesInput input)
    ...
    ListMultimap<String, List<String>> responseBody = ArrayListMultimap.create();
    ...
    try (BatchUpdate bu =
      ...
      for (String change : input.changes) {
        ...
        if (cn.isEmpty()) {
          responseBody.put(String.valueOf(FAILURE), List.of(change));
        } else if (cn.size() > 1) {
          responseBody.put(String.valueOf(NOT_UNIQUE), List.of(change));
        } else {
      ...
      for (String change : deletableChange) {
        responseBody.put(String.valueOf(SUCCESS), List.of(change));
      }
  ...
  //    switch (status) {
  //      case SUCCESS:
  //        responseMsg.computeIfAbsent(String.valueOf(SUCCESS), k -> new
  // ArrayList<>()).add(change);
  //        break;
  //      case FAILURE:
  //        responseMsg.computeIfAbsent(String.valueOf(FAILURE), k -> new
  // ArrayList<>()).add(change);
  //        break;
  //      case NOT_UNIQUE:
  //        responseMsg.computeIfAbsent(String.valueOf(NOT_UNIQUE), k -> new
  // ArrayList<>()).add(change);
  //        break;
  //    }
  //  }
