...
/**
 * REST endpoint to retrieve submit requirement templates from the requested project's hierarchy.
 *
 * <p>Templates are stored as {@code [submit-requirement-template "name"]} sections in {@code
 * project.config}. The effective list is assembled from the requested project and its parents,
 * where child projects may override parent templates if the parent template allows overriding in
 * child projects.
 *
