...
public class PutConfig implements RestModifyView<ProjectResource, ConfigInput> {
    ...
    try (MetaDataUpdate md = metaDataUpdateFactory.get().create(projectName)) {
      ...
      if (input.notifyConfigsAdditions != null) {
        for (NotifyConfigInput notifyConfig : input.notifyConfigsAdditions) {
          if (notifyConfig.addresses.stream()
              .anyMatch(address -> outgoingEmailValidator.isValid(address) == false)) {
          ...
          if (notifyConfig.groupsIds.isEmpty() && notifyConfig.addresses.isEmpty()) {
  ...
  private void assertValidAddress(String address) throws OutOfScopeException {
    if (!outgoingEmailValidator.isValid(address)) {
      throw new OutOfScopeException("is an invalid email address");
    }
  }
