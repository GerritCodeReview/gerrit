  ...
  public ConfigInfo apply(ProjectState projectState, ConfigInput input)
    ...
    try (MetaDataUpdate md = metaDataUpdateFactory.get().create(projectName)) {
      ...
      if (input.notifyConfigUpdates != null) {
        if (input.notifyConfigUpdates.notifyConfigRemovals != null) {
          Collection<String> notifyConfigsNames = projectConfig.getNotifyConfigNames();
          for (String notifyConfigsName : input.notifyConfigUpdates.notifyConfigRemovals) {
          ...
          input.notifyConfigUpdates.notifyConfigRemovals.stream()
              .forEach(projectConfig::removeNotifyConfig);
        }
        if (input.notifyConfigUpdates.notifyConfigAdditions != null) {
          for (NotifyConfigInput notifyConfig : input.notifyConfigUpdates.notifyConfigAdditions) {
