  ...
  public ConfigInfo apply(ProjectState projectState, ConfigInput input)
    ...
    try (MetaDataUpdate md = metaDataUpdateFactory.get().create(projectName)) {
      ...
      if (input.notifyConfigsRemovals != null) {
        Collection<String> notifyConfigsNames = projectConfig.getNotifyConfigsNames();
        for (String notifyConfigsName : input.notifyConfigsRemovals) {
          if (!notifyConfigsNames.contains(notifyConfigsName)) {
            throw new BadRequestException(
                String.format(
                    "NotifyConfig %s doesn't exist and could not be removed.", notifyConfigsName));
          }
        }
        input.notifyConfigsRemovals.stream().forEach(projectConfig::removeNotifyConfig);
      }
      if (input.notifyConfigsAdditions != null) {
        for (NotifyConfigInput notifyConfig : input.notifyConfigsAdditions) {
          if (notifyConfig.name == null || notifyConfig.name.equals("")) {
            throw new BadRequestException("Name of the Notify Config must not be empty");
          }
          if (projectConfig.getNotifyConfigsNames().contains(notifyConfig.name)) {
            throw new BadRequestException(
                String.format(
          ...
          if (notifyConfig.emails.stream()
            ...
            throw new BadRequestException(
                String.format("NotifyConfig %s had an invalid email", notifyConfig.name));
          }
          if (notifyConfig.filter != null && !notifyConfig.filter.equals("*")) {
            try {
              changeQueryBuilder.parse(
                  notifyConfig
                      .filter); // asserts that filter is legal, if not, throws QueryParseException.
            } catch (QueryParseException err) {
              throw new BadRequestException(
                  String.format(
                      "NotifyConfig %s has an invalid filter %s",
                      notifyConfig.name, notifyConfig.filter));
            }
          }
          if (notifyConfig.groupsIds.isEmpty() && notifyConfig.emails.isEmpty()) {
            throw new BadRequestException(
          ...
          for (String id : notifyConfig.groupsIds) {
            try {
