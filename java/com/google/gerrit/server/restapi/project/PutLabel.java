...
import com.google.gerrit.extensions.api.projects.PutLabelInput.LabelTypeInput;
...
public class PutLabel implements RestModifyView<ProjectResource, PutLabelInput> {
  ...
  private void updateLabelConfigs(
    ...
    try (MetaDataUpdate md = metaDataUpdateFactory.get().create(project)) {
      ...
      projectConfig.getLabelSections();
  ...
  private ImmutableMap<String, LabelType> updateLabelConfigs(
      Map<String, LabelType> labelSections, PutLabelInput putLabelInput) {
    ImmutableMap.Builder<String, LabelType> builder = new ImmutableMap.Builder<>();

    // Adds new labels.
    for (LabelTypeInput input : putLabelInput.adds) {}

    for (Map.Entry<String, LabelType> entry : labelSections.entrySet()) {
