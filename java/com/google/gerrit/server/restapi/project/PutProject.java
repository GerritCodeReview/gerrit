...
public class PutProject implements RestModifyView<ProjectResource, ProjectInput> {
  ...
  @Inject
  PutProject(PermissionBackend permissionBackend, CreateProject createProject) {
  ...
  public Response<?> apply(ProjectResource projectResource, ProjectInput input)
    ...
    if (input.initOnly) {
      // only project owners are allowed to rerun the project init as it overrides the existing
      // config
      permissionBackend
          .currentUser()
