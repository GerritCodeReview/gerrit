...
public class RequireNonAuthorApprovalRule implements SubmitRule {
  ...
  @Inject
  RequireNonAuthorApprovalRule(PatchSetUtil patchSetUtil, Emails emails) {
  ...
  public Collection<SubmitRecord> evaluate(ChangeData cd, SubmitRuleOptions options) {
    ...
    boolean shouldIgnoreAuthorApproval =
        labelTypes.stream().anyMatch(l -> l.ignoreAuthorSelfApproval());
    if (!shouldIgnoreAuthorApproval) {
    ...
    try {
      PersonIdent authorIdent =
          patchSetUtil.getRevCommit(cd.project(), cd.currentPatchSet()).getAuthorIdent();
      Set<Account.Id> authors = emails.getAccountForExternal(authorIdent.getEmailAddress());
      if (authors.size() != 1) {
        logger.atWarning().log(
            E_UNABLE_TO_FETCH_CHANGE_AUTHOR
                + " Email "
                + authorIdent.getEmailAddress()
                + " not uniquely associated with an account.");
        return singletonRuleError(E_UNABLE_TO_FETCH_CHANGE_AUTHOR);
      }
      author = Iterables.getOnlyElement(authors);
    } catch (IOException | OrmException e) {
      logger.atWarning().withCause(e).log(E_UNABLE_TO_FETCH_CHANGE_AUTHOR);
      return singletonRuleError(E_UNABLE_TO_FETCH_CHANGE_AUTHOR);
    }
    ...
    for (LabelType t : labelTypes) {
      ...
      SubmitRecord.Label ignoreAuthorCheckResult =
          labelFunction.check(t, getNonAuthorApprovalsForLabel(approvals, t, author));
      SubmitRecord.Label authorCheckResult =
          labelFunction.check(t, getAllApprovalsForLabel(approvals, t));

      if (labelCheckPassed(authorCheckResult) && !labelCheckPassed(ignoreAuthorCheckResult)) {
