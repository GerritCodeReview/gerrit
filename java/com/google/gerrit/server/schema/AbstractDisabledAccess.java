...
abstract class AbstractDisabledAccess<T, K extends Key<?>> implements Access<T, K> {
  ...
  public final void beginTransaction(K key) {
    // Keep track of when we've started a transaction so that we can avoid
    // calling commit/rollback
    // on the underlying ReviewDb. This is just a simple arm's-length approach,
    // and may produce
    ...
    // These kinds of things are already misuses of ReviewDb, and shouldn't be
    // happening in
    // current code anyway.
