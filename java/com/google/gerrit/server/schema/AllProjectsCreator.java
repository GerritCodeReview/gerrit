  ...
  private void initDefaultSubmitRequirements(ProjectConfig config) {
    config.upsertSubmitRequirement(
        SubmitRequirement.builder()
            ...
            .build());
    config.upsertSubmitRequirement(SubmitRequirement.builder()
        .setName("Code-Review")
        .setDescription(
            Optional.of("Changes must have at least one MAX Code-Review vote and no MIN to be submittable."))
        .setSubmittabilityExpression(SubmitRequirementExpression.create("label:Code-Review=MAX AND -label:Code-Review=MIN"))
        .setAllowOverrideInChildProjects(false)
        .build());
  }
