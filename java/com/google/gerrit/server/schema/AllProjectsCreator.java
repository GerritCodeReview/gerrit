...
public class AllProjectsCreator {
  ...
  public void create(AllProjectsCreatorInput input)
  ...
  private void initAllProjects(Repository git, AllProjectsCreatorInput input)
      throws IOException, ConfigInvalidException, OrmException {
    ...
    try (MetaDataUpdate md =
      ...
      p.setDescription(
          input.projectDescription().isPresent()
              ? input.projectDescription().get()
              : "Access inherited by all other projects.");

      input.booleanProjectConfigs().ifPresent(e -> e.forEach(p::setBooleanConfig));
      ...
      GroupReference admin = input.administratorsGroup();
      GroupReference batch = input.batchUsersGroup();
