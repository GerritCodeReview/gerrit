...
public class AllProjectsCreator {
  ...
  private void initDefaultAclsForBatchUsers(
    ...
    Permission.Builder priority = capabilities.getPermission(GlobalCapability.PRIORITY);
    priority.add(rule(config, batchUsersGroup).setAction(Action.BATCH));

    Permission.Builder stream = capabilities.getPermission(GlobalCapability.STREAM_EVENTS);
  ...
  private void initDefaultAclsForAdmins(
    ...
    config.upsertAccessSection(
        ...
        meta -> {
          meta.getPermission(Permission.READ).setExclusiveGroup(true);
