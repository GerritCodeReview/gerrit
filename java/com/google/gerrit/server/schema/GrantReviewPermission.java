...
/**
 * This class adds the "review" permission to all hosts that call this method with the relevant
 * projectName. This class should be called with AllProjects as the project, by all hosts before
 * enabling the "review" permission.
 */
public class GrantReviewPermission {
  ...
  public void execute(Project.NameKey projectName) throws IOException, ConfigInvalidException {
    ...
    try (Repository repo = repoManager.openRepository(projectName)) {
      ...
      projectConfig.upsertAccessSection(
          AccessSection.ALL,
          all -> {
            Permission permissionOnRefsStar = all.build().getPermission(Permission.REVIEW);
            ...
            grant(projectConfig, all, Permission.REVIEW, registeredUsers);
          });

      md.getCommitBuilder().setAuthor(serverUser);
      md.getCommitBuilder().setCommitter(serverUser);
      md.setMessage("Add Review permission for all registered users\n");
