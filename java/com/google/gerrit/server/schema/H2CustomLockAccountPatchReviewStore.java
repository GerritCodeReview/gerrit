    ...
    return new Lock() {
      ...
      public boolean tryLock(long time, TimeUnit unit) throws InterruptedException {
        ...
        synchronized (this) {
          ...
          while (true) {
            ...
            if (!rawLocked && isTaskedToTryRaw(waiter) && raw.tryLock()) {
              rawLocked = true;
              try {
                admitBatch();
              } finally {
      ...
      public synchronized void unlock() {
        try {
          ...
          unlockRawIfUnlockedByAllAcquired();
        }
      }

      private boolean isTaskedToTryRaw(Waiter waiter) {
        // Only the head waiter tries the raw lock. If it times out or is interrupted,
        // stopWaiting() removes it and the next waiter becomes tasked to try.
        return waiters.peek() == waiter;
      }

      private void stopWaiting(Waiter waiter) {
      ...
      private void admitBatch() {
        ...
        for (Waiter waiter : waiters) {
          if (localAdmitted++ > lockBatchSize) {
            break;
          }
      ...
      private void unlockRawIfUnlockedByAllAcquired() {
        if (rawLocked && numAcquired == 0) {
          try {
            raw.unlock();
          } finally {
            rawLocked = false;
