 ...
 * in-process callers can share one held lock instead of each acquiring/releasing separately.
 */
  ...
  private Lock newBatchingLock(Lock raw) {
    return new Lock() {
      ...
      public boolean tryLock(long time, TimeUnit unit) throws InterruptedException {
        ...
        synchronized (this) {
          while (true) {
            if (released < acquired && acquired < lockBatchSize) {
              acquired++;
              return true;
            }
            if (released == acquired && raw.tryLock()) {
              acquired = 1;
              released = 0;
              notifyAll();
              return true;
