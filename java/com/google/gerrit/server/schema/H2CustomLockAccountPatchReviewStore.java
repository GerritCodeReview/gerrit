 ...
 * retry and backoff strategy, up to the given wait time. {@link #newLock()} is called exactly once,
 * lazily on the first {@link #getConnection()}, and the same {@link Lock} instance is reused for
...
abstract class H2CustomLockAccountPatchReviewStore extends H2AccountPatchReviewStore {
  ...
  /**
   * Returns the single {@link Lock} instance used for the lifetime of this store, creating it on
   * first use. Deferring creation until after construction (rather than initializing eagerly in the
   * constructor) avoids invoking the overridden {@link #newLock()} before subclass fields are
   * initialized.
   */
