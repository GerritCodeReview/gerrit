...
public class MigrateLabelConfigToCopyCondition {
  ...
  public void execute(Project.NameKey projectName) throws IOException, ConfigInvalidException {
    ...
    try (Repository repo = repoManager.openRepository(projectName);
      ...
      md.setMessage("Migrate label configs to copy conditions\n");
  ...
  private static String computeCopyCondition(Config cfg, String labelName) {
    ...
    if (copyConditions.size() == 1) {
      // If there is only a single copy condition, just return it as it is. This prevents adding
      // parentheses for existing complex copy conditions when no deprecated fields are migrated.
      return copyConditions.get(0);
    }
