...
public class NoteDbSchemaVersionCheck implements LifecycleListener {
  ...
  public void start() {
    try {
      ...
      if (current > expected && gerritConfig.getBoolean("gerrit", "rollingUpgrade", false)) {
        logger.atWarning().log(
            "Detected schema version %d while expected schema version was %d. Rolling upgrade may be in progress, continuing.",
            current, expected);
