...
public class SchemaModule extends FactoryModule {
  ...
  @Inject
  SchemaModule(@GerritServerConfig Config gerritConfig) {
  ...
  protected void configure() {
    ...
    if (gerritConfig.getStringList(GerritImportedServerIdsProvider.SECTION, null, GerritImportedServerIdsProvider.KEY).length > 0) {
      bind(ChangeNumberVirtualIdAlgorithm.class).to(ChangeNumberBitmapMaskAlgorithm.class).in(SINGLETON);
    }
