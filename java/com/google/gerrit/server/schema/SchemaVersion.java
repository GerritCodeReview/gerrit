...
public abstract class SchemaVersion {
  ...
  private static ExecutorService backgroundGcThread = Executors.newFixedThreadPool(1);
  ...
  protected void runGcInBackground(
      GitRepositoryManager repoManager, Project.NameKey project, UpdateUI ui) {
    GcTask task = new GcTask(project, repoManager, ui);
    if (!((ThreadPoolExecutor) backgroundGcThread).getQueue().contains(task)) {
      backgroundGcThread.execute(task);
    }
  }
  ...
  private class GcTask implements Runnable {
    ...
    public boolean equals(Object other) {
      ...
      if (!(other instanceof GcTask)) {
        return false;
      }
      return this.project.equals(((GcTask) other).project);
    }
