...
public class Schema_169 extends SchemaVersion {
  private static final String COMMIT_MSG =
      "Rewrite project.config to get rid of the refs/for/ ACLs";
    ...
    for (Project.NameKey projectName : repoManager.list()) {
      try (Repository git = repoManager.openRepository(projectName);
        ...
        for (AccessSection accessSection : config.getAccessSections()) {
          List<Permission> permissions = new ArrayList<>(accessSection.getPermissions());
  ...
  public static void updateRule(
      AccessSection destSection, Permission sourcePermission, PermissionRule sourceRule) {
