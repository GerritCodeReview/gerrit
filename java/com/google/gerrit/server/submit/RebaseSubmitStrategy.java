...
public class RebaseSubmitStrategy extends SubmitStrategy {
  ...
  public ImmutableList<SubmitStrategyOp> buildOps(Collection<CodeReviewCommit> toMerge) {
    ...
    if (!rebaseMergeCommits) {
      ...
      // branch
      // the same way as if MERGE_IF_NECESSARY was the submit strategy. This means if needed we
      // create
      ...
      // change,
      // because it already gets integrated by merging the merge change. If the rebasing of the
      // normal
      ...
      // implicitly integrated by merging the series. Then we use the MergeIfNecessaryOp to
      // integrate
      // the whole series.
      ...
      // the merge change by a merge and then integrate the normal change by a rebase. In this case
      // we
