...
class SubmoduleCommitHelper {

  private static final FluentLogger logger = FluentLogger.forEnclosingClass();

  private PersonIdent myIdent;
  private VerboseSuperprojectUpdate verboseSuperProject;
  private MergeOpRepoManager orm;
  private long maxCombinedCommitMessageSize;
  private long maxCommitMessages;
  private CachedBranchTips branchTips;

  SubmoduleCommitHelper(
      MergeOpRepoManager orm, PersonIdent myIdent, Config cfg, CachedBranchTips branchTips) {
    this.orm = orm;
   ...
   *     null if nothing has changed. Subproject tips are read from the cached branched tips
   *     (defaulting to the mergeOpRepoManager).
   */
  @Nullable
  CodeReviewCommit composeGitlinksCommit(
    ...
    return or.getCodeReviewRevWalk().parseCommit(id);
  }
