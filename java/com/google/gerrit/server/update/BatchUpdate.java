...
public class BatchUpdate implements AutoCloseable {
  ...
  private class ChangeContextImpl extends ContextImpl implements ChangeContext {
    ...
    ChangeContextImpl(ChangeNotes notes) {
      ...
      commitTimestamp = when.orElse(this.notes.getChange().getLastUpdatedOn());
    }
    ...
    private ChangeUpdate getNewChangeUpdate(PatchSet.Id psId) {
      ChangeUpdate u = changeUpdateFactory.create(notes, user, commitTimestamp);
  ...
  private final Optional<Timestamp> when;
  ...
  private final Optional<PersonIdent> refLogIdent;
  ...
  BatchUpdate(
    ...
    this.refLogIdent = user.isIdentifiedUser() ? Optional.of(user.asIdentifiedUser().newRefLogIdent(when, tz)) : Optional.empty();
  }
  ...
  BatchUpdate(
    ...
    tz = serverIdent.getTimeZone();
    this.refLogIdent = user.isIdentifiedUser() ? Optional.of(user.asIdentifiedUser().newRefLogIdent(TimeUtil.nowTs(), tz)) : Optional.empty();
  }
  ...
  private ChangesHandle executeChangeOps(
    ...
    ChangesHandle handle =
        new ChangesHandle(
            ...
            dryrun);
    if (refLogIdent.isPresent()) {
      handle.manager.setRefLogIdent(refLogIdent.get());
    }
