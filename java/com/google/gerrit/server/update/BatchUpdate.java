...
public class BatchUpdate implements AutoCloseable {
  ...
  private List<Map.Entry<Project.NameKey, BatchRefUpdate>> batchRefUpdate;
  ...
  private void fireRefChangeEvent() {
    batchRefUpdate.forEach(
        entry -> gitRefUpdated.fire(entry.getKey(), entry.getValue(), getAccount().orElse(null)));
  }
  ...
  private class ChangesHandle implements AutoCloseable {
    ...
    boolean requiresReindex() {
      ...
      List<Map.Entry<Project.NameKey, BatchRefUpdate>> bru = BatchUpdate.this.batchRefUpdate;
      return !(bru.isEmpty()
          || bru.stream().allMatch(entry -> entry.getValue().getCommands().isEmpty())
          || bru.stream()
              .map(entry -> entry.getValue().getCommands())
