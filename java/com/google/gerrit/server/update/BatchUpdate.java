...
public class BatchUpdate implements AutoCloseable {
  ...
  private void postUpdate(ContextImpl ctx, RepoOnlyOp op) throws Exception {
    try (TraceContext.TraceTimer ignored =
  ...
  private void asyncPostUpdate(ContextImpl ctx, RepoOnlyOp op) {
    executorService.execute(new ExecuteAsyncPostUpdates(op, ctx, user, requestContext));
  }
