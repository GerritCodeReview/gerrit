...
public class BatchUpdate implements AutoCloseable {
  ...
  private void fireAttentionSetUpdateEvents(Map<Change.Id, ChangeData> changeDatas) {
    boolean isFeatureEnabled = experimentFeatures.isFeatureEnabled(
    ...
    if (isFeatureEnabled) {
      java.util.Set<Account.Id> allAttentionSetAccounts = attentionSetUpdates.values().stream()
