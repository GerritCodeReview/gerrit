...
public class BatchUpdate implements AutoCloseable {
  ...
  private class ChangeContextImpl extends ContextImpl implements ChangeContext {
    ...
    ChangeContextImpl(ChangeNotes notes) {
      ...
      commitTimestamp = when.orElseGet(() -> this.notes.getChange().getLastUpdatedOn());
    }
  ...
  private final Timestamp refLogTimestamp;
  ...
  BatchUpdate(
    ...
    this.refLogTimestamp = TimeUtil.nowTs();
    tz = serverIdent.getTimeZone();
  }
  ...
  private ChangesHandle executeChangeOps(
    ...
    if (user.isIdentifiedUser()) {
      handle.manager.setRefLogIdent(user.asIdentifiedUser().newRefLogIdent(refLogTimestamp, tz));
    }
