...
public class BatchUpdate implements AutoCloseable {
   ...
   *
   * <p>Must be called after a failed {@link #execute()} before retrying, to avoid the
   * "cannot chain ref update CREATE with different new ID" error from {@link
   * ChainedReceiveCommands} when the same CREATE commands are re-added.
   */
