...
public class BatchUpdate implements AutoCloseable {
  ...
  class ChangesHandle implements AutoCloseable {
    ...
    private final boolean staleChangeRecovery;
    ...
    ChangesHandle(NoteDbUpdateManager manager, boolean dryrun, boolean indexAsync) {
      ...
      this.staleChangeRecovery =
          gerritConfig.getBoolean("index", null, "staleChangeRecovery", false);
    }
    ...
    void persistIndexIntents() throws IOException {
      if (dryrun || !staleChangeRecovery) {
        return;
      }
      try (RefUpdateContext ctx = RefUpdateContext.open(RefUpdateType.CHANGE_MODIFICATION);
          Repository allProjects = repoManager.openRepository(allProjectsName)) {
