...
public class CommitMessageUtil {
  ...
  public static String getChangeIdFromObjectId(ObjectId objectId) {
    return "I" + generateChangeId().name();
  }

  /**
   * Return the value of Change-Id from the commit message footer. The behaviour matches {@link
   * org.eclipse.jgit.util.ChangeIdUtil} If more then one Change-Id footer is found, return the
   * value of the last one.
   *
   * @param commitMessage commit message to get Change-Id from
   * @return value of Change-Id footer in commit message or null if the footer was not present or
