 ...
 * the commit message.
 *
 * <p>The JJ change-id format is a 32-character lowercase string (reverse-hex encoding), e.g.
 * {@code mlqnqnkrxpuvuuxzlzoltostwlwyskpx}, which is distinct from Gerrit's {@code I}-prefixed
 * SHA1-based Change-Id.
 */
public final class JujutsuChangeIdUtil {

  /** Pattern matching a Jujutsu change ID: exactly 32 lowercase letters. */
  public static final Pattern JJ_CHANGE_ID_PATTERN = Pattern.compile("^[a-z]{32}$");

  /** The {@code change-id} header name as raw bytes, including the trailing space. */
  private static final byte[] CHANGE_ID_HEADER = Constants.encode("change-id ");

  private JujutsuChangeIdUtil() {}

  /**
   * Returns {@code true} if the given string looks like a Jujutsu change ID (32 lowercase
   * letters).
   */
   ...
   *
   * <p>This is NOT the same as a {@code Change-Id:} footer in the commit message. JJ writes the
   * change ID as a git commit-object header (a line in the header section before the blank line),
   * e.g.:
   *
   * <pre>
   * tree ...
