...
public class VerifiersByRepositoryNotes extends VersionedMetaData {
   ...
   * <p>Invalid verifier UUIDs are silently ignored.
   */
  private static ImmutableSortedSet<String> parseVerifierUuidsFromNote(
      ObjectId noteId, byte[] raw, ObjectId blobId) {
    ImmutableSortedSet<String> lines = parseNote(raw);
    ImmutableSortedSet.Builder<String> verifierUuids = ImmutableSortedSet.naturalOrder();
    lines.forEach(
        line -> {
  ...
  private static void insert(
    ...
    newLinesBuilder.add(verifierUuid);
  ...
  private static void remove(
    ...
    if (noteMap.contains(noteId)) {
      ...
      oldLines.stream().filter(line -> !line.equals(verifierUuid)).forEach(newLinesBuilder::add);
    }
