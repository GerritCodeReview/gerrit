...
public class InactiveAccountDisconnector implements AccountActivationListener {
  ...
  public void onAccountDeactivated(int id) {
    try {
            ...
            if (sessionUser.isIdentifiedUser()) {
              ...
              if (identifiedUserId == id) {
                logger.atInfo().log(
                    ...
                    abstractSession, identifiedUser.getUserName().orElse(""), identifiedUserId);
                try {
      ...
      // Ssh Daemon no longer running. Since we're only disconnecting connections anyways, no need
      // to escalate the Failure.
    }
  }
}
