...
public class SshMetrics {
  ...
  SshMetrics(MetricMaker metrics) {
    this.count =
        metrics.newCounter(
            ...
            Field.ofString("command_name", Metadata.Builder::requestType)
    ...
    this.errorCount =
        metrics.newCounter(
            ...
            Field.ofString("request_type", Metadata.Builder::requestType)
                .description("The type of the failed request.")
                .build(),
            Field.ofString("exception", Metadata.Builder::restViewName)
  ...
  public void countRequest(RequestInfo requestInfo, Throwable error) {
    if (error == null) {
      count.increment(requestInfo.commandName());
    } else {
      errorCount.increment(requestInfo.commandName(), error.getClass().getSimpleName());
    }
  }
}
