...
public class CheckProjectAccessCommand extends SshCommand {
  ...
  protected void run() throws Failure, ConfigInvalidException, IOException {
                  ...
                  user -> {
                    try {
                      ...
                      return (String.format(
                          "Username: '%s', Email: '%s', Full Name: '%s', Result: TRUE\n",
                          user.getName(),
                          user.getNameEmail()
                              .substring(
                                  user.getNameEmail().indexOf("<") + 1,
                                  user.getNameEmail().indexOf(">")),
                          user.getLoggableName()));
                    } catch (AuthException e) {
                      return (String.format(
                          "Username: '%s', Email: '%s',  Full Name: '%s', Result: FALSE\n",
                          user.getName(),
                          user.getNameEmail()
                              .substring(
                                  user.getNameEmail().indexOf("<") + 1,
                      ...
                      return ("Fatal: Error reading project: '"
                          + projectName
                          + "', for Username: '"
                          + user.getLoggableName()
                          + "', Email: '"
    ...
    } catch (ConfigInvalidException | ResourceNotFoundException | IllegalArgumentException e) {
      throw die(e);
    }
  }
