...
public class CheckProjectVisibilityCommand extends SshCommand {
  ...
  protected void run() throws Failure, org.eclipse.jgit.errors.ConfigInvalidException, IOException {
    try {
      stdout.print(getVisibility(userName, projectName));
  ...
  private String getVisibility(String userName, String projectName)
    ...
    return getUserList(userName).stream()
        .map(
            user ->
                String.format(
                    "%s %s %s\n",
                    user.getName(), user.getLoggableName(), isVisibleTo(user, projectName)))
        .collect(Collectors.joining(""));
  }

  private boolean isVisibleTo(IdentifiedUser userName, String projectName) {
    try {
      permissionBackend
          .user(userName)
