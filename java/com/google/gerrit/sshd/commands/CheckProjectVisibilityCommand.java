...
public class CheckProjectVisibilityCommand extends SshCommand {
  ...
  protected void run() throws Failure, org.eclipse.jgit.errors.ConfigInvalidException, IOException {
    try {
      checkVisibility(userName, projectName);
  ...
  private void checkVisibility(String userName, String projectName)
      throws org.eclipse.jgit.errors.ConfigInvalidException, IOException,
      ResourceNotFoundException {
    IdentifiedUser identifiedUser = getUser(userName);
    stdout.print(String.format("%s %s %s\n", identifiedUser.getName(), identifiedUser.getLoggableName(), ifVisible(identifiedUser, projectName)));

  }

  private boolean ifVisible(IdentifiedUser userName, String projectName) {
    try {
      permissionBackend
          .user(userName)
  ...
  private IdentifiedUser getUser(String userName)
      throws org.eclipse.jgit.errors.ConfigInvalidException, IOException,
      ResourceNotFoundException {
    List<IdentifiedUser> list = getIdList(userName).stream().map(userFactory::create).collect(Collectors.toList());
    return list.get(0);
  }
