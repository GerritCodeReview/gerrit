...
public class CheckProjectVisibilityCommand extends SshCommand {
  ...
  private void checkVisibility(String userName, String projectName)
    ...
    stdout.print(
        String.format(
            "%s %s %s\n",
            identifiedUser.getName(),
            identifiedUser.getLoggableName(),
            ifVisible(identifiedUser, projectName)));
  }
  ...
  private IdentifiedUser getUser(String userName)
    ...
    List<IdentifiedUser> list =
        getIdList(userName).stream().map(userFactory::create).collect(Collectors.toList());
    return list.get(0);
  }
