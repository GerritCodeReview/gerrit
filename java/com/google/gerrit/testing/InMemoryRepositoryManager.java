...
public class InMemoryRepositoryManager implements GitRepositoryManager {
  ...
  public static class Repo extends InMemoryRepository {
    ...
    private static class RefUpdateContextValidator {
       ...
       *
       * Each ref must match no more than 1 special ref from the list below. If ref is not matched
       * to any special ref predicate, then it is checked against the standard rules - check the
       * code of the {@link #validateRefUpdateContext} for details.
       */
      public static final RefUpdateContextValidator INSTANCE = new RefUpdateContextValidator()
      ...
      /**
       * Returns true if a ref is updated using one of the method in {@link org.eclipse.jgit.junit.TestRepository}.
       *
       * The {@link org.eclipse.jgit.junit.TestRepository} used only in tests and allows to
