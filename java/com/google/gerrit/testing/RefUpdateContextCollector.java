...
public class RefUpdateContextCollector implements TestRule {
  ...
  public ImmutableList<ImmutableList<RefUpdateContext>> getContextsByRef(String refName) {
    return touchedRefsWithContexts.stream()
        .filter(entry -> entry.getKey().equals(refName))
        .map(Entry::getValue)
        .collect(toImmutableList());
  }

  public ImmutableList<ImmutableList<RefUpdateContext>> getContextsByRefPrefix(String refPrefix) {
    return touchedRefsWithContexts.stream()
        .filter(entry -> entry.getKey().startsWith(refPrefix))
        .map(Entry::getValue)
        .collect(toImmutableList());
  }
