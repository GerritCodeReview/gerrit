 ...
 * 5869), domain-separated by a caller-provided {@code info} label; the input key bytes are zeroed
 * after derivation. Each value is sealed as {@code "gcm:v1:" + Base64(iv || ciphertext || tag)}
 * with a fresh 96-bit IV, and the caller supplies opaque additional authenticated data (AAD) that
 * is bound into the tag.
 *
 * <p>The HKDF composition over {@link Mac} is implemented locally because the oldest supported
 * runtime (JDK 21) has no HKDF API; all primitives (AES-GCM, HMAC-SHA256, {@link SecureRandom},
 * {@link Base64}) are JDK/JCE. This class has no dependency on anything outside the JDK.
 */
public final class AesGcmCipher {
  ...
  public static byte[] hkdfSha256(byte[] ikm, byte[] info, int length)
    ...
    if (length < 0 || length > 255 * SHA256_BYTES) {
      throw new IllegalArgumentException("Invalid HKDF output length.");
    }
    byte[] salt = new byte[SHA256_BYTES];
