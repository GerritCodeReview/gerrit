    ...
    try (RevWalk revWalk = new RevWalk(StoredValues.REPOSITORY.get(engine))) {
      ...
      for (PatchListEntry entry : patches) {
        ...
        if (isSubmodule(engine, commit.getTree(), entry.getNewName())) {
          structureTerm =
              new StructureTerm("file", fileNameTerm, changeType, SymbolTerm.create("SUBMODULE"));
        } else {
          structureTerm =
              new StructureTerm("file", fileNameTerm, changeType, SymbolTerm.create("REGULAR"));
        }
        listHead = new ListTerm(structureTerm, listHead);
      }
