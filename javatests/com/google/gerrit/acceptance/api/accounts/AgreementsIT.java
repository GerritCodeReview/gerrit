...
public class AgreementsIT extends AbstractDaemonTest {
  ...
  public void createAccessChangeRespectsCLA() throws Exception {
    ...
    TestProjectUpdate.TestPermission permission =
        TestProjectUpdate.allow(Permission.PUSH)
            .ref("refs/for/refs/meta/config")
            .group(REGISTERED_USERS)
            .build();
    projectOperations.project(project).forUpdate().add(permission).update();
    projectOperations
        .project(project)
        .forUpdate()
    ...
    setUseContributorAgreements(InheritableBoolean.FALSE);
    RestResponse rep = createPermissionsChange("agreement not required");
    rep.assertCreated();
    ChangeInfo got;
    try (JsonReader jsonReader = new JsonReader(rep.getReader())) {
    ...
    setUseContributorAgreements(InheritableBoolean.TRUE);
    rep = createPermissionsChange("agreement required - fails");
    rep.assertForbidden();
    assertThat(rep.hasContent()).isTrue();
    assertThat(rep.getEntityContent()).startsWith("No Contributor Agreement on file ");
    ...
    rep = createPermissionsChange("agreement required - passes");
    rep.assertCreated();
    try (JsonReader jsonReader = new JsonReader(rep.getReader())) {
      jsonReader.setStrictness(Strictness.LENIENT);
      got = newGson().fromJson(jsonReader, ChangeInfo.class);
