...
public class AutoAccountCreationIT extends AbstractDaemonTest {
  @Inject private DynamicItem<AutoAccountCreator> autoAccountCreators;
  ...
  public void autoCreateAccountOnAddingMemberToGroup() throws Exception {
    ...
    TestAutoAccountCreator testAutoAccountCreator = new TestAutoAccountCreator();
    RegistrationHandle registrationHandle =
        autoAccountCreators.set(testAutoAccountCreator, "myPlugin");
    try {
  ...
  public void autoCreateAccountOnAddingChangeReviewer() throws Exception {
    ...
    try {
      ...
      // check that adding a non-existing user as reviewer fails if auto account creation is
      // disabled
      String user2 = "bar";
      testAutoAccountCreator.setEnabled(false);
      in.reviewer = user2;
  ...
  public void autoCreateAccountOnSettingChangeAssignee() throws Exception {
    ...
    try {
      ...
      // check that adding a non-existing user as assignee fails if auto account creation is
      // disabled
      String user2 = "bar";
      testAutoAccountCreator.setEnabled(false);
  ...
  @Test
  public void autoCreateAccountOnAccountAccess() throws Exception {
    String user = "foo";
    assertThatUserDoesNotExist(user);

    RegistrationHandle registrationHandle =
        autoAccountCreators.set(new TestAutoAccountCreator(), "myPlugin");
    try {
      assertThatUserExists(user);
    } finally {
      registrationHandle.remove();
  ...
  private class TestAutoAccountCreator implements AutoAccountCreator {
    ...
    @Override
    public Optional<Account.Id> createAccount(String userId)
      ...
      return Optional.of(newAccountState.getAccount().getId());
    }
  }
}
