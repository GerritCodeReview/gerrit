...
public class ApplyPatchIT extends AbstractDaemonTest {
  ...
  public void applyDeletedFilePatch() throws Exception {
    ...
    assertDiffForDeletedFile(diff, DELETED_FILE_NAME, DELETED_FILE_ORIGINAL_CONTENT);
  }
  ...
  public void applyRenamedFilePatch() throws Exception {
    ...
    assertDiffForDeletedFile(
        originalFileDiff, RENAMED_FILE_ORIGINAL_NAME, MODIFIED_FILE_ORIGINAL_CONTENT);
    DiffInfo newFileDiff = fetchDiffForFile(result, RENAMED_FILE_NEW_NAME);
    assertDiffForNewFile(
        newFileDiff, result.currentRevision, RENAMED_FILE_NEW_NAME, MODIFIED_FILE_NEW_CONTENT);
  }

  @Test
  public void applyGerritBasedPatchWithSingleFile() throws Exception {
  ...
  @Test
  public void applyGerritBasedPatchWithMultipleFiles() throws Exception {
  ...
  @Test
  public void applyPatchWithConflictFails() throws Exception {
  ...
  @Test
  public void applyPatchWithCustomMessage() throws Exception {
    initDestBranch(DESTINATION_BRANCH, HEAD);
    ApplyPatchAsPatchSetInput in = buildInput(ADDED_FILE_DIFF);
    in.subject = "custom subject";

    ChangeInfo result = applyPatch(in);

    assertThat(gApi.changes().id(result.id).current().commit(false).message).contains(in.subject);
  }

  @Test
  public void applyPatchWithBaseCommit() throws Exception {
  ...
  @Test
  public void applyPatchWithDefaultAuthor() throws Exception {
    initDestBranch(DESTINATION_BRANCH, HEAD);
    ApplyPatchAsPatchSetInput in = buildInput(ADDED_FILE_DIFF);
  ...
  @Test
  public void applyPatchWithAuthorOverrideMissingEmailIllegalArgument() throws Exception {
    initDestBranch(DESTINATION_BRANCH, HEAD);
    ApplyPatchAsPatchSetInput in = buildInput(ADDED_FILE_DIFF);
  ...
  @Test
  public void applyPatchWithAuthorOverrideMissingNameIllegalArgument() throws Exception {
    initDestBranch(DESTINATION_BRANCH, HEAD);
    ApplyPatchAsPatchSetInput in = buildInput(ADDED_FILE_DIFF);
    in.author = new AccountInput();
    in.author.name = null;
  ...
  @Test
  public void applyPatchWithAuthorOverride() throws Exception {
    initDestBranch(DESTINATION_BRANCH, HEAD);
    ApplyPatchAsPatchSetInput in = buildInput(ADDED_FILE_DIFF);
    in.author = new AccountInput();
    in.author.email = "gerritlessjane@invalid";
  ...
  @Test
  public void applyPatchWithAuthorWithoutPermissions() throws Exception {
    initDestBranch(DESTINATION_BRANCH, HEAD);
    ApplyPatchAsPatchSetInput in = buildInput(ADDED_FILE_DIFF);
    in.author = new AccountInput();
    in.author.name = "Jane";
