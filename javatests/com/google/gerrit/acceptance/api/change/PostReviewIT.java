...
public class PostReviewIT extends AbstractDaemonTest {
  ...
  public void validateCommentsInInput_commentOK() throws Exception {
    ...
    ReviewInput input = defaultReviewInput();
  ...
  @Test
  @GerritConfig(name = "change.maxCumulativeSizeBytes", value = "7000")
  public void validateCumulativeCommentSize() throws Exception {
    PushOneCommit.Result r = createChange();
    when(mockCommentValidator.validateComments(eq(contextFor(r)), any()))
        .thenReturn(ImmutableList.of());

    // Use large sizes because autogenerated comments already have O(100) bytes.
  ...
  // ö Why did the other tests pass even though the change message wasn't included?
  ...
  private ReviewInput defaultReviewInput() {
    ReviewInput reviewInput = new ReviewInput();
    reviewInput.message(COMMENT_TEXT);
    return reviewInput;
  }
