...
public class PostReviewIT extends AbstractDaemonTest {
  ...
  public void validateCommentsInInput_commentOK() throws Exception {
    ...
    when(mockCommentValidator.validateComments(eq(contextFor(r, user.email())), captor.capture()))
  ...
  public void validateCumulativeCommentSize() throws Exception {
    PushOneCommit.Result r = createChange();
    when(mockCommentValidator.validateComments(eq(contextFor(r, user.email())), any()))
        .thenReturn(ImmutableList.of());
  ...
  private static CommentValidationContext contextFor(PushOneCommit.Result result, String email) {
    return CommentValidationContext.create(
        result.getChange().getId().get(), result.getChange().project().get(), email);
  }
