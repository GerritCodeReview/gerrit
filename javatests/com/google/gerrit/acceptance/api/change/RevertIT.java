...
public class RevertIT extends AbstractDaemonTest {
  ...
  @Test
  public void revertSubmissionIdenticalTree() throws Exception {
    PushOneCommit.Result irrelevantResult = createChange("change1", "a.txt", "message");
    approve(irrelevantResult.getChangeId());
    gApi.changes().id(irrelevantResult.getChangeId()).current().submit();

    PushOneCommit.Result firstResult = createChange("change1", "a.txt", "message");
    approve(firstResult.getChangeId());
    gApi.changes().id(firstResult.getChangeId()).addReviewer(user.email());
    PushOneCommit.Result secondResult = createChange("change2", "b.txt", "message");
    approve(secondResult.getChangeId());
    gApi.changes().id(secondResult.getChangeId()).addReviewer(user.email());
    ...
    List<ChangeApi> revertChanges =
        getChangeApis(gApi.changes().id(secondResult.getChangeId()).revertSubmission(revertInput));
    assertThat(revertChanges.size()).isEqualTo(1);

    List<Message> messages = sender.getMessages();

    // Message is sent only for the second change (first revert) - the one that is actually made.
    // The second revert is empty since that change is empty (same as the irrelevantChange).
    assertThat(messages).hasSize(2);
