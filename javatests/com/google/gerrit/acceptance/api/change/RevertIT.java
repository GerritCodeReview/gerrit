...
public class RevertIT extends AbstractDaemonTest {
  ...
  public void cantCreateRevertSubmissionWithoutProjectWritePermission() throws Exception {
    ...
    String result2 =
        createChange(secondRepo, "master", "second change", "b.txt", "message", topic)
            .getChangeId();
    gApi.changes().id(result1).current().review(ReviewInput.approve());
    gApi.changes().id(result2).current().review(ReviewInput.approve());
    gApi.changes().id(result1).current().submit();
    ...
    ResourceConflictException thrown =
        assertThrows(
            ResourceConflictException.class, () -> gApi.changes().id(result1).revertSubmission());
    assertThat(thrown).hasMessageThat().contains(expected);
    ...
    thrown =
        assertThrows(
            ResourceConflictException.class, () -> gApi.changes().id(result2).revertSubmission());
    assertThat(thrown).hasMessageThat().contains(expected);
  }
  ...
  public void cantCreateRevertSubmissionWithoutCreateChangePermission() throws Exception {
    ...
    PermissionDeniedException thrown =
        assertThrows(
            PermissionDeniedException.class, () -> gApi.changes().id(result1).revertSubmission());
    assertThat(thrown)
        .hasMessageThat()
        .contains("not permitted: create change on refs/heads/master");
    ...
    thrown =
        assertThrows(
            PermissionDeniedException.class, () -> gApi.changes().id(result2).revertSubmission());
    assertThat(thrown)
        .hasMessageThat()
        .contains("not permitted: create change on refs/heads/master");
  }
  ...
  public void cantCreateRevertSubmissionWithoutReadPermission() throws Exception {
    ...
    ResourceNotFoundException resourceNotFoundException =
        assertThrows(
            ResourceNotFoundException.class, () -> gApi.changes().id(result1).revertSubmission());
    assertThat(resourceNotFoundException).hasMessageThat().isEqualTo("Not found: " + result1);

    // assert that if the first repository has no READ permissions and a change from another
    // repository is trying to revert the submission, it will fail.
    AuthException authException =
        assertThrows(AuthException.class, () -> gApi.changes().id(result2).revertSubmission());
  ...
  public void revertSubmissionPreservesReviewersAndCcs() throws Exception {
    String changeResult = createChange("first change", "a.txt", "message").getChangeId();
    ...
    gApi.changes().id(changeResult).current().review(in);
    gApi.changes().id(changeResult).current().submit();
    ...
    Map<ReviewerState, Collection<AccountInfo>> result =
        getChangeApis(gApi.changes().id(changeResult).revertSubmission()).get(0).get().reviewers;
    assertThat(result).containsKey(ReviewerState.REVIEWER);
  ...
  @Test
  public void revertSubmissionIdenticalTreeAreAllowed() throws Exception {
