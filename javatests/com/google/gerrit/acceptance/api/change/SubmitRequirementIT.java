...
public class SubmitRequirementIT extends AbstractDaemonTest {
  ...
  @Test
  public void submitRequirement_wantLgtmAll() throws Exception {
    projectOperations
        .project(project)
        .forUpdate()
    ...
    configSubmitRequirement(
        ...
        SubmitRequirement.builder()
            ...
            .setApplicabilityExpression(
                Optional.of(
                    SubmitRequirementExpression.create(
                        "-label:Code-Review=MAX,user=all_reviewers")))
            .setSubmittabilityExpression(
                SubmitRequirementExpression.create("label:Code-Review=MAX,user=all_reviewers"))
            .setAllowOverrideInChildProjects(false)
            .build());
    ...
    // Want-Code-Review-From-All is not applicable (since there is no reviewer on the change
    // "label:Code-Review=MAX,user=all_reviewers" is satisfied).
    assertSubmitRequirementStatus(
        gApi.changes().id(changeId).get().submitRequirements,
    ...
    // (reviewer1 and reviewer2) which makes "label:Code-Review=MAX,user=all_reviewers" satisfied.
    assertSubmitRequirementStatus(
        gApi.changes().id(changeId).get().submitRequirements,
        "Want-Code-Review-From-All",
        Status.NOT_APPLICABLE,
  ...
  @Test
  public void submitRequirement_wantLgtmAll_enabledByFooter() throws Exception {
    projectOperations
        .project(project)
        .forUpdate()
        .add(allowLabel("Code-Review").ref("refs/*").group(REGISTERED_USERS).range(-2, 2))
    ...
    configSubmitRequirement(
        ...
        SubmitRequirement.builder()
            ...
            .setApplicabilityExpression(
                Optional.of(
                    SubmitRequirementExpression.create(
                        "footer:\"WANT-LGTM: all\" -label:Code-Review=MAX,user=all_reviewers")))
            .setSubmittabilityExpression(
                SubmitRequirementExpression.create("label:Code-Review=MAX,user=all_reviewers"))
            .setAllowOverrideInChildProjects(false)
            .build());
