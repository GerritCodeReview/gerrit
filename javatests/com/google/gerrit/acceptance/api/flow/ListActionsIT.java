...
public class ListActionsIT extends AbstractDaemonTest {
  ...
  public void listActions_noActionsExist() throws Exception {
    ...
    try (Registration registration = extensionRegistry.newRegistration().set(flowService)) {
      List<FlowActionInfo> actions = gApi.changes().id(changeIdentifier).flowsActions();
  ...
  public void listActions() throws Exception {
    ...
    class TestFlowServiceWithActions extends TestExtensions.TestFlowService {
      @Override
      public ImmutableList<FlowActionInfo> listActions(
          com.google.gerrit.entities.Project.NameKey projectName,
          com.google.gerrit.entities.Change.Id changeId) {
        FlowActionInfo action1 = new FlowActionInfo();
        action1.name = "action1";

        FlowActionInfo action2 = new FlowActionInfo();
        action2.name = "action2";
        return ImmutableList.of(action1, action2);
      }
    }

    try (Registration registration =
        extensionRegistry.newRegistration().set(new TestFlowServiceWithActions())) {
      List<FlowActionInfo> actions = gApi.changes().id(changeIdentifier).flowsActions();
