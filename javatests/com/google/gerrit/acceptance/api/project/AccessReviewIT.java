...
public class AccessReviewIT extends AbstractDaemonTest {
  ...
  public void createPermissionsChangeWithDefaultMessage() throws Exception {
    ...
    p.rules =
        ImmutableMap.of(SystemGroupBackend.REGISTERED_USERS.get(), new PermissionRuleInfo(PermissionRuleInfo.Action.ALLOW, false));
    ...
    RestResponse rep =
        adminRestSession.put(
            "/projects/"
                + defaultMessageProject.get()
                + "/access:review", in);
    rep.assertCreated();

    List<ChangeInfo> result = gApi.changes().query("project:" + defaultMessageProject.get() + " AND ref:refs/meta/config").get();
  ...
  public void createPermissionsChangeWithCustomMessage() throws Exception {
    ...
    RestResponse rep =
        adminRestSession.put(
            "/projects/"
                + customMessageProject.get()
                + "/access:review", in);
    rep.assertCreated();

    List<ChangeInfo> result = gApi.changes().query("project:" + customMessageProject.get() + " AND ref:refs/meta/config").get();
