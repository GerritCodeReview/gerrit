...
public class CommitIT extends AbstractDaemonTest {
  ...
  public void includedInRefsOpenChange() throws Exception {
    Result result = createChange();
    assertThat(getIncludedInRefs(result.getCommit().getId(), Arrays.asList(R_HEADS + "master")))
        .isEmpty();
  }
  ...
  public void includedInRefsMergedChange() throws Exception {
    ...
    assertThat(getIncludedInRefs(result.getCommit().getId(), Collections.emptyList())).isEmpty();
    assertThat(getIncludedInRefs(result.getCommit().getId(), Arrays.asList(R_HEADS + "foo")))
        .isEmpty();
    assertThat(getIncludedInRefs(result.getCommit().getId(), Arrays.asList(R_HEADS + "master")))
    ...
    assertThat(getIncludedInRefs(result.getCommit().getId(), Arrays.asList(R_TAGS + "test-tag")))
        .containsExactly(R_TAGS + "test-tag");
    assertThat(
            getIncludedInRefs(
                result.getCommit().getId(), Arrays.asList(R_HEADS + "master", R_TAGS + "test-tag")))
  ...
  public void includedInRefsMergedChange_filtersOutNonVisibleTagRefs() throws Exception {
    ...
    Result baseChange = createAndSubmitChange("refs/for/master");
    createLightWeightTag(tagBase);
    assertThat(getIncludedInRefs(baseChange.getCommit().getId(), Arrays.asList(R_TAGS + tagBase)))
  ...
  private Collection<String> getIncludedInRefs(ObjectId id, List<String> refs) throws Exception {
