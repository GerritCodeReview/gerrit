...
public class ApplyDirectFixIT extends AbstractDaemonTest {
  ...
  public void directFixWithinALineCanBeApplied() throws Exception {
    DirectFixInput directFixInput = createDirectFixInput(FILE_NAME, "Modified content", 3, 1, 3, 3);
  ...
  @Test
  public void applyDirectFixRestAPItestForASimpleFix() throws Exception {
    DirectFixInput directFixInput = createDirectFixInput(FILE_NAME, "Modified content", 3, 1, 3, 3);
    RestResponse resp =
        adminRestSession.post(
            "/changes/" + changeId + "/revisions/" + commitId + "/apply_direct_fix",
            directFixInput);
  ...
  @Test
  public void applyDirectFixRestWithoutAddPatchSetPermissionCannotBeApplied() throws Exception {
    DirectFixInput directFixInput = createDirectFixInput(FILE_NAME, "Modified content", 3, 1, 3, 3);
  ...
  public void directFixSpanningMultipleLinesCanBeApplied() throws Exception {
    DirectFixInput directFixInput =
        createDirectFixInput(FILE_NAME, "Modified content\n5", 3, 2, 5, 3);
  ...
  @Test
  public void directFixOnPreviousPatchSetCannotBeApplied() throws Exception {
    DirectFixInput directFixInput = createDirectFixInput(FILE_NAME, "Modified content", 3, 1, 3, 3);
  ...
  @Test
  public void directFixWithoutAddPatchSetPermissionCannotBeApplied() throws Exception {
    gApi.changes().id(changeId).edit().create();
    // projectOperations.project(project).forUpdate().add(block(Permission.ADD_PATCH_SET).ref("refs/for/*").group(REGISTERED_USERS)).update();
  }

  @Test
  public void directFixOnPreviousPatchSetWithExistingChangeEditCanNotBeApplied() throws Exception {
  ...
  public void directFixOnCommitMessageCanBeApplied() throws Exception {
    ...
    DirectFixInput directFixInput =
        createDirectFixInput(Patch.COMMIT_MSG, "Modified line\n", 7, 0, 8, 0);
  ...
  @Test
  public void twoDirectFixesOnCommitMessageCannotBeApplied() throws Exception {
    // Set a dedicated commit message.
    String footer = "\nChange-Id: " + changeId + "\n";
    String originalCommitMessage =
        "Line 1 of commit message\nLine 2 of commit message\nLine 3 of commit message\n" + footer;
  ...
  @Test
  public void applyingDirectFixTwiceRaisesException() throws Exception {
    DirectFixInput directFixInput = createDirectFixInput(FILE_NAME, "Modified content", 3, 1, 3, 3);
    gApi.changes().id(changeId).current().applyDirectFix(directFixInput);
    // Apply the fix again.
    BadRequestException thrown =
