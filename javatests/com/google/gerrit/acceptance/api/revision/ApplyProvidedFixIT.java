...
public class ApplyProvidedFixIT extends AbstractDaemonTest {
  ...
  public void applyProvidedFixWithinALineCanBeApplied() throws Exception {
    ...
    gApi.changes().id(changeId).current().applyProvidedFix(applyProvidedFixInput);
  ...
  public void applyProvidedFixRestAPItestForASimpleFix() throws Exception {
    ...
    RestResponse resp =
        adminRestSession.post(
            "/changes/" + changeId + "/revisions/" + commitId + "/apply_provided_fix",
  ...
  @Test
  public void applyProvidedFixRestWithoutAddPatchSetPermissionCannotBeApplied() throws Exception {
    ApplyProvidedFixInput applyProvidedFixInput =
        createApplyProvidedFixInput(FILE_NAME, "Modified content", 3, 1, 3, 3);
        ...
        assertThrows(
            ...
            () ->
                gApi.changes()
                    ...
                    .applyProvidedFix(applyProvidedFixInput));
    assertThat(thrown).hasMessageThat().contains("current");
  }

  @Test
  public void applyProvidedFixOnPreviousPatchSetWithExistingChangeEditCanNotBeApplied()
      throws Exception {
  ...
  @Test
  public void twoApplyProvidedFixesOnCommitMessageCannotBeApplied() throws Exception {
    // Set a dedicated commit message.
    String footer = "\nChange-Id: " + changeId + "\n";
    String originalCommitMessage =
        "Line 1 of commit message\nLine 2 of commit message\nLine 3 of commit message\n" + footer;
    ...
    gApi.changes().id(changeId).current().applyProvidedFix(applyProvidedFixInput1);

    BadRequestException thrown =
        assertThrows(
            BadRequestException.class,
            () -> gApi.changes().id(changeId).current().applyProvidedFix(applyProvidedFixInput2));
    assertThat(thrown).hasMessageThat().contains("already exists");
  }

  @Test
  public void applyingProvidedFixTwiceRaisesException() throws Exception {
    ApplyProvidedFixInput applyProvidedFixInput =
        createApplyProvidedFixInput(FILE_NAME, "Modified content", 3, 1, 3, 3);
    gApi.changes().id(changeId).current().applyProvidedFix(applyProvidedFixInput);
    // Apply the fix again.
