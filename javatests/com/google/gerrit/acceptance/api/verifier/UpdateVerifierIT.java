...
import static com.google.gerrit.acceptance.testing.verifier.VerifierRefSubject.assertThat;
...
public class UpdateVerifierIT extends AbstractDaemonTest {
  ...
  @Inject private VerifierRefSubject.Factory verifierRefSubjectFactory;
  ...
  public void updateVerifierNameToNameThatIsAlreadyUsed() throws Exception {
    ...
    assertVerifierRef(info.uuid).configText().isEqualTo("[verifier]\n\tname = other-verifier\n");
  }
  ...
  public void updateVerifierWithoutAdministrateVerifiersCapabilityFails() throws Exception {
    ...
    // Fails with ResourceNotFoundException because verifier resource is not visible.
    exception.expect(ResourceNotFoundException.class);
    exception.expectMessage("Not found: " + verifierUuid);
    gApi.verifiers().id(verifierUuid).update(input, EnumSet.of(UpdateVerifierOption.NAME));
  }

  private VerifierRefSubject assertVerifierRef(String verifierUuid) {
    return assertThat(verifierRefSubjectFactory, verifierUuid);
  }
}
