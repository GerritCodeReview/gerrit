...
public class FileUploadIT extends AbstractDaemonTest {
  ...
  private static final byte[] CONTENT_NEW_DECODED = "Hellow, World!".getBytes(UTF_8);
  ...
  public void createAndUploadFileInChangeEditInOneRequestRest() throws Exception {
    ...
    in.path = FILE_NAME;
    adminRestSession.put(urlEditUpload(changeId), in).assertCreated();
  ...
  public void createAndUploadFileWithoutPath() throws Exception {
    ...
    BadRequestException thrown =
        assertThrows(
            BadRequestException.class, () -> gApi.changes().id(changeId).edit().upload(in));
    assertThat(thrown).hasMessageThat().contains("Invalid path: ");
  }
