...
public abstract class AbstractImplicitMergeOnSubmit extends AbstractDaemonTest {
  PushOneCommit.Result createApprovedChange(String targetBranch, RevCommit... parents) throws Exception{
     PushOneCommit.Result result = pushTo("refs/for/" + targetBranch, ImmutableMap.of(), parents);
      gApi.changes().id(result.getChangeId()).current().review(ReviewInput.approve());
      return result;
  }
  PushOneCommit.Result createApprovedChange(String targetBranch, PushOneCommit.Result... parents) throws Exception{
    return createApprovedChange(targetBranch, Arrays.stream(parents).map(PushOneCommit.Result::getCommit).toArray(RevCommit[]::new));
  }
