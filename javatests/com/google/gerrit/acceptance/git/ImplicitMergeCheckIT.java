...
//    assertThat(RawParseUtils.decode(data2)).isEqualTo("0");
  ...
  ImmutableMap<String, String> getRemoteBranchRootPathContent(String targetBranch) throws Exception {
    testRepo.git().fetch().setRemote("origin").call();
    testRepo.git().checkout().setName("origin/" + targetBranch).call();
    ImmutableMap.Builder<String, String> builder = ImmutableMap.builder();
    for(DirCacheEntry entry: testRepo.getRepository().readDirCache().getEntriesWithin("")) {
      String content = RawParseUtils.decode(testRepo.getRepository().getObjectDatabase().open(entry.getObjectId()).getBytes());
      builder.put(entry.getPathString(), content);
    }
