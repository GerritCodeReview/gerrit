...
public class MigrateLabelConfigToCopyConditionIT extends AbstractDaemonTest {
  ...
  @Test
  public void nothingToMigrate_noFlags() throws Exception {
  ...
  @Test
  public void nothingToMigrate_copyConditionExists() throws Exception {
  ...
  @Test
  public void nothingToMigrate_complexCopyConditionExists() throws Exception {
    setCopyConditionOnCodeReviewLabel("is:MIN has:unchanged-files");
    RevCommit refsMetaConfigHead = projectOperations.project(project).getHead(RefNames.REFS_CONFIG);

    runMigration();

    // verify that refs/meta/config was not touched (e.g. no parentheses have been added around the
    // copy condition)
    assertThat(projectOperations.project(project).getHead(RefNames.REFS_CONFIG))
        .isEqualTo(refsMetaConfigHead);
  }
  ...
  @Test
  public void nothingToMigrate_nonOrderedCopyConditionExists() throws Exception {
    setCopyConditionOnCodeReviewLabel("is:MIN OR has:unchanged-files");
    RevCommit refsMetaConfigHead = projectOperations.project(project).getHead(RefNames.REFS_CONFIG);
  ...
  private void assertDeprecatedFieldsUnset(String labelName) {
    assertUnset(labelName, ProjectConfig.KEY_COPY_ANY_SCORE);
    assertUnset(labelName, ProjectConfig.KEY_COPY_MIN_SCORE);
    assertUnset(labelName, ProjectConfig.KEY_COPY_MAX_SCORE);
    assertUnset(labelName, ProjectConfig.KEY_COPY_VALUE);
    assertUnset(labelName, ProjectConfig.KEY_COPY_ALL_SCORES_IF_NO_CHANGE);
    assertUnset(labelName, ProjectConfig.KEY_COPY_ALL_SCORES_IF_NO_CODE_CHANGE);
    assertUnset(labelName, ProjectConfig.KEY_COPY_ALL_SCORES_ON_MERGE_FIRST_PARENT_UPDATE);
    assertUnset(labelName, ProjectConfig.KEY_COPY_ALL_SCORES_ON_TRIVIAL_REBASE);
    assertUnset(labelName, ProjectConfig.KEY_COPY_ALL_SCORES_IF_LIST_OF_FILES_DID_NOT_CHANGE);
  }
