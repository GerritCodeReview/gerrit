...
import static com.google.gerrit.server.restapi.account.PutHttpPassword.LEGACY_ID;
...
public class MigratePasswordsToTokensIT extends StandaloneSiteTest {
  ...
  public void httpPasswordIsBeingMigratedToToken() throws Exception {
    ...
    try (ServerContext ctx = startServer()) {
      GerritApi gApi = ctx.getInjector().getInstance(GerritApi.class);
      List<TokenInfo> actual = gApi.accounts().id(accountId).getTokens();
      assertThat(actual.size()).isEqualTo(1);
      assertThat(actual.get(0).id).isEqualTo(LEGACY_ID);
