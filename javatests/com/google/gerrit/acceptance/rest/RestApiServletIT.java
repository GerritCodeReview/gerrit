...
public class RestApiServletIT extends AbstractDaemonTest {
  ...
  public void testCommentLinkWithPrefixRedirects() throws Exception {
    int changeNumber = createChange().getChange().getId().get();
    CommentsUtil.addComment(gApi, String.valueOf(changeNumber));

    CommentInfo ci = getLastComment(String.valueOf(changeNumber));

    String redirectUri = String.format("/c/%s/+/%d/comment/%s", project.get(), changeNumber, ci.id);

    anonymousRestSession
        .get(String.format("/c/%s/comment/%s", changeNumber, ci.id))
  ...
  public void testCommentLinkWithOutPrefixRedirects() throws Exception {
    ...
    anonymousRestSession
        .get(String.format("/%s/comment/%s", changeNumber, ci.id))
        .assertTemporaryRedirect(redirectUri);
  }

  private CommentInfo getLastComment(String changeId) throws Exception {

    Map<String, List<CommentInfo>> result = gApi.changes().id(changeId).current().comments();
    assertThat(result).isNotEmpty();
    return Iterables.getOnlyElement(result.get(FILE_NAME));
  }
