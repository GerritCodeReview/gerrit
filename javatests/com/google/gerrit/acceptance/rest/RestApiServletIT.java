  ...
  public void xGerritUpdatedRefSetForDifferentWriteRequests() throws Exception {
    ...
    assertThat(response.getHeader(X_GERRIT_UPDATED_REF))
        .isEqualTo(
            String.format(
                "%s~%s~%s~%s",
                project, metaRef, originalMetaRefSha1.getName(), firstMetaRefSha1.getName()));
    ...
    assertThat(response.getHeader(X_GERRIT_UPDATED_REF))
        .isEqualTo(
            String.format(
                "%s~%s~%s~%s",
                project, metaRef, firstMetaRefSha1.getName(), secondMetaRefSha1.getName()));
  ...
  public void xGerritUpdatedRefDeleted() throws Exception {
    ...
    assertThat(headers)
        .containsExactly(
            String.format(
                "%s~%s~%s~%s",
                project, metaRef, originalMetaRefSha1.getName(), ObjectId.zeroId().getName()),
            String.format(
                "%s~%s~%s~%s",
                project,
                patchSetRef,
    ...
    try (Repository repository = repoManager.openRepository(project)) {
      ...
      assertThat(headers)
          .containsExactly(
              ...
              String.format(
                  "%s~%s~%s~%s",
                  project, branch, originalDestinationBranchSha1.getName(), branchSha1));
    }
  }
