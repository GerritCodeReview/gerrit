...
public class IsRobotIT extends AbstractDaemonTest {
  ...
  @Test
  public void setIsRobot_failsForOtherUser() {
    requestScopeOperations.setApiUser(user.id());

    assertThrows(AuthException.class, () -> gApi.accounts().id(admin.id().get()).setIsRobot(true));
    assertThrows(AuthException.class, () -> gApi.accounts().id(admin.id().get()).setIsRobot(false));
  }

  @Test
  public void setIsRobot_succeedsForOtherUserWithModifyAccounts() throws Exception {
    projectOperations
    ...
    gApi.accounts().id(admin.id().get()).setIsRobot(true);
    assertThat(gApi.accounts().id(admin.id().get()).getIsRobot()).isTrue();
    gApi.accounts().id(admin.id().get()).setIsRobot(false);
    assertThat(gApi.accounts().id(admin.id().get()).getIsRobot()).isFalse();
  }
