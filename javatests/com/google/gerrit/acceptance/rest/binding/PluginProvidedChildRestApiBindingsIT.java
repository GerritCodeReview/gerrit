...
public class PluginProvidedChildRestApiBindingsIT extends AbstractDaemonTest {
  ...
  private static final ImmutableSet<RestCall> TEST_CALLS =
      ImmutableSet.of(
          ...
          // Same tests but with the plugin name as part of the collection name. This works as long
          // as there is no core collection with the same name (which takes precedence) and no other
          // plugin binds a collection with the same name. We highly encourage plugin authors to use
          // the fully qualified collection name instead.
