...
public class AttentionSetIT extends AbstractDaemonTest {
  ...
  public void addMultipleUsers() throws Exception {
    ...
    int accountId2 =
        change(r).addToAttentionSet(new AttentionSetInput(admin.email(), "admin"))._accountId;
  ...
  public void removeUser() throws Exception {
    ...
    fakeClock.advance(Duration.ofSeconds(42));
    change(r).removeFromAttentionSet(new AttentionSetInput(user.email(), "removed"));
    AttentionSetUpdate expectedAttentionSetUpdate =
        AttentionSetUpdate.createFromRead(
    ...
    fakeClock.advance(Duration.ofSeconds(42));
    change(r).removeFromAttentionSet(new AttentionSetInput(user.email(), "removed again"));
    assertThat(r.getChange().attentionSet()).containsExactly(expectedAttentionSetUpdate);
  }
  ...
  public void removeUnrelatedUser() throws Exception {
    PushOneCommit.Result r = createChange();
    change(r).removeFromAttentionSet(new AttentionSetInput(user.email(), "foo"));
    assertThat(r.getChange().attentionSet()).isEmpty();
  }

  private ChangeApi change(Result r) throws RestApiException {
