...
public class ChangeMessagesIT extends AbstractDaemonTest {
  ...
  public void listChangeMessages() throws Exception {
    ...
    List<ChangeMessageInfo> messages2 = new ArrayList<>(
        gApi.changes().id(changeNum).get().messages);
  ...
  public void getOneChangeMessage() throws Exception {
    int changeNum = createOneChange();
    List<ChangeMessageInfo> messages = new ArrayList<>(
        gApi.changes().id(changeNum).get(EnumSet.of(ListChangesOption.MESSAGES, ListChangesOption.DETAILED_ACCOUNTS)).messages);
  ...
  private void deleteOneChangeMessage(
      ...
      throws Exception {
    List<ChangeMessageInfo> messagesBefore = getChangeMessages(changeNum);
  ...
  private void assertMessagesAfterDeletion(
      ...
      String deleteReason)
      throws Exception {
    List<ChangeMessageInfo> messagesAfterDeletion = getChangeMessages(changeNum);
  ...
  private List<ChangeMessageInfo> getChangeMessages(int changeNum) throws Exception {
    return new ArrayList<>(
        gApi.changes().id(changeNum).get(EnumSet.of(ListChangesOption.MESSAGES)).messages);
  }
