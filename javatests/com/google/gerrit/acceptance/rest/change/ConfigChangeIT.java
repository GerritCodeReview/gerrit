...
public class ConfigChangeIT extends AbstractDaemonTest {
  ...
  public void notifySectionErasingNotifyConfigs() throws Exception {
    ...
    input.notifyConfigsAdditions.add(notifyConfigInput);
    input.notifyConfigsAdditions.add(notifyConfigInput2);
    gApi.projects().name(project.get()).config(input);
    ...
    assertThat(configInfo.notifyConfigs.containsKey("example2")).isTrue();
  }
  ...
  public void notifySectionsNameMustBeValid() {
    ...
    input.notifyConfigsAdditions.clear();
  ...
  public void notifySectionCantDeleteNonExisting() throws Exception {
    ConfigInput input = new ConfigInput();
    input.notifyConfigsRemovals = new ArrayList<>();
    input.notifyConfigsRemovals.add("non_existing");
    BadRequestException thrown =
        assertThrows(
            BadRequestException.class, () -> gApi.projects().name(project.get()).config(input));
