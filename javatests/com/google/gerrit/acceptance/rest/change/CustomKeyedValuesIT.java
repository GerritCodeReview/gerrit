...
public class CustomKeyedValuesIT extends AbstractDaemonTest {
  ...
  public void addInvalidCustomKeyedValue() throws Exception {
    ...
    BadRequestException thrown =
        assertThrows(BadRequestException.class, () -> addCustomKeyedValues(r, ImmutableMap.of("key=", "value")));
...
*/
  ...
  public void addCustomKeyedValuesWithoutPermissionNotAllowed() throws Exception {
    ...
    AuthException thrown = assertThrows(AuthException.class, () -> addCustomKeyedValues(r, ImmutableMap.of("key1", "value1")));
    assertThat(thrown).hasMessageThat().contains("edit custom keyed values not permitted");
  }

  @Test
  public void addCustomKeyedValuesWithPermissionAllowed() throws Exception {
    PushOneCommit.Result r = createChange();
    ChangeMessageInfo last = getLastMessage(r);
    projectOperations
        .project(project)
        .forUpdate()
        .add(allow(Permission.EDIT_CUSTOM_KEYED_VALUES).ref("refs/heads/master").group(REGISTERED_USERS))
        .update();
    requestScopeOperations.setApiUser(user.id());
    addCustomKeyedValues(r, ImmutableMap.of("key1", "value1"));
    assertThatGet(r).containsExactly("key1", "value1");
  ...
  private void addCustomKeyedValues(PushOneCommit.Result r, ImmutableMap<String, String> toAdd) throws Exception {
    CustomKeyedValuesInput input = new CustomKeyedValuesInput();
    input.add = toAdd;
    change(r).setCustomKeyedValues(input);
  }

  private void removeCustomKeys(PushOneCommit.Result r, ImmutableSet<String> toRemove) throws Exception {
    CustomKeyedValuesInput input = new CustomKeyedValuesInput();
