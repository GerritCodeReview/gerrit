...
public class GetMetaDiffIT extends AbstractDaemonTest {
  ...
  public void metaDiffUnreachableOldSha1UsesDefault() throws Exception {
    PushOneCommit.Result ch1 = createChange();
    PushOneCommit.Result ch2 = createChange();
    gApi.changes().id(ch1.getChangeId()).topic("new-topic");
    gApi.changes().id(ch1.getChangeId()).topic(null);
    ...
    assertThat(difference.added().topic).isNull();
  ...
  public void metaDiffNoOldMetaGivenUsesLastPatchSet() throws Exception {
    ...
    chApi.topic(newTopic);
    ChangeInfo info = chApi.get();

    ChangeInfoDifference difference = chApi.metaDiff(info.metaRevId);

    assertThat(difference.added().topic).isEqualTo(newTopic);
    assertThat(difference.removed().topic).isNull();
  }
  ...
  public void metaDiffNoNewMetaGivenUsesCurrentPatchSet() throws Exception {
    ...
    ChangeInfoDifference difference = chApi.metaDiffWithOld(info.metaRevId);
  ...
  public void metaDiffWithoutOptionDoesNotIncludeExtraInformation() throws Exception {
    ...
    ChangeInfoDifference difference = gApi.changes().id(ch.getChangeId()).metaDiff(info.metaRevId);
  ...
  public void metaDiffWithOptionIncludesExtraInformation() throws Exception {
    PushOneCommit.Result ch = createChange();
    ChangeApi chApi = gApi.changes().id(ch.getChangeId());
    ChangeInfo info1 = chApi.get(ListChangesOption.CURRENT_REVISION);
    amendChange(ch.getChangeId());
    ChangeInfo info2 = chApi.get(ListChangesOption.CURRENT_REVISION);

    ChangeInfoDifference difference =
        chApi.metaDiff(info2.metaRevId, ListChangesOption.CURRENT_REVISION);

    assertThat(difference.added().currentRevision).isEqualTo(info2.currentRevision);
    assertThat(difference.removed().currentRevision).isEqualTo(info1.currentRevision);
  }
}
