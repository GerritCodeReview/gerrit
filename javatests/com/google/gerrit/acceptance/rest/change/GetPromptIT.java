...
public class GetPromptIT extends AbstractDaemonTest {
  ...
  public void returnsPatchContent_whenCommandIsPatchContent() throws Exception {
    ...
    RestResponse r =
        adminRestSession.get("/changes/" + changeId + "/revisions/current/ai_prompt?command=patch_content");
    r.assertOK();
  ...
  public void returnsCodeReviewPromptTemplate_whenCommandIsCodeReview() throws Exception {
    Path siteDir = createPromptFile("code_review_prompt_template.txt", "This is the code review template with {{Patch}}");
  ...
  public void returnsCommitMessagePromptTemplate_whenCommandCommitMessage() throws Exception {
    Path siteDir = createPromptFile("commit_Message_prompt_template.txt", "This is the commit message template with {{Patch}}");
    SitePaths sitePaths = new SitePaths(siteDir);
    GetPatch getPatch = fetchPatchContent();
    GetPrompt getPrompt = new GetPrompt(sitePaths, getPatch);
  ...
  private static GetPatch fetchPatchContent() throws BadRequestException, ResourceConflictException, IOException, ResourceNotFoundException {
  ...
  public void returnsBadRequest_whenCommandMissing() throws Exception {
    ...
    RestResponse r =
        adminRestSession.get("/changes/" + changeId + "/revisions/current/ai_prompt");
    r.assertBadRequest();
  }
