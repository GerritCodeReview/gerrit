...
public class SubmitByMergeIfNecessaryIT extends AbstractSubmitByMerge {
  ...
  @Test
  public void submitTwoWithFastForwardWhereOneAlreadyAvailableInDifferentBranch() throws Throwable {
    RevCommit tip = projectOperations.project(project).getHead("master");
    RevCommit c1 = testRepo.commit().parent(tip).message("c1").insertChangeId().create();
    RevCommit c2 = testRepo.commit().parent(c1).message("c2").insertChangeId().create();
    testRepo.reset(c2);
    ...
    createBranchWithRevision(BranchNameKey.create(project, "test"), tip.name());
    testRepo.reset(c1);
    RevCommit c3 = testRepo.commit().parent(c1).message("c3").insertChangeId().create();
    testRepo.reset(c3);
    GitUtil.pushHead(testRepo, "refs/for/test%base=" + tip.name());
    int changeId_1 = Iterables.getOnlyElement(changeQuery.get().byCommit(c1)).getId().get();
    int changeId_3 = Iterables.getOnlyElement(changeQuery.get().byCommit(c3)).getId().get();
    approve(Integer.toString(changeId_1));
    approve(Integer.toString(changeId_3));
    submit(Integer.toString(changeId_3));
  }
}
