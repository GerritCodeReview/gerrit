...
public class SubmitWholeTopicIT extends AbstractDaemonTest {
  ...
  /**
   * Concurrency test: user A calls submit.topic while user B concurrently submits one of the topic
   * changes individually.
   *
   * <p>The two changes live in separate projects so their ref locks do not contend. An {@link
   * com.google.gerrit.server.git.validators.OnSubmitValidationListener} is used as the
   * synchronisation point. It fires inside thread A's {@code BatchUpdate}, right before a branch
   * ref is written. At that moment:
   *
   * <ol>
   *   <li>Thread A counts down {@code submitTopicInProgress} to tell thread B it is safe to race.
   *   <li>Thread A blocks on {@code concurrentSubmitDone}, waiting for thread B to finish.
   *   <li>Thread B submits changeB individually via a normal REST call.
   *   <li>Thread B counts down {@code concurrentSubmitDone}, unblocking thread A.
   * </ol>
   *
   * <p>Thread A either succeeds on the first attempt or retries with {@code allowMerged=true},
   * skipping the already-merged changeB. Either way both changes must be MERGED afterwards.
   */
  @Test
  @GerritConfig(name = "change.submitWholeTopic", value = "OPTIONAL")
  public void submitWholeTopic_concurrentSingleSubmitInTopic_bothChangesEndUpMerged()
      throws Exception {
    String topic = "concurrent-topic";

    // Use two separate projects so the ref locks for the two changes never
    // contend with each other, keeping the retry logic simple.
    Project.NameKey projectA = projectOperations.newProject().create();
    Project.NameKey projectB = projectOperations.newProject().create();
    projectOperations
        .project(projectA)
    ...
    // Latch 1: thread A signals thread B once submit.topic is inside
    //          preBranchUpdate (right before a ref write).
    CountDownLatch submitTopicInProgress = new CountDownLatch(1);
    // Latch 2: thread B signals thread A that its concurrent single submit finished.
    CountDownLatch concurrentSubmitDone = new CountDownLatch(1);

    AtomicReference<Throwable> listenerError = new AtomicReference<>();
    try (Registration reg =
        extensionRegistry
            .newRegistration()
            .add(
                (OnSubmitValidationListener)
                    args -> {
                      // Pause only on the very first preBranchUpdate call.
                      // Retries must not be paused; they would deadlock.
                      if (submitTopicInProgress.getCount() == 0) {
                        return;
                      }
                      submitTopicInProgress.countDown();
                      try {
                        if (!concurrentSubmitDone.await(30, TimeUnit.SECONDS)) {
                          listenerError.set(
                              new AssertionError("concurrentSubmitDone latch timed out"));
                        }
      ...
      try (ExecutorService executor = Executors.newSingleThreadExecutor()) {
        // Thread A runs submit.topic on a background thread so the test
        // thread (thread B) can race against it.
        Future<RestResponse> threadA =
            executor.submit(
                () -> adminRestSession.post("/changes/" + changeA.getChangeId() + "/submit.topic"));

        // Wait until thread A has entered preBranchUpdate.
        assertThat(submitTopicInProgress.await(30, TimeUnit.SECONDS)).isTrue();

        // Thread B: submit changeB individually while thread A is paused.
        // Because the two projects have separate ref namespaces there is no
        // ref-lock contention. Both 200 and 409 are valid outcomes.
        RestResponse concurrentResponse =
            adminRestSession.post("/changes/" + changeB.getChangeId() + "/submit");
        assertThat(concurrentResponse.getStatusCode())
            .isAnyOf(HttpStatus.SC_OK, HttpStatus.SC_CONFLICT);
        concurrentSubmitDone.countDown();

        // Thread A resumes and must succeed, retrying with allowMerged=true if
        // changeB was already merged.
        RestResponse topicResponse = threadA.get(30, TimeUnit.SECONDS);
  ...
  /**
   * Concurrency test: user A calls submit.topic across two projects while user B concurrently
   * submits the project-B change individually.
   *
   * <p>Cross-project topic submission is <em>not atomic</em>: project A and project B are updated
   * in separate {@code BatchUpdate} executions. The listener pauses thread A after project A's refs
   * are prepared but before they are written, giving thread B a window to race on project B.
   *
   * <p>Synchronisation:
   *
   * <ol>
   *   <li>Thread A's listener fires for project A, counts down {@code projectAInProgress}, then
   *       blocks on {@code concurrentSubmitDone}.
   *   <li>Thread B submits changeB individually.
   *   <li>Thread B counts down {@code concurrentSubmitDone}.
   *   <li>Thread A continues; it either succeeds or retries with {@code allowMerged=true} for
   *       changeB.
   * </ol>
   */
  @Test
  @GerritConfig(name = "change.submitWholeTopic", value = "OPTIONAL")
  public void submitWholeTopic_concurrentSingleSubmitInOtherProject_bothChangesEndUpMerged()
      throws Exception {
    String topic = "cross-project-concurrent";
    ...
    // Latch 1: fires when thread A enters preBranchUpdate for project A
    //          (project A ref about to be written; project B not started yet).
    CountDownLatch projectAInProgress = new CountDownLatch(1);
    // Latch 2: fires when thread B's concurrent single-change submit finishes.
    CountDownLatch concurrentSubmitDone = new CountDownLatch(1);
    ...
    try (Registration reg =
        extensionRegistry
            ...
            .add(
                (OnSubmitValidationListener)
                    args -> {
                      // Only pause on the project-A call (project B must not be
                      // blocked — that is where the race happens). Skip retries.
                      if (!args.getProject().equals(projectA)
                          || projectAInProgress.getCount() == 0) {
                        return;
                      }
                      projectAInProgress.countDown();
                      try {
                        if (!concurrentSubmitDone.await(30, TimeUnit.SECONDS)) {
                          listenerError.set(
                              new AssertionError("concurrentSubmitDone latch timed out"));
      ...
      try (ExecutorService executor = Executors.newSingleThreadExecutor()) {
        ...
        // Wait until thread A has started writing project A (paused in listener).
        assertThat(projectAInProgress.await(30, TimeUnit.SECONDS)).isTrue();

        // Thread B: submit changeB individually while thread A is between projects.
        RestResponse concurrentResponse =
            adminRestSession.post("/changes/" + changeB.getChangeId() + "/submit");
        assertThat(concurrentResponse.getStatusCode())
            .isAnyOf(HttpStatus.SC_OK, HttpStatus.SC_CONFLICT);
  ...
  /**
   * Verifies that if the triggering change was already merged by a concurrent submit before
   * submit.topic is processed, the endpoint returns 409 Conflict rather than silently succeeding or
   * throwing an internal error.
   *
   * <p>This is an important correctness boundary: the response should be deterministic even when
   * the race is lost entirely.
   */
  @Test
  @GerritConfig(name = "change.submitWholeTopic", value = "OPTIONAL")
  public void submitWholeTopic_triggeringChangeAlreadyMerged_returns409() throws Exception {
    String topic = "already-merged-topic";
    PushOneCommit.Result change1 = createChange("Change 1", "a.txt", "content", topic);
    PushOneCommit.Result change2 = createChange("Change 2", "b.txt", "content", topic);
    approve(change1.getChangeId());
    approve(change2.getChangeId());

    // Simulate the race: a concurrent normal submit merges change1 before
    // submit.topic gets to run. OPTIONAL mode only submits change1, leaving
    // change2 open.
    gApi.changes().id(change1.getChangeId()).current().submit();
    assertMerged(change1.getChangeId());
    assertThat(gApi.changes().id(change2.getChangeId()).get().status).isEqualTo(ChangeStatus.NEW);

    // submit.topic on the already-merged change must fail: mergeChange() checks
    // change.isNew() and raises ResourceConflictException -> 409.
    RestResponse response =
        adminRestSession.post("/changes/" + change1.getChangeId() + "/submit.topic");
    response.assertConflict();

    // change2 was not submitted by either operation.
    assertThat(gApi.changes().id(change2.getChangeId()).get().status).isEqualTo(ChangeStatus.NEW);
  }
