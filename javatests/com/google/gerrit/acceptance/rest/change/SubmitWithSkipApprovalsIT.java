...
public class SubmitWithSkipApprovalsIT extends AbstractDaemonTest {
  @Inject
  ...
  public void submitWithSkipApprovals() throws Throwable {
    ...
    // Check footers
    assertWithMessage(head.getFullMessage()).that(head.getFooterLines(FooterConstants.CHANGE_ID)).isNotEmpty();
    assertWithMessage(head.getFullMessage()).that(head.getFooterLines(FooterConstants.REVIEWED_ON)).isNotEmpty();
  ...
  public void submitWithAddApprovals() throws Throwable {
    ...
    assertWithMessage(head.getFullMessage()).that(head.getFooterLines(FooterConstants.CHANGE_ID)).isNotEmpty();
    assertWithMessage(head.getFullMessage()).that(head.getFooterLines(FooterConstants.REVIEWED_ON)).isNotEmpty();
    assertWithMessage(head.getFullMessage()).that(head.getFooterLines(FooterConstants.REVIEWED_BY)).isNotEmpty();
  }
}
