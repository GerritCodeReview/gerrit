...
public class SuggestReviewersIT extends AbstractDaemonTest {
  ...
  //   @Test
  // public void suggestRecentReviewerCaseInsensitivelyViaAccountMatchesQuery() throws Exception {
  //     requestScopeOperations.setApiUser(admin.id());

  //     // This makes user4 a "recent reviewer" for admin
  //     String previousChangeId = createChange().getChangeId();
  //     ReviewerInput addReviewerInput = new ReviewerInput();
  //     addReviewerInput.reviewer = user4.email();
  //     gApi.changes().id(previousChangeId).addReviewer(addReviewerInput);

  //     String targetChangeId = createChange().getChangeId();

  //     // Query for "john" (lowercase) to match user4's full name "John Doe".
  //     List<SuggestedReviewerInfo> suggestionsByFullName =
  //         suggestReviewers(targetChangeId, "john", 5);

  //     assertThat(
  //             suggestionsByFullName.stream()
