...
public class SuggestReviewersIT extends AbstractDaemonTest {
  ...
  public void suggestsUsersWhoRecentlyTouchedSameFilesFirstWhenQueried() throws Exception {
    ...
    List<Integer> reviewerIds = reviewers.stream().map(r -> r.account._accountId).collect(toList());
    assertThat(reviewerIds.subList(0, 2)).containsExactly(user1.id().get(), user2.id().get());
