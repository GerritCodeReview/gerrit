...
public class IndexChangesIT extends AbstractDaemonTest {
    ...
    try (Registration registration =
      ...
      IndexChanges.Input in =
          new IndexChanges.Input(
              ImmutableSet.of(projectAndChangeNumId(project, change.getChange().getId())),
              PRESERVE_MISSING);
  ...
  public void indexChangeNotInIndex() throws Exception {
    ...
    adminRestSession
        .post(
            ...
            new IndexChanges.Input(
                ImmutableSet.of(projectAndChangeNumId(project, changeId)), PRESERVE_MISSING))
        .assertOK();
    assertThat(getChangeFromIndex(changeId)).isPresent();
    ...
    adminRestSession
        .post(
            "/config/server/index.changes",
            new IndexChanges.Input(
                ImmutableSet.of(projectAndChangeNumId(project, changeId)), DELETE_MISSING))
        .assertOK();
    assertThat(getChangeFromIndex(changeId)).isEmpty();
  }
  ...
  public void indexNonVisibleChange() throws Exception {
    ...
    try (Registration registration =
      ...
      projectOperations
          ...
          .update();
      IndexChanges.Input in = new IndexChanges.Input(ImmutableSet.of(changeId), PRESERVE_MISSING);
      changeIndexedCounter.clear();
      adminRestSession.post("/config/server/index.changes", in).assertOK();
  ...
  public void indexChangeWithPlainNumericIdAccepted() throws Exception {
    ...
    IndexChanges.Input in =
        new IndexChanges.Input(ImmutableSet.of(String.valueOf(changeId.get())), PRESERVE_MISSING);
    adminRestSession.post("/config/server/index.changes", in).assertOK();
  }
  ...
  public void deleteMissingChangeFromIndexWithPlainNumericIdRejected() throws Exception {
    IndexChanges.Input in =
        new IndexChanges.Input(ImmutableSet.of(TEST_CHANGE_NUM), DELETE_MISSING);
  ...
  public void indexChangeWithTripletIdAccepted() throws Exception {
    ...
    IndexChanges.Input in =
        new IndexChanges.Input(ImmutableSet.of(project + "~master~" + changeId), PRESERVE_MISSING);
    adminRestSession.post("/config/server/index.changes", in).assertOK();
  }
  ...
  public void deleteMissingChangeFromIndexWithTripletIdRejected() throws Exception {
    IndexChanges.Input in =
        new IndexChanges.Input(
            ImmutableSet.of(project + "~master~" + TEST_CHANGE_ID), DELETE_MISSING);
    adminRestSession.post("/config/server/index.changes", in).assertBadRequest();
  }
