...
public class IndexFlushIT extends AbstractDaemonTest {
  ...
  public void flushAndCommitInvalidIndex() throws Exception {
    RestResponse response = adminRestSession.post("/config/server/indexes/invalidIndex/commit");

    response.assertNotFound();
  }

  @Test
  public void flushAndCommitForbiddenForUnathorisedUsers() throws Exception {
    RestResponse response = userRestSession.post("/config/server/indexes/changes/commit");
  ...
  private void assertIndexFlushAndCommit(String indexName) throws Exception {
    RestResponse response =
        adminRestSession.post(String.format("/config/server/indexes/%s/commit", indexName));
