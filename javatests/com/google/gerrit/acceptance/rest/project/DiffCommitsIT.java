...
public class DiffCommitsIT extends AbstractDaemonTest {
    ...
    ResourceNotFoundException thrown =
        assertThrows(
            ...
            () ->
                gApi.projects()
                    .name(project.get())
                    .diffFiles(initialCommit.name(), nonExistentSha));
    ...
    ResourceNotFoundException thrown =
        assertThrows(
            ...
            () ->
                gApi.projects()
                    .name(project.get())
                    .diffFiles(newCommit.name(), initialCommit.name()));
  ...
  public void listDiffFiles_includesFilesFromIntermediateCommits() throws Exception {
    ...
    Map<String, FileInfo> files =
        gApi.projects().name(project.get()).diffFiles(initialCommit.name(), commit2.name());
  ...
  public void listDiffFiles_includesFilesFromMultipleIntermediateCommits() throws Exception {
    ...
    Map<String, FileInfo> files =
        gApi.projects().name(project.get()).diffFiles(initialCommit.name(), commit4.name());
  ...
  public void listDiffFiles_includesFilesAfterRebase() throws Exception {
    ...
    Map<String, FileInfo> files =
        gApi.projects().name(project.get()).diffFiles(initialCommit.name(), rebasedCommitSha);
