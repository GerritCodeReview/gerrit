...
public class GetCommitIT extends AbstractDaemonTest {
  ...
  public void getOpenChange_NotFound() throws Exception {
    // Need to unblock read to allow the push operation to succeed if not, when retrieving the advertised refs during 
	// the push, the client won't be sent the initial commit and will send it again as part of the change.
    unblockRead();
    PushOneCommit.Result r = pushFactory.create(admin.newIdent(), testRepo).to("refs/for/master");
    r.assertOkStatus();
