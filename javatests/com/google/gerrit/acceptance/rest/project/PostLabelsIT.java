...
public class PostLabelsIT extends AbstractDaemonTest {
  ...
  public void anonymous() throws Exception {
    ...
    AuthException thrown =
        assertThrows(
            AuthException.class,
            () -> gApi.projects().name(allProjects.get()).labelBatchUpdate(new PostLabelsInput()));
  ...
  public void deleteNonExistingLabel() throws Exception {
    PostLabelsInput input = new PostLabelsInput();
    input.delete = ImmutableList.of("Foo");

    UnprocessableEntityException thrown =
        assertThrows(
            UnprocessableEntityException.class,
            () -> gApi.projects().name(allProjects.get()).labelBatchUpdate(input));
  ...
  public void deleteLabels() throws Exception {
    ...
    gApi.projects().name(project.get()).labelBatchUpdate(input);
    ResourceNotFoundException thrown =
        assertThrows(
            ResourceNotFoundException.class,
            () -> gApi.projects().name(project.get()).label("Foo").get());
    assertThat(thrown).hasMessageThat().contains("Not found: Foo");
    thrown =
        assertThrows(
            ResourceNotFoundException.class,
            () -> gApi.projects().name(project.get()).label("Bar").get());
    assertThat(thrown).hasMessageThat().contains("Not found: Bar");
  }

  @Test
  public void deleteLabelsLabelNamesAreTrimmed() throws Exception {
    configLabel("Foo", LabelFunction.NO_OP);
    configLabel("Bar", LabelFunction.NO_OP);
  ...
  @Test
  public void createLabelsLabelNamesAreTrimmed() throws Exception {
    LabelDefinitionInput fooInput = new LabelDefinitionInput();
    fooInput.name = " Foo ";
    fooInput.values = ImmutableMap.of("+1", "Looks Good", " 0", "Don't Know", "-1", "Looks Bad");
  ...
  @Test
  public void updateLabelsLabelNameAreTrimmed() throws Exception {
    configLabel("Foo", LabelFunction.NO_OP);
    configLabel("Bar", LabelFunction.NO_OP);
