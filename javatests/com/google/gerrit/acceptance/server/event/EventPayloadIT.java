...
public class EventPayloadIT extends AbstractDaemonTest {
  ...
  @Test
  @GerritConfig(name = "event.payload.listChangeOptions", value = "CURRENT_FILES")
  public void configuredOptions() throws Exception {
    createChange();

    // We are not skipping MERGEABLE, so we expect a value
    assertThat(lastEvent.getChange().mergeable).isNotNull();
