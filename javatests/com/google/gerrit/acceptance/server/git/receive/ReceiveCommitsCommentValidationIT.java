...
public class ReceiveCommitsCommentValidationIT extends AbstractDaemonTest {
  ...
  @Test
  public void validateComments_attentionSetUpdatedReviewerAdded() throws Exception {
    PushOneCommit.Result result = createChange();
    String changeId = result.getChangeId();
  ...
  @Test
  public void validateComments_attentionSetUpdatedReviewerNotAddedWhenRemoved() throws Exception {
    PushOneCommit.Result result = createChange();
    String changeId = result.getChangeId();
    gApi.changes().id(changeId).addReviewer(user.email());
    gApi.changes().id(changeId).attention(user.email()).remove(new AttentionSetInput("removed"));
  ...
  @Test
  public void validateComments_attentionSetNotUpdatedWhenNoCommentsPublished() throws Exception {
    PushOneCommit.Result result = createChange();
    String changeId = result.getChangeId();
    gApi.changes().id(changeId).addReviewer(user.email());
    gApi.changes().id(changeId).attention(user.email()).remove(new AttentionSetInput("removed"));
