...
public class ReceiveCommitsIT extends AbstractDaemonTest {
  ...
  public void validateComments_commentOK() throws Exception {
    ...
    DraftInput comment = testCommentUtil.newDraft(COMMENT_TEXT);
    testCommentUtil.addDraft(changeId, revId, comment);
    assertThat(testCommentUtil.getPublishedComments(result.getChangeId())).isEmpty();
    ...
    assertThat(testCommentUtil.getPublishedComments(result.getChangeId())).hasSize(1);
  }
  ...
  public void validateComments_inlineVsFileComments_allOK() throws Exception {
    ...
    testCommentUtil.addDraft(changeId, revId, draftFile);
    DraftInput draftInline = testCommentUtil.newDraft(
        result.getChange().currentFilePaths().get(0), Side.REVISION, 1, COMMENT_TEXT);
    testCommentUtil.addDraft(changeId, revId, draftInline);
    assertThat(testCommentUtil.getPublishedComments(result.getChangeId())).isEmpty();
    amendChange(changeId, "refs/for/master%publish-comments", admin, testRepo);
    assertThat(testCommentUtil.getPublishedComments(result.getChangeId())).hasSize(2);
