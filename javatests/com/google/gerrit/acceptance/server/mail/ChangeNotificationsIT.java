...
public class ChangeNotificationsIT extends AbstractNotificationTest {
  ...
  private void testMergeByOwnerAllSubmitStrategies(boolean advanceBranchBeforeSubmitting)
    ...
    for (SubmitType submitType : SubmitType.values()) {
      ...
      String nameSuffix = advanceBranchBeforeSubmitting ? " after branch has advanced" : ""
      assertThat(sender)
          .named("%s sender%s", submitType, nameSuffix)
          .sent("merged", sc)
          .cc(sc.reviewer, sc.ccer)
          .cc(sc.reviewerByEmail, sc.ccerByEmail)
          .bcc(sc.starrer)
          ...
          .noOneElse();
      assertThat(sender).named("%s sender%s", submitType, nameSuffix).didNotSend();
    }
  }
