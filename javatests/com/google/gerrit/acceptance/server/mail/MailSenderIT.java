...
public class MailSenderIT extends AbstractMailIT {
  ...
  public void outgoingMailWithACommentsInUnchangedFile() throws Exception {
    ...
    CommentInfo commentInfo = comments.get(1);
    ChangeInfo changeInfo = gApi.changes().id(changeId).get();
    String expectedBodyAsString =
        emailBodyWithCommentsInUnchangeFile(
            getChangeUrl(changeInfo), commentInfo.id, adminInlineComment, userInlineComment);
    String bodyAsString = sender.getMessages().iterator().next().body();
    assertTrue(
        "The expected body is not in the received body",
        bodyAsString.contains(expectedBodyAsString));
  }
