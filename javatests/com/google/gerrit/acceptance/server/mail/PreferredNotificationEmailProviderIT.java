...
public class PreferredNotificationEmailProviderIT extends LightweightPluginDaemonTest {
  ...
  public void validatePreferredNotificationEmailAddress() throws Exception {
    ...
    assertThat(sender.getMessages().get(0).rcpt().size()).isEqualTo(2);
    Set<String> rcpts =
        sender.getMessages().get(0).rcpt().stream().map(a -> a.email()).collect(Collectors.toSet());
    assertThat(rcpts).contains(SET_BY_PLUGIN + admin.email());
    assertThat(rcpts).contains(SET_BY_PLUGIN + user.email());
  }
