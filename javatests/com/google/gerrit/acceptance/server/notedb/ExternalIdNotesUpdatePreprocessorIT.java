...
import com.google.gerrit.server.account.externalids.ExternalIdUpdatePreprocessor;
...
/** Tests {@link ExternalIdUpdatePreprocessor}. */
@TestPlugin(
    name = "external-id-update-preprocessor",
    sysModule =
        "com.google.gerrit.acceptance.server.notedb.ExternalIdNotesUpdatePreprocessorIT$Module")
public class ExternalIdNotesUpdatePreprocessorIT extends LightweightPluginDaemonTest {
  ...
  public static class Module extends AbstractModule {
    ...
    protected void configure() {
      DynamicItem.bind(binder(), ExternalIdUpdatePreprocessor.class).to(TestPreprocessor.class);
    }
  }
  ...
  @Test
  public void insert_update() throws Exception {
    ...
    assertThat(testPreprocessor.removed).isEmpty();

    testPreprocessor.reset();
    accountsUpdateProvider.get().update("test", id, u -> u.deleteExternalId(extId));
    assertThat(testPreprocessor.upserted).isEmpty();
    assertThat(testPreprocessor.removed).containsExactly(extId);
  }
  ...
  public void replaceByKeys() throws Exception {
    ...
    assertThat(testPreprocessor.removed).containsExactly(extId1);
  }
  ...
  @Test
  public void upsert_deleteByKey() throws Exception {
    Account.Id id = Account.id(sequences.nextAccountId());
    ...
    try (Repository allUsersRepo = repoManager.openRepository(allUsers);
        MetaDataUpdate md = metaDataUpdateFactory.create(allUsers)) {
      ExternalIdNotes extIdNotes = extIdNotesFactory.load(allUsersRepo);
      extIdNotes.deleteByKeys(ImmutableSet.of(extId2.key()));
      extIdNotes.commit(md);
    }
    assertThat(testPreprocessor.upserted).isEmpty();
    assertThat(testPreprocessor.removed).containsExactly(extId2);
  }

  @Test
  public void delete_byKey() throws Exception {
    Account.Id id = Account.id(sequences.nextAccountId());
    ExternalId extId = ExternalId.create("foo", "bar", id);
    accountsUpdateProvider.get().insert("test", id, u -> u.addExternalId(extId));
    testPreprocessor.reset();
    try (Repository allUsersRepo = repoManager.openRepository(allUsers);
        MetaDataUpdate md = metaDataUpdateFactory.create(allUsers)) {
      ExternalIdNotes extIdNotes = extIdNotesFactory.load(allUsersRepo);
      extIdNotes.delete(id, ImmutableSet.of(extId.key()));
      extIdNotes.commit(md);
    }
    assertThat(testPreprocessor.upserted).isEmpty();
    assertThat(testPreprocessor.removed).containsExactly(extId);
  ...
  public void delete_byId() throws Exception {
    ...
    try (Repository allUsersRepo = repoManager.openRepository(allUsers);
        MetaDataUpdate md = metaDataUpdateFactory.create(allUsers)) {
      ExternalIdNotes extIdNotes = extIdNotesFactory.load(allUsersRepo);
      extIdNotes.delete(ImmutableSet.of(extId));
      extIdNotes.commit(md);
    }
    assertThat(testPreprocessor.upserted).isEmpty();
    assertThat(testPreprocessor.removed).containsExactly(extId);
  ...
  public void blockUpsert() throws Exception {
    Account.Id id = Account.id(sequences.nextAccountId());
    ExternalId extId = ExternalId.create("foo", "bar", id);
    testPreprocessor.throwOnUpsert = true;
    StorageException e =
        assertThrows(
  ...
  public void blockRemove() throws Exception {
    ...
    accountsUpdateProvider.get().insert("test", id, u -> u.addExternalId(extId));
    testPreprocessor.reset();
    testPreprocessor.throwOnRemove = true;
    StorageException e =
        assertThrows(
            StorageException.class,
            () -> accountsUpdateProvider.get().update("test", id, u -> u.deleteExternalId(extId)));
    assertThat(e).hasMessageThat().contains("remove not good");
    assertThat(testPreprocessor.upserted).isEmpty();
    assertThat(testPreprocessor.removed).isEmpty();
  }
  ...
  @Singleton
  public static class TestPreprocessor implements ExternalIdUpdatePreprocessor {
    List<ExternalId> upserted = new ArrayList<>();
    List<ExternalId> removed = new ArrayList<>();

    boolean throwOnUpsert = false;
    boolean throwOnRemove = false;

    @Override
    public void upsert(ExternalId extId) {
      assertThat(extId.blobId()).isNull();
      if (throwOnUpsert) {
    ...
    public void remove(ExternalId extId) {
      assertThat(extId.blobId()).isNull();
      if (throwOnRemove) {
        throw new StorageException("remove not good");
      }
      removed.add(extId);
    }
