...
public class ExternalUserPermissionIT extends AbstractDaemonTest {
  ...
  private final AccountGroup.UUID externalGroup = AccountGroup.uuid("company-auth:it-department");
  ...
  public void defaultRefFilter_changeVisibilityIsAgnosticOfProvidedGroups() throws Exception {
    ExternalUser user =
        externalUserFactory.create(
            ImmutableSet.of(ExternalId.Key.parse("company-auth:it-department-1")));
  ...
  public void externalUser_isContainedInternalGroupThatContainsExternalGroup() {
    AccountGroup.UUID internalGroup =
        groupOperations.newGroup().addSubgroup(externalGroup).create();
    ExternalUser user =
        externalUserFactory.create(
            ImmutableSet.of(ExternalId.Key.parse("company-auth:it-department-1")));
    assertThat(user.getEffectiveGroups().contains(internalGroup)).isTrue();
    assertThat(user.getEffectiveGroups().contains(externalGroup)).isTrue();
