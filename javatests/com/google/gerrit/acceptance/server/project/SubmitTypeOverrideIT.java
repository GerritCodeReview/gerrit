...
public class SubmitTypeOverrideIT extends AbstractDaemonTest {
    ...
    updateProjectConfig(
        cfg -> {
          ...
          cfg.upsertSubmitType(
              SubmitTypeOverrideRule.builder()
                  .setType(CHERRY_PICK)
                  .setApplicabilityExpression("branch:cherry")
                  .build());
          cfg.upsertSubmitType(
              SubmitTypeOverrideRule.builder()
                  .setType(FAST_FORWARD_ONLY)
                  .setApplicabilityExpression("branch:ff")
                  .build());
        });
    ...
    updateProjectConfig(
        cfg ->
            cfg.upsertSubmitType(
                SubmitTypeOverrideRule.builder()
                    .setType(MERGE_ALWAYS)
                    .setApplicabilityExpression("branch:feature")
                    .build()));
    assertSubmitType(MERGE_ALWAYS, r.getChangeId());
    ...
    updateProjectConfig(
        cfg -> {
          ...
          cfg.upsertSubmitType(
              SubmitTypeOverrideRule.builder()
                  .setType(FAST_FORWARD_ONLY)
                  .setApplicabilityExpression("branch:release")
                  .build());
          ...
          cfg.upsertSubmitType(
              SubmitTypeOverrideRule.builder()
                  .setType(CHERRY_PICK)
                  .setApplicabilityExpression("branch:new-feature")
                  .build());
        });
  ...
  /**
   * An override defined in a parent project is inherited by a child project when the child defines
   * no override for that submit type.
   */
  @Test
  public void inheritance_parentOverrideAppliesInChild() throws Exception {
    Project.NameKey parent =
        createProjectOverAPI("parent", allProjects, /* createEmptyCommit= */ true, null);
    Project.NameKey child =
        createProjectOverAPI("child", parent, /* createEmptyCommit= */ true, null);

    // Parent defines MERGE_ALWAYS for branch:stable; project default is MERGE_IF_NECESSARY.
    updateProjectConfigFor(
        parent,
        cfg -> {
          cfg.updateProject(p -> p.setSubmitType(MERGE_IF_NECESSARY));
          cfg.upsertSubmitType(
    ...
    Project.NameKey savedProject = project;
    project = child;
    testRepo = cloneProject(child);
    ...
    // Inherited override applies on stable, project default on master.
    assertSubmitType(MERGE_ALWAYS, onStable.getChangeId());
    assertSubmitType(MERGE_IF_NECESSARY, onMaster.getChangeId());

    project = savedProject;
  }

  /**
   * A child project can override a parent's override for the same submit type. Both parent and
   * child overrides are present in the effective list, with child overrides evaluated first. When
   * parent and child define different branch expressions for the same submit type key, both remain
   * active: the child's expression is checked first, and the parent's expression is checked second.
   *
   * <p>This test uses distinct branches to show that neither level "removes" the other's override —
   * they coexist, with child evaluated first.
   */
  @Test
  public void inheritance_childOverridesParentByDefault() throws Exception {
    Project.NameKey parent =
        createProjectOverAPI("parent", allProjects, /* createEmptyCommit= */ true, null);
    Project.NameKey child =
        createProjectOverAPI("child", parent, /* createEmptyCommit= */ true, null);

    // Parent: CHERRY_PICK for branch:stable.
    updateProjectConfigFor(
        parent,
        cfg -> {
          cfg.updateProject(p -> p.setSubmitType(REBASE_IF_NECESSARY));
    ...
    // Child: MERGE_ALWAYS for branch:feature — a different branch.
    updateProjectConfigFor(
        child,
        cfg -> {
          cfg.updateProject(p -> p.setSubmitType(SubmitType.INHERIT));
          cfg.upsertSubmitType(
              SubmitTypeOverrideRule.builder()
                  .setType(MERGE_ALWAYS)
                  .setApplicabilityExpression("branch:feature")
    ...
    PushOneCommit.Result seed = createChangeOnBranch("master");
    RevCommit branchPoint = seed.getCommit().getParent(0);
    for (String b : new String[] {"stable", "feature"}) {
      gApi.projects()
          .name(child.get())
          .branch(b)
          .create(
    ...
    PushOneCommit.Result onStable = createChangeOnBranch("stable");
    testRepo.reset(branchPoint);
    PushOneCommit.Result onFeature = createChangeOnBranch("feature");
    PushOneCommit.Result onMaster = createChangeOnBranch("master");

    // Both overrides are active; child's is evaluated first.
    // stable → CHERRY_PICK (parent's override, child has no override for stable).
    // feature → MERGE_ALWAYS (child's override).
    // master → project default (no override matches).
    assertSubmitType(CHERRY_PICK, onStable.getChangeId());
    assertSubmitType(MERGE_ALWAYS, onFeature.getChangeId());
    assertSubmitType(REBASE_IF_NECESSARY, onMaster.getChangeId());
  ...
  /**
   * Child's override wins over parent's for the same branch: child overrides are evaluated first
   * regardless of submit type key.
   *
   * <pre>All-Projects → parent[MERGE_ALWAYS/stable] → child[CHERRY_PICK/stable]</pre>
   *
   * Child's CHERRY_PICK entry is evaluated before the parent's MERGE_ALWAYS entry, so CHERRY_PICK
   * wins for branch:stable.
   */
  @Test
  public void inheritance_childOverrideWinsForSameBranch() throws Exception {
    Project.NameKey parent =
        createProjectOverAPI("parent", allProjects, /* createEmptyCommit= */ true, null);
    Project.NameKey child =
        createProjectOverAPI("child", parent, /* createEmptyCommit= */ true, null);

    // Parent: MERGE_ALWAYS for branch:stable.
    updateProjectConfigFor(
        parent,
        cfg -> {
          cfg.updateProject(p -> p.setSubmitType(REBASE_IF_NECESSARY));
    ...
    // Child: CHERRY_PICK for branch:stable — same branch, different type.
    updateProjectConfigFor(
        child,
        cfg -> {
          cfg.updateProject(p -> p.setSubmitType(SubmitType.INHERIT));
    ...
    // Child's CHERRY_PICK is evaluated first and matches branch:stable.
    assertSubmitType(CHERRY_PICK, onStable.getChangeId());
  ...
  /**
   * When a parent defines override type A for {@code branch:stable} and a child defines override
   * type B for {@code branch:stable}, the child's override B must win on {@code branch:stable}
   * because child overrides are evaluated before parent overrides.
   *
   * <p>This is the cross-type scenario: the two overrides have different {@link
   * com.google.gerrit.extensions.client.SubmitType} keys so the deduplication map does not collapse
   * them — both are present in the effective override list. Correct evaluation requires the child's
   * entry to be positioned before the parent's entry so the child wins the first-match race when
   * both expressions match the same change.
   */
  @Test
  public void inheritance_childOverrideWinsOverParentOverrideForSameBranch() throws Exception {
    Project.NameKey parent =
        createProjectOverAPI("parent", allProjects, /* createEmptyCommit= */ true, null);
    Project.NameKey child =
        createProjectOverAPI("child", parent, /* createEmptyCommit= */ true, null);
    ...
    // Child: CHERRY_PICK for branch:stable — different submit type, same branch expression.
    updateProjectConfigFor(
        child,
        cfg -> {
          cfg.updateProject(p -> p.setSubmitType(SubmitType.INHERIT));
    ...
    // Child's CHERRY_PICK override must win, not the parent's MERGE_ALWAYS.
    assertSubmitType(CHERRY_PICK, onStable.getChangeId());

    project = savedProject;
  }

  private void updateProjectConfigFor(
      Project.NameKey projectName, Consumer<ProjectConfig> configUpdater) throws Exception {
    try (MetaDataUpdate md = metaDataUpdateFactory.create(projectName)) {
      md.setMessage("Update project.config");
      ProjectConfig cfg = projectConfigFactory.read(md);
      configUpdater.accept(cfg);
      cfg.commit(md);
    }
