...
public class MultipleQuotaPluginsIT extends AbstractDaemonTest {
  ...
  public void refillsOnException() {
    ...
    QuotaRequestContext ctx = QuotaRequestContext.builder().user(identifiedAdmin).build();
    when(quotaEnforcerA.requestTokens("testGroup", ctx, 1)).thenThrow(exception);
    when(quotaEnforcerB.requestTokens("testGroup", ctx, 1)).thenReturn(QuotaResponse.ok());
    ...
    verify(quotaEnforcerB).refill("testGroup", ctx, 1);
  }
