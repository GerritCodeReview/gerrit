...
public class SshTraceIT extends AbstractDaemonTest {
  ...
  public void sshCallWithoutTrace() throws Exception {
    String response = adminSshSession.exec("gerrit create-project new1");
    adminSshSession.assertSuccess();
    assertThat(response).doesNotContain(RequestId.Type.TRACE_ID.name());
    assertThat(projectCreationListener.foundTraceId).isFalse();
  }

  @Test
  public void sshCallWithTrace() throws Exception {
    String response = adminSshSession.exec("gerrit create-project new2 --trace");
    adminSshSession.assertSuccess();
    assertThat(response).contains(RequestId.Type.TRACE_ID.name());
