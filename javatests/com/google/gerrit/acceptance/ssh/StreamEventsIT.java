...
public class StreamEventsIT extends AbstractDaemonTest {
  ...
  private ChannelExec streamEventsSshChannel = null;
  private BufferedReader streamEventsReader = null;

  @Override
  protected void initSsh() throws Exception {
    super.initSsh();
    streamEventsSshChannel = adminSshSession.execAndReturnChannel("gerrit stream-events");
    streamEventsReader =
        new BufferedReader(
            new InputStreamReader(streamEventsSshChannel.getInputStream(), StandardCharsets.UTF_8));
  }
  ...
  public void commentOnChangeShowsUpInStreamEvents() throws Exception {
    ...
    assertThat(findReviewComment(streamEventsReader, TEST_REVIEW_COMMENT)).hasSize(1);
  }

  private List<String> findReviewComment(BufferedReader reader, String reviewComment)
      throws IOException {
