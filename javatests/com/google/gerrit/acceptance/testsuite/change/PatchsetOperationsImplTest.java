...
public class PatchsetOperationsImplTest extends AbstractDaemonTest {
  ...
  public void robotCommentIsCreatedWithRobotId() throws Exception {
    ...
    RobotCommentInfo comment = getRobotCommentFromServer(changeId, commentUuid);
  ...
  @Test
  public void robotCommentIsCreatedWithFixSuggestion() throws Exception {
    Change.Id changeId = changeOperations.newChange().create();
    TestFixSuggestion.TestFixReplacement testFixReplacement =
        TestFixSuggestion.TestFixReplacement.builder()
            .path("path")
            .replacement("replacement")
            .range(
                TestRange.builder()
                    .setStart(TestRange.Position.builder().line(3).charOffset(4).build())
                    .setEnd(TestRange.Position.builder().line(1).charOffset(2).build())
                    .build())
            .build();
    TestFixSuggestion testFixSuggestion =
        TestFixSuggestion.builder()
            .setFixId("fixId")
            .setDescription("description")
            .addFixReplacement(testFixReplacement)
            .build();
    ...
    String commentUuid =
        changeOperations
            ...
            .currentPatchset()
            .newRobotComment()
            .addFixSuggestion(testFixSuggestion)
            .create();

    RobotCommentInfo comment = getRobotCommentFromServer(changeId, commentUuid);
    assertThat(comment).onlyFixSuggestion().description().isEqualTo("description");
    assertThat(comment).onlyFixSuggestion().fixId().isEqualTo("fixId");
    assertThat(comment).onlyFixSuggestion().onlyReplacement().path().isEqualTo("path");
    assertThat(comment).onlyFixSuggestion().onlyReplacement().range().startLine().isEqualTo(3);
    assertThat(comment).onlyFixSuggestion().onlyReplacement().range().startCharacter().isEqualTo(4);
    assertThat(comment).onlyFixSuggestion().onlyReplacement().range().endLine().isEqualTo(1);
    assertThat(comment).onlyFixSuggestion().onlyReplacement().range().endCharacter().isEqualTo(2);
    assertThat(comment)
        .onlyFixSuggestion()
        .onlyReplacement()
        .replacement()
  ...
  private RobotCommentInfo getRobotCommentFromServer(Change.Id changeId, String uuid)
      throws RestApiException {
    return gApi.changes().id(changeId.get()).robotCommentsAsList().stream()
        .filter(comment -> comment.id.equals(uuid))
        .findAny()
        .orElseThrow(
            () ->
