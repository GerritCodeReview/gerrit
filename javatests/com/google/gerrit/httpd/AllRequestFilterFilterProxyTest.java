...
public class AllRequestFilterFilterProxyTest {
  ...
  public void singleFilterNoBubbling() throws Exception {
    ...
    verify(filter).doFilter(eq(req), eq(res), any(FilterChain.class));
  ...
  public void singleFilterBubbling() throws Exception {
    ...
    verify(filter).init(config);
    //verify(filter).doFilter(eq(req), eq(res), capturedChain.capture());
  ...
  public void twoFiltersNoBubbling() throws Exception {
    ...
    verify(filterA).init(config);
    verify(filterB).init(config);
    verify(filterA).doFilter(eq(req), eq(res), any(FilterChain.class));
    verify(filterA).destroy();
    verify(filterB).destroy();
  }
