...
public class IndexServletTest {
  ...
  public void renderTemplateWithPrivateToPublicHostMap() throws Exception {
    ExperimentFeatures experimentFeatures = new ExperimentFeatures() {
      @Override
    ...
    IndexServlet servlet =
        new IndexServlet(
            null, null, null, gerritApi, experimentFeatures,
            req -> Optional.of(
                ImmutableMap.of("gerrit-review.private.com", "gerrit-review.googlesource.com",
                    "chrome-review.private.com", "chrome-review.googlesource.com")));
    FakeHttpServletResponse response = new FakeHttpServletResponse();
    servlet.doGet(new FakeHttpServletRequest(), response);
    String output = response.getActualBodyString();
    assertThat(output).contains("window.PRIVATE_TO_PUBLIC_HOST_MAP = JSON.parse('\\x7b"
