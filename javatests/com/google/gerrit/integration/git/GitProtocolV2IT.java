...
public class GitProtocolV2IT extends StandaloneSiteTest {
  ...
  public void testGitWireProtocolV2WithSsh() throws Exception {
    try (ServerContext ctx = startServer()) {
      ...
      setProtocolV2InProject(project);
  ...
  public void testGitWireProtocolV2HidesRefMetaConfig() throws Exception {
    try (ServerContext ctx = startServer()) {
      ...
      setProtocolV2InProject(allRefsVisibleProject);
  ...
  public void testGitWireProtocolV2ShowsOnlyHead() throws Exception {
    try (ServerContext ctx = startServer()) {
      ...
      List<BranchInfo> branches = gApi.projects().create(pi).branches().get();

      String headRevision =
          branches.stream().filter(b -> b.ref.equals(RefNames.HEAD)).findAny().get().revision;
      String headSha1 =
          branches.stream()
              .filter(b -> b.ref.equals(RefNames.REFS_HEADS + headRevision))
              .findAny()
              .get()
              .revision;

      setProtocolV2InProject(showHeadProject);
      ...
      assertThat(outFetchRef).containsMatch(String.format("fetch< %s %s", headSha1, RefNames.HEAD));
  ...
  public void testGitWireProtocolV2FetchIndividualRef() throws Exception {
    try (ServerContext ctx = startServer()) {
      ...
      setProtocolV2InProject(privateProject);
  ...
  private void setProtocolV2InProject(Project.NameKey projectName) throws Exception {
    execute(
