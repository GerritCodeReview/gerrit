...
// TODO(davido): In addition to push over HTTP also implement push over SSH
public class PushToRefsUsersIT extends StandaloneSiteTest {
  ...
  public void testPushToRefsUsersOverHttp() throws Exception {
    try (ServerContext ctx = startServer()) {
      ...
      // Push
      String out = execute(GIT_PUSH_USERS_SELF);

      assertThat(out).contains("Processing changes: refs: 1, done");

      // Retrieve status
      AccountDetailInfo detail = gApi.accounts().id(admin.id().get()).detail();
      assertThat(detail.status).isEqualTo("OOO");
    }
  }
