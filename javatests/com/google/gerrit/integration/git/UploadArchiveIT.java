...
public class UploadArchiveIT extends StandaloneSiteTest {
  ...
  @Test
  public void uploadAllFormat() throws Exception {
    try (ServerContext ctx = startServer()) {
      setUpTestHarness(ctx);
      setUpChange();
      ...
      try (InputStream fi = Files.newInputStream(outputPath);
          ...
          ArchiveInputStream archive = new ZipArchiveInputStream(bi)) {
        assertEntries(archive);
      }

      execute(
          cmd("tar", commit.commit),
          sitePaths.data_dir.toFile(),
          ImmutableMap.of("GIT_SSH_COMMAND", GIT_SSH_COMMAND + identityPath),
          outputPath);
      try (InputStream fi = Files.newInputStream(outputPath);
          InputStream bi = new BufferedInputStream(fi);
          ArchiveInputStream a = new TarArchiveInputStream(bi)) {
        assertEntries(a);
      }

      execute(
          cmd("tar.gz", commit.commit),
          sitePaths.data_dir.toFile(),
          ImmutableMap.of("GIT_SSH_COMMAND", GIT_SSH_COMMAND + identityPath),
          outputPath);
      try (InputStream fi = Files.newInputStream(outputPath);
      ...
      try (InputStream fi = Files.newInputStream(outputPath);
          ...
          ArchiveInputStream a = new TarArchiveInputStream(bz2)) {
        assertEntries(a);
      }

      execute(
          cmd("tar.xz", commit.commit),
          sitePaths.data_dir.toFile(),
          ImmutableMap.of("GIT_SSH_COMMAND", GIT_SSH_COMMAND + identityPath),
          outputPath);
      try (InputStream fi = Files.newInputStream(outputPath);
          ...
          ArchiveInputStream a = new TarArchiveInputStream(xzi)) {
        assertEntries(a);
      }
    }
  }
