...
public class ParserUtilTest {
  ...
  @Test
  public void isCommentUrlMatchesPatchsetLevelCommentsTabLink() {
    HumanComment comment = newComment("patchset-comment-uuid", Patch.PATCHSET_LEVEL, 1, 0);

    // The comments-tab link CommentChangeEmailDecoratorImpl#getCommentsTabLink() emits for
    // patchset-level comments, with and without the "?usp=email" query parameter.
    assertThat(ParserUtil.isCommentUrl(CHANGE_URL + "?tab=comments&usp=email", CHANGE_URL, comment))
        .isTrue();
    assertThat(ParserUtil.isCommentUrl(CHANGE_URL + "?tab=comments", CHANGE_URL, comment)).isTrue();

    // A non-patchset-level comment must not match this link, even though the string itself
    // doesn't depend on any per-comment identifier.
    HumanComment fileComment = newComment("some-uuid", "gerrit-server/test.txt", 1, 3);
    assertThat(
            ParserUtil.isCommentUrl(
                CHANGE_URL + "?tab=comments&usp=email", CHANGE_URL, fileComment))
        .isFalse();
  }
