...
public class BucketedCallbackTest {
  ...
  String CODE_NAME = "name";
  String KEY_NAME = "foo";
  String COLLIDING_KEY_NAME = "setToFoo";

  String metricName(String fieldValues) {
  ...
  public void shouldRegisterMetricWithNewKey() {
    ...
    String newKey = "bar";
    bc.getOrCreate(newKey);
    assertThat(registry.getNames()).containsExactly(metricName(KEY_NAME), metricName(newKey));
  }
  ...
  public void shouldStoreKeyValueInCellsAndRegisterSubmetricName() {
    BucketedCallback<Long> bc = new CallbackMetricTestImpl();
    bc.getOrCreate(COLLIDING_KEY_NAME);
    assertThat(bc.getCells().keySet()).containsExactly(COLLIDING_KEY_NAME);
    assertThat(registry.getNames()).containsExactly(metricName(KEY_NAME));
  }
  ...
  public void shouldErrorIfKeyIsDifferentButNameCollides() {
    ...
    assertThrows(IllegalArgumentException.class, () -> bc.getOrCreate(COLLIDING_KEY_NAME));
    assertThat(bc.getCells().keySet()).containsExactly(KEY_NAME);
    assertThat(registry.getNames()).containsExactly(metricName(KEY_NAME));
  }
  ...
  private class CallbackMetricTestImpl extends BucketedCallback<Long> {
    ...
    String name(Object key) {
      if (key.equals(COLLIDING_KEY_NAME)) {
        return KEY_NAME;
      } else {
        return key.toString();
      }
