...
/**
 * End-to-end check that SameSiteFilter survives the round-trip through Jetty 12 ee11's
 * cookie-rendering pipeline -- i.e. that the {@code SameSite} attribute set via {@code
 * Cookie.setAttribute("SameSite", ...)} actually appears in the {@code Set-Cookie} header on the
 * wire. The unit test in {@link SameSiteFilterTest} only verifies that the filter mutates the
 * cookie; this test verifies that ee11's {@code ServletApiResponse.HttpCookieFacade} reads that
...
public class SameSiteFilterServerTest {
  ...
  private static String setCookieHeaderFor(String sameSiteConfig) throws Exception {
    ...
    context.addFilter(
        new FilterHolder(new SameSiteFilter(cfg)), "/*", EnumSet.of(DispatcherType.REQUEST));
