...
public class AuthTokenExpiryNotificationConfigTest {
  ...
  public void featureCanBeDisabled() {
    Config config = new Config();
    config.setBoolean("auth", null, "tokenExpiryNotificationEnabled", false);
    AuthTokenExpiryNotificationConfig notificationConfig =
        new AuthTokenExpiryNotificationConfig(config);
  ...
  public void doesNotSuggestNotificationWhenDisabled() {
    ...
    assertThat(notificationConfig.shouldBeNotified(Instant.now().plus(7, ChronoUnit.DAYS)))
        .isFalse();
  }
  ...
  public void suggestsNotificationOnConfiguredDays() {
    ...
    assertThat(notificationConfig.shouldBeNotified(Instant.now().plus(1, ChronoUnit.HOURS)))
        .isTrue();
    assertThat(
            notificationConfig.shouldBeNotified(
                Instant.now().plus(1, ChronoUnit.HOURS).plus(7, ChronoUnit.DAYS)))
        .isTrue();
    assertThat(
            notificationConfig.shouldBeNotified(
                Instant.now().plus(1, ChronoUnit.HOURS).plus(14, ChronoUnit.DAYS)))
        .isTrue();
    assertThat(
            notificationConfig.shouldBeNotified(
                Instant.now().plus(1, ChronoUnit.HOURS).plus(21, ChronoUnit.DAYS)))
        .isTrue();

    assertThat(
            notificationConfig.shouldBeNotified(
                Instant.now().plus(1, ChronoUnit.HOURS).plus(3, ChronoUnit.DAYS)))
        .isFalse();
    assertThat(
            notificationConfig.shouldBeNotified(
                Instant.now().plus(1, ChronoUnit.HOURS).plus(30, ChronoUnit.DAYS)))
        .isFalse();
  }
  ...
  public void doesNotSuggestNotificationWhenExpired() {
    ...
    AuthTokenExpiryNotificationConfig notificationConfig =
        new AuthTokenExpiryNotificationConfig(config);
    assertThat(notificationConfig.shouldBeNotified(Instant.now().minus(3, ChronoUnit.DAYS)))
        .isFalse();
  }
}
