...
public class AuthTokenExpiryNotifierTest {
  private static final Instant CHECK_TIME = Instant.now().truncatedTo(ChronoUnit.DAYS);
  private static final Instant LOWER_BOUND = CHECK_TIME.minus(1, ChronoUnit.DAYS);

  @Test
  public void shouldNotifyExpiredReturnsTrueWhenTokenExpiredWithinLast24Hours() {
    // Token expired 12 hours ago
    Instant expirationDate = CHECK_TIME.minus(12, ChronoUnit.HOURS);

    assertThat(AuthTokenExpiryNotifier.shouldNotifyExpired(CHECK_TIME, expirationDate)).isTrue();
  }
  ...
  public void shouldNotifyExpiredReturnsFalseWhenTokenExpiredMoreThan24HoursAgo() {
    ...
    Instant expirationDate = CHECK_TIME.minus(30, ChronoUnit.HOURS);

    assertThat(AuthTokenExpiryNotifier.shouldNotifyExpired(CHECK_TIME, expirationDate)).isFalse();
  }

  @Test
  public void shouldNotifyExpiredReturnsFalseWhenTokenNotYetExpired() {
    // Token expires in the future
    Instant expirationDate = CHECK_TIME.plus(7, ChronoUnit.DAYS);
  ...
  public void shouldNotifyExpiredReturnsTrueWhenTokenJustExpired() {
    ...
    Instant expirationDate = CHECK_TIME;
