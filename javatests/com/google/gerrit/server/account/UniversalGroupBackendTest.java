...
public class UniversalGroupBackendTest {
  ...
  public void suggestInterleaved() {
    ...
    // Test interleaving
    Collection<GroupReference> suggestions = backend.suggest("user", null);
    assertEquals(
        ImmutableList.of("Users", "Test Users", "Administrators", "Project Owners"),
        suggestions.stream().map(GroupReference::getName).collect(Collectors.toList()));

    // Test exact match first
    suggestions = backend.suggest("Users", null);
    assertEquals(
        ImmutableList.of("Users", "Test Users", "Administrators", "Project Owners"),
        suggestions.stream().map(GroupReference::getName).collect(Collectors.toList()));
  ...
  public void suggestWithDifferentSizeLists() {
    ...
    Collection<GroupReference> suggestions = backend.suggest("a", null);
    assertEquals(
        ImmutableList.of("a", "cc", "bbb", "dddd", "eeeee"),
        suggestions.stream().map(GroupReference::getName).collect(Collectors.toList()));
  }
}
