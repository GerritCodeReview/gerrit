...
public class AllExternalIdsTest {
  ...
  public void allExternalIdsMethods() {
    assertThatSerializedClass(AllExternalIds.class)
        .hasAutoValueMethods(
            ImmutableMap.of(
                "byAccount", new TypeLiteral<SetMultimap<Account.Id, ExternalId>>() {}.getType(),
                "byEmail", new TypeLiteral<SetMultimap<String, ExternalId>>() {}.getType()));
  }
  ...
  private static AllExternalIds allExternalIds(ExternalId... externalIds) {
    return AllExternalIds.create(
        Arrays.stream(externalIds).collect(toImmutableListMultimap(ExternalId::accountId, e -> e)));
  }
