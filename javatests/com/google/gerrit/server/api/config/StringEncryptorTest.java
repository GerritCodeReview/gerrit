...
public class StringEncryptorTest {
  ...
  public void setUp() throws Exception {
    ...
    expect(config.algorithm()).andReturn(ALGORITHM).anyTimes();
    expect(config.transformation()).andReturn(TRANSFORMATION).anyTimes();
    replay(config);

    expect(manager.getSecretKey()).andReturn(ENCRYPTION_KEY);
    expect(manager.getSecretKeySpec())
        .andReturn(new SecretKeySpec(KeyManager.base64Decode(ENCRYPTION_KEY), config.algorithm()))
        .anyTimes();
    replay(manager);
