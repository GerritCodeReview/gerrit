...
public class PerThreadCacheTest {
  ...
  public void endToEndCache() {
    try (PerThreadCache ignored = PerThreadCache.create(null)) {
      ...
      String value1 = cache.getWithLoader(key1, () -> "value1", null);
      ...
      assertThat(cache.getWithLoader(key1, neverCalled, null)).isEqualTo("value1");
    }
  }
  ...
  public void cleanUp() {
    ...
    try (PerThreadCache ignored = PerThreadCache.create(null)) {
      PerThreadCache cache = PerThreadCache.get();
      String value1 = cache.getWithLoader(key, () -> "value1", null);
      assertThat(value1).isEqualTo("value1");
    }
    ...
    try (PerThreadCache ignored = PerThreadCache.create(null)) {
      PerThreadCache cache = PerThreadCache.get();
      String value1 = cache.getWithLoader(key, () -> "value2", null);
  ...
  public void clearOutStaleEntriesAfterReadonlyWindow() throws Exception {
    ...
    try (PerThreadCache cache = PerThreadCache.create(null)) {
      try (ReadonlyRequestWindow outerWindow = PerThreadCache.openReadonlyRequestWindow()) {
        assertThat(PerThreadCache.get(key, () -> cachedValue, v -> {})).isEqualTo(cachedValue);
        assertThat(PerThreadCache.get(key, () -> updatedValue, v -> {})).isEqualTo(cachedValue);
      }

      assertThat(PerThreadCache.get(key, () -> updatedValue, v -> {})).isEqualTo(updatedValue);
    }
  }
  ...
  public void enforceMaxSize() {
    try (PerThreadCache cache = PerThreadCache.create(null)) {
      ...
      assertThat(cache.getWithLoader(key, () -> "new value", null)).isNull();
  ...
  @Test
  public void returnNullWhenCacheIsFull() {
    try (PerThreadCache cache = PerThreadCache.create(null)) {
      fillTheCache(cache);
