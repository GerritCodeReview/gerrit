...
public class PerThreadCacheTest {
  ...
  public void clearOutStaleEntriesAfterReadonlyWindow() throws Exception {
    ...
    try (PerThreadCache cache = PerThreadCache.create(null)) {
      try (ReadonlyRequestWindow outerWindow = PerThreadCache.openReadonlyRequestWindow()) {
        assertThat(PerThreadCache.getOrCompute(key, () -> cachedValue, v -> {}))
            .isEqualTo(cachedValue);
        assertThat(PerThreadCache.getOrCompute(key, () -> updatedValue, v -> {}))
            .isEqualTo(cachedValue);
      }
            ...
            assertThrows(
                ...
                () -> {
                  try (ReadonlyRequestWindow outerWindow =
                    ...
                    PerThreadCache.getOrCompute(key, () -> new TestCacheValue(""), v -> {});
                  }
                });
  ...
  public void allowStaleEntriesAfterReadonlyWindow() {
    ...
    try (PerThreadCache cache = PerThreadCache.create(null)) {
      try (ReadonlyRequestWindow window = PerThreadCache.openReadonlyRequestWindow()) {
        ...
        cachedValue = PerThreadCache.getOrCompute(key, () -> value, v -> {});
      }
    }
      ...
      try (PerThreadCache cache = PerThreadCache.createReadOnly()) {
        assertThat(
                PerThreadCache.getOrCompute(
                    PerThreadCache.Key.create(String.class, "key"), () -> "oldValue", v -> {}))
            .isEqualTo("oldValue");
        assertThat(
                PerThreadCache.getOrCompute(
                    PerThreadCache.Key.create(String.class, "key"), () -> "newValue", v -> {}))
            .isEqualTo("newValue");

        assertThat(
                PerThreadCache.getOrCompute(
                    PerThreadCache.Key.create(Integer.class, "key"), () -> 1, v -> {}))
            .isEqualTo(1);
        assertThat(
                PerThreadCache.getOrCompute(
                    PerThreadCache.Key.create(Integer.class, "key"), () -> 2, v -> {}))
            .isEqualTo(2);

        assertThat(
                PerThreadCache.getOrCompute(
                    PerThreadCache.Key.create(Long.class, "key"), () -> 1L, v -> {}))
            .isEqualTo(1L);
        assertThat(
                PerThreadCache.getOrCompute(
                    PerThreadCache.Key.create(Long.class, "key"), () -> 2L, v -> {}))
            .isEqualTo(1L);
      }
    } finally {
