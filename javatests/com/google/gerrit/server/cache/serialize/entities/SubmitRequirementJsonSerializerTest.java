...
import com.google.gerrit.server.notedb.ChangeNoteJson.ImmutableListAdapter;
...
public class SubmitRequirementJsonSerializerTest {
  ...
  public void submitRequirementResult_roundTrip() throws Exception {
    TypeAdapter<SubmitRequirementResult> adapter = SubmitRequirementResult.typeAdapter();
  ...
  @Test
  public void submitRequirementResult_canDeserializeIfSerializedByDefaultAdapter()
      throws Exception {
    Gson gson =
        new GsonBuilder()
            ...
            .setPrettyPrinting()
            .create();
    TypeAdapter<SubmitRequirementResult> defaultAdapter =
        SubmitRequirementResult.defaultTypeAdapter(gson);
    String defaultSerial = defaultAdapter.toJson(srReqResult);
    assertThat(defaultSerial)
        .contains(
            "{\"w1\":1180937118,\"w2\":-1632331231,\"w3\":1315497487,"
                + "\"w4\":266719414,\"w5\":-196118978}");

    // Deserialize using the Gerrit implemented deserializer
    assertThat(SubmitRequirementResult.typeAdapter().fromJson(defaultSerial))
        .isEqualTo(srReqResult);
  }

  @Test
  public void submitRequirementExpressionFields() {
    // If any new field is added to this entity, please also serialize it to Json.
    Method[] declaredMethods = SubmitRequirementExpression.class.getDeclaredMethods();
    List<String> fields =
        Arrays.stream(declaredMethods)
            .filter(SubmitRequirementJsonSerializerTest::isAbstract)
            .map(Method::getName)
            .collect(Collectors.toList());
    List<String> expectedFields = Arrays.asList("expressionString");
    assertThat(fields).containsExactlyElementsIn(expectedFields);
  }

  @Test
  public void submitRequirementFields() {
    // If any new field is added to this entity, please also serialize it to Json.
    Method[] declaredMethods = SubmitRequirement.class.getDeclaredMethods();
    List<String> fields =
        Arrays.stream(declaredMethods)
            .filter(SubmitRequirementJsonSerializerTest::isAbstract)
            .map(Method::getName)
  ...
  @Test
  public void submitRequirementExpressionResultFields() {
    // If any new field is added to this entity, please also serialize it to Json.
    Method[] declaredMethods = SubmitRequirementExpressionResult.class.getDeclaredMethods();
    List<String> fields =
        Arrays.stream(declaredMethods)
            .filter(SubmitRequirementJsonSerializerTest::isAbstract)
            .map(Method::getName)
            .collect(Collectors.toList());
    List<String> expectedFields =
        Arrays.asList("expression", "status", "errorMessage", "passingAtoms", "failingAtoms");
    assertThat(fields).containsExactlyElementsIn(expectedFields);
  }

  @Test
  public void submitRequirementResultFields() {
    // If any new field is added to this entity, please also serialize it to Json.
    Method[] declaredMethods = SubmitRequirementResult.class.getDeclaredMethods();
    List<String> fields =
        Arrays.stream(declaredMethods)
            .filter(SubmitRequirementJsonSerializerTest::isAbstract)
            .map(Method::getName)
