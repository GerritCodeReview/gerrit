...
public class RelatedChangesSorterTest {
  ...
  public void linearChain() throws Exception {
    ...
    assertThat(sortedFrom3)
        .containsExactly(patchSetData(cd3, c3), patchSetData(cd2, c2), patchSetData(cd1, c1))
        .inOrder();
  ...
  public void benchmarkChangeChainSortingLatency() throws Exception {
    ...
    for (int chainLength : chainSizes) {
      ...
      for (int i = 1; i <= chainLength; i++) {
        RevCommit cPs1 =
            p.commit().parent(parent).message("Commit " + i + " ps 1\n\nChange details").create();
        RevCommit cPs2 =
            p.commit()
                .parent(parent)
                .message("Commit " + i + " ps 2\n\nUpdated change details")
                .create();
      ...
      System.out.printf(
          "RelatedChangesSorter Benchmark [Chain=%d]: sort() avg = %.2f us, sortAncestors() avg ="
              + " %.2f us%n",
