...
public class EmptyContentTest {
  ...
  public void insertSingleLineNoEOL() throws Exception {
    FixResult fixResult = FixCalculatorTest.calculateFixSingleReplacement("", 1, 0, 1, 0, "Abc");
  ...
  public void insertMultilineNoEOL() throws Exception {
    FixResult fixResult =
        FixCalculatorTest.calculateFixSingleReplacement("", 1, 0, 1, 0, "Abc\nDEFGH");
    assertThat(fixResult).text().isEqualTo("Abc\nDEFGH");
    GitEditSubject editSubject = assertThat(fixResult).edits().onlyElement();
    editSubject.isInsert(0, 0, 2);
    editSubject.internalEdits().onlyElement().isInsert(0, 0, 9);
  }
