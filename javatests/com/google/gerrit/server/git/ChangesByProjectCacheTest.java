...
public final class ChangesByProjectCacheTest extends AbstractChangeNotesTest {
  ...
  public void cachedProjectChangesSerializer() throws Exception {
    ...
    for (Map.Entry<Change.Id, PrivateChange> e : deserialized.privateChangeById.entrySet()) {
      PrivateChange deserializedPrivateChange = e.getValue();
      PrivateChange origPrivateChange = cachedProjectChanges.privateChangeById.get(e.getKey());
      assertThat(deserializedPrivateChange.metaRevision())
          .isEqualTo(origPrivateChange.metaRevision());
      assertThat(deserializedPrivateChange.reviewers()).isEqualTo(origPrivateChange.reviewers());
      assertThat(deserializedPrivateChange.change().toString())
          .isEqualTo(origPrivateChange.change().toString());
    }
  }
