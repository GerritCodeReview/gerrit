...
public class DeleteZombieCommentsRefsTest {
  ...
  public void cleanZombieDraftsLarge() throws Exception {
    try (Repository usersRepo = repoManager.createRepository(allUsersProject)) {
      final int goodRefsCnt = 5000;
      final int zombieRefsCnt = 5000;
      final int userIdGoodRefs = 1000001;
      final int userIdBadRefs = 1000002;

      Ref nonEmptyBaseRef = createRefWithNonEmptyTreeCommit(usersRepo, 0, userIdGoodRefs);
      Ref emptyBaseRef = createRefWithEmptyTreeCommit(usersRepo, 0, userIdBadRefs);
  ...
  private static String getRefName(long changeId, long userId) {
    return String.format("refs/draft-comments/%d/%d/%d", changeId % 100, changeId, userId);
  }

  private static Ref createRefWithNonEmptyTreeCommit(
      Repository usersRepo, long changeId, long userId) throws IOException {
  ...
  private static Ref createRefWithEmptyTreeCommit(Repository usersRepo, long changeId, long userId)
      throws IOException {
