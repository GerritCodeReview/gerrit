...
public class TagSetTest {
  ...
  @Test
  public void refreshRemovesUndecoratedNodesFromDecoratedSet() throws Exception {
    InMemoryRepositoryManager repoManager = new InMemoryRepositoryManager();
    TestRepository<Repo> tr =
        new TestRepository<>(repoManager.createRepository(Project.nameKey("project")));
    ...
    ObjectId c1Id = c1.getId();
    ObjectId c2Id = c2.getId();

    String refName = "refs/heads/master";
    RefUpdate createRef = repo.updateRef(refName);
    createRef.setNewObjectId(c1Id);
    testRefAction(() -> assertThat(createRef.update()).isEqualTo(RefUpdate.Result.NEW));

    TagSet tagSet = new TagSet(Project.nameKey("project"));
    tagSet.build(repo, null, null);

    RefUpdate moveRef = repo.updateRef(refName);
    moveRef.setNewObjectId(c2Id);
    testRefAction(() -> assertThat(moveRef.update()).isEqualTo(RefUpdate.Result.FAST_FORWARD));

    Map<String, ObjectId> idByRef = new HashMap<>();
    idByRef.put(refName, c2Id);
    tagSet.refresh(repo, idByRef);

    Node oldNode = tagSet.dag.get(c1Id);
    Node newNode = tagSet.dag.get(c2Id);
    assertThat(oldNode.decorations).isEmpty();
    assertThat(tagSet.dag.decorated).contains(newNode);
    assertThat(tagSet.dag.decorated).doesNotContain(oldNode);
  }
