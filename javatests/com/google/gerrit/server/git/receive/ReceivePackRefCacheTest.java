...
public class ReceivePackRefCacheTest {
  ...
  public void noCache_tipsFromObjectIdDelegatesToRefDbAndFiltersByPrefix() throws Exception {
    Ref refBla = mock(Ref.class);
    when(refBla.getName()).thenReturn("refs/bla");

    Ref refheads = mock(Ref.class);
    when(refheads.getName()).thenReturn(RefNames.REFS_HEADS);
  ...
  public void advertisedRefs_prefixScans() throws Exception {
    Ref refBla = mock(Ref.class);
    when(refBla.getName()).thenReturn("refs/bla/1");
  ...
  private Map<String, Ref> setupTwoChanges() {
    ...
    when(ref1.getName()).thenReturn("refs/changes/01/1/1");
    when(ref1.getObjectId())
        .thenReturn(ObjectId.fromString("badc0feebadc0feebadc0feebadc0feebadc0fee"));
    Ref ref2 = mock(Ref.class);
    when(ref2.getName()).thenReturn("refs/changes/02/2/1");
    when(ref2.getObjectId())
        .thenReturn(ObjectId.fromString("deadbeefdeadbeefdeadbeefdeadbeefdeadbeef"));
