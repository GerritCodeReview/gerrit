...
public class GroupRebuilderTest extends GerritBaseTests {
  ...
  public void minimalGroupFields() throws Exception {
    ...
    assertThat(owner(g)).isEqualTo(new GroupReference(g.getGroupUUID(), g.getName()));
  }
  ...
  public void allGroupFields() throws Exception {
    ...
    assertThat(reload(g)).isEqualTo(b.toInternalGroup());
    assertThat(log(g)).containsExactly("Create group");
    assertThat(owner(g)).isEqualTo(new GroupReference(g.getOwnerGroupUUID(), "Group owner"));
  }
  ...
  @Test
  public void auditsAtSameTimestampResultInDistinctCommits() throws Exception {
    AccountGroup g = newGroup("a");
    Timestamp t = TimeUtil.nowTs();
    GroupBundle b =
        builder()
            .group(g)
            .members(member(g, 1), member(g, 2))
            .memberAudit(
                addMember(g, 1, 8, t), addMember(g, 2, 8, t), addAndRemoveMember(g, 3, 8, t, 8, t))
            .byId(byId(g, "x"), byId(g, "y"))
            .byIdAudit(
                addById(g, "x", 8, t), addById(g, "y", 8, t), addAndRemoveById(g, "z", 8, t, 8, t))
            .build();
  ...
  private InternalGroup reload(AccountGroup g) throws Exception {
    return GroupConfig.loadForGroup(
            allUsers,
            repo,
            g.getGroupUUID(),
            project -> {
  ...
  private GroupReference owner(AccountGroup g) throws Exception {
    ProjectConfig pc = new ProjectConfig(allUsers);
    pc.load(repo);
    String ref = RefNames.refsGroups(g.getGroupUUID());
    AccessSection sec = pc.getAccessSection(ref);
    assertThat(sec).named("section for %s", ref).isNotNull();
    Permission perm = sec.getPermission(Permission.PUSH);
    assertThat(perm).named("push permission").isNotNull();
    assertThat(perm.getRules()).hasSize(1);
    PermissionRule rule = perm.getRules().get(0);
