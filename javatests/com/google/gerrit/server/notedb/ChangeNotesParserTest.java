...
public class ChangeNotesParserTest extends AbstractChangeNotesTest {
  ...
  @Test
  public void hashTagExtractionWhenAllowChangeNotesStateFromAnyServer() throws Exception {

    PersonIdent author =
        new PersonIdent(
            "Change Owner",
            "anyUser@anotherServerId.com",
            serverIdent.getWhen(),
            serverIdent.getTimeZone());
    ...
    RevCommit commit =
        writeCommit(
            "Update patch set 1\n"
                + "\n"
                + "Hashtag added: palo-alto-2018\n"
                + "\n"
                + "Patch-set: 1\n"
                + "Hashtags: test-tag\n"
                + "Tag: autogenerated:gerrit:setHashtag\n",
            author);
    ChangeNotesState state = newParser(commit, true).parseAll();
    assertThat(state.hashtags()).isNotEmpty();
    assertThat(state.hashtags()).isEqualTo(ImmutableSet.of("test-tag"));
  }

  @Rule public final ExpectedException exception = ExpectedException.none();

  @Test
  public void throwExceptionWhenNotAllowChangeNotesStateFromAnyServer() throws Exception {
    PersonIdent author =
        new PersonIdent(
            "Change Owner",
            "anyUser@anotherServerId.com",
    ...
    RevCommit parsableCommit =
        writeCommit(
            "Update patch set 1\n"
                ...
                + "\n"
                + "Patch-set: 1\n"
                + "Hashtags: palo-alto-2018\n"
                + "Tag: autogenerated:gerrit:setHashtag\n",
            author);

    exception.expect(ConfigInvalidException.class);
    newParser(parsableCommit, false).parseAll();
  }

  @Test
  public void noExceptionWhenAllowChangeNotesStateFromAnyServer() throws Exception {
    PersonIdent author =
        new PersonIdent(
            "Change Owner",
            "anyUser@anotherServerId.com",
    ...
    ChangeNotesState state = newParser(parsableCommit, true).parseAll();
  ...
  private ChangeNotesParser newParser(ObjectId tip) throws Exception {
    return newParser(tip, false);
  }

  private ChangeNotesParser newParser(ObjectId tip, Boolean allowAnyServerId) throws Exception {
    walk.reset();
    ...
    return new ChangeNotesParser(
        newChange().getId(), tip, walk, changeNoteJson, reader, args.metrics, allowAnyServerId);
  }
}
