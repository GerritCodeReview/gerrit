...
public class OpenRepoTest extends AbstractChangeNotesTest {
  ...
  public void attentionSetUpdateShouldNotContributeToOperationsCount() throws Exception {
    Optional<Integer> maxUpdates = Optional.of(2);
    FakeChainedReceiveCommands fakeChainedReceiveCommands = new FakeChainedReceiveCommands(repo);
    OpenRepo openRepo = new OpenRepo(repo, rw, null, fakeChainedReceiveCommands, false);
    ...
    ListMultimap<String, ChangeUpdate> changeUpdates1 =
        new ImmutableListMultimap.Builder<String, ChangeUpdate>().put("one", update1).build();

    openRepo.addUpdates(changeUpdates1, maxUpdates, MAX_PATCH_SETS);
    ...
    ListMultimap<String, ChangeUpdate> changeUpdates2 =
        new ImmutableListMultimap.Builder<String, ChangeUpdate>().put("one", update2).build();

    openRepo.addUpdates(changeUpdates2, maxUpdates, MAX_PATCH_SETS);
