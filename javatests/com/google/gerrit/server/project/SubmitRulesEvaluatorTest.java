...
public class SubmitRulesEvaluatorTest {
   ...
   *
   * <p>The second submit_rule is always valid, and provides two labels: OK and Code-Verified.
   *
  ...
  public void abortsEarlyWithOkayRecord() {
    ...
    submitRecordLabel1.appliedBy = ADMIN_USER;
    ...
    submitRecordLabel2.appliedBy = ADMIN_USER;
    ...
    submitRecordLabel3.appliedBy = ADMIN_USER;

    List<Term> terms = new ArrayList<>();

    StructureTerm label1 = makeLabel(submitRecordLabel1.label, "reject", ADMIN_USER);

    StructureTerm label2 = makeLabel(submitRecordLabel2.label, "ok", ADMIN_USER);

    StructureTerm label3 = makeLabel(submitRecordLabel3.label, "reject", ADMIN_USER);
  ...
  private Term makeTerm(String status, StructureTerm labels) {
  ...
  private StructureTerm makeLabel(String name, String status, Account.Id account) {
    StructureTerm user = new StructureTerm("user", new IntegerTerm(account.get()));
    return new StructureTerm("label", new StructureTerm(name), new StructureTerm(status, user));
  }

  private StructureTerm makeLabels(StructureTerm... labels) {
