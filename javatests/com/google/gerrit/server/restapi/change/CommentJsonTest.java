...
public class CommentJsonTest {
  ...
  @Test
  public void benchmarkCommentFormatting() throws Exception {
    CommentJson commentJson =
        newCommentJson()
            .setFillAccounts(false)
            .setFillPatchSet(true)
    ...
    int numComments = 5000;
    List<HumanComment> comments = new ArrayList<>(numComments);
    for (int i = 0; i < numComments; i++) {
      String file = "dir/file" + (i % 50) + ".java";
      comments.add(newComment("comment-" + i, file, 1, i + 1, "Message " + i));
    }

    CommentJson.HumanCommentFormatter formatter = commentJson.newHumanCommentFormatter();

    // Warmup
    for (int w = 0; w < 25; w++) {
      var unusedMap = formatter.format(comments);
      var unusedList = formatter.formatAsList(comments);
      assertThat(unusedMap).isNotEmpty();
      assertThat(unusedList).isNotEmpty();
    }

    // Measure format (as map)
    int iterations = 50;
    long startMap = System.nanoTime();
    for (int it = 0; it < iterations; it++) {
      var mapRes = formatter.format(comments);
      assertThat(mapRes).isNotEmpty();
    }
    long elapsedMapNs = System.nanoTime() - startMap;
    double avgMapMs = (double) elapsedMapNs / (iterations * 1_000_000.0);

    // Measure formatAsList
    long startList = System.nanoTime();
    for (int it = 0; it < iterations; it++) {
      var listRes = formatter.formatAsList(comments);
      assertThat(listRes).isNotEmpty();
    }
    long elapsedListNs = System.nanoTime() - startList;
    double avgListMs = (double) elapsedListNs / (iterations * 1_000_000.0);

    System.out.printf(
        "Benchmark: 5000 comments with context - format: %.2f ms, formatAsList: %.2f ms%n",
        avgMapMs, avgListMs);
  }
}
