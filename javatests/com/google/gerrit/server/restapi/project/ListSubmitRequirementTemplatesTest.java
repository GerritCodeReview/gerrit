...
public class ListSubmitRequirementTemplatesTest {
  ...
  private static SubmitRequirement newTemplate(String name, String expression) {
    return SubmitRequirement.builder()
        .setName(name)
        .setSubmittabilityExpression(SubmitRequirementExpression.create(expression))
  ...
  public void readsTemplatesFromCurrentProjectAndParents() throws Exception {
    SubmitRequirement parentTemplate = newTemplate("Parent-Template", "label:Code-Review=+1");
    SubmitRequirement childTemplate = newTemplate("Child-Template", "label:Code-Review=+2");
    ...
    assertTemplateInfo(response.value().get(0), "Parent-Template", PARENT);
    assertTemplateInfo(response.value().get(1), "Child-Template", CHILD);
  }
  ...
  @Test
  public void skipUnreadableParentProject() throws Exception {
    PermissionBackend permissionBackend = mock(PermissionBackend.class);
    PermissionBackend.WithUser withUser = mock(PermissionBackend.WithUser.class);

    when(permissionBackend.currentUser()).thenReturn(withUser);

    ProjectState currentProjectState = projectResource.getProjectState();

    // Make the current project readable.
    PermissionBackend.ForProject currentPermissions = mock(PermissionBackend.ForProject.class);

    when(withUser.project(currentProjectState.getNameKey())).thenReturn(currentPermissions);

    // Make the parent unreadable.
    ProjectState parentProjectState = currentProjectState.treeInOrder().iterator().next();

    PermissionBackend.ForProject parentPermissions = mock(PermissionBackend.ForProject.class);

    when(withUser.project(parentProjectState.getNameKey())).thenReturn(parentPermissions);

    doThrow(new AuthException("not permitted"))
        .when(parentPermissions)
        .check(ProjectPermission.READ_CONFIG);

    ListSubmitRequirementTemplates endpoint =
  ...
  public void returnsCurrentProjectTemplatesWhenParentIsUnreadable() throws Exception {
    ...
    assertTemplateInfo(response.value().get(0), "Child-Template", CHILD);
  }

  @Test
  public void returnsTemplatesFromReadableAncestorsWhenOneMiddleParentIsUnreadable()
      throws Exception {
    ...
    when(withUser.project(child)).thenReturn(childPermissions);
    // Unreadable intermediate parent is skipped, readable ancestors still contribute templates.
    doThrow(new AuthException("not permitted"))
        .when(middleUnreadablePermissions)
        .check(ProjectPermission.READ_CONFIG);
    ...
    SubmitRequirement grandparentTemplate =
        newTemplate("Grandparent-Template", "label:Code-Review=+1");
    SubmitRequirement childTemplate = newTemplate("Child-Template", "label:Code-Review=+2");
    SubmitRequirement middleReadableTemplate =
        newTemplate("Middle-Readable-Template", "label:Verified=+1");
    ...
    assertTemplateInfo(response.value().get(0), "Grandparent-Template", grandparent);
    assertTemplateInfo(response.value().get(1), "Middle-Readable-Template", middleReadableParent);
    assertTemplateInfo(response.value().get(2), "Child-Template", child);
  }
}
