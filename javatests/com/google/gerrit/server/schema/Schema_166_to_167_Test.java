...
public class Schema_166_to_167_Test {
  ...
  private static Config createConfigWithServerId() {
    ...
    // TODO(aliceks): Switch to SQL and then remove these settings (currently necessary as we wrap
    // ReviewDb and disallow reading from the groups tables.)
    config.setBoolean(SECTION_NOTE_DB, GROUPS.key(), WRITE, false);
  ...
  public InMemoryTestEnvironment testEnv =
      new InMemoryTestEnvironment(Schema_166_to_167_Test::createConfigWithServerId);
  ...
  @Test
  public void allGroupsAreMigratedToNoteDb() throws Exception {
  ...
  @AutoValue
  public abstract static class TestGroup {
    ...
    public static Builder builder() {
      return new AutoValue_Schema_166_to_167_Test_TestGroup.Builder().setVisibleToAll(false);
    }
    ...
    public abstract static class Builder {
      ...
      public AccountGroup build() {
        ...
        AccountGroup.NameKey name =
            testGroup.getNameKey().orElse(new AccountGroup.NameKey("users"));
        AccountGroup.Id id =
            testGroup.getId().orElse(new AccountGroup.Id(Math.abs(name.hashCode())));
