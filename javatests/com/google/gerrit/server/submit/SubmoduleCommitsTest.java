...
public class SubmoduleCommitsTest {
  ...
  public void createGitlinksCommit_subprojectMoved() throws Exception {
    ...
    SubmoduleCommits helper =
        new SubmoduleCommits(mergeOpRepoManager, ident, new Config(), new CachedBranchTips());
    CodeReviewCommit newGitLinksCommit =
        helper.composeGitlinksCommit(BranchNameKey.create(superProject, MASTER), Arrays.asList(ss));

    assertThat(newGitLinksCommit.getParent(0)).isEqualTo(superprojectTip);
    assertThat(readGitLink(superProject, newGitLinksCommit, "dir-x"))
  ...
  public void amendGitlinksCommit_subprojectMoved() throws Exception {
    ...
    CodeReviewCommit amendedCommit =
        helper.amendGitlinksCommit(
            BranchNameKey.create(superProject, MASTER), superprojectTip, Arrays.asList(ss));
  ...
  // TODO(ifrade): From SubmoduleSubscriptionIT. API for gitlink manipulation?
  protected CodeReviewCommit directUpdateSubmodule(
