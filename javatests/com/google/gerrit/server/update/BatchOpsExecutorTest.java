...
public class BatchOpsExecutorTest {
  ...
  public void changeUpdateOpBatch_notCleared_executeFails() throws Exception {
    ...
    try (BatchOpsExecutor batchOpsExecutor =
      ...
      IllegalStateException thrown =
          assertThrows(
              IllegalStateException.class,
              () -> batchOpsExecutor.addOp(createChange(), new PostChangeMessageUpdateOp()));
      assertThat(thrown)
          .hasMessageThat()
          .contains("update already executed, start new update or call clear()");
    }
  ...
  @Test
  public void changeUpdateOpBatch_addOp_forSameChangeFails() throws Exception {
    ...
    try (BatchOpsExecutor batchOpsExecutor =
      ...
      for (Change.Id changeId : changes) {
        batchOpsExecutor.addOp(changeId, new PostChangeMessageUpdateOp());
      }
      ...
      IllegalArgumentException thrown =
          assertThrows(
              ...
              () -> batchOpsExecutor.addOp(changes.get(0), new PostChangeMessageUpdateOp()));
      assertThat(thrown)
          .hasMessageThat()
  ...
  @Test
  public void changeUpdateOpBatch_addOps_forSameChangeFails() throws Exception {
    List<Change.Id> changes = new ArrayList<>();
    for (int i = 0; i < 10; i++) {
      Change.Id changeId = createChange();
      changes.add(changeId);
  ...
  public void changeUpdateOpBatch_withDistinctUpdate_allCounted() throws Exception {
    ...
    // Distinct updates result in separate commits in NotedDb, but should be still counted as a
    // singlre ref
    for (int i = 0; i < 10; i++) {
      Change.Id changeId = createChange();
      ops.put(
  ...
  public void testAllRefsCounted(
    ...
    try (BatchOpsExecutor batchOpsExecutor =
      ...
      for (RepoOnlyOp repoOnlyOp : repoOnlyOps) {
        batchOpsExecutor.addRepoOnlyOp(repoOnlyOp);
      }
      for (Change.Id changeId : ops.keySet()) {
        batchOpsExecutor.addOps(changeId, ops.get(changeId));
      }
      assertThat(batchOpsExecutor.isExecuted()).isFalse();
  ...
  public void testNonAtomicChangeUpdateOpForSameChangeFails(
    ...
    try (BatchOpsExecutor batchOpsExecutor =
        batchOpsExecutorFactory.create(
            project, user.get(), TimeUtil.nowTs(), /*nonAtomic=*/ true)) {
      batchOpsExecutor.addOps(changeId, ops);
      for (RepoOnlyOp repoOnlyOp : repoOnlyOps) {
        batchOpsExecutor.addRepoOnlyOp(repoOnlyOp);
      }
      assertThat(batchOpsExecutor.isExecuted()).isFalse();
  ...
  private static class ThrowingUpdateRepoOnly implements RepoOnlyOp {
    ...
    @Override
    public void updateRepo(RepoContext ctx) throws BadRequestException {
