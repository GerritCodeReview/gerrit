...
public class ConfigSuiteTest {
  ...
  public void methodsExecuteInCorrectOrder() throws Exception {
    ...
    inOrder.verify(configBasedTestListener, Mockito.times(1))
        .classRuleExecuted();
    inOrder.verify(configBasedTestListener, Mockito.times(1))
        .beforeClassExecuted();
    // default config
    inOrder.verify(configBasedTestListener, Mockito.times(1))
        .configRuleExectued();
    inOrder.verify(configBasedTestListener, Mockito.times(1))
        .beforeConfigExecuted();
    inOrder.verify(configBasedTestListener, Mockito.times(2))
        .testExecuted(any(), any(), any());
    inOrder.verify(configBasedTestListener, Mockito.times(1))
        .afterConfigExecuted();
    // first config
    inOrder.verify(configBasedTestListener, Mockito.times(1))
  ...
  public void beforeConfigExecutedForEachConfig() throws Exception {
    ...
    verify(configBasedTestListener, Mockito.times(3))
        .beforeConfigExecuted();
  }
  ...
  public void testsExecuteWithCorrectConfigAndName() throws Exception {
    new ConfigSuite(ConfigBasedTest.class).run(notifier);
    verify(configBasedTestListener, Mockito.times(6))
        .testExecuted(any(), any(), any());

    verify(configBasedTestListener, Mockito.times(1))
        .testExecuted("test1", "default", null);
    verify(configBasedTestListener, Mockito.times(1))
        .testExecuted("test2", "default", null);
    verify(configBasedTestListener, Mockito.times(1))
  ...
  public static class ConfigBasedTest {
    ...
    @ConfigRule
    public static TestRule configRule = new TestRule() {
      @Override
      public Statement apply(Statement statement,
          Description description) {
        return new Statement() {
