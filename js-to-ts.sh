...
rulesdir=~/gerrit/tools/js/eslint-rules

echo "rulesdir: $rulesdir"
echo "Fixing files: Step 1:"
node ~/gerrit/node_modules/eslint/bin/eslint.js ${allTsFiles[@]} --fix --rulesdir $rulesdir

echo "Fixing files: Step 2:"
node ~/gerrit/node_modules/eslint/bin/eslint.js ${allTsFiles[@]} --fix --rulesdir $rulesdir --rule "polymer-element:2"
