...
public class Main {
  ...
  @Option(name = "-D", usage = "destination output directory (default: no default, must specify)")
  ...
  private String mapInFileToOutFileWithDir(String inFile) {
    String outFile = destinationDir;
    if (!outFile.endsWith(File.separator)) {
      outFile += File.separator;
    }
    outFile += mapInFileToOutFile(inFile);
    return outFile;
  }

  public Options createOptions(String inFile) {
    OptionsBuilder optionsBuilder = OptionsBuilder.options();

    optionsBuilder.backend(backend).docType(DOCTYPE).eruby(ERUBY);
    // XXX(fishywang): ideally we should just output to a string and add the content into
    // zip. But asciidoctor actually will ignore all attributes if not output to a file.
    // So we have to output to file then read the content of the file into zip.
    optionsBuilder.toFile(new File(mapInFileToOutFileWithDir(inFile)));
  ...
  private void invoke(String... parameters) {
    ...
    try {
      ...
      for (String inputFile : inputFiles) {
        Options options = createOptions(inputFile);
        renderInput(options, inputFile);

        FileInputStream f = new FileInputStream(new File(mapInFileToOutFileWithDir(inputFile)));
        int len;
