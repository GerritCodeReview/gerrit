...
`resolved.json` is generated by `npm_translate_lock` from the checked-in
lock data, so the Bazel compiler tracks the resolved TypeScript version and
integrity used by `@npm`. The compiler pin lives in the root `package.json`;
`tools/node_tools/package.json` carries the matching pin for node-tools.
