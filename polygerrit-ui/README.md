...
```sh
bazel run //polygerrit-ui:server -- --plugins=plugins/my_plugin/static/my_plugin.js
```

Loading multiple plugin files:

```sh
bazel run //polygerrit-ui:server -- --plugins=plugins/my_plugin/static/my_plugin.js,plugins/my_plugin/static/my_plugin.html
```
