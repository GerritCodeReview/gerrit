...
Frontend dependencies are managed with `pnpm`, and lockfiles are checked into
the repository. The `pnpm-lock.yaml` files are the source of truth.

In normal development you usually do **not** need to run `pnpm install`
manually in subdirectories. Bazel will set up the required npm dependencies for
you.

For first-time setup, building Gerrit is typically enough:

```sh
bazel build gerrit
```

If you need to refresh lockfiles after changing a `package.json`, then regenerate
the corresponding `pnpm-lock.yaml` from the affected workspace.

Do not reintroduce `yarn.lock` files. Gerrit uses `pnpm` now.
