...
The `tsc` used by the Bazel build is selected by `rules_ts` in
`polygerrit-ui/polygerrit.MODULE.bazel`:
...
`resolved.json` is generated by `npm_translate_lock` from the lockfile, so the
Bazel compiler tracks the version and integrity that yarn actually resolved —
the same flow as the dependency management above. The exact pin still lives in
the `typescript` entry of the root `package.json` (and `tools/node_tools/
package.json`); `resolved.json` just reports whatever that pin resolved to, and
supplies the integrity from the lockfile rather than from an `rules_ts` mirror
table. To change the compiler version, edit the `package.json` pin and rerun
the yarn → pnpm workflow above.
