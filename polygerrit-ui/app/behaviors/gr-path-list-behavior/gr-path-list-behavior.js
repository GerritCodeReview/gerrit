...
export const PathListBehavior = {
  ...
  specialFilePathCompare(a, b) {
    ...
    if (a === SpecialFilePath.COMMIT_MESSAGE_PATH) {
      return -1;
    }
    if (b === SpecialFilePath.COMMIT_MESSAGE_PATH) {
      return 1;
    }

    // The merge list always comes next.
    if (a === SpecialFilePath.MERGE_LIST_PATH) {
      return -1;
    }
    if (b === SpecialFilePath.MERGE_LIST_PATH) {
      return 1;
    }
  ...
  shouldHideFile(file) {
    return (file === SpecialFilePath.PATCHSET_LEVEL_COMMENTS_FILE_PATH);
  },
  ...
  computeDisplayPath(path) {
    if (path === SpecialFilePath.COMMIT_MESSAGE_PATH) {
      return 'Commit message';
    } else if (path === SpecialFilePath.MERGE_LIST_PATH) {
      return 'Merge list';
    }
    return path;
  },
  ...
  isMagicPath(path) {
    return !!path &&
        (path === SpecialFilePath.COMMIT_MESSAGE_PATH || path ===
            SpecialFilePath.MERGE_LIST_PATH);
  },
