...
(function() {
  ...
  Polymer({
    ...
    _createProject() {
      //const promises = [];
      //promises.push(this._projectConfig = []);
      this._projectConfig = []; //Promise.all(promises);
    },
      ...
      return this.$.restAPI.createProject(
          ...
          projectRegistered => {
              if (projectRegistered.status === 201) {
                ...
                this.$.projectCreated.open();
              }
              if (projectRegistered.status === 409) {
                this._projectRegistered = true;
              }
          });
    },
