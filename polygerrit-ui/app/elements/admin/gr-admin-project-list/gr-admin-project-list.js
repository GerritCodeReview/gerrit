...
(function() {
  ...
  Polymer({
    ...
    _preventDefaultAndNavigateToInputVal(e) {
      ...
      if (this._inputVal) {
        page.show('/admin/projects' + ',' + this.encodeURL(this._inputVal, false));
      }
    },
    ...
    _paramsChanged(value) {
      ...
      return this.$.restAPI.getProjects(this._project, this._projectsPerPage, this._offset)
          .then(projects => {
            if (!projects) {
              this._projects = [];
              return;
    ...
    _hideNextArrow(hideNext, projects) {
      if (projects.length < 25) {
        hideNext = true;
      }
      return hideNext || !projects || !projects.length;
    },
