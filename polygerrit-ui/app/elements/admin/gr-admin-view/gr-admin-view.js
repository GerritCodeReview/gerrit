...
(function() {
  ...
  Polymer({
    ...
    properties: {
      ...
      _formattedLinks: {
        type: Array,
        computed: '_computeSideLinks(_unformattedLinks)',
      },
      _unformattedLinks: {
        type: Array,
        value() { return []; },
    ...
    reload() {
      return this.$.restAPI.getAccount().then(account => {
        ...
        if (!account) {
          return this._unformattedLinks = ADMIN_LINKS.filter(link => {
    ...
    _loadAccountCapabilities() {
      return this.$.restAPI.getAccountCapabilities(ACCOUNT_CAPABILITIES)
          .then(capabilities => {
            this._unformattedLinks = ADMIN_LINKS.filter(link => {
    ...
    _computeAdminTopLinks(adminLinks) {
      if (!adminLinks || !adminLinks.length) {
        return ADMIN_LINKS.filter(link => link.viewableByAll);
      }
      return adminLinks;
    },
