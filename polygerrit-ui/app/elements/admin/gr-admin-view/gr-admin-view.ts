...
export class GrAdminView extends LitElement {
  ...
  private renderRepoList() {
    if (!isAdminView(this.viewState)) return;
    if (this.viewState.adminView !== AdminChildView.REPOS) return;
  ...
  private renderGroupList() {
    if (!isAdminView(this.viewState)) return;
    if (this.viewState.adminView !== AdminChildView.GROUPS) return;
  ...
  private renderPluginList() {
    if (!isAdminView(this.viewState)) return;
    if (this.viewState.adminView !== AdminChildView.PLUGINS) return;
  ...
  private renderRepoMain() {
    if (!isRepoView(this.viewState)) return;
    const detail = this.viewState.detail ?? RepoDetailView.GENERAL;
    if (detail !== RepoDetailView.GENERAL) return;
  ...
  private renderGroup() {
    if (!isGroupView(this.viewState)) return;
    if (this.viewState.detail !== undefined) return;
  ...
  private renderGroupMembers() {
    if (!isGroupView(this.viewState)) return;
    if (this.viewState.detail !== GroupDetailView.MEMBERS) return;
  ...
  private renderGroupAuditLog() {
    if (!isGroupView(this.viewState)) return;
    if (this.viewState.detail !== GroupDetailView.LOG) return;
  ...
  private renderRepoCommands() {
    if (!isRepoView(this.viewState)) return;
    if (this.viewState.detail !== RepoDetailView.COMMANDS) return;
  ...
  private renderRepoAccess() {
    if (!isRepoView(this.viewState)) return;
    if (this.viewState.detail !== RepoDetailView.ACCESS) return;
  ...
  private renderRepoDashboards() {
    if (!isRepoView(this.viewState)) return;
    if (this.viewState.detail !== RepoDetailView.DASHBOARDS) return;
