  ...
  Polymer({
    ...
    handleCreateChange() {
      if (this.$.privateChangeCheckBox.checked) {
        this.opt_params.is_private = true;
      }
      if (this.$.wipChangeCheckBox.checked) {
        this.opt_params.work_in_progress = true;
      }
      this.project = this.projectName;
      if (!this.subject) {
        this.subject = 'Insert the description of the change.';
      }
      return this.$.restAPI.createChange(this.project, this.branch,
          this.subject, this.topic, this.opt_params)
          .then(changeCreated => {
            if (!changeCreated) {
              return '';
            }
