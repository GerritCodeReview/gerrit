...
export class GrCreateChangeDialog extends LitElement {
  ...
  @state() private query?: (input: string) => Promise<{name: BranchName}[]>;
        ...
        <section class=${this.baseChange ? 'hide' : ''}>
          ...
          <span class="value">
            <gr-autocomplete
              ...
              @text-changed=${this.handleBranchTextChanged}
            >
            </gr-autocomplete>
          </span>
        </section>
        <section class=${this.baseChange ? 'hide' : ''}>
          ...
          <span class="value">
            <iron-input
              ...
              @bind-value-changed=${this.handleBaseCommitBindValueChanged}
            >
              <input
                id="baseCommitInput"
                maxlength="40"
        ...
        <section>
          ...
          <span class="value">
            <iron-input
              ...
              @bind-value-changed=${this.handleTopicBindValueChanged}
            >
              <input
                id="tagNameInput"
                maxlength="1024"
        ...
        <section id="description">
          ...
          <span class="value">
            <iron-autogrow-textarea
              ...
              @bind-value-changed=${this.handleSubjectBindValueChanged}
            >
            </iron-autogrow-textarea>
          </span>
        </section>
        <section class=${this.privateChangesEnabled ? 'hide' : ''}>
          ...
          <span class="value">
            <input
              ...
              ?checked=${this.formatBooleanString(this.privateByDefault)}
            />
          </span>
        </section>
      </div>
  ...
  /* private but used in test */
  formatBooleanString(config?: InheritedBooleanInfo) {
    if (config?.configured_value === InheritedBooleanInfoConfiguredValue.TRUE) {
      return true;
    } else if (
  ...
  private handleBranchTextChanged(e: CustomEvent) {
    this.branch = e.detail.value;
  }

  private handleBaseCommitBindValueChanged(e: BindValueChangeEvent) {
    this.baseCommit = e.detail.value;
  }

  private handleTopicBindValueChanged(e: BindValueChangeEvent) {
    this.topic = e.detail.value;
  }

  private handleSubjectBindValueChanged(e: BindValueChangeEvent) {
