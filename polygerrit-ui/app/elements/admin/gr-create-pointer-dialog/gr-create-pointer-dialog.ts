...
export class GrCreatePointerDialog extends LitElement {
      ...
      <div class="gr-form-styles">
        <div id="form">
          ...
          <section
            ...
            class=${this.itemDetail === RepoDetailView.TAGS
              ? 'hideItem'
              : ''}
          >
  ...
  handleCreateItem() {
    ...
    const CREATE_EMPTY_COMMIT = this.createEmptyCommit ? this.createEmptyCommit : false;
    if (this.itemDetail === RepoDetailView.BRANCHES) {
      const CREATE_BRANCH_INPUT = CREATE_EMPTY_COMMIT ? {create_empty_commit: true} : {revision: USE_HEAD};
