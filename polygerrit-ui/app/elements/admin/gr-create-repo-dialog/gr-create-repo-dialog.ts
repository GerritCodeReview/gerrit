...
export class GrCreateRepoDialog extends LitElement {
  ...
  handleCreateRepo() {
    ...
    const repoRegistered = await this.restApiService.createRepo(this.repoConfig);
  ...
  async _getRepoSuggestions(input: string) {
  ...
  async _getGroupSuggestions(input: string) {
  ...
  private handleNameBindValueChanged(e: CustomEvent) {
    fireEvent(this, 'has-new-repo-name', !!e.detail.value);
