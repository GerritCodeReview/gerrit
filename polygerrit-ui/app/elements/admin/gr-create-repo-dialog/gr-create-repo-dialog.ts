...
export class GrCreateRepoDialog extends LitElement {
  ...
  async _retrieveDefaultBranch() {
    ...
    if (config && config.gerrit.default_branch) this.defaultBranch = config.gerrit.default_branch;
  }

  async handleCreateRepo() {
    if (this.selectedDefaultBranch) this.repoConfig.branches = [this.selectedDefaultBranch];
