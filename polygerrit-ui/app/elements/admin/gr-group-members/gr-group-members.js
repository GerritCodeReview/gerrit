    ...
    _loadGroupDetails() {
      ...
      return this.$.restAPI.getGroupConfig(this.groupId).then(
          config => {
            ...
            promises.push(this.$.restAPI.getIsGroupOwner(config.name).then(
                configs => {
                  if (configs) {
                    this._groupOwner = true;
                  }
                }));
            promises.push(this.$.restAPI.getGroupMembers(config.name).then(members => {
