...
export class GrGroup extends LitElement {
  ...
  @state() private groupOwnerName?: string;

  @state() private groupDescriptionName?: string;

  @state() private groupOptionsVisibleToAll?: boolean;
  ...
  override render() {
    const groupNameEdited = this.groupName !== this.groupConfig?.name;
    const groupOwnerNameEdited =
      this.groupOwnerName !== this.groupConfig?.owner;
    const groupDescriptionEdited =
      this.groupDescriptionName !== this.groupConfig?.description;
    const groupOptionsEdited =
      this.groupOptionsVisibleToAll !==
        ...
        <div id="loadedContent" class="${this.computeLoadingClass()}">
          ...
          <div id="form">
            <fieldset>
              ...
              <h3
                id="groupName"
                class="heading-3 ${this._computeHeaderClass(groupNameEdited)}"
              >
                Group Name
              </h3>
              <fieldset>
              ...
              <h3
                id="groupOwner"
                class="heading-3 ${this._computeHeaderClass(
                  groupOwnerNameEdited
                )}"
              >
                Owners
              </h3>
              <fieldset>
                <span class="value">
                  <gr-autocomplete
                    ...
                    .value=${convertToString(this.groupIdNameDirty)}
                    .query=${this.query}
                    ?disabled=${this._computeGroupDisabled()}
              ...
              <fieldset>
                <div>
                  <gr-textarea
                    ...
                    @text-changed=${this.handleDescriptionBindValueChanged}
                  >
                </div>
                <span class="value" ?disabled=${this._computeGroupDisabled()}>
                  <gr-button
              ...
              <fieldset>
                <section>
                  ...
                  <span class="value">
                    ...
                    >
                      <select ?disabled=${this._computeGroupDisabled()}>
  ...
  async handleSaveOwner() {
    ...
    if (this.groupIdNameDirty) {
      owner = decodeURIComponent(this.groupIdNameDirty);
    }
    if (!owner) return;
  ...
  async handleSaveOptions() {
    ...
    this.groupOptionsVisibleToAll = this.groupConfig?.options?.visible_to_all;
    this.requestUpdate();
  }
  ...
  private handleOwnerValueChanged(e: CustomEvent) {
    ...
    this.groupIdNameDirty = e.detail.value;
    this.requestUpdate();
  }
