...
export class GrGroup extends LitElement {
  ...
  private renderGroupDeleteButton() {
    return html`
      <h3 id="deleteGroup" class="heading-3 ${this.computeHeaderClass(false)}">
        Delete Group
      </h3>
      <fieldset>
        <span class="value">
  ...
  async handleDeleteGroup() {
    ...
    await this.restApiService.deleteGroup(this.originalName).then(_ => {
      this.restApiService.invalidateGroupsCache();
      this.deleteGroupModal?.close();
      window.location.href = `${getBaseUrl()}/admin/groups`;
    });
  }
