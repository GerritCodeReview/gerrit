...
export class GrPermission extends LitElement {
   ...
   */
  handleAddRuleItem(e: AutocompleteCommitEvent) {
    if (!this.permission || !this.rules) {
      return;
    }
  ...
  private handleRuleChanged(e: CustomEvent, index: number) {
    ...
    this.rules.splice(index, (e as CustomEvent).detail.value);
    this.handleRulesChanged();
    this.requestUpdate();
    // this.splice('rules', index, (e as CustomEvent).detail.value);
  }
}
