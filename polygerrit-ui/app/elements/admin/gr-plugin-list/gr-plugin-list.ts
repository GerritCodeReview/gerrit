...
export class GrPluginList extends LitElement {
  ...
  private renderPluginList(plugin: PluginInfoWithName) {
    return html`
      <tr class="table">
        <td class="name">
          ${plugin.index_url
            ? html`<a href=${this._computePluginUrl(plugin.index_url)}
                >${plugin.id}</a
              >`
            : plugin.id}
        </td>
        ...
        <td class="status">${this._status(plugin)}</td>
      </tr>
    `;
  }
  ...
  paramsChanged() {
    ...
    return this._getPlugins(this.filter, this.pluginsPerPage, this.offset);
  }
