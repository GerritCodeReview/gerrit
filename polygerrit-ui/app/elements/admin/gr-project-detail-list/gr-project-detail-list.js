  ...
  Polymer({
    ...
    _determineIfOwner(project) {
      return this._getLoggedIn().then(loggedIn => {
        if (loggedIn) {
          this.$.restAPI.getProjectAccess(project).then(access => {
            // If the user is not an owner, is_owner is not a property.
            this._isOwner = access[project].is_owner;
          });
        }
      });
    },
    ...
    _computeCanEditClass(ref, detailType, isOwner) {
      return isOwner && this._stripRefs(ref, detailType) === 'HEAD' ? 'canEdit'
          : '';
    },
    ...
    _setProjectHead(project, ref, event) {
      return this.$.restAPI.setProjectHead(project, ref).then(
          res => {
            if (res.status < 400) {
              this._isEditing = false;
