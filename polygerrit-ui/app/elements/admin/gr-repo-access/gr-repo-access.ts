...
export type EditableAccessSectionInfo = AccessSectionInfo & PropertyTreeNode;
...
export class GrRepoAccess extends GestureEventListeners(
  ...
  _query: AutocompleteQueryCallback;
    ...
    const sectionsPromises = this.$.restAPI
      ...
      .then(res => {
        ...
        this._inheritFromFilter = res.inherits_from
          ...
          : ('' as RepoName);
        this._local = (res.local as unknown) as EditableLocalAccessSectionInfo;
  ...
  _recursivelyUpdateAddRemoveObj(
    ...
    for (const k in obj) {
      ...
      if (typeof node === 'object') {
        ...
        if (node.deleted) {
          this._updateRemoveObj(addRemoveObj, pathFromRoot.concat(k));
          continue;
        } else if (node.modified) {
          this._updateRemoveObj(addRemoveObj, pathFromRoot.concat(k));
          this._updateAddObj(addRemoveObj, pathFromRoot.concat(ref), node);
