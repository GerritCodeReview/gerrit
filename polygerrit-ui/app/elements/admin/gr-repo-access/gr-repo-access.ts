...
export class GrRepoAccess extends LitElement {
        ...
        >
          ...
          >
            ...
            <gr-autocomplete
              ...
              .query=${this._query}
              @commit=${this._handleUpdateInheritFrom}
              @bind-value-changed=${this._handleUpdateInheritFrom}
              @text-changed=${(e: CustomEvent) => { this._handleEditInheritFromTextChanged(e); }}
            ></gr-autocomplete>
          </h3>
          ...
          <div>
            <gr-button id="editBtn" @click=${() => { this._handleEdit(); }}
              >${this._editOrCancel(this._editing)}</gr-button
            >
            <gr-button
              id="saveBtn"
  ...
  private renderPermissionSections(
    ...
    return html`
      <gr-access-section
        ...
        .groups=${this._groups}
        .repo=${this.repo}
        @added-section-removed=${this._handleAddedSectionRemoved}
        @section-changed=${(e: CustomEvent) =>
  ...
  override willUpdate(changedProperties: PropertyValues) {
    ...
    if (changedProperties.has('_editing')) {
      this._handleEditingChanged(
        this._editing,
        changedProperties.get('_editing') as boolean
      );
    }
  }
  ...
  async _handleCreateSection() {
    ...
    queryAndAssert<GrAccessSection>(this, 'gr-access-section:last-of-type').editReference();
    this.requestUpdate();
  }
  ...
  _handleAccessSectionChanged(e: CustomEvent, index: number) {
