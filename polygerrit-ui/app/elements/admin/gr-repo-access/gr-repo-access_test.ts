...
suite('gr-repo-access tests', () => {
  ...
  test('inherit section', async () => {
    ...
    assert.equal(getComputedStyle(queryAndAssert<HTMLHeadingElement>(element, '#inheritsFrom')).display, 'none');
    ...
    assert.notEqual(getComputedStyle(queryAndAssert<HTMLHeadingElement>(element, '#inheritsFrom')).display, 'none');
    assert.notEqual(
      getComputedStyle(queryAndAssert<HTMLAnchorElement>(element, '#inheritFromName')).display,
      'none'
    );
    assert.equal(
      getComputedStyle(
    ...
    assert.notEqual(getComputedStyle(queryAndAssert<HTMLHeadingElement>(element, '#inheritsFrom')).display, 'none');
    assert.equal(getComputedStyle(queryAndAssert<HTMLAnchorElement>(element, '#inheritFromName')).display, 'none');
    assert.notEqual(
      getComputedStyle(
        queryAndAssert<GrAutocomplete>(element, '#editInheritFromInput')
      ).display,
  ...
  suite('with defined sections', () => {
    ...
    test('button visibility for non ref owner', () => {
      assert.equal(getComputedStyle(queryAndAssert<GrButton>(element, 'saveReviewBtn')).display, 'none');
      assert.equal(getComputedStyle(queryAndAssert<GrButton>(element, 'editBtn')).display, 'none');
    });
    ...
    test('_computeAddAndRemove new section', async () => {
      ...
      MockInteractions.tap(queryAndAssert<GrButton>(element, '#addReferenceBtn'));
      await element.updateComplete;
      assert.deepEqual(element._computeAddAndRemove(), expectedInput);
    ...
    test('_handleSave', async () => {
      ...
      MockInteractions.tap(queryAndAssert<GrButton>(element, '#saveBtn'));
      await element.updateComplete;
      assert.equal(queryAndAssert<GrButton>(element, '#saveBtn').hasAttribute('loading'), true);
    ...
    test('_handleSaveForReview', async () => {
      ...
      element._modified = true;
      MockInteractions.tap(queryAndAssert<GrButton>(element, '#saveReviewBtn'));
      await element.updateComplete;
      assert.equal(queryAndAssert<GrButton>(element, '#saveReviewBtn').hasAttribute('loading'), true);
