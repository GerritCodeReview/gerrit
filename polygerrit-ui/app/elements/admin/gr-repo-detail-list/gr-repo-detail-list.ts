...
export class GrRepoDetailList extends LitElement {
  ...
  @state() private revisedRef?: GitRef;
  ...
  private renderTagger(tagger?: TagInfo) {
    if (!tagger) return;
  ...
  private determineIfOwner(repo: RepoName) {
  ...
  private stripRefs(item: GitRef, detailType?: RepoDetailView) {
    if (detailType === RepoDetailView.BRANCHES) {
  ...
  private handleSaveRevision(index: number) {
