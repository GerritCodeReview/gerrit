...
import { RepoName, ProjectInfo } from '../../../types/common';
...
export class GrRepoDetailList extends ListViewMixin(
  ...
  _getItems(filter: string, repo: RepoName | undefined, itemsPerPage: number, offset: number, detailType: string) {
    this._loading = true;
    this._items = [];
    flush();
  ...
  _stripRefs(item: string, detailType: string) {
    if (detailType === DETAIL_TYPES.BRANCHES) {
