...
(function() {
  ...
  Polymer({
    ...
    _getNotifications(notifyConfig) {
      ...
      result.sort((a, b) =>
          a.editable_data.name.localeCompare(b.editable_data.name));
      return result;
    },
    ...
    _handleNotificationChanged(e) {
      const index = this._getNotificiationIndexFromEvent(e);
      this.set(['_notifications', index, 'modified'], true);
    },
    ...
    _createEditableData(originalData) {
      const emails = originalData.emails ? originalData.emails
          .map(email => { return {email}; }) : [];
      const groups = originalData.groups ? originalData.groups
          .map(groupInfo=> {
    ...
    _handleSave() {
      ...
      return this.$.restAPI.notifications.changeRepoNotifications(
          this.repo,
          this._description,
          changes.removals,
          changes.additions,
