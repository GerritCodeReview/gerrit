...
(function() {
  ...
  Polymer({
    ...
    _computePluginConfigOptions(dataRecord) {
      if (!dataRecord || !dataRecord.base) { return []; }
      const {config} = dataRecord.base;
      return Object.keys(config).map(_key => ({_key, info: config[_key]}));
    },

    _isArray(type) {
      return type === EntryTypes.ARRAY;
    },

    _isBoolean(type) {
      return type === EntryTypes.BOOLEAN;
    },

    _isList(type) {
      return type === EntryTypes.LIST;
    },

    _isString(type) {
      // Treat numbers like strings for simplicity.
      return type === EntryTypes.STRING ||
          type === EntryTypes.INT ||
          type === EntryTypes.LONG;
    },
    ...
    _handleChange({_key, info, notifyPath}) {
      ...
      this.dispatchEvent(new CustomEvent('plugin-config-changed',
          {detail, bubbles: true}));
    },
  });
})();
