...
import { RestApiService, ErrorCallback } from '../../../services/services/gr-rest-api/gr-rest-api';
import { ConfigInfo, RepoName, InheritedBooleanInfo } from '../../../types/common';
...
export class GrRepo extends GestureEventListeners(LegacyElementMixin(PolymerElement)) {
  ...
  @property({type: Array})
  _submitTypes: unknown = Object.values(SUBMIT_TYPES);
  ...
  _computePluginData(configRecord: PolymerDeepPropertyChange<>) {
  ...
  _formatRepoConfigForSave(repoConfig: ConfigInfo) {
  ...
  _computeCommands(repo, schemesObj, _selectedScheme) {
    ...
    for (const title in commandObj) {
      if (!commandObj.hasOwnProperty(title)) {
        continue;
      }
      commands.push({
        title,
