...
export class GrRepo extends GestureEventListeners(
  ...
  // PluginData cannot be null - they are from gr-repo-plugin-config.ts
  @property({
    type: Array,
  ...
  // This is workaround to have _schemes with default value [], because assignement
  // doesn't work when property has computed attribute.
  @property({type: Array})
  ...
  // TODO(TS): clean this code and types
  _formatRepoConfigForSave(repoConfig: ConfigInfo): ConfigInput {
    const configInputObj: {[key: string]: unknown} = {};
    for (const key in repoConfig) {
      if (hasOwnProperty(repoConfig, key)) {
        ...
        if (key === 'plugin_config') {
          configInputObj.plugin_config_values =
            repoConfig[key as keyof ConfigInfo];
        } else if (typeof repoConfig[key as keyof ConfigInfo] === 'object') {
          const repoConfigObj: any = repoConfig[key as keyof ConfigInfo];
          if (repoConfigObj.configured_value) {
            configInputObj[key] = repoConfigObj.configured_value;
          }
        } else {
          configInputObj[key] = repoConfig[key as keyof ConfigInfo];
        }
      }
    }
    return configInputObj as ConfigInput;
  }

  _handleSaveRepoConfig() {
    if (!this._repoConfig)
      return Promise.reject(new Error('repoConfig undefined'));
    return this.$.restAPI
      .saveRepoConfig(
        this.repo,
