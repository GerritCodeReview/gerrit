...
export class GrServerInfo extends LitElement {
  ...
  private renderServerInfoTable() {
    ...
    return html`
      <tbody>
        ${Array.from(this.metadataList, (metadata) => this.renderServerInfo(metadata))}
      </tbody>
    `;
  }
    ...
    return this.restApiService
      ...
      .then(serverInfo => {
        ...
        if (serverInfo?.accounts?.visibility) {
          let accountsVisibilityMetadata = {
            name: "accounts.visibility",
            value: serverInfo.accounts.visibility,
            description: "https://gerrit-review.googlesource.com/Documentation/config-gerrit.html#accounts.visibility",
          }
