...
@customElement('gr-change-list-abandon-button')
export class GrChangeListAbandonButton extends LitElement {
  @state()
  ...
  private shouldEnableButton() {
    // sample enable logic. Normally we would use permissions from model data
    // somehow.
  ...
  override render() {
    return html`
      <gr-button
        ?disabled=${!this.shouldEnableButton()}
        flatten
      ...
      <gr-overlay>
        <gr-dialog @cancel=${() => this.overlay?.close()}>
          <div slot="header">Abandon Changes</div>
          <div slot="main">
...
declare global {
  interface HTMLElementTagNameMap {
    'gr-change-list-abandon-button': GrChangeListAbandonButton;
  }
}
