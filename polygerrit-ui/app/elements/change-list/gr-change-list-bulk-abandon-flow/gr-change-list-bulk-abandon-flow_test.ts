...
suite('gr-change-list-bulk-abandon-flow tests', () => {
  ...
  test('closing dialog when all requests succeed triggers a reload', async () => {
    const changes: ChangeInfo[] = [
    ...
    await waitUntil(
      () =>
        element.progress.get(2 as NumericChangeId) === ProgressStatus.SUCCESSFUL
    );
  ...
  test('closing dialog when some request fails does not triggers a reload', async () => {
    const changes: ChangeInfo[] = [
      {...change1, actions: {abandon: {}}},
      {...change2, actions: {abandon: {}}},
    ];
