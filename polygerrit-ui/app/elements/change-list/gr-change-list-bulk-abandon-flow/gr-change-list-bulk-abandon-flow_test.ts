...
suite('gr-change-list-bulk-abandon-flow tests', () => {
  ...
  async function selectChange(change: ChangeInfo) {
    ...
    // const changes = model.getState().selectedChangeNums.map(num =>
    //   {return {...createChange(), _number: num}})
    // await model.sync(changes);
  }
  ...
  test.only('button state updates as changes are updated', async () => {
    debugger;
