...
export class GrChangeListBulkVoteFlow extends LitElement {
  ...
  override render() {
    ...
    const nonTriggerLabels = this.computeCommonPermittedLabels().filter(
  ...
  private isFlowEnabled() {
    return this.computeCommonPermittedLabels().length > 0;
  }
  ...
  computeCommonTriggerLabels() {
    ...
    return this.computeCommonPermittedLabels().filter(label =>
