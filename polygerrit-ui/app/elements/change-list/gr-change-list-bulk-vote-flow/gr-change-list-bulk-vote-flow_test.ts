...
suite('gr-change-list-bulk-vote-flow tests', () => {
  ...
  test('computeCommonPermittedLabels', async () => {
    ...
    const changes: ChangeInfo[] = [
      createChangeWithLabels(1 as NumericChangeId, ['a', 'b', 'c'], ['b']),
      createChangeWithLabels(2 as NumericChangeId, ['b', 'c', 'd'], ['b']),
    ...
    assert.deepEqual(element.computeCommonPermittedLabels(element.computePermittedLabels()), [
      {name: 'b', value: null},
      {name: 'c', value: null},
    ]);
    assert.deepEqual(element.computeCommonTriggerLabels(), [
