...
import {CopyLink} from '../../change/gr-copy-links/gr-copy-links';
...
export class GrChangeListCopyLinkFlow extends LitElement {
  static override get styles() {
    return css`
      ...
      iron-dropdown {
        --iron-dropdown-transition-duration: 0.2s;
      }
    `;
  }
  ...
  private getCopyLinks(): CopyLink[] {
    if (this.selectedChangeNums.length === 0) return [];
    const changeIds = this.selectedChangeNums.map(num => `change:${num}`).join('+OR+');
    ...
    return [
      ...
      },
      {
        label: 'Markdown',
        shortcut: 'm',
        value: this.selectedChanges.map(change => {
  ...
  override render() {
    ...
    return html`
      <gr-button
        ...
        link
        @click=${() => {
          const copyLinks = this.shadowRoot?.querySelector('gr-copy-links');
          if (copyLinks) {
            copyLinks.openDropdown();
          }
        }}
      >
        Copy Link
