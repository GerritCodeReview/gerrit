...
export class GrChangeListHashtagFlow extends LitElement {
  ...
  @state() private hashtagsToRemove: Set<Hashtag> = new Set();
  ...
  private renderExistingHashtagsMode() {
    ...
    const removeDisabled =
      this.hashtagsToRemove.size === 0 ||
      this.overallProgress === ProgressStatus.RUNNING;
    const applyToAllDisabled = this.hashtagsToRemove.size !== 1;
    return html`
      <div class="chips">
        ${hashtags.map(name => this.renderHashtagRemoveChip(name))}
      </div>
      <div class="footer">
  ...
  private renderHashtagRemoveChip(name: Hashtag) {
    const chipClasses = {
      chip: true,
      selected: this.hashtagsToRemove.has(name),
    };
    return html`
      <span
        class=${classMap(chipClasses)}
        @click=${() => this.toggleHashtagToRemove(name)}
      >
        ${name}
      </span>
    `;
    ...
    this.trackPromises(
      this.selectedChanges
        .filter(
          change =>
            change.hashtags &&
            change.hashtags.some(hashtag => this.hashtagsToRemove.has(hashtag))
        )
        .map(change =>
          this.restApiService.setChangeHashtag(change._number, {
            remove: Array.from(this.hashtagsToRemove.values()),
          })
        )
    );
  }
  ...
  private applyHashtagToAll() {
    ...
    this.trackPromises(
      this.selectedChanges.map(change =>
        this.restApiService.setChangeHashtag(change._number, {
          add: Array.from(this.hashtagsToRemove.values()),
        })
      )
    );
  }
  ...
  private toggleHashtagToRemove(name: Hashtag) {
    if (this.hashtagsToRemove.has(name)) {
      this.hashtagsToRemove.delete(name);
    } else {
      this.hashtagsToRemove.add(name);
    }
    this.requestUpdate();
  }
}
