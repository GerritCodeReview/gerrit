...
class GrChangeListItem extends mixinBehaviors( [
  ...
  _computeReviewers(change) {
    ...
    const all = change.reviewers.REVIEWER;
    const attention = all.filter(r => this._hasAttention(r));
    const noAttention = all.filter(r => !this._hasAttention(r));
    return attention.concat(noAttention);
  }
