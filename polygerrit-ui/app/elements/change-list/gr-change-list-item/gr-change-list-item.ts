...
class GrChangeListItem extends ChangeTableMixin(
  ...
  _computeLabelTitle(change: ChangeInfo, labelName: string) {
    const label: QuickLabelInfo | undefined =
  ...
  _computeLabelClass(change: ChangeInfo, labelName: string) {
    const label: QuickLabelInfo | undefined =
      change.labels && change.labels[labelName];
    // Mimic a Set.
    const classes: {
  ...
  _computeLabelValue(change: ChangeInfo, labelName: string) {
    const label: QuickLabelInfo | undefined =
      change.labels && change.labels[labelName];
    if (!label) {
      return '';
   ...
   */
  _computeRepoDisplay(change: ChangeInfo, truncate: boolean) {
