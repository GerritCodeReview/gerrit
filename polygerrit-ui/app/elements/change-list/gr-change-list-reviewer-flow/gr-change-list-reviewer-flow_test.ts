...
import {handleAdd} from '../../shared/gr-account-list/gr-account-list_test';
...
suite('gr-change-list-reviewer-flow tests', () => {
    ...
    expect(element).shadowDom.to.equal(/* HTML */ `
      ...
      >
        <gr-dialog role="dialog">
          <div slot="header">Set reviewers for 2 changes</div>
          <div slot="main">
            <div>
              <span>Reviewers:</span>
  ...
  suite('dialog', () => {
    ...
    test('adds reviewer', async () => {
      ...
      handleAdd(accountList, {account: accounts[2]});
      await flush();
    ...
    test('removes reviewer', async () => {
      const accountList = queryAndAssert<GrAccountList>(
        dialog,
        'gr-account-list'
      );
      accountList.removeAccount(accounts[0]);
      await flush();
      dialog.confirmButton!.click();
      await element.updateComplete;
