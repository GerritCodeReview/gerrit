...
export class GrChangeListSection extends LitElement {
  ...
  override render() {
    ...
    const results =
      this.limitInitialResults && !this.showFullSection
        ? this.changeSection.results.slice(0, INITIAL_RESULT_LIMIT)
  ...
  private renderLoadMore(colSpan: number) {
    if (
      ...
      this.changeSection.results.length <= INITIAL_RESULT_LIMIT
    ) {
      return;
    }
  ...
  private handleLoadMore() {
    ...
    // TODO: Add handling for this in GrDashboardView so that extra changes are fetched
    this.dispatchEvent(
