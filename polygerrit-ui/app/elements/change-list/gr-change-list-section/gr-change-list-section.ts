...
export class GrChangeListSection extends LitElement {
  ...
  override render() {
    ...
    return html`
      ...
      <tbody class="groupContent">
        ${this.isEmpty(this.changeSection)
          ? this.renderNoChangesRow()
          : this.renderColumnHeaders()}
        ${this.changeSection.results.map((change, index) =>
          this.renderChangeRow(change, index)
        )}
      </tbody>
    `;
  }
  ...
  private renderNoChangesRow() {
    return html`
      <tr class="noChanges">
        ...
        ></td>
        <td class="cell" colspan="${this.computeColspan(this.changeSection)}">
  ...
  private renderSectionHeader() {
    if (!this.changeSection || !this.changeSection.name) return;
  ...
  private renderColumnHeaders() {
    return html`
      <tr class="groupTitle">
        ...
        ${this.computeColumns(this.changeSection).map(item =>
  ...
  private renderChangeRow(change: ChangeInfo, index: number) {
    const ariaLabel = this.computeAriaLabel(change, this.changeSection!.name);
    ...
    const visibleChangeTableColumns = this.computeColumns(this.changeSection);
    return html`
      <gr-change-list-item
