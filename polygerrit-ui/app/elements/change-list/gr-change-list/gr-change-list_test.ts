...
suite('gr-change-list basic tests', () => {
  ...
  test('computeRelativeIndex', () => {
    ...
    assert.equal(element.computeRelativeIndex(selectedChangeIndex, 0), 0);

    // index lies outside the first section
    assert.equal(
      element.computeRelativeIndex(selectedChangeIndex, 1),
      undefined
    );
    ...
    // index lies outside the first section
    assert.equal(
      element.computeRelativeIndex(selectedChangeIndex, 0),
      undefined
    );

    // 3rd change belongs to the second section
    assert.equal(element.computeRelativeIndex(selectedChangeIndex, 1), 1);
  });
