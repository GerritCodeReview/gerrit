...
suite('gr-dashboard-view tests', () => {
  ...
  test('preserve isOutgoing sections', async () => {
    const sections = [
    ...
    assert.isTrue(element.results![0].isOutgoing);
    assert.isNotOk(element.results![1].isOutgoing);
  });
  ...
  test('_computeUserHeaderClass', async () => {
  ...
  test('params change triggers dashboardDisplayed()', async () => {
    ...
    const dashboardDisplayedStub = sinon.stub(
      element.reporting,
      'dashboardDisplayed'
    );
    element.params = {
