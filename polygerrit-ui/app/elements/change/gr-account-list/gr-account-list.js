  ...
  _template: Polymer.html`
    ...
    <div class="list">
      <template id="chips" is="dom-repeat" items="[[accounts]]" as="account">
        <gr-account-chip account="[[account]]" class\$="[[_computeChipClass(account)]]" data-account-id\$="[[account._account_id]]" removable="[[_computeRemovable(account)]]" on-keydown="_handleChipKeydown" tabindex="-1">
        </gr-account-chip>
      </template>
    </div>
    <gr-account-entry borderless="" hidden\$="[[_computeEntryHidden(maxCount, accounts.*, readonly)]]" id="entry" change="[[change]]" filter="[[filter]]" placeholder="[[placeholder]]" on-add="_handleAdd" on-input-keydown="_handleInputKeydown" allow-any-input="[[allowAnyInput]]" allow-any-user="[[allowAnyUser]]">
    </gr-account-entry>
`,
