...
test comment`;
  ...
  suite('dataAnonymization', () => {
    test('assigns alias starting from User1', () => {
      ...
      const result = (element as any).dataAnonymization([thread]);
    ...
    test('same author (by _account_id) gets the same alias', () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const result = (element as any).dataAnonymization([thread1, thread2]);
      assert.equal(result[0].comments[0].anonymizedAuthor, 'User1');
    ...
    test('returns empty array for empty input', () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const result = (element as any).dataAnonymization([]);
