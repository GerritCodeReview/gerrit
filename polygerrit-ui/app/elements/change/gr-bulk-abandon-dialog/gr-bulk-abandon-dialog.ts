...
export class GrBulkAbandonDialog extends LitElement {
  ...
  @state() progress: Map<ChangeInfoId, ProgressStatus> = new Map();
  ...
  override connectedCallback() {
    ...
    this.getBulkActionsModel().state$.subscribe(
      state => (this.progress = state.progress)
    );
  }
  ...
  private getStatus(changeId: ChangeInfoId) {
    return this.progress.has(changeId)
      ? this.progress.get(changeId)
