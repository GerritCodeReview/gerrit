  ...
  class GrChangeActions extends Polymer.mixinBehaviors( [
    ...
    _send(method, payload, actionEndpoint, revisionAction, cleanupFn, action,
      ...
      return this.fetchChangeUpdates(change, this.$.restAPI)
          .then(result => {
            ...
            const changeNum = change ? change._number : this.changeNum;
    ...
    _handleCherrypickTap() {
      ...
      const promises = [
        ...
        this.$.restAPI.getChangesSubmittedTogether(this.change._number),
      ];
      Promise.all(promises)
          .then(result => {
            const changes = result[0];
            const nonVisibleChangesCount = result[1].non_visible_changes;
