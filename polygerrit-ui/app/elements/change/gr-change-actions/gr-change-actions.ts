  ...
  implements GrChangeActionsElement
{
  ...
  async handleCherryPickChainRequested(e: CustomEvent<{changes: RelatedChangeAndCommitInfo[]}>) {
    const relatedChanges = e.detail.changes;
    const changeNums = relatedChanges.map(c => c._change_number).filter(isDefined);
    if (changeNums.length === 0) return;

    const detailedChanges = await this.restApiService.getDetailedChangesWithActions(changeNums);
    if (!detailedChanges || detailedChanges.length === 0) return;

    // Sort to match relation chain order, then reverse to get parent-first
    const orderedDetails = relatedChanges.map(rc =>
      detailedChanges.find(dc => dc._number === rc._change_number)
    ).filter(isDefined);
    ...
    assertIsDefined(this.confirmCherrypick, 'confirmCherrypick');
    this.confirmCherrypick.branch = '' as BranchName;
    this.confirmCherrypick.updateChanges(parentFirstChanges, CherryPickType.RELATION_CHAIN);
    this.showActionDialog(this.confirmCherrypick);
  }
