...
export class GrChangeActions
  ...
  showRevertDialog() {
    ...
    this.restApiService.getChanges(0, query).then(changes => {
      if (!changes) {
        this.reporting.reportError(new Error('changes is undefined'));
        return;
      }
  ...
  _handleRevertDialogConfirm(e: CustomEvent<ConfirmRevertEventDetail>) {
    ...
    switch (revertType) {
      ...
      case RevertType.REVERT_SUBMISSION:
        ...
        break;
      default:
        this.reporting.reportError(new Error('invalid revert type'));
    }
  }
  ...
  _handleCherrypickTap() {
    ...
    this.restApiService
      ...
      .then(changes => {
        if (!changes) {
          this.reporting.reportError(new Error('getChanges returns undefined'));
          return;
        }
