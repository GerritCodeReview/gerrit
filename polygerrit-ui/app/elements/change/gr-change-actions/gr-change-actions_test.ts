...
suite('gr-change-actions tests', () => {
  ...
  suite('basic tests', () => {
    ...
    test('buttons exist', async () => {
      ...
      const menuItems = queryAndAssert<GrDropdown>(element, '#moreActions').items;
    ...
    test('delete buttons have explicit labels', async () => {
      await element.updateComplete;
      const deleteItems = queryAndAssert<GrDropdown>(element, '#moreActions').items!.filter(item =>
    ...
    test('submit change', async () => {
      ...
      sinon.stub(queryAndAssert<GrOverlay>(element, '#overlay'), 'open').returns(Promise.resolve());
      element.change = {
        ...createChangeViewChange(),
        revisions: {
          rev1: {...createRevision(), _number: 1 as PatchSetNum},
      ...
      await element.updateComplete;
      assert.isTrue(showSpy.calledWith(queryAndAssert<GrConfirmSubmitDialog>(element, '#confirmSubmitDialog')));
    });

    test('submit change, tap on icon', async () => {
      const submitted = mockPromise();
      sinon
        .stub(queryAndAssert<GrConfirmSubmitDialog>(element, '#confirmSubmitDialog'), 'resetFocus')
    ...
    test('rebase change', async () => {
      ...
      const fetchChangesStub = sinon
        .stub(queryAndAssert<GrConfirmRebaseDialog>(element, '#confirmRebase'), 'fetchRecentChanges')
        .returns(Promise.resolve([]));
      element._hasKnownChainState = true;
      await element.updateComplete;
    ...
    test("rebase dialog gets recent changes each time it's opened", async () => {
      ...
      await element.updateComplete;
      queryAndAssert<GrConfirmRebaseDialog>(element, '#confirmRebase').dispatchEvent(
    ...
    test('two dialogs are not shown at the same time', async () => {
      ...
      tap(rebaseButton);
      await element.updateComplete;
      assert.isFalse(queryAndAssert<GrConfirmRebaseDialog>(element, '#confirmRebase').hidden);
      stubRestApi('getChanges').returns(Promise.resolve([]));
      element._handleCherrypickTap();
      await element.updateComplete;
      assert.isTrue(queryAndAssert<GrConfirmRebaseDialog>(element, '#confirmRebase').hidden);
      assert.isFalse(queryAndAssert<GrConfirmCherrypickDialog>(element, '#confirmCherrypick').hidden);
    });
    ...
    test('fullscreen-overlay-opened hides content', () => {
      ...
      assert.isTrue(spy.called);
      assert.isTrue(queryAndAssert<Element>(element, '#mainContent').classList.contains('overlayOpen'));
    });
    ...
    test('fullscreen-overlay-closed shows content', () => {
      ...
      assert.isTrue(spy.called);
      assert.isFalse(queryAndAssert<Element>(element, '#mainContent').classList.contains('overlayOpen'));
    });
    ...
    suite('change edits', () => {
      ...
      test('shows confirm dialog for delete edit', async () => {
        ...
        assert.isFalse(queryAndAssert<GrDialog>(element, '#confirmDeleteEditDialog').hidden);
        tap(
          queryAndAssert(
    ...
    suite('cherry-pick', () => {
      ...
      suite('cherry pick topics', () => {
        ...
        test('cherry pick topic dialog is rendered', async () => {
          const dialog = queryAndAssert<GrConfirmCherrypickDialog>(element, '#confirmCherrypick');
          await element.updateComplete;
    ...
    suite('move change', () => {
      ...
      test('works', () => {
        ...
        queryAndAssert<GrConfirmMoveDialog>(element, '#confirmMove').branch = 'master' as BranchName;
        element._handleMoveConfirm();
      ...
      test('branch name cleared when re-open move', () => {
        ...
        element._handleMoveTap();
        assert.equal(queryAndAssert<GrConfirmMoveDialog>(element, '#confirmMove').branch, emptyBranchName);
      });
    });
    ...
    suite('abandon change', () => {
      ...
      test('abandon change with message', async () => {
        ...
        queryAndAssert<GrConfirmAbandonDialog>(element, '#confirmAbandonDialog').message = newAbandonMsg;
        await element.updateComplete;
        ...
        assert.equal(queryAndAssert<GrConfirmAbandonDialog>(element, '#confirmAbandonDialog').message, newAbandonMsg);
      });
      ...
      test('abandon change with no message', async () => {
        ...
        assert.equal(queryAndAssert<GrConfirmAbandonDialog>(element, '#confirmAbandonDialog').message, '');
      });

      test('works', () => {
        queryAndAssert<GrConfirmAbandonDialog>(element, '#confirmAbandonDialog').message = 'original message';
        ...
        queryAndAssert<GrConfirmAbandonDialog>(element, '#confirmAbandonDialog').message = 'foo message';
    ...
    suite('revert change', () => {
      ...
      test('revert change with plugin hook', async () => {
        ...
        sinon
          .stub(queryAndAssert<GrConfirmRevertDialog>(element, '#confirmRevertDialog'), 'modifyRevertMsg')
        ...
        tap(revertButton);
        await element.updateComplete;
        assert.equal(queryAndAssert<GrConfirmRevertDialog>(element, '#confirmRevertDialog').message, newRevertMsg);
      });
      ...
      suite('revert change submitted together', () => {
        ...
        test('confirm revert dialog shows both options', async () => {
          ...
          const confirmRevertDialog = queryAndAssert<GrConfirmRevertDialog>(element, '#confirmRevertDialog');
        ...
        test('submit fails if message is not edited', async () => {
          ...
          const confirmButton = queryAndAssert(
            queryAndAssert(queryAndAssert<GrConfirmRevertDialog>(element, '#confirmRevertDialog'), 'gr-dialog'),
            '#confirm'
          );
          tap(confirmButton);
          await element.updateComplete;
    ...
    suite('mark change private', () => {
      ...
      test('private change', async () => {
        await element.updateComplete;
        assert.isOk(
          query(queryAndAssert<GrDropdown>(element, '#moreActions'), 'span[data-id="private-change"]')
        );
    ...
    suite('unmark private change', () => {
      ...
      test('unmark the private change', async () => {
        await element.updateComplete;
        assert.isOk(
          query(queryAndAssert<GrDropdown>(element, '#moreActions'), 'span[data-id="private.delete-change"]')
        );
    ...
    suite('quick approve', () => {
      ...
      test('is first in list of secondary actions', () => {
        const approveButton =
        queryAndAssert<HTMLElement>(element, '#secondaryActions').querySelector('gr-button');
  ...
  suite('getChangeRevisionActions returns only some actions', () => {
    ...
    test('confirmSubmitDialog and confirmRebase properties are changed', () => {
      ...
      element.reload();
      assert.strictEqual(queryAndAssert<GrConfirmSubmitDialog>(element, '#confirmSubmitDialog').action, null);
      assert.strictEqual(queryAndAssert<GrConfirmRebaseDialog>(element, '#confirmRebase').rebaseOnCurrent, null);
    });
