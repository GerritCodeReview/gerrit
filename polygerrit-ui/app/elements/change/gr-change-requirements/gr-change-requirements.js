  ...
  Polymer({
    ...
    _computeRequirements(change) {
      ...
      if (!!change.work_in_progress) {
        _requirements.push({type: "is_wip", status: "not_ready", data: {}});
      }
      ...
      for (const label in labels) {
        ...
        _requirements.push({type: "need_label", status: status, data: {label: label}});
      }

      // two fake requirements, for demo purposes
      _requirements.push({type: "demo", status: "not_ready", data: {status: "pending"}});
      _requirements.push({type: "unresolved_comments", status: "ok", data: {}});
      // TODO(maximeg) remove the fake requirements
