...
function renderResultChip(icon: string, results: RunResult[]) {
  if (results.length === 0) return;
  if (results.length > 1) return renderChecksChipCount(icon, results.length);
  const result = results[0];
  const chipClass = `checksChip ${icon}`;
  const grIcon = `gr-icons:${icon}`;
  return html`
    <div class="${chipClass}">
      <iron-icon icon="${grIcon}"></iron-icon>
      <div class="checkName">${result.checkName}</div>
      ${renderResultLinks(result.links ?? [])}
    </div>
  `;
}

function renderResultLinks(links: Link[]) {
  return links
...
function renderChecksChipCount(icon: string, count: number) {
  if (count === 0) return;
  const chipClass = `checksChip ${icon}`;
  const grIcon = `gr-icons:${icon}`;
  return html`
      ...
      <div>
        <table>
          <tr ?hidden=${!this.ciRebootChecksEnabled}>
            ...
            <td class="value">
              ${renderResultChip('error', errors)}
              ${renderResultChip('warning', warnings)}
              ${renderChecksChipCount('info-outline', infos.length)}
              <span ?hidden=${!hasRunningOrCompleted(runs)} class="runs"
                >Runs</span
              >
              ${renderChecksChipCount('check', completed.length)}
              ${renderChecksChipCount('timelapse', running.length)}
            </td>
          </tr>
          <tr hidden>
