...
export class GrChangeSummary extends LitElement {
  ...
  constructor() {
    ...
    subscribe(
      this,
      () => this.getCommentsModel().mentionedUsersInUnresolvedComments$,
    ...
    subscribe(
      ...
      ([selfAccount, mentionedUsersInUnresolvedComments]) => {
        if (!selfAccount) return;
        this.mentionCount = mentionedUsersInUnresolvedComments.filter(
