...
suite('gr-change-summary test', () => {
  ...
  suite('ai chips', () => {
    ...
    test('collapsed chip has isAi=true if any run is AI powered', async () => {
      element.runs = [
        createRun({
          status: RunStatus.COMPLETED,
          isAiPowered: true,
      ...
      assert.isTrue(successChip!.isAi);
      assert.equal(successChip!.text, '2');
    });

    test('plus-more chip has isAi=true if any overflow run is AI powered', async () => {
      const runs = [];
      for (let i = 0; i < 8; i++) {
        runs.push(
          createRun({
      ...
      assert.isTrue(plusMoreChip.isAi);
    });
  });
