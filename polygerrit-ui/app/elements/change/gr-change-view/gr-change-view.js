...
(function() {
  ...
  class GrChangeView extends Polymer.mixinBehaviors( [
    ...
    _findIfTabMatches(currentTab, tabs) {
      // Check if currentTab is included in tabs
      // If tabs is not an array, then that means a single tab
      return Array.isArray(tabs) ? tabs.includes(currentTab) :
        (tabs === currentTab);
    }
    ...
    _handleShowTab(e) {
      const tabs = this.shadowRoot.querySelector('#primaryTabs')
      ...
      this.shadowRoot.querySelector('#primaryTabs').selected = idx;
