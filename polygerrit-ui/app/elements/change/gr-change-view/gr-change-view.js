...
(function() {
  ...
  Polymer({
    ...
    _basePatchUpdated(patchNew, patchOld) {
      if (!patchOld) { return; }

      this._changePatchNum(this._basePatchNum, this._patchNum, true);
    },
    ...
    _computeHideEditCommitMessage(loggedIn, editing, change) {
      if (!loggedIn || editing || change.status === this._changeStatus.MERGED) {
        return true;
      }
    ...
    _maybeShowRevertDialog() {
      Gerrit.awaitPluginsLoaded()
          ...
          .then(loggedIn => {
            if (!loggedIn || this._change.status !== this._changeStatus.MERGED) {
