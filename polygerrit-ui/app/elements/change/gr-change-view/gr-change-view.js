...
class GrChangeView extends mixinBehaviors( [
  ...
  attached() {
    ...
    Gerrit.awaitPluginsLoaded()
        ...
        .then(() => this._setTabToActive(this.params));
  ...
  _setActiveTab(paperTabs, activeDetails) {
    ...
    if (this._activeTabs.includes(tabName)) return;
  ...
  _paramsChanged(value) {
    ...
    Gerrit.awaitPluginsLoaded().then(() => {
      this._setTabToActive(value);
    });
  }

  _setTabToActive(params = {}) {
    // primary
    let primaryTab = PRIMARY_TABS.FILES;
    if (params.queryMap && params.queryMap.has('primaryTab')) {
      primaryTab = params.queryMap.get('primaryTab');
    }
