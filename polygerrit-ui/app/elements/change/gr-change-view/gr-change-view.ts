...
export type EditRevisionInfo = Partial<RevisionInfo>;

export interface ChangeInfoModifiedWithEdit
  extends Omit<ChangeInfo, 'revisions'> {
...
export class GrChangeView extends KeyboardShortcutMixin(
  ...
  // TODO(TS): could be removed, not used
  @property({type: Object})
  _files?: unknown;
  ...
  @property({type: Object, computed: '_computeReplyDisabled(_serverConfig)'})
  _replyDisabled = true;
  ...
  attached() {
    ...
    this.addEventListener('reload', e => {
      ...
      this._reload(
        /* opt_isLocationChange= */ false,
        /* opt_clearPatchset= */ evt.detail && evt.detail.clearPatchset
      );
    });
  }
  ...
  _setDiffViewMode(opt_reset?: boolean) {
    if (!opt_reset && this.viewState.diffMode) {
      return;
    }
  ...
  _robotCommentCountPerPatchSet(threads: CommentThread[]) {
    return threads.reduce(
      (robotCommentCountMap: {[patchset: string]: number}, thread) => {
        const comments = thread.comments;
        const robotCommentsCount = comments.reduce(
          (acc, comment) => ((comment as UIRobot).robot_id ? acc + 1 : acc),
          0
        );
  ...
  _handleRobotCommentPatchSetChanged(e: CustomEvent<{value: string}>) {
    ...
    this._currentRobotCommentsPatchSet = patchSet as PatchSetNum;
  }
  ...
  _computeRobotCommentThreads(
    ...
    const threads = commentThreads.filter(thread => {
      ...
      return (
        comments.length &&
        (comments[0] as UIRobot).robot_id &&
  ...
  _initActiveTabs(params?: AppElementChangeViewParams) {
    let primaryTab: string = PrimaryTab.FILES as string;
    if (params && params.queryMap && params.queryMap.has('tab')) {
      primaryTab = params.queryMap.get('tab') as string;
    }
  ...
  _changeChanged(change: ChangeInfo) {
   ...
   */
  _getBasePatchNum(change: ChangeInfo, patchRange: PatchRange) {
  ...
  _computeMergedCommitInfo(
    ...
    if (!rev.commit.commit) {
      rev.commit.commit = current_revision as CommitId;
    }
    return rev.commit;
  }
  ...
  _computeReplyButtonLabel(
    ...
    if (!changeRecord || !canStartReview) {
      return 'Reply';
    }
  ...
  // TODO(TS): This code looks broken. Maybe remove.
  _handleLabelRemoved(splices: PolymerSplice<LabelInfo[]>[], path: string) {
    for (const splice of splices) {
      for (const removed of splice.removed) {
        ...
        const removedLabel = removed as QuickLabelInfo;
        if (
          labelDict.approved &&
          removedLabel.approved &&
          labelDict.approved._account_id === removedLabel.approved._account_id
        ) {
          this._reload();
          return;
        }
  ...
  _labelsChanged(
    ...
    if (changeRecord.value) {
      const value = changeRecord.value as PolymerSpliceChange<LabelInfo[]>;
      if (value.indexSplices) {
        this._handleLabelRemoved(value.indexSplices, changeRecord.path);
      }
    }
   ...
   * @param opt_section
   */
  _openReplyDialog(opt_section?: FocusTarget) {
   ...
   * case an edit exists.
   */
  _processEdit(change: ChangeInfo | ParsedChangeInfo, edit?: EditInfo | false) {
    if (!edit) {
      return;
    }
    const changeWithEdit = change as ChangeInfoModifiedWithEdit;
    if (changeWithEdit.revisions && edit?.commit.commit)
      changeWithEdit.revisions[edit.commit.commit as string] = {
  ...
  _getChangeDetail() {
    ...
    return Promise.all([detailCompletes, editCompletes, prefCompletes]).then(
      ([change, edit, prefs]) => {
        ...
        if (!change.topic) {
          change.topic = undefined;
        }
        if (!change.reviewer_updates) {
          change.reviewer_updates = undefined;
        }
        ...
        this._change = change as ChangeInfo;
        if (
          !this._patchRange ||
          !this._patchRange?.patchNum ||
          ...
          if (!this._change?.revisions) return false;
  ...
  _recomputeComments(comments: ChangeComments) {
    ...
    this._draftCommentThreads = this._commentThreads
      .filter(
        thread =>
          (thread.comments[thread.comments.length - 1] as DraftInfo).__draft
      )
      .map(thread => {
   ...
   * Reload the change.
   *
   * @param opt_isLocationChange Reloads the related changes
   * when true and ends reporting events that started on location change.
   * @param opt_clearPatchset Reloads the related changes
   * ignoring any patchset choice made.
   * @return A promise that resolves when the core data has loaded.
   * Some non-core data loading may still be in-flight when the core data
   * promise resolves.
   */
  _reload(opt_isLocationChange?: boolean, opt_clearPatchset?: boolean) {
    if (!this._change) {
      return Promise.reject(new Error('missing required change property'));
    }
  ...
  _startUpdateCheckTimer() {
    if (
      ...
      Number(this._serverConfig.change.update_delay) <= MIN_CHECK_INTERVAL_SECS
    ) {
      return;
    }
    ...
    }, Number(this._serverConfig.change.update_delay) * 1000);
  }
