...
export class GrChangeView extends KeyboardShortcutMixin(
  ...
  _reloadComments() {
    ...
    const allCommentsPromise = this.$.commentAPI.loadAll(this._changeNum);
    Promise.all([portedCommentsPromise, allCommentsPromise]).then(() => {
      this.reporting.timeEnd(PORTING_COMMENTS_CHANGE_LATENCY_LABEL);
    });
    return allCommentsPromise.then(comments => {
