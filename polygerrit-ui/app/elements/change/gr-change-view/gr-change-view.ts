...
import {css, html, LitElement, nothing, PropertyValueMap, PropertyValues} from 'lit';
...
export class GrChangeView extends LitElement {
  ...
  @property({
    type: Object,
    computed:
      '_computeCurrentRevision(change.current_revision, ' +
      'change.revisions)',
  ...
  @state()
  private diffDrafts?: {[path: string]: DraftInfo[]} = {};
  ...
  @state()
  private private updateCheckTimerHandle?: number | null;
  ...
  selectedTabPluginEndpoint?: string;

  @state()
  // The dynamic heading of the plugin added tab
  private selectedTabPluginHeader?: string;
  ...
  private firstConnectedCallback() {
    ...
    getPluginLoader()
      ...
      .then(() => this.initActiveTabs(this.params));
  ...
  override render() {
    return html`
      ...
      >
        ...
        ${this.renderPaperTabs()}
        ${this.renderPatchInfoSection()}
      </div>
      ...
      >
        ${when(this.replyOverlayOpened, () => html`
          <gr-reply-dialog
            id="replyDialog"
  ...
  private renderChangeInfoSection() {
    ...
    const resolveWeblinks = this._computeResolveWeblinks();
    const replyButtonLabel = this.computeReplyButtonLabel(
      this.diffDrafts, this.canStartReview);
    const commitCollapsible = this.computeCommitCollapsible(
      this.latestCommitMessage);
    const hideEditCommitMessage = this.computeHideEditCommitMessage(
      this.loggedIn, this.editingCommitMessage, this.change, this.editMode);
    return html`
        ...
        <div class=${this.computeHeaderClass()}>
          ...
          <div class="headerTitle">
            <div class="changeStatuses">
              ${(this.changeStatuses).map(status => html`
                <gr-change-status
                  .change=${this.change}
          ...
          <div class="commitActions">
            <gr-change-actions
              ...
              .editPatchsetLoaded=${this.patchRange ? hasEditPatchsetLoaded(this.patchRange) : false}
              .editMode=${this.editMode}
              .editBasedOnCurrentPatchSet=${hasEditBasedOnCurrentPatchSet(this.allPatchSets ?? [])}
              ...
              <div class="commitContainer">
                ...
                <div>
                  <gr-button
                    id="replyBtn"
                    class="reply"
                    title=${this.createTitle(Shortcut.OPEN_REPLY_DIALOG,
                                             ShortcutSection.ACTIONS)}
                    ?hidden=${!this.loggedIn}
                    primary=""
                    .disabled=${this.replyDisabled}
                    @click=${this.handleReplyTap}
                    >${replyButtonLabel}</gr-button
                  >
                </div>
                <div id="commitMessage" class="commitMessage">
                  <gr-editable-content
                    ...
                    .storageKey=${
                      `c${this.change._number}_rev${this.change.current_revision}`
                    }
                    .hideEditCommitMessage=${hideEditCommitMessage}
                    .commitCollapsible=${commitCollapsible}
    ...
    return html`
      ...
      >
        ...
        ${when(this.showChecksTab, () => html`
          ...
          >
        `)}
        ${this.dynamicTabHeaderEndpoints.map(tabHeader => html`
  ...
  private renderPatchInfoSection() {
    ...
    return html`
      <section class="patchInfo">
        <div
          ?hidden$=${!this.isTabActive(PrimaryTab.FILES)}
        >
          <gr-file-list-header
            id="fileListHeader"
            .account=${this.account}
            ...
            .diffPrefsDisabled=${!this.loggedIn}
        ...
        ${when(this.isTabActive(PrimaryTab.COMMENT_THREADS), () => html`
          ...
          ></gr-thread-list>
        `)}
        ${when(this.isTabActive(PrimaryTab.CHECKS), () => html`
          ...
          ></gr-checks-tab>
        `)}
        ${when(this.isTabActive(PrimaryTab.FINDINGS), () => html`
          <gr-dropdown-list
          ...
          ${when(this.showRobotCommentsButton, () => html`
            <gr-button
        ...
        ${when(this.isTabActive(this.selectedTabPluginHeader), () => html`
      ...
      <paper-tabs id="secondaryTabs">
        <paper-tab
          data-name=${SecondaryTab.CHANGE_LOG}
          class="changeLog"
        >
          Change Log
        </paper-tab>
  ...
  handleCommitMessageSave(e: EditableContentSaveEvent) {
    ...
    if (!this.commitMessageEditor ||
        this.commitMessageEditor.disabled) return;
  ...
  private computeChangeStatusChips( ) {
    ...
    return changeStatuses(this.change, options);
  }
  ...
  // Private but used in tests.
  computeText(patch: RevisionInfo, commentThreads: CommentThread[]) {
  ...
  private computeRobotCommentsPatchSetDropdownItems() {
    if (!this.change || !this.commentThreads || !this.change.revisions) return [];
  ...
  _handleCurrentRevisionUpdate(currentRevision?: RevisionInfo) {
    this.currentRobotCommentsPatchSet = currentRevision?._number;
  }
  ...
  private computeRobotCommentThreads() {
  ...
  rovate setShownFiles(e: CustomEvent<{length: number}>) {
    this.shownFileCount = e.detail.length;
  }
  ...
  _paramsChanged(value: AppElementChangeViewParams) {
    ...
    if (value.patchNum === undefined) {
      value.patchNum = computeLatestPatchNum(this.allPatchSets);
    }
    ...
    getPluginLoader()
      .awaitPluginsLoaded()
      .then(() => {
        this.initActiveTabs(value);
      });
  }

  private initActiveTabs(params?: AppElementChangeViewParams) {
  ...
  sendShowChangeEvent() {
    if (!this.patchRange)
      throw new Error('missing required patchRange property');
  ...
  changeChanged() {
    ...
    this.patchRange = {
      ...this.patchRange,
      basePatchNum: parent,
      patchNum: this.patchRange.patchNum || computeLatestPatchNum(this.allPatchSets)
    }
    this.updateTitle(this.change);
  }
  ...
  private computeChangeUrl(forceReload?: string) {
  ...
  getProjectConfig() {
    ...
    return this.restApiService
      .getProjectConfig(this.change.project)
      .then(config => {
  ...
  processEdit(change: ParsedChangeInfo) {
    ...
    if (!this.routerPatchNum && latestPsNum === editParentRev._number) {
      this.set('patchRange.patchNum', EDIT);
  ...
  computeRevertSubmitted(change?: ChangeInfo | ParsedChangeInfo) {
    ...
    Promise.all(
      ...
      if (submittedRevert) {
        ...
        this.changeStatuses = this.changeStatuses.concat([ChangeStates.REVERT_SUBMITTED])
      } else {
        if (changes[0]) this.revertedChange = changes[0];
        this.changeStatuses = this.changeStatuses.concat([ChangeStates.REVERT_CREATED])
      }
    });
  }
  ...
  async performPostChangeLoadTasks() {
    ...
    const prefCompletes = this.getPreferences();
    ...
    const latestRevisionSha = this._getLatestRevisionSHA(this.change);
    ...
    coreDataPromise.then(() => {
      ...
      if (isLocationChange) {
        this.reporting.changeDisplayed(
          roleDetails(this.change, this.account)
        );
      }
    });
  ...
  private computeCommitCollapsible(commitMessage?: string) {
    if (!commitMessage) {
      return false;
    }
  ...
  @observe('patchRange.patchNum')
  _patchNumChanged(patchNumStr?: PatchSetNum) {
    if (!this.selectedRevision || !patchNumStr) {
      return;
    }
    assertIsDefined(this.change, 'change');
    ...
    if (patchNum === this.selectedRevision._number) {
      return;
    }
