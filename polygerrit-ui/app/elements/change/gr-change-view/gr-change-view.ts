...
interface TemporaryFileList {
  openDiffPrefs: () => void;
  reload: () => void;
  collapseAllDiffs: () => void;
  expandAllDiffs: () => void;
  reloadCommentsForThreadWithRootId: (
...
export interface GrChangeView {
  $: {
    ...
    fileList: TemporaryFileList & Element;
...
export class GrChangeView extends KeyboardShortcutMixin(
  ...
  attached() {
    ...
    this.addEventListener('show-primary-tab', e =>
      this._setActivePrimaryTab(
        e as CustomEvent<{tab: string; value: number; scrollIntoView: boolean}>
      )
    );
  ...
  _setActiveTab(
    ...
    activeDetails: {
      activeTabName: string | SecondaryTab | PrimaryTab;
  ...
  _handleLabelRemoved(splices: LabelInfo, path: string) {
  ...
  _handleGetChangeDetailError(response: Response) {
    this.dispatchEvent(
  ...
  _recomputeComments(comments: ChangeComments) {
    ...
    this._draftCommentThreads = this._commentThreads
      .filter(thread => thread.comments[thread.comments.length - 1] as DraftInfo).__draft)
      .map(thread => {
