...
export class GrChangeView extends LitElement {
  ...
  private setupSubscriptions() {
    ...
    subscribe(
      ...
      openReplyDialog => {
        if (openReplyDialog) this.handleOpenReplyDialog();
      }
    );
    subscribe(
      this,
    ...
    subscribe(
      ...
      revision => {
        console.log(`asdf revision sub ${JSON.stringify(revision)}`);
        console.log(
          `asdf revision sub commmit ${JSON.stringify(
            revision?.commit?.commit
          )} ${JSON.stringify(revision?.commit)}`
        );
        this.revision = revision;
        this.commitInfo = revision?.commit;
      }
    );
    subscribe(
      this,
