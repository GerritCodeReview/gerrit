      ...
      <gr-button
        ...
        @click=${(e: MouseEvent) => {
          if (this.changeStar && this.changeNumberLink) {
            const PADDING = 4; // or 8, as you prefer
            const starRect = this.changeStar.getBoundingClientRect();
            const linkRect = this.changeNumberLink.getBoundingClientRect();
            const x = e.clientX;
            const y = e.clientY;

            const inStar =
              x >= starRect.left - PADDING &&
              x <= starRect.right + PADDING &&
              y >= starRect.top - PADDING &&
              y <= starRect.bottom + PADDING;
