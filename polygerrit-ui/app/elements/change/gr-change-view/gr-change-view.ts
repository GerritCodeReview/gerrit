...
export class GrChangeView extends LitElement {
  ...
  protected override willUpdate(changedProperties: PropertyValues): void {
    ...
    if (changedProperties.has('patchRange')) {
  ...
  // Private but used in tests.
  paramsChanged(value: AppElementChangeViewParams) {
  ...
  // Private but used in tests.
  changeChanged(_oldChange: ParsedChangeInfo | undefined ) {
    this.allPatchSets = computeAllPatchSets(this.change);
    if (!this.change) return
    if (this.change.current_revision && this.change.revisions &&
