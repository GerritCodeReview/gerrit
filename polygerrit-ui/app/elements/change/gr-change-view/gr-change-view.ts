...
export class GrChangeView extends KeyboardShortcutMixin(
  ...
  @property({type: Boolean, computed: '_computeReplyDisabled(_serverConfig)'})
  _replyDisabled = true;
  ...
  _handleLabelRemoved(splices: PolymerSplice<ApprovalInfo[]>[], path: string) {
  ...
  _labelsChanged(
    ...
    if (changeRecord.value) {
      const value = changeRecord.value as PolymerSpliceChange<ApprovalInfo[]>;
  ...
  _reload(opt_isLocationChange?: boolean, opt_clearPatchset?: boolean) {
    if (opt_clearPatchset && this._change) {
