...
export class GrChangeView extends LitElement {
  ...
  @state() prevPatchNum?: RevisionPatchSetNum = undefined;

  @state() prevChangeLabel?: LabelNameToInfoMap;
          ...
          <div class="changeInfo">
            <div class="changeInfo-column changeMetadata hideOnMobileOverlay">
              <gr-change-metadata
                ...
                .repoConfig=${this._projectConfig}
  ...
  override willUpdate(changedProperties: PropertyValues) {
    ...
    if (changedProperties.has('params') || changedProperties.has('change')) {
      changedProperties.set('_numFilesShown', this._numFilesShown);
    ...
    if (this._patchRange?.patchNum !== this.prevPatchNum) {
      this.prevPatchNum = this._patchRange?.patchNum;
    ...
    if (this._change?.labels !== this.prevChangeLabel) {
      this.prevChangeLabel = this._change?.labels;
      this._labelsChanged();
    }
  }
  ...
  paramsChanged() {
    ...
    setTimeout(() => {
      if (this.fileList) {
        ...
        this.reporting.error(new Error("fileList wasn't resolved"));
      }
    }, 1);
  ...
  _reloadPatchNumDependentResources(patchNumChanged?: boolean) {
    ...
    setTimeout(() => {
      if (this.fileList) {
        promises.push(this.fileList.reload());
      } else {
        this.reporting.error(new Error("fileList wasn't resolved"));
      }
    }, 1);
