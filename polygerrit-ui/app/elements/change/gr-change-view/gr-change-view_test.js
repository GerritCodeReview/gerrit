...
suite('gr-change-view tests', () => {
  ...
  suite('keyboard shortcuts', () => {
    ...
    test('shift + R should fetch and navigate to the latest patch set',
          ...
          navigateToChangeStub = sinon.stub(GerritNav, 'navigateToChange').callsFake(
  ...
  test('topic is coalesced to null', done => {
    ...
    sinon.stub(element.$.restAPI, 'getChangeDetail').callsFake( () => Promise.resolve({
      id: '123456789',
      labels: {},
  ...
  test('revert dialog opened with revert param', done => {
    sinon.stub(element.$.restAPI, 'getLoggedIn').callsFake( () => Promise.resolve(true));
    sinon.stub(pluginLoader, 'awaitPluginsLoaded').callsFake(() => Promise.resolve());
  ...
  suite('related changes expand/collapse', () => {
    ...
    test('relatedChangesToggle shown height greater than changeInfo height',
        () => {
          ...
          sinon.stub(window, 'matchMedia').callsFake(() => { return {matches: true}; });
    ...
    test('relatedChangesToggle functions', () => {
      sinon.stub(element, '_getOffsetHeight').callsFake(() => 50);
      sinon.stub(window, 'matchMedia').callsFake(() => { return {matches: false}; });
