...
suite('gr-change-view tests', () => {
  ...
  suite('plugins adding to file tab', () => {
    ...
    test('plugin added tab shows up as a dynamic endpoint', () => {
      assert(
        element.dynamicTabHeaderEndpoints.includes(
  ...
  suite('Findings robot-comment tab', () => {
    ...
    test('changing patchsets resets robot comments', async () => {
      element.set('change.current_revision', 'rev3');
      await element.updateComplete;
  ...
  suite('ChangeStatus revert', () => {
    test('do not show any chip if no revert created', async () => {
      ...
      assert.isTrue(element.submitEnabled);
      await element.updateComplete;
  ...
  test('getBasePatchNum', () => {
    ...
    assert.equal(element.getBasePatchNum(change, patchRange), PARENT);
    ...
    assert.equal(element._getBasePatchNum(_change2, patchRange), -1);

    patchRange.patchNum = 1 as RevisionPatchSetNum;
    assert.equal(element._getBasePatchNum(_change2, patchRange), PARENT);
  });
  ...
  test('reply button is disabled until server config is loaded', async () => {
    assert.isTrue(element.replyDisabled);
    // fetches the server config on attached
    await element.updateComplete;
    assert.isFalse(element.replyDisabled);
  });
  ...
  test('selectedRevision updates when patchNum is changed', () => {
  ...
  suite('handleEditTap', () => {
    ...
    test('edit exists in revisions', async () => {
      ...
      element.change.revisions.rev2._number = EDIT;
    ...
    test('no edit exists in revisions, non-latest patchset', async () => {
      ...
      assertIsDefined(element.change);
      element.change.revisions.rev2._number = 2 as RevisionPatchSetNum;
      element.requestUpdate('change');
    ...
    test('no edit exists in revisions, latest patchset', async () => {
      ...
      element.set('change.revisions.rev2', {_number: 2});
      element.patchRange = {patchNum: 2 as RevisionPatchSetNum};
      await element.updateComplete;
  ...
  suite('gr-reporting tests', () => {
    setup(() => {
      ...
      sinon.stub(element, 'getProjectConfig').returns(Promise.resolve());
