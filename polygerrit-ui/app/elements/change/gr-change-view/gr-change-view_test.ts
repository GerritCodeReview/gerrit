...
import {changeModelToken, LoadingStatus} from '../../../models/change/change-model';
...
suite('gr-change-view tests', () => {
  ...
  suite('keyboard shortcuts', () => {
    ...
    test('m should toggle diff mode', async () => {
      ...
      testResolver(userModelToken).setPreferences(prefs);
      ...
      testResolver(userModelToken).setPreferences(newPrefs);
      await element.updateComplete;
      element.handleToggleDiffMode();
      assert.isTrue(
  ...
  test('related changes are updated when loadData is called', async () => {
    ...
    element.viewState = createChangeViewState();
    testResolver(changeModelToken).setState({
  ...
  test('selectedRevision updates when patchNum is changed', async () => {
    ...
    testResolver(userModelToken).setPreferences(createPreferences());
