...
export class GrConfirmCherrypickDialog extends GestureEventListeners(
  ...
  _query: (input: string) => Promise<Suggestion[]>;
  ...
  _statuses: {[changeId: string]: Status};
  ...
  _computeDisableCherryPick(
    cherryPickType: CHERRY_PICK_TYPES,
  ...
  _computeIfSinglecherryPick(cherryPickType: CHERRY_PICK_TYPES) {
    return cherryPickType === CHERRY_PICK_TYPES.SINGLE_CHANGE;
  }

  _computeIfCherryPickTopic(cherryPickType: CHERRY_PICK_TYPES) {
  ...
  _getProjectBranchesSuggestions(input: string): Promise<Suggestion[]> {
    if (!this.project) {
