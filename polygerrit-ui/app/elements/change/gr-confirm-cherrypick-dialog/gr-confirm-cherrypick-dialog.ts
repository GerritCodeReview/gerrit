  ...
  implements ChangeActionDialog
{
  ...
  @state()
  private bulkType: CherryPickType.TOPIC | CherryPickType.RELATION_CHAIN = CherryPickType.TOPIC;
        ...
        <div class="main" slot="main">
          <gr-endpoint-decorator name="cherrypick-main">
            ...
            ${choose(this.cherryPickType, [
              ...
              [CherryPickType.RELATION_CHAIN, () => this.renderCherrypickTable()],
            ])}
  ...
  private renderCherrypickTopicLayout() {
    const labelText = this.bulkType === CherryPickType.RELATION_CHAIN
  ...
  updateChanges(changes: (ParsedChangeInfo | ChangeInfo)[], type?: CherryPickType) {
    ...
    if (type === CherryPickType.RELATION_CHAIN || type === CherryPickType.TOPIC) {
      this.bulkType = type;
  ...
  private computeTopicErrorMessage() {
    if (this.cherryPickType === CherryPickType.TOPIC && this.duplicateProjectChanges) {
  ...
  private async handleCherryPickChain() {
    ...
    const topic = this.generateRandomCherryPickTopic(changes[0]);
    let currentBase: CommitId | null = null;
