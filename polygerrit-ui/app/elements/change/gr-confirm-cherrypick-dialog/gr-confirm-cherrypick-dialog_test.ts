...
import {CherryPickInput} from '../../../types/common';
...
suite('gr-confirm-cherrypick-dialog tests', () => {
  ...
  suite('cherry pick relation chain', () => {
    ...
    test('cherry pick relation chain submit sequential', async () => {
      ...
      assert.equal((args1[4] as CherryPickInput).destination, 'master');
      assert.isUndefined((args1[4] as CherryPickInput).base);

      // Second call
      const args2 = executeChangeActionStub.args[1];
      assert.equal(args2[0], 2);
      assert.equal((args2[4] as CherryPickInput).destination, 'master');
      assert.equal((args2[4] as CherryPickInput).base, 'new-commit-1');
    });
    ...
    test('cherry pick relation chain stops on failure', async () => {
      ...
      const executeChangeActionStub = stubRestApi('executeChangeAction')
        .onCall(0)
        .resolves(new Response(null, {status: 400}));
