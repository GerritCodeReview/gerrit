...
export class GrFileListHeader extends KeyboardShortcutMixin(
  ...
  _computeExpandedClass(filesExpanded: string) {
    const classes = [];
  ...
  _getPatchsetHash(
    ...
    for (const rev in revisions) {
      if (hasOwnProperty(revisions, rev) && revisions[rev] === patchSet) {
        return rev;
      }
    }
  ...
  _updateDescription(desc: string, e: CustomEvent) {
    ...
    const target = (dom(e) as EventApi).rootTarget as any;
    if (target) {
      target.disabled = true;
    }
    const rev = getRevisionByPatchNum(
      sortRevisions(Object.values(this.change.revisions)),
      this.patchNum
    )!;
