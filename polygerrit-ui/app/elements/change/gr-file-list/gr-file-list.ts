...
export class GrFileList extends LitElement {
  ...
  override willUpdate(changedProperties: PropertyValues): void {
    ...
    if (changedProperties.has('files')) {
      ...
      const newExtensionsArr = Array.from(newExtensions).sort();
      if (
        this.fileExtensions.length !== newExtensionsArr.length ||
        !this.fileExtensions.every((val, index) => val === newExtensionsArr[index])
      ) {
  ...
  private shouldShowFile(status?: FileInfoStatus, path?: string) {
    ...
    if (
      this.visibleFileExtensions &&
      this.visibleFileExtensions.length > 0
    ) {
