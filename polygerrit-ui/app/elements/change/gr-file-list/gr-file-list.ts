...
export class GrFileList extends LitElement {
  ...
  private renderReviewed(file: NormalizedFileInfo) {
    ...
    return html` <div class="reviewed hideOnEdit" role="gridcell">
      ...
      <span
        ...
        role="switch"
        tabindex="0"
        @click=${this.reviewedClick}
        @keydown=${this.reviewedClick}
        aria-label="Reviewed"
    ...
    return html` <div class="show-hide" role="gridcell">
      ...
      >
        ...
        <iron-icon
          ...
          tabindex="-1"
          id="icon"
          icon="[[_computeShowHideIcon(file.__path, expandedFiles.*)]]"
        >
        </iron-icon>
      </span>
    </div>`;
  ...
  private calculatePatchChange(): PatchChange {
  ...
  toggleFileExpanded(file: PatchSetFile) {
    ...
    if (indexInExpanded === -1) {
      ...
      this.expandedFilesChanged([file]);
    } else {
  ...
  expandAllDiffs() {
    ...
    this.expandedFilesChanged(newFiles);
  }

  collapseAllDiffs() {
    this.expandedFiles = [];
    this.filesExpanded = this.computeExpandedFiles();
    // TODO: Is it possible to replace this line with
    // this.expandedFilesChanged([])?
  ...
  private handleCursorNext(e: KeyboardEvent) {
  ...
  private handleCursorPrev(e: KeyboardEvent) {
    if (this.filesExpanded === FilesExpandedState.ALL) {
  ...
  _computeShowHideIcon(
    ...
    expandedFilesRecord: ElementPropertyDeepChange<GrFileList, 'expandedFiles'>
  ) {
  ...
  @observe('filesByPath', 'changeComments', 'patchRange', 'reviewed', 'loading')
  _computeFiles(
    filesByPath?: FileNameToFileInfoMap,
    changeComments?: ChangeComments,
    patchRange?: PatchRange,
   ...
   */
  async expandedFilesChanged(newFiles: Array<PatchSetFile>) {
    ...
    if (newFiles.length) {
      this.renderInOrder(newFiles, this.diffs, newFiles.length);
    }
