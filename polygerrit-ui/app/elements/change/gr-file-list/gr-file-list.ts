...
export class GrFileList extends KeyboardShortcutMixin(PolymerElement) {
  ...
  reload() {
    if (!this.changeNum || !this.patchRange?.patchNum || !this.change) {
    ...
    // When viewing Auto Merge base vs a patchset, add an additional magic path
    // that knows how many files were cleanly merged. This requires an
    // additional RPC for the diffs between target parent and the patch set.
    // The cleanly merged files are all the files in the second RPC that weren't
    // in the first RPC.
    if (new RevisionInfo(this.change).isMergeCommit(this.patchRange.patchNum)) {
      ...
      promises.push(getFilesByPath.then(filesByPath => {
        this._filesByPath = filesByPath;
      }));
    }
  ...
  _computeCleanlyMergedText(cleanlyMergedPaths: string[]): string {
    return `${cleanlyMergedPaths.length} files merged cleanly in Parent 1`;
  }
  ...
  _getMutiPath(file: NormalizedFileInfo) {
    return file || null;
  }
}
