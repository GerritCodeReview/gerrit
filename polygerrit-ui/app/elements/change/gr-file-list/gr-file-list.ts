...
export class GrFileList extends LitElement {
  ...
  openSelectedFile(index?: number) {
    ...
    this.getNavigation().setUrl(
      this.getViewModel().diffUrl({
        diffView: {path: this.files[idx].__path},
        patchNum: this.patchNum,
        basePatchNum: this.getChangeModel().urlBasePatchNum(
          this.basePatchNum,
          this.patchNum
  ...
  private computeNumFilteredOut(): number {
    ...
    return this.files.filter(
      f => !this.shouldShowFile(f.__path)
    ).length;
  }
  ...
  private computeShowAllWarning() {
    ...
    return `Warning: showing all ${this.computeFilteredFiles().length} files may take several seconds.`;
  }
