...
suite('gr-file-list tests', () => {
  ...
  suite('basic tests', () => {
    ...
    test('rendering each row calls the reportRenderedRow method', () => {
    ...
    test('calculate totals for patch number', () => {
      ...
      assert.isTrue(element.shouldHideBinaryChangeTotals());
      assert.isFalse(element.shouldHideChangeTotals());
    ...
    test('binary only files', () => {
      ...
      assert.isFalse(element.shouldHideBinaryChangeTotals());
      assert.isTrue(element.shouldHideChangeTotals());
    });
    ...
    test('comment filtering', () => {
      ...
      element.patchRange = _1To2;
      assert.equal(
        element.computeDraftsString(element.changeComments, _1To2, {
          __path: 'file_added_in_rev2.txt',
          size: 0,
          size_delta: 0,
        }),
      ...
      element.patchRange = parentTo2;
      assert.equal(
        element.computeCommentsStringMobile(element.changeComments, parentTo2, {
          __path: 'myfile.txt',
          size: 0,
          size_delta: 0,
        }),
    ...
    test('handleFileListClick editMode', () => {
      element.filesByPath = {
        '/COMMIT_MSG': {size: 0, size_delta: 0},
        'f1.txt': {size: 0, size_delta: 0},
        'f2.txt': {size: 0, size_delta: 0},
    ...
    test('_expandedFilesChanged', async () => {
      ...
      element.push('expandedFiles', {path});
      await promise;
    });
    ...
    test('filesExpanded value updates to correct enum', () => {
      ...
      flush();
      assert.equal(element.filesExpanded, FilesExpandedState.NONE);
      element.push('expandedFiles', {path: 'baz.bar'});
      flush();
      assert.equal(element.filesExpanded, FilesExpandedState.SOME);
      element.push('expandedFiles', {path: 'foo.bar'});
      flush();
      assert.equal(element.filesExpanded, FilesExpandedState.ALL);
      element.collapseAllDiffs();
      flush();
    ...
    test('renderInOrder logged in', async () => {
      ...
      element.renderInOrder([{path: 'p2'}], diffs, 1);
      await element.updateComplete;
  ...
  suite('size bars', () => {
    ...
    test('_computeSizeBarsClass', () => {
      ...
      assert.equal(
        element._computeSizeBarsClass('foo/bar.baz'),
        'sizeBars hide'
      );
      element.showSizeBars = true;
      assert.equal(
        element._computeSizeBarsClass('/COMMIT_MSG'),
        'sizeBars invisible'
      );
      assert.equal(element._computeSizeBarsClass('foo/bar.baz'), 'sizeBars ');
    });
  });
