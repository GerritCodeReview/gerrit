...
suite('gr-file-list tests', () => {
  ...
  suite('basic tests', () => {
    ...
    test('checkbox shows/hides diff inline', async () => {
      ...
      assert.include(element._expandedFiles.map(f => f.path), 'myfile.txt');
    });
    ...
    test('_toggleFileExpanded', async () => {
      ...
      assert.include(element._expandedFiles.map(f => f.path), path);
      element._toggleFileExpanded({path});
      await flush();
      ...
      assert.equal(renderSpy.callCount, 1);
      assert.notInclude(element._expandedFiles.map(f => f.path), path);
