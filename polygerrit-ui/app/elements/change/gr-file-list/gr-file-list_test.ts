...
import {createDefaultDiffPrefs, DiffViewMode} from '../../../constants/constants';
  ...
  suite('basic tests', async () => {
    ...
    test('renders file row', async () => {
      ...
      >
        ...
        <div
          aria-hidden="true"
          class="editFileControls showOnEdit" role="gridcell"
        >
        </div>
    ...
    test('file review status', async () => {
      ...
      let fileRows = queryAll(element, '.row:not(.header-row)');
      ...
      let commitReviewLabel = fileRows[0].querySelector('.reviewedLabel');
      let markReviewLabel = fileRows[0].querySelector('.markReviewed');
      ...
      markReviewLabel = fileRows[0].querySelector('.markReviewed');
      ...
      fileRows = queryAll(element, '.row:not(.header-row)');
      commitReviewLabel = fileRows[0].querySelector('.reviewedLabel');
      markReviewLabel = fileRows[0].querySelector('.markReviewed');
    ...
    test('expandedFilesChanged', async () => {
      ...
      element.expandedFiles.push({path});
      element.expandedFilesChanged([{path}]);
      await promise;
    });
