...
suite('gr-create-flow tests', () => {
  ...
  const setupTest = async (actions: FlowActionInfo[]) => {
    const restApi = getAppContext().restApiService;
    sinon.stub(restApi, 'listFlowActions').resolves(actions);
  ...
  suite('add-reviewer action', () => {
    ...
    test('renders md-outlined-text-field for non-add-reviewer action', async () => {
      ...
      assert.isNull(query(element, '.autocomplete-input'));
    });
    ...
    test('renders gr-autocomplete for add-reviewer action', async () => {
      ...
      assert.isNull(query(element, '.textfield-input'));
    });
