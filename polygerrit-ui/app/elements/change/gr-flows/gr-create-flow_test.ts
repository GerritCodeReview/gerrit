...
suite('gr-create-flow tests', () => {
  ...
  suite('parameter input field', () => {
    ...
    test('gr-autocomplete text change updates currentParameter', async () => {
      const actionInput = queryAndAssert<MdOutlinedSelect>(
        element,
        'md-outlined-select[label="Action"]'
      );
      ...
      autocomplete.dispatchEvent(
        new CustomEvent('text-changed', {detail: {value: 'test@'}})
      );
      await element.updateComplete;

      assert.equal(element['currentParameter'], 'test@');
    });
  ...
  suite('reviewer suggestions', () => {
    ...
    test('fetches and formats suggestions', async () => {
      const suggestions = await element['reviewerSuggestions']('', 'test');
    ...
    test('queries with the last email in a comma-separated list', async () => {
      await element['reviewerSuggestions']('', 'abc@example.com,test');
      assert.isTrue(queryAccountsStub.calledWith('test'));
    });

    test('formats the value correctly for comma-separated lists', async () => {
      const suggestions = await element['reviewerSuggestions'](
        '',
        'abc@example.com, test'
      );
      assert.equal(suggestions[0].value, 'abc@example.com,test1@example.com');
      assert.equal(suggestions[1].value, 'abc@example.com,test2@example.com');
    });
  });
