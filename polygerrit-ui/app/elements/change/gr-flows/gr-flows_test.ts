...
suite('gr-flows tests', () => {
  ...
  test('renders flows', async () => {
    ...
    assert.shadowDom.equal(
      ...
      `,
      {ignoreAttributes: ['style', 'class', 'account', 'changenum', 'datestr', 'aria-disabled', 'role', 'tabindex']}
    );
  });
    ...
    const flows: FlowInfo[] = [
      {
        ...
        owner: {name: 'owner1'},
        created: '2025-01-01T10:00:00.000Z' as Timestamp,
        stages: [ { expression: {condition: 'label:Code-Review=+1'}, state: FlowStageState.DONE, }, ],
      },
    ];
    stubRestApi('listFlows').returns(Promise.resolve(flows));
    const deleteFlowStub = stubRestApi('deleteFlow').returns(Promise.resolve(new Response()));
    await element['loadFlows']();
    await element.updateComplete;
