...
(function() {
  ...
  Polymer({
    ...
    properties: {
      ...
      _hashtags: Array,
      /** @type {?Object} */
      _hashtagPendingConfirmation: {
        type: Object,
        observer: '_hashtagPendingConfirmationUpdated',
      },
    ...
    observers: [
      '_changeUpdated(change.hashtags.*)',
      '_hashtagsChanged(_hashtags.splices)',
    ],
    ...
    getFocusStops() {
      return {
        start: this.$.hashtags.focusStart,
    ...
    _processHashtagChange(indexSplices, type) {
      for (const splice of indexSplices) {
        for (const account of splice.removed) {
          if (!this._hashtagsPendingRemove[type]) {
            console.err('Invalid type ' + type + ' for reviewer.');
            return;
          }
          this._hashtagsPendingRemove[type].push(account);
        }
      }
    },
     ...
     */
    _purgeHashtagsPendingRemove(isCancel, opt_accountIdsTransferred) {
      let reviewerArr;
      const keep = opt_accountIdsTransferred || {};
      for (const type in this._hashtagsPendingRemove) {
        if (this._hashtagsPendingRemove.hasOwnProperty(type)) {
          if (!isCancel) {
            reviewerArr = this._hashtagsPendingRemove[type];
          ...
          this._hashtagsPendingRemove[type] = [];
        }
      }
    },
    ...
    _changeUpdated(changeRecord) {
      this._rebuildHashtagArrays(changeRecord.base);
    },

    _rebuildHashtagArrays(change) {
      let hashtags = [];
    ...
    cancel() {
      ...
      this._purgeHashtagsPendingRemove(true);
      this._rebuildHashtagArrays(this.change.hashtags);
    },
    ...
    _hashtagPendingConfirmationUpdated(hashtag) {
      if (hashtag === null) {
        this.$.hashtagConfirmationOverlay.close();
      } else {
        this._pendingConfirmationDetails = this._hashtagPendingConfirmation;
        this.$.hashtagConfirmationOverlay.open();
      }
    },

    _confirmPendingHashtag() {
      this.$.hashtags.confirmGroup(this._hashtagPendingConfirmation.group);
      this._focusOn('hashatag');
    },

    _cancelPendingHashatag() {
      this._hashtagPendingConfirmation = null;

      this._focusOn('hashtag');
    },
