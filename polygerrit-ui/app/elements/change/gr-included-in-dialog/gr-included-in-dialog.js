...
(function() {
  ...
  Polymer({
    ...
    _loadIncludedIn() {
      ...
      return this.$.restAPI.getChangeIncludedIn(this.changeNum).then(
          configs => {
            ...
            if (configs && configs.external) {
              this._includedInExternal = Object.keys(configs.external)
               .map(key => {
                 const config = configs.external[key];
                 config.name = key;
                 return config;
               });
            }
    ...
    _formatBranchesAndTags(includedIn) {
    ...
    _formatExternalListData(includedIn) {
      if (!includedIn) { return; }
