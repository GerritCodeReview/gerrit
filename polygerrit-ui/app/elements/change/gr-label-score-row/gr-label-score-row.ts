...
export class GrLabelScoreRow extends LitElement {
  ...
  private renderBlankItems(position: string) {
    ...
    return new Array(blankItemCount).fill('').map(
      () => html`
  ...
  computeBlankItemsCount(side: string) {
    if (
      ...
      !this.labelValues?.length
    ) {
      return 0;
    }
    const orderedLabelValues = this.labelValues;
    ...
    return orderedLabelValues[orderedLabelValues.length - 1] -
