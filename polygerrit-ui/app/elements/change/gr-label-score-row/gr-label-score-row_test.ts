...
suite('gr-label-row-score tests', () => {
  ...
  test('label display values from plugins', async () => {
    ...
    const customLabelValues = new Map<string, Map<string, string>>([
      [
        'Commit-Queue', new Map<string, string>([
          ['+1', 'DRY-RUN'],
          ['+2', 'CQ'],
        ])
      ],
    ])
    const getReviewLabelValuesStub = sinon
        .stub(element.jsAPI, 'getReviewLabelValues').returns(customLabelValues);
    element.render();
