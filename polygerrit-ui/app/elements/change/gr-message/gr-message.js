...
class GrMessage extends GestureEventListeners(
  ...
  _patchsetCommentSummary(commentThreads) {
    const idx = this.message.id;
    if (!idx) return;
    let summary = '';
    commentThreads.forEach(thread => {
      // figure out if this was created in this message(?)
      // if user posts a draft reply then length === 2
      if (thread.path == SpecialFilePath.PATCHSET_LEVEL_COMMENTS) {
        thread.comments.forEach(comment => {
          if (comment.change_message_id === idx) { summary = comment.message; }
        });
      }
    });
    return summary;
