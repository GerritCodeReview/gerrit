...
export class GrMessage extends LitElement {
  ...
  override willUpdate(changedProperties: PropertyValues) {
    if (
      ...
      changedProperties.has('hideAutomated')
    ) {
      const isAutomated = !!this.message && this.computeIsAutomated();
      this.hidden = this.hideAutomated && isAutomated;
    }
  ...
  override render() {
    ...
    return html` <div class=${this.computeClass()}>
      <div class="contentContainer">
        ${this.renderAuthor()} ${this.maybeRenderCommentsSummary()}
        ${this.maybeRenderMessageContent()} ${this.maybeRenderReviewerUpdate()}
  ...
  private renderAuthor() {
    ...
    return html` <div class="author" @click=${this.handleAuthorClick}>
      <span ?hidden=${!this.computeShowOnBehalfOf()}>
  ...
  private maybeRenderCommentsSummary() {
  ...
  private maybeRenderMessageContent() {
    ...
    return html` <div class="content messageContent">
      ...
      ${this.maybeRenderExpandedMessageContent()}
    </div>`;
  }

  private maybeRenderExpandedMessageContent() {
  ...
  private renderActionContainer() {
    return html` <div
      ...
      ?hidden=${!this.computeShowReplyButton()}
    >
      <gr-button
        class="replyBtn"
        link=""
  ...
  private maybeRenderReviewerUpdate() {
    assertIsDefined(this.message, 'message');
  ...
  private showViewDiffButton() {
    return (
      this._isNewPatchsetTag(this.message?.tag) ||
      this._isMergePatchset(this.message)
    );
  }
