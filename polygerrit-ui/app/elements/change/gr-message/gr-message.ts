...
export class GrMessage extends LitElement {
  ...
  private getCommitUrl(sha: string): string | undefined {
    // Try to build a URL from the current revision's weblinks by replacing
    // the revision SHA in an existing weblink URL.
    if (this.change?.revisions) {
      for (const rev of Object.values(this.change.revisions)) {
        ...
        if (weblink?.url) {
          // Replace the revision's commit SHA in the URL with our target SHA
