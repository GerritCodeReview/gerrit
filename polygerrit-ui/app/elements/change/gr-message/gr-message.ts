...
export class GrMessage extends LitElement {
  ...
  computeMessageContent(
    ...
    if (isExpanded && tag === 'autogenerated:gerrit:merged') {
      const pattern = /(Change has been successfully (?:rebased and submitted|cherry-picked) as )([0-9a-f]{40})/g;
      return mappedLines.map(line => line.replace(pattern, '$1[$2](/q/$2)')).join('\n').trim();
    }
