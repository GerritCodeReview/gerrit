...
function updateTag(message) {
  if (!message.tag) {
    ...
    if (isRobot) {
      message.tag = 'autogenerated:has-robot-comments';
    }
  } else {
    ...
    if (message.tag === MessageTag.TAG_UNSET_ASSIGNEE) {
      message.tag = MessageTag.TAG_SET_ASSIGNEE;
    }
    if (message.tag === MessageTag.TAG_UNSET_PRIVATE) {
      message.tag = MessageTag.TAG_SET_PRIVATE;
    }
    if (message.tag === MessageTag.TAG_SET_WIP) {
      message.tag = MessageTag.TAG_SET_READY;
    }

    message.tag = message.tag.replace(/~.*/, '');
  }
}
 ...
 * number already set.
 */
function setRevisionNumber(message, allMessages) {
  if (message._revision_number > 0) return;
  let revision = 0;
  for (const m of allMessages) {
    if (m.date > message.date) break;
    if (m._revision_number > 0) revision = m._revision_number;
  }
  if (revision > 0) message._revision_number = revision;
}
...
class GrMessagesListExperimental extends mixinBehaviors( [
  ...
  _computeCombinedMessages(messages, reviewerUpdates, changeComments) {
    ...
    combinedMessages.forEach(m => {
      ...
      m.commentThreads = this._computeThreadsForMessage(changeComments, m);
      setRevisionNumber(m, combinedMessages);
      updateTag(m);
      m.isImportant = isImportant(m, combinedMessages);
    });
