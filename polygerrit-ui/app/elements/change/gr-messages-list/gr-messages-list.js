...
function computeTag(message) {
  if (!message.tag) {
    ...
    const comments = threads.map(
        t => { return t.comments.find(c => { return c.change_message_id === message.id; }); });
    const isRobot = comments.some(c => { return c && !!c.robot_id; });
...
function computeIsImportant(message, allMessages) {
  ...
  const hasSameTag = m => { return m.tag === message.tag; };
  const revNumber = message._revision_number || 0;
  const hasHigherRevisionNumber = m => { return m._revision_number > revNumber; };
...
class GrMessagesList extends KeyboardShortcutMixin(
  ...
  scrollToMessage(messageID) {
    ...
    if (!el) {
      ...
      setTimeout(() => { return this.scrollToMessage(messageID); });
      return;
    }
  ...
  _computeCombinedMessages(messages, reviewerUpdates, changeComments) {
    ...
    if (params.some(o => { return o === undefined; })) return [];
  ...
  _isVisibleShowAllActivityToggle(messages = []) {
    return messages.some(m => { return !m.isImportant; });
  }

  _computeHiddenEntriesCount(messages = []) {
    return messages.filter(m => { return !m.isImportant; }).length;
  }
  ...
  _combinedMessagesChanged(combinedMessages) {
    if (combinedMessages) {
      ...
      const tags = combinedMessages.map(
          message => {
            return message.tag || message.type ||
