  ...
  Polymer({
    ...
    properties: {
      ...
      _computedMessagesList: {
        type: Array,
        computed: '_computeItems(messages, reviewerUpdates, _hideAutomated)',
      },
      _visibleMessages: {
        type: Array,
        computed: '_computeVisibleMessages(_visibleMessageCount,' +
          ' _computedMessagesList)',
      },
      _visibleMessageCount: {
        type: Number,
        value: MAX_INITIAL_SHOWN_MESSAGES,
      },
    },
    ...
    _computeItems: function(messages, reviewerUpdates, hideAutomated) {
      ...
      for (var i = 0; i < messages.length; i++) {
        messages[i]._index = i;
        if (hideAutomated && this._computeIsAutomated(messages[i])) {
          messages.splice(i, 1);
          i--;
        }
      }
    ...
    */
    ...
    /**
     * When automated messages are toggled, _visibleMessagesCount may need to be
     * recomputed.
     *
     * When automated messages are being hidden, _visibleMessagesCount === the
     * number of non-automated messages that are already displayed.
     *
     * When automated messages are being shown, _visibleMessagesCount === the
     * length of _visibleMessages plus every automated message between the first
     * and the last non-automated message.
     */
    _handleAutomatedMessageToggleTap: function(e) {
      e.preventDefault();

      var messages =
          this._computeItems(this.messages, this.reviewerUpdates, false);
      // Don't mutate list length if user has already expanded all.
      if (this._visibleMessageCount !== messages.length) {
        if (this._hideAutomated) {
          var lastMessage = this._visibleMessages[0];
          this._visibleMessageCount = messages.length - messages.indexOf(lastMessage);
        } else {
          this._visibleMessageCount = this._visibleMessages.filter(function(msg) {
            return !this._computeIsAutomated(msg);
          }.bind(this)).length;
        }
      }
    ...
    _hasAutomatedMessages: function(messages) {
      for (var i = 0; messages && i < messages.length; i++) {
        if (this._computeIsAutomated(messages[i])) {
          return true;
        }
      }
      return false;
    ...
    _getDelta: function(messages, listLength) {
      return Math.min(messages.length - listLength, MESSAGES_INCREMENT);
    },

    _computeIncrementText: function(messages, listLength) {
      return 'Show ' + this._getDelta(messages, listLength) + ' more';
    },

    _computeShowHideTextHidden: function(messages, len) {
      return messages.length <= len;
    },

    _handleShowAllTap: function() {
      this._visibleMessageCount = this._computedMessagesList.length;
    },

    _handleIncrementShownMessages: function() {
      this._visibleMessageCount = this._visibleMessageCount +
          this._getDelta(this._computedMessagesList, this._visibleMessageCount);
    },

    _computeVisibleMessages: function(listLength, messages) {
      return messages.slice(-listLength);
    },

    _computeShowHideText: function(messages) {
      return 'Show all ' + messages.length + ' messages';
    },
  });
})();
