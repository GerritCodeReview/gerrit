...
function isReviewerUpdateMessage(message: CombinedMessage): boolean {
  if ((message as FormattedReviewerUpdateInfo).type === 'REVIEWER_UPDATE') {
    return true;
  }
  if (
    message.tag === MessageTag.TAG_REVIEWER_UPDATE ||
    message.tag === MessageTag.TAG_DELETE_REVIEWER
  ) {
    return true;
  }
  const text = (message as ChangeMessageInfo).message;
  return !!(
    text?.includes('Added to reviewer:') ||
    text?.includes('Removed from reviewer:')
  );
}
