...
function computeIsImportant(
  ...
  const LUCI_TAG = 'autogenerated:cq:new-patchset-run:';

  const hasSameTag = (m: CombinedMessage) => {
    if (message.tag?.startsWith(LUCI_TAG) && m.tag?.startsWith(LUCI_TAG))
      return true;
    return m.tag === message.tag;
  };
