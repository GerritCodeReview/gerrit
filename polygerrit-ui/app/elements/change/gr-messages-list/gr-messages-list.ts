...
function computeTag(message: CombinedMessage) {
  if (!message.tag) {
    ...
    const isRobot = comments.some(c => c && isRobotComment(c));
...
export class GrMessagesList extends KeyboardShortcutMixin(
  ...
  // _computeLabelExtremes always returns an object
  @property({type: Object, computed: '_computeLabelExtremes(labels.*)'})
  _labelExtremes!: {[lableName: string]: LabelExtreme};
  ...
  _computeLabelExtremes(
    ...
    const extremes: {[lableName: string]: LabelExtreme} = {};
