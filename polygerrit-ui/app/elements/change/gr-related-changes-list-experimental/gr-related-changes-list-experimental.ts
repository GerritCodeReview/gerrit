...
export class GrRelatedChangesListExperimental extends GrLitElement {
  ...
  sectionSizeFactory(
    ...
    let totalSize = sectionSizes.reduce((acc, val) => acc + val.size, 0);

    for (let i = 0; i < sectionSizes.length; i++) {
      const sizeObj = sectionSizes[i];
      const defaultSectionSize = sizeObj.size;
      const newSize = Math.min(
        Math.max(MAX_SIZE - 1 - totalSize + defaultSectionSize, 3),
        sizeObj.len
      );
      if (newSize !== sizeObj.size) {
        totalSize += newSize + 1 - defaultSectionSize;
    ...
    return (section: Section) => {
      ...
      return MAX_CHANGES_WHEN_COLLAPSED;
    };
  }
  ...
  markersPredicateFactory(
    ...
    numChangesShownWhenCollapsed = MAX_CHANGES_WHEN_COLLAPSED
...
export class GrRelatedCollapse extends GrLitElement {
  ...
  @property()
  numChangesWhenCollapsed = MAX_CHANGES_WHEN_COLLAPSED;
