...
import {hasType} from '../../../utils/common-util';
...
export class GrRelatedChangesList extends GestureEventListeners(
  ...
  clear() {
    ...
    this._submittedTogether = {changes: [], non_visible_changes: 0};
    this._conflicts = [];
    this._cherryPicks = [];
    this._sameTopic = [];
  }
  ...
  reload() {
    ...
    promises.push(
      this._getServerConfig().then(config => {
        ...
        if (change.topic && !config.change.submit_whole_topic) {
          return this.$.restAPI
            .getChangesWithSameTopic(change.topic, change._number)
            .then(response => {
              if (!response) {
                throw new Error('getChangesWithSameTopic returned undefined');
              }
  ...
  _getChangeNumber(change?: ChangeInfo | RelatedChangeAndCommitInfo) {
    ...
    if (hasType<RelatedChangeAndCommitInfo>(change, '_change_number')) {
