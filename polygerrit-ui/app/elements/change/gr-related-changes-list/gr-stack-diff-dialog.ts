...
export class GrStackDiffDialog extends LitElement {
  ...
  private getCommitsInPath(): {commit: CommitId; parent: CommitId}[] {
    const path: {commit: CommitId; parent: CommitId}[] = [];
    if (!this.targetCommitId || !this.baseCommitId) return path;

    const targetIndex = this.relatedChanges.findIndex(
      c => c.commit.commit === this.targetCommitId
    );
    if (targetIndex === -1) return path;

    for (let i = targetIndex; i < this.relatedChanges.length; i++) {
      const change = this.relatedChanges[i];
      const commit = change.commit.commit;

      if (commit === this.baseCommitId) {
        break;
      }

      if (change.commit.parents?.length > 0) {
        const parent = change.commit.parents[0].commit;
        path.push({commit, parent});
        if (parent === this.baseCommitId) {
          break;
        }
      }
    }
  ...
  private async loadDiffFileList() {
    ...
    this.error = undefined;
    try {
      const segments = this.getCommitsInPath();
      if (segments.length === 0) {
        ...
        this.files = files ?? {};
        delete this.files['/COMMIT_MSG'];
      } else {
        const diffPromises = segments.map(seg =>
          this.restApiService.getProjectCommitDiff(repo, seg.commit, seg.parent)
        );
        const diffResults = await Promise.all(diffPromises);

        const mergedFiles: FileNameToFileInfoMap = {};
        for (const filesMap of diffResults) {
          if (!filesMap) continue;
          for (const [path, info] of Object.entries(filesMap)) {
            if (path === '/COMMIT_MSG') continue;
            if (mergedFiles[path]) {
              mergedFiles[path] = {
                ...mergedFiles[path],
                lines_inserted:
                  (mergedFiles[path].lines_inserted ?? 0) +
                  (info.lines_inserted ?? 0),
                lines_deleted:
                  (mergedFiles[path].lines_deleted ?? 0) +
                  (info.lines_deleted ?? 0),
              };
            } else {
              mergedFiles[path] = {...info};
            }
          }
        }
