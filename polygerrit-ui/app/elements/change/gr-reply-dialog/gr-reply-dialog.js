  ...
  Polymer({
    ...
    send(includeComments, startReview) {
      ...
      return this._saveReview(obj, errFn).then(response => {
        ...
        if (startReview && !result.review_started) {
          // If we don't see review_started in the response, then we're talking
          // to a backend that doesn't support moving out of WIP at the same
          // time as posting a review. Fall back to sending a second API call
          // to start review and block until that returns.
