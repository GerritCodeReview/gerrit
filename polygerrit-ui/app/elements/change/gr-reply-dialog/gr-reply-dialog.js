...
(function() {
  ...
  Polymer({
    ...
    send: function() {
      ...
      function reviewerMapper(reviewer) {
        var reviewerId;
        var confirmed;
        if (reviewer.account) {
      ...
      obj.reviewers = this.$.reviewers.additions().map(reviewerMapper);
      if (this.serverConfig.note_db_enabled) {
        this.$$('#ccs').additions().forEach(function(reviewer) {
          reviewer = reviewerMapper(reviewer);
          reviewer.state = 'CC';
    ...
    _filterSuggestedReviewerAccount: function(suggestion) {
      ...
      for (var i = 0; i < this._ccs.length; i++) {
    ...
    _confirmPendingReviewer: function() {
      if (this._ccPendingConfirmation) {
        console.log('pending confirmation:', this._ccPendingConfirmation);
