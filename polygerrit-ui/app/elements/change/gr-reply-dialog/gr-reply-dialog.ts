...
import {removeServiceUsers} from '../../../utils/account-util';
...
import {CommentThread, HumanCommentInfoWithPath} from '../../diff/gr-comment-api/gr-comment-api';
...
export class GrReplyDialog extends KeyboardShortcutMixin(
  ...
  @property({type: Object})
  filterReviewerSuggestion: unknown;

  @property({type: Object})
  filterCCSuggestion: unknown;

  @property({type: Object})
  permittedLabels?: unknown;
  ...
  @property({type: Object})
  _pendingConfirmationDetails?: unknown;
  ...
  ready() {
    ...
    this._owner = this.change.owner;
    if (!this._owner) throw new Error('missing required owner property');
  ...
  _reviewerTypeChanged(
    ...
    if (splices && splices.indexSplices) {
      ...
      let key: AccountId | GroupId | undefined;
      let index;
      let account;
    ...
    for (const type of PENDING_REMOVAL_KEYS) {
      if (!isCancel) {
        ...
        for (let i = 0; i < reviewerArr.length; i++) {
          ...
          if (!isAccount(reviewer) || !keep[reviewer._account_id]) {
  ...
  _removeAccount(
    ...
    return this.$.restAPI
      .removeChangeReviewer(this.change._number, account._account_id)
  ...
  _mapReviewer(addition: AccountAddition): ReviewerInput {
    if (addition.account) {
      return {reviewer: addition.account._account_id};
    }
    if (addition.group) {
    ...
    throw new Error('reviewer must be either account or group');
  }
  ...
  send(includeComments: boolean, startReview: boolean) {
    ...
    const accountAdditions: {[key: number]: boolean} = {};
    ...
    if (ccsEl) {
      for (const addition of ccsEl.additions()) {
        if (addition.account) {
          accountAdditions[addition.account._account_id] = true;
        }
  ...
  _handle400Error(response?: Response | null) {
    ...
    return jsonPromise.then((parsed: ParsedJSON) => {
      ...
      if (response && response.status === 400 && result) {
        const errors: string[] = [];
        if (result.reviewers) {
        ...
        if (result.ccs) {
          const addCCs = Object.values(result.ccs);
          addCCs.forEach(r => errors.push(r.error ?? 'no explanation'));
        }
        response = {
          ...response,
          ok: false,
  ...
  _accountOrGroupKey(entry: AccountInfo | GroupInfo) {
    if (isAccount(entry)) return entry._account_id;
