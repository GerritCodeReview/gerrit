...
export class GrReplyDialog extends LitElement {
  ...
  isOwner(account?: AccountInfo, change?: ParsedChangeInfo) {
  ...
  getReviewerSuggestionsProvider(change?: ParsedChangeInfo) {
    if (!change) return;
  ...
  getCcSuggestionsProvider(change?: ParsedChangeInfo) {
    if (!change) return;
    const provider = new GrReviewerSuggestionsProvider(
      this.restApiService,
      ReviewerState.CC,
