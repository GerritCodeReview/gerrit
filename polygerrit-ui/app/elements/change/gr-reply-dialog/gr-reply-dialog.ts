...
export class GrReplyDialog extends LitElement {
  ...
  computeNewAttention() {
    ...
    if (this.change.status === ChangeStatus.NEW) {
      ...
      this.computeCommentAccountsForAttention(draftCommentThreads, isUploader).forEach(id =>
        newAttention.add(id)
      );
   ...
   */
  computeCommentAccountsForAttention(threads: CommentThread[], isUploader: boolean) {
