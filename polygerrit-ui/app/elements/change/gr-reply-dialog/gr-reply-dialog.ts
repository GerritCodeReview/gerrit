...
export class GrReplyDialog extends LitElement {
  ...
  private renderActionsSection() {
    return html`
      <section class="actions">
        <div class="left">
          ${when(
            this.isState(this.knownLatestState, LatestPatchState.CHECKING),
            () => html`
          ...
          ${when(
            this.isState(this.knownLatestState, LatestPatchState.NOT_LATEST),
            () => html`
  ...
  isState(knownLatestState?: LatestPatchState, value?: LatestPatchState) {
  ...
  computeSendButtonDisabled() {
    ...
    const hasDrafts = this.includeComments && this.draftCommentThreads.length > 0;
    return (
      !hasDrafts &&
      !this.draft.length &&
