...
export class GrReplyDialog extends LitElement {
  ...
  computeReviewers() {
    ...
    let additions = this.reviewersList?.additions() ?? [];
    reviewers.push(
      ...additions.map(v => toReviewInput(v, ReviewerState.REVIEWER))
    );

    additions = this.ccsList?.additions() ?? [];
    reviewers.push(...additions.map(v => toReviewInput(v, ReviewerState.CC)));

    let removals = (this.reviewersList?.removals() ?? [])
      .filter(
        r =>
    ...
    removals = (this.ccsList?.removals() ?? [])
      .filter(
        r =>
