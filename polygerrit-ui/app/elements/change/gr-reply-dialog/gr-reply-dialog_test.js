...
suite('gr-reply-dialog tests', () => {
  ...
  test('migrate reviewers between states', async () => {
    ...
    expect(mutations[4]).to.deep.equal({reviewer: 33, state: 'REMOVED'});
    expect(mutations[5]).to.deep.equal({reviewer: 35, state: 'REMOVED'});
    expect(mutations[6]).to.deep.equal({reviewer: 37, state: 'REMOVED'});
  });
