...
suite('gr-reply-dialog tests', () => {
  ...
  test('update attention section when reviewers and ccs change', async () => {
    ...
    // computeNewAttention() sets it to false
    ...
    element.reviewers.push(makeAccount());
    element.ccs.push(makeAccount());
    await element.updateComplete;
  ...
  test.only('migrate reviewers between states', async () => {
    debugger;
    await element.updateComplete;
    ...
    assert.deepEqual(element.reviewers.map(v => accountKey(v)), [reviewer2, cc1].map(v => accountKey(v)));
    assert.deepEqual(element.ccs.map(v => accountKey(v)), [cc2, reviewer1].map(v => accountKey(v)));

    // Add to other field which will automatically remove from the other field.
    reviewers.entry!.dispatchEvent(
      new CustomEvent('add', {
    ...
    assert.deepEqual(element.reviewers.map(v => accountKey(v)), [reviewer2, cc1, cc2].map(v => accountKey(v)));
    assert.deepEqual(element.ccs.map(v => accountKey(v)), [reviewer1].map(v => accountKey(v)));
    ...
    assert.deepEqual(element.reviewers.map(v => accountKey(v)), [cc1, cc2].map(v => accountKey(v)));
    assert.deepEqual(element.ccs.map(v => accountKey(v)), [reviewer1, reviewer2].map(v => accountKey(v)));
