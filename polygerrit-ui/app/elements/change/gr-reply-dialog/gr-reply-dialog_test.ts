...
suite('gr-reply-dialog tests', () => {
  ...
  function stubSaveReview(jsonResponseProducer: unknown) {
    return sinon.stub(element, '_saveReview').callsFake(
      review =>
        new Promise((resolve, reject) => {
          try {
            const result = (jsonResponseProducer as any)(review) || {};
  ...
  function checkComputeAttention(
    ...
    if (hasDraft) {
      draftThreads = [{...createCommentThread([
        {
          ...createDraft(),
          __draft: true,
          unresolved: true,
    ...
    attSetIds?.forEach(
      id => {
        if (!change.attention_set) return;
