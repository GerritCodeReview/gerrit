...
suite('gr-reply-dialog tests', () => {
  ...
  test.only('computeNewAttention when adding reviewers', async () => {
    debugger;
    ...
    element.change = {
      ...
      attention_set: {},
      reviewers: {
        REVIEWER: [createAccountWithId(1), createAccountWithId(2)]
      }
    };
    element.ccs = [];
  ...
  test('label picker', async () => {
    ...
    assert.deepEqual(review, {
      ...
      reviewers: [],
      add_to_attention_set: [{user: 999, reason: '<GERRIT_ACCOUNT_1> replied on the change'}],
      remove_from_attention_set: [],
      ignore_automatic_attention_set_rules: true,
    });
  });
