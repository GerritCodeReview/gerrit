...
suite('gr-reply-dialog tests', () => {
  ...
  test.only('manually added users are not lost when view updates.', async () => {
    assert.sameMembers(
      [...element.newAttentionSet],
      []
    );

    element.reviewers = [createAccountWithId(1), createAccountWithId(2), createAccountWithId(3)];
    ...
    assert.sameMembers([...element.newAttentionSet], [1 as AccountId, 2 as AccountId, 3 as AccountId, 999 as AccountId]);
