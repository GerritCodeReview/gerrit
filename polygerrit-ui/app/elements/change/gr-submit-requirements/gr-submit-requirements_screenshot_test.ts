...
suite('gr-submit-requirements screenshot tests', () => {
  ...
  test('submit requirements with AI Code-Coverage vote', async () => {
    ...
    try {
      ...
      await visualDiffDarkTheme(
        container,
        'gr-submit-requirements-ai-indicator'
      );
    } finally {
  ...
  test('vote chips comparison AI vs Human', async () => {
    ...
    const container = await fixture<HTMLDivElement>(html`
      <div
        style="display: inline-flex; flex-direction: column; gap: 16px; padding: 24px; background: var(--view-background-color); border: 1px solid var(--border-color); border-radius: 8px; font-family: var(--font-family);"
      >
        <div
          style="font-size: 16px; font-weight: 500; color: var(--primary-text-color); margin-bottom: 4px;"
        >
          Vote Provenance &amp; AI Indicator Comparison
        </div>
        <div
          style="font-size: 12px; color: var(--deemphasized-text-color); margin-bottom: 8px;"
        >
          Distinct AI spark icon (✦) indicates votes predicted by AI vs runtime
          execution
        </div>
        <div
          style="display: grid; grid-template-columns: 140px 100px 100px; gap: 12px; align-items: center; color: var(--primary-text-color); font-size: 13px;"
        >
          <span style="font-weight: 500; color: var(--deemphasized-text-color);"
            >Vote Type</span
          >
          <span style="font-weight: 500; color: var(--deemphasized-text-color);"
            >Runtime</span
          ...
          <gr-vote-chip
            .vote=${{value: 2, is_ai: false}}
            .label=${testLabel}
          ></gr-vote-chip>
          <gr-vote-chip
            .vote=${{value: 2, is_ai: true}}
            .label=${testLabel}
          ></gr-vote-chip>

          <span>Recommendation (+1)</span>
          <gr-vote-chip
            .vote=${{value: 1, is_ai: false}}
            .label=${testLabel}
          ></gr-vote-chip>
          <gr-vote-chip
            .vote=${{value: 1, is_ai: true}}
            .label=${testLabel}
          ></gr-vote-chip>
    ...
    const chips = container.querySelectorAll<GrVoteChip>('gr-vote-chip');
    await Promise.all(Array.from(chips).map(c => c.updateComplete));
