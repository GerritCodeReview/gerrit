...
(function() {
  ...
  Polymer({
    ...
    _computeSortedThreads(threads) {
      // In order to not trigger sub-property updates upon reordering, first
      // reset everything so that the previous values of properties such as
      // unresolved are undefined, and thus don't trigger an unwanted save
      // event.
      this._sortedThreads = [];
      Polymer.dom.flush();
      this._sortedThreads = this.threads.map(t =>
          this.updateThreadProperties(t)).sort((c1, c2) => {
