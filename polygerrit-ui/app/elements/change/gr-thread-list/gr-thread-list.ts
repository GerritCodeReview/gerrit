...
export class GrThreadList extends GestureEventListeners(
  ...
  _computeResolvedCommentsMessage(
    ...
    if (onlyShowRobotCommentsWithHumanReply) {
      threads = this.filterRobotThreads(threads) ?? [];
    }
  ...
  _countUnresolved(threads?: CommentThread[]) {
    return this.filterRobotThreads(threads)?.filter(isUnresolved).length ?? 0;
  }

  _countAllThreads(threads?: CommentThread[]) {
    return this.filterRobotThreads(threads)?.length ?? 0;
  }

  _countDrafts(threads?: CommentThread[]) {
    return this.filterRobotThreads(threads)?.filter(isDraftThread).length ?? 0;
  }
  ...
  filterRobotThreads(threads?: CommentThread[]) {
