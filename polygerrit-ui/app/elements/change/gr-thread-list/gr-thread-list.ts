...
export function compareThreads(
  ...
  // Sorting should not be based on the thread being unresolved or
  // being a draft thread, because that would be surprising re-sort
  // when the thread changes state.
...
export class GrThreadList extends LitElement {
  ...
  private renderCommentThreads() {
    ...
    return repeat(
      ...
      (thread, index) => {
        ...
        return this.renderCommentThread(thread, index, isFirst);
      }
    );
  }
  ...
  private renderCommentThread(
    ...
    console.log(`render comment-thread ${thread.rootId} ${index} ${isFirst}`);
    return html`
      ${isFirst && index !== 0
      ...
      <gr-comment-thread
        ...
        @comment-thread-editing-changed="${(e: CustomEvent) => {
          console.log(`thread-list editing changed: ${e.detail}`);
          this.requestUpdate();
        }}"
      ></gr-comment-thread>
    `;
  ...
  private renderAuthorChips() {
    console.log(`renderAuthorChips: ${this.getDisplayedThreads().length}`);
    const authors = getCommentAuthors(this.getDisplayedThreads(), this.account);
    console.log(`renderAuthorChips: ${authors.length}`);
