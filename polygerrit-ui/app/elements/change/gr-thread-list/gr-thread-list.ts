...
export class GrThreadList extends LitElement {
   ...
   */
  private editingThreads = new Map<string, boolean>();
  ...
  override willUpdate(changed: PropertyValues) {
    console.log('gr-thread-list.willUpdate');
  ...
  private renderEmptyThreadsMessage() {
    ...
    const button = html`
      <gr-button
        ...
        link
        @click="${this.handleResolvedCommentsMessageClick}"
  ...
  private handleResolvedCommentsMessageClick() {
  ...
  private compareThreads(c1: CommentThread, c2: CommentThread) {
    if (
      ...
      return lastUpdated(c2).getTime() - lastUpdated(c1).getTime();
    }
    ...
    if (lastUpdated(c2).getTime() !== lastUpdated(c1).getTime()) {
      return lastUpdated(c2).getTime() - lastUpdated(c1).getTime();
    }
