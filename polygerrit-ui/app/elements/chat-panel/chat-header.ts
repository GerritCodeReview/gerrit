...
export class ChatHeader extends LitElement {
  ...
  constructor() {
    ...
    subscribe(
      this,
      () => this.getChatModel().selectedModelId$,
      x => (this.selectedModelId = x)
    );
    subscribe(
      this,
  ...
  get selectedModel() {
    return this.availableModels.find(
      model => model.model_id === this.selectedModelId
    );
  }
          ...
          option => html`
            <md-menu-item @click=${() => this.onSwitchModel(option)}>
              <md-icon
                ...
                style=${styleMap({
                  visibility:
                    this.selectedModelId === option.model_id
                      ? 'visible'
                      : 'hidden',
                })}
                >done</md-icon
