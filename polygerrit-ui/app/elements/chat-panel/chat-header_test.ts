...
suite('chat-header tests', () => {
  ...
  let selectModelStub: sinon.SinonStub;
  let setModeSpy: sinon.SinonSpy;
  let startEmptyNewChatStub: sinon.SinonStub;
  ...
  setup(async () => {
    ...
    selectModelStub = sinon.stub(chatModel, 'selectModel');
    setModeSpy = sinon.spy(chatModel, 'setMode');
    startEmptyNewChatStub = sinon.stub(chatModel, 'startEmptyNewChat');
  ...
  test('handles switching model', async () => {
    ...
    assert.isTrue(selectModelStub.calledWith('gemini-ultra'));
  });
  ...
  test('handles show history', async () => {
    ...
    historyButton.click();
    assert.isTrue(setModeSpy.calledWith(ChatPanelMode.HISTORY));
  });
  ...
  test('handles back to chat', async () => {
    ...
    backButton.click();
    assert.isTrue(setModeSpy.calledWith(ChatPanelMode.CONVERSATION));
  });
  ...
  test('handles start new conversation', async () => {
    ...
    addButton.click();
    assert.isTrue(setModeSpy.calledWith(ChatPanelMode.CONVERSATION));
    assert.isTrue(startEmptyNewChatStub.calledWith(true));
  });
