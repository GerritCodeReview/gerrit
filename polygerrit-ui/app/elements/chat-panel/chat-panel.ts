...
import {chatModelToken, ChatPanelMode, Turn} from '../../models/chat/chat-model';
...
export class ChatPanel extends LitElement {
  ...
  static override styles = css`
    ...
    .prompt-section {
      ...
      border: 1px solid var(--separator-border-color);
      border-radius: 16px;
    }
    .google-symbols {
  ...
  constructor() {
    ...
    subscribe(
        this, () => this.getChatModel().conversationId$,
        x => (this.conversationId = x));
    subscribe(
        this, () => this.getChatModel().nextTurnIndex$,
        x => (this.nextTurnIndex = x));
    subscribe(
        this, () => this.getChatModel().mode$, x => (this.chatPanelMode = x));
    subscribe(
        this, () => this.getChatModel().userInput$, x => (this.userInput = x));
  }
    ...
    return html`
      <div class="prompt-section">
        <prompt-box
          ...
          @user-input-change=${this.onUserInputChange}
        ></prompt-box>
        <div class="ai-policy">
          Review agent may display inaccurate info.
          <a href="https://goto.google.com/code-ai-policy" target="_blank"
            >AI privacy policy</a
          >
        </div>
      </div>
  ...
  get mode() {
    ...
    if (this.turns.length === 0 ||
        (this.turns.length === 1 &&
         this.turns[0].userMessage.isBackgroundRequest)) {
      return Mode.SPLASH_PAGE;
    }
    return Mode.CHAT;
  }

  onUserInputChange(userInput: string) {
    this.getChatModel().updateUserInput(userInput);
  }
  ...
  override updated(changedProperties: Map<string, unknown>) {
    if (changedProperties.has('turns') && this.scrollableDiv) {
      ...
      if (lastUserMessageElement) {
        const scrollTop =
            computeScrollTop(scrollableDivElement, lastUserMessageElement);
      ...
      if (lastUserMessageElement && lastGeminiMessageElement) {
        const minHeight = computeGeminiMessageMinHeight(
            scrollableDivElement, lastUserMessageElement,
...
function computeScrollTop(
    scrollableDivElement: HTMLElement, lastUserMessageElement: HTMLElement) {
...
function computeGeminiMessageMinHeight(
    scrollableDivElement: HTMLElement, userMessageElement: HTMLElement,
