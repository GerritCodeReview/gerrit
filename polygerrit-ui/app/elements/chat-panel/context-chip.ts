...
export class ContextChip extends LitElement {
  ...
  static override styles = css`
    ...
    md-filter-chip.suggested-chip {
      opacity: 0.5;
      border-style: dashed;
      border-color: var(--chat-card-placeholder-text-color);
    }
    ...
    md-filter-chip {
      --md-sys-color-primary: var(--primary-foreground-color);
  ...
  override render() {
    ...
    return html`
      <md-filter-chip
        class="context-chip ${this.isSuggestion ? 'suggested-chip' : ''} ${
        this.isCustomAction ? 'custom-action-chip' : ''}"
        .label=${this.text}
  ...
  private navigateToUrl() {
    ...
    if (!safeUrl) return;
    windowOpen(window, safeUrl, '_blank', 'noopener,noreferrer');
  }
}
