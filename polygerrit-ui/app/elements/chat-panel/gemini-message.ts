...
export class GeminiMessage extends LitElement {
  ...
  @state() private account?: AccountInfo;

  @state() private change?: ChangeInfo;
  ...
  constructor() {
    ...
    subscribe(
      ...
      x => (this.change = x as ChangeInfo | undefined)
    );
    subscribe(
      this,
  ...
  private sortedComments() {
    return (this.message() ?? []).responseParts
  ...
  private reportSuggestionsShown() {
    ...
    if (!this.change || !agentId || !this.conversationId) return;
