...
import {ContextItem, ContextItemType, ModelInfo} from '../../api/ai-code-review';
import {chatModelToken, Turn} from '../../models/chat/chat-model';
import {contextItemEquals, searchForContextLinks} from '../../models/chat/context-item-util';
...
export class PromptBox extends LitElement {
  ...
  constructor() {
    ...
    subscribe(
        this, () => select(chatModel.state$, state => state.modelsLoadingError),
        x => (this.hasModelLoadingError = !!x));
    subscribe(this, () => chatModel.turns$, x => {
    ...
    subscribe(
        this, () => select(chatModel.state$, state => state.errorMessage),
        x => (this.errorMessage = x));
    subscribe(this, () => chatModel.activeModel$, x => (this.activeModel = x));
    subscribe(
        this, () => chatModel.contextItemTypes$,
        x => (this.contextItemTypes = x));
    subscribe(
        this,
        () => select(
            chatModel.state$, state => state.draftUserMessage.contextItems),
  ...
  get chatInputDisabledText() {
    ...
    if (!!this.errorMessage ||
        (geminiMessage !== undefined && !geminiMessage.responseComplete &&
         !isBackgroundRequest)) {
      return 'Thinking ...';
    }
    return '';
  ...
  protected get regularContextItems() {
    return this.contextItems;
  }

  get numExcessContextItems() {
    const regularContextItems = this.regularContextItems;
  ...
  override render() {
    return html`
      ...
      <md-chip-set class="context-chip-set">
        <context-input-chip
          @context-item-added=${
        (e: CustomEvent<ContextItem>) => this.onContextItemEmitted(e.detail)}
        ></context-input-chip>
        <context-chip
          class="cl-context"
          .isRemovable=${false}
        ...
        ></context-chip>
        ${
        (this.showAllContextItems ? this.regularContextItems :
                                    this.regularContextItems.slice(
                                        0, MAX_VISIBLE_CONTEXT_ITEMS_COLLAPSED))
            .map(contextItem => html`
            <context-chip
        ...
        (this.showAllContextItems ?
             ...
             this.suggestedContextItems.slice(
                 0, MAX_VISIBLE_SUGGESTED_CONTEXT_ITEMS_COLLAPSED))
            .map(
                contextItem => html`
            <context-chip
  ...
  protected onContextItemEmitted(contextItem: ContextItem) {
    this.addContextItem(contextItem);
  }

  protected addContextItem(contextItem: ContextItem) {
    this.getChatModel().addContextItem(contextItem);
  }

  protected removeContextItem(contextItem: ContextItem) {
    this.getChatModel().removeContextItem(contextItem);
  }

  protected acceptContextItemSuggestion(contextItem: ContextItem) {
    this.addContextItem(contextItem);
  ...
  tabChipVisible() {
    return (
        !this.chatInputDisabled && !this.promptHasInput &&
  ...
  private handleArrowKey(event: KeyboardEvent) {
    ...
    const previousUserMessages = this.previousUserMessages;
    const index = event.key === 'ArrowUp' ? previousMessageIndex - 1 :
  ...
  private onEnter(event: Event) {
    ...
    if (isLastTurnBackgroundRequest && !isLastTurnComplete) {
      ...
      this.getChatModel().startNewChatWithUserInput(
          userInput, 'text_input', undefined, undefined, true);
    } else {
  ...
  private adjustInputHeight() {
    ...
    this.promptInput.style.height =
        `${Math.min(currentHeight, this.maxInputHeight)}px`;
  ...
  private updateDynamicContextItemsDebounced() {
    this.updateDynamicContextItemsTask =
        debounce(this.updateDynamicContextItemsTask, () => {
          this.updateDynamicContextItems();
        }, 200);
  }

  private updateDynamicContextItems() {
    const suggestedContextItems =
        searchForContextLinks(this.userInput, this.contextItemTypes)
            .map(item => ({
                   ...item,
                   isProvisional: true,
                 }));
