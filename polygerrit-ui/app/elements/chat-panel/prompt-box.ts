...
export class PromptBox extends LitElement {
  ...
  constructor() {
    ...
    subscribe(
      this,
      () => select(chatModel.state$, state => state.modelsLoadingError),
    ...
    subscribe(
      this,
      () => select(chatModel.state$, state => state.errorMessage),
      x => (this.errorMessage = x)
    );
    subscribe(
      this,
      () => chatModel.activeModel$,
      x => (this.activeModel = x)
    );
    subscribe(
      this,
      () => chatModel.contextItemTypes$,
      x => (this.contextItemTypes = x)
    );
    subscribe(
      this,
      () =>
        select(chatModel.state$, state => state.draftUserMessage.contextItems),
      ...
      <md-chip-set class="context-chip-set">
        <context-input-chip
          @context-item-added=${(e: CustomEvent<ContextItem>) =>
            this.onContextItemEmitted(e.detail)}
        ></context-input-chip>
        <context-chip
          class="cl-context"
          .isRemovable=${false}
