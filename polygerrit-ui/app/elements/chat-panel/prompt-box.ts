...
export class PromptBox extends LitElement {
  ...
  private renderThisClChip() {
    // This CL is implicitly added to the context, so we don't need to add it.
    // The chip makes it clear to the user that it is already in the context.
    if (!this.change) return nothing;
    const clContextItem: ContextItem = {
      type_id: 'gerrit',
      link: `${window.location.origin}/c/${this.change.project}/+/${this.change._number}`,
      title: 'This CL',
      identifier: this.change.id,
      tooltip:
        'File diffs (against depot base), CL description, and CL comments.',
    };
    return html`
      <context-chip
        class="cl-context"
        .contextItem=${clContextItem}
        .isRemovable=${false}
      ></context-chip>
    `;
  }
