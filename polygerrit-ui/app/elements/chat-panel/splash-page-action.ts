...
import {Action, ContextItem, ContextItemType} from '../../api/ai-code-review';
...
export class SplashPageAction extends LitElement {
  ...
  constructor() {
    super();
    subscribe(
        this, () => this.getChatModel().contextItemTypes$,
  ...
  override render() {
    ...
    return html`
      <div
        ...
        @click=${(e: MouseEvent) => this.handleAction(e)}
      >
        <gr-icon class="action-icon" icon=${this.action.icon}></gr-icon>
      ...
      'has-subtext': !!this.action.subtext,
          ...
          ${when(this.action.subtext, () => html` <span
              class=${classMap({
                                 'action-subtext': true
                                      })}
  ...
  handleAction(event: MouseEvent) {
    ...
    const source = `button:splash_page:${action.display_text.toLowerCase()}`;
