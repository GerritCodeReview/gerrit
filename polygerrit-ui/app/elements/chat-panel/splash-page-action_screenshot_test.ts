...
suite('splash-page-action screenshot tests', () => {
  ...
  test('modal rendering targeted', async () => {
    const action: Action = {
      id: 'test-action',
      display_text: 'Test Action',
      initial_user_prompt: 'Test prompt',
    ...
    const modal = element.shadowRoot?.querySelector('#detailsModal') as HTMLElement;
    assert.isOk(modal);

    await visualDiff(modal, 'splash-page-action-modal-only');
    await visualDiffDarkTheme(modal, 'splash-page-action-modal-only');
  });
});
