...
suite('splash-page-action tests', () => {
  ...
  let predefinedPromptSpy: sinon.SinonSpy;
  let userInputSpy: sinon.SinonSpy;
  ...
  setup(async () => {
    ...
    predefinedPromptSpy = sinon.spy(
      chatModel,
      'startNewChatWithPredefinedPrompt'
    );
    userInputSpy = sinon.spy(chatModel, 'startNewChatWithUserInput');
  ...
  test('handles click with predefined prompt', async () => {
    ...
    assert.isTrue(predefinedPromptSpy.calledOnce);
    assert.isTrue(predefinedPromptSpy.calledWith('test-action', []));
    assert.isFalse(userInputSpy.called);
  });
  ...
  test('handles click with user input', async () => {
    ...
    assert.isTrue(userInputSpy.calledOnce);
    assert.isTrue(userInputSpy.calledWith('Initial prompt', 'test-action', []));
    assert.isFalse(predefinedPromptSpy.called);
  });
});
