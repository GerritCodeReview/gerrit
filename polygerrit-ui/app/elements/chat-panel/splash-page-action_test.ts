...
suite('splash-page-action tests', () => {
      ...
      /* HTML */ `
        ...
        <dialog id="detailsModal" tabindex="-1">
          <div role="dialog" aria-labelledby="detailsTitle">
            <h3 class="heading-3 diffHeader" id="detailsTitle">Test Action</h3>
            <div class="detailsContent">
              <div class="description-section">Test description</div>
              <div class="modal-row">
  ...
  test('renders with action without description', async () => {
    const action: Action = {
      id: 'test-action-no-desc',
      display_text: 'Test Action No Desc',
      hover_text: 'Test hover',
      subtext: 'Test subtext',
      icon: 'test-icon',
    };
    ...
    assert.isOk(modal);
    const descSection = modal?.querySelector('.description-section');
    assert.equal(descSection?.textContent?.trim(), 'No description provided');
  });
  ...
  test('opens details dialog on info button click', async () => {
    ...
    const dialog = element.shadowRoot?.querySelector('#detailsModal') as HTMLDialogElement;
    assert.isOk(dialog);
