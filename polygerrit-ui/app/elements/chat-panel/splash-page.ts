...
export class SplashPage extends LitElement {
  ...
  constructor() {
    ...
    subscribe(
        this, () => this.getChatModel().actions$,
        x =>
            (this.actions =
                 (x ?? []).filter(action => !!action.enable_splash_page_card)));
    subscribe(
      ...
      expanded: this.isBackgroundRequestExpanded,
        ...
        <div class="expansion-button-container">
          <button
            ...
            aria-label=${
        this.isBackgroundRequestExpanded ? 'Hide' : 'Show more'}
  ...
  private renderActionChipSet() {
    return html`
      <md-chip-set class="action-container">
        ${this.actions.map((action, index, array) => html`
          <splash-page-action
              .action=${action}
              .isFirst=${index === 0}
