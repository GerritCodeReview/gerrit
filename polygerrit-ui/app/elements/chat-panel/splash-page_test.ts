...
suite('splash-page tests', () => {
  ...
  let startNewChatSpy: sinon.SinonSpy;
  ...
  setup(async () => {
    ...
    startNewChatSpy = sinon.spy(chatModel, 'startNewChatWithPredefinedPrompt');
  ...
  test('clicking action starts new chat', async () => {
    ...
    assert.equal(startNewChatSpy.firstCall.args[0], 'action1');
  });
});
