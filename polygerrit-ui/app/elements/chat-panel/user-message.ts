...
/**
 * A component to display a single user message.
 */
...
export class UserMessage extends LitElement {
  ...
  constructor() {
    super();
    subscribe(
        this, () => this.getUserModel().account$, x => (this.account = x));
  }
  ...
  static override styles = css`
    :host {
      display: flex;
      /* Original scss had 'flex-direction: column', but 'margin-right' on user-picture
         suggests a row layout which is more common for chat messages.
         Reverting to default row direction. */
      padding-bottom: var(
        --spacing-xl
      ); /* --spacing-v-minus-2 -> --spacing-xl */
    }

    .user-input-container {
      /* This was likely to align content when user image is present.
         With flex layout, this might not be needed or needs adjustment.
         Original: padding-top: var(--spacing-m); */
    }
    ...
    md-filter-chip.context-toggle-chip {
      margin: 0;
      margin-left: auto;
      --md-filter-chip-unselected-outline-color: var(
        --chat-panel-chip-border-color
      );
      ...
      --md-filter-chip-label-text-line-height: var(--line-height-small, 1.25rem);
      ...
      <div class="user-input-container">
        ...
        <div class="context-chip-set">
          <context-chip
            ...
            .isRemovable=${false}
            text="This CL"
            tooltip="File diffs (against depot base), CL description, and CL comments."
          ></context-chip>
          ${
        map(this.showAllContextItems ?
                this.regularContextItems :
                this.regularContextItems.slice(
                    0, MAX_VISIBLE_CONTEXT_ITEMS_COLLAPSED),
            (contextItem: ContextItem) => html`
              ...
              ></context-chip>
            `)}
          ${when(this.shouldShowContextToggle, () => html`
              <md-filter-chip
