...
export class GrChecksResults extends LitElement {
  ...
  private onAttemptSelected(e: CustomEvent<{value: string | undefined}>) {
    ...
    assert(!isNaN(value) && value >= 0, `unexpected attemt number ${value}`);
    const attempt = value > 0 ? value : undefined;
  ...
  private createAttemptDropdownItems() {
    ...
    const items: DropdownItem[] = this.runs
      ...
      .map(i => {
        return {
          value: i,
          text: i === 0 ? 'Latest Attempt per run' : `Attempt ${i}`,
        };
      });
    return items;
  }
