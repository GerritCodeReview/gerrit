...
export class GrResultRow extends LitElement {
  ...
  renderTag(tag: Tag) {
    return html`<gr-tooltip-content has-tooltip ?position-below=${true} title=${
      tag.tooltip ?? 'A category tag for this check result. Click to filter.'
    }>
      <button class="tag ${tag.color}" @click=${(e: MouseEvent) =>
      this.tagClick(e, tag.name)}>
        <span>${tag.name}</span>
      </button>
    </<gr-tooltip-content>`;
  }
        ...
        >
          <h3 class="left heading-3">
            ...
            <gr-tooltip-content
              has-tooltip
              ?position-below=${true}
              title=${CATEGORY_TOOLTIPS.get(category)}
            >
