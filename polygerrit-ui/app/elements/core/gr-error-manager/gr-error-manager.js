...
(function() {
  ...
  Polymer({
    ...
    _handleServerError(e) {
      const {req, response} = e.detail;
      Promise.all([response.text(), this._getLoggedIn()]).then(values => {
        const errorText = values[0];
        const loggedIn = values[1];
        const url = req && (req.anonymizedUrl || req.url);
