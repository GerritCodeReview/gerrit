...
(function() {
  ...
  class GrErrorManager extends Polymer.mixinBehaviors( [
    ...
    attached() {
      ...
      this._authErrorHandler = Gerrit.on('auth-error', event => {
    ...
    _handleServerError(e) {
      ...
      response.text().then(errorText => {
        ...
        if (response.status === 403
          ...
          // Re-check on auth
          this.$.restAPI.getLoggedIn(/* skipCache= */ true);
    ...
    _checkSignedIn() {
      ...
      this.$.restAPI.getLoggedIn(/* skipCache= */ true).then(isLoggedIn => {
        if (!isLoggedIn) {
          // check later
          // 1. guest mode
          // 2. or signed out
