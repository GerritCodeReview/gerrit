...
export class GrKeyBindingDisplay extends GrLitElement {
  ...
  render() {
    return html`${this.binding.map(this.renderBinding.bind(this))}`;
  }

  renderBinding(binding: string[], index: number) {
    const or = index > 0 ? ' or ' : '';
    return html`${or}${this._computeModifiers(binding).map(this.renderModifier)}
      <span class="key">${this._computeKey(binding)}</span>`;
  }

  renderModifier(modifier: string) {
    return html`<span class="key modifier">${modifier}</span> `;
  }
