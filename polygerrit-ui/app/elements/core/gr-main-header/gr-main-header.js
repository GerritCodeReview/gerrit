...
(function() {
  ...
  var USER_LINKS = [
    ...
    {
      url: getBaseUrl() + '/q/has:draft',
      name: 'Draft Comments',
    },
    {
      url: getBaseUrl() + '/q/has:edit',
      name: 'Edits',
    },
    {
      url: getBaseUrl() + '/q/is:watched+is:open',
      name: 'Watched Changes',
    },
    {
      url: getBaseUrl() + '/q/is:starred',
  ...
  Polymer({
    ...
    _handleLocationChange: function(e) {
      if (getBaseUrlTrueOrFalse()) {
        this._loginURL = getBaseUrl() + '/login/' + encodeURIComponent(
            window.location.pathname +
            window.location.search +
            window.location.hash).replace(getBaseUrl().replace('/', ''), '');
      } else {
        this._loginURL = getBaseUrl() + '/login/' + encodeURIComponent(
            window.location.pathname +
            window.location.search +
  ...
  function getBaseUrlTrueOrFalse() {
    if (window.CONFIG_BASE_URL) {
      return true;
    }
