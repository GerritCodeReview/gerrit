...
export class GrMainHeader extends LitElement {
  ...
  <nav>
    <a href=${`//${
      window.PRIVATE_TO_PUBLIC_HOST_MAP?.[window.location.host] ??
      window.location.host
    }${getBaseUrl()}/`} class="bigTitle">
    ...
    <div class="rightItems">
      ...
      ${this.renderCopyPublicAvailableUrlButton()}
      </div>
      ${this.renderAccount()}
    </div>
  </nav>
  ...
  private renderCopyPublicAvailableUrlButton() {
    if (window.location.href === this.publicAvailableUrl) return;
    return html`
      <a
        href=${this.publicAvailableUrl}
