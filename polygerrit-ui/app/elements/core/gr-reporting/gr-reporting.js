...
(function() {
  ...
  let GrReporting = Polymer({
    ...
    reporter(type, category, eventName, eventValue, eventDetails, opt_noLog) {
      ...
      report.apply(this, [eventInfo, opt_noLog]);
    },
     ...
     *     logged to the JS console.
     */
    _defaultReporter(eventInfo, opt_noLog) {
      document.dispatchEvent(new CustomEvent(eventInfo.type, {eventInfo}));
      if (opt_noLog) { return; }
      if (eventInfo.type === ERROR.TYPE &&
          eventInfo.category === ERROR.CATEGORY) {
        console.error(eventInfo.value && eventInfo.value.error
            || eventInfo.name);
      } else {
        if (eventInfo.eventValue !== undefined) {
          console.log(`Reporting: ${eventInfo.name}: ${eventInfo.value}`);
        } else {
          console.log(`Reporting: ${eventInfo.name}`);
        }
      }
    },
     ...
     *     logged to the JS console.
     */
    _cachingReporter(eventInfo, opt_noLog) {
      if (eventInfo.type === ERROR.TYPE
          && eventInfo.category === ERROR.CATEGORY) {
        console.error(eventInfo.value && eventInfo.value.error
            || eventInfo.name);
      ...
      if (this._isMetricsPluginLoaded()) {
        ...
        this._defaultReporter(eventInfo, opt_noLog);
      } else {
        pending.push([eventInfo, opt_noLog]);
      }
    },

    _createEventInfo(type, category, eventName, eventValue, eventDetails) {
      const eventInfo = {
        type,
        category,
        name: eventName,
