  ...
  Polymer({
    ...
    _generateUrl(params) {
      ...
      if (params.view === Gerrit.Nav.View.SEARCH) {
        const operators = [];
        if (params.owner) {
          operators.push('owner:' + this.encodeURL(params.owner));
        }
        if (params.project) {
          operators.push('project:' + this.encodeURL(params.project));
        }
        if (params.branch) {
          operators.push('branch:' + this.encodeURL(params.branch));
        }
        if (params.topic) {
          operators.push('topic:"' + this.encodeURL(params.topic) + '"');
        }
        if (params.hashtag) {
          operators.push('hashtag:"' +
              this.encodeURL(params.hashtag.toLowerCase()) + '"');
        }
        if (params.statuses) {
          if (params.statuses.length === 1) {
            operators.push('status:' + this.encodeURL(params.statuses[0]));
          } else if (params.statuses.length > 1) {
            operators.push(
                '(' +
                params.statuses.map(s => `status:${this.encodeURL(s)}`).join(' OR ') +
                ')');
          }
        }
    ...
    _normalizeLegacyRouteParams(params) {
      ...
      return this._restAPI.getFromProjectLookup(params.changeNum).then(project => {
