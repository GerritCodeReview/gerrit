...
(function() {
  ...
  Polymer({
    ...
    _firstCodeBrowserWeblink(weblinks) {
      ...
      for (var i = 0; i < codeBrowserLinks.length; i++) {
        const weblink = weblinks.find(weblink => weblink.name === codeBrowserLinks[i]);
    ...
    _getChangeWeblinks({repo, commit, options: {weblinks, config}}) {
      ...
      return weblinks.filter(weblink =>
        !commitWeblink || !commitWeblink.name || weblink.name !== commitWeblink.name).map(
          ({name, url}) => {
