...
const RoutePattern = {
  ...
  // Matches /c/<project>/+/<changeNum>/commentsTab/<commentId>/
  // Navigates to the commentId inside the Comments Tab
  COMMENTS_TAB: /^\/c\/(.+)\/\+\/(\d+)\/commentsTab\/(\w+)\/?$/,
...
export class GrRouter extends PolymerElement {
  ...
  _startRouter() {
    ...
    this._mapRoute(RoutePattern.COMMENTS_TAB, '_handleCommentsTabRoute');
  ...
  _handleCommentsTabRoute(ctx: PageContextWithQueryMap) {
    const changeNum = Number(ctx.params[1]) as NumericChangeId;
    const params: GenerateUrlChangeViewParameters = {
      project: ctx.params[0] as RepoName,
      changeNum,
