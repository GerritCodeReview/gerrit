...
export class GrRouter implements Finalizable, NavigationService {
  ...
  /**
   * Similar to mapRoute() above, but delegates more into view model files where
   * appropriate.
   */
  mapRouteState<T extends ViewState>(
    ...
    const newHandler = (ctx: PageContext) => {
      const state = route.createState(ctx);
      this.setState(state as AppElementParams);
      viewModel.setState(state);
    };
    this.mapRoute(route.pattern, handlerName, newHandler, authRedirect);
  }
