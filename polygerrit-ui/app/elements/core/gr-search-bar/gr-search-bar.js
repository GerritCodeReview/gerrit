...
(function() {
  ...
  Polymer({
    ...
    _preventDefaultAndNavigateToInputVal: function(e) {
      ...
      if (this._inputVal) {
        ...
        if (trimmedInput.match(CHANGE_NUM_REGEX)) {
          ...
          page.show('/c/' + trimmedInput);
        } else if (trimmedInput.match(CHANGE_ID_REGEX)) {
          // Matches a change ID.

          this.$.searchInput.disabled = true;
          this._getSingleChangeNumberFromID(trimmedInput).then(function(num) {
            if (num) {
    ...
    /**
     * Searches for changes with the ID supplied. If one unique change number is
     * found, return that number.
     */
    _getSingleChangeNumberFromID: function(id) {
      return this.$.restAPI.getChanges(10, id).then(function(res) {
        if (res.length === 1) {
