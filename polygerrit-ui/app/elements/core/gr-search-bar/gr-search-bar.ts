...
export class GrSearchBar extends LitElement {
  ...
  @query('#searchInput') protected searchInput!: GrAutocomplete;
  ...
  constructor() {
    super();
    this.shortcuts.addLocal({key: Key.SEARCH}, () => this.handleSearch());
  ...
  private preventDefaultAndNavigateToInputVal(e: Event) {
    ...
    if (queryUtil(target, '#input')) {
      (queryUtil(target, '#input') as HTMLElement)!.blur();
    } else {
      target.blur();
    }
