(function() {
  ...
  function CoveragePlugin(gerritPlugin) {
    ...
    this._annotationApi = this.gerritPlugin_.annotationApi()
      .addNotifier(notifyFunc => {
      ...
      }).addLayer(annotationActionsContext => {
        if (Object.keys(coverageData).length === 0) {
          // Coverage data is not ready yet.
          return;
        }
        const path = annotationActionsContext.path;
        const line = annotationActionsContext.line;
        const el = annotationActionsContext.el;
        ...
        if (coverageData[path] &&
            coverageData[path].changeNum === parseInt(annotationActionsContext.changeNum) &&
            coverageData[path].patchNum === parseInt(annotationActionsContext.patchNum)) {
          var linesMissingCoverage = coverageData[path].linesMissingCoverage
          if (linesMissingCoverage.includes(line.afterNumber)) {
            annotationActionsContext.annotateRangeFunc(0, line.text.length, el, cssClass);
          }
        }
      });
  }
