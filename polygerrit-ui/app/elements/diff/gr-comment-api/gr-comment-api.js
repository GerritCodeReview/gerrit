...
(function() {
  ...
  Polymer({
    ...
    /**
     * Load all comments for the given change number. The returned promise
     * resolves when the comments have loaded, but does not yield the comment
     * data.
     *
     ...
     * path contains diff comments in the given patch set.
     *
     * A path has comments iff it maps to true. A path without comments is
     * represented by either mapping the path to false, or by having no mapping
     * for that path.
     *
    ...
    /**
     * Get the comments for a path and patch-range. Returns an object with
     * `left` and `right` properties mapping to arrays of comments in on either
     * side of the patch range for that path.
     *
     * @param {!string} path
     * @param {!Object} patchRange The patch-range object containing patchNum
     *     and basePatchNum properties to represent the range.
    ...
    _isInBaseOfPatchRange(comment, range) {
      if (range.basePatchNum === PARENT && comment.side === PARENT) {
        return true;
      }
