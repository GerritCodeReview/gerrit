...
(function() {
  ...
  Polymer({
    ...
    loadForPath(changeNum, path) {
      const promises = [];
      promises.push(this.$.restAPI.getDiffComments(changeNum, null, null, path));
      promises.push(this.$.restAPI.getDiffRobotComments(changeNum, null, null, path));
      promises.push(this.$.restAPI.getDiffDrafts(changeNum, null, null, path));

      return Promise.all(promises).then(([comments, robotComments, drafts]) => {
	    let pathComments = {}
        let pathRobotComments = {}
        let pathDrafts = {}
