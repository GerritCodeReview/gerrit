...
export interface NonRobotCommentInfoWithPath extends CommentInfo {
  path: string;
  __draft?: boolean;
}
...
class ChangeComments {
  ...
  ): NonRobotCommentInfoWithPath | RobotCommentInfoWithPath | undefined {
  ...
  getCommentsBySideForPath(
    ...
    drafts.forEach(d => {
      // drafts doesn't include robot comments
      (d as NonRobotCommentInfoWithPath).__draft = true;
    });
...
class GrCommentApi extends GestureEventListeners(
  ...
  created() {
    ...
    this.addEventListener('reload-drafts', changeNum =>
      // TODO(TS): This is a wrong code, however keep it as is for new
