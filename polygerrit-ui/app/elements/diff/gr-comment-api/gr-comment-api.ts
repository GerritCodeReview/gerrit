...
interface CommentsInfoWithPath extends CommentInfo {
  path: string;
}

type PathToCommentsInfoWithPathMap = {[path: string]: CommentsInfoWithPath[]};

interface RobotCommentsInfoWithPath extends RobotCommentInfo {
  path: string;
}

type PathToRobotCommentsInfoWithPathMap = {
  [path: string]: RobotCommentsInfoWithPath[];
};

class ChangeComments {
  private readonly _comments: PathToCommentsInfoWithPathMap;

  private readonly _robotComments: PathToRobotCommentsInfoWithPathMap;
   ...
   *
   * @param - map between file path and comments
   */
  _addPath(
    comments: PathToCommentsInfoMap = {}
  ...
  ): RobotCommentsInfoWithPath | CommentsInfoWithPath | undefined {
    const findComment = (
      comments:
        | PathToCommentsInfoWithPathMap
        | PathToRobotCommentsInfoWithPathMap
    ) => {
      let comment;
   ...
   * are not mapped.
   *
   * @param opt_patchRange The patch-range object containing
   ...
   *
   * @param comments
   * @param parentOnly whether the only comments returned should have
   * the side attribute set to PARENT
   * @param commentSide whether the comment was left on the left or the
   * right side regardless or unified or side-by-side
   * @param opt_line line number, can be undefined if file comment
   * @return an array of comments
   */
  _filterCommentsBySideAndLine(comments, parentOnly, commentSide, opt_line) {
    return comments
      .filter(c => {
   ...
   * Gets all the comments and robot comments for the given change.
   *
   * @param opt_includeDrafts
   * @param opt_patchNum
   * @return
   */
  getAllComments(opt_includeDrafts, opt_patchNum) {
   ...
   * @return
   */
  getAllCommentsForPath(path, opt_patchNum, opt_includeDrafts) {
   ...
   * @return
   */
  getAllDraftsForPath(path, opt_patchNum) {
   ...
   * @param opt_projectConfig Optional project config object to
   * include in the meta sub-object.
   * @return
   */
  getCommentsBySideForPath(path, patchRange, opt_projectConfig) {
    let comments = [];
    let drafts = [];
    let robotComments = [];
  ...
  _sortComments(comments) {
    return comments.slice(0).sort((c1, c2) => {
      const dateDiff = parseDate(c1.updated) - parseDate(c2.updated);
      if (dateDiff) {
        return dateDiff;
      }
      return c1.id - c2.id;
  ...
  reloadDrafts(changeNum) {
    ...
    return this.$.restAPI.getDiffDrafts(changeNum).then(drafts => {
      this._changeComments = new ChangeComments(
        ...
        this._changeComments.robotComments,
        drafts,
        changeNum
      );
      return this._changeComments;
