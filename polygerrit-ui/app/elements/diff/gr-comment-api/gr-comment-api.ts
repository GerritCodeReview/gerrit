  ...
  getPortedCommentThreads(
    ...
    return createCommentThreads(allComments)
      .filter(thread => {
        ...
        if (
          patchNumEquals(thread.patchNum, patchRange!.basePatchNum) ||
          patchNumEquals(thread.patchNum, patchRange!.patchNum)
        ) {
      ...
      .map(thread => {
        ...
        if (thread.commentSide === CommentSide.PARENT) {
          ...
          // W,X,Y,Z are patchsets such that Z > Y > X > W
          // comparing Base vs Y, X vs Y, Y vs Z or W vs X, we show the comment
          // on the left side of the diff as file level comments as range of
          // ported comments is only applicable to the right side of the diff
