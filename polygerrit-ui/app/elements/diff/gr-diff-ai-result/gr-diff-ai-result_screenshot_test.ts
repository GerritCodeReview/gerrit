...
import {CreateCommentPart, ResponsePartType} from '../../../models/chat/chat-model';
...
suite('gr-diff-ai-result screenshot tests', () => {
  ...
  setup(async () => {
    element = await fixture(
      html`<gr-diff-ai-result></gr-diff-ai-result>`
    );
  });
  ...
  test('renders', async () => {
    element.result = {
      ...
      content: '',
      comment: {
        message: 'Here is a suggestion: please consider using a more functional approach.',
        path: 'test.ts',
      },
