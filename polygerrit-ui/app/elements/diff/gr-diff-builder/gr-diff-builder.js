...
(function(window, GrDiffGroup, GrDiffLine) {
   ...
   * Note that no attempt is currently made to handle other unicode grapheme
   * segmentation rules, like combining marks, or other zero-width joiners (LRO,
   * ZWSP, ZWNBSP, etc).
   *
  ...
  function GrDiffBuilder(diff, comments, prefs, projectName, outputEl, layers) {
    ...
    this._brNode = this._createElement('span', 'br');
    this._contentTextNode = this._createElement('div', 'contentText');
    this._tdNode = this._createElement('td');
  }
  ...
  GrDiffBuilder.prototype._createContextControl = function(section, line) {
    ...
    const td = this._tdNode.cloneNode();
  ...
  GrDiffBuilder.prototype._createTextEl = function(line, opt_side) {
    ...
    const contentText = this._formatText(line.text, tabSize, lineLimit);
    if (opt_side) {
  ...
  GrDiffBuilder.prototype._formatText = function(text, tabSize, lineLimit) {
    const contentText = this._contentTextNode.cloneNode();

    if (!(lineLimit > 0)) {
      lineLimit = 1;
    }

    // This regex segments |text| along tabs and surrogate pairs.
    let columnPos = 0;
    let textOffset = 0;
    for (let segment of text.split(/\t|[\uD800-\uDBFF][\uDC00-\uDFFF]/)) {
      if (segment) {
        ...
        while (rowEnd < segment.length) {
          ...
          contentText.appendChild(this._brNode.cloneNode());
          columnPos = 0;
          rowStart = rowEnd;
          rowEnd += lineLimit;
        }
      ...
      // Handle special characters captured by the regexp.
