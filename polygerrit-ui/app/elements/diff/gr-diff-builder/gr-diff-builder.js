...
(function(window, GrDiffGroup, GrDiffLine) {
  ...
  GrDiffBuilder.prototype._createContextControl = function(section, line) {
    ...
    const numLines = line.contextGroups[line.contextGroups.length - 1].lineRange.left.end - line.contextGroups[0].lineRange.left.start + 1;
    if (numLines === 0) return null;
    const firstLines = contextGroups[0].lines;
    const lastLines = contextGroups[contextGroups.length - 1].lines;
    const firstContextInFile = firstLines.length > 0 ? firstLines[0].firstInFile : false;
    const lastContextInFile = lastLines.length > 0 ? lastLines[lastLines.length - 1].lastInFile: false;
    let contextPlacement = ContextPlacement.IN_BETWEEN;
    ...
    let minimalForStepExpansion = (contextPlacement === ContextPlacement.IN_BETWEEN) ? 
    ...
    if (showStepExpansion) {
      td.appendChild(this._createContextButton(
          GrDiffBuilder.ContextButtonType.STEP, section, line, numLines, contextPlacement));
    }
    return td;
  };
  ...
  GrDiffBuilder.prototype._createContextButton = function(type, section, line,
    ...
    if (type === GrDiffBuilder.ContextButtonType.ALL) {
      ...
      const linesToShowAbove = contextPlacement === ContextPlacement.FIRST ? 0 : PARTIAL_CONTEXT_AMOUNT;
      const linesToShowBelow = contextPlacement === ContextPlacement.LAST ? 0 : PARTIAL_CONTEXT_AMOUNT;
