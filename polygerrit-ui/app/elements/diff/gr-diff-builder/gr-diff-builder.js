...
(function(window, GrDiffGroup, GrDiffLine) {
   ...
   * @param {boolean} isRemove If the line the comment is of type 'remove'.
  ...
  GrDiffBuilder.prototype._determineIsOnParent = function(
      firstCommentSide, patchRange, isRemove, opt_side) {
    return ((isRemove ||
  ...
  GrDiffBuilder.prototype._commentThreadGroupForLine = function(
    ...
    for (const threadEl of threadElsForGroup) {
      // TODO(oler): Can isOnParent be true for a thread even though side !=
      //             PARENT for it's first comment?
      // threadEl.isOnParent = this._determineIsOnParent(
      //     threadEl.comments[0].side, this._patchRange,
      //     line.type === GrDiffLine.Type.REMOVE, commentSide);
