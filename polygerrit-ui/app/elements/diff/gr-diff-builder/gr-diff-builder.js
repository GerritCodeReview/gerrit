...
(function(window, GrDiffGroup, GrDiffLine) {
   ...
   */
  const REGEX_GLYPHS_OF_SAME_WIDTH = new RegExp(
      '(?:[\\uD800-\\uDBFF][\\uDC00-\\uDFFF])+|[^\\t\\uD800-\\uDBFF]+|\\t+',
      'g');
  ...
  GrDiffBuilder.prototype._createTextEl = function(line, opt_side) {
    ...
    const tabSize = +this._prefs.tab_size;
    const lineLimit =
        !this._prefs.line_wrapping ? +this._prefs.line_length : Infinity;

    let contentText = this._formatText(line.text, tabSize, lineLimit);
    if (opt_side) {
  ...
  GrDiffBuilder.prototype._formatText = function(
      text, tabSize = 8, lineLimit = Infinity) {
    ...
    // This regex segments |text| into runs of wide characters (surrogate
    // pairs), normal characters, and tab characters.
    let columnPos = 0;
    for (const segment of text.match(REGEX_GLYPHS_OF_SAME_WIDTH)) {
      const codePoint = segment.codePointAt(0);
      if (codePoint === 9) {
        // |segment| is a run of '\t' characters.
        const tabWrapColumn = lineLimit - (lineLimit % tabSize);
        for (let i = 0; i < segment.length; i++) {
          if (columnPos >= tabWrapColumn) {
            contentText.appendChild(this._createElement('span', 'br'));
            columnPos = 0;
          }
          const effectiveTabSize = Math.min(
        ...
        // glyph), or a run of surrogate pairs (two string elements per glyph).
        // |surrogateShift| accounts for the difference.
        const charsPerGlyph = 1 + (codePoint > 0xFFFF);

        // If |segment| doesn't fit entirely on the current line,
        // append chunks of |segment| followed by line breaks.
        let rowStart = 0;
        let rowEnd = (lineLimit - columnPos) * charsPerGlyph;
