...
export abstract class GrDiffBuilder {
  ...
  private _createBlockExpansionButtons(
    ...
    if (showAbove) {
      aboveBlockButton = this._createBlockButton(section,contextGroups,
        ContextButtonType.BLOCK_ABOVE,numLines,rightStart - 1,rightSyntaxTree);
    }
    if (showBelow) {
      belowBlockButton = this._createBlockButton(section,contextGroups,
        ContextButtonType.BLOCK_BELOW,numLines,rightEnd + 1,rightSyntaxTree);
    }
  ...
  private _createBlockButton(
    ...
    if(containingBlock){
      ...
      const targetLine = buttonType === ContextButtonType.BLOCK_ABOVE ? range.end_line : range.start_line;
      const distanceToBlockEnd = targetLine - referenceLine;
      if(distanceToBlockEnd < numLines){
        linesToExpand = distanceToBlockEnd;
      }
    }
