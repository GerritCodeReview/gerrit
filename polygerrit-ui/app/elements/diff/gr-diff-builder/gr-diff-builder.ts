...
function findMostlyNestedContainingBlock(
  lineNum: number,
  blocks?: SyntaxBlock[]
  ...
  const containingChildBlock = containingBlock
    ? findMostlyNestedContainingBlock(lineNum, containingBlock?.children)
    : undefined;
  ...
  private _createBlockExpansionButtons(
    ...
    if (this._renderPrefs?.use_block_expansion) {
      ...
      if (showAbove) {
        ...
        const containingBlock = findMostlyNestedContainingBlock(
          rightLineAbove,
          rightSyntaxTree
        );
