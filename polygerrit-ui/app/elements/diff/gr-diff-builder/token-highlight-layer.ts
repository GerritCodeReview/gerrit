...
export class TokenHighlightLayer implements DiffLayer {
  ...
  private updateTokenHighlight(
    ...
    if (this.currentHighlight === newHighlight &&
  ...
  getSortedLinesForSide(
    ...
    lineNumber: number): Array<number> {
    if (!token) return [];
  ...
  notifyForToken(token: string | undefined, lineNumber: number) {
    let leftLines = this.getSortedLinesForSide(
      this.tokenToLinesLeft, token, lineNumber);
    for (const line of leftLines) {
      this.notifyListeners(line, Side.LEFT);
    }
    let rightLines = this.getSortedLinesForSide(
      this.tokenToLinesRight, token, lineNumber);
