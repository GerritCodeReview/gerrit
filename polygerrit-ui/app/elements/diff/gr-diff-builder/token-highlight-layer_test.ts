...
function makeMouseEvent(type: string, xy: {x: number, y: number}, node: Element) {
  const props = {
    bubbles: true,
    cancellable: true,
    composed: true,
...
suite('token-highlight-layer', () => {
  ...
  function annotate(el: HTMLElement, side: Side = Side.LEFT, line: number = 1) {
  ...
  function createLine(text: string, line: number = 1) {
  ...
  suite('highlight', () => {
    test('highlighting hover delay', async () => {
      ...
      makeMouseEvent('mouseover',
        MockInteractions.middleOfNode(words1), words1);
