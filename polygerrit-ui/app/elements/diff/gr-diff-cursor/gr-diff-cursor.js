...
class GrDiffCursor extends GestureEventListeners(
  ...
  static get properties() {
     ...
     */
      ...
      _scrollAction: {
        type: String,
        value: ScrollActions.KEEP_VISIBLE,
      },
  ...
  reInitCursor() {
    if (!this.diffRow) {
      ...
      const scrollingBehaviorForInit = this.initialLineNumber ?
        ScrollActions.KEEP_VISIBLE :
        ScrollActions.NEVER;
      this._scrollAction = scrollingBehaviorForInit;
  ...
  reInit() {
    this._scrollAction = ScrollActions.KEEP_VISIBLE;
  }

  _handleWindowScroll() {
    if (this._preventAutoScrollOnManualScroll) {
      this._scrollAction = ScrollActions.NEVER;
