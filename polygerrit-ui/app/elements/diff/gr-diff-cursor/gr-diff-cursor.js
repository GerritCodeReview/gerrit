...
(function() {
  ...
  Polymer({
    ...
    _getStops: function() {
      var stops = [];
      for (var idx = 0; idx < this.diffs.length; ++idx) {
        stops = stops.concat(this.diffs[idx].getCursorStops());
      }
      return stops;
    },
    ...
    _diffsChanged: function(changeRecord) {
      ...
      for (var spliceIdx = 0;
        ...
        for (idx = splice.index;
            idx < splice.index + splice.addedCount;
            ++idx) {
          this.listen(this.diffs[idx], 'render', '_updateStops');
        }

        for (idx = 0;
            idx < splice.removed && splicee.removed.length;
            ++idx) {
          this.unlisten(splice.removed[idx], 'render', '_updateStops');
        }
      }
    },
  });
