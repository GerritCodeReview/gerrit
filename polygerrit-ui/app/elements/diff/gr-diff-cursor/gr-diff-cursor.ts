...
export class GrDiffCursor extends PolymerElement {
  ...
  @property({type: String, notify: true, observer: 'updateSide'})
  side = Side.RIGHT;

  @property({type: Object, notify: true, observer: 'rowChanged'})
  diffRow?: HTMLElement;
  ...
  private rowChanged(_: HTMLElement, oldRow: HTMLElement) {
    if (oldRow) {
  ...
  private updateSide(_: Side, oldSide: Side) {
    if (!this.diffRow) {
      return;
    }
    if (oldSide) {
