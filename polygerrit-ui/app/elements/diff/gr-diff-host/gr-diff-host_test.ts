...
suite('gr-diff-host tests', () => {
  ...
  suite('revert change in edit mode', () => {
    ...
    test('handleRevertDelta uses restoreFileInChangeEdit when reverting only delta of ADDED file', async () => {
      const applyFixStub = stubRestApi('applyFixSuggestion');
      ...
      assert.equal(restoreFileStub.firstCall.args[1], 'added.ts');
    });
