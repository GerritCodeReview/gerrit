...
(function() {
  ...
  Polymer({
    ...
    _processNext(state, sections) {
      ...
      if (firstUncommonSectionIndex === state.sectionIndex) {
        ...
        return {
          ...
          groups: this._groupsFromSection(
    ...
    _processCommonSections(state, sections, firstUncommonSectionIndex) {
      ...
      // In line numbers, relative to sections[start], not including offsets.
      const hiddenStart = this.context === WHOLE_FILE ? lineCount : (
          state.sectionIndex !== 0 ? this.context : 0);
      const hiddenEnd = lineCount - (
          firstUncommonSectionIndex !== sections.length &&
          this.context !== WHOLE_FILE ? this.context : 0);

      const groups = [];
      let hiddenGroupsStart;
      let hiddenGroupsEnd;
      let sectionStart = 0;
      for (const section of commonSections) {
        ...
        const [before, hidden, after] = this._groupsFromSection(
            section, offsetLeft, offsetRight,
            Math.max(hiddenStart - sectionStart, 0),
            Math.min(hiddenEnd - sectionStart, sectionLength));

        if (before.lines.length) {
          groups.push(before);
        }

        if (hidden.lines.length) {
          if (before.lines.length || hiddenStart === sectionStart) {
            hiddenGroupsStart = groups.length;
          }
          groups.push(hidden);
          if (after.lines.length || hiddenEnd === sectionEnd) {
            hiddenGroupsEnd = groups.length;
          }
        }
      ...
      if (hiddenGroupsStart != undefined && hiddenGroupsEnd != undefined) {
        const ctxLine = new GrDiffLine(GrDiffLine.Type.CONTEXT_CONTROL);
        ctxLine.contextGroups =
            groups.slice(hiddenGroupsStart, hiddenGroupsEnd);
        const ctxGroup = new GrDiffGroup(
            GrDiffGroup.Type.CONTEXT_CONTROL, [ctxLine]);
        groups.splice(
            hiddenGroupsStart, ctxLine.contextGroups.length, ctxGroup);
      }
      return {
        lineDelta: {
          left: lineCount,
    ...
    _groupsFromSection(
        section, offsetLeft, offsetRight, opt_split1, opt_split2) {
      ...
      for (const line of lines) {
        ...
        const lineOffset = line.beforeNumber ?
            line.beforeNumber - offsetLeft :
            line.afterNumber - offsetRight;
        if (opt_split1 != undefined && lineOffset < opt_split1) {
          before.push(line);
        } else if (opt_split2 != undefined && opt_split2 <= lineOffset) {
          after.push(line);
        } else hidden.push(line);
      }

      const groupedLines = [];
      if (opt_split1 != undefined) {
        groupedLines.push(before);
      }
      groupedLines.push(hidden);
      if (opt_split2 != undefined) {
        groupedLines.push(after);
      }
      return groupedLines
          .map(lines => {
