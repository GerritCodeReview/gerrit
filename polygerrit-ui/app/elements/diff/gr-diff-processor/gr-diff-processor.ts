...
export class GrDiffProcessor extends GestureEventListeners(
  ...
  process(
    ...
    this.groups = [];
    if (portedThreadsWithoutRange.left || portedThreadsWithoutRange.right)
      this.push('groups', this._makePortedCommentsWithoutRange());
  ...
  _makePortedCommentsWithoutRange() {
    const line = new GrDiffLine(GrDiffLineType.BOTH);
    line.beforeNumber = 'LOST';
    line.afterNumber = 'LOST';
    const group = new GrDiffGroup(GrDiffGroupType.BOTH, [line]);
    group.portedCommentsWithoutRange = true;
    return group;
  }
