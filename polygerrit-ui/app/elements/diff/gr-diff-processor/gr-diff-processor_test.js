...
suite('gr-diff-processor tests', () => {
  ...
  const loremIpsum =
      ...
      'fugit assum per.';
  const hasPortedThreadsWithoutRange = {left: false, right: false};
  ...
  suite('not logged in', () => {
    ...
    test('process loaded content', () => {
      ...
      return element.process(content, false, hasPortedThreadsWithoutRange).then(
          () => {
    ...
    suite('context groups', () => {
      test('at the beginning, larger than context', () => {
        ...
        return element.process(content, false, hasPortedThreadsWithoutRange
        ).then(
            () => {
              const groups = element.groups;
      ...
      test('at the beginning with skip chunks', async () => {
        ...
        await element.process(content, false, hasPortedThreadsWithoutRange);
    ...
    test('scrolling pauses rendering', () => {
      ...
      element.process(content, false, hasPortedThreadsWithoutRange);
    ...
    test('image diffs', () => {
      ...
      const content = _.times(200, _.constant(contentRow));
      sinon.stub(element, 'async');
      element.process(content, true, hasPortedThreadsWithoutRange);
      assert.equal(element.groups.length, 1);
