...
class GrDiffView extends KeyboardShortcutMixin(
  ...
  _processPortedComments(comments) {
    ...
    const threads = createThreads(allComments).filter(thread =>
      thread.comments.some(comment => portedComments.some(portedComment =>
        portedComment.id === comment.id))
    );

    // assign range to threads based on ported comments
