...
(function() {
  ...
  Polymer({
    ...
    _computeDownloadDropdownLinks(
        project, changeNum, patchRange, path, diff, editMode) {
        ...
        links.push(
            {
              url: this._computeDownloadFileLink(
                  project, changeNum, patchRange, leftPath, editMode, true),
              name: 'Left Content',
            }
        );
      }
      ...
      if (diff && diff.meta_b) {
        links.push(
            {
              url: this._computeDownloadFileLink(
                  project, changeNum, patchRange, path, editMode, false),
              name: 'Right Content',
            }
        );
      }
    ...
    _computeDownloadFileLink(
        project, changeNum, patchRange, path, editMode, opt_isBase) {
      let patchNum = patchRange.patchNum;
      if (opt_isBase && patchRange.basePatchNum !== 'PARENT' || editMode) {
      ...
      if (opt_isBase && patchRange.basePatchNum &&
          patchRange.basePatchNum === 'PARENT') {
        url += '?parent=1';
      }
