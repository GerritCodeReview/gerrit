...
class GrDiffView extends KeyboardShortcutMixin(
  ...
  _createThreads(comments) {
    ...
    const sortedComments = _sortComments(comments);
    const threads = [];
  ...
  _processPortedComments(comments) {
    ...
    comments[this._path].forEach(c => {
      c.ported = true;
      c.path = this._path;
      const thread = threads.find(t => t.comments.some(comment => comment.id === c.id));
      if (c.side === 'PARENT') {
        ...
        if (this._patchRange.basePatchNum === 'PARENT' &&
          ...
          // comparing Base vs Y
          portedComments.left.push(this.convertToFileComment(c));
