...
class GrDiffView extends mixinBehaviors( [
  ...
  _isFileUnchanged(diff) {
    ...
    let fileUnchanged = true;
    diff.content.forEach(content => {
      if ((content.a && !content.common) ||
  ...
  _paramsChanged(value) {
    ...
    return Promise.all(promises)
        .then(r => {
          ...
          if (value.commentLink || value.fileLink) {
            const latestPatchNum =
              this.computeLatestPatchNum(this._allPatchSets);
            if (!this.patchNumEquals(value.patchNum, latestPatchNum)) {
              this._patchRange = {
                patchNum: latestPatchNum,
                basePatchNum: value.patchNum,
              };
            }
          }
