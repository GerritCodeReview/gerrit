  ...
  private getSidebarSide(): 'left' | 'right' {
    ...
    for (const info of details) {
      if (info.moduleName) {
        ...
        if (cls) {
          // @ts-expect-error: static property on plugin component
          const staticSide = cls.sidebarPosition;
          const protoSide = (cls.prototype as {sidebarPosition?: string})
            ?.sidebarPosition;
          if (staticSide === 'right' || protoSide === 'right') {
            return 'right';
          }
        }
      }
