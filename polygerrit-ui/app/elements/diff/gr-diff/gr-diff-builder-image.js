...
(function(window, GrDiffBuilder) {
  ...
  GrDiffBuilderImage.prototype._emitImageLabels = function(section) {
    ...
    label.innerHTML = this._getImageLabel(this._baseImage);
    ...
    label.innerHTML = this._getImageLabel(this._revisionImage);
    td.appendChild(label);
    tr.appendChild(td);
  ...
  GrDiffBuilderImage.prototype._getImageLabel = function(image) {
    if (image) {
      return image._width + '&times;' + image._height + ' ' + image.type;
    } else {
      return 'No image';
    }
  };
