...
function _splitGroupInTwo(group: GrDiffGroup, leftSplit: number, rightSplit: number) {
  ...
  if (group.skip) {
    ...
    if (before.length) {
      beforeSplit = before.length === group.lines.length
        ? group
        : group.cloneWithLines(before);
    }
    if (after.length) {
      afterSplit = after.length === group.lines.length
        ? group
...
function _splitCommonGroups(
  ...
  for (const group of groups) {
    ...
    if (isCompletelyBefore) {
      ...
      const {beforeSplit, afterSplit} = _splitGroupInTwo(group, leftSplit, rightSplit);
      if (beforeSplit) {
