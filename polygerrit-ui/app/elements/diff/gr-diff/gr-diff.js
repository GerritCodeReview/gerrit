...
(function() {
  ...
  Polymer({
    ...
    _lastChunkForSide(diff, leftSide) {
      if (!diff.content.length) { return false; }
    ...
    /**
     * Does the specified side of the diff have a trailing newline.
     * @param {!Object} diff
     * @param {boolean} leftSide true if checking the base of the diff,
     *     false if testing the revision.
     * @return {boolean|null} true if the side has a trailing newline, false
     *     if it doesn't and null if the anser cannot be determined (for
