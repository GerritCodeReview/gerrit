...
import '../gr-ranged-comment-layer/gr-ranged-comment-chip';
...
import {GrRangedCommentChip} from '../gr-ranged-comment-layer/gr-ranged-comment-chip';
  ...
  _handleRenderContent() {
    ...
    this._incrementalNodeObserver = (dom(
      ...
      for (const threadEl of removedThreadEls) {
        const range = getRange(threadEl);
        const slotAtt = threadEl.getAttribute('slot');
        const slot = this.shadowRoot?.querySelector(`slot[name="${slotAtt}"`);
        if (!slot) continue;
        (slot as HTMLSlotElement)
          .assignedElements()
          .filter(
            element =>
              element.tagName === 'GR-RANGED-COMMENT-CHIP' &&
              rangesEqual((element as GrRangedCommentChip).range, range)
          )
