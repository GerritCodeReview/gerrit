...
suite('gr-diff tests', () => {
  ...
  suite('_get{PatchNum|IsParentComment}ByLineAndContent', () => {
    ...
    suite('_getPatchNumByLineAndContent', () => {
      test('right side', () => {
        ...
        assert.equal(
            element._getPatchNumByLineAndContent(lineEl, contentEl),
            4
        );
      });
  ...
  suite('not logged in', () => {
    ...
    test('view does not start with displayLine classList', () => {
      assert.isFalse(
          element.shadowRoot
              ...
              .classList.contains('displayLine')
      );
    });
    ...
    test('_handleTap content', done => {
      ...
      sinon
          .stub(element.$.diffBuilder, 'getLineElByChild')
          .callsFake(() => lineEl);
    ...
    test('adds .hiddenscroll', () => {
      ...
      element.displayLine = true;
      assert.include(
          element.shadowRoot.querySelector('.diffContainer').className,
          'hiddenscroll'
      );
    });
  });
  ...
  suite('logged in', () => {
    ...
    test('adds long range comment chip', done => {
      const content = [{
      ...
      element.addEventListener('create-range-comment', commentSpy);
      const createRangeCommentEvent = new CustomEvent('create-range-comment', {
        detail: {
          side: 'right',
          range: {
            start_line: 0,
      ...
      flush(() => {
        element.dispatchEvent(createRangeCommentEvent);
        assert.isTrue(commentSpy.called);
        assert.isNotEmpty(
            element.shadowRoot.querySelectorAll('gr-ranged-comment-chip')
        );
        done();
      });
      // sinon.stub(element, '_createComment');
  ...
  suite('trailing newline warnings', () => {
    ...
    test('_prefsEqual', () => {
      ...
      assert.isFalse(
          element._prefsEqual({x: 1, y: 'abc'}, {x: 1, y: 'abcd'})
      );
  ...
  suite('key locations', () => {
    ...
    setup(() => {
      ...
      element.prefs = {};
      renderStub = sinon
          .stub(element.$.diffBuilder, 'render')
          .returns(new Promise(() => {}));
    });
