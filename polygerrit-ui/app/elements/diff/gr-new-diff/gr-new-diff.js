  ...
  Polymer({
    ...
    _getDiffCommentsAndDrafts: function() {
      ...
      return Promise.all(promises).then(function(results) {
        ...
        var baseDrafts = results[1].baseComments.map(markAsDraft);
        var drafts = results[1].comments.map(markAsDraft);
        console.table(drafts)
        return Promise.resolve({
          left: results[0].baseComments.concat(baseDrafts),
          right: results[0].comments.concat(drafts),
        });
      });
    },
