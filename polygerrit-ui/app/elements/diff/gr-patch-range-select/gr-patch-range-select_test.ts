...
import {createEditRevision, createRevision} from '../../../test/test-data-generators';
import {PatchSet} from '../../../utils/patch-set-util';
import {DropdownItem, GrDropdownList} from '../../shared/gr-dropdown-list/gr-dropdown-list';
...
suite('gr-patch-range-select tests', () => {
  ...
  test('_computePatchDropdownContent called when basePatchNum updates', () => {
    ...
    // Should be recomputed for each available patch
    const baseDropDownStub = sinon.stub(element, '_computePatchDropdownContent');
  ...
  test('_computePatchSetCommentsString', () => {
    ...
    assert.equal(
      element._computePatchSetCommentsString(element.changeComments, 1 as PatchSetNum),
  ...
  test('patch-range-change fires', () => {
    ...
    queryAndAssert<GrDropdownList>(element, '#basePatchDropdown')._handleValueChange('2', [{text: '', value: '2'}]);
    ...
    queryAndAssert<GrDropdownList>(element, '#patchNumDropdown')._handleValueChange('edit', [{text: '', value: 'edit'}]);
    assert.deepEqual(handler.lastCall.args[0].detail, {
      basePatchNum: 1,
      patchNum: 'edit',
    });
