...
export class GrSyntaxLayerWorker implements DiffLayer {
  ...
  annotate(el: HTMLElement, _: HTMLElement, line: GrDiffLine) {
    console.log(`syntax layer worker annotate0 ${this.enabled}`);
    ...
    console.log(
      `syntax layer worker annotate1 ${side} ${line.beforeNumber} ${line.afterNumber} ${highlightedLine}`
    );
    if (!highlightedLine) return;

    const ranges = this._rangesFromString(highlightedLine);
    console.log(
      `syntax layer worker annotate2 ${side} ${line.beforeNumber} ${line.afterNumber} ${ranges.length}`
    );
  ...
  process() {
    ...
    const reviWorker = createHljsWorker(revi).then(h => {
      this.revisionLines = h;
      for (const line of h.slice(0, 50)) {
        console.log(line);
      }
    });
  ...
  notify() {
    if (this.baseLines?.length > 0) {
      this.notifyRange(1, this.baseLines?.length - 1, Side.LEFT);
    }
    if (this.revisionLines?.length > 0) {
      this.notifyRange(1, this.revisionLines?.length - 1, Side.RIGHT);
    }
  }

  notifyRange(start: number, end: number, side: Side) {
    for (const listener of this.listeners) {
      console.log(`syntax layer worker notify range ${start} ${end} ${side}`);
