...
export class GrEditControls extends GestureEventListeners(
  ...
  @property({type: Array})
  hiddenActions: unknown = [GrEditConstants.Actions.RESTORE.id];

  @property({type: Array})
  _actions: unknown = Object.values(GrEditConstants.Actions);
  ...
  _getDialogFromEvent(e: Event): GrDialog | undefined {
    return (dom(e) as EventApi).path.find(element => {
      ...
      if (!htmlElement.classList) return false;

      return htmlElement.classList.contains('dialog');
