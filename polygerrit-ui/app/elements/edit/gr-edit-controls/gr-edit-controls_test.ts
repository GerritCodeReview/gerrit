...
suite('gr-edit-controls tests', () => {
  ...
  suite('edit button CUJ', () => {
    ...
    test('open', async () => {
      ...
      assert.isTrue(queryAndAssert<GrDialog>(element, '#openDialog').disabled);
      ...
      assert.isFalse(queryAndAssert<GrDialog>(element, '#openDialog').disabled);
      MockInteractions.tap(
        queryAndAssert(
          queryAndAssert<GrDialog>(element, '#openDialog'),
          'gr-button[primary]'
        )
      );
    ...
    test('cancel', () => {
      ...
      return showDialogSpy.lastCall.returnValue.then(async () => {
        assert.isTrue(
          queryAndAssert<GrDialog>(element, '#openDialog').disabled
        );
        openAutoComplete.noDebounce = true;
        openAutoComplete.text = 'src/test.cpp';
        await element.updateComplete;
  ...
  suite('delete button CUJ', () => {
    ...
    setup(() => {
      ...
      const deleteDialog = queryAndAssert<GrDialog>(element, '#deleteDialog');
      queryAndAssert<GrAutocomplete>(deleteDialog, 'gr-autocomplete');
    ...
    test('delete', async () => {
      ...
      await showDialogSpy.lastCall.returnValue;
      assert.isTrue(
        queryAndAssert<GrDialog>(element, '#deleteDialog').disabled
      );
      assert.isFalse(queryStub.called);
      // Setup _focused manually - in headless mode Chrome sometimes don't
      // setup focus. flush and/or flushAsynchronousOperations don't help
      ...
      MockInteractions.tap(
        queryAndAssert(
          queryAndAssert<GrDialog>(element, '#deleteDialog'),
          'gr-button[primary]'
        )
      );
      await element.updateComplete;
    ...
    test('cancel', () => {
      ...
      return showDialogSpy.lastCall.returnValue.then(async () => {
        ...
        queryAndAssert<GrDialog>(element, '#deleteDialog').querySelector(
          'gr-autocomplete'
        )!.text = 'src/test.cpp';
        await element.updateComplete;
        assert.isFalse(
  ...
  suite('rename button CUJ', () => {
    ...
    setup(() => {
      ...
      const renameDialog = queryAndAssert<GrDialog>(element, '#renameDialog');
    ...
    test('rename', async () => {
      ...
      await showDialogSpy.lastCall.returnValue;
      assert.isTrue(
        queryAndAssert<GrDialog>(element, '#renameDialog').disabled
      );
      assert.isFalse(queryStub.called);
      // Setup _focused manually - in headless mode Chrome sometimes don't
      // setup focus. flush and/or flushAsynchronousOperations don't help
      ...
      queryAndAssert<IronInputElement>(element, '#newPathIronInput').bindValue =
        'src/test.newPath';
      await element.updateComplete;
      ...
      MockInteractions.tap(
        queryAndAssert(
          queryAndAssert<GrDialog>(element, '#renameDialog'),
          'gr-button[primary]'
        )
      );
      await element.updateComplete;
    ...
    test('cancel', () => {
      ...
      return showDialogSpy.lastCall.returnValue.then(async () => {
        ...
        const renameDialog = queryAndAssert<GrDialog>(element, '#renameDialog');
        queryAndAssert<GrAutocomplete>(renameDialog, 'gr-autocomplete').text =
          'src/test.cpp';
  ...
  suite('restore button CUJ', () => {
    ...
    test('restore', () => {
      ...
      return showDialogSpy.lastCall.returnValue.then(async () => {
        MockInteractions.tap(
          queryAndAssert(
            queryAndAssert<GrDialog>(element, '#restoreDialog'),
            'gr-button[primary]'
          )
        );
        await element.updateComplete;
  ...
  test('openOpenDialog', async () => {
    ...
    queryAndAssert<GrDialog>(element, '#openDialog');
    assert.isFalse(
      queryAndAssert<GrDialog>(element, '#openDialog').hasAttribute('hidden')
    );
    const openDialog = queryAndAssert<GrDialog>(element, '#openDialog');
    assert.equal(
      queryAndAssert<GrAutocomplete>(openDialog, 'gr-autocomplete').text,
      'test/path.cpp'
    );
  });
  ...
  test('getDialogFromEvent', async () => {
    ...
    MockInteractions.tap(queryAndAssert<GrDialog>(element, '#openDialog'));
    await element.updateComplete;
    assert.equal(spy.lastCall.returnValue!.id, 'openDialog');

    MockInteractions.tap(queryAndAssert<GrDialog>(element, '#deleteDialog'));
    await element.updateComplete;
