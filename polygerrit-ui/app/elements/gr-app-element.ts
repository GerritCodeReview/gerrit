...
export class GrAppElement extends LitElement {
  ...
  private renderChangeView() {
    // Because cache() is aggresive if we want to invalidate it
    // we have to first load the html without the cache then call cache().
    // noCache is only set then unset when RECREATE_CHANGE_VIEW or
    // RECREATE_DIFF_VIEW is triggered.
    // This is similar to dom-if restamp behaviour.
    if (this.noCache) {
      if (!this.showChangeView) return nothing;
      return this.changeViewTemplate();
    }
    return cache(!this.showChangeView ? nothing : this.changeViewTemplate());
  }
  ...
  private renderDiffView() {
    ...
    // This is similar to dom-if restamp behaviour.
    if (this.noCache) {
      if (!this.showDiffView) return nothing;
      return this.diffViewTemplate();
    }
    return cache(!this.showDiffView ? nothing : this.diffViewTemplate());
  }
  ...
  private handleRecreateView() {
    // This brings dom-if unsetting/setting restamp behaviour to lit.
    // By first loading the html uncached then loading it cached
    // we can trigger a recreation of the view when needed.
    // If the change number has not changed we get a cached view
    // and the dom is not recreated.
    setTimeout(() => {
      this.noCache = true;
      setTimeout(() => {
        this.noCache = false;
