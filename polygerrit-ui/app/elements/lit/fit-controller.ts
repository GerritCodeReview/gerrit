...
export class FitController implements ReactiveController {
  ...
  private position() {
    ...
    const hostMinWidth = Number(hostStyles.minWidth) || 0;
    const hostMinHeight = Number(hostStyles.minHeight) || 0;
    ...
    leftPosition = Math.max(
      ...
      Math.min(leftPosition, right - hostMinWidth)
    );
    topPosition = Math.max(
      windowRect.top + hostMargin.top,
      Math.min(topPosition, bottom - hostMinHeight)
    );

    // Use right/bottom to set maxWidth/maxHeight, and respect
    // minWidth/minHeight.
    const maxWidth = Math.max(right - leftPosition, hostMinWidth);
    const maxHeight = Math.max(bottom - topPosition, hostMinHeight);

    this.host.style.maxWidth = maxWidth.toString() + 'px';
    this.host.style.maxHeight = maxHeight.toString() + 'px';

    // Remove the offset caused by any stacking context.
    leftPosition = leftPosition - hostRect.left;
    topPosition = topPosition - hostRect.top;
