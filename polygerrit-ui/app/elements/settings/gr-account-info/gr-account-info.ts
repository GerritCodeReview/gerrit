...
export class GrAccountInfo extends LitElement {
  ...
  static override styles = [
    ...
    css`
      ...
      section.visibility ul {
        list-style-type: disc;
        list-style-position: inside;
      }
      section.visibility a {
        cursor: pointer;
      }
    `,
  ];
  ...
  override render() {
    ...
    return html`<div class="gr-form-styles">
      ...
      <section class="visibility">
        ...
        <div class="value">${this.renderVisibilityDetails()}</div>
      </section>
      <section>
        <span class="title">
          <gr-tooltip-content
  ...
  private renderVisibilityDetails() {
    if (!this.serverConfig) {
      return nothing;
    }
    this.serverConfig.accounts.visibility = AccountsVisibility.SAME_GROUP;
    switch (this.serverConfig.accounts.visibility) {
      case AccountsVisibility.ALL:
        return 'Your account is visible to everyone because this host makes all users visibile to all other users, including anonymous users.';
      case AccountsVisibility.VISIBLE_GROUP:
        return html`
          This host bases account visibility on group visibility.
          ${this.renderVisibleGroupsList()}
        `;
      case AccountsVisibility.SAME_GROUP:
        return 'Your account is only visible to other members of your groups.';
      case AccountsVisibility.NONE:
        return 'Your account is not visible to others.';
      default:
  ...
  private renderVisibleGroupsList() {
    ...
    if (visibleGroupNames.length === 0) {
      return 'Your account is only visible to other members of your groups because you are not a member of any publicly visible group.';
    }
    ...
    return html`
      Your account is visible to everyone because you are a member of
