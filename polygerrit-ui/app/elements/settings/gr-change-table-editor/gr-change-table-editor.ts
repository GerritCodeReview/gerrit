...
export class GrChangeTableEditor extends LitElement {
  ...
  private handleTargetClick(e: Event) {
    ...
    if (checked) {
      this.localChangeTableColumns = [...this.localChangeTableColumns, column];
    } else {
      this.localChangeTableColumns = this.localChangeTableColumns.filter(
        c => c !== column
      );
    }
  }
