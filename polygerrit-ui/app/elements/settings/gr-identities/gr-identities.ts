...
export class GrIdentities extends LitElement {
  ...
  @state() _identities: AccountExternalIdInfo[] = [];

  @state() _idName?: string;

  @property({type: Object}) serverConfig?: ServerInfo;

  @state({}) _showLinkAnotherIdentity?: boolean;
  ...
  override willUpdate(changedProperties: PropertyValues) {
    if (changedProperties.has('serverConfig')) {
      this._showLinkAnotherIdentity = this.computeShowLinkAnotherIdentity();
    }
  }

  // private but used in test
  getIdentities() {
    return this._identities.filter(
  ...
  private handleDeleteItem(e: Event) {
    const target = e.target;
    if (!(target instanceof Element)) return;
    const indexStr = target.getAttribute('data-index');
    if (indexStr === null) return;
    const index = Number(indexStr);
    const name = this.getIdentities()[index].identity;
    if (!name) {
      return;
    }
    this._idName = name;
