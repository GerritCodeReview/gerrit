...
export class GrSettingsView extends ChangeTableMixin(PolymerElement) {
  ...
  _handleDateFormat(e: Event) {
    ...
    if (!el) return;
    this.set(
      '_localPrefs.date_format',
      el.value as DateFormat
    );
  }
  ...
  _handleTimeFormat(e: Event) {
    ...
    if (!el) return;
    this.set(
      '_localPrefs.time_format',
      el.value as TimeFormat
    );
  }
  ...
  _handleEmailFormat(e: Event) {
    ...
    if (!el) return;
    this.set(
      '_localPrefs.email_format',
      el.value as EmailFormat
    );
  }
   ...
   */
  _convertToString(key?: DateFormat | DefaultBase | DiffViewMode | EmailFormat | EmailStrategy | TimeFormat | number) {
    if (!key) return '';
    return String(key);
  }
}
