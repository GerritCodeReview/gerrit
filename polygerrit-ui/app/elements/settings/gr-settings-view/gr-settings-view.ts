...
export class GrSettingsView extends ChangeTableMixin(PolymerElement) {
  ...
  _handleChangesPerPage() {
    this.set('_localPrefs.changes_per_page', Number(this.$.changesPerPageSelect.value));
  }
  ...
  _handleDefaultBaseForMerges() {
    this.set('_localPrefs.default_base_for_merges', this.$.defaultBaseForMergesSelect.value);
  }

  _handleDiffView() {
    this.set('_localPrefs.diff_view', this.$.diffViewSelect.value as DiffViewMode);
  }
