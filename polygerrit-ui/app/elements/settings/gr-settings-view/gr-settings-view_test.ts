...
suite('gr-settings-view tests', () => {
  ...
  test('add email validation', async () => {
    ...
    element._addingEmail = true;
    await element.updateComplete;
    assert.isFalse(element._computeAddEmailButtonEnabled());
    element._newEmail = 'vaguely@valid.email';
    element._addingEmail = true;
    await element.updateComplete;
    assert.isFalse(element._computeAddEmailButtonEnabled());
    element._newEmail = 'vaguely@valid.email';
    element._addingEmail = false;
    await element.updateComplete;
    assert.isTrue(element._computeAddEmailButtonEnabled());
  });
  ...
  test('_showHttpAuth', async () => {
    ...
    element._serverConfig = serverConfig;
    await element.updateComplete;
    assert.isTrue(element._showHttpAuth());

    element._serverConfig.auth.git_basic_auth_policy = 'HTTP_LDAP';
    await element.updateComplete;
    assert.isTrue(element._showHttpAuth());

    element._serverConfig.auth.git_basic_auth_policy = 'LDAP';
    await element.updateComplete;
    assert.isFalse(element._showHttpAuth());

    element._serverConfig.auth.git_basic_auth_policy = 'OAUTH';
    await element.updateComplete;
    assert.isFalse(element._showHttpAuth());

    element._serverConfig = undefined;
    await element.updateComplete;
    assert.isFalse(element._showHttpAuth());
  });
