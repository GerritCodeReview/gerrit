...
suite('gr-watched-projects-editor tests', () => {
  ...
  test('renders', () => {
    let rows = queryAndAssert(element, 'table').querySelectorAll('tbody tr');
  ...
  test('_handleRemoveProject', () => {
    ...
    const button = queryAndAssert(element, 'table tbody tr:nth-child(2) gr-button');
    MockInteractions.tap(button);

    flush();

    let rows = queryAndAssert(element, 'table tbody').querySelectorAll('tr');
