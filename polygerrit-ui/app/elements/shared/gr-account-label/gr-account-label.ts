...
export class GrAccountLabel extends LitElement {
  ...
  static override get styles() {
    return [
      css`
        ...
        :host([clickable]) .clickable:hover .name {
  ...
  private renderLink(span: TemplateResult) {
    if (!this.clickable) return span;
    const url = this._computeOwnerLink();
    if (!url) return span;
    return html` <a class="clickable" href="${url}" tabindex="-1">
      ${span}
    </a>`;
  }
