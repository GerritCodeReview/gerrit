  ...
  Polymer({
    ...
    _getSuggestionsProvider() {
      return Polymer.dom(this).getEffectiveChildNodes()
          .filter(node => this._isSuggestionsProvider(node));
    },

    _getSuggestions(input) {
      // Polymer2: querySelectorAll returns NodeList instead of Array.
      const _suggestors = this._getSuggestionsProvider();
      const suggests = _suggestors
          .map(suggestor => suggestor.getSuggestions(input));
      return Promise.all(suggests)
          .then(result => {
            return result.reduce((flat, item) => flat.concat(item), []);
          });
    },

    _handleAdd(e) {
      this._addAccount(e.detail.value);
    },

    _addAccount(item) { // TODO: bad naming
