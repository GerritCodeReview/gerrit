...
export class GrAccountList extends LitElement {
  ...
  computeRemovable(account: AccountInput) {
    ...
    if (this.removableValues) {
      const removable = this.removableValues.some(value => 
        getUserId(value) === getUserId(account)
      ) 
      return removable || isAccountNewlyAdded(account, this.reviewerState, this.change);
    }
    return true;
  }
  ...
  removeAccount(toRemove?: AccountInput) {
    ...
    const index = this.accounts.findIndex(account => 
      getUserId(toRemove) === getUserId(account)
    );
    if (index !== -1) {
