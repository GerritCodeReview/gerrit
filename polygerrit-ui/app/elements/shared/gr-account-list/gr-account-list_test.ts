...
suite('gr-account-list tests', () => {
  ...
  test('submitEntryText', async () => {
    ...
    getTextStub.onFirstCall().returns(Promise.resolve(''));
    getTextStub.onSecondCall().returns(Promise.resolve('test'));
    getTextStub.onThirdCall().returns(Promise.resolve('test@test'));

    // When entry is empty, return true.
    const clearStub = sinon.stub(element.$.entry, 'clear');
    assert.isTrue(await element.submitEntryText());
    assert.isFalse(clearStub.called);

    // When entry is invalid, return false.
    assert.isFalse(await element.submitEntryText());
    assert.isFalse(clearStub.called);
