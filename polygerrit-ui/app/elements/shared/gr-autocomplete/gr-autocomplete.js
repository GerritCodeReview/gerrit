...
Polymer({
  _template: Polymer.html`
    ...
    <paper-input no-label-float="" id="input" class\$="[[_computeClass(borderless)]]" is="iron-input" disabled\$="[[disabled]]" value="{{text}}" placeholder="[[placeholder]]" on-keydown="_handleKeydown" on-focus="_onInputFocus" on-blur="_onInputBlur" autocomplete="off">
      <!-- slot is for future use (2.x) while prefix attribute is for 1.x
        (current) -->
      <iron-icon icon="gr-icons:search" slot="prefix" prefix="" class\$="searchIcon [[_computeShowSearchIconClass(showSearchIcon)]]">
      </iron-icon>
    </paper-input>
