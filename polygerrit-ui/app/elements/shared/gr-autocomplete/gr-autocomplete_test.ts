...
suite('gr-autocomplete tests', () => {
  ...
  test('renders', () => {
    expect(element).shadowDom.to.equal(/* HTML */ `
      ...
      >
        <div slot="prefix">
          <iron-icon class="searchIcon" icon="gr-icons:search"> </iron-icon>
        </div>
        <div slot="suffix">
  ...
  test('renders with suggestions', async () => {
    ...
    expect(element).shadowDom.to.equal(/* HTML */ `
      ...
      <gr-autocomplete-dropdown
        ...
        id="suggestions"
        role="listbox"
        style="position: fixed; box-sizing: border-box; left: 8px; top: 39px; max-width: 39.5469px; max-height: 90px;"
        vertical-align="top"
      >
      </gr-autocomplete-dropdown>
    `);
