  ...
  Polymer({
    ...
    _handleEnterByKey(e) {
      if (!this._hideAutocomplete) {
        e.preventDefault();
        e.stopPropagation();
        this.text = this._getText(this.$.emojiSuggestions.getCursorTarget()
            .dataset.value);
        this._resetEmojiDropdown();
      }
    },
    ...
    _updateSelectorPosition() {
      ...
      const top = caratPosition.top + fontSize + EXTRA_HEIGHT + 'px';
    ...
    _determineSuggestions(emojiText) {
      ...
      const matches = this._allSuggestions.filter(suggestion => {
