...
import {AccountDetailInfo, AccountInfo, ServerInfo} from '../../../types/common';
...
export class GrAvatarStack extends LitElement {
  ...
  override updated(changedProperties: PropertyValues) {
    if (changedProperties.has('accounts')) {
      if (this.forceFetch && this.accounts.length > 0 && isBareAccount(this.accounts[0])) {
        Promise.all(this.accounts.map(account => this.restApiService.getAccountFor(account._account_id!))).then((accounts) => {
