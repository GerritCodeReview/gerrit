...
export class GrCommentThread extends KeyboardShortcutMixin(
  ...
  ready() {
    super.ready();
    if (!this.changeNum) {
      throw new Error('required changeNum property undefined');
    }
    if (!this.patchNum) {
      throw new Error('required patchNum property undefined');
    }
    if (!this.path) {
      throw new Error('required path property undefined');
    }
    if (!this.projectName) {
      throw new Error('required projectName property undefined');
    }
  }
  ...
  _sortedComments(comments: Comment[]) {
    return comments.slice().sort((c1, c2) => {
      ...
      if (c2.__draft && !c1.__draft) {
        return -1;
      }
      if (c1.__draft && !c2.__draft) {
        return 1;
      }
      if (dateCompare === 0 && (!c1.id || !c1.id.localeCompare)) {
        return 0;
      }
      if (!c1.id) throw new Error('A published comment is missing the id.');
      if (!c2.id) throw new Error('A published comment is missing the id.');
  ...
  _indexOf(comment: Comment, arr: Comment[]) {
    for (let i = 0; i < arr.length; i++) {
      const c = arr[i];
      if (
        (c.__draftID !== null && c.__draftID === comment.__draftID) ||
