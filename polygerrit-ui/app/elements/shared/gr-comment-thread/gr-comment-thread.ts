...
export class GrCommentThread extends LitElement {
  ...
  private getUrlForFileComment() {
    if (this.isPatchsetLevel()) return undefined;
    if (!this.repoName || !this.changeNum) return undefined;
    const comment = this.getFirstComment();
    if (!comment) return undefined;
    ...
    if (typeof this.thread?.line === 'number') {
      ...
      line = comment.line;
    } else if (comment.range) {
      line =
        comment.range.end_line < comment.range.start_line
