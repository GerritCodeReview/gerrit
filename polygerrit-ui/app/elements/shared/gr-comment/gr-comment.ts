...
export class GrComment extends LitElement {
  ...
  private rawSave(options: {showToast: boolean}) {
    ...
    const prefix = isNew(this.comment) ? 'CreateDraftComment' : 'UpdateDraftComment';
    ...
    const fixTimer = this.reporting.getTimer(`${prefix} - isFixSuggestionChanged`);
    if (this.isFixSuggestionChanged()) {
      draft.fix_suggestions = this.getFixSuggestions();
    }
    fixTimer.end();
    
    const hintTimer = this.reporting.getTimer(`${prefix} - reportHintInteractionSaved`);
