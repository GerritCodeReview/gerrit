...
export class GrComment extends LitElement {
    ...
    return html`
      ...
      >
        <div class="headerLeft">
          ${this.renderAiIcon()} ${this.renderAuthor()}
          ${this.renderPortedCommentMessage()} ${this.renderDraftLabel()}
        </div>
  ...
  private isAiGenerated() {
    return isAiGenerated(this.comment);
  }

  private renderAiIcon() {
    if (!this.isAiGenerated()) return nothing;
    return html` <gr-icon icon="ai" class="gemini-sparkle"></gr-icon> `;
  }
