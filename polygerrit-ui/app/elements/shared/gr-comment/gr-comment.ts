...
export class GrComment extends LitElement {
  ...
  // TODO(milutin): should be shown also in non-draft changes
  ...
  private renderSuggestEditButton() {
    ...
    if (!this.comment || hasUserSuggestion(this.comment)) return nothing;
    // TODO(milutin): show only for reviewer (non-author)
  ...
  async createSuggestEdit() {
    ...
    const end = '\n```';
    // TODO(milutin): This doesn't work with commit message
