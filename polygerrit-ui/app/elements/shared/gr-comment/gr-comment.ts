...
export class GrComment extends LitElement {
  ...
  private renderCollapsedContent() {
    console.log(`renderCollapsedContent ${this.collapsed}`);
  ...
  firstWillUpdate() {
    ...
    console.log(
      `gr-comment firstWillUpdate ${isDraftOrUnsaved(this.comment)} ${
        this.comment!.id
  ...
  getRandomNum(minIncluded: number, maxExcluded: number) {
    return Math.floor(
      Math.random() * (maxExcluded - minIncluded) + minIncluded
    );
  }
