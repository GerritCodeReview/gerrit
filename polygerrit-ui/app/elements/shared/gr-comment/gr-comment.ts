...
export class GrComment extends LitElement {
  ...
  private isAiGenerated() {
    if (!this.comment?.tag) return false;
    return (
      this.comment.tag.includes('ai_code_review') ||
      this.comment.tag.includes('ai_review') ||
      this.comment.tag.startsWith('autogenerated:ai')
    );
  }
