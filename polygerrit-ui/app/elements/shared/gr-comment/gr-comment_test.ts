...
suite('gr-comment tests', () => {
  ...
  suite('gr-comment draft tests', () => {
    ...
    test('author name fallback to email', done => {
      ...
      element.collapsed = false;
      flush(() => {
        const authorName = queryAndAssert(queryAndAssert(element, 'gr-account-label')
          , 'span.name') as HTMLSpanElement;
    ...
    test('storage is cleared only after save success', () => {
      ...
      stubRestApi('getResponseObject').returns(Promise.resolve({new Response()}));
    ...
    test('resolved comment state indicated by checkbox', () => {
      ...
      assert.isTrue((queryAndAssert(element, '.resolve input') as HTMLInputElement).checked);
