...
suite('gr-comment tests', () => {
  ...
  suite('gr-comment draft tests', () => {
    ...
    test('discard', async () => {
      ...
      const stub = sinon
        .stub(element.commentsModel(), 'discardDraft')
    ...
    test('resolved comment state indicated by checkbox', async () => {
      const saveStub = sinon.stub(element.commentsModel(), 'saveDraft');
      element.comment = {
        ...createComment(),
        __draft: true,
        unresolved: false,
    ...
    test('saving empty text calls discard()', async () => {
      const saveStub = sinon.stub(element.commentsModel(), 'saveDraft');
      const discardStub = sinon.stub(element.commentsModel(), 'discardDraft');
      element.comment = createDraft();
      element.editing = true;
      await element.updateComplete;
