...
suite('gr-comment tests', () => {
    ...
    test('delete comment', done => {
      ...
      flush(() => {
        element.confirmDeleteOverlay.open.lastCall.returnValue.then(() => {
          const dialog = window.confirmDeleteOverlay.querySelector(
  ...
  suite('gr-comment draft tests', () => {
    ...
    test('button visibility states', () => {
      ...
      assert.isFalse((queryAndAssert(element, '#deleteBtn') as HTMLElement).hidden);
      ...
      flush();
      assert.isTrue(
        queryAndAssert(element, '.robotRun.link').textContent ===
          'Run Details'
      );
      ...
      flush();
      assert.notEqual(
        getComputedStyle(queryAndAssert(element, '.robotRun.link'))
          .display,
        'none'
      );

      // Delete button is hidden for robot comments
      assert.isTrue((queryAndAssert(element, '#deleteBtn') as HTMLElement).hidden);
    });
    ...
    test('draft saving/editing', done => {
      ...
      element._xhrPromise
        ...
        .then(() => {
          tap(element.shadowRoot.querySelector('.edit'));
          element._messageText =
    ...
    test('proper event fires on resolve, comment is not saved', done => {
      ...
      tap(element.shadowRoot.querySelector('.resolve input'));
    });

    test('resolved comment state indicated by checkbox', () => {
      sinon.stub(element, 'save');
      element.comment = {unresolved: false};
      assert.isTrue(element.shadowRoot.querySelector('.resolve input').checked);
      element.comment = {unresolved: true};
      assert.isFalse(
        element.shadowRoot.querySelector('.resolve input').checked
      );
    });
    ...
    test('_handleFix fires create-fix event', done => {
      ...
      tap(element.shadowRoot.querySelector('.fix'));
    });
    ...
    test('do not show Please Fix button if human reply exists', () => {
      ...
      flush();
      assert.isNull(element.shadowRoot.querySelector('robotActions gr-button'));
    });
    ...
    test('_handleShowFix fires open-fix-preview event', done => {
      ...
      tap(element.shadowRoot.querySelector('.show-fix'));
    });
  });
  ...
  suite('respectful tips', () => {
    ...
    test('show tip when no cached record', done => {
      GrComment = draftFixture.instantiate();
    ...
    test('add 14-day delays once dismissed', done => {
      ...
      flush(() => {
        ...
        tap(element.shadowRoot.querySelector('.respectfulReviewTip .close'));
        flush();
    ...
    test('do not show tip when fall out of probability', done => {
      ...
      flush(() => {
        ...
        assert.isFalse(!!queryAndAssert(element, '.respectfulReviewTip'));
        done();
      });
    });
