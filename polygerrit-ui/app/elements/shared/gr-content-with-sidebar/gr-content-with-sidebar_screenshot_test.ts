...
suite('gr-content-with-sidebar screenshot tests', () => {
  ...
  setup(async () => {
    wrapper = await fixture<HTMLDivElement>(
      html`
        <div style="position: relative; padding-top: 50px; background: pink; width: 800px;">
          <div style="height: 50px; background: orange;">Header / Banner</div>
          <gr-content-with-sidebar style="--sidebar-top: 50px;">
            <div slot="main" style="height: 500px; background: lightblue; width: 400px; padding: 20px; box-sizing: border-box;">
               Main content area
            </div>
            <div slot="side" style="height: 600px; background: lightgray; padding: 20px; box-sizing: border-box;">
               Sidebar content
            </div>
          </gr-content-with-sidebar>
        </div>`
    );
    const element = wrapper.querySelector<GrContentWithSidebar>('gr-content-with-sidebar')!;
