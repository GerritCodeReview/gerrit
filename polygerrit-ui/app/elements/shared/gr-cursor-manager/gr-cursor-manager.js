...
class GrCursorManager extends GestureEventListeners(
  ...
  setCursor(element, opt_noScroll) {
    ...
    if (opt_noScroll) {
      ...
      this.scrollBehavior = ScrollActions.NEVER;
    }
  ...
  _targetIsVisible(top) {
    const dims = this._getWindowDims();
    return this.scrollBehavior === ScrollActions.KEEP_VISIBLE &&
  ...
  _scrollToTarget() {
    if (!this.target || this.scrollBehavior === ScrollActions.NEVER) {
      return;
    }
