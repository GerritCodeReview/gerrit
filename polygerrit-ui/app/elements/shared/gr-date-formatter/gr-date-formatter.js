...
import {parseDate, fromNow, isValidDate, isWithinDay, isWithinHalfYear, formatDate, utcOffsetString} from '../../../scripts/time-util.js';
...
class GrDateFormatter extends mixinBehaviors( [
  ...
  constructor() {
    super();
    this.util = {parseDate, fromNow, isValidDate, isWithinDay,
      isWithinHalfYear, formatDate, utcOffsetString};
  }
  ...
  _computeDateStr(
    ...
    const date = this.util.parseDate(dateStr);
    if (!this.util.isValidDate(date)) { return ''; }
    if (relative) {
      return this.util.fromNow(date);
    }
    const now = new Date();
    let format = dateFormat.full;
    if (this.util.isWithinDay(now, date)) {
      format = timeFormat;
    } else {
      if (this.util.isWithinHalfYear(now, date)) {
    ...
    return this.util.formatDate(date, format);
  }
  ...
  _computeFullDateStr(dateStr, timeFormat, dateFormat) {
    ...
    return this.util.formatDate(date, format) + this.util.utcOffsetString();
  }
}
