...
export class GrDateFormatter extends LitElement {
  ...
  override render() {
    ...
    return html`
      <gr-tooltip-content
        has-tooltip
        title=${fullDateStr}
        >${this.renderDateString()}</gr-tooltip-content>
    `;
  }

  private renderDateString() {
    return html`
      <span>${this._computeDateStr()}</span>`;
  }
  ...
  _loadPreferences() {
    return this._getLoggedIn().then(loggedIn => {
      ...
      return Promise.all([this.loadTimeFormat(), this.loadRelative()]);
    });
  }

  private loadTimeFormat() {
  ...
  private computeFullDateStr() {
    ...
    if ([this.dateStr, this.timeFormat].includes(undefined) ||
        !this.dateFormat) {
      return undefined;
    }
    ...
    let format = this.dateFormat.full + ', ';
    format += this.timeFormat === TimeFormats.TIME_12
