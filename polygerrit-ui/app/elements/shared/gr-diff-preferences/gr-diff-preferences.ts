...
export class GrDiffPreferences extends LitElement {
  ...
  async save() {
    ...
    // We have to fire this twice (so the test works in gr-diff-preferences-dialog)
    fireEvent(this, 'has-unsaved-changes-changed');

    // We need to wrap the fireEvent in a setTimeout
    // to ensure originalDiffPrefs is updated ontime.
    setTimeout(() => fireEvent(this, 'has-unsaved-changes-changed'), 1);
  }
}
