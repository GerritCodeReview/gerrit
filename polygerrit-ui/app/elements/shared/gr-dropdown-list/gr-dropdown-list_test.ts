...
suite('gr-dropdown-list tests', () => {
  ...
  test('options are selected and laid out correctly', async () => {
    ...
    assert.equal(
      query<HTMLDivElement>(items[0], '.topContent div')!.innerText,
    ...
    assert.equal(
      query<HTMLDivElement>(items[1], '.topContent div')!.innerText,
    ...
    assert.equal(
      query<HTMLDivElement>(items[2], '.topContent div')!.innerText,
