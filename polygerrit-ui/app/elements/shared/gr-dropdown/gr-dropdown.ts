...
export class GrDropdown extends LitElement {
  ...
  private renderDropdownLink(link: DropdownLink, index: number) {
    return html`
      ...
      >
        ...
        >
          <div slot="headline">${link.name}</div>
        </md-menu-item>
      </gr-tooltip-content>
  ...
  private handleEnter() {
    ...
    if (this.dropdown.open) {
      ...
      if (this.cursor.target !== null) {
        ...
        // The below fixes it. It's an ugly hack but works for now.
        const el =
          this.cursor.target.shadowRoot?.querySelector(':not([hidden])');
        if (el) {
          this.cursor.target?.removeAttribute('selected');
  ...
  /**
   * Because <a> is added internally in md-menu-item, we have to do a workaround to add the
   * attributes we want.
   * @param e Event
