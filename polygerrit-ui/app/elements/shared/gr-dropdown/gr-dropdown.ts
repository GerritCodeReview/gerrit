...
export class GrDropdown extends LitElement {
  ...
  set opened(opened) {
    ...
    this._opened = opened;
    fire(this, 'opened-changed', {value: this._opened});
  }
  ...
  override connectedCallback() {
    ...
    this.cleanups.push(
      addShortcut(this, {key: Key.UP}, () => this.handleUp())
    );
    this.cleanups.push(
    ...
    return html`
      <div class="dropdown-content" slot="dropdown-content">
        <ul>
          ...
          ${incrementalRepeat({
            values: this.items ?? [],
            mapFn: (link) => this.renderDropdownLink(link as DropdownLink),
            initialCount: 75
          })}
        </ul>
      </div>`;
  ...
  private renderTopContent() {
    ...
    return html`
      <div class="topContent">
        ${incrementalRepeat({
          values: this.topContent,
          mapFn: (item) => this.renderTopContentItem(item as DropdownContent),
          initialCount: 75
        })}
      </div>
    `;
  ...
  private renderTopContentItem(item: DropdownContent) {
    return html`
      <div
        class="${this.getClassIfBold(item.bold)} top-item"
        tabindex="-1"
      >
        ${item.text}
      </div>
   ...
   */
  private async resetCursorStops() {
    if (this.items && this.items.length > 0 && this.dropdown?.opened) {
      await this.updateComplete
      this.cursor.stops = Array.from(this.querySelectorAll('li'));
    }
  }
