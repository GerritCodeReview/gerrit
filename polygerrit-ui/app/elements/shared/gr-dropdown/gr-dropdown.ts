...
export class GrDropdown extends LitElement {
   ...
   * Do not use, use opened instead through the getters and setters.
   * This is not a regular @state because we need to also send the
   * 'opened-changed' event when it is changed. And if we rely on @state
   * then the name to look for in willUpdate/update/updated is '_opened'
