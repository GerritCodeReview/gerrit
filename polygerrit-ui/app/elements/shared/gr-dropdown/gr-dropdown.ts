...
export class GrDropdown extends LitElement {
  ...
  private renderDropdownLink(link: DropdownLink, index: number) {
    this.handleAdditionalLinkAttributes(link, index);
    return html`
      <gr-tooltip-content
   ...
   */
  private handleAdditionalLinkAttributes(link: DropdownLink, index: number) {
    const anchor = this.shadowRoot
      ?.querySelector(`md-menu-item[data-index="${index}"]`)
