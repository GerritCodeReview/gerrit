...
export class GrEditableContent extends LitElement {
  ...
  // New method to check if formatting changes only affect the current line
  private isOnlyCurrentLineFormatting(
    ...
    // If line count differs, there are other changes besides the current line
    ...
    // If we got here, only the current line differs (or no lines differ)
    return true;
  }
  ...
  // New method to filter out trailing space errors on the active line
