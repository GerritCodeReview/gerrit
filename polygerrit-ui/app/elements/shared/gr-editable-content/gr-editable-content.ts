...
export class GrEditableContent extends LitElement {
  ...
  override render() {
    return html`
      <gr-endpoint-decorator name="commit-message">
        ...
        ></gr-endpoint-param>
        <div
          class="viewer ${this.commitCollapsed && this.commitCollapsible
            ? 'collapsed'
            : ''}"
          ?hidden="${this.editing}"
        >
        ...
        <div
          ...
          ?hidden="${this.computeHideShowAllContainer()}"
        >
          <gr-button
            link=""
  ...
  private computeHideShowAllContainer() {
    if (this.editing) return false;
    return (
      (!this.commitCollapsible || this.editing) && this.hideEditCommitMessage
    );
  }
