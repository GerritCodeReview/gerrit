...
export class GrFixSuggestions extends LitElement {
  ...
  override render() {
    ...
    return html`<div class="header">
        <div class="title">
          ${this.renderTitle(fix_suggestions)}
          <a
  ...
  renderTitle(fix_suggestions: FixSuggestionInfo[]) {
    const isMLFixSuggestion =
      this.suggestionsProvider?.isFixSuggestionFromML?.(fix_suggestions) ??
      false;
    if (isMLFixSuggestion) {
      return html`<span>ML-suggested edit</span>`;
    } else {
      return html`<span>Suggested edit</span>`;
    }
  }

  getDocumentationLink(fix_suggestions: FixSuggestionInfo[]) {
    const documentationLink =
      this.suggestionsProvider?.getDocumentationLink?.(fix_suggestions) ?? '';

    if (documentationLink !== '') {
      return documentationLink;
    } else {
      return getDocUrl(this.docsBaseUrl, 'user-suggest-edits.html');
    }
  }
