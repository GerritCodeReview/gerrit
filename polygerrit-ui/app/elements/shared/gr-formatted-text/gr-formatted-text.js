      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (this._isCodeMarkLine(lines[i])) {
          ...
          if (this._isCodeMarkLine(lines[nextI])) {
            result.push(
                this._makeCode(lines.slice(i+1, nextI).join('\n'))
            );
            i = nextI;
            continue;
          }
        ...
        if (this._isSingleLineCode(lines[i])) {
          ...
          result.push(this._makeCode(codeContent));
        } else if (this._isList(lines[i])) {
          let nextI = i + 1;
          while (this._isList(lines[nextI])) nextI++;
          result.push(this._makeList(lines.slice(i, nextI)));
          i = nextI - 1;
        } else if (this._isQuote(lines[i])) {
          let nextI = i + 1;
          while (this._isQuote(lines[nextI])) nextI++;
          const blockLines = lines.slice(i, nextI)
              .map(l => l.replace(/^[ ]?>[ ]?/, ''));
          result.push(this._makeQuote(blockLines));
          i = nextI - 1;
