...
suite('gr-formatted-text tests', () => {
  ...
  suite('as markdown', () => {
    ...
    test('renders inline links with auto-linked URLs correctly', async () => {
      element.content = `[build page](https://cr-buildbucket.appspot.com/build/8682334566330890001)`;
      await element.updateComplete;
    ...
    test('renders raw HTML link with URL correctly', async () => {
      const eventPromise = new Promise(resolve => {
        element.addEventListener('marked-render-complete', resolve, {once: true});
      });
      element.content = `<a href="https://cr-buildbucket.appspot.com/build/8682334566330890001">build page</a>`;
      await eventPromise;

      const div = element.shadowRoot!.querySelector('.markdown-html');
      assert.isNotNull(div);
      assert.equal(div!.innerHTML, '<p>&lt;a href="https://cr-buildbucket.appspot.com/build/8682334566330890001"&gt;build page&lt;/a&gt;</p>\n');
    });
