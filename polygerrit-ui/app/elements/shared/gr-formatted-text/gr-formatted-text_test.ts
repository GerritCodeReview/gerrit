...
suite('gr-formatted-text tests', () => {
  ...
  suite('as plaintext', () => {
    ...
    test('does default linking', async () => {
      const checkLinking = async (url: string, expectLinkified: boolean = true) => {
        element.content = url;
        await element.updateComplete;
        const a = query<HTMLElement>(element, 'a');

        if (expectLinkified) {
          assertIsDefined<HTMLElement>(a);
      ...
      await checkLinking('www.google.com', false);
    });
  });
  ...
  suite('as markdown', () => {
    ...
    test('does default linking', async () => {
      const checkLinking = async (url: string, expectLinkified: boolean = true) => {
        ...
        await element.updateComplete;
        const a = query<HTMLElement>(element, 'a');
        const p = query<HTMLElement>(element, 'p');
