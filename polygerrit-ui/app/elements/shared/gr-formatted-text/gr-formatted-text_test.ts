...
suite('gr-formatted-text tests', () => {
  ...
  function assertSpan(actual: Span, expected: Span) {
    assert.equal(actual.type, expected.type);
    assert.equal(actual.text, expected.text);
    switch (actual.type) {
      case 'img':
        assert.equal(actual.url, (expected as ImgSpan).url);
        break;
      case 'link':
  ...
  function assertTextBlock(block: Block, spans: Span[]) {
    assert.equal(block.type, 'paragraph');
    const textBlock = block as TextBlock;
    assert.equal(textBlock.spans.length, spans.length);
    for (let i = 0; i < textBlock.spans.length; ++i) {
      assertSpan(textBlock.spans[i], spans[i]);
    }
  }
  ...
  function assertSimpleTextBlock(block: Block, text: string) {
    assertTextBlock(block, [{type: 'text', text: text}]);
  }
  ...
  for (const text of [
    ...
    test('parse simple', () => {
      const comment = {type: 'text', text: text} as TextSpan;
  ...
  test('nested list will NOT be recognized', () => {
    ...
    assertListBlock(result[2], [
      {spans: [{type: 'text', text: 'line 2'}]}
    ]);
  });
