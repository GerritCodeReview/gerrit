...
(function() {
  ...
  Polymer({
    ...
    properties: {
      ...
      /**
       * Travesal state when matching highlightText.
       */
      _nodeTraversalState: {
        type: Object,
        value() {
    ...
    _updateHighlights() {
      ...
      this._highlightElement({childNodes: nodes});
    },
    ...
    _highlightTextNode(textNode) {
      this._nodeTraversalState = this._nodeTraversalState || {
        matchedTextInPreviousNode: '',
        highlightCallbackForPreviousNode: null,
      };
      let {
      ...
      for (let i = 0; i < text.length; i++) {
        ...
        if (matchedText === highlightText) {
          ...
          return this._highlightTextNode(tailNode);
      ...
      this._nodeTraversalState.matchedTextInPreviousNode = matchedText;
      if (matchedText) {
        this._nodeTraversalState.highlightCallbackForPreviousNode = () => {
          if (highlightCallbackForPreviousNode) {
            highlightCallbackForPreviousNode();
          }
        ...
        this._nodeTraversalState.highlightCallbackForPreviousNode = null;
      }
    },
