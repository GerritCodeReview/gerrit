...
import {isServiceUser, canRemoveReviewer, accountKey} from '../../../utils/account-util';
import {getDisplayName} from '../../../utils/display-name-util';
import {customElement, property} from '@polymer/decorators';
import {RestApiService} from '../../../services/services/gr-rest-api/gr-rest-api';
import {AccountInfo, ChangeInfo, ServerInfo, ReviewInput} from '../../../types/common';
...
export class GrHovercardAccount extends GestureEventListeners(
  ...
  _handleChangeReviewerOrCCStatus() {
    ...
    this._handleRemoveReviewerOrCC().then(() => {
      console.log('time to toggle');
      this.$.restAPI.saveChangeReview(
        this.change!._number,
        ...
        if (!response || !response.ok) {
          this.dispatchEvent(
            new CustomEvent('show-alert', {
              detail: {message: 'something went wrong when removing user'},
              bubbles: true,
              composed: true,
            })
          );
