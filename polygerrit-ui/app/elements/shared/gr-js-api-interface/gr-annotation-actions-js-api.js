...
(function(window) {
  ...
  function GrAnnotationActionsInterface(plugin) {
    ...
    this._addLayerFunc = (el, line, path, changeNum, patchNum, annotateRange) => {};
  }
  ...
  GrAnnotationActionsInterface.prototype.notify = function(path, startRange, endRange, side) {
  ...
  GrAnnotationActionsInterface.prototype.getLayer = function(path, changeNum, patchNum) {
    var annotationLayer = new AnnotationLayer(path, changeNum, patchNum, this._addLayerFunc);
  ...
  AnnotationLayer.prototype.annotate = function(el, line) {
    this._addLayerFunc(el, line, this._path, this._changeNum, this._patchNum, this._annotateRange);
  };

  // Helper method used to invoke all listeners.
  AnnotationLayer.prototype.notifyListeners = function(startRange, endRange, side) {
    for (const listener of this._listeners) {
      listener(startRange, endRange, side);
    }
  };

  // Helper method which is passed to plugins.
  AnnotationLayer.prototype._annotateRange = function(startRange, endRange, el, cssClass) {
    GrAnnotation.annotateElement(
        el, startRange, endRange,
        'style-scope gr-diff ' + cssClass);
  };
