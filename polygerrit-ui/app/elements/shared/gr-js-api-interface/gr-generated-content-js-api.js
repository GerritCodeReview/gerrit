 ...
 */
 (function(window) {
  ...
  function GrGeneratedContentInterface(plugin) {
    this.plugin = plugin;
    // Return this isntance when there is a generatecontent event.
  ...
  GrGeneratedContentInterface.prototype.addContentGenerator = function(
      validityFunc, generatorFunc) {
    this._contentGenerators.push(new ContentGenerator(validityFunc, generatorFunc));
  };

  GrGeneratedContentInterface.prototype.addContentRequiredContentGenerator =
      function(validityFunc, generatorFunc) {
    this._contentRequiredContentGenerators.push(new ContentRequiredContentGenerator(
        validityFunc, generatorFunc));
  };

  GrGeneratedContentInterface.prototype.numContentRequiredContentGenerators = 
      function() {
  ...
  GrGeneratedContentInterface.prototype.getMatchingContentRequiredGenerators =
  ...
  ContentGenerator.prototype.isValid(path) {
    return this._validityFunc(path);
  }

  ContentGenerator.prototype.generateContent(path, content) {
    return this._generatorFunc(path, content);
  };

  ContentGenerator.prototype.isImageType() {
  ...
  ContentRequiredContentGenerator.prototype.isValid(path, content) {
