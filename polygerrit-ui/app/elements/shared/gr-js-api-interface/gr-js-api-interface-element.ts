...
export class GrJsApiInterface implements JsApiService, Finalizable {
  ...
  async handleBeforeCommitMessage(change: ChangeInfo | ParsedChangeInfo, msg: string): Promise<boolean> {
    let okay = true;
    for (const cb of this._getEventCallbacks(EventType.BEFORE_COMMIT_MSG_EDIT)) {
      try {
        okay = (await cb(change, msg)) && okay;
      } catch (err: unknown) {
        this.reportError(err, EventType.BBEFORE_COMMIT_MSG_EDIT);
      }
    }
    return okay;
  }
