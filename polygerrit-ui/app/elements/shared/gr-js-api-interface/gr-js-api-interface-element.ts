...
export class GrJsApiInterface implements JsApiService, Finalizable {
  ...
  async handleBeforePublishEdit(change: ChangeInfo, revision?: RevisionInfo | null): boolean {
    for (const cb of this._getEventCallbacks(EventType.BEFORE_PUBLISH_EDIT)) {
      try {
        if (!cb(change, revision)) return false;
      } catch (err: unknown) {
  ...
  async handleBeforeReplySent(reviewInput: ReviewInput): boolean {
    await this.waitForPluginsToLoad();
    for (const cb of this._getEventCallbacks(EventType.BEFORE_REPLY_SENT)) {
      try {
        if (!cb(reviewInput)) return false;
      } catch (err: unknown) {
