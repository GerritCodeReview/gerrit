...
suite("GrJsApiInterface tests", () => {
  ...
  test("revert event", () => {
    ...
    assert.equal(
      element.modifyRevertMsg(null, "test", "origTest"),
      "test\n> origTest\ninfo"
    );
    assert.isTrue(errorStub.calledOnce);
  ...
  test("getLoggedIn", () => {
    ...
    sinon
      .stub(window, "fetch")
      .callsFake(() => Promise.resolve({ status: 204 }));
    return plugin
      .restApi()
      .getLoggedIn()
      .then((loggedIn) => {
  ...
  test("getAdminMenuLinks", () => {
    ...
    const getCallbacksStub = sinon
      .stub(element, "_getEventCallbacks")
      .returns([
  ...
  suite("popup", () => {
    ...
    test("popup(moduleName) creates popup with component", () => {
      const openStub = sinon
        .stub(GrPopupInterface.prototype, "open")
        .callsFake(function () {
  ...
  suite("screen", () => {
    ...
    test("works", () => {
      ...
      assert.isTrue(
        plugin.registerCustomComponent.calledWith(
