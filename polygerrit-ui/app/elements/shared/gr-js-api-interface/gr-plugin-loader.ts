...
type PluginCallbackMap = {[name: string]: PluginCallback};
...
export class PluginLoader {
  ...
  _loadingResolver: ((value?: unknown) => void) | null = null;
  ...
  install(
    ...
    const script: HTMLScriptElement | null =
      gerritScript && (gerritScript.__importElement || gerritScript);
    ...
    if (opt_version && opt_version !== API_VERSION) {
      this._failToLoad(
        `Plugin ${src} install error: only version ` +
          API_VERSION +
          ' is supported in PolyGerrit. ' +
          opt_version +
          ' was given.',
        src
      );
    ...
    if (!url) {
      this._failToLoad('Failed to determine plugin URL.', src);
      return;
    }
  ...
  _importHtmlPlugin(pluginUrl: string, opts: PluginOption = {}) {
    ...
    if (urlWithAP === undefined || urlWithoutAP === undefined) return;
  ...
  _createScriptTag(url: string, onerror?: (e: string | Event) => void) {
    if (!onerror) {
  ...
  _urlFor(pathOrUrl?: string, assetsPath?: string) {
    if (!pathOrUrl) {
      return pathOrUrl;
    }
