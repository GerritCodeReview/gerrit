...
(function(window) {
  ...
  Plugin.prototype._registerEndpointModule =
      function(endpoint, type, module) {
    var endpoints = Gerrit._endpoints;
  ...
  Gerrit._getModulesForEndoint = function(name, opt_options) {
    var modeulesData = Gerrit._getEndpointDetails(name, opt_options);
    if (!modeulesData.length) {
      return [];
    }
    return modeulesData.map(m => m.moduleName);
  };
  ...
  Gerrit._getPluginsForEndpoint = function(name, opt_options) {
    ...
    return Array.from(new Set(modeulesData.map(m => m.pluginUrl)));
  };

  // TODO (viktard): namespace.
  Plugin.prototype.setReplyTextChangedHandler = function(handler) {
    // TODO (viktard): pool, registry, disposal.
    var randomName = this.getPluginName() + '-autogenerated-'
    ...
    Polymer({
      ...
      attached: function() {
        ...
        this.content.addEventListener('value-changed', function(e) {
          handler(e.detail.value);
        });
      },
    });
    this.registerCustomComponent('reply-text', randomName);
  };
