...
(function(window) {
  ...
  function Plugin(url, pluginsConfig) {
    if (!url) {
    ...
    this._config = pluginsConfig[this._name];
  }

  Plugin._sharedAPIElement = document.createElement('gr-js-api-interface');

  Plugin.prototype._config = null;
  ...
  Gerrit.install = function(callback, opt_version, opt_src) {
    ...
    if (!configPromise) {
      configPromise = getRestAPI().getConfig().then(function(config) {
        if (config && config.plugin && config.plugin.config_json_url) {
          return getRestAPI().fetchJSON(config.plugin.config_json_url);
        }
        return {};
      });
    }
    configPromise.then(function(pluginsConfig) {
      callback(new Plugin(src, pluginsConfig));
    });
  };
