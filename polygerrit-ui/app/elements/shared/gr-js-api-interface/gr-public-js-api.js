...
(function(window) {
  ...
  Plugin.prototype.registerStyleModule = function(name, url) {
    if (!Gerrit._styleModules[name]) {
      Gerrit._styleModules[name] = [url];
    } else {
      Gerrit._styleModules[name].push(url);
    }
  };
  ...
  // Hash of style modules to be applied, name to url.
