...
(function(window) {
  ...
  Plugin.prototype.delete = function(url, opt_callback) {
    console.warn('.delete() is deprecated! Use .restApi().delete()');
    return getRestAPI().send('DELETE', this.url(url)).then(response => {
  ...
  Gerrit.get = function(url, callback) {
    console.warn('Gerrit.get() is deprecated! Use plugin.restApi().get()');
    send('GET', url, callback);
  };

  Gerrit.post = function(url, payload, callback) {
    console.warn('Gerrit.post() is deprecated! Use plugin.restApi().post()');
    send('POST', url, callback, payload);
  };

  Gerrit.put = function(url, payload, callback) {
    console.warn('Gerrit.put() is deprecated! Use plugin.restApi().put()');
    send('PUT', url, callback, payload);
  };

  Gerrit.delete = function(url, opt_callback) {
    console.warn('Gerrit.post() is deprecated! Use plugin.restApi().post()');
    return send('DELETE', url).then(response => {
      if (response.status !== 204) {
        return response.text().then(text => {
          if (text) {
            return Promise.reject(text);
