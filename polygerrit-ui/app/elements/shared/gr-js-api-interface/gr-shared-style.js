...
(function(window) {
  ...
  let sharedStylesCount = 0;

  function GrSharedStyle(rulesStr) {
    const styleIndex = sharedStylesCount;
    this._rulesStr = rulesStr;
    this._className = `__pg_js_api_class_${styleIndex}`;
    sharedStylesCount++;
  }

  /**
   * Create a new unique CSS class and injects it in a root node of the element
   ...
   * root node.
   * @param {HTMLElement} Appropriate class name for this element is returned
   * @return {string}
   */
  GrSharedStyle.prototype.getClassName = function(element) {
   ...
   * @param {HTMLElement} Element to apply style
   */
  GrSharedStyle.prototype.apply = function(element) {
    element.classList.add(this.getClassName(element));
  };

  window.GrSharedStyle = GrSharedStyle;
})(window);
