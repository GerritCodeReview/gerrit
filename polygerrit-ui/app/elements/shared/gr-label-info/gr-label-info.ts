...
export class GrLabelInfo extends LitElement {
  ...
  renderReviewerVote(reviewer: AccountInfo) {
    ...
    const noVoteYet = !approvalInfo || hasNeutralStatus(labelInfo, approvalInfo);
    return html`<div class="reviewer-row"><gr-account-chip
      .account="${reviewer}"
      .change="${this.change}"
    >
      <gr-vote-chip
    ...
    ${noVoteYet ? 
      ...
      html`<gr-tooltip-content has-tooltip title="Remove vote">
      <gr-button
        link
