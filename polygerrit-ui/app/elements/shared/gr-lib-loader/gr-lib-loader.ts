...
export class GrLibLoader {
  private readonly libraryStates = new Map<LibraryConfig, Promise<unknown>>();
  ...
  getLibrary(config: LibraryConfig): Promise<unknown> {
    if (!this.libraryStates.has(config)) {
      const configured = this._loadScript(this._getPath(config.src)).then(config.configureCallback);
      this.libraryStates.set(config, configured);
    }
    return this.libraryStates.get(config)!;
  }
  ...
  getHLJS(): Promise<HighlightJS | undefined> {
    return this.getLibrary(HLJS_LIBRARY_CONFIG).then(() => {
      return window.hljs;
    });
  }
