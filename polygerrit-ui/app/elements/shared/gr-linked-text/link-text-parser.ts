...
export class GrLinkTextParser {
   ...
   *
   */
  private parseChunk(text: string, href?: string) {
    ...
    if (href) {
      ...
      if (result) {
        ...
        if (prefixText.length > 0) {
          ...
          // any patterns in it.
          this.processLinks(prefixText, []);
          // trim prefix before calling back
   ...
   */
  private parseLinks(text: string, config: LinkTextParserConfig) {
   ...
   *     matches.
   */
  private processLinks(text: string, outputArray: CommentLinkItem[]) {
   ...
   * reverse order.
   */
  private sortArrayReverse(outputArray: CommentLinkItem[]) {
