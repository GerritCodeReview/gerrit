  ...
  debounceReload(filter?: string) {
    this.reloadTask = debounce(
      ...
      () => {
        ...
        // this is to prevent an issue where switching between
        // repos, groups and so on with a filter inputed and
        // does not go back to the previous screen that had the filter.
        if (this.path && window.location.pathname.includes(this.path)) {
          if (filter) {
            return page.show(
