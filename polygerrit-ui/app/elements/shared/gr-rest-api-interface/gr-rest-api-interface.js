...
(function() {
  ...
  Polymer({
    ...
    fetchJSON(url, opt_errFn, opt_cancelCondition, opt_params, opt_opts) {
      opt_opts = opt_opts || {};
    ...
    _urlWithParams(url, opt_params) {
      ...
      for (const p in opt_params) {
        ...
        const values = [].concat(opt_params[p]);
    ...
    _getDiffComments(changeNum, endpoint, opt_basePatchNum, opt_patchNum,
        opt_path) {
    ...
    getCommitFileContents(projectName, commit, path) {
      return this._fetchB64File(
    ...
    getImagesForDiff(project, commit, changeNum, diff, patchRange) {
      let promiseA;
      let promiseB;
        ...
        if (patchRange.basePatchNum === 'PARENT') {
          ...
          promiseA = this.getChangeFileContents(changeNum, patchRange.patchNum,
              ...
              .catch(result =>
                ...
                this._getImageFromCommit(project, commit, diff.meta_a.name)
              );
        } else {
     ...
     */
    _getImageFromCommit(project, commit, path) {
      return this.getCommitInfo(project, commit).then(info => {
        if (info.parents.length !== 1) {
          return Promise.reject('Change commit has multiple parents.');
        }
        const parent = info.parents[0].commit;
    ...
    setChangeTopic(changeNum, topic) {
      return this.send('PUT',
          `/changes/${encodeURIComponent(changeNum)}/topic`, {topic});
    },
