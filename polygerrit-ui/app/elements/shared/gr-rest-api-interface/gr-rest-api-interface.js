...
(function() {
  ...
  Polymer({
     ...
     */
    getChangeDetail(changeNum, opt_errFn, opt_cancelCondition, project, preheat) {
      ...
      return this._getChangeDetail(
          changeNum, optionsHex, opt_errFn, opt_cancelCondition, project, preheat)
     ...
     * @param {function()=} opt_cancelCondition
     */
    _getChangeDetail(changeNum, optionsHex, opt_errFn, opt_cancelCondition, project, preheat) {
      if(project) {
        const url =  `/changes/${encodeURIComponent(project)}~${changeNum}/detail`;
        const result = this._getChangeDetail2(url, changeNum, optionsHex, opt_errFn, opt_cancelCondition, project);
        if (preheat) {
          this._cache.set(url, result);
        }
        return result;
      }
      const changeActionUrl = this.getChangeActionURL(changeNum, null, '/detail');
    ...
    _getChangeDetail2(url, changeNum, optionsHex, opt_errFn, opt_cancelCondition, project) {
     ...
     * @return {!Promise<!Object>}
     */
    getDiffComments(changeNum, opt_basePatchNum, opt_patchNum, opt_path, project, preheat) {
     ...
     * @return {!Promise<!Object>}
     */
    getDiffRobotComments(changeNum, opt_basePatchNum, opt_patchNum, opt_path, project, preheat) {
    ...
    _getChangeURLAndFetch(req, project, preheat) {
      if(project) {
        const url =  `/changes/${encodeURIComponent(project)}~${req.changeNum}${req.endpoint}`;
