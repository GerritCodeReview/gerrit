...
(function() {
  ...
  const CHECK_SIGN_IN_DEBONCE_MS = 3 * 1000;
  ...
  Polymer({
    ...
    _fetchRawJSON(url, opt_errFn, opt_cancelCondition, opt_params,
      ...
      return auth.fetch(urlWithParams, opt_options).then(response => {
        ...
        if (isLoggedIn && err && err.message === 'Failed to fetch') {
          if (!this.isDebouncerActive('checkCredentials')) {
            this.checkCredentials();
          }
          this.debounce('checkCredentials', this.checkCredentials,
              CHECK_SIGN_IN_DEBONCE_MS);
          return;
        }
        if (opt_errFn) {
