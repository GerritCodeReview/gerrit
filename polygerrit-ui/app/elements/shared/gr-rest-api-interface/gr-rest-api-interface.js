...
(function() {
  ...
  Polymer({
    ...
    _getFileInChangeEdit(changeNum, path) {
      const endpoint = '/edit/' + encodeURIComponent(path);
      const headers = {Accept: 'application/json'};
      return this._getChangeURLAndFetch({changeNum, endpoint, headers});
    },
    ...
    _send(req) {
      ...
      const xhr = this._auth.fetch(url, options).then(response => {
        if (!response.ok) {
          if (req.errFn) {
    ...
    _sendDiffDraftRequest(method, changeNum, patchNum, draft) {
      ...
      const promise = this.getChangeURLAndSend(
