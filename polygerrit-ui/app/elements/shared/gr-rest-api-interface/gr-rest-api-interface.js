...
(function() {
  ...
  Polymer({
    ...
    getConfig: function() {
      return this._fetchSharedCacheURL(baseUrl() + '/config/server/info');
    },

    getProjectConfig: function(project) {
      return this._fetchSharedCacheURL(
          baseUrl() + '/projects/' + encodeURIComponent(project) + '/config');
    },

    getVersion: function() {
      return this._fetchSharedCacheURL(baseUrl() + '/config/server/version');
    },

    getDiffPreferences: function() {
      return this.getLoggedIn().then(function(loggedIn) {
        if (loggedIn) {
          return this._fetchSharedCacheURL(baseUrl() + '/accounts/self/preferences.diff');
        }
    ...
    savePreferences: function(prefs, opt_errFn, opt_ctx) {
      ...
      return this.send('PUT', baseUrl() + '/accounts/self/preferences', prefs, opt_errFn,
          opt_ctx);
    },

    saveDiffPreferences: function(prefs, opt_errFn, opt_ctx) {
      // Invalidate the cache.
      this._cache[baseUrl() + '/accounts/self/preferences.diff'] = undefined;
      return this.send('PUT', baseUrl() + '/accounts/self/preferences.diff', prefs,
          opt_errFn, opt_ctx);
    },

    getAccount: function() {
      return this._fetchSharedCacheURL(baseUrl() + '/accounts/self/detail', function(resp) {
        if (resp.status === 403) {
          this._cache[baseUrl() + '/accounts/self/detail'] = null;
        }
      }.bind(this));
    },

    getAccountEmails: function() {
      return this._fetchSharedCacheURL(baseUrl() + '/accounts/self/emails');
    },

    addAccountEmail: function(email, opt_errFn, opt_ctx) {
      return this.send('PUT', baseUrl() + '/accounts/self/emails/' +
          encodeURIComponent(email), null, opt_errFn, opt_ctx);
    },

    deleteAccountEmail: function(email, opt_errFn, opt_ctx) {
      return this.send('DELETE', baseUrl() + '/accounts/self/emails/' +
          encodeURIComponent(email), null, opt_errFn, opt_ctx);
    },
    ...
    setPreferredAccountEmail: function(email, opt_errFn, opt_ctx) {
      return this.send('PUT', baseUrl() + '/accounts/self/emails/' +
        ...
        var cachedEmails = this._cache[baseUrl() + '/accounts/self/emails'];
        if (cachedEmails) {
          ...
          this._cache[baseUrl() + '/accounts/self/emails'] = emails;
        }
      }.bind(this));
    },

    setAccountName: function(name, opt_errFn, opt_ctx) {
      return this.send('PUT', baseUrl() + '/accounts/self/name', {name: name}, opt_errFn,
          opt_ctx).then(function(response) {
            // If result of getAccount is in cache, update it in the cache
            // so we don't have to invalidate it.
            var cachedAccount = this._cache[baseUrl() + '/accounts/self/detail'];
            if (cachedAccount) {
              return this.getResponseObject(response).then(function(newName) {
                // Replace object in cache with new object to force UI updates.
                // TODO(logan): Polyfill for Object.assign in IE
                this._cache[baseUrl() + '/accounts/self/detail'] = Object.assign(
    ...
    setAccountStatus: function(status, opt_errFn, opt_ctx) {
      return this.send('PUT', baseUrl() + '/accounts/self/status', {status: status},
    ...
    getAccountGroups: function() {
      return this._fetchSharedCacheURL(baseUrl() + '/accounts/self/groups');
    },
    ...
    getAccountCapabilities: function(opt_params) {
      ...
      return this._fetchSharedCacheURL(baseUrl() + '/accounts/self/capabilities' +
          queryString);
    },
    ...
    getPreferences: function() {
      return this.getLoggedIn().then(function(loggedIn) {
        if (loggedIn) {
          return this._fetchSharedCacheURL(baseUrl() + '/accounts/self/preferences').then(
              function(res) {
    ...
    getWatchedProjects: function() {
      return this._fetchSharedCacheURL(baseUrl() + '/accounts/self/watched.projects');
    },

    saveWatchedProjects: function(projects, opt_errFn, opt_ctx) {
      return this.send('POST', baseUrl() + '/accounts/self/watched.projects', projects,
          opt_errFn, opt_ctx)
    ...
    deleteWatchedProjects: function(projects, opt_errFn, opt_ctx) {
      return this.send('POST', baseUrl() + '/accounts/self/watched.projects:delete',
    ...
    getChanges: function(changesPerPage, opt_query, opt_offset) {
      ...
      return this.fetchJSON(baseUrl() + '/changes/', null, null, params);
    },
    ...
    getSuggestedGroups: function(inputVal, opt_n, opt_errFn, opt_ctx) {
      ...
      return this.fetchJSON(baseUrl() + '/groups/', opt_errFn, opt_ctx, params);
    },

    getSuggestedProjects: function(inputVal, opt_n, opt_errFn, opt_ctx) {
      var params = {p: inputVal};
      if (opt_n) { params.n = opt_n; }
      return this.fetchJSON(baseUrl() + '/projects/', opt_errFn, opt_ctx, params);
    },

    getSuggestedAccounts: function(inputVal, opt_n, opt_errFn, opt_ctx) {
      var params = {q: inputVal, suggest: null};
      if (opt_n) { params.n = opt_n; }
      return this.fetchJSON(baseUrl() + '/accounts/', opt_errFn, opt_ctx, params);
    },
    ...
    saveChangeStarred: function(changeNum, starred) {
      var url = baseUrl() + '/accounts/self/starred.changes/' + changeNum;
    ...
    _changeBaseURL: function(changeNum, opt_patchNum) {
      var v = baseUrl() + '/changes/' + changeNum;
      if (opt_patchNum) {
    ...
    getCommitInfo: function(project, commit) {
      return this.fetchJSON(
          baseUrl() + '/projects/' + encodeURIComponent(project) +
    ...
    getChangeFileContents: function(changeId, patchNum, path) {
      return this._fetchB64File(
          baseUrl() + '/changes/' + encodeURIComponent(changeId) +
    ...
    setChangeTopic: function(changeNum, topic) {
      return this.send('PUT', baseUrl() + '/changes/' + encodeURIComponent(changeNum) +
          '/topic', {topic: topic});
    },

    deleteAccountHttpPassword: function() {
      return this.send('DELETE', baseUrl() + '/accounts/self/password.http');
    },

    generateAccountHttpPassword: function() {
      return this.send('PUT', baseUrl() + '/accounts/self/password.http', {generate: true})
          .then(this.getResponseObject);
    },

    getAccountSSHKeys: function() {
      return this._fetchSharedCacheURL(baseUrl() + '/accounts/self/sshkeys');
    },

    addAccountSSHKey: function(key) {
      return this.send('POST', baseUrl() + '/accounts/self/sshkeys', key, null, null,
          'plain/text')
          .then(function(response) {
    ...
    deleteAccountSSHKey: function(id) {
      return this.send('DELETE', baseUrl() + '/accounts/self/sshkeys/' + id);
    },

    deleteVote: function(changeID, account, label) {
      return this.send('DELETE', baseUrl() + '/changes/' + changeID +
    ...
    confirmEmail: function(token) {
      return this.send('PUT', baseUrl() + '/config/server/email.confirm', {token: token})
          .then(function(response) {
