...
export class GrRestApiInterface
  ...
  private queryProjects: boolean = false;
  ...
  getRepos(
    ...
    // TODO(kaspern): Rename rest api from /projects/ to /repos/ once backend
    // supports it.
    //If query then return directlu as the result will be expected to be an array
    ...
    var result: Promise<NameToProjectInfoMap[] | undefined> = this._fetchSharedCacheURL({
      url, // The url contains query,so the response is an array, not map
      anonymizedUrl: '/projects/?*',
  ...
  _transformToArray(
    res: Promise<NameToProjectInfoMap[] | undefined>
      ...
      return res.then(response => {
        var reposList: ProjectInfoWithName[] = [];
        for(const [name, project] of Object.entries(response?? {})){
          var projectInfo : ProjectInfoWithName = {
            ...
            branches: project.branches as unknown as {[branchName: string]: CommitId},
