...
suite('gr-rest-api-interface tests', () => {
  ...
  test('parent diff comments are properly grouped', done => {
    sinon.stub(element._restApiHelper, 'fetchJSON').callsFake( () => Promise.resolve({
      '/COMMIT_MSG': [],
      'sieve.go': [
        {
  ...
  suite('draft comments', () => {
    test('_sendDiffDraftRequest pending requests tracked', () => {
      ...
      sinon.stub(element, '_getChangeURLAndSend').callsFake(() => mockPromise());
  ...
  test('_fetchJSON gets called by getSuggestedAccounts', () => {
    const _fetchJSONStub = sinon.stub(element._restApiHelper, 'fetchJSON').callsFake(
        () => Promise.resolve());
