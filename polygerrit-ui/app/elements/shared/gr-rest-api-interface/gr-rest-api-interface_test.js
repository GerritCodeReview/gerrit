...
suite('gr-rest-api-interface tests', () => {
  ...
  test('normal use', () => {
    ...
    assert.equal(element._getReposUrl('test', 25),
        [false, '/projects/?n=26&S=0&m=test']);

    assert.equal(element._getReposUrl(null, 25),
        [false, `/projects/?n=26&S=0&m=${defaultQuery}`]);

    assert.equal(element._getReposUrl('test', 25, 25),
        [false, '/projects/?n=26&S=25&m=test']);

    assert.equal(element._getReposUrl('inname:test', 25, 25),
        [true, '/projects/?n=26&S=25&query=inname%3Atest']);
  });
  ...
  suite('getRepos', () => {
    ...
    test('with blank', () => {
      ...
      assert.equal(fetchCacheURLStub.lastCall.args[0].url,
          '/projects/?n=26&S=0&m=test%2Ftest');
    });
