...
export class FetchPromisesCache {
  private _data: {[url: string]: Promise<ParsedJSON> | undefined};
  ...
  invalidatePrefix(prefix: string) {
    const newData: {[url: string]: Promise<ParsedJSON> | undefined} = {};
...
s   */
  ...
  /**
   * @return Promise which resolves to an object with parsed json and unmodified
   *     (raw) text from response
   */
  readResponsePayload(
    response: Response
  ...
  /**
   * @return Promise resolves to Response only if the request is successull
