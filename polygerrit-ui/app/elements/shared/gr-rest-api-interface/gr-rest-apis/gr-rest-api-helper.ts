...
function addOrRemoveBaseUrl(key: string) {
  if (getBaseUrl() && key.startsWith(getBaseUrl())) return key;
...
export class SiteBasedCache {
  ...
  has(key: string) {
    return this._cache().has(addOrRemoveBaseUrl(key));
  }

  get(key: string): ParsedJSON | undefined {
    return this._cache().get(addOrRemoveBaseUrl(key));
  }

  set(key: string, value: ParsedJSON) {
    this._cache().set(addOrRemoveBaseUrl(key), value);
  }

  delete(key: string) {
    this._cache().delete(addOrRemoveBaseUrl(key));
  }

  invalidatePrefix(prefix: string) {
    const newMap = new Map<string, ParsedJSON>();
    for (const [key, value] of this._cache().entries()) {
      if (!key.startsWith(addOrRemoveBaseUrl(prefix))) {
...
export class FetchPromisesCache {
  ...
  has(key: string): boolean {
    return !!this.data[addOrRemoveBaseUrl(key)];
  }
  ...
  set(key: string, value: Promise<ParsedJSON | undefined> | undefined) {
    this.data[addOrRemoveBaseUrl(key)] = value;
  }
