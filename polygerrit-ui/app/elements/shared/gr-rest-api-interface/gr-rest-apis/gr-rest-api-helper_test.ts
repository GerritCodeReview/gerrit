...
suite('gr-rest-api-helper tests', () => {
  ...
  test('429 still calls errFn when not retried', async () => {
    authFetchStub.returns(Promise.resolve({
      ...new Response(),
      status: 429,
      ok: false,
  ...
  test('429 still pass through correctly when not retried', async () => {
    authFetchStub.returns(Promise.resolve({
      ...new Response(),
      status: 429,
      ok: false,
  ...
  test('429 errors are retried', async () => {
    helper = new GrRestApiHelper(
      cache,
      getAppContext().authService,
      fetchPromisesCache,
      new RetryScheduler<Response>(readScheduler, 1),
      writeScheduler);
    authFetchStub.returns(Promise.resolve({
      ...new Response(),
      status: 429,
