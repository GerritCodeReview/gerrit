...
export class GrSelectorElement extends LitElement {
  ...
  private setItemSelected_(item: Element | null, isSelected: boolean) {
    ...
    this.dispatchEvent(
      new CustomEvent(isSelected ? 'select' : 'deselect', {
        bubbles: true,
        composed: true,
        detail: {item},
      })
  ...
  itemsChanged() {
    ...
    this.dispatchEvent(
      new CustomEvent('items-changed', {bubbles: true, composed: true})
    );
  }
      ...
      if (idx >= 0) {
        ...
        this.dispatchEvent(
          new CustomEvent('activate', {
            bubbles: true,
            composed: true,
            detail: {item: element, selected: value},
          })
        );
        this.select(value);
        return;
