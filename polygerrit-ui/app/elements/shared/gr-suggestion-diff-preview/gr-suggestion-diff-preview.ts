...
export class GrSuggestionDiffPreview extends LitElement {
   ...
   */
  public applyFixSuggestions() {
    if (this.suggestion || !this.fixSuggestionInfo) return;
    this.applyFixSuggestion(this.fixSuggestionInfo);
  }
  ...
  public applyUserSuggestedFix() {
    ...
    this.applyFixSuggestion(fixSuggestions[0]);
  }

  public async applyFixSuggestion(fixSuggestions: FixSuggestionInfo) {
    const changeNum = this.changeNum;
    const basePatchNum = this.comment?.patch_set as BasePatchSetNum;
    if (!changeNum || !basePatchNum || !fixSuggestions) return;

    this.reporting.time(Timing.APPLY_FIX_LOAD);
    const res = await this.restApiService.applyFixSuggestion(
      changeNum,
      basePatchNum,
      fixSuggestions.replacements
    );
    this.reporting.timeEnd(Timing.APPLY_FIX_LOAD, {
      method: '1-click',
      description: fixSuggestions.description,
    });
    if (res?.ok) {
