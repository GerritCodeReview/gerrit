...
function createSuite(grTextareaEnabled: boolean) {
  ...
  const setText = async (
    ...
    element.wrapper.setCursorPosition(pos === -1 ? text.length : pos);
    if (!preventTextChanged) element.handleTextChanged();
    await element.updateComplete;
  };
  ...
  test('emoji selector is open when a colon is typed & the textarea has focus', async () => {
    ...
    await waitUntil(() => element.wrapper.isFocused() === true);
    await setText(':');
    console.log(`${Date.now() % 100000} asdf teeeeeeeeeesttt`);
    assert.equal(listenerStub.lastCall.args[0].detail.value, ':');
    assert.isTrue(element.wrapper.isFocused());
    await element.updateComplete;
    await element.textarea!.updateComplete;
