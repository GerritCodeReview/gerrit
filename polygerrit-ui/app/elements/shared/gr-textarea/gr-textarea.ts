...
export class GrTextarea extends LitElement {
  ...
  @state() hideReviewerAutocomplete = true;
  ...
  private handleEscKey(e: KeyboardEvent) {
    if (this.hideEmojiAutocomplete && this.hideReviewerAutocomplete) {
      return;
    }
    e.preventDefault();
    e.stopPropagation();
  ...
  private handleUpKey(e: KeyboardEvent) {
    ...
    if (this.emojiSuggestions && !this.emojiSuggestions!.isHidden)
      ...
      this.reviewerSuggestions!.cursorUp();
  ...
  private handleDownKey(e: KeyboardEvent) {
    ...
    if (this.emojiSuggestions && !this.emojiSuggestions!.isHidden)
      ...
      this.reviewerSuggestions!.cursorDown();
    this.textarea!.textarea.focus();
    this.disableEnterKeyForSelectingSuggestion = false;
  }
  ...
  private handleTabKey(e: KeyboardEvent) {
    ...
    // has only typed ':'.
    if (this.hideEmojiAutocomplete || this.disableEnterKeyForSelectingSuggestion) {
      return;
    }
    e.preventDefault();
    e.stopPropagation();
  ...
  handleEnterByKey(e: KeyboardEvent) {
    ...
    if ((this.hideEmojiAutocomplete && this.hideReviewerAutocomplete) || this.disableEnterKeyForSelectingSuggestion) {
      this.indent(e);
      return;
    }
    ...
    if (this.emojiSuggestions && !this.emojiSuggestions!.isHidden)
      ...
      this.setReviewer(this.reviewerSuggestions!.getCurrentText());
  }
  ...
  updateReviewerCaratPosition() {
    this.hideReviewerAutocomplete = false;
    if (typeof this.textarea!.value === 'string') {
      this.hiddenText!.textContent = this.textarea!.value.substr(
        0,
        this.textarea!.selectionStart
  ...
  resetReviewerDropdown() {
    ...
    this.requestUpdate();
    this.reviewerSuggestionString = '';
    this.hideReviewerAutocomplete = true;
