...
export class GrTextarea extends LitElement {
  ...
  closeDropdown() {
    if (this.isMentionsDropdownActive()) this.mentionsSuggestions?.close();
    else this.emojiSuggestions?.close();
  }
  ...
  async determineReviewerSuggestions() {
    const response = await this.restApiService.getSuggestedAccounts(
      this.currentSearchString ?? '',
      undefined,
      this.changeNum,
      true
