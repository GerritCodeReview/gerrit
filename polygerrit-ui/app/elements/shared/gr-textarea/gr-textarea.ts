...
export class GrTextarea extends LitElement {
  ...
  private handleUpKey(e: KeyboardEvent) {
    ...
    else this.reviewerSuggestions!.cursorUp();
  ...
  private handleDownKey(e: KeyboardEvent) {
    ...
    else this.reviewerSuggestions!.cursorDown();
    this.textarea!.textarea.focus();
    this.disableEnterKeyForSelectingSuggestion = false;
  }
  ...
  handleEnterByKey(e: KeyboardEvent) {
    ...
    if (
      (this.hideEmojiAutocomplete && this.hideReviewerAutocomplete) ||
      this.disableEnterKeyForSelectingSuggestion
    ) {
      this.indent(e);
      return;
  ...
  async onValueChanged(e: BindValueChangeEvent) {
    ...
    if (this.atIndex) {
      ...
      this.reviewers = reviewers
        .filter(
          r =>
            r.value && isReviewerAccountSuggestion(r.value) &&
            !!(r.value as SuggestedReviewerAccountInfo)?.account?.email
        )
        .map(
          r =>
            ({
