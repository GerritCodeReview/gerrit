...
export class GrTextarea extends LitElement {
  ...
  onValueChanged(e: BindValueChangeEvent) {
    ...
    if (this.atIndex !== null) {
      ...
      if (this.shouldResetDropdown(text, this.atIndex, this.mentions, '@')) {
        this.resetDropdown();
      } else if (this.mentionsSuggestions!.isHidden) {
        // Otherwise open the dropdown and set the position to be just below the
        // cursor.
        this.mentionsSuggestions!.positionTarget = this.updateCaratPosition();
