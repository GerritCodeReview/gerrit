...
declare global {
  interface HTMLElementTagNameMap {
    'onedev-textarea': OnedevTextarea;
  }
}
 ...
 *
 * Example usage:
 * <onedev-textarea></onedev-textarea>
 */
@customElement('onedev-textarea')
export class OnedevTextarea extends LitElement {
  ...
  @query('.editableDiv')
  readonly editableDivElement?: HTMLDivElement;
    ...
    return [
      css`
        ...
        .editableDiv {
          ...
          border: var(--onedev-textarea-border-width, 2px) solid
            var(--onedev-textarea-border-color, white);
          border-radius: 4px;
          ...
          overflow-x: auto;
          padding: var(--onedev-textarea-padding, 12px);
  ...
  override async focus() {
    ...
    if (this.putCursorAtEndOnFocus && !this.isFocused) {
  ...
  async getCursorPositionAsync() {
    return this.getCursorPositionForDiv(await this.editableDiv);
  }

  getCursorPositionForDiv(divElement?: HTMLElement) {
    const selection = this.getSelection();
    ...
    if (divElement === selection.focusNode) {
      // If focus node is the top textarea then focusOffset is the number of
      // child nodes before the cursor position.
      const partOfNodes = Array.from(divElement.childNodes).slice(
        0,
        selection.focusOffset
      );
      findCursorPosition(partOfNodes);
    } else {
      findCursorPosition(Array.from(divElement.childNodes));
    }
