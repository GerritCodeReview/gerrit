...
import {query, mockPromise} from '../../../test/test-utils';
...
suite('gr-tooltip-content tests', () => {
  ...
  test('does not add window scroll listener if disconnected during show', async () => {
    ...
    try {
      ...
      sinon.stub(GrTooltip.prototype, 'updateComplete').get(() => updateComplete);
      ...
      const scrollCalls = addListenerSpy.getCalls().filter(c => (c.args[0] as string) === 'scroll');
      assert.equal(scrollCalls.length, 0, 'Scroll listener should not be added after disconnect');
    } finally {
      clock.restore();
