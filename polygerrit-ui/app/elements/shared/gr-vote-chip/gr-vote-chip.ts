...
import {ApprovalInfo, DetailedLabelInfo} from '../../../api/rest-api';
...
export class GrVoteChip extends GrLitElement {
  ...
  render() {
    ...
    const plusSign = this.vote.value > 0 ? '+' : '';
    return html`<span class="chipVote ${className}"
      >${plusSign}${this.vote.value}</span
    >`;
  }

  computeClass(vote: Number, label?: DetailedLabelInfo) {
