...
@customElement('gr-diff-section-new')
export class GrDiffSectionNew extends LitElement {
  @queryAll('gr-diff-row-new')
  diffRows?: NodeListOf<GrDiffRowNew>;
  ...
  getDiffRows(): GrDiffRowNew[] {
    return [...this.querySelectorAll<GrDiffRowNew>('gr-diff-row-new')];
  }
  ...
  findRow(side: Side, lineNumber: LineNumber): GrDiffRowNew | undefined {
    return (
      this.querySelector<GrDiffRowNew>(
        `gr-diff-row-new.${side}-${lineNumber}`
      ) ?? undefined
    );
  }
...
declare global {
  interface HTMLElementTagNameMap {
    'gr-diff-section-new': GrDiffSectionNew;
  }
}
