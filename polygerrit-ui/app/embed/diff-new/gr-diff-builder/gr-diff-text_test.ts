...
import './gr-diff-text';
import {GrDiffTextNew} from './gr-diff-text';
...
suite('gr-diff-text test', () => {
  let element: GrDiffTextNew;

  setup(async () => {
    element = await fixture<GrDiffTextNew>(
      html`<gr-diff-text-new tabsize="4" linelimit="10"></gr-diff-text-new>`
    );
  });
