...
/** A subset of the GrDiffNew API that the cursor is using. */
...
export class GrDiffCursorNew implements GrDiffCursorApi {
  ...
  getTargetDiffRow(): GrDiffRowNew | undefined {
    if (this.diffRowTR?.parentElement?.tagName !== 'GR-DIFF-ROW') {
      return undefined;
    }
    return this.diffRowTR.parentElement as GrDiffRowNew;
  }
  ...
  getTargetDiffElement(): GrDiffNew | undefined {
    ...
    if (hostOwner?.host?.tagName === 'GR-DIFF') {
      return hostOwner.host as GrDiffNew;
    }
    return undefined;
  }
