...
import {GrDiffCursorNew} from './gr-diff-cursor';
...
suite('gr-diff-cursor tests', () => {
  let cursor: GrDiffCursorNew;
  let diffElement: GrDiffNew;
  let diff: DiffInfo;

  setup(async () => {
    diffElement = await fixture(html`<gr-diff-new></gr-diff-new>`);
    cursor = new GrDiffCursorNew();
  ...
  test('expand context updates stops', async () => {
    ...
    const controls = queryAndAssert(diffElement, 'gr-context-controls-new');
  ...
  suite('multi diff', () => {
    let diffElements: GrDiffNew[];

    setup(async () => {
      diffElements = [
        await fixture(html`<gr-diff-new></gr-diff-new>`),
        await fixture(html`<gr-diff-new></gr-diff-new>`),
        await fixture(html`<gr-diff-new></gr-diff-new>`),
      ];
      cursor = new GrDiffCursorNew();
