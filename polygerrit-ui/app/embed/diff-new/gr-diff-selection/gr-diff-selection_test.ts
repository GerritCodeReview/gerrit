...
import {GrDiffSelectionNew} from './gr-diff-selection';
...
suite('gr-diff-selection', () => {
  let element: GrDiffSelectionNew;
  let diffTable: HTMLElement;
  let grDiff: GrDiffNew;
  ...
  setup(async () => {
    grDiff = await fixture<GrDiffNew>(html`<gr-diff-new></gr-diff-new>`);
  ...
  test('copies content correctly', () => {
    ...
    const texts = diffTable.querySelectorAll<HTMLElement>('gr-diff-text-new');
