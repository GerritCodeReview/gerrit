...
import {
  ...
  FULL_CONTEXT,
  diffClasses,
  DiffContextExpandedEventDetail as DiffContextExpandedEventDetailNew,
  GrDiffCommentThread,
} from './gr-diff-utils';
import {BlameInfo, CommentRange, ImageInfo} from '../../../types/common';
import {DiffInfo, DiffPreferencesInfo} from '../../../types/diff';
import {GrDiffHighlightNew} from '../gr-diff-highlight/gr-diff-highlight';
...
import {GrDiffSelectionNew} from '../gr-diff-selection/gr-diff-selection';
import {
  customElement,
  property,
  query,
...
import {GrDiffRowNew} from '../gr-diff-builder/gr-diff-row';
import '../gr-diff-builder/gr-diff-section';
import {GrDiffSectionNew} from '../gr-diff-builder/gr-diff-section';
import {repeat} from 'lit/directives/repeat.js';
import {DiffContextExpandedEventDetail} from '../../diff/gr-diff-builder/gr-diff-builder';
...
@customElement('gr-diff-new')
export class GrDiffNew extends LitElement implements GrDiffApi {
  /**
  ...
  @queryAll('gr-diff-section-new')
  diffSections?: NodeListOf<GrDiffSectionNew>;
  ...
  // Private but used in tests.
  diffSelection = new GrDiffSelectionNew();

  // Private but used in tests.
  highlights = new GrDiffHighlightNew();
  ...
  constructor() {
    ...
    this.addEventListener('diff-context-expanded', (e: CustomEvent) =>
      this.onDiffContextExpanded(
        e as CustomEvent<DiffContextExpandedEventDetailNew>
      )
    );
  ...
  private commentThreadRedispatcher = (
    ...
    const model = toCommentThreadModel(target);
    if (model) fire(target, eventName, model);
  };
  ...
  private onDiffContextExpanded = (
    e: CustomEvent<DiffContextExpandedEventDetailNew>
  ) => {
  ...
  private getDiffRows() {
    assertIsDefined(this.diffTable, 'diffTable');
    const sections = [
      ...this.diffTable.querySelectorAll<GrDiffSectionNew>(
        'gr-diff-section-new'
      ),
    ];
  ...
  private findSection(group: GrDiffGroup): GrDiffSectionNew | undefined {
    assertIsDefined(this.diffTable, 'diffTable');
    const leftClass = `left-${group.startLine(Side.LEFT)}`;
    const rightClass = `right-${group.startLine(Side.RIGHT)}`;
    return (
      this.diffTable.querySelector<GrDiffSectionNew>(
        `gr-diff-section-new.${leftClass}.${rightClass}`
      ) ?? undefined
    );
  }
  ...
  // TODO: Remove this legacy method. Just keeping around for compatibility.
  cancel() {}
...
declare global {
  ...
  interface HTMLElementEventMap {
    ...
    'diff-context-expanded': CustomEvent<
      DiffContextExpandedEventDetail | DiffContextExpandedEventDetailNew
    >;
