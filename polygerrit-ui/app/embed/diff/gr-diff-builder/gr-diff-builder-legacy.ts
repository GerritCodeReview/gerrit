...
export abstract class GrDiffBuilderLegacy extends GrDiffBuilder {
  ...
  override getBlameTdByLine(lineNum: number): Element | undefined {
    return (
      this._outputEl.querySelector(`td.blame[data-line-number="${lineNum}"]`) ??
      undefined
    );
  }
  ...
  override _renderContentByRange(
    start: LineNumber,
    end: LineNumber,
    side: Side
  ) {
  ...
  override _renderBlameByRange(blame: BlameInfo, start: number, end: number) {
   ...
   */
  _createBlameCell(lineNumber: LineNumber): HTMLTableCellElement {
    ...
    const blameInfo = this._getBlameCommitForBaseLine(lineNumber);
