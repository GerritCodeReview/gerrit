...
export abstract class GrDiffBuilder implements DiffBuilder {
  ...
  protected readonly _numLinesLeft: number;
  ...
  abstract getBlameTdByLine(lineNum: number): Element | undefined;

  abstract getContentByLine(
    lineNumber: LineNumber,
    side?: Side,
    root?: HTMLElement
  ): HTMLElement | null;
   ...
   * diff content.
   */
  abstract _renderContentByRange(
    start: LineNumber,
    end: LineNumber,
    side: Side
  ): void;
  ...
  setBlame(blame: BlameInfo[] | null) {
    ...
    for (const commit of blame) {
      for (const range of commit.ranges) {
        this._renderBlameByRange(commit, range.start, range.end);
      }
    }
  }
