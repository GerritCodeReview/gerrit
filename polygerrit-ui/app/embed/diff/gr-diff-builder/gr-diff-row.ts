...
export class GrDiffRow extends LitElement {
  ...
  override render() {
    ...
    const isUnified = this.viewMode === DiffViewMode.UNIFIED;
    const classes = isUnified ? ['unified'] : ['side-by-side'];
    const unifiedType = this.unifiedType();
    if (isUnified && unifiedType) classes.push(unifiedType);
    const row = html`
      <tr
        ${ref(this.tableRowRef)}
        class=${diffClasses('diff-row', ...classes)}
        left-type=${ifDefined(this.type(Side.LEFT))}
        right-type=${ifDefined(this.type(Side.RIGHT))}
        tabindex="-1"
        aria-labelledby=${this.ariaLabelIds().join(' ')}
      >
  ...
  private ariaLabelIds() {
    ...
    if (!isUnified) ids.push(this.contentId(Side.LEFT));
    ids.push(this.lineNumberId(Side.RIGHT));
    if (!isUnified) ids.push(this.contentId(Side.RIGHT));
    if (isUnified) ids.push(this.contentId(this.unifiedSide()));
  ...
  private renderLineNumberCell(side: Side): TemplateResult {
    ...
    if (!line || !lineNumber || isBlank || this.layersApplied) {
      const blankClass =
        isBlank && this.viewMode === DiffViewMode.SIDE_BY_SIDE
          ? 'blankLineNum'
          : '';
      return html`<td
  ...
  private renderSignCell(side: Side) {
    if (this.viewMode === DiffViewMode.UNIFIED) return nothing;
    const line = this.line(side);
    assertIsDefined(line, 'line');
    const isBlank = line.type === GrDiffLineType.BLANK;
  ...
  private type(side?: Side): string | undefined {
    if (this.viewMode === DiffViewMode.UNIFIED) return undefined;
