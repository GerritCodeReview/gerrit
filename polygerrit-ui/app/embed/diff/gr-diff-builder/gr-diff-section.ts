...
export class GrDiffSection extends LitElement {
  ...
  // TODO: Refine this obviously simplistic approach to optimized rendering.
  // One known issue here is that when you step through Ctrl-F results, then the
  // browser jumps to a result, highlights it, then it is re-rendered and the
  // browser has lost the highlighting.
  private renderInvisible() {
    ...
    console.log(`gr-diff-section renderInvisible ${pairs.length}`);
    return html`
      <tbody>
        ${pairs.map(
  ...
  override render() {
    ...
    const isControl = this.group.type === GrDiffGroupType.CONTEXT_CONTROL;
    const pairs = isControl ? [] : this.group.getSideBySidePairs();
    console.log(`gr-diff-section renderVisible ${pairs.length}`);
    const body = html`
