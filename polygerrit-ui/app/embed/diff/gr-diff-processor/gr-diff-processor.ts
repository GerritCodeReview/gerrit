...
export interface GroupConsumer {
  groupAdded(group: GrDiffGroup): void;
  groupsCleared(): void;
}
/**
...
export class GrDiffProcessor {
  ...
  process(chunks: DiffContent[], isBinary: boolean) {
    ...
    this.consumer.groupsCleared();
    this.consumer.groupAdded(this.makeGroup('LOST'));
    this.consumer.groupAdded(this.makeGroup(FILE));
      ...
      new Promise(resolve => {
        ...
        const nextStep = () => {
          ...
          for (const group of stateUpdate.groups) {
            assertIsDefined(this.consumer, 'consumer');
            this.consumer.groupAdded(group);
