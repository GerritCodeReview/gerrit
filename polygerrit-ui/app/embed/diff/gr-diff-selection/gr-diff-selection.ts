...
export class GrDiffSelection {
  ...
  handleDown = (e: Event) => {
    ...
    const blameSelected = this.elementDescendedFromClass(target, 'blame');
    ...
    if (blameSelected) {
      ...
      const commentSelected = this.elementDescendedFromClass(
        target,
        'gr-comment'
      );
   ...
   */
  elementDescendedFromClass(element: Element, className: string) {
  ...
  handleCopy = (e: ClipboardEvent) => {
    ...
    if (!this.elementDescendedFromClass(target, 'diff-row')) return;
    if (!this.diffTable) return;
  ...
  getCommentLines(sel: Selection, side: Side) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (sel.containsNode(el, true)) {
        ...
        if (
          el.id === 'output' &&
          !this.elementDescendedFromClass(el, 'collapsed')
        ) {
