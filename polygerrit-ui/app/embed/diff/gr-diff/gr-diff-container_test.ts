...
suite('gr-diff-container tests', () => {
  let element: GrDiffContainer;
  let model: DiffModel;
  ...
  setup(async () => {
    ...
    element = (
      await fixture(
        wrapInProvider(
          html`<gr-diff-container></gr-diff-container>`,
          diffModelToken,
          model
        )
      )
    ).querySelector('gr-diff-container')!;
  });
  ...
  suite('trailing newline warnings', () => {
    ...
    const getWarning = (element: GrDiffContainer) => {
