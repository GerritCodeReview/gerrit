...
export function diffClasses(...additionalClasses: string[]) {
  return `class="style-scope gr-diff ${additionalClasses.join(' ')}"`;
}
...
export function createLineBreak(responsive: boolean) {
  return responsive
...
export function lineBreak(responsive: boolean) {
  return responsive
    ? html`<wbr ${diffClasses()}></wbr>`
    : html`<span ${diffClasses('br')}></span>`;
}
  ...
  for (const segment of text.split(REGEX_TAB_OR_SURROGATE_PAIR)) {
    if (segment) {
      ...
      while (rowEnd < segment.length) {
        ...
        contentText.appendChild(createLineBreak(responsive));
        columnPos = 0;
        rowStart = rowEnd;
        rowEnd += lineLimit;
      }
