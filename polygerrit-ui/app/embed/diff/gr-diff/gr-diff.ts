...
import {debounceP, DelayedCancelation, DelayedPromise} from '../../../utils/async-util';
...
function extractAddedNodes(mutations: MutationRecord[]) {
  ...
  for (const mutation of mutations) {
    result.push(...mutation.addedNodes);
  }
  return result;
}
...
function extractRemovedNodes(mutations: MutationRecord[]) {
  const result = [];
  for (const mutation of mutations) {
    result.push(...mutation.removedNodes);
  }
  return result;
}
...
export class GrDiff extends LitElement implements GrDiffApi {
  ...
  @state()
  private commentRanges: CommentRangeLayer[] = [];
  ...
  private addedSelectionListeners = false;
    ...
    return [
      ...
      css`
        ...
        /*
          ...
          mode (check GrDiff.updatePreferenceStyles).
          */
        .section {
  ...
  constructor() {
    ...
    this.addEventListener('moved-link-clicked', e => this.movedLinkClicked(e));
  }
  ...
  protected override willUpdate(changedProperties: PropertyValues): void {
    if (
      ...
      (changedProperties.has('prefs') &&
        !deepEqual(this.prefs, changedProperties.get('prefs')))
    ) {
      this.prefsChanged();
    }
  ...
  private renderHeader() {
    ...
    return html`
      <div id="diffHeader" ?hidden=${diffheaderItems.length === 0}>
  ...
  private renderContainer() {
    return html`
      <div class=${this.computeContainerClass()} @click=${this.handleTap}>
        <table
          id="diffTable"
  ...
  private addSelectionListeners() {
    if (this.addedSelectionListeners) return;
    this.addedSelectionListeners = true;
  ...
  private removeSelectionListeners() {
    ...
    this.addedSelectionListeners = false;
  }
  ...
  private computeContainerClass() {
    const classes = ['diffContainer'];
    if (this.viewMode === DiffViewMode.UNIFIED) classes.push('unified');
    if (this.viewMode === DiffViewMode.SIDE_BY_SIDE) classes.push('sideBySide');
    if (getHiddenScroll()) classes.push('hiddenscroll');
    if (this.loggedIn) classes.push('canComment');
    if (this.displayLine) classes.push('displayLine');
    return classes.join(' ');
  }
  ...
  private movedLinkClicked(e: MovedLinkClickedEvent) {
  ...
  private lineOfInterestObserver() {
  ...
  private debounceRenderDiffTable() {
    ...
    this.renderDiffTableTask.catch(e => {
      if (e instanceof DelayedCancelation) return;
      console.error(e);
    });
  }
