...
export const htmlTemplate = html`
  <style include="shared-styles">
    ...
    .diff-row.unified.target-row.target-side-left td.right .lineNumButton{
    ...
    /* Increase prominency by using the primary text color*/
    .diff-row.target-row.target-side-left td.left.lineNum .lineNumButton,
    .diff-row.target-row.target-side-right td.right.lineNum .lineNumButton{
    ...
    /**
    * Defines specific focus behavior for unified diff - don't interupt the outline
    * (top/bottom borders) for right line number cell even if selection is on the left side.
    *
    * Not using .lineNum here, because we also want to target cells in right line number
    * column that don't contain an actual line number.
    */
    .diff-row.unified.target-row.target-side-left td.right {
      padding: 1px 0;
      /* remove 2px for button height to make sure added padding (1px top, 1px bottom) won't affect overall row height */
      line-height: calc(var(--line-height-code) - 2px);
      box-shadow:
    ...
    .diff-row.unified.target-row.target-side-left td.content {
      box-shadow: 
