...
export class GrRangedCommentLayer implements DiffLayer {
  ...
  annotate(el: HTMLElement, _: HTMLElement, line: GrDiffLine) {
    ...
    if (
      ...
      console.log(`annotate left ${line.beforeNumber} ${ranges.length}`);
    }
    if (
      ...
      console.log(`annotate right ${line.afterNumber} ${ranges.length}`);
    }
    this.printMap();
  ...
  private printMap() {
    for (const side of Object.keys(this.rangesMap)) {
      for (const line of Object.keys(this.rangesMap[side as Side])) {
        for (const range of this.rangesMap[side as Side][
          line as unknown as number
        ]) {
          console.log(`range ${side} ${line} ${JSON.stringify(range)}`);
        }
      }
    }
  }
  ...
  private notifyUpdateRange(start: number, end: number, side: Side) {
    console.log(`notifyUpdateRange ${side} ${start} ${end}`);
  ...
  private addRange(commentRange: CommentRangeLayer) {
    console.log(`add range ${commentRange.rootId}`);
  ...
  private removeRange(commentRange: CommentRangeLayer) {
    console.log(`remove range ${commentRange.rootId}`);
    const {side, range, rootId, hovering} = commentRange;
    this.updateRangesMap({
      side,
      range,
      ...
      operation: forLine => {
        ...
        console.log(`remove range splice ${commentRange.rootId}`);
