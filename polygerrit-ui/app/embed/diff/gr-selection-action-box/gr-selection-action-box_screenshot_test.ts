...
suite('gr-selection-action-box screenshot tests', () => {
  ...
  setup(async () => {
    container = await fixture<HTMLDivElement>(html`
      <div style="position: relative; height: 100px; width: 300px; background-color: var(--background-color-primary);">
        <!-- eslint-disable-next-line lit/prefer-static-styles -->
        <style>
          ${grDiffElementStyles}
          gr-selection-action-box .menu {
  ...
  test('renders with one item', async () => {
    await visualDiff(element, 'gr-selection-action-box-one-item');
    await visualDiffDarkTheme(element, 'gr-selection-action-box-one-item');
  });
});
