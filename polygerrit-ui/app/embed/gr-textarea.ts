...
export class GrTextarea extends LitElement implements GrTextareaApi {
  ...
  private setCursorPositionForDiv(
    ...
    console.log(
      `${
        Date.now() % 100000
      } asdf gr setCursorPosition ${position} ${editableDivElement} ${selection}`
    );
    if (!editableDivElement || !selection) {
      console.log(`${Date.now() % 100000} asdf gr setCursorPosition RETURN`);
      return;
    }
    editableDivElement.focus();
  ...
  private setFocusOnNode(
    ...
    console.log(
      `${Date.now() % 100000} asdf gr setFocusOnNode '${
        editableDivElement.innerHTML
      }' ${editableDivElement.childNodes.length}`
    );
    const range = document.createRange();
  ...
  private async onInput(event: Event) {
    ...
    console.log(`${Date.now() % 100000} asdf onInput ${value}`);
  ...
  private addHintSpanAtEndOfContent(editableDivElement: Node, hint: string) {
    console.log(
      `${Date.now() % 100000} asdf addHintSpanAtEndOfContent ${hint}`
    );
  ...
  private async updateValueInDom() {
    ...
    if (editableDivElement) {
      console.log(
        `${Date.now() % 100000} asdf updateValueInDom '${this.value}'`
      );
  ...
  private getCursorPositionForDiv(editableDivElement?: HTMLDivElement) {
    const selection = this.getSelection();
    console.log(`${Date.now() % 100000} asdf gr getCursorPositionForDiv`);
