...
export const HovercardMixin = <T extends Constructor<LitElement>>(
  ...
  class Mixin extends superClass {
    ...
    constructor(...args: any[]) {
      ...
      keyboardController.addGlobal({key: Key.ESC}, () => this.hide(true));
    }
    ...
    private addTargetEventListeners() {
      ...
      if (this._target) {
        this.targetCleanups.push(
          addShortcut(this._target, {key: Key.ENTER}, (_e: KeyboardEvent) => {
            this.show(true);
          })
        );
        this.targetCleanups.push(
          addShortcut(this._target, {key: Key.SPACE}, (_e: KeyboardEvent) => {
            this.show(true);
          })
        );
      }
