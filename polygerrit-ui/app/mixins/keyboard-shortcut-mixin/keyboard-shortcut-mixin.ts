...
interface CustomKeyboardEvent extends CustomEvent, EventApi {
    ...
    _help.forEach((shortcutList, section) => {
      shortcutList.forEach(shortcutHelp => {
        if (activeShortcuts.has(shortcutHelp.shortcut)) {
          ...
          const sectionBindings = activeShortcutsBySection.get(section);
          if (sectionBindings) {
            sectionBindings.push(shortcutHelp);
          }
        }
      });
    });
...
const InternalKeyboardShortcutMixin = dedupingMixin(
  (
    superClass: PolymerElementConstructor &
      ElementMixinConstructor &
      IronA11yKeysMixinConstructor
  ) => {
    /**
     * @polymer
     * @mixinClass
    ...
    class Mixin extends superClass {
      ...
      constructor() {
        super();
        this._shortcut_go_table = new Map();
        this._shortcut_v_table = new Map();
      }
      ...
      protected isModifierPressed(e: CustomKeyboardEvent, modifier: Modifier) {
        return getKeyboardEvent(e)[modifier];
      }

      protected shouldSuppressKeyboardShortcut(event: CustomKeyboardEvent) {
        const e = getKeyboardEvent(event);
      ...
      protected getKeyboardEvent(e: CustomKeyboardEvent) {
...
export const KeyboardShortcutMixin = (
  superClass: PolymerElementConstructor &
    ElementMixinConstructor &
    IronA11yKeysMixinConstructor
) =>
