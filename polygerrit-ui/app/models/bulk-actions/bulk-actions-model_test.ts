...
suite('bulk actions model test', () => {
  ...
  suite('mark as wip', () => {
    ...
    setup(async () => {
      ...
      detailedActionsStub.returns(
        Promise.resolve([
          {...c1, actions: {wip: {method: HttpMethod.POST, enabled: true}}},
          {...c2},
        ])
      );
    ...
    test('skips markAsWip on changes that do not support it', () => {
      const actionStub = stubRestApi('executeChangeAction').resolves();
  ...
  suite('mark as ready', () => {
    ...
    setup(async () => {
      ...
      detailedActionsStub.returns(
        Promise.resolve([
          {...c1, actions: {ready: {method: HttpMethod.POST, enabled: true}}},
          {...c2},
        ])
      );
    ...
    test('skips markAsReady on changes that do not support it', () => {
      const actionStub = stubRestApi('executeChangeAction').resolves();
