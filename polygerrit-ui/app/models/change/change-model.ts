...
export class ChangeModel extends Model<ChangeState> {
  ...
  async fetchChangeUpdates(change: ChangeInfo | ParsedChangeInfo) {
    ...
    // getChange to avoid passing all the options, we need to call
    // We need the messages grouped so that we properly compare the message length
    ...
    const parsedChange = GrReviewerUpdatesParser.parse(detail)!;
