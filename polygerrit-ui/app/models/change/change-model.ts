...
export function updateRevisionsWithCommitShas(changeIn?: ParsedChangeInfo) {
  if (!changeIn?.revisions) return changeIn;
  const changeOut = {...changeIn, revisions: {...changeIn.revisions}};
  for (const sha of Object.keys(changeOut.revisions)) {
    const revision = changeOut.revisions[sha];
    if (sha && revision?.commit && !revision.commit.commit) {
      changeOut.revisions[sha] = {
        ...revision,
