...
import {Action, Actions, AiCodeReviewProvider, ChatRequest, ChatResponse, ChatResponseListener, ChatResponsePart, ContextItem, ContextItemType, Conversation, ConversationTurn, CreateCommentAction, ModelInfo, Models, Reference} from '../../api/ai-code-review';
import {FileRangeSelection, TextRange} from '../../api/diff';
...
declare interface ResponsePartBase {
...
export declare interface CreateCommentPart extends ResponsePartBase {
  ...
  // Optional. If not set, the comment is added at the CL level.
  readonly filePath?: string;
  // Optional. If not set, but the depotPath is set, the comment is added at
  // the file level.
  readonly range?: TextRange;
  readonly patchsetNumber?: number;
...
interface ConvTurnId {
  conversationId: string;
  turnId: UniqueTurnId;
}

interface ConvTurnPartId extends ConvTurnId {
  partId: number;
}
...
export class ChatModel extends Model<ChatState> {
  readonly models$: Observable<readonly ModelInfo[]> =
      select(this.state$, chatState => chatState.models?.models ?? []);

  readonly defaultModelId$: Observable<string> = select(
      this.state$, chatState => chatState.models?.default_model_id ?? '');

  readonly availableModelsMap$: Observable<ReadonlyMap<string, ModelInfo>> =
      select(this.models$, models => {
        return new Map(models.map((model) => [model.model_id, model])) as
            ReadonlyMap<string, ModelInfo>;
      });

  readonly activeModel$: Observable<ModelInfo|undefined> = select(
      combineLatest([this.availableModelsMap$, this.defaultModelId$]),
      ([availableModelsMap, defaultModelId]) =>
          availableModelsMap.get(defaultModelId));

  readonly actions$: Observable<readonly Action[]> =
      select(this.state$, chatState => chatState.actions?.actions ?? []);

  readonly defaultActionId$: Observable<string|undefined> =
      select(this.state$, chatState => chatState.actions?.default_action_id);
  ...
  readonly contextItemTypes$: Observable<readonly ContextItemType[]> =
      select(this.state$, chatState => chatState.contextItemTypes ?? []);

  readonly turns$: Observable<readonly Turn[]|undefined> =
      select(this.state$, chatState => chatState.turns);

  readonly nextTurnIndex$: Observable<number> =
      select(this.turns$, turns => turns?.length ?? 0);

  readonly conversationId$: Observable<string|undefined> =
      select(this.state$, chatState => chatState.id);

  readonly mode$: Observable<ChatPanelMode> =
      select(this.state$, chatState => chatState.mode);

  readonly userInput$: Observable<string> =
      select(this.state$, chatState => chatState.draftUserMessage.content);
  ...
  regenerateMessage(turnId: UniqueTurnId) {
    ...
    this.sendChatRequest(turnId.turnIndex, 'button:regenerate_button');
  }
  ...
  chat(
    ...
    this.updateState({
      ...state,
      // It's possible that the conversation ID hasn't been set yet. This would
      // happen if this is the very first conversation.
    ...
    this.sendChatRequest(turnIndex, source);
  }
  ...
  sendChatRequest(turnIndex: number, source: string) {
    ...
    const listener: ChatResponseListener = {
      emitResponse: (response: ChatResponse) => {
        ...
        const geminiMessage: Partial<GeminiMessage> = {
          responseParts: extractResponseParts(
              response, {turnId, conversationId: state.id}),
      ...
      emitError: (errorMessage: string) => {
        const state = this.getState();
        if (state.id !== conversationId) return state;
  ...
  startNewChatWithPredefinedPrompt(
      source: string, actionId: string|undefined,
      contextItems: ContextItem[] = [], isBackgroundRequest = false) {
    const action = this.getAction(actionId);
    assertIsDefined(action, 'action');
    ...
    this.sendChatRequest(0, source);
  }

  startNewChatWithUserInput(
      userInput: string, source: string, actionId: string|undefined,
      contextItems: ContextItem[] = [], useCurrentContext = true) {
    const state = this.getState();
    const message: UserMessage = {
      userType: UserType.USER,
      content: userInput,
      actionId,
      contextItems: useCurrentContext ? state.draftUserMessage.contextItems :
                                        contextItems,
    };
  ...
  startEmptyNewChat(source: string, useCurrentContext: boolean) {
    const state = this.getState();
    const currentDraftUserMessage = state.draftUserMessage;
    const draftUserMessage = {
      ...initialConversationState.draftUserMessage,
      contextItems: useCurrentContext ? currentDraftUserMessage.contextItems :
                                        [],
    };
  ...
  listConversations() {
    ...
    return this.plugin?.listChatConversations?.(this.change)
  ...
  loadConversation(
    ...
    if (!this.change) return;
    return this.plugin?.getChatConversation?.(this.change, conversationId)
        .then((turns: ConversationTurn[]) => {
          const conversationState =
              stateFromConversationResponse(turns, conversationId);
          this.updateState({
            ...conversationState,
          });
        })
  ...
  getModels() {
    if (!this.change) return;
    return this.plugin?.getModels?.(this.change)
        .then((models: Models) => {
          this.updateState({
            models,
  ...
  getActions() {
    if (!this.change) return;
    return this.plugin?.getActions?.(this.change)
  ...
  getContextItemTypes() {
    return this.plugin?.getContextItemTypes?.()
...
function buildCommentCreationId({
  ...
  return `chat-panel-generated-comment:${conversationId}:${turnId.turnIndex}:${
      turnId.regenerationIndex}:${partId}`;
}
...
function asGeminiResponsePart(
  ...
  if (part.text) {
    ...
    return convertCreateCommentAction({
      action: part.create_comment_action,
      partId: part.id,
...
function convertCreateCommentAction(kwargs: {
  action: CreateCommentAction; partId: number; commentCreationId: string;
}): CreateCommentPart|undefined {
  return {
    type: ResponsePartType.CREATE_COMMENT,
    id: kwargs.partId,
    commentCreationId: kwargs.commentCreationId,
    content: kwargs.action?.comment_text ?? '',
    filePath: kwargs.action?.path ?? '',
    range: kwargs.action?.range,
    patchsetNumber: kwargs.action?.patchset,
    side: kwargs.action?.side,
  };
}
...
function stateFromConversationResponse(
  ...
  for (let index = 0; index < responseTurns.length; index++) {
    ...
    const clientData: ClientData =
        JSON.parse(userInput.client_data ?? '{}') as ClientData;
    ...
    const geminiMessage: GeminiMessage = {
      ...
      timestamp: turn.timestamp_millis ? new Date(turn.timestamp_millis) :
                                         undefined,
    };
