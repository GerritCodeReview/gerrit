...
import {TextRange} from '../../api/diff';
...
export class ChatModel extends Model<ChatState> {
  readonly models$: Observable<readonly ModelInfo[]> = select(
    this.state$,
    chatState => chatState.models?.models ?? []
  );

  readonly defaultModelId$: Observable<string> = select(
    this.state$,
    chatState => chatState.models?.default_model_id ?? ''
  );

  readonly selectedModelId$: Observable<string> = select(
    this.state$,
    chatState => chatState.selectedModelId ?? ''
  );
  ...
  readonly availableModelsMap$: Observable<ReadonlyMap<string, ModelInfo>> =
    select(
      this.models$,
      models =>
        new Map(models.map(model => [model.model_id, model])) as ReadonlyMap<
          string,
          ModelInfo
        >
    );
