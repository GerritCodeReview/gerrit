...
suite('chat-model tests', () => {
  ...
  test('regenerateMessage increments regenerationIndex when no error', () => {
    const turn: Turn = {
      userMessage: {content: 'hello', userType: UserType.USER, contextItems: []},
      geminiMessage: {userType: UserType.GEMINI, responseParts: [], regenerationIndex: 0, references: [], citations: []},
    };
    model.updateState({
      ...model.getState(),
      turns: [turn],
  ...
  test('regenerateMessage does not increment regenerationIndex when error exists', () => {
    const turn: Turn = {
      userMessage: {content: 'hello', userType: UserType.USER, contextItems: []},
      geminiMessage: {userType: UserType.GEMINI, responseParts: [], regenerationIndex: 0, references: [], citations: [], errorMessage: 'error'},
    };
    model.updateState({
      ...model.getState(),
      turns: [turn],
