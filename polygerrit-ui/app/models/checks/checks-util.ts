...
function getAiAgentEventDetails(
  ...
  try {
    ...
    if (!agentId || !conversationId || turnIndex === undefined) return;
    return {
      agentId,
      conversationId,
 ...
 */
export function reportGetAIFix(reporting: Reporting, runResult: RunResult) {
 ...
 */
export function reportCommentDraft(reporting: Reporting, runResult: RunResult) {
  const eventDetails = getAiAgentEventDetails(runResult);
  if (!eventDetails) return;
  reporting.reportInteraction(
