...
import {FlowsModel, SUBMIT_CONDITION, SUBMIT_ACTION_NAME} from './flows-model';
...
suite('flows-model tests', () => {
  ...
  test('enabled$ switches based on changeNum and API', async () => {
    const isEnabledStub = stubRestApi('getIfFlowsIsEnabled').resolves({
      enabled: true,
    });

    let enabled = false;
    flowsModel.enabled$.subscribe(e => (enabled = e));
    ...
    assert.isTrue(isEnabledStub.calledWith(123 as NumericChangeId));
  ...
  test('deleteFlow calls API and reloads', async () => {
    ...
    const deleteFlowStub = stubRestApi('deleteFlow').resolves(undefined as any);
  ...
  test('createFlow calls API and reloads', async () => {
    ...
    assert.isTrue(
      createFlowStub.calledWith(123 as NumericChangeId, flowInput)
    );
    assert.isTrue(reloadSpy.called);
  });
  ...
  test('createAutosubmitFlow uses provider values', async () => {
    ...
    const provider: FlowsAutosubmitProvider = {
      ...
      getSubmitAction: () => ({name: 'custom action'}),
    };
  ...
  test('isAutosubmitEnabled$ reflects providers', async () => {
    let isEnabled = false;
    flowsModel.isAutosubmitEnabled$.subscribe(e => (isEnabled = e));
    assert.isFalse(isEnabled);

    const provider: FlowsAutosubmitProvider = {
      isAutosubmitEnabled: () => true,
      getSubmitCondition: () => 'cond',
      getSubmitAction: () => ({name: 'act'}),
    };
    pluginsModel.registerFlowsAutosubmitProvider({
      pluginName: 'test',
      provider,
