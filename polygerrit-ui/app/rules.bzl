...
def compile_ts(name, srcs, ts_outdir, additional_deps = [], ts_project = "tsconfig_bazel.json", emitJs = True):
    ...
    native.genrule(
        ...
        cmd = " && ".join([
            "echo $(location {})".format(ts_project),
            "echo $(location {})".format((additional_deps[0] if len(additional_deps) > 0 else ts_project)),
            "pwd",
            "cat $(location {})".format(ts_project),
