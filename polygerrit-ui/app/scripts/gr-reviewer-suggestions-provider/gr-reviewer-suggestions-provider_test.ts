...
const changes: ChangeInfo[] = [
  ...
  {
    ...createChange(),
    _number: 43 as NumericChangeId,
    reviewers: {REVIEWER: [accounts[2]], CC: [accounts[2]]},
  },
];
...
suite('GrReviewerSuggestionsProvider tests', () => {
  ...
  test('only returns REVIEWER suggestions shared by all changes', async () => {
    getChangeSuggestedReviewersStub
      ...
      .onSecondCall()
      .resolves([suggestions[1], suggestions[2]]);
    provider = new GrReviewerSuggestionsProvider(
      getAppContext().restApiService,
      ReviewerState.REVIEWER,
      createServerInfo(),
    ...
    // change.
    // suggestions[3] is included because the second change has the suggestion
