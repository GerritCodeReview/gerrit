...
export function updateState(screenWidth: number) {
  privateState$.next({...privateState$.getValue(), screenWidth});
}

export const isScreenTooSmall$ = viewState$.pipe(
  map(state => state.screenWidth ?? 0 < MAX_UNIFIED_DEFAULT_WINDOW_WIDTH_PX),
  distinctUntilChanged()
);

export const diffView$: Observable<DiffViewMode> = combineLatest([
  isScreenTooSmall$,
