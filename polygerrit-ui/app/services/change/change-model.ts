...
export const changeAndRouterConsistent$ = combineLatest([
  ...
  tap(_ => console.log(`tap changeAndRouterConsistent$ before filter`)),
  filter(([routerState, changeState]) => {
    const changeNum = changeState.change?._number;
    const routerChangeNum = routerState.changeNum;
    console.log(`tap changeAndRouterConsistent$ filter: ${changeNum} ${routerChangeNum} ${changeNum === routerChangeNum}`)
    return changeNum === undefined || changeNum === routerChangeNum;
  }),
  tap(_ => console.log(`tap changeAndRouterConsistent$ after filter`)),
...
export const latestPatchNum$ = change$.pipe(
  ...
  distinctUntilChanged(),
  tap(p => console.log(`tap latestPatchNum ${p}`))
);
...
> = changeAndRouterConsistent$.pipe(
  tap(_ => console.log(`tap currentPatchNum$ changeAndRouterConsistent$`)),
  withLatestFrom(routerPatchNum$, latestPatchNum$),
  map(([_, routerPatchNum, latestPatchNum]) => {
    console.log(`tap currentPatchNum$ ${routerPatchNum} ${latestPatchNum}`)
