...
import {addDraftProp, addPath, DraftInfo} from '../../utils/comment-util';
...
export function updateStateSetDraft(draft: DraftInfo) {
  ...
  if (!draft.__draft) throw new Error('draft is not a draft');
  if (!draft.__draftID && !draft.id) throw new Error('draft has no id');
