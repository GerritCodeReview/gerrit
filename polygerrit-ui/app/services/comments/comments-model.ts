...
export function reset(_state: CommentState, _val?: unknown): CommentState {
  return {...initialState};
}
...
export function setRobotComments(
  ...
  const nextState = {...state};
  if (deepEqual(robotComments, nextState.robotComments)) return state;
...
export function setDrafts(
  ...
  const nextState = {...state};
  if (deepEqual(drafts, nextState.drafts)) return state;
...
export function setPortedComments(
  ...
  const nextState = {...state};
  if (deepEqual(portedComments, nextState.portedComments)) return state;
...
export function setPortedDrafts(
  ...
  const nextState = {...state};
  if (deepEqual(portedDrafts, nextState.portedDrafts)) return state;
...
export class CommentsModel implements Finalizable {
  ...
  reloadPortedComments(
    ...
    return this.restApiService
      ...
      .then(portedComments =>
        this.updateState(setPortedComments, portedComments)
      );
  }
  ...
  reloadPortedDrafts(
    ...
    return this.restApiService
      ...
      .then(portedDrafts => this.updateState(setPortedDrafts, portedDrafts));
  }
  ...
  async restoreDraft(id: UrlEncodedCommentId) {
    ...
    this.updateState(deleteDiscardedDraft, id);
  }
  ...
  async saveDraft(draft: DraftInfo | UnsavedInfo, showToast = true) {
    ...
    this.updateState(setDraft, updatedDraft);
  ...
  async discardDraft(draftId: UrlEncodedCommentId) {
    ...
    this.showEndRequest();
    this.updateState(deleteDraft, draft);
