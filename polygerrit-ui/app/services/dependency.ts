 ...
 * ```
 *
 * Providers may not provide undefined as value.  Undefined is meant to
 * represent that a value has not been provided yet.
 *
 * Lifetime guarantees
 * ---
  ...
  implements ReactiveController, ResolvedDependency<T>
{
  ...
  hostConnected() {
    ...
    if (!this.resolved) {
      throw new DependencyError(
        this.dependency, 'Could not resolve dependency');
    }
  }
