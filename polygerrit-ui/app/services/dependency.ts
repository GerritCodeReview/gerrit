 ...
 * ```
 *
 * Providers may provide null but not undefined as value.  Undefined is
 * meant to represent that a value has not been provided yet.
 *
 * Lifetime guarantees
 * ---
 ...
 */
export interface ResolvedDependency<T> {
  get(): T;
  wait(): Promise<T>;
}
...
/**
 * A consumer of a service will resolve a given dependency token.
 */
...
class DependencyProvider<T>
  ...
  private readonly fullfill = (
    ev: DependencyRequestEvent<unknown>
  ) => {
...
export function provide<T>(
  ...
  host.addController(new DependencyProvider<T>(host, dependency, factory));
}
...
export class DIPolymerElement
  ...
  removeController(controller: ReactiveController) {
    ...
    // splice do nothing.
    this.___controllers?.splice(
      this.___controllers.indexOf(controller) >>> 0,
      1);
  }
