...
suite('gr-auth', () => {
  ...
  suite('default (xsrf token header)', () => {
    ...
    test('GET', async () => {
      ...
      const [url, options] = (fetch as sinon.SinonStub).lastCall.args;
  ...
  suite('cors (access token)', () => {
    ...
    test('base url support', async () => {
      ...
      const [url] = (fetch as sinon.SinonStub).lastCall.args;
    ...
    test('getToken refreshes token', async () => {
      ...
      const [[firstUrl], [secondUrl]] = (fetch as sinon.SinonStub).args;
