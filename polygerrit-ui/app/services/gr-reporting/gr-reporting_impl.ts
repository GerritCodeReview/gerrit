...
export function initErrorReporter(appContext: AppContext) {
  ...
  const onError = function (
    ...
    reportingService.error(error, 'onError', {line, column, url, msg});
    return true;
  };
  ...
  const catchErrors = function (opt_context?: any) {
    ...
    context.addEventListener(
      ...
      (e: PromiseRejectionEvent) => {
        reportingService.error(e.reason, 'unhandledrejection');
      }
    );
  };
...
export class GrReporting implements ReportingService {
  ...
  error(err: Error | unknown, errorSource?: string, details?: EventDetails) {
    let msg = '';
