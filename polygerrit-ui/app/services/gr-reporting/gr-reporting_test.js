...
suite('gr-reporting tests', () => {
  ...
  test('dedup', () => {
    assert.isFalse(service.dedup('a', undefined, undefined));
    assert.isFalse(service.dedup('a', undefined, undefined));

    let deduping = Deduping.EVENT_ONCE_PER_SESSION;
    assert.isFalse(service.dedup('b', {x: 'foo'}, deduping));
    assert.isTrue(service.dedup('b', {x: 'foo'}, deduping));
    assert.isTrue(service.dedup('b', {x: 'bar'}, deduping));

    deduping = Deduping.DETAILS_ONCE_PER_SESSION;
    assert.isFalse(service.dedup('c', {x: 'foo'}, deduping));
    assert.isTrue(service.dedup('c', {x: 'foo'}, deduping));
    assert.isFalse(service.dedup('c', {x: 'bar'}, deduping));
    assert.isTrue(service.dedup('c', {x: 'bar'}, deduping));

    deduping = Deduping.EVENT_ONCE_PER_CHANGE;
    service.setChangeId(1);
    assert.isFalse(service.dedup('d', {x: 'foo'}, deduping));
    assert.isTrue(service.dedup('d', {x: 'foo'}, deduping));
    assert.isTrue(service.dedup('d', {x: 'bar'}, deduping));
    ...
    service.setChangeId(1);
    assert.isFalse(service.dedup('e', {x: 'foo'}, deduping));
    assert.isTrue(service.dedup('e', {x: 'foo'}, deduping));
    assert.isFalse(service.dedup('e', {x: 'bar'}, deduping));
    assert.isTrue(service.dedup('e', {x: 'bar'}, deduping));
    service.setChangeId(2);
    assert.isFalse(service.dedup('e', {x: 'foo'}, deduping));
    assert.isTrue(service.dedup('e', {x: 'foo'}, deduping));
    assert.isFalse(service.dedup('e', {x: 'bar'}, deduping));
