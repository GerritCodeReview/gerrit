...
suite('gr-rest-api-service-impl tests', () => {
  ...
  suite('queryAccounts', () => {
    ...
    test('url with email and canSee changeId', () => {
      element.queryAccounts('bro', undefined, 341682 as NumericChangeId);
      assert.isTrue(fetchStub.calledOnce);
      assert.equal(
        fetchStub.firstCall.args[0].url,
    ...
    test('url with email and canSee changeId and isActive', () => {
      element.queryAccounts('bro', undefined, 341682 as NumericChangeId, true);
      assert.isTrue(fetchStub.calledOnce);
      assert.equal(
        fetchStub.firstCall.args[0].url,
