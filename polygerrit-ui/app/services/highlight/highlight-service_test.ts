...
import {HljsWorkerMessage, HljsWorkerResult} from '../../types/worker-api';
import {HighlightService} from './highlight-service';

class FakeWorker implements Worker {
  messages: HljsWorkerMessage[] = [];

  constructor(private readonly autoRespond = true) {}

  postMessage(message: HljsWorkerMessage) {
    this.messages.push(message);
    if (this.autoRespond) this.sendResult({ranges: []});
  }

  sendResult(result: HljsWorkerResult) {
    if (this.onmessage)
      this.onmessage({data: result} as MessageEvent<HljsWorkerResult>);
  }

  onmessage: ((e: MessageEvent<HljsWorkerResult>) => any) | null = null;
...
export class MockHighlightService extends HighlightService {
  ...
  sendToAll(result: HljsWorkerResult) {
