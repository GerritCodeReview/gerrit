...
import {DiffPreferencesInfo as DiffPreferencesInfoAPI, DiffViewMode} from '../../api/diff';
...
export class UserModel implements Finalizable {
  private readonly privateState$: BehaviorSubject<UserState> = new BehaviorSubject({
    preferences: createDefaultPreferences(),
    diffPreferences: createDefaultDiffPrefs(),
  });

  readonly account$: Observable<AccountDetailInfo | undefined>;

  /** Note that this may still be true, even if credentials have expired. */
  readonly loggedIn$: Observable<boolean>;
  readonly preferences$: Observable<PreferencesInfo>;
  readonly diffPreferences$: Observable<DiffPreferencesInfo>;
  readonly preferenceDiffViewMode$: Observable<DiffViewMode>;

  private readonly subscriptions: Subscription[] = [];

  get userState$(): Observable<UserState> { return this.privateState$; }

  constructor(readonly restApiService: RestApiService) {
    this.account$ = this.privateState$.pipe(
      map(userState => userState.account),
      distinctUntilChanged()
    );
    this.loggedIn$ = this.account$.pipe(
      map(account => !!account),
      distinctUntilChanged()
    );
    this.preferences$ = this.privateState$.pipe(
      map(userState => userState.preferences),
      distinctUntilChanged()
    );
    this.preferenceDiffViewMode$ = this.preferences$.pipe(
      map(preference => preference.diff_view ?? DiffViewMode.SIDE_BY_SIDE),
      distinctUntilChanged()
    );
    this.diffPreferences$ = this.privateState$.pipe(
