...
declare global {
  interface Window {
    ...
    fixture(fixtureId: string, model: unknown): void;
    stub<T extends keyof HTMLElementTagNameMap>(
      tagName: T,
      implementation: {[methodName: string]: Function}
    ): void;
  }
  let assert: typeof chai.assert;
  let expect: typeof chai.expect;
  let stub: typeof window.stub;
}
...
window.fixture = function (fixtureId: string, model) {
...
window.stub = function <T extends keyof HTMLElementTagNameMap>(
  tagName: T,
  implementation: {[methodName: string]: (...args: unknown[]) => unknown}
) {
  // This method is inspired by web-component-tester method
  const proto = document.createElement(tagName).constructor.prototype;
  const stubs = Object.keys(implementation).map(key =>
    sinon.stub(proto, key).callsFake(implementation[key])
  );
  registerTestCleanup(() => {
    stubs.forEach(stub => {
      stub.restore();
