...
import {GrAdminGroupList} from '../../../../elements/admin/gr-admin-group-list/gr-admin-group-list';
...
export class GrAdminGroupListCheck extends GrAdminGroupList
{
  templateCheck()
  {
    {
      ...
      el.items = this._groups;
      el.itemsPerPage = this._groupsPerPage;
    ...
    }
    {
      const el: HTMLElementTagNameMap['th'] = null!;
      useVars(el);
      el.setAttribute('class', `visibleToAll topHeader`);
    }
    {
      const el: HTMLElementTagNameMap['tr'] = null!;
      useVars(el);
    ...
    }
    {
      ...
      for(const item of this._shownGroups!)
      {
        {
          const el: HTMLElementTagNameMap['tr'] = null!;
          useVars(el);
        ...
        }
        {
          ...
          useVars(el);
          el.setAttribute('href', `${this._computeGroupUrl(__f(item)!.id)}`);
        }
...

        {
          const el: HTMLElementTagNameMap['td'] = null!;
          useVars(el);
          el.setAttribute('class', `visibleToAll`);
        }
        setTextContent(`${this._visibleToAll(item)}`);
    ...
    }
    {
      ...
      el.disabled = !this._hasNewGroupName;
      el.confirmLabel = `Create`;
      el.confirmOnEnter = true;
      el.addEventListener('confirm', this._handleCreateGroup.bind(this));
    ...
    {
      const el: HTMLElementTagNameMap['gr-create-group-dialog'] = null!;
      useVars(el);
      el.hasNewGroupName = this._hasNewGroupName;
      this._hasNewGroupName = el.hasNewGroupName;
