...
import {GrAccountDropdown} from '../../../../elements/core/gr-account-dropdown/gr-account-dropdown';
...
export class GrAccountDropdownCheck extends GrAccountDropdown
{
  templateCheck()
  {
    {
      ...
      el.link = true;
      el.items = this.links;
      el.topContent = this.topContent;
      el.addEventListener('tap-item-shortcuts', this._handleShortcutsTap.bind(this));
      el.horizontalAlign = `right`;
    }
    {
      const el: HTMLElementTagNameMap['span'] = null!;
      useVars(el);
      el.setAttribute('hidden', `${this._hasAvatars}`);
    }
    setTextContent(`${this._accountName(this.account)}`);

    {
      const el: HTMLElementTagNameMap['gr-avatar'] = null!;
      useVars(el);
      el.account = this.account;
      el.setAttribute('hidden', `${!this._hasAvatars}`);
    }
  }
}
