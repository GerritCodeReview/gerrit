...
import {GrErrorManager} from '../../../../elements/core/gr-error-manager/gr-error-manager';
...
export class GrErrorManagerCheck extends GrErrorManager
{
  templateCheck()
  {
    {
      ...
      useVars(el);
      el.setAttribute('id', `errorOverlay`);
    }
    {
      const el: HTMLElementTagNameMap['gr-error-dialog'] = null!;
      useVars(el);
      el.setAttribute('id', `errorDialog`);
      el.addEventListener('dismiss', this._handleDismissErrorDialog.bind(this));
      el.loginUrl = this.loginUrl;
    }
    {
      const el: HTMLElementTagNameMap['gr-overlay'] = null!;
      useVars(el);
      el.setAttribute('id', `noInteractionOverlay`);
    }
  }
}
