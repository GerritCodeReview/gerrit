...
import {GrAgreementsList} from '../../../../elements/settings/gr-agreements-list/gr-agreements-list';
...
export class GrAgreementsListCheck extends GrAgreementsList
{
  templateCheck()
  {
    {
    ...
    }
    {
      const el: HTMLElementTagNameMap['th'] = null!;
      useVars(el);
      el.setAttribute('class', `descriptionColumn`);
    }
    {
    ...
    }
    {
      ...
      for(const item of this._agreements!)
      {
        {
          const el: HTMLElementTagNameMap['tr'] = null!;
          useVars(el);
        ...
        }
        {
          ...
          useVars(el);
          el.setAttribute('href', `${this.getUrlBase(__f(item)!.url)}`);
        }
        setTextContent(`
                ${__f(item)!.name}
              `);
    ...
    }
    {
      const el: HTMLElementTagNameMap['a'] = null!;
      useVars(el);
      el.setAttribute('href', `${this.getUrl()}`);
    }
  }
}
