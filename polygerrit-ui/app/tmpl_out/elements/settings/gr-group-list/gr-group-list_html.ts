...
import {GrGroupList} from '../../../../elements/settings/gr-group-list/gr-group-list';
...
export class GrGroupListCheck extends GrGroupList
{
  templateCheck()
  {
    {
    ...
    }
    {
      const el: HTMLElementTagNameMap['th'] = null!;
      useVars(el);
      el.setAttribute('class', `descriptionHeader`);
    }
    {
      const el: HTMLElementTagNameMap['th'] = null!;
      useVars(el);
      el.setAttribute('class', `visibleCell`);
    }
    {
    ...
    }
    {
      ...
      for(const item of this._groups!)
      {
        ...
        }
        {
          ...
          useVars(el);
          el.setAttribute('class', `nameColumn`);
        }
        {
          const el: HTMLElementTagNameMap['a'] = null!;
          useVars(el);
          el.setAttribute('href', `${this._computeGroupPath(item)}`);
        }
        ...
        setTextContent(`${this._computeVisibleToAll(item)}`);
