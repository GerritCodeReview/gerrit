...
import {GrAutocompleteDropdown} from '../../../../elements/shared/gr-autocomplete-dropdown/gr-autocomplete-dropdown';
...
export class GrAutocompleteDropdownCheck extends GrAutocompleteDropdown
{
  templateCheck()
  {
    {
      ...
      {
        {
          ...
          el.setAttribute('dataValue', `${__f(item)!.dataValue}`);
          el.setAttribute('ariaLabel', `${__f(item)!.name}`);
          el.setAttribute('class', `autocompleteOption`);
          el.addEventListener('click', e => this._handleClickItem.bind(this, wrapInPolymerDomRepeatEvent(e, item))());
        }
        {
          const el: HTMLElementTagNameMap['span'] = null!;
          useVars(el);
        }
        setTextContent(`${__f(item)!.text}`);

        {
          const el: HTMLElementTagNameMap['span'] = null!;
          useVars(el);
          el.setAttribute('class', `label ${this._computeLabelClass(item)}`);
        }
        setTextContent(`${__f(item)!.label}`);
