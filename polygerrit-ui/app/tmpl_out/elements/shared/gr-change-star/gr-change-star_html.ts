...
import {GrChangeStar} from '../../../../elements/shared/gr-change-star/gr-change-star';
...
export class GrChangeStarCheck extends GrChangeStar
{
  templateCheck()
  {
    {
      const el: HTMLElementTagNameMap['button'] = null!;
      useVars(el);
      el.ariaLabel = this._computeAriaLabel(__f(this.change)!.starred);
      el.addEventListener('click', this.toggleStar.bind(this));
    }
    {
      const el: HTMLElementTagNameMap['iron-icon'] = null!;
      useVars(el);
      el.setAttribute('class', `${this._computeStarClass(__f(this.change)!.starred)}`);
      el.setAttribute('icon', `${this._computeStarIcon(__f(this.change)!.starred)}`);
    }
  }
}
