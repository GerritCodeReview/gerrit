...
import {GrLinkedChip} from '../../../../elements/shared/gr-linked-chip/gr-linked-chip';
...
export class GrLinkedChipCheck extends GrLinkedChip
{
  templateCheck()
  {
    {
      ...
      useVars(el);
      el.setAttribute('class', `container ${this._getBackgroundClass(this.transparentBackground)}`);
    }
    {
    ...
    {
      const el: HTMLElementTagNameMap['gr-limited-text'] = null!;
      useVars(el);
      el.limit = this.limit;
      el.text = this.text;
    }
    {
      ...
      el.link = true;
      el.setAttribute('hidden', `${!this.removable}`);
      el.setAttribute('class', `remove ${this._getBackgroundClass(this.transparentBackground)}`);
      el.addEventListener('click', this._handleRemoveTap.bind(this));
    }
    {
