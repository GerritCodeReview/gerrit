...
declare global {
  interface Window {
    ...
    /**
     * Gerrit can be available from a different host inside the corporate/internal
     * network (due to some internal policies). As a result all users within
     * internal network see different ULRs in browser which makes them harder
     * to share.
     *
     * This property allows to define a mapping from internal host to the public
     * available one (the map is generated by server). Gerrit uses the map to
     * determine public available url and to show it in the ui.
     */
