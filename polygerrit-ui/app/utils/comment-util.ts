...
export function isAiGenerated(comment?: Comment): boolean {
  if (comment?.is_ai) return true;
  if (!comment?.tag) return false;
  return (
    comment.tag.includes('ai_code_review') ||
    comment.tag.includes('ai_review') ||
    comment.tag.startsWith('autogenerated:ai')
  );
}
