 ...
 * */
export function getShadowOrDocumentSelection(shadowRoot: Document | ShadowRoot | null, contentEditableRange: Range | null ) {
    ...
    if (isSafari()) {
      ...
      if ((selection as any).getComposedRanges) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const composedRanges = (selection as any).getComposedRanges(shadowRoot)[0];
        const ranges = new Range();
        ranges.setStart(composedRanges.startContainer, composedRanges.startOffset);
