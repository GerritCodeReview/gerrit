...
function getPathFromNode(el: Node | Element) {
  ...
  if (
    !tagName ||
    ['GR-APP', '#document'].includes(tagName) ||
...
export function getEventPath(e: PolymerEvent) {
  ...
  let path = e.path as Array<Node | Element>;
  if (!path || !path.length) {
    path = [];
    let el = e.target as Node | ShadowRoot | null;
    while (el) {
  ...
  return path.reduce<string>((domPath: string, curEl: Node | Element) => {
