...
export class FitController implements ReactiveController {
  ...
  get __shouldPosition() {
    return (this.horizontalAlign || this.verticalAlign) && this._defaultPositionTarget;
  }
  ...
  _discoverInfo: function() {
    ...
    var target = (window as Window).getComputedStyle(this);
    var sizer = (window as Window).getComputedStyle(this);
  ...
  resetFit: function() {
    ...
    for (var property in info.sizerInlineStyle) {
      this.style[property] = info.sizerInlineStyle[property];
    }
  ...
  position: function() {
    ...
    var positionRect = this.__getNormalizedRect(this._defaultPositionTarget);
    var fitRect = this.__getNormalizedRect(window);
    ...
    if (this.expandSizingTargetForScrollbars) {
      unpositionedOffsetWidth = this.offsetWidth;
      unpositionedOffsetHeight = this.offsetHeight;
      unpositionedClientWidth = this.clientWidth;
      unpositionedClientHeight = this.clientHeight;
    }
    ...
    // Keep left/top within window respecting the margin.
    left = Math.max(
        fitRect.left + margin.left,
        Math.min(left, right - window.sizedBy.minWidth));
    top = Math.max(
        fitRect.top + margin.top,
        Math.min(top, bottom - window.sizedBy.minHeight));
    ...
    if (this.expandSizingTargetForScrollbars) {
      ...
      const positionedOffsetHeight = this.offsetHeight;
      const positionedClientHeight = this.clientHeight;
      ...
      if (sizingTargetScrollbarHeight > 0) {
        ...
        this.style.maxHeight = `${newMaxHeight}px`;
      ...
      const positionedOffsetWidth = this.offsetWidth;
      const positionedClientWidth = this.clientWidth;
      ...
      if (sizingTargetScrollbarWidth > 0) {
        ...
        this.style.maxWidth = `${newMaxWidth}px`;
        ...
        // scrollbar size.
        const offsetWidth = this.offsetWidth + maxWidthBugOffset;
  ...
  __sizeDimension: function(rect, positionedBy, start, end, extent) {
    ...
    var sizingOffset = this[offsetExtent] - this[offsetExtent];
    this.style['max' + extent] =
  ...
  center: function() {
    ...
    var positionedBy = window.positionedBy;
