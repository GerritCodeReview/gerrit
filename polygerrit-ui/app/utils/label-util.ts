...
function isDetailedLabelInfo(label: LabelInfo): label is DetailedLabelInfo {
...
export function getVotingRange(label?: LabelInfo): VotingRangeInfo {
  ...
  values.sort((a, b) => a - b);
  if (!values.length) throw new Error('Missing "values" in LabelInfo.');
