...
export function mergeLabelMaps(
  ...
  for (const key of Object.keys(a)) ans[key] = mergeLabelValues(a[key], b[key]);
  return ans;
}

export function mergeLabelValues(a?: string[], b?: string[]) {
  if (!a || !b) return [];
  const range1 = new Set(a.map(v => Number(v)));
  const range2 = new Set(b.map(v => Number(v)));
  // compute intersection of ranges for the label
  return Array.from(range1)
