...
export function getRevertCommitHash(change: ChangeInfo | ParsedChangeInfo) {
  const msg = change.messages?.find(m => m.tag === MessageTag.REVERT_TAG);
...
export function isRevertCreated(change: ChangeInfo | ParsedChangeInfo) {
  return change.messages?.some(m => m.tag === MessageTag.REVERT_TAG);
}
