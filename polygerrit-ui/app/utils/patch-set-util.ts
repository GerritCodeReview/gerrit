 ...
 *     doesn't exist.
 */
export function findEditParentRevision(revisions: Array<RevisionInfo>) {
 ...
 */
export function findEditParentPatchNum(revisions: Array<RevisionInfo>) {
 ...
 * 3, edit, 2, 1.
 *
 * @param {!Array<!RevisionWithSha>} revisions The revisions array
 * @return {!Array<!RevisionWithSha>} The sorted {revisions} array
 */
export function sortRevisions(revisions: Array<RevisionWithSha>) {
...
export function computeAllPatchSets(change: ChangeInfo) {
  ...
  let patchNums: Array<PatchSet> = [];
  if (change.revisions && Object.keys(change.revisions).length) {
    const revisions: Array<RevisionWithSha> = Object.keys(change.revisions).map(
      sha => {
        return {sha, ...change.revisions[sha]};
      }
    );
...
function _computeWipForPatchSets(
  ...
  for (let i = 0; i < change.messages.length; i++) {
    ...
    if (typeof msg.tag === 'string' && WIP_TAGS.includes(msg.tag)) {
      wip = true;
    } else if (typeof msg.tag === 'string' && READY_TAGS.includes(msg.tag)) {
      wip = false;
    }
...
export function computeLatestPatchNum(allPatchSets: Array<PatchSet>) {
...
/** @return {boolean} */
export function hasEditBasedOnCurrentPatchSet(allPatchSets: Array<PatchSet>) {
 ...
 */
export function fetchChangeUpdates(change: ChangeInfo, restAPI) {
 ...
 *
 * @param {string} rangeBase
 * @return {number}
 */
export function getParentIndex(rangeBase: string) {
