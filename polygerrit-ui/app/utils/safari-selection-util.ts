...
const contentEditableRange = new ContentEditableRange();
    ...
    () => {
      if (!processing) {
        ...
        if (active && active.id === 'diffTable') {
          ...
          contentEditableRange.removeRange();
        }
        processing = false;
      }
    },
  ...
  window.addEventListener(
    ...
    event => {
      if (processing) {
        ...
        const ranges = (event as any).getTargetRanges();
        ...
        contentEditableRange.setRange(newRange);
        ...
        if (active === null || active.id === 'diffTable') {
          event.preventDefault();
          event.stopImmediatePropagation();
        }
      }
...
export function getContentEditableRange() {
  return contentEditableRange.getRange()
}
