...
suite('string-util tests', () => {
  ...
  test('translateMeInQuery', () => {
    ...
    assert.equal(translateMeInQuery('uploader:me', dollarEmail), `uploader:${dollarEmail}`);
    assert.equal(
    ...
    assert.equal(translateMeInQuery('owner:me-awesome', email), 'owner:me-awesome');
    assert.equal(translateMeInQuery('owner:awesome-me', email), 'owner:awesome-me');
    assert.equal(translateMeInQuery('owner:me@google.com', email), 'owner:me@google.com');
    assert.equal(translateMeInQuery('owner:foo@me.com', email), 'owner:foo@me.com');

    // New operators
    assert.equal(translateMeInQuery('draftby:me', email), `draftby:${email}`);
    assert.equal(translateMeInQuery('reviewedby:me', email), `reviewedby:${email}`);
    assert.equal(translateMeInQuery('attention:me', email), `attention:${email}`);

    // Ignore me elsewhere (e.g. project name or message content)
    assert.equal(translateMeInQuery('project:awesome-me', email), 'project:awesome-me');
    assert.equal(translateMeInQuery('message:me', email), 'message:me');
    assert.equal(translateMeInQuery('me', email), 'me');
    assert.equal(translateMeInQuery('fixing me in comments', email), 'fixing me in comments');
    assert.equal(translateMeInQuery('something,me,somethingElse', email), 'something,me,somethingElse');
