 ...
 * contains the `loginUrl` in the `auth` section than that custom url
 * will be used, defaults to `/login` otherwise.
 * @param authConfig the auth section of gerrit configuration if defined
 * @param location the current page address
 */
export function loginUrl(
  authConfig: AuthInfo | undefined,
  location: Location
): string {
  const baseUrl = getBaseUrl();
  const customLoginUrl: string | undefined = authConfig?.login_url;
  const authType: AuthType | undefined = authConfig?.auth_type;
  if (
    customLoginUrl &&
    authType &&
