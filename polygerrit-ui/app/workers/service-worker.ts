...
// "Push" is used for testing purposes, since it is easy to trigger from dev tools.
...
class ServiceWorker {
  showNotification(change: ParsedChangeInfo) {
    // TODO(milutin): Replace with getReason from attention-set-util. You need AccountInfo.
    const body = 'Administrator replied on the change';
    // TODO(milutin): Implement event.action that focus on firstWindowClient and open change there.
