...
const runUnderBazel = process.env["RUNFILES_DIR"] ? true : false;
...
function resolvePlugin(pluginModuleName) {
  if(process.env['NODE_PRESERVE_SYMLINKS']) {
    // see the polygerrit-ui/karma_test.sh for details why
    // NODE_PRESERVE_SYMLINKS is used
    return require.resolve(
      `${process.cwd()}/external/ui_dev_npm/node_modules/${pluginModuleName}`);
  }
  return require.resolve(pluginModuleName);
}
...
module.exports = function(config) {
  ...
  config.set({
    ...
    basePath: '../',
    plugins: [
      // Do not use karma-* - it doesn't not work with NODE_PRESERVE_SYMLINKS
      resolvePlugin(`@open-wc/karma-esm`),
      resolvePlugin('karma-mocha'),
      resolvePlugin('karma-chrome-launcher'),
      resolvePlugin('karma-mocha-reporter'),
    ],
    // frameworks to use
