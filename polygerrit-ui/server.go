...
func handleSrcRequest(compiledSrcPath string, dirListingMux *http.ServeMux, writer http.ResponseWriter, originalRequest *http.Request) {
	...
	if isOriginalFileRequest {
		...
		moduleImportRegexp = regexp.MustCompile(`(?m)(import[^;'"]*|export[^;]*from[\s]*)['"]([^;]*?)(\.(m?)js)?['"];`)
		data = moduleImportRegexp.ReplaceAll(data, []byte(`$1'$2.${4}js';`))

		moduleImportRegexp = regexp.MustCompile(`(?m)(import[^;'"]*|export[^;]*from[\s]*)['"]([^/.][^;"']*)['"];`)
		...
		// 'lit-XYZ' imports and exports have to be resolved to 'lit-XYZ/lit-XYZ.js'.
		...
		// '@lit/reactive-element' imports and exports have to be resolved to '@lit/reactive-element/reactive-element.js'.
		moduleImportRegexp = regexp.MustCompile(`(?m)((import|export)[^;'"]*['"]/node_modules/)@lit/reactive-element.js['"];`)
		data = moduleImportRegexp.ReplaceAll(data, []byte(`${1}@lit/reactive-element/reactive-element.js';`))

		// 'lit' imports and exports have to be resolved to 'lit/index.js'.
