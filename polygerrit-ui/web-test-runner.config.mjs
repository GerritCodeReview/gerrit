...
const pathBazel = runUnderBazel ? 'polygerrit-ui/' : ''

const testFiles = getArgValue('--test-files') ?? `${pathBazel}app/**/*_test.{ts,js}`;
...
const config = {
  ...
  files: [
    testFiles,
    `!${pathBazel}**/node_modules/**/*`,
    ...(process.argv.includes("--run-screenshots")
      ? []
      : [`!${pathBazel}app/**/*_screenshot_test.{ts,js}`]),
  ],
  port: 9876,
  nodeResolve: {
  ...
  plugins: [
    esbuildPlugin({
      ts: true,
      target: "es2020",
      //path.join(process.cwd(), 'plugins/code-owners/web/tsconfig.json')
      tsconfig: tsConfig,
    }),
