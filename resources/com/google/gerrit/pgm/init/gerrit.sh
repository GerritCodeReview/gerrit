...
usage() {
    me=`basename "$0"`
    echo >&2 "Usage: $me {start|stop|restart|check|status|run|supervise|threads|gc-histogram} [-d site] [--debug [--debug-port|--debug-address ...] [--suspend]] [--count=n]"
    exit 1
}
...
GERRIT_GC_HISTOGRAM="$GERRIT_LOGS/gc-histogram"
...
case "$ACTION" in
  ...
  gc-histogram)
    if running "$GERRIT_PID" ; then
      mkdir -p -- "$GERRIT_GC_HISTOGRAM"
      HISTOGRAM="$GERRIT_GC_HISTOGRAM/histogram-`ztime`"
      gc_histogram "$GERRIT_PID" > "$HISTOGRAM" || exit 1
      echo "$HISTOGRAM"
      exit 0
    else
      echo "Gerrit not running?"
