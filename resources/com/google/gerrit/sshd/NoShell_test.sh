...
# after it ends. It also verifies that the command doesn't time out.
...
messageReceived=$({ timeout --foreground 30s ssh -p "$port" "$user"@"$host"; } 2>&1)
sshErrorMessage="
There was an error in the ssh connection to the site.
...
timeoutError="The test fails. The connection has timed out."
connectionTimeoutSuccess="The connection didn't time out.
Therefore, it passed the first verification."

if [[ "$messageReceived" != *"closed"* ]]; then
  echo "$timeoutError"
  exit 1
fi
echo "$connectionTimeoutSuccess"
...
if [ "$fdDifference" -le 0 ]; then
  echo "The test is successful. The count of file descriptors didn't increase."
else
  failureMessage="The test fails. The count of file descriptors increased by ${fdDifference}."
  failureNote="-Note that something else from the application might have caused this increase."
  printf "%s\n" "${failureMessage}" "${failureNote}" \
