...
public class PatchTable extends Composite {
   ...
   * if we are at the first or last patch.
   */
  public DirectScreenLink getPreviousPatchLink(int index, Patch.PatchType patchType) {
   ...
   * file.
   */
  public DirectScreenLink getNextPatchLink(int index, Patch.PatchType patchType) {
   ...
   */
  private PatchLink createLink(int index, Patch.PatchType patchType, 
  ...
  private ChangeLink createParentChangeLink() {
    PatchSet.Id parentChange = patchList.get(0).getKey().getParentKey();
    return parentChange != null
        ? new ChangeLink("Back to the patch", psid.getParentKey())
        : null;
  }
