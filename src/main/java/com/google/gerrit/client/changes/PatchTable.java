...
public class PatchTable extends Composite {
  ...
  private PatchLink createLink(int index, PatchScreen.Type patchType,
    ...
    int s = fileName.indexOf(File.pathSeparator);
    if (s >= 0) {
      fileName.substring(s + 1);
    }
    ...
    if (patchType == PatchScreen.Type.SIDE_BY_SIDE
      ...
      result = new PatchLink.SideBySide(linkText, thisKey, index, this);
    } else {
      result = new PatchLink.Unified(linkText, thisKey, index, this);
    }
