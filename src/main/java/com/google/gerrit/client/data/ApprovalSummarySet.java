...
public class ApprovalSummarySet {
  ...
  public ApprovalSummarySet(final Map<Change.Id, ApprovalSummary> map) {
    summaries = new HashMap<Change.Id, ApprovalSummary>();
    for (Map.Entry<Change.Id, ApprovalSummary> pair : map.entrySet()) {
      summaries.put(pair.getKey(), pair.getValue());
    }
  }
