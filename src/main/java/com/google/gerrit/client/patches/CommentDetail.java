...
public class CommentDetail {
  ...
  private static List<PatchLineComment> orderComments(List<PatchLineComment> comments) {
    ...
    // their correct parent but not necessarily in the correct chronological order
    ...
    for (PatchLineComment c : comments) {
      ...
      l.add(c);
      if (parentUuid == null) firstComment = c;
    }
    ...
    List<PatchLineComment> currentList = new ArrayList<PatchLineComment>();
    currentList.add(firstComment);

    do {
      List<PatchLineComment> nextList = null;
      for (PatchLineComment current : currentList) {
        List<PatchLineComment> children = parentMap.get(current.getKey().get());
        if (children != null) {
            result.addAll(children);
            if (nextList == null) nextList = new ArrayList<PatchLineComment>();
            nextList.addAll(children);
        }
      }
