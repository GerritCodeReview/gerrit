...
public abstract class PatchScreen extends Screen {
  ...
  private void onResult() {
    if (script != null && comments != null) {
      ...
      boolean hasDifferences = !script.getEdits().isEmpty() && !script.getPatchHeader().isEmpty();
      // True if this change is a mode change or a pure rename/copy
      boolean modeChange = script.getEdits().isEmpty() && !script.getPatchHeader().isEmpty();

      if (contentTable instanceof SideBySideTable && modeChange) {
