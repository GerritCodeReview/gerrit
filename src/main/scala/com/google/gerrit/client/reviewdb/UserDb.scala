...
import org.spearce.jgit.lib.{Constants, RepositoryCache, Config}
...
final class UserDb @Inject()(@GerritServerConfig private val cfg: Config) {
  ...
  private val gimdDb: Database = {
    val section = "auth"
    val entryName = "accountRepository"
    val repoLocation = cfg.getString(section, null, entryName)
    if (repoLocation == null)
      error("The %1s.%2s option is not configured in your Gerrit config.".format(section, entryName))
    val repo = RepositoryCache.FileKey.lenient(new java.io.File(repoLocation)).open(true)
    var ref = cfg.getString("auth", null, "accountBranch");
    if (ref == null)
      ref = "refs/heads/accounts";
    if (!ref.startsWith(Constants.R_HEADS))
      ref = Constants.R_HEADS + ref;
    new JGitDatabase(JGitBranch(repo, ref))
  }
  ...
  private object AccountType extends UserType[Account] {
    ...
    def toUserObject(m: Message) = {
      ...
      val account = Account.newInstance(oldId)
  ...
  private object SshUserNameFileType extends FileType[SshUserName] {
    ...
    def name(m: Message) = m.one("sshUserName").stringField.value
  }
  ...
  private object ExternalIdType extends UserType[AccountExternalId] {
    ...
    def toUserObject(m: Message) = {
      ...
      val externalId = AccountExternalId.newInstance(rawOldAccountId, rawExternalId)
      externalId.emailAddress = m.oneOption("emailAddress") match {
        case Some(x) => x.stringField.value
        case None => null
      }
      externalId.accountId = m.one("accountId").stringField.value
  ...
  def byOldId(rawId: Int)(implicit s: Snapshot): Account = {
    val account = queryAccount(_.getRawOldAccountId == rawId).take(2).toList match {
      case Nil => null
      case x :: Nil => x
      case _ =>
  ...
  def byId(accountId: String)(implicit s: Snapshot) = {
    val account = queryAccount(_.accountId == accountId).take(2).toList match {
      case Nil => null
      case x :: Nil => x
      case _ =>
  ...
  def byPreferredEmail(email: String)(implicit s: Snapshot): ResultSet[Account] =
    toResultSet(queryAccount(_.preferredEmail == email).take(2).map(fillSshUsername(_)))

  def byFullName(name: String)(implicit s: Snapshot): ResultSet[Account] =
    toResultSet(queryAccount(_.fullName == name).take(2).map(fillSshUsername(_)))
  ...
  def suggestByFullName(nameA: String, nameB: String, limit: Int)(implicit s: Snapshot):
    ResultSet[Account] = {
    val accounts = queryAccount(x => x.fullName >= nameA && x.fullName <= nameB).take(limit).toList
  ...
  def suggestByPreferredEmail(emailA: String, emailB: String, limit: Int)(implicit s: Snapshot):
    ...
    val accounts = queryAccount(p).take(limit).toList
  ...
  def suggestBySshUserName(nameA: String, nameB: String, limit: Int)(implicit s: Snapshot):
    ResultSet[Account] = {
    val accounts = queryAccount(x => x.sshUserName >= nameA && x.sshUserName <= nameB).
            take(limit).toList
  ...
  def byExternalId(externalId: String)(implicit s: Snapshot): AccountExternalId =
    queryExternalId(_.getRawExternalId == externalId).take(2).toList match {
  ...
  def byAccount(rawId: Int)(implicit s: Snapshot): ResultSet[AccountExternalId] =
    toResultSet(queryExternalId(_.getRawOldAccountId == rawId))

  def byEmail(email: String)(implicit s: Snapshot): ResultSet[AccountExternalId] =
    toResultSet(queryExternalId(_.emailAddress == email))

  def suggestByEmail(emailA: String, emailB: String, limit: Int)(implicit s: Snapshot):
    ResultSet[AccountExternalId] = {
    val accounts = queryExternalId(x => x.emailAddress >= emailA && x.emailAddress <= emailB).
          take(limit).toList
  ...
  def updateAccount(a: Account, m: DatabaseModification, s: Snapshot) = {
    val p = (x: Account) => x.getRawOldAccountId == a.getRawOldAccountId
    s.query(AccountFileType, p).toList match {
      case (handle, _) :: Nil => {
        m.modify(handle, a)
      }
      case xs => throw new OrmException(("Query returned more than one Account with id '%1s'. " +
              "Returned result is %2s").format(a.getRawOldAccountId, xs))
    }
  }
  ...
  private def fillSshUsername(a: Account)(implicit snapshot: Snapshot): Account = {
    findSshUserName(_.accountId == a.accountId).
  ...
  private def queryAccount(p: Account => Boolean)(implicit s: Snapshot): Iterator[Account] =
    s.query(AccountFileType, p).map(_._2)

  private def queryExternalId(p: AccountExternalId => Boolean)(implicit s: Snapshot):
