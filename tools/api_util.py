...
if not exists(gendir):
  print("gendir dosn't exists: %s" % gendir, file=stderr)
  exit(1)
...
try:
  check_output(cmd[action] +
               ...
               cmd['common'] +
               cmd['jar'] +
               ['-Dfile=%s/plugin-api-%s.jar' % (gendir, version)])
  check_output(cmd[action] +
               cmd['plugin'] +
               cmd['common'] +
               cmd['src'] +
               ['-Dfile=%s/plugin-api-%s-src.jar' % (gendir, version)])
  check_output(cmd[action] +
               cmd['extension'] +
               cmd['common'] +
               cmd['jar'] +
               ['-Dfile=%s/extension-api-%s.jar' % (gendir, version)])
  check_output(cmd[action] +
               cmd['extension'] +
               cmd['common'] +
               cmd['src'] +
               ['-Dfile=%s/extension-api-%s-src.jar' % (gendir, version)])
