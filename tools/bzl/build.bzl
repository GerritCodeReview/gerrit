...
def _war_impl(ctx):
  ...
  cmd += ['mkdir -p %s/WEB-INF/pgm-lib' % build_output]

  # Add lib
  transitive_lib_deps = set()
  for l in ctx.attr.libs:
    print('transitive_deps: %s' % l.java.transitive_deps)
    print('transitive_runtime_deps: %s' % l.java.transitive_runtime_deps)
  ...
  # Add zip war
  cmd += _make_war(build_output, war)
