...
def _bundle_impl(ctx):
    ...
    hermetic_npm_binary = " ".join([
        "$p/" + ctx.executable._bundler_archive.path,
        "--inline-scripts",
        "--inline-css",
        "--strip-comments",
        "--out-file",
    ...
    ctx.actions.run_shell(
        ...
        outputs = [bundled],
        command = cmd,
        tools = [ctx.executable._bundler_archive],
        **node_tweaks
    )

    if ctx.attr.split:
        hermetic_npm_command = " ".join([
            ctx.executable._crisper_archive.path,
            "--always-write-script",
            "--source",
            bundled.path,
            "--html",
        ...
        ctx.actions.run_shell(
            ...
            tools = [ctx.executable._crisper_archive],
            **node_tweaks
        )
...
_bundle_rule = rule(
    ...
    attrs = {
        ...
        "_bundler_archive": attr.label(
            default = Label("//tools/node_tools:polymer-bundler"),
            executable = True,
            cfg = "host",
            allow_files = True,
        ),
        "_crisper_archive": attr.label(
            default = Label("//tools/node_tools:crisper"),
            executable = True,
            cfg = "host",
            allow_files = True,
        ),
