...
def _bower_component_impl(ctx):
  ...
  transitive_licenses = set(ctx.files.license)
  for d in ctx.attr.deps:
...
def _js_component(ctx):
  ...
  if name.endswith(".zip"):
    name = name[:-4]
  dest = "%s/bower_components/%s" % (dir, name)
  cmd = "mkdir -p %s && cp %s %s/ && cd %s && zip -qr ../%s bower_components/"% (
    dest, ' '.join([s.path for s in ctx.files.srcs]), dest, dir, ctx.outputs.zip.basename)
...
_bower_component = rule(
  ...
  attrs=_common_attrs + {
    ...
    "license": attr.label(allow_files=True),
...
def _bower_component_bundle_impl(ctx):
  ...
  ctx.action(
    inputs=list(zips),
    outputs=[out_zip],
    command="p=$PWD ; mkdir %s.dir ; cd %s.dir ; for z in %s; do unzip -q $p/$z ; done; zip -qr $p/%s *" % (
  ...
  ctx.action(
    ...
    # The empty string dummy is fugly.
