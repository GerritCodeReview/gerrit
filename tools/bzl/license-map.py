...
#   Each item is a string, but not nessecary a real name of a file.
...
def load_xmls(xmls):
    ...
    Args:
         xmls: list of string; each string is a filename
    Returns:
...
def get_licensed_files(json_licensed_file_dict):
    ...
    if kind == "OnlySpecificFiles" or kind == "AllFilesExceptSpecific":
        return LicensedFiles(kind=kind, files=json_licensed_file_dict["files"])
...
def load_jsons(jsons):
    ...
    Args:
         jsons: list of string; each string is a filename
    Returns:
        list of LicenseMapItem
    """
    if not jsons:
