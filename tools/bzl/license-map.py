...
PackageInfo = namedtuple("PackageInfo", ["name", "licensed_files"])
...
def load_xmls(xmls):
  ...
  for n in sorted(graph.keys()):
    ...
    result.append(LicenseMapItem(
        ...
        packages = [PackageInfo(name = name, licensed_files = LicensedFiles(kind = "All", files = [])) for name in packages_names]
        )
    )
  ...
  for json_map in jsons:
    ...
    for licenseId, license in licenses_list.items():
      ...
      for p in license["packages"]:
        package = PackageInfo(name = p["name"], licensed_files = get_licensed_files(p["licensedFiles"]))
...
for data in xml_data + json_map_data:
  ...
  for p in data.packages:
    ...
    if p.licensed_files.kind == "OnlySpecificFiles":
      files = " (only %s)" % (", ".join(p.licensed_files.files))
    elif p.licensed_files.kind == "AllFilesExceptSpecific":
      files = " (except %s)" % (", ".join(p.licensed_files.files))
