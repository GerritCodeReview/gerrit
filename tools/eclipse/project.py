...
while not os.path.exists(path.join(ROOT, 'WORKSPACE')):
...
def retrieve_ext_location():
  return subprocess.check_output([args.bazel_exe, 'info', 'output_base']).strip()

def gen_bazel_path():
  bazel = subprocess.check_output(['which', args.bazel_exe]).strip().decode('UTF-8')
...
def gen_factorypath(ext):
  doc = xml.dom.minidom.getDOMImplementation().createDocument(None, 'factorypath', None)
...
try:
  ...
  try:
    subprocess.check_call([args.bazel_exe, 'build', MAIN, GWT, '//gerrit-patch-jgit:libEdit-src.jar'])
  except subprocess.CalledProcessError:
    exit(1)
