...
def build_bower_json():
  ...
  with open('bazel-out/local-fastbuild/bin/polygerrit-ui/components-versions.json','r') as f:
    j = json.load(f)
    if "dummy" in j:
      del j["dummy"]  # TODO(hanwen): fugly.
    bower_json['dependencies'] = j
...
def interpret_bower_json(d):
  ...
  for f in sorted(out.split('\n')):
    ...
    pkg["bazel-sha1"] = bowerutil.hash_bower_component(hashlib.sha1(), os.path.dirname(f)).hexdigest()
