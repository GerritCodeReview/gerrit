...
class Rule(object):
  def __init__(self, bower_json_path):
    ...
    self.sha1 = hash_dir(os.path.dirname(bower_json_path))
...
# TODO(dborowitz): Share with download_bower.
def hash_file(h, p):
  with open(p, 'rb') as f:
    while True:
      b = f.read(8192)
...
def build_bower_json(target, buck_out):
  ...
  deps = subprocess.check_output([
      'buck', 'query', '-v', '0',
      "filter('__download_bower', deps(%s))" % target])
  deps = deps.replace('__download_bower', '__bower_version').split()
  subprocess.check_call(['buck', 'build'] + deps)
