...
def locations():
  d = Popen('buck audit dependencies api'.split(), stdin=None, stdout=PIPE, stderr=PIPE)
  t = Popen('xargs buck targets --show_output'.split(), stdin=d.stdout, stdout=PIPE, stderr=PIPE)
  out = t.communicate()[0]
  d.wait()
  targets = []
  outs = []
...
if not args.dryrun:
  check_output(['buck', 'build', 'api'])
target = check_output(['buck', 'targets', '--json', 'api_%s' % args.action])

s = loads(target)[0][u'cmd']
