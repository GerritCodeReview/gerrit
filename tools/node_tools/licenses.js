...
function getSortedByPathExceptions(licenseInfo) {
  ...
  const result = [];
  for(const exception of licenseInfo.exceptions) {
...
function getLicensesSortedByPath(rootFolder, relativePathToModule, licenseInfo) {
  const sortedByPathLicenses = getSortedByPathExceptions(licenseInfo);
...
if(process.argv.length < 3 || (process.argv.length >= 4 && process.argv[3] !== "--remove-invalid")) {
  const usage = `Usage:
  1)  Print licenses for each file in node_modules folder:
    node licenses.js root_folder
  2) Remove files in node_modules folder with invalid licenses:
    node licenses.js root_folder --remove-invalid [list of allowed licenses]
  `;
  exitWithError(`usage`);
}
...
if(process.argv.length < 4) {
  licensesPerFile.forEach((filePath, licenseInfo) => {
    console.log(filePath, licenseInfo.licenseType);
  });
} else if(process.argv[3] === "--remove-invalid") {
  const validLicenses = process.argv.slice(4)
  licensesPerFile.forEach((filePath, licenseInfo) => {
    console.log(filePath, licenseInfo.licenseType);
  });
}
