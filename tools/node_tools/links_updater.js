...
if(process.argv.length < 6) {
  console.info("Usage:\n\tnode links_updater.js rootDir outputRootDir dirToUpdate redirectFile\noutputRootDir can be the same as rootDir for inplace update");
  process.exit(1);
}

const rootDir = path.resolve(process.cwd(), process.argv[2]);
const outputRootDir = path.resolve(process.cwd(), process.argv[3]);
const dirToUpdate = path.relative(rootDir, path.resolve(rootDir, process.argv[4]));
...
if(!fs.existsSync(path.join(rootDir, dirToUpdate))) {
  console.error(`Input directory doesn't exists: ${dirToUpdate}`);
  process.exit(1);
}

const isHtmlFilePredicate = (dirEnt, dirEntPath) => {
  return (dirEnt.isFile() || dirEnt.isSymbolicLink()) && path.extname(dirEntPath) === ".html";
};
...
const updateLinksAction = (dirEnt, dirEntPath) => {
  ...
  FileUtils.saveFile(outputRootDir, dirEntPath, newContent);
};
