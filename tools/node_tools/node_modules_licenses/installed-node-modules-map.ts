...
export class InsalledPackagesBuilder {
  ...
  public addFile(file: string) {
    let pack;
    if (file.search("@file+") != -1) {
      pack = this.findLocalPackageForFile(file);
    } else {
      pack = this.findPackageForFile(file)!;
    }
  ...
  private findPackageForFile(filePath: FilePath): InstalledPackage {
    ...
    while(!currentDir.endsWith("node_modules")) {
  ...
  private findLocalPackageForFile(filePath: FilePath): InstalledPackage {
    ...
    let pack = currentDir.split('/', 1)[0];
    if(this.rootPathToPackageMap.has(pack)) {
      return this.rootPathToPackageMap.get(pack)!;
    }
    let installedPack: InstalledPackage = {
      name: pack,
      version: "Snapshot",
      rootPath: filePath.substring(0, filePath.lastIndexOf(pack) + pack.length),
      files: [filePath]
    };
  ...
  private getPackagePathRelativeToNodeModules(path: string): string {
    const node_modules_path_part = "/node_modules/";
    return path.substring(path.lastIndexOf(node_modules_path_part) + node_modules_path_part.length)
  }
}
