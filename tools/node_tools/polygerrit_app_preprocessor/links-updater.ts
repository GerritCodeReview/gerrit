...
import {FileUtils, RedirectsResolver, fail, Redirect} from "./utils";
import {Node} from 'dom5';
import {readMultilineParamFile} from "../utils/command-line";

interface JSONRedirects {
  redirects: Redirect[]
}
...
class HtmlFileUpdater {
  ...
  public updateFile(srcFile: string, targetFile: string) {
    ...
    FileUtils.saveFile(path.dirname(targetFile), path.basename(targetFile), newContent);
  }

  private getResolvedPath(parentHtml: string, href: string) {
    const originalPath = '/' + FileUtils.getPathRelativeToRoot(parentHtml, href);
    ...
    return FileUtils.getPathRelativeToRoot(parentHtml, resolvedInfo.target);
  }
