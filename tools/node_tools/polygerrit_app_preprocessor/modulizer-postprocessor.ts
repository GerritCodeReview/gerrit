...
const originalFiles = readMultilineParamFile(process.argv[4]);
...
class TsFileUpdater {
  ...
  private replaceImportPath(file: SourceFile, oldPath: string, newPath: string): ImportDeclaration | undefined {
    if(oldPath.startsWith('.') || oldPath.startsWith('/')) {
      fail(`Only module names supported`);
    }
  ...
  private replacePageJsImport(file: SourceFile): ImportDeclaration | undefined {
    const importDecl = this.replaceImportPath(file, 'page/page.js', 'page/page.mjs');
    if(!importDecl) {
      return undefined;
    }
    importDecl.setDefaultImport("page");
    file.insertStatements(importDecl.getChildIndex() + 1, "self.page = page;");
  }
  ...
  public updateFile(relativePath: string) {
    ...
    this.replaceImportPath(updatedSourceFile, 'moment/moment.js', 'moment/src/moment.js');
    this.replaceImportPath(updatedSourceFile, 'whatwg-fetch/dist/fetch.umd.js', 'whatwg-fetch/fetch.js');
    this.replacePageJsImport(updatedSourceFile);
...
const modulizedFilesSet = new Set<string>(modulizedFiles);
for(const originalFile of originalFiles) {
  if(!modulizedFilesSet.has(originalFile)) {
    console.log(`Missed modulized file for: ${originalFile}`);
  }
}