...
//const modulizedFiles = ["./behaviors/keyboard-shortcut-behavior/keyboard-shortcut-behavior.js"];
//const files = ["./elements/plugins/gr-external-style/gr-external-style.js"];
//const files = ["./elements/core/gr-router/gr-router.js"];
...
class TsFileUpdater {
  ...
  private fixImportMaxLen(file: SourceFile) {
    const importDeclarations = file.getImportDeclarations();
    for(const imp of importDeclarations) {
      const fullText = imp.getText();
      if(fullText.length > 80) {
        imp.getModuleSpecifier().getPreviousSibling()!.appendWhitespace(writer => writer.newLine());
      }
    }
  }

  private getFileLicenseComment(file: SourceFile): {statement: Statement, commentRange: CommentRange} | undefined {
    const statements = file.getStatementsWithComments();
    if(statements.length === 0) {
      return;
    }
    const commentRanges = statements[0].getLeadingCommentRanges();
    if(commentRanges.length === 0) {
      return;
    }

    const licenseComment = commentRanges[0].getText();
    if(licenseComment.indexOf('@license') < 0) {
      return;
    }
    return {
      statement: statements[0],
      commentRange: commentRanges[0]
    };
  }

  private fixJsLicenseComment(file: SourceFile) {
    const licenseComment = this.getFileLicenseComment(file);
    if(!licenseComment) {
      fail('The file must have a @license comment at the start');
    }
    const commentLines = licenseComment.commentRange.getText().split('\n');
    ...
    const newCommentText = commentLines.join('\n');
    file.replaceText([licenseComment.commentRange.getPos(), licenseComment.commentRange.getEnd()], newCommentText);
  }
  ...
  public updateFile(relativePath: string): Set<string> {
    ...
    this.fixImportMaxLen(updatedSourceFile);
