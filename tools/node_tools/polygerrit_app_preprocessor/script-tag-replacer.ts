...
class HtmlFileUpdater {
  ...
  public async updateFile(root: string, srcFile: string) {
    ...
    if(!srcFile.endsWith("_test.html")) {
      await this.checkScriptTagsLocations(srcFilePath, srcFile);
    }
  ...
  private async checkScriptTagsLocations(srcFilePath: string, srcFile: string): Promise<void> {
    const readStream = fs.createReadStream(srcFilePath, {encoding: "utf-8"});
    const tmpFileName = srcFilePath + "_checks.tmp";
    const writeStream = fs.createWriteStream(tmpFileName, {encoding: "utf-8"});
    const rewriter = new RewritingStream();

    let insideDomModule = false;
    let lastText: string = "";
    let insideInlineScriptTag = false;
    rewriter.on("startTag", (tag: any) => {
      if(tag.tagName === "dom-module") {
        if(insideDomModule) {
          this.reportError(srcFile, 'Nested dom-module element');
        }
        insideDomModule = true;
      } else if(tag.tagName == "script") {
        if(insideDomModule) {
          if(HtmlFileUpdater.Predicates.isScriptWithSrcTag(tag)) {
            const src = dom5.getAttribute(tag, "src")!;
            ...
            lastText = "";
            insideInlineScriptTag = true;
          }
        }
      }
    });
    rewriter.on("text", (textToken: any) => {
      lastText = textToken.text;
    });
    rewriter.on("endTag", (tag: any) => {
      if(tag.tagName === "dom-module") {
        if(!insideDomModule) {
          this.reportError(srcFile, 'dom-module closed without starting tag');
        }
        insideDomModule = false;
      } else if(tag.tagName === "script" && insideInlineScriptTag) {
        insideInlineScriptTag = false;
        if(!lastText.trimLeft().startsWith("Polymer(")) {
