...
import {LegacyPolymerComponent, PolymerComponentParser, PolymerComponentType} from "./polymerComponentParser";
...
async function updateLegacyComponent(component: LegacyPolymerComponent) {
  ...
  const transformResult = tsUtils.replaceNode(component.parsedFile, polymerFuncCallStatement, [classDeclaration, componentRegistration]);
  try {


    //const sourceFile = ts.updateSourceFileNode(component.parsedFile, [classDeclaration]);
    // ts.updateSourceFileNode(sourceFile, [classDeclaration]);
    // builder.appendString(printer.printFile(newSource));
    // builder.appendString(`customElements.define('${component.componentSettings.is}', ${classDeclaration.name!.text});`);
    // builder.appendString(fullText.substring(component.polymerFuncCallExpr.getEnd(), component.parsedFile.getEnd()));
    //builder.writeToFile(component.jsFile + ".new");
    fs.writeFileSync(component.jsFile + ".new", printer.printFile(transformResult.transformed[0]));
  }
  finally {
...
function generatePolymerV2ClassFromLegacyComponent(component: LegacyPolymerComponent): { classDeclaration: ts.ClassDeclaration, componentRegistration: ts.CallExpression} {
  const legacySettings = component.componentSettings;
  if(!legacySettings.is) {
    throw new Error("Component doesn't have 'is' property");
  }
  const className = generateClassNameFromTagName(legacySettings.is);
  const members: ClassElement[] = [];
  if(legacySettings.properties) {
    const returnStatement = ts.createReturn(legacySettings.properties);
  ...
  return {
    classDeclaration: ts.createClassDeclaration(undefined, undefined, className, undefined, undefined, members),
    componentRegistration: ts.createCall(ts.createPropertyAccess(ts.createIdentifier("customElements"), "define"), undefined, [ts.createStringLiteral(legacySettings.is), ts.createIdentifier(className)]),
  };
}
