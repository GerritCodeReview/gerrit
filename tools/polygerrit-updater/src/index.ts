  ...
  for(const name of LegacyLifecycleMethodsArray) {
    ...
    if(name === "created" && reservedDeclarations.listeners) {
      for(const listener of reservedDeclarations.listeners.properties) {
        ...
        const methodCall = ts.createCall(ts.createPropertyAccess(ts.createThis(), "addEventListener"), [], [eventNameLiteral, eventHandlerAccess]);
    ...
    if(name === "ready" && reservedDeclarations.hostAttributes) {
      for(const listener of reservedDeclarations.hostAttributes.properties) {
        ...
        const attributeNameLiteral= tsUtils.assertNodeKind(propertyAssignment.name, ts.SyntaxKind.StringLiteral) as ts.StringLiteral;
        const attributeValueLiteral = tsUtils.assertNodeKind(propertyAssignment.initializer, ts.SyntaxKind.StringLiteral) as ts.StringLiteral;
