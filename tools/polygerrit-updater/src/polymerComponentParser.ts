...
export class PolymerComponentParser {
    ...
    for(const property of info.properties) {
      ...
      switch(name.kind) {
        ...
        case ts.SyntaxKind.ComputedPropertyName:
          continue;
        default:
          utils.unexpetedValue(ts.SyntaxKind[name.kind]);
      }
    }
  ...
  private getEventsComments(polymerComponentSource: string): string[] {
    return tsUtils.collectAllComments(polymerComponentSource).filter(c => c.indexOf('@event') > 0 || c.indexOf('gr-comment-thread exposes the following attributes that allow a') > 0);
  }
