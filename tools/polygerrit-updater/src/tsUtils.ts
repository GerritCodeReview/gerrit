...
export function assertNodeKind<T extends U, U extends ts.Node>(node: U, expectedKind: ts.SyntaxKind): T {
  if (node.kind !== expectedKind) {
    throw new Error(`Invlid node kind. Expected: ${expectedKind}, actual: ${node.kind}`);
  }
  return node as T;
}
...
export function replaceStatement(file: ts.SourceFile, statement: ts.Statement, newStatements: ts.Statement[]): ts.SourceFile {
