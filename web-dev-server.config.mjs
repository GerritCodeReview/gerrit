...
import cors from "@koa/cors";
  ...
  function LoadESMModule(prefix, importPath) {
  return async (context, next) => {
    const isMjs = context.url.includes(`${prefix}.mjs`);
    if (isMjs) {
      // Load the .ts file of the entrypoint instead of .js to trigger esbuild
      // which will convert every .ts file to .js on request.
      context.url = context.url.replace(`${prefix}.mjs`, `${prefix}.ts`);
      console.log(prefix, importPath);
    }
    ...
    if (!isMjs && context.url.includes(`${prefix}.js`)) {
      context.body = `import('${importPath}')`;
    };
  }
}
...
export default {
  ...
  middleware: [
    ...
    cors({ origin: "*" }),
    LoadESMModule('monaco-editor', 'workers/monaco-editor.mjs'),
    LoadESMModule('gr-app', './gr-app.mjs'),
  ],
};
G
